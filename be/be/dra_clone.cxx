...
#include "dra_export.h"         // Exported DRA interface
#include "dra_internal.h"       // Internal DRA interface
#include "fb_whirl.h"           // for feedback related functions
               ...
               WN* pu)
{
  ...
  if (clone_requests != NULL) {
    DRA_Clone_Instantiate(pu_info, clone_requests, dra_table);
  }
               ...
               char *arg_sig)
{
  ...
  //
  if (Cur_PU_Feedback) {
    Is_True(0, ("Need to fix up feedback in DRA_Add_Clone\n"));
  }
#if TODO
  ...
  FOREACH_SYMBOL (GLOBAL_SYMTAB, st, i) {
    ...
    if (st_sclass == SCLASS_COMMON &&
      ...
      if (dra != NULL) {              // reshaped array
        ...
        if (non_dra_end - non_dra_start > 0) {
          bufptr += sprintf(bufptr, " %" LLD_FMT "", non_dra_end - non_dra_start);
        }
        non_dra_start = common_offset + ST_ofst(st) + TY_size(ty);
        non_dra_end = non_dra_start;

        bufptr += 
          sprintf(bufptr, " DRA_%" LLD_FMT "_%d", TY_size(TY_AR_etype(ty)), ndims);
          ...
          switch (dra->Distr_Type(dim)) {
            ...
            case DISTRIBUTE_CYCLIC_CONST:
              *bufptr++ = DRA_CYCLIC_CODE; 
              bufptr += sprintf(bufptr, "%" LLD_FMT "", dra->Chunk_Const_Val(dim));
              break;
    ...
    if (ty && TY_kind(ty) == KIND_ARRAY) {
      ...
      if (dra != NULL) {
        ...
        *bufptr++ = '\n';
        ...
        // not reshaped
        //
        bufptr += sprintf (bufptr, " %" LLD_FMT "", TY_size(ty));
        *bufptr++ = '\n';
      }
      write(DRA_file_desc, (void*)buf, bufptr-buf);
    }
