 ...
 *
 * Module: driver.cxx
 * $Source: /isms/cmplrs.src/osprey1.0/be/be/RCS/driver.cxx,v $
 *
 * Revision history:
...
/* TENSILICA: We need DLL_SHARED to be an import for be.exe 
	      and DLL_SHARED to be an export for be.dll on NT.
              but the sgi makefiles don't have an easy way to define
              something two different ways in the same Makefile.
...
#include "cg_prof.h"          	    /* basic block profiling */
#include "mem_ctr.h"
...
extern DLL_SHARED WN_MAP Prompf_Id_Map; /* Maps WN constructs to unique identifiers */

/* Keep track of which optional components are loaded, where we need
 * to do so.
 */
static BOOL   wopt_loaded = FALSE;
extern DLL_SHARED BOOL   Prompf_anl_loaded; /* Defined in cleanup.c */
extern DLL_SHARED BOOL   Purple_loaded;     /* Defined in cleanup.c */
extern DLL_SHARED BOOL   Whirl2f_loaded;    /* Defined in cleanup.c */
extern DLL_SHARED BOOL   Whirl2c_loaded;    /* Defined in cleanup.c */

extern DLL_SHARED void *Current_Dep_Graph;
FILE *DFile = stderr;
#ifndef _WIN32
#define CG_DLL "cg.so"
#define WOPT_DLL "wopt.so"
#define IPL_DLL "ipl.so"
#define LNO_DLL "lno.so"
#define PURPLE_DLL "purple.dll"
#define PROMPF_DLL "prompf_anl.so"
#define WHIRL2C_DLL "whirl2c.so"
#define WHIRL2F_DLL "whirl2f.so"
#else
#define CG_DLL "cg.dll"
#define WOPT_DLL "wopt.dll"
#define IPL_DLL "ipl.dll"
#define LNO_DLL "lno.dll"
#define PURPLE_DLL "purple.dll"
#define PROMPF_DLL "prompf_anl.dll"
#define WHIRL2C_DLL "whirl2c.dll"
#define WHIRL2F_DLL "whirl2f.dll"
#endif
...
#ifdef _WIN32
    char *driver_path = strdup(argv[0]);
    char *p = driver_path + strlen(driver_path) - 1; 
    while (p != driver_path && !is_dir_separator(*p)) p--; 
    *p = '\0';
#else
    ...
    if (Run_cg) {
      ...
      load_so (CG_DLL, CG_Path ? CG_Path : driver_path, Show_Progress);
    ...
    if (Run_wopt || Run_preopt || Run_lno || Run_autopar) {
      ...
      load_so (WOPT_DLL, WOPT_Path ?  WOPT_Path : driver_path, Show_Progress);
      wopt_main (phase_argc, phase_argv, argc, argv);
      wopt_loaded = TRUE;
    }
    if (Run_ipl) {
      Get_Phase_Args (PHASE_IPL, &phase_argc, &phase_argv);
      load_so (IPL_DLL, Ipl_Path ? Ipl_Path : driver_path, Show_Progress);
    ...
    if (Run_lno || Run_autopar) {
      ...
      load_so (LNO_DLL, LNO_Path ? LNO_Path : driver_path, Show_Progress);
    ...
    if (Run_prompf || Run_w2fc_early) {
      ...
      load_so(PROMPF_DLL, Prompf_Anl_Path, Show_Progress);
    ...
    if (Run_purple) {
      ...
      load_so(PURPLE_DLL, Purple_Path, Show_Progress);
      Purple_loaded = TRUE;
	...
	)
    {
      ...
      load_so(WHIRL2C_DLL, W2C_Path ? W2C_Path : driver_path, Show_Progress);
      Whirl2c_loaded = TRUE;
      if (Run_prompf)
	...
	(Run_purple && Prp_Needs_Whirl2f()))
    {
      ...
      load_so(WHIRL2F_DLL, W2F_Path, Show_Progress);
      Whirl2f_loaded = TRUE;
      if (Run_prompf)
...
Phase_Init (void)
{
    ...
    /* It shouldn't be possible to have both autotie and ipl turned on
       at the same time, the argument parser is not supposed to allow
       that (in fact, I'd like the driver to not even pass -xpres
       when running ipl, but that doesn't seem to work). */
    FmtAssert(!Run_Autotie || !Run_ipl, ("can't specify both ipl and XPRES"));
    
    if (Run_Autotie || CG_fusion || CG_find_updating) {
      /* Initialize libfusion and libauto if necessary. */
      if (!AT_Init_XCC())
	...
	need_autotie_output = need_lno_output = need_wopt_output = FALSE;
    }

    if (Run_Autotie) {
	need_autotie_output = TRUE;
	need_ipl_output = need_lno_output = need_wopt_output = FALSE;
    }
    ...
    if (need_autotie_output) {
	Write_ALIAS_CLASS_Map = FALSE;
	Write_BE_Maps = FALSE;

	char *ofile = ((Obj_File_Name) ?
		       Obj_File_Name :
		       New_Extension(output_file_name, ".o"));
	ir_output = Open_Output_Info (ofile);

	XT_TRACE(AT_Libauto_Trace(), ("Writing autotie info to %s\n", ofile));
    }
    if (Emit_Global_Data) {
	Write_BE_Maps = FALSE;
    ...
    if ((Run_cg || Run_wopt) && !Read_Global_Data) {
        Process_Fill_Align_Pragmas(NULL);
...
Ipl_Processing (PU_Info *current_pu, WN *pu)
{
    ...
    if (ST_asm_function_st(*WN_st(pu)))  {
        ErrMsg(EC_Inv_Asm,"File level ASMs not supported with ipa");
    }
...
LNO_Processing (PU_Info *current_pu, WN *pu)
{
  ...
  for (REGION_CS_NoEarlierSub_First(&rgn_iter, pu, RID_TYPE_func_entry);
	...
	if (Get_Trace(TP_REGION,TT_REGION_ALL)) {
	  ...
	  if ( Cur_PU_Feedback ) {
	    Cur_PU_Feedback->Verify("After LNO Processing");
	  }
	}
      } else if (Run_preopt) {
...
Do_WOPT_and_CG_with_Regions (PU_Info *current_pu, WN *pu)
{
	...
	if (RID_options(rid) != NULL) {
	  Process_Pragma_Options(RID_options(rid));
	...
	if ( Cur_PU_Feedback ) {
          if (Feedback_Strict_Verify)
	    Cur_PU_Feedback->Verify("after LOWER_TO_CG");
          else
	    Cur_PU_Feedback->Verify_and_guess("after LOWER_TO_CG");
	}
...
void
Process_Pragma_Options(char *options_string)
{
  if (options_string == NULL) // no options pragma in this PU or region
    return;

  Is_Trace(Options_Stack->Trace(),
  ...
  INT32 argc = Options_Stack->str2argv(options_string, &argv, &Options_pool);
  // set the various options based on options string
  Process_Command_Line(argc, argv);
  MEM_POOL_Pop(&Options_pool);

#ifdef Is_True_On
  if (Options_Stack->Trace()) {
...
static void Update_EHRegion_Inito_Used (WN *wn) {
  ...
  OPERATOR opr = WN_operator(wn);
    ...
    /* For xtensa, we don't want to emit 'ereg_supp', it is used only
       to pass information internally from the front-end. So don't
       mark as used. */
...
static void Update_EHRegion_Inito (WN *pu) {
  ...
  FOREACH_INITO (CURRENT_SYMTAB, ino, i) {
    ST *st = INITO_st(ino);
    if (ST_sclass(st) == SCLASS_EH_DESC ||
...
private:
  ...
  UINT32 lno_outer_unroll_max;
  UINT32 lno_split_tiles;

public:
  // If optimize_for_space set OPT_Space and implied flags
  OPT_Space_Context(BOOL optimize_for_space) :
    opt_space(OPT_Space),
    opt_space_flix(OPT_Flix_Space),
    opt_unroll_size(OPT_unroll_size),
    cg_memmove_inst_count(CG_memmove_inst_count),
    lno_outer_unroll_max(LNO_Outer_Unroll_Max),
    lno_split_tiles(LNO_Split_Tiles)
  {
  ...
  // Restore original values for OPT_Space and implied flags
  ~OPT_Space_Context() 
  {
    OPT_Space = opt_space;
    OPT_unroll_size = opt_unroll_size;
...
{
    OPT_Space_Context os(PU_optimize_for_space(Get_Current_PU()));
	...
	Is_True(pu == PU_Info_tree_ptr(Current_PU_Info), ("Stale current_pu_info_ptr"));
    }
...
Preprocess_PU (PU_Info *current_pu)
{
  ...
  if ( Cur_PU_Feedback ) {
    ...
    Set_PU_Info_state(current_pu, WT_FEEDBACK, Subsect_InMem);
  }
  ...
  /* If running autotie, then initialize and attach an autotie PU
     object. */
  if (Run_Autotie) {
    INT64 wnl = WN_Get_Linenum(PU_Info_tree_ptr(current_pu));
    AT_Create_PU(current_pu, strdup(Src_File_Name), 
                 strdup(ST_name(PU_Info_proc_sym(current_pu))),
                 SRCPOS_linenum(wnl));
  }
  else {
    Set_PU_Info_autotie_ptr(current_pu, NULL);
    Set_PU_Info_state(current_pu, WT_AUTOTIE, Subsect_Missing);
  }
...
Postprocess_PU (PU_Info *current_pu)
{
  ...
  if (Run_Autotie)
    Write_Autotie_PU_Info(current_pu);
...
Preorder_Process_PUs (PU_Info *current_pu)
{
  ...
  // clear the offset for the "begin literal section"
  extern void Clear_Literal_Pool(void);// FIXME: this should go in data_layout.h
  Clear_Literal_Pool();
...
Process_Feedback_Options (OPTION_LIST* olist)
{
  if (Feedback_File_Name) {
    // loop over ever file name, each one specified with -ff,one_file_name
    char *one_file_name = new char[strlen(Feedback_File_Name)+2];
    char *ptr = Feedback_File_Name;
    char *endp = Feedback_File_Name+strlen(Feedback_File_Name);
    BOOL feedback_file_found = FALSE;
    while (ptr < endp && *ptr) {
      while (ptr < endp && *ptr != ',') ptr++;
      ptr++;
      int i=0;
      while (ptr < endp && *ptr != ' ') one_file_name[i++] = *ptr++;
      one_file_name[i] = 0;
      if (i) {
        ...
        // Find last '/' in path
        INT t = strlen(one_file_name);
        Is_True(t < FEEDBACK_PATH_MAXLEN - 16,
	        ("Process_Feedback_Options: Feedback file name too long(%d)", t));
        while (t > 0 && !is_dir_separator(one_file_name[t - 1]))
          t--;
    
        // Separate one_file_name into directory(path) and a file
        // name prefix.  The last character in path must be '/', so that
        // feedback file names can be appended.
        char path[FEEDBACK_PATH_MAXLEN];
        char *prefix = one_file_name + t;
        if (t > 0) {
          // Split at the '/'
          strncpy(path, one_file_name, t);
        } else {
          // No '/' in one_file_name; use "./" for current directory
          path[0] = '.';
          path[1] = '/';
          t = 2;
        }
        ...
        if (dirp == NULL) {
          ErrMsg(EC_FB_FILE_MISSING, one_file_name);
          return;
        }
    ...
    if (!feedback_file_found) {
      ErrMsg(EC_FB_FILE_MISSING, one_file_name);
    }
    delete one_file_name;
  }
...
#ifdef Is_True_On
  // string buffer for some print routines called in debugger
  // e.g. LRANGE::Format() in be/cg/gra_mon/gra_trace.cxx
  char debug_str[1024];
#endif
}
#include <unistd.h>
...
main (int argc, char **argv)
{
  ...
  /* setlinebuf is unavailable on solaris versions < 2.6 */
...
#endif
#if !_WIN32  
/* Maximize the number of open file descriptors permitted: */
  struct rlimit filedesc_limit;
  if (getrlimit(RLIMIT_NOFILE, &filedesc_limit) == 0) {
    filedesc_limit.rlim_cur = filedesc_limit.rlim_max;
    setrlimit(RLIMIT_NOFILE, &filedesc_limit);
  }
  ...
  Prepare_Source ();
  Initialize_Stats ();
  /* core mtypes have to be initialized efore target configuration */
  Mtype_initialize ();
  Configure ();
  ...
  // See be/com/be_util.cxx for how to use it
  Advance_CU_Count();
  ...
  // process result data file from CG basic block profiling
  Process_Profile_Data_File (Profile_File_Name);
  ...
  if (need_wopt_output || need_lno_output ||
      need_ipl_output || need_autotie_output) {
    if (need_autotie_output)
      Write_Global_Autotie_Info (pu_tree);
    else
      Write_Global_Info (pu_tree);
    if (need_ipl_output)
      Ipl_Extra_Output (ir_output);
    Close_Output_Info (need_autotie_output ? ET_AUTOTIE : ET_IR);
  }
