 ...
 * ====================================================================
 */
/* see the note in driver.cxx for the reason for this series of 
...
#include "config_lno.h"             /* for Run_autopar */
...
#include "dra_export.h"             /* for Run_Dsm_* */
#include "be_util.h"                /* for Set_CU_Skip_File_Name() */
...
#ifdef _WIN32
#define IPL_EXE_NAME "ipl.exe"
#else
#define IPL_EXE_NAME "ipl"
#endif
...
Process_Command_Line (INT argc, char **argv)
{
	...
	if ( !dashdash_flag && argv[i] != NULL && *(argv[i]) == '-' ) {
		...
		if (*cp == 0)
			...
			Obj_File_Name = cp + 2;
#ifdef TARG_XTENSA
			/* Xtensa cg can't generate object files
                           directly, so don't pass this flag to
                           it. This flag is used by BE with autotie to
                           indicate the name of the file to which the
                           autotie info is written. */
			break;
#else
			/* fall through */
#endif
	    ...
	    case 'W':		    /* */
		if (strncmp(cp, "unaligned", 9) == 0) {
		  Warn_Unaligned = TRUE;
		} else if (strncmp(cp, "no-unaligned", 12) == 0) {
    ...
    if (myname[0] == 'i' && strcmp (myname, IPL_EXE_NAME) == 0) {
	Run_ipl = TRUE;
    ...
    if (Run_Autotie && Run_ipl)
        Run_Autotie = FALSE;

    if (LNO_SIMD_Level > 0 && !Run_lno)
        ErrMsg ( EC_Simd_Opt_Lev );
