...
static void Fill_Align_Symbol (ST *st, BOOL is_fill, INT fa_value, WN *body_wn);
...
//
// return fill amount for corresponding ST via st_attr table
struct find_st_fill {
        ST_IDX st;
        find_st_fill (const ST *s) : st (ST_st_idx (s)) {}

        BOOL operator () (UINT, const ST_ATTR *st_attr) const {
            return (ST_ATTR_kind (*st_attr) == ST_ATTR_FILL &&
                    ST_ATTR_st_idx (*st_attr) == st);
        }
};

mUINT32
Find_Fill_For_ST (const ST *st)
{
    ...
    d = For_all_until (St_Attr_Table, ST_IDX_level (idx),
                          find_st_fill(st));
    FmtAssert(d != 0, ("didn't find fill amount ST %s", ST_name(st)));
    return ST_ATTR_fill(St_Attr_Table(ST_IDX_level (idx), d));
}
...
void Process_Fill_Align_Pragmas (WN* wn) {
  ...
  FOREACH_SYMBOL(CURRENT_SYMTAB, cst, i) {
    if (ST_is_fill_align (*cst)) {
      Fill_Align_Symbol (cst, TRUE, 1 << Find_Fill_For_ST(cst), wn);
    }
  }
...
static BOOL Static_Variable (ST* st) {
...
static void Fill_Align_Symbol (WN* wn, WN* func_wn) {
  ...
  Fill_Align_Symbol(st,is_fill,fa_value,body_wn);
}
...
static void Fill_Align_Symbol (ST *st, BOOL is_fill, INT fa_value, WN *body_wn) {
  ...
  if (!(ST_class(st) == CLASS_VAR && (Local_Variable(st) ||
                                      ...
                                      Global_Variable(st) ||
                                      Common_Variable(st)))) {
    ErrMsg(EC_Bad_Pragma_Abort, ST_name(st),
  ...
  FmtAssert (Common_Variable(st) || ST_ofst(st) == 0,
             ("Fill/Align_Symbol (%s): ST has an unexpected offset %d\n",
  ...
  if (Global_Variable(st) || Static_Variable(st) || (Local_Variable(st) && Known_Size(st))) {

    /* global, or non-vla local. similar processing. */

    /*
     * printf ("%s (%s): glob/fixed-local. oldsize %d, new_size %d, pad %" LLD_FMT " ofst %d\n",
    ...
    if (Global_Variable(st) || Static_Variable(st)) {
      Allocate_Object (st);
    }
  }
    ...
    /* find the alloca */
    FmtAssert (body_wn, ("No body_wn for local alloca of %s\n",
                           ST_name(st)));
