 ...
 * Module: bb.h
 * $Revision: 2.176 $
 * $Date: 2000/09/28 23:06:42 $
 * $Author: srinivas $
 * $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/bb.h,v $
 *
 * Description:
 *
 ...
 *   INT32 BB_Loop_Filenum(BB *bb);
 *     <bb> is the head of a loop; return the starting SRCPOS/line/filenum number
 ...
 *
 *   UINT BB_size (BB *bb)
 *      Return the total size of the ops in 'bb', in bytes.
 *
 *   BOOL BB_Defs_TN(BB *bb, TN *tn)
 *      Return a boolean to indicate if tn is defined within bb or not.
 *
 *   bool BB_Refs_TN(BB *bb, TN *tn)
 *      Return a boolean to indicate if tn is referenced within bb or not.
 *
...
typedef	struct bb {
  ...
  float         secondary_cost; /* additional cycle cost of min reg schedule */
...
#define	BB_secondary_cost(x)	 (CAN_USE_BB(x)->secondary_cost)
...
#define BBM_ASM_CLOBBER		0x400000 // has asm that clobbers hard registers
#define BBM_END_PREAMBLE	0x800000 // last of preamble BB
#define BBM_HAS_GLUE_COPY	0x1000000 // has glue copy (most likely an entry/exit of a region)
#define BBM_FOR_LAYOUT		0x2000000 // created by BB Layout, so should not be merged/deleted.
...
#define	BB_asm_clobber(x)	(BB_flag(x) & BBM_ASM_CLOBBER)
#define	BB_end_preamble(x)	(BB_flag(x) & BBM_END_PREAMBLE)
#define	BB_has_glue_copy(x)	(BB_flag(x) & BBM_HAS_GLUE_COPY)
#define	BB_for_layout(x)	(BB_flag(x) & BBM_FOR_LAYOUT)
...
#define	Set_BB_asm_clobber(x)	(BB_flag(x) |= BBM_ASM_CLOBBER)
#define	Set_BB_end_preamble(x)	(BB_flag(x) |= BBM_END_PREAMBLE)
#define	Set_BB_has_glue_copy(x)	(BB_flag(x) |= BBM_HAS_GLUE_COPY)
#define	Set_BB_for_layout(x)	(BB_flag(x) |= BBM_FOR_LAYOUT)
...
#define	Reset_BB_asm_clobber(x)	(BB_flag(x) &= ~BBM_ASM_CLOBBER)
#define	Reset_BB_end_preamble(x)	(BB_flag(x) &= ~BBM_END_PREAMBLE)
#define	Reset_BB_has_glue_copy(x)	(BB_flag(x) &= ~BBM_HAS_GLUE_COPY)
#define	Reset_BB_for_layout(x)		(BB_flag(x) &= ~BBM_FOR_LAYOUT)
...
#define FOR_ALL_BBs(start_bb, bb) \
	for (bb = start_bb; bb != NULL; bb = BB_next(bb))
...
extern void BBlist_Initialize();
extern void BBlist_Finalize();
...
inline INT32 BB_Loop_Filenum(BB *bb)
{
  SRCPOS pos = BB_Loop_Srcpos(bb);
  return SRCPOS_filenum(pos);
}
...
extern void Split_BBs (void);
extern UINT BB_size (BB *bb);
extern bool BB_Defs_TN (BB *bb, TN *tn);
extern bool BB_Refs_TN (BB *bb, TN *tn);
