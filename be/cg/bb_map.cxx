 ...
 *  Module: bb_map.c
 *  $Revision: 1.15 $
 *  $Date: 2000/04/06 01:42:35 $
 *  $Author: mtibuild $
 *  $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/bb_map.cxx,v $
 *
 *  Revision comments:
 *
...
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/bb_map.cxx,v $ $Revision: 1.15 $";
#endif /* _KEEP_RCS_ID */
...
BB_MAP BB_MAP_create_kind(_BB_MAP_KIND kind)
{
  ...
  case _BB_MAP_I64:
    ...
    break;
  case _BB_MAP_FP:
    result->values.fp = TYPE_P_ALLOC_N(float, length);
    break;
  default:
...
void BB_MAP_grow(BB_MAP map, BB *bb)
{
  ...
  case _BB_MAP_FP:
    map->values.fp =
      TYPE_MEM_POOL_REALLOC_N(float, &MEM_phase_pool, map->values.fp,
			      map->length, new_length);
    break;
  default:
