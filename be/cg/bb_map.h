 ...
 *  void BB_MAPFP_Set(BB_MAP map, BB *bb, float value)
 ...
 *  float BB_MAPFP_Get(BB_MAP map, BB *BB)
...
inline void BB_MAPFP_Set(BB_MAP map, BB *bb, float value)
{
  UINT32 idx = BB_map_idx(bb);
  DevAssert(map->kind == _BB_MAP_FP, ("BB_MAP has wrong kind"));
  if (value && idx >= map->length)
    BB_MAP_grow(map, bb);
  if (idx < map->length) {
    map->values.fp[idx] = value;
    map->value_gens[idx] = map->gen;
  }
}
...
inline float BB_MAPFP_Get(BB_MAP map, BB *bb)
{
  UINT64 idx = BB_map_idx(bb);
  DevAssert(map->kind == _BB_MAP_FP, ("BB_MAP has wrong kind"));
  return idx < map->length && map->value_gens[idx] == map->gen ?
...
#define BB_MAPFP_Create() BB_MAP_create_kind(_BB_MAP_FP)
