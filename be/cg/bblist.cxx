...
/*
   Memory pool for bblist, created at the begining of each PU and 
   freed at the end of each PU.
 */
static MEM_POOL  pu_bblist_pool;

void BBlist_Initialize()
{
  MEM_POOL_Initialize (&pu_bblist_pool, "bblist_pool", FALSE);
  MEM_POOL_Push(&pu_bblist_pool);
}

void BBlist_Finalize()
{
  BBlist_Free_List = NULL;
  num_bblist_buffers = 0;

  MEM_POOL_Pop(&pu_bblist_pool);
  MEM_POOL_Delete(&pu_bblist_pool);
}
...
bblist_alloc (void)
{
  ...
  if (tmp_list == NULL) {
    tmp_list = (BBLIST *) MEM_POOL_Alloc (&pu_bblist_pool, BBLIST_BLOCK_SIZE);
    num_bblist_buffers++;
    if (tmp_list == NULL) {
			 ...
			 BOOL via_feedback, BOOL set_prob)
{
  ...
  if (p == NULL) {
    p = bblist_alloc ();
    BBLIST_item(p) = bb;
    if (via_feedback || Cur_PU_Feedback) {
      BBLIST_prob(p) = prob;
      Set_BBLIST_prob_fb_based(p);
    } 
    else if (set_prob || FREQ_Frequencies_Computed()) {
      BBLIST_prob(p) = prob;
  ...
  else if (FREQ_Frequencies_Computed()) {
    BBLIST_prob(p) = prob;
    Reset_BBLIST_prob_fb_based(p);
  }
  return p;
