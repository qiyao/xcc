 ...
 * Module: bbutil.c
 * $Revision: 2.222 $
 * $Date: 2000/09/20 19:15:37 $
 * $Author: lesniak $
 * $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/bbutil.cxx,v $
 *
 * Revision history:
...
)
{
  ...
  TOP  new_top = CGTARG_Invert_Branch(old_top);

  Is_True(new_top != TOP_UNDEFINED, 
	  ("unable to negate branch: %s", TI_TOP_Name(old_top)));
...
Print_LOOPINFO(LOOPINFO *info)
{
  ...
  if (WN_loop_at_region_id(loop_info) != AT_REGION_ID_UNKNOWN)
    fprintf(TFile, "    at_region_id = %u\n", WN_loop_at_region_id(loop_info));
  fprintf(TFile, "    flags = ");
  ...
  if (WN_Loop_Dowhile(loop_info)) fprintf(TFile, "DOWHILE ");
  fprintf(TFile, "\n");
  ...
  if (LOOPINFO_max_trip_count_tn(info)) {
    fprintf(TFile, "    max trip count TN = ");
    Print_TN(LOOPINFO_max_trip_count_tn(info),FALSE);
    fprintf(TFile, "\n");
  }
  Emit_SWP_Failure_Note((SWP_RETURN_CODE)LOOPINFO_swp_failure_code(info), TFile,
			/*prefix=*/"    ", LOOPINFO_swp_unallococatable_rc(info));
}

/* ====================================================================
 *
...
Print_BB_Header ( BB *bp, BOOL flow_info_only, BOOL print_tn_info )
{
  ...
  if ( BB_entry(bp) ) {
    ...
    fprintf ( TFile, "  Dedicated TNs for Incoming Parameters: ");
    if (ENTRYINFO_in_params(ent) != NULL) {
      Print_TN_List(TFile, ENTRYINFO_in_params(ent));
    } else {
      fprintf ( TFile, "\n");
    }
  ...
  if ( BB_exit(bp) ) {
    ...
    fprintf ( TFile, "  Dedicated TNs for Return Value: ");
    if (EXITINFO_ret_value(exit) != NULL) {
      Print_TN_List(TFile, EXITINFO_ret_value(exit));
    } else {
      fprintf ( TFile, "\n");
    }
  ...
  if ( BB_call(bp) ) {
    ANNOTATION *ant = ANNOT_Get (BB_annotations(bp), ANNOT_CALLINFO);
    CALLINFO *centry = ANNOT_callinfo (ant);

    fprintf ( TFile, "  Dedicated TNs for Actuals: ");
    if (CALLINFO_actual_params(centry) != NULL) {
      Print_TN_List(TFile, CALLINFO_actual_params(centry));
    } else {
      fprintf ( TFile, "\n");
    }

    fprintf ( TFile, "  Dedicated TNs for Return Value: ");
    if (CALLINFO_ret_value(centry) != NULL) {
      Print_TN_List(TFile, CALLINFO_ret_value(centry));
    } else {
      fprintf ( TFile, "\n");
    }
  }
...
Print_BB_Pragmas( BB *bp )
{
  if ( BB_has_pragma(bp) ) {
	...
	fprintf(TFile, WN_pragmas[WN_pragma(wn)].name);
      ...
      switch (pragma) {
      case WN_PRAGMA_SUPER_SWP:
	fprintf(TFile, " (ii=%d,unroll=%d)\n", arg1, arg2);
	break;
      case WN_PRAGMA_UNROLL:
	fprintf(TFile, " %d times\n", arg1);
	break;
...
BB_branch_op( BB *bb )
{
  ...
  /* Test the last non-noop OP looking for the terminating branch.
   */
  while (op) {
...
#if 0
    /* remove for XTENSA to simplify search */
    ...
    BB   *new_target)	/* the new target basic block */
{
  ...
  FOREACH_INITV(INITO_val(inito), initv) {
    ...
    } else if (INITV_kind(initv) == INITVKIND_SYMDIFF) {
      if (Is_Label_For_BB(INITV_lab1(initv), old_target)) {
	if (!changed) {
	...
	Set_INITV_lab1(initv, lab);
      }
    } else {
      FmtAssert(FALSE, ("Illegal switch table entry"));
    }
  }
...
BB_Move_Delay_Slot_Op (BB *bb)
{
  ...
  FmtAssert(TI_PROC_Property_Set(PROP_has_branch_delay_slot), ("no delay slot to move"));
...
static void Split_BB(BB *bb)
{
  OP* op;
  INT i;
  const INT low = (INT)(Split_BB_Length * .75);
  const INT high  = (INT)(Split_BB_Length * 1.25);
  const INT len = BB_length(bb);
  mINT8 fatpoint[TI_ISA_REGCLASS_MAX+1];
  ...
  for (i = low;;) {
    if (i == high_idx) {
      if ( xt_prefer_const16 && 
           (OP_code(op_vec[min_idx]) == TOP_const16lo) ) {
        // Don't split const16hi and const16lo. 
        // Assume that the previous OP is TOP_const16hi, if not
        // we must search up to find it (not done yet). 
        min_idx--;
      }
...
struct BB_REGION_SET {
  ...
  ~BB_REGION_SET() { if (bbs) MEM_POOL_Pop(&pool); }
};
...
BB_size (BB *bb) 
{
  ...
  OP *op;
  FOR_ALL_BB_OPs_FWD(bb, op)
    w += OP_Real_Inst_Bytes(op);
  
  return w;
}

/* If BB does define tn, return true; otherwise, return false */
bool
BB_Defs_TN(BB *bb, TN *tn)
{
  OP *op;
...
/* If BB does reference tn, return true; otherwise, return false */
bool
BB_Refs_TN(BB *bb, TN *tn)
{
  OP *op;
