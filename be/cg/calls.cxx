 ...
 * Module: calls.cxx
 * $Revision: 3.250 $
 * $Date: 2000/10/23 21:46:00 $
 * $Author: mpm $
 * $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/calls.cxx,v $
 *
 * Revision history:
...
Setup_GP_TN_For_PU( ST *pu)
{
  ...
  /* On machines w/o GP, disable GP setup. */
  if( GP_TN == NULL ) {
...
Init_Callee_Saved_Regs_for_REGION ( ST *pu, BOOL is_region )
{
  ...
  if (NULL != RA_TN /* IA-32 doesn't have ra reg. */) {
    ...
    if (Target_ABI == ABI_CALL0) {
      Set_TN_is_gra_cannot_split(SAVE_tn(Return_Address_Reg));
    }
  }
  ra_intsave_tn = NULL;
  Caller_GP_TN = NULL;
  ...
  Caller_Pfs_TN = NULL;
  if (Pfs_TN) {
  	Caller_Pfs_TN = Gen_Typed_Register_TN (Pointer_type, Pointer_Size);
  ...
  Callee_Saved_Regs = (SAVE_REG *)Pu_Alloc (  (TI_ISA_REGCLASS_MAX + 1) 
					    * (REGISTER_MAX + 1)
					    * sizeof(SAVE_REG) );
  ...
  FOR_ALL_ISA_REGCLASS( cl ) {
    ...
    for ( reg = REGISTER_SET_Choose(regset);
      ...
      callee_tn = Gen_Typed_Register_TN (
			TN_mtype(ded_tn),
			(REGISTER_bit_size(cl,
			 REGISTER_CLASS_last_register(cl))/8) );
      Set_TN_save_creg (callee_tn, TN_class_reg(ded_tn));
      if (Target_ABI == ABI_CALL0) {
	/* FIXME: eventually, when we do correct dwarf CFE's, we
	   will want to get rid of this. Watch out for C++ exceptions
	   though.  */
  ...
  if (LC_TN) {
	...
	TN *callee_tn = Build_TN_Of_Mtype(MTYPE_U4);
...
Generate_Entry (BB *bb, BOOL gra_run )
{
  ...
  if (!BB_handler(bb)) {
    ...
    if ( Gen_Frame_Pointer && !PUSH_FRAME_POINTER_ON_STACK ) {
      ...
      if (TI_ABI_Property_Set (ABI_PROPERTY_callee,
			       TN_register_class(FP_TN),
			       REGISTER_machine_id(TN_register_class(FP_TN),
						   TN_register(FP_TN)) ))
      {
	// save old fp
      	if ( Caller_FP_TN == NULL ) {
        	...
        	if ( stn = PREG_To_TN_Array[ Caller_FP_Preg ] )
  			...
  			Caller_FP_TN = Gen_Typed_Register_TN ( 
...
#ifdef TARG_XTENSA
      /* If FP is an incoming argument register, we must be sure to
         save the argument value before overwritting FP_TN with the
         stack pointer. So copy the current value of FP_TN to a temp
         before copying SP_TN to FP_TN. */
      else if (TI_ABI_Property_Set (ABI_PROPERTY_func_arg,
				    TN_register_class(FP_TN),
				    REGISTER_machine_id(TN_register_class(FP_TN),
							TN_register(FP_TN)) ))
      {
	/* Search 'bb' for a copy from or store of FP_TN. If we find
           one before a define of FP_TN, we assume it is the use of
           FP_TN acting as an incoming argument. */
	BOOL stop = FALSE, copy = FALSE;
	TN *tmp = Gen_Typed_Register_TN(Pointer_type, Pointer_Size);

	for (OP *op = BB_first_op(bb); op && !stop; op = OP_next(op)) {
	  if (OP_copy(op)) {
	    INT src_idx = TI_ISA_Copy_Operand(OP_code(op), TRUE);
	    if (OP_opnd(op, src_idx) == FP_TN) {
	      Set_OP_opnd(op, src_idx, tmp);
	      copy = TRUE;
	      break;
	    }
	  }
	    ...
	    if ((storeval_idx >= 0) && (OP_opnd(op, storeval_idx) == FP_TN)) {
	      Set_OP_opnd(op, storeval_idx, tmp);
	      copy = TRUE;
	      break;
	    }
	  }
  ...
  if (NULL != RA_TN) {
	...
	&& TN_register_class(RA_TN) != TI_ISA_Regclass_Integer())
      {
	...
	if (ra_intsave_tn == NULL) {
        	ra_intsave_tn = Build_TN_Of_Mtype(MTYPE_I4);
  ...
  if (GP_Setup_Code == need_code) {
	...
	if ( stn = PREG_To_TN_Array[ Caller_GP_Preg ] )
	  ...
	  Caller_GP_TN = Gen_Typed_Register_TN (Pointer_type, Pointer_Size);
	...
	TN *got_disp_tn = Gen_Typed_Register_TN (Pointer_type,Pointer_Size);
...
Can_Be_Tail_Call(ST *pu_st, BB *exit_bb)
{
  ...
  FOR_ALL_BB_OPs_FWD(exit_bb, op) {
    if (OP_copy(op)) {
      TN *src = OP_opnd(op,TI_ISA_Copy_Operand(OP_code(op), TRUE));
...
Target_Unique_Exit (
  BB *bb,
  BB *unique_exit_bb,
  TN *rtn_tns[TI_ISA_REGCLASS_MAX+1][REGISTER_MAX+1])
{
  OP *op;
  ...
  if (FREQ_Frequencies_Computed()) {
    BB_freq(unique_exit_bb) += BB_freq(bb);
    if (BB_freq_fb_based(bb))
      Set_BB_freq_fb_based(unique_exit_bb);
    else
      Reset_BB_freq_fb_based(unique_exit_bb);
  }
}
...

{
  ...
  /* Restore the frame pointer. 
     windowed ABI will use the windowed register mechanism to restore it. */
  if (Target_ABI == ABI_CALL0 && Gen_Frame_Pointer) {
  ...
  /* we want the world to consider the moves to the SP 
     and FP as the stack adjust. Not this final one */
  if (!(Target_ABI == ABI_CALL0 && Gen_Frame_Pointer))
    EXITINFO_sp_adj(exit_info) = OPS_last(&ops);
  ...
  if (Target_ABI == ABI_WINDOWED) {
    /* For xtensa we don't need to emit a return since the exit spadjust
       is converted into a return, since the return also adjusts the
       sp. Maybe this could cause problems if we tried to do a tail
       call, but those aren't allowed on xtensa anyways (because we need
     the return to adjust the sp and register windows). */
    FmtAssert(!CG_tail_call, ("tail calls not supported for xtensa"));
  }
  else {
    if (!BB_call(bb_epi)) { 
 ...
 */
static TN *
Gen_Prolog_LDIMM64(TYPE_ID rtype, UINT64 val, OPS *ops)
{
  TN *src = Gen_Literal_TN(val, MTYPE_byte_size(rtype));
  TN *result = Build_TN_Of_Mtype(rtype);
  Exp_Immediate (result, src, FALSE, ops);
  return result;
}
...
Adjust_Entry(BB *bb)
{
  ...
  if (Trace_EE) {
    ...
    fprintf(TFile, "\nFinal frame size: %" LLU_FMT " (0x%" LLX_FMT ")\n", frame_len, frame_len);
  }
   ...
   */
  if (Target_ABI == ABI_CALL0 && frame_len == 0) {
    BB_Remove_Op(bb, sp_adj);

    if (sp_adj == fp_adj) {
      ent_adj = NULL;
    } else {
      FmtAssert(0, ("Invalidated prolog temps"));
      OPS ops = OPS_EMPTY;
      ...
      Set_OP_profile_bb_id(ent_adj,OP_profile_bb_id(fp_adj));
      OP * op;
      FOR_ALL_OPS_OPs_FWD(&ops, op) Set_OP_no_fusion(op);
     ...
     */
    if (!CGTARG_Can_Fit_Immediate_In_Entry_Instruction (frame_len)) {
	...
	FOR_ALL_ISA_REGCLASS(cl) {
      ...
      UINT64 frame_len_adj;
      if (Target_ABI == ABI_WINDOWED) {
	/* For windowed abi we will adjust by 16 with the entry instruction
	   itself, so we reduce the remaining frame size by that
	   much. */
	frame_len_adj = frame_len - 16;
      }
      else {
	frame_len_adj = frame_len;
      }
      
      incr = Gen_Prolog_LDIMM64(TN_mtype(SP_TN), frame_len_adj, &ops);
     ...
     */
    EETARG_Adjust_SP_For_Entry( incr, REGISTER_SET_Choose(temps[REGISTER_CLASS_sp]), &ops );
    FOR_ALL_OPS_OPs_FWD(&ops, op) {
      OP_srcpos(op) = OP_srcpos(sp_adj);
      Set_OP_profile_bb_id(op, OP_profile_bb_id(sp_adj));
    }
    ent_adj = OPS_last(&ops);
    FOR_ALL_OPS_OPs_FWD(&ops, op) Set_OP_no_fusion(op);
    ...
    if (fp_adj != sp_adj) {
       ...
       */
      Exp_COPY(FP_TN, SP_TN, &ops);
      OP_srcpos(OPS_last(&ops)) = OP_srcpos(fp_adj);
      Set_OP_profile_bb_id(OPS_last(&ops), OP_profile_bb_id(fp_adj));
...
Adjust_Exit(ST *pu_st, BB *bb)
{
   ...
   */
  if ( Target_ABI == ABI_WINDOWED ||
       !Gen_Frame_Pointer || PUSH_FRAME_POINTER_ON_STACK ) {
    incr = OP_opnd(sp_adj, OP_find_opnd_use(sp_adj, OU_opnd2));
  }
  else if (Gen_Frame_Pointer && Target_ABI == ABI_CALL0)
    incr = Frame_Len_TN;
  ...
  if (Gen_Frame_Pointer && !PUSH_FRAME_POINTER_ON_STACK) {
    if (Target_ABI == ABI_WINDOWED) {
      /* For windowed abi we don't restore the stack pointer from FP_TN since
	 our return/register windows does that, so don't check for a
	 copy. */
      FmtAssert( OP_code(sp_adj) == TOP_spadjust,
		 ("Unexpected form of exit SP-adjust OP 1"));
      FmtAssert( OP_results(sp_adj) == 1,
		 ("Unexpected form of exit SP-adjust OP 2"));
      FmtAssert(  (OP_result(sp_adj,0) == SP_TN),
		 ("Unexpected form of exit SP-adjust OP 3"));
      FmtAssert(  (OP_opnd(sp_adj,OP_find_opnd_use(sp_adj, OU_opnd1)) == SP_TN),
		 ("Unexpected form of exit SP-adjust OP 4"));
      FmtAssert( incr == Frame_Len_TN,
		 ("Unexpected form of exit SP-adjust OP 5"));
      FmtAssert(  ( ! OP_has_predicate(sp_adj)
		    || OP_opnd(sp_adj, OP_PREDICATE_OPND) == True_TN), 
		 ("Unexpected form of exit SP-adjust OP 1"));
    }
    else {
      if (Gen_Frame_Pointer) {
	FmtAssert( OP_code(sp_adj) == TOP_mov_n,
		   ("Unexpected form of exit SP-adjust OP A"));
	FmtAssert(  (OP_opnd(sp_adj,OP_find_opnd_use(sp_adj, OU_opnd1)) == FP_TN),
		    ("Unexpected form of exit SP-adjust OP B"));
      }
      else {
	FmtAssert( OP_code(sp_adj) == TOP_spadjust,
		   ("Unexpected form of exit SP-adjust OP C"));
	FmtAssert(  (OP_result(sp_adj,0) == SP_TN),
		    ("Unexpected form of exit SP-adjust OP D"));
      }		  	
      FmtAssert( OP_results(sp_adj) == 1,
		 ("Unexpected form of exit SP-adjust OP E"));
      FmtAssert(  (OP_result(sp_adj,0) == SP_TN),
		  ("Unexpected form of exit SP-adjust OP F"));

      FmtAssert(  ( ! OP_has_predicate(sp_adj)
		    || OP_opnd(sp_adj, OP_PREDICATE_OPND) == True_TN), 
		  ("Unexpected form of exit SP-adjust OP G"));
      FmtAssert(TN_is_sp_reg(OP_result(sp_adj,0)), ("Unexpected exit SP adjust OP 2"));
      FmtAssert(Frame_Len == 0 || incr != NULL, ("Unable to get frame size"));
    }
  } else {
    FmtAssert(   OP_code(sp_adj) == TOP_spadjust 
	      && OP_results(sp_adj) == 1
  ...
  if (PUSH_FRAME_POINTER_ON_STACK) {
    ...
    if (Target_ABI == ABI_CALL0 && Gen_Frame_Pointer) {
      while (OP_code(op) != TOP_spadjust)
	op = OP_next(op);
    }

    FmtAssert(OP_code(op) == TOP_spadjust, ("No spadjust found"));

    /* Now, op is the real sp_adj */
    REGISTER_SET temps[TI_ISA_REGCLASS_MAX + 1];
    REG_LIVE_Epilog_Temps(Get_Current_PU_ST(), bb, op, temps);

    /* Replace the SP adjust placeholder with the new adjustment OP
     */
    EETARG_Adjust_SP_For_Exit( incr, REGISTER_SET_Choose(temps[REGISTER_CLASS_sp]), &ops );
    ...
    if (Target_ABI == ABI_WINDOWED || !Gen_Frame_Pointer)
      sp_adj = OPS_last(&ops);
    ...
    FmtAssert(FALSE, ("Can't handle stack de-allocation of 0x%" LLX_FMT "", frame_len));
#endif
  }
