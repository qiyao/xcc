 ...
 * Module: cflow.cxx
 * $Revision: 3.243 $
 * $Date: 2000/09/20 19:15:41 $
 * $Author: lesniak $
 * $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/cflow.cxx,v $
 *
 * Description:
 *
...
#include "config_opt.h"  // using Instrumentation_Enabled
...
static BOOL Can_Append_Succ(BB *b, BB *suc, BB *merged_pred, BOOL delete_suc, BOOL trace);
static void Append_Succ(BB *b, BB *suc, BOOL in_cgprep, BOOL delete_suc);
...
static int branch_range_for_freq_order = 0x80; // 8bits
...
Format_Succ(BB *bb, INT isucc)
{
  ...
  if (nsuccs == 0) {
    ...
    if (offset != 0) {
      len += sprintf(buf + len, "+%" LLD_FMT "", offset);
    }
    if (freqs_computed) {
...
Print_BB_Info(BB *bb)
{
  ...
  case BBKIND_GOTO:
    ...
    break;
  case BBKIND_LOGIF:
    fprintf(TFile, BBINFO_b_likely(bb) ? " (likely)\n" : "\n");
...
Delete_BB_Contents(BB *bp)
{
  ...
  if (BBINFO_kind(bp) == BBKIND_VARGOTO) {
    ...
    if (refcount != NULL && --(*refcount) == 0) {
      /* If we remove a jump table, we rely on ebo to remove the l32r
	 that references the jump table. If we don't run ebo, we will
	 have a literal referring to a jump table that no longer
	 exists. */
      if ((CG_opt_level > 1) && Enable_CG_Peephole) {
  ...
  if (!CG_localize_tns) {
    BB_live_in(bp) = GTN_SET_ClearD(BB_live_in(bp));
    BB_live_out(bp) = GTN_SET_ClearD(BB_live_out(bp));
    BB_defreach_in(bp) = GTN_SET_ClearD(BB_defreach_in(bp));
    BB_defreach_out(bp) = GTN_SET_ClearD(BB_defreach_out(bp));
    BB_live_use(bp) = GTN_SET_ClearD(BB_live_use(bp));
    BB_live_def(bp) = GTN_SET_ClearD(BB_live_def(bp));
  }
  ...
  case BBKIND_GOTO:
    if (BBINFO_nsuccs(bp)) {
      ...
      Is_True(   !freqs_computed
		 ...
		 ("BB:%d expecting succ freq == 1.0", BB_id(bp)));
...
Initialize_BB_Info(void)
{
  ...
  for (bb = REGION_First_BB; bb; bb = BB_next(bb)) {
	 ...
	 */
	{
	  ...
	  TN *tn2;
	  OP *cmp;
	  INT variant = CGTARG_Analyze_Compare(br, &tn1, &tn2, &cmp);
	...
	if (bbkind != BBKIND_INDGOTO) { // Exclude BBKIND_INDGOTO, PR#5879
...
Is_Empty_BB(BB *bb)
{
  ...
  case 2:
    if (!TI_PROC_Property_Set(PROP_has_branch_delay_slot)
	|| !OP_noop(BB_last_op(bb))) return FALSE;
    /*FALLTHROUGH*/
  case 1:
...
Collapse_Empty_Goto(BB *bp, BB *targ, float in_freq)
{
...
#ifdef TARG_XTENSA
  /* If the compare op doesn't allow retargeting, then don't do
     anything. */

  if (BBINFO_kind(bp) == BBKIND_LOGIF)
  {
    OP *cop = BBINFO_compare_op(bp);
    if (cop != NULL)
    {
      TOP top = OP_code(cop);
      if ((top == TOP_loop) || (top == TOP_loopgtz)
	  || (top == TOP_loopnez) || (top == TOP_loop_end))
	return targ;
    }
  }
#endif
  ...
  while (   BBINFO_kind(new_targ) == BBKIND_GOTO
	 ...
	 && !BB_for_layout(new_targ)
	 && !BB_Has_Exc_Label(new_targ)
	 && !BB_loophead(new_targ)
	 && BBINFO_nsuccs(new_targ)
	 && BBINFO_succ_bb(new_targ, 0) != new_targ
  ) {
    BB *last_targ;

    /* do not collapse empty block with frequency hint */ 
    if (!freqs_computed && BB_has_pragma(new_targ)) {

      ANNOTATION *ant;
      bool has_frequency_hint = false;

      for ( ant = ANNOT_First(BB_annotations(new_targ), ANNOT_PRAGMA);
	ant != NULL && !has_frequency_hint;
...
Redundant_Logif(BB *pred, BB *succ, BOOL *pnegated)
{
  ...
  BOOL negated;
  INT pred_variant = BBINFO_variant(pred);
  INT succ_variant = BBINFO_variant(succ);

  /* TENSILICA: Don't try to optimize any unknown branch variants
   */
  if( pred_variant == V_BR_NONE || succ_variant== V_BR_NONE )
    return FALSE;
...
Convert_Indirect_Goto_To_Direct ( BB *bp )
{
  ...
  /* When using -fb_create, don't do convertion. See pr11953
   */
  if (Instrumentation_Enabled) {
    return FALSE;
  }

  /* If we remove a jump table, we rely on ebo to remove the l32r that
     references the jump table. If we don't run ebo, we will have a
     literal referring to a jump table that no longer exists. */
  if ((CG_opt_level <= 1) || !Enable_CG_Peephole)
    return FALSE;
...
Convert_If_To_Goto ( BB *bp )
{
  ...
  /* remove loops with only nops */
  compare_op = BBINFO_compare_op(bp);
  if (compare_op != NULL) {
    TOP top = OP_code(compare_op);
    if ((top == TOP_loop) || (top == TOP_loopgtz) ||
	(top == TOP_loopnez)) {
      BB* loop_body = BB_next(bp);
      OP* scan = BB_first_op(loop_body);
      while (scan && (OP_code(scan)==TOP_nop || OP_code(scan)==TOP_nop_n)) {
	scan = OP_next(scan);
      }
      if (scan && OP_code(scan)==TOP_loop_end) {
	if (BBINFO_succ_bb(bp,0)==loop_body)
	  result = 0;
	else
	  result = 1;
	goto convert_it;
  ...
  default:
    #pragma mips_frequency_hint NEVER
    DevWarn("Unhandled BBINFO_variant (%s) for BB:%d at line %d of %s",
	    BR_Variant_Name(BBINFO_variant(bp)),BB_id(bp),__LINE__,__FILE__);
    return FALSE;
  }
...
convert_it:
  {
     ...
     */
    if (!result && BBINFO_b_likely(bp) && br != BB_last_op(bp)) {
...
try_identities:
  if ( tn1 == tn2 ) {
    ...
    case V_BR_U8LE:
    case V_BR_I8EQ:
    case V_BR_U8EQ: result = TRUE; goto convert_it;
    case V_BR_I4LT:
    case V_BR_U4GT:
    case V_BR_U4LT:
    case V_BR_I4NE:
    ...
    case V_BR_U8LT:
    case V_BR_I8NE:
    case V_BR_U8NE: result = FALSE; goto convert_it;
    default:
      #pragma mips_frequency_hint NEVER
      DevWarn("Unhandled BBINFO_variant (%s) for BB:%d at line %d of %s",
	      BR_Variant_Name(BBINFO_variant(bp)),BB_id(bp),__LINE__,__FILE__);
...
Convert_Goto_To_If ( BB *bp, mBOOL *used_branch_around )
{
  ...
  /* If the LOGIF has more than one predecessor, this optimization will create
     extra branches. If the LOGIF has only one predecessor, this optimization
     will be taken care of by Merge_Blocks */
  if (OPT_Space) return FALSE;
 ...
 */
static BOOL
Convert_Goto_To_Return ( BB *bp, BOOL in_cgprep )
{
  OP *new_op;
  OP *br_op;
  OP *br_delay_op;
...
#ifdef TARG_XTENSA
  /* Xtensa's return is represented by the spadjust, and clone is
     called at least once before that spadjust is converted into the
     actual return instruction. */
  if ( !OP_br(rtn_op) && (OP_code(rtn_op) != TOP_spadjust) ) return FALSE;
#else
   ...
   */
#ifdef TARG_XTENSA
  if (!Can_Append_Succ(bp, targ, NULL, FALSE, CFLOW_Trace_Branch))
    return FALSE;

  Append_Succ(bp, targ, in_cgprep, FALSE);
#else
  ...
  if ( CFLOW_Trace_Branch ) {
    #pragma mips_frequency_hint NEVER
    fprintf(TFile, "changing BB:%d from GOTO-BB:%d+%" LLD_FMT " to RETURN\n",
		   BB_id(bp),
		   BB_id(targ),
		   offset);
  }
 ...
 */
static BOOL
Optimize_Branches(BOOL in_cgprep)
{
  INT pass;
     ...
     if (rid && RID_level(rid) >= RL_CGSCHED) continue;
	  ...
	  if (new_tgt != old_tgt) {
	    ...
	    Cflow_Change_Succ(bp, 1, old_tgt, new_tgt);
          ...
          // In case there is only ONE succ, we must remove conditional br
          if (changed && (BBINFO_succ_bb(bp, 0) == BBINFO_succ_bb(bp, 1))) {
            BB_Remove_Op(bp, BB_branch_op(bp));
	...
	if (new_tgt != old_tgt) {
	  ...
	  Cflow_Change_Succ(bp, 0, old_tgt, new_tgt);
        ...
        if (new_tgt != BB_next(bp)) {
	      ...
	      || Convert_Goto_To_Return(bp, in_cgprep)
          ) changed = TRUE;
	} else if (   pass == 1
		   && BB_branch_op(bp)
...
#pragma mips_frequency_hint NEVER
  ...
  /* Reject if <pred> is a switch jump to avoid switch table update
   */
  if (BBINFO_kind(pred) == BBKIND_VARGOTO) {
    return FALSE;
  }
  ...
  /* Copy has_glue_copy flag
   */
  if (BB_has_glue_copy(b))
    Set_BB_has_glue_copy(pred);
 ...
 * Can_Append_Succ
 *
 * Return true if it is possible to append BB <suc> to BB <b>, and if
 * <delete_suc> is true, if it is also to delete <suc> is deleted
 * after the append.
 *
 * ==================================================================== */
static BOOL
  ...
  BOOL trace)
{
  ...
  case BBKIND_LOGIF:
#ifdef TARG_XTENSA
    /* Reject if 'suc' contains a zcl instruction, and 'b' is not prev
       BB to 'suc'. We don't want the zcl block being moved since it
       is tied to the loop end. */
    if (suc != BB_next(b))
    {
      OP *cop = BBINFO_compare_op(suc);
      if (cop != NULL)
      {
	TOP top = OP_code(cop);
	if ((top == TOP_loop) || (top == TOP_loopgtz)
	    || (top == TOP_loopnez) || (top == TOP_loop_end))
	{
	  if (trace)
	  {
            #pragma mips_frequency_hint NEVER
	    fprintf(TFile, "rejecting %s of BB:%d into BB:%d"
		    " (zero-overhead loop BB can only merge with fall through pred)\n",
		    oper_name, BB_id(suc), BB_id(b));
	  }
	  return FALSE;
	}
  ...
  /* Reject if BB is for layout purpose */
  if (BB_for_layout(b) || BB_for_layout(suc)) {
    if (trace) {
      #pragma mips_frequency_hint NEVER
      fprintf(TFile, "rejecting %s of BB:%d into BB:%d"
		     " (either BB is for layout)\n",
		     oper_name, BB_id(suc), BB_id(b));
    }
    return FALSE;
  }
  ...
  /* don't merge entry and exit blocks together. only GRA is
     smart enough to allocate save tns to their original registers
     if possible, allowing copy prop to eliminate the copies.
  */
  if (BB_entry(b) && BB_exit(suc)) {
    if (trace) {
      #pragma mips_frequency_hint NEVER
      fprintf(TFile, "rejecting %s of BB:%d into BB:%d"
		     " (successor is an exit block)\n",
		     oper_name, BB_id(suc), BB_id(b));
    }
    return FALSE;
  }
   ...
   *
   * If neither BB is in a loop (e.g., fully unrolled single-BB loop),
   * allow them to merge (see PR10522).
   */
  if (BB_unrollings(b) || BB_unrollings(suc)) {
    if (BB_loop_head_bb(b)==NULL && BB_loop_head_bb(suc)==NULL) {
    } else if (   !BB_unrollings(b)
	|| !BB_unrollings(suc)
...
Copy_Jump_Table(ST *old_listvar)
{
  ...
  listvar = Gen_Read_Only_Symbol(table, ".Ljump_table");
  ...
  BOOL delete_suc)
{
  ...
  if (!CG_localize_tns) {
    ...
    if (!(current_flags & CFLOW_AFTER_LRA)) {
  ...
  BB_freq(suc) -= BB_freq(b) * BBLIST_prob(BB_Find_Succ(b, suc));
  ...
  if (BB_asm_clobber(suc)) {
    Set_BB_asm_clobber(b);
  }

  /* Copy has_glue_copy flag
   */
  if (BB_has_glue_copy(suc))
    Set_BB_has_glue_copy(b);
...
Merge_Blocks ( BOOL in_cgprep )
{
  ...
  for (b = REGION_First_BB; b; b = next_b) {
    ...
    // Since this func will be invoked from IGLS, so need
    // to reset schedul bit
    if (b_changed) Reset_BB_scheduled(b);
  }
...
static double feedback_tolerance = 0.40;
 ...
 * order, i.e. larger weights come first. If weights are equal we
 * use unique edge id's to ensure that the ordering is well-defined.
 *
 * ====================================================================
 */
static INT
...
static inline BOOL Is_OP_tie(OP *op)
{
  TOP a_top = OP_code(op);
  if (OP_generic(op)) {
    TOP tops[256];
    int num_tops = TI_TOP_Get_Special_Tops(OP_code(op), tops, 256);
    FmtAssert(num_tops > 0, ("Wrong generic op %s", TI_TOP_Name(OP_code(op))));

    a_top = tops[0];  // If the first isn't TIE, assume it's not a TIE.
  }

  return TI_ISA_Property_Is_Tie(a_top);
}
...
Init_Edges(EDGE *edges)
{
  ...
  INT taken_cycle;
  CGTARG_Compute_Branch_Parameters(&idummy, &fixed_branch_cost, &taken_cycle, &ddummy);

  double taken_branch_cost = (float)taken_cycle * 1.1; // 10% alignment penalty
  double thr = 1.0f - (float)(taken_branch_cost)/(1.0f + 3.0 * (float)(taken_branch_cost));
  ...
  for (bb = REGION_First_BB; bb; bb = BB_next(bb)) {
	...
	if (succ0 != bb && succ1 != bb) {
	  ...
	  if (OP_xfer(br_op) && Is_OP_tie(br_op)) {
	      /* for TIE branch, assuming no complementary branch for now,
		 we need to favor the fall-through (i.e., successor 1)
	       */
	      weight0 = 0.49;
	      weight1 = 0.51;
	  } else if (BB_next(bb)==succ0 && BB_next(succ0)==succ1 &&
	      BBINFO_succ_bb(succ0, 0)==succ1 && prob1 < thr) {
	    /* for if-then-endif flow, it is almost always better to stack the
	     * then-block unless it is really biased toward taken target
	     */
	    weight0 = 0.51;
	    weight1 = 0.49;
	  } else if (BB_next(bb)==succ1 && BB_next(succ1)==succ0 &&
	      BBINFO_succ_bb(succ1, 0)==succ0 && prob0 < thr) {
	    /* for if-then-endif flow, it is almost always better to stack the
	     * then-block unless it is really biased toward taken target
	     */
	    weight0 = 0.49;
	    weight1 = 0.51;
          } else if (prob0 >= 0.50 - delta && prob0 <= 0.50 + delta) {
	    if (BB_next(bb) == succ0) {
	      weight0 = 0.51;
	      weight1 = 0.49;
  ...
  for (bb = REGION_First_BB; bb; bb = BB_next(bb)) {
	...
	if (prob > (1.0 - avg_prob) + delta) goto next_bb;
	if (prob > avg_prob + delta && prob < (1.0 - avg_prob) - delta) goto next_bb;
      }
    }
...
typedef struct bbchain {
  ...
  BOOL has_before_bb;   /* chain contains a BB that must occur before some other BB */
  BOOL has_after_bb;    /* chain contains a BB that must occur after some other BB */
  BBCHAIN_LIST *first_pred;  /* list of chains that must precede this one. */
  BBCHAIN_LIST *first_succ;  /* list of chains that must succeed this one. */
  BBCHAIN_LIST *first_near;  /* list of chains that must be close to this one */
  BBCHAIN *zcl_chain;  /* the chain that the current one must be close to. */
} BBCHAIN;

static BB_LIST *before_bbs;
static BB_LIST *after_bbs;
...
/* ====================================================================
 *
 * Add_Chain_Dependence
 *
 * Show that chain 'succ' must follow (not necessarily immediately)
 * 'pred' in the final layout.
 *
 * ====================================================================
 */
static void
Add_Chain_Dependence (BBCHAIN *pred, BBCHAIN *succ)
{
  FmtAssert(pred != succ, ("attempt to make chain dependent on itself"));
  pred->first_succ = BBCHAIN_LIST_Push(succ, pred->first_succ, &MEM_pu_pool);
  succ->first_pred = BBCHAIN_LIST_Push(pred, succ->first_pred, &MEM_pu_pool);
}


/* ====================================================================
 *
 * Remove_Chain_Dependence
 *
 * Clear that chain 'succ' must follow (not necessarily immediately)
 * 'pred' in the final layout.
 *
 * ====================================================================
 */
static void
Remove_Chain_Dependence (BBCHAIN *pred, BBCHAIN *succ)
{
  pred->first_succ = BBCHAIN_LIST_Delete(succ, pred->first_succ);
  succ->first_pred = BBCHAIN_LIST_Delete(pred, succ->first_pred);
}


/* ====================================================================
 *
 * Move_Chain_Dependencies
 *
 * Move all of 'schain's dependencies to 'pchain'. If 'pchain' is
 * NULL, simply remove 'schain' dependencies.
 *
 * ==================================================================== */
static void
Move_Chain_Dependencies (BBCHAIN *pchain, BBCHAIN *schain)
{
  for (BBCHAIN_LIST *succs = schain->first_succ;
       succs; succs = BBCHAIN_LIST_rest(succs))
  {
    BBCHAIN *succ = BBCHAIN_LIST_first(succs);
    Remove_Chain_Dependence(schain, succ);
    if (pchain && (pchain != succ))
      Add_Chain_Dependence(pchain, succ);
  }

  for (BBCHAIN_LIST *preds = schain->first_pred;
       preds; preds = BBCHAIN_LIST_rest(preds))
  {
    BBCHAIN *pred = BBCHAIN_LIST_first(preds);
    Remove_Chain_Dependence(pred, schain);
    if (pchain && (pchain != pred))
      Add_Chain_Dependence(pred, pchain);
  }

  FmtAssert((schain->first_pred == NULL) && (schain->first_succ == NULL),
	    ("unexpected pred/succ"));
}
 ...
 *
 *
 * Chain_Dependence_Distance
 *
 * If there is a direct or indirect requirement that 'pchain' be layed
 * out before 'schain', then return the number of intervening chains
 * in the dependence + 1. For example if the dependence is 'pchain' ->
 * 'schain', return 1; if the dependence is 'pchain' -> X -> Y ->
 * 'schain', return 3. If there are multiple dependence paths between
 * 'pchain' and 'schain', return the maximum distance. Return 0 if
 * there is no requirement that 'pchain' be layed out before 'schain'.
 *
 * ==================================================================== */
static UINT
Chain_Dependence_Distance (BBCHAIN *pchain, BBCHAIN *schain, UINT distance =0)
{
  UINT new_distance = 0;

  /* We don't have to check for already visited chains because the
     dependencies must form a dag (i.e. no directed cycles). */

  for (BBCHAIN_LIST *succs = pchain->first_succ;
       succs; succs = BBCHAIN_LIST_rest(succs))
  {
    BBCHAIN *chain = BBCHAIN_LIST_first(succs);

    if (chain == schain)
      new_distance = MAX(distance + 1, new_distance);
    else
      new_distance = MAX(new_distance,
			 Chain_Dependence_Distance(chain, schain, distance + 1));
  }
...
Init_Chains(BBCHAIN *chains)
{
  ...
  BBCHAIN *prev = NULL;
  BOOL inside_zcl=FALSE;
  BBCHAIN *last_zcl_chain=NULL;
  BBCHAIN *chains_save = chains;
  ...
  for (bb = REGION_First_BB; bb; bb = next_bb) {
    ...
    // We keep loop starting block (block end with zero-cost loop instruction
    // and its immediate successor in the same chain
    // We also keep loop ending block (block with loop_end instruction
    // and its immediate successor in the same chain
    // We associate chains inside a zcl loop with the chain that starts the
    // zcl. This is needed to generate cold code inside a zcl not too far
    // away from the zcl to avoid assembler relaxation due to out of range
    // branch.
    next_bb = bb;
    BOOL keep_growing;
	...
	keep_growing=TRUE;
      ...
      if (br_op != NULL)
      {
	TOP br_opcode = OP_code(br_op);
	if (OP_loop_start(br_op)) {
	  if (inside_zcl==TRUE)
	    DevWarn("Bad zcl nesting");
	  inside_zcl=TRUE;
	  keep_growing=TRUE;
	  loop_end=FALSE;
	  loop_start=TRUE;
	} else if (br_opcode==TOP_loop_end) {
	  if (inside_zcl==FALSE)
	    DevWarn("Bad zcl nesting");
	  inside_zcl=FALSE;
	  keep_growing=TRUE;
      ...
      } else if (BB_call(tail)) { // call is not a branch_op

	// need to keep call block and the next block
	// (and its uses of return value) together
	if (BB_next(tail)) {
	  BB* succ = BB_next(tail);
	  OP* op = BB_first_op(succ);
	  if (op && OP_copy(op)) {
	    TN* copy_opnd = OP_opnd(op,0);
	    if (TN_is_dedicated(copy_opnd))
	      keep_growing=TRUE;
	  }
	}
      }
      ...
      if ( next_bb &&
	   ((BB_rid(next_bb) != first_rid) ||
	    (BBINFO_eh_rgn(tail) && BBINFO_eh_rgn(next_bb))))
	keep_growing=TRUE;
    } while (next_bb && keep_growing);
    //Is_True(inside_zcl==FALSE,("Bad zcl nesting"));

#ifdef TARG_XTENSA
#if 0
    /* If 'tail' ends in a loop instruction, then we want to include
       the next block (the first block in the loop) in the chain, so
       that the loop instruction is not separated from the loop
       itself. Similarly, if 'tail' ends in a loop_end instruction, we
       want to include the next block (the first block in the epilog)
       in the chain, so that the loop_end instruction is not separated
       from the block targeted by the loop instruction. */
    while (next_bb)
    {
      OP *br_op = BB_branch_op(tail);
      if (br_op != NULL)
      {
	TOP br_opcode = OP_code(br_op);
	if ((br_opcode == TOP_loop) || (br_opcode == TOP_loopgtz)
	    || (br_opcode == TOP_loopnez) || (br_opcode == TOP_loop_end))
	{
	  // the check below is not adequate any more since the epilog of a
	  // remainder loop can be merged with the prolog of the unrolled loop
	  // so the rid of the merged block may be different from that of the tail
	  //Is_True(BB_rid(tail) == BB_rid(next_bb),
		  //("loop instruction in different region than loop at BB:%d", BB_id(tail)));
	  tail = next_bb;
	  BB_MAP_Set(chain_map, tail, chains);
	  next_bb = BB_next(tail);
	  continue;
    ...
    chains->has_before_bb = FALSE;
    chains->has_after_bb = FALSE;
    chains->first_pred = NULL;
    chains->first_succ = NULL;
    chains->first_near = NULL;
    chains->zcl_chain = NULL;

    if (last_zcl_chain) {
      last_zcl_chain->first_near =
	      BBCHAIN_LIST_Push(chains, last_zcl_chain->first_near, &MEM_pu_pool);
      chains->zcl_chain = last_zcl_chain;
    }

    if (loop_start)
      last_zcl_chain = chains;
    else if (loop_end) {
      last_zcl_chain = NULL;
    }
  ...
  // associate conditional branches inside innermost loops with their targets
  for (BBCHAIN *bbchain = chains_save; bbchain; bbchain = bbchain->next)
  {
    if (bbchain->zcl_chain==NULL) {
      BB *bb = bbchain->head;
      BB *loop_head = BB_loop_head_bb(bb);
      if (loop_head && bb!=loop_head && BB_innermost(loop_head)) {
	BBCHAIN *loopchain = BB_Chain(chain_map, loop_head);

	// loop_head may not be at the head of a chain but still in the same chain
	// check to avoid making a chain to be a near chain of itself (PR10629)
	if (loopchain != bbchain) {
          loopchain->first_near =
	      BBCHAIN_LIST_Push(bbchain, loopchain->first_near, &MEM_pu_pool);
	  bbchain->zcl_chain = loopchain;
	}
      }
    }
  }
 ...
 * Record_Block_Pair
 *
 * Record that BB 'before' must precede BB 'after' in the final
 * layout.
 *
 * ==================================================================== */
static void
Record_Block_Pair (BB *before, BB *after, BB_MAP chain_map)
{
  FmtAssert(before != after, ("can't pair a block with itself"));

  BBCHAIN *before_chain = BB_Chain(chain_map, before);
  BBCHAIN *after_chain = BB_Chain(chain_map, after);

  before_chain->has_before_bb = TRUE;
  after_chain->has_after_bb = TRUE;

  /* If the chains are different, then show that 'after_chain' must be
     after 'before_chain' in the final layout. If the chains are the
     same then the blocks must already be correctly ordered within the
     chain. */
  if (before_chain != after_chain)
    Add_Chain_Dependence(before_chain, after_chain);

  /* Record in parallel lists the 'before'/'after' pair. */

  before_bbs = BB_LIST_Push(before, before_bbs, &MEM_pu_pool);
  after_bbs = BB_LIST_Push(after, after_bbs, &MEM_pu_pool);
}
 ...
 * Block_Ordering
 *
 * Return the ordering required for 'bb'. 'bb' only requires an
 * ordering if there is another BB paired with 'bb' in the same chain.
 *
 * ==================================================================== */
enum BLOCK_ORDERS { BO_NONE, BO_BEFORE, BO_AFTER, BO_BEFORE_AFTER };

static BLOCK_ORDERS
Block_Ordering (BB *bb, BB_MAP chain_map)
{
  BB_LIST *before_scan = before_bbs;
  BB_LIST *after_scan = after_bbs;
  BLOCK_ORDERS order = BO_NONE;

  while (before_scan)
  {
    FmtAssert(after_scan, ("Paired BB lists corrupt, after_bbs missing entries"));

    BB *before = BB_LIST_first(before_scan);
    BB *after = BB_LIST_first(after_scan);
    BBCHAIN *before_chain = BB_Chain(chain_map, before);
    BBCHAIN *after_chain = BB_Chain(chain_map, after);

    if (before_chain == after_chain)
    {
      if ((order != BO_BEFORE) && (bb == before))
	order = ((order == BO_NONE) ? BO_BEFORE : BO_BEFORE_AFTER);
      else if ((order != BO_AFTER) && (bb == after))
	order = ((order == BO_NONE) ? BO_AFTER : BO_BEFORE_AFTER);
    }

    before_scan = BB_LIST_rest(before_scan);
    after_scan = BB_LIST_rest(after_scan);
  }

  FmtAssert(before_scan == after_scan, ("Paired BB lists corrupt, after_bbs extra entries"));
  return order;
}
 ...
 * Pair_Chains
 *
 * Find pairs of BBs that must occur in order in the final layout, but
 * where the before block does not necessarily need to immediatly
 * preceed the after block (those cases are handled in Init_Chains).
 *
 * ==================================================================== */
static void
Pair_Chains(BBCHAIN *chains, BB_MAP chain_map)
{
#ifdef TARG_XTENSA
  /* For xtensa the block containing a loop instruction must precede
     the block that follows immediately the block containing
     the corresponding loop-end instruction. */
  for (BBCHAIN *chain = chains; chain; chain = chain->next)
  {
    for (BB *bb = chain->head; bb; bb = BB_next(bb))
    {
      /* Look for a block containing a loop-end instruction. From there
	 we can find the loop block. */
      OP *end_op = BB_branch_op(bb);
      if ((end_op != NULL) && (OP_code(end_op) == TOP_loop_end))
      {
	BBLIST *target_edge = BB_succs(bb);
	BB *target = BBLIST_item(target_edge);
	if (target == BB_next(bb))
	{
	  target_edge = BBLIST_next(target_edge);
	  target = BBLIST_item(target_edge);
	}

	BB *loop_block = BB_prev(target);
	OP *loop_op = BB_branch_op(loop_block);
	FmtAssert((loop_op != NULL) &&
		  ((OP_code(loop_op) == TOP_loop) || (OP_code(loop_op) == TOP_loopgtz)
		   || (OP_code(loop_op) == TOP_loopnez)),
		  ("expecting BB:%d to be loop block", BB_id(loop_block)));

	Record_Block_Pair(loop_block, BB_next(bb), chain_map);

	if (CFLOW_Trace_Freq_Order)
	  fprintf(TFile, "  pairing blocks BB:%d and BB:%d\n", BB_id(loop_block), BB_id(bb));
      }
    }
  }
#endif
  ...
  RID *cold_rid)
{
  ...
  if (BBINFO_kind(bb) == BBKIND_GOTO) {
    OP *br = BB_branch_op(bb);
    OP *delay_slot = (TI_PROC_Property_Set(PROP_has_branch_delay_slot) && br)
      ? OP_next(br) : NULL;
 ...
 *
 * The first chain will be the chain containing the entry
 * point. Subsequent chains are selected according to their weight --
 * chains with a heavier weight are chosen first. The weight of a
 * unordered chain is the sum of the edge frequencies from the ordered
 * chains to that chain. Once a chain is ordered, its weight is
 * irrelevant. Chains are also ordered so that they obey any chain
 * dependencies.
 *
 * This ordering has the effect of somewhat clustering related chains,
 * but more certainly, it causes frequently executed chains to end up
 * near the end of the region.
 *
 * Note that we only consider the edges from the ordered chains to the
 * unordered chains when computing the weight. Ideally we should
 * consider the edges from the unordered chains to the ordered chains
 * as well.
 *
 * ==================================================================== */
static BBCHAIN *
Order_Chains(BBCHAIN *unordered, BB_MAP chain_map)
{
  ...
  Move_Chain_Dependencies(NULL, chain);
  ...
  while (unordered) {
     ...
     * normal BBs over those with a NEVER freq hint. Do not select a
     * chain if it depends on any chain not yet ordered.
     */
    chain = NULL;
    for (ch = unordered; ch; ch = ch->next) {
      if (((chain == NULL) ||
	   (ch->never == chain->never && ch->weight > max_weight) ||
    ...
    FmtAssert(chain != NULL, ("cyclic chain dependencies, unable to find non-dependent chain"));

    /* If the input freqs/probs are bogus (usually NaNs) we might not
     * find a candidate. Strongly complain and then just take the
     * first thing on the list that does not have a dependence
     * (i.e. 'chain').
     */
    if (max_weight < 0.0) {
      if (CG_warn_bad_freqs) {
    ...
    /* Move the selected chain from the unordered list to the tail of
     * the ordered list. Remove all dependencies so that chains
     * dependent on 'chain' may now be considered for ordering.
     */
    Move_Chain_Dependencies(NULL, chain);
    unordered = Remove_Chain(unordered, chain);
    chain->next = NULL;
...
Optimize_Cyclic_Chain(BBCHAIN *chain, BB_MAP chain_map)
{
  ...
  BOOL *non_tail_bb = NULL;

  if (CFLOW_Trace)
  {
    fprintf(TFile, "  Optimize_Cyclic_Chain: ");
    Print_Chain_BBs(chain);
  }

  /* We may need to disallow some blocks from being the tail of the
     chain. If the chain contains any block pairs that must remain
     ordered (i.e. one of the blocks must remain before the other in
     the chain), then don't let any of the intervening blocks become
     the tail, since that would rotate the "after" to before the
     "before" block. */
  if (chain->has_before_bb && chain->has_after_bb)
  {
    INT no_tail_cnt = 0;
    non_tail_bb = (BOOL *)alloca((PU_BB_Count + 2) * sizeof(BOOL));
    for (bb = chain->tail; bb; bb = BB_prev(bb))
    {
      Is_True(BB_id(bb) < (PU_BB_Count+2), ("unexpected BB id, %d", BB_id(bb)));
      non_tail_bb[BB_id(bb)] = (no_tail_cnt > 0);

      BLOCK_ORDERS order = Block_Ordering(bb, chain_map);
      if ((order == BO_BEFORE) || (order == BO_BEFORE_AFTER))
      {
	FmtAssert(no_tail_cnt > 0, ("found before block BB:%d, after its after block", BB_id(bb)));
	no_tail_cnt--;
      }
      if ((order == BO_AFTER) || (order == BO_BEFORE_AFTER))
      {
	no_tail_cnt++;
      }
    }

    FmtAssert(no_tail_cnt == 0, ("found after block without corresponding before block"));
  }
  ...
  for (bb = chain->tail; bb; bb = BB_prev(bb)) {
...
#ifdef TARG_XTENSA
	/* Account for the static cost of the unconditional branch. */
	if (no_fall)
	  tail_gain -= br_static_cost;
#endif
      }
      break;
    case BBKIND_GOTO:
    ...
    if (CFLOW_Trace)
    {
      if ((non_tail_bb == NULL) || !non_tail_bb[BB_id(bb)])
	fprintf(TFile, "    BB:%d gain as tail, %.3f\n", BB_id(bb), tail_gain);
      else
	fprintf(TFile, "    BB:%d cannot be tail\n", BB_id(bb));
    }
 ...
 * Chain_Size
 *
 * Compute the size (in bytes) of the chain to estimate how far a
 * branch inside a zcl chain can jump.
 *
 * ====================================================================
 */
INT Chain_Size(BBCHAIN *chain) {
  BB *bb = chain->head;
  INT size = 0;
  while (bb) {
    UINT w = 0;
 ...
 * Append_Near_Chains
 *
 * Append chains on the near list to the associated zcl chain so the
 * branch inside zcl will not be out of range.
 *
 * ====================================================================
 */
static void
Append_Near_Chains(BBCHAIN *chains, BBCHAIN *chain, BB_MAP chain_map) {

  if (CFLOW_Trace_Freq_Order) {
    fprintf(TFile, "  Append_Near_Chains: ");
    Print_Chain_BBs(chain);
  }

  // move all near chains of schain to pchain
  // we maintain the same order of the list after the move
  // which is normally with the early chains pushed in first
  BBCHAIN_LIST *tmp_chain_list=NULL;

  for (BBCHAIN_LIST *nears = chain->first_near;
       nears; nears = BBCHAIN_LIST_rest(nears)) {
    BBCHAIN *Xnear = BBCHAIN_LIST_first(nears);
    chain->first_near = BBCHAIN_LIST_Delete(Xnear, chain->first_near);
    tmp_chain_list = BBCHAIN_LIST_Push(Xnear, tmp_chain_list, &MEM_pu_pool);
  }
  for (BBCHAIN_LIST *nears = tmp_chain_list;
       nears; nears = BBCHAIN_LIST_rest(nears)) {

    BBCHAIN *Xnear = BBCHAIN_LIST_first(nears);

    if (CFLOW_Trace_Freq_Order) {
      fprintf(TFile, "\tNear chains: ");
      Print_Chain_BBs(Xnear);
    }

    tmp_chain_list = BBCHAIN_LIST_Delete(Xnear, tmp_chain_list);
    Chain_BBs(chain->tail, Xnear->head);
    chain->tail = Xnear->tail;

    Move_Chain_Dependencies(chain, Xnear);

    chain->has_before_bb |= Xnear->has_before_bb;
    chain->has_after_bb |= Xnear->has_after_bb;

    /* Update the BB-to-chain mapping for the BBs we're moving.  */
    for (BB *bb = Xnear->head; ; bb = BB_next(bb)) {
      BB_MAP_Set(chain_map, bb, chain);
      if (bb == Xnear->tail) break;
    }

    /* Remove the near chain.  */
    chains = Remove_Chain(chains, Xnear);
  }

  if (CFLOW_Trace_Freq_Order) {
    fprintf(TFile, "  After Append_Near_Chains: ");
    Print_Chain_BBs(chain);
  }
}
...
Grow_Chains(BBCHAIN *chains, EDGE *edges, INT n_edges, BB_MAP chain_map)
{
  ...
  for (i = 0; i < n_edges; ++i) {
     ...
     * another, then we can combine the chains if:
     *
     * 1. there is not another chain, X, that must be layed out after
     * 'pchain' and before 'schain', i.e. there is not pchain ->
     * ... -> X -> ... -> schain. We can't combine 'pchain' and
     * 'schain' in this case since that would create an impossible
     * requirement that chain X be both before and after the new
     * chain.
     *
     * 2. 'pchain' must not be required to be layed out after
     * 'schain'. */
    if (pchain != schain && pchain->tail == pred && schain->head == succ) {
      ...
      if ((Chain_Dependence_Distance(pchain, schain) > 1) ||
	  (Chain_Dependence_Distance(schain, pchain) != 0))
      {
	if (CFLOW_Trace_Freq_Order)
	  fprintf(TFile, "Ordering dependence prevents combination of chain ... BB:%d -> BB:%d ...\n",
		  BB_id(pchain->tail), BB_id(schain->head));

	continue;
      }

      BOOL schain_is_near_chain=FALSE;
      if (pchain->zcl_chain && schain->zcl_chain &&
	  pchain->zcl_chain != schain->zcl_chain) {

        // cannot merge chains which are near chains of different zcl chains
	continue;

      } else if (pchain->zcl_chain!=NULL && schain->zcl_chain==NULL) {

        // cannot append zcl chain to a near chain
	continue;

      } else if (pchain->zcl_chain==NULL && schain->zcl_chain!=NULL) {

	if (schain->zcl_chain!=pchain)

          // cannot append near chain to a different zcl chain
	  continue;
	else
	  schain_is_near_chain=TRUE;

      }

      bool target_in_range = true;
      if (total_size - Chain_Size(pchain) > branch_range_for_freq_order)
	target_in_range = false;	// this can be refined

      // if the merged chain is too big such that a branch from inside
      // the zcl of pchain will be out of range, append the near chains now
      if (!target_in_range && pchain->first_near &&
	  Chain_Size(pchain)+Chain_Size(schain) > branch_range_for_freq_order) {

	Append_Near_Chains(chains, pchain, chain_map);

	if (schain_is_near_chain) {
	  // schain has been merged in Append_Near_Chains()
	  continue;
	}
	...
	pred = pchain->tail;
      ...
      /* Special-check if-then-else-endif pattern to make sure the target
       * is within the branch range.  This only handle a simple pattern in
       * which both then and else branches have a single BB (see pr11156)
       */
      if (!target_in_range && BBINFO_kind(pred)==BBKIND_LOGIF) {
        BB* succ0 = BBINFO_succ_bb(pred, 0);
        BB* succ1 = BBINFO_succ_bb(pred, 1);
        UINT succ0_size = BB_size(succ0);
        UINT succ1_size = BB_size(succ1);

        if (BBINFO_kind(succ0)==BBKIND_GOTO &&
            BBINFO_kind(succ1)==BBKIND_GOTO &&
            BBINFO_succ_bb(succ0,0)!= NULL &&
            BBINFO_succ_bb(succ0,0)==BBINFO_succ_bb(succ1,0)) {
          if ( ((succ == succ0) && 
                (succ0_size > branch_range_for_freq_order) && 
                (succ1_size <= branch_range_for_freq_order)) ||
               ((succ == succ1) && 
                (succ1_size > branch_range_for_freq_order) && 
                (succ0_size <= branch_range_for_freq_order)) )
            continue;
      ...
      /* special check for if-then-endif pattern to favor in-lined then block
       * if the out-line penalty is too high
       */
      bool ignore = false;
      if (!target_in_range && BBINFO_kind(pred)==BBKIND_LOGIF) {
        BB* succ0 = BBINFO_succ_bb(pred, 0);
        BB* succ1 = BBINFO_succ_bb(pred, 1);
	INT side_succ_id = -1;
	BBCHAIN* side_succ_chain = NULL;

	if (succ0==succ && BBINFO_nsuccs(succ1)==1) {
	  while (BBINFO_kind(succ1)==BBKIND_CALL &&
		 BBINFO_nsuccs(succ1)!= 0 &&
		 BBINFO_succ_bb(succ1,0)!=succ)
	    succ1 = BBINFO_succ_bb(succ1, 0);
	  if (BBINFO_nsuccs(succ1)>0 && BBINFO_succ_bb(succ1,0)==succ) {
	    side_succ_id = 1;
	    side_succ_chain = BB_Chain(chain_map, succ1);
	  }
	} else if (succ1==succ && BBINFO_nsuccs(succ0)==1) {
	  while (BBINFO_kind(succ0)==BBKIND_CALL &&
		 BBINFO_nsuccs(succ0)!= 0 &&
		 BBINFO_succ_bb(succ0,0)!=succ)
	    succ0 = BBINFO_succ_bb(succ0, 0);
	  if (BBINFO_nsuccs(succ0)>0 && BBINFO_succ_bb(succ0,0)==succ) {
	    side_succ_id = 0;
	    side_succ_chain = BB_Chain(chain_map, succ0);
	  }
	}

	if (side_succ_chain && side_succ_id!= -1) {
	  BB* tail = schain->tail;
	  INT nsuccs = BBINFO_nsuccs(tail);
	  if (nsuccs) {
	    INT remaining_succ = -1;
	    INT num_remaining_succ = 0;
            for (INT j = 0; j < nsuccs; j++) {
	      BB* succ_j = BBINFO_succ_bb(tail, j);
	      BBCHAIN* succ_j_chain = BB_Chain(chain_map, succ_j);

	      // ignore loop back edge
	      if (BB_loop_head_bb(tail)==succ_j)
		continue;
	    ...
	    if (num_remaining_succ>1) {
	      // more than 2 successors of endif block are not layed out so conservatively
	      // force in-line of the then block by ignoring the skip path (current edge)
	      ignore = true;
	    } else if (num_remaining_succ==1) {
	      double fall_through_weight =
		      BB_freq(pred)*BBINFO_succ_prob(pred, 1 - side_succ_id);
	      double side_weight = BB_freq(pred)*BBINFO_succ_prob(pred, side_succ_id);
	      double succ_succ_weight = BB_freq(tail)* BBINFO_succ_prob(tail, remaining_succ);
	      double in_line_cost =
		      fall_through_weight*(1.0f+taken_branch_cost) +
		      side_weight + succ_succ_weight;
	      double out_line_cost =
		      fall_through_weight +
		      (side_weight*2.0f+succ_succ_weight)*(1.0f+taken_branch_cost);
	      if (in_line_cost < out_line_cost)
		ignore = true;
	    } else
	      ignore = true;
	  }
	...
	continue;
      ...
      if (CFLOW_Trace_Freq_Order) {
	#pragma mips_frequency_hint NEVER
	fprintf(TFile, "  merging BB:%d", BB_id(pred));
	if (pchain->head!=pchain->tail) {
	  fprintf(TFile, " in ");
	  Print_Chain_BBs(pchain);
	}
	fprintf(TFile, "    with BB:%d", BB_id(succ));
	if (schain->head==schain->tail)
	  fprintf(TFile, "\n");
	else {
	  fprintf(TFile, " in ");
	  Print_Chain_BBs(schain);
	}
      }
      ...
      Move_Chain_Dependencies(pchain, schain);

      pchain->has_before_bb |= schain->has_before_bb;
      pchain->has_after_bb |= schain->has_after_bb;
      ...
      for (BBCHAIN_LIST *nears = schain->first_near;
        nears; nears = BBCHAIN_LIST_rest(nears)) {
	  BBCHAIN *Xnear = BBCHAIN_LIST_first(nears);
	  schain->first_near = BBCHAIN_LIST_Delete(Xnear, schain->first_near);
	  tmp_chain_list =
		  BBCHAIN_LIST_Push(Xnear, tmp_chain_list, &MEM_pu_pool);
      }
      for (BBCHAIN_LIST *nears = tmp_chain_list;
        nears; nears = BBCHAIN_LIST_rest(nears)) {
	  BBCHAIN *Xnear = BBCHAIN_LIST_first(nears);
	  tmp_chain_list = BBCHAIN_LIST_Delete(Xnear, tmp_chain_list);
	  pchain->first_near =
		  BBCHAIN_LIST_Push(Xnear, pchain->first_near, &MEM_pu_pool);
	  Xnear->zcl_chain = pchain;
      }
	...
	if (bb == tail) break;
      ...
      // if schain is one of the near chain of a zcl chain, take it out of the
      // near chain list since the pchain now contains schain and it is on the
      // same list
      if (schain->zcl_chain)
	schain->zcl_chain->first_near =
		BBCHAIN_LIST_Delete(schain, schain->zcl_chain->first_near);
  ...
  // append near chains for all chains if they are not appended already
  for (BBCHAIN *chain = chains; chain; chain = chain->next) {
      if (chain->first_near)
	Append_Near_Chains(chains, chain, chain_map);
  }
 ...
 */
static void
Combine_Chains(BBCHAIN *chains, BB_MAP chain_map)
{
  BBCHAIN *chain;
  BB *tail = NULL;
  BB *head = chains->head;
  for (chain = chains; chain; chain = chain->next) {
    ...
    /* Update the BB-to-chain mapping for the BBs we're moving.  */
    for (BB *bb = chain->head; ; bb = BB_next(bb)) {
      BB_MAP_Set(chain_map, bb, chains);
      if (bb == chain->tail) break;
    }
  ...
  for (chain = chains->next; chain; ) {
    BBCHAIN* next_chain = chain->next;
    chains = Remove_Chain(chains, chain);
    chain = next_chain;
  }
...
/* ====================================================================
 *
 * Transform_If_Then_Else
 *
 * Find and move the hot branches so it will fall-through to endif
 * in order to remove a jump in hot path
 * if the endif is later duplicated for both branch, also move the
 * hot branches back to be immediate successors of the if block
 *
 * ====================================================================
 */
static bool
Transform_If_Then_Else()
{
  ...
  BB *bb = NULL;
  for (bb = REGION_First_BB; bb; bb = BB_next(bb)) {
    if (BBINFO_kind(bb)==BBKIND_LOGIF) {
      BB* succ0 = BBINFO_succ_bb(bb, 0);
      BB* succ1 = BBINFO_succ_bb(bb, 1);
      if (succ0 && succ1 &&
	  BBINFO_kind(succ0)==BBKIND_GOTO &&
	  BBINFO_kind(succ1)==BBKIND_GOTO &&
	  BBINFO_succ_bb(succ0,0)!= NULL &&
	  BBINFO_succ_bb(succ0,0)==BBINFO_succ_bb(succ1,0)) {

	  BB *endif = BBINFO_succ_bb(succ0,0);

	  double freq0 = BB_freq(bb) * BBINFO_succ_prob(bb, 0);
	  double freq1 = BB_freq(bb) * BBINFO_succ_prob(bb, 1);

	  if (freq0>freq1 && BB_next(bb)==succ0 &&
	      BB_next(succ0)==succ1 && BB_next(succ1)==endif) {
	    Chain_BBs(bb, succ1);
	    Chain_BBs(succ1, succ0);
	    Chain_BBs(succ0, endif);
	    change = true;
	  } else if (freq0<freq1 && BB_next(bb)==succ1 &&
	      BB_next(succ1)==succ0 && BB_next(succ0)==endif) {
	    Chain_BBs(bb, succ0);
	    Chain_BBs(succ0, succ1);
	    Chain_BBs(succ1, endif);
	    change = true;
	  }
      } else if (succ0 && succ1 &&
	  ((BBINFO_kind(succ0)==BBKIND_GOTO &&
	    BBINFO_succ_bb(succ0,0)!= BB_next(succ0)) ||
	   BBINFO_kind(succ0)==BBKIND_RETURN) &&
	  ((BBINFO_kind(succ1)==BBKIND_GOTO &&
	    BBINFO_succ_bb(succ1,0)!= BB_next(succ1)) ||
	   BBINFO_kind(succ1)==BBKIND_RETURN)) {
	  ...
	  if (freq0<freq1 && BB_next(bb)==succ0 &&
	      BB_next(succ0)==succ1) {
	    Chain_BBs(bb, succ1);
	    Chain_BBs(succ0, BB_next(succ1));
	    Chain_BBs(succ1, succ0);
	    change = true;
	  } else if (freq0>freq1 && BB_next(bb)==succ1 &&
	      BB_next(succ1)==succ0) {
	    Chain_BBs(bb, succ0);
	    Chain_BBs(succ1, BB_next(succ0));
	    Chain_BBs(succ0, succ1);
	    change = true;
	  }
      }
...
Freq_Order_Blocks(void)
{
  ...
  if (TI_ISA_Has_Wide_Branches()) {
     branch_range_for_freq_order = TI_Wide_Branch_MinRange(); 
  }
...
// 11-sep-97 re-enable for 7.3 -- it seems to work now
  ...
  /* Collect pairs of BBs/chains that must be in order (though not
     necessarily sequential) in the final layout. */
  Pair_Chains(chains, chain_map);
   ...
   */
  Combine_Chains(chains, chain_map);
 ...
 */
static INT callee_saves[TI_ISA_REGCLASS_MAX + 1];
...
#ifdef TARG_XTENSA
  /* For xtensa's windowed abi, there are never any callee saves
     restores for the integer registers, and currently the calling
     standard for all the others is caller-saved. */
  if (TI_ABI_Value() != ABI_PROPERTIES_ABI_windowed)
#endif
  for (bb = REGION_First_BB; bb; bb = BB_next(bb)) {
    INT tn_count[TI_ISA_REGCLASS_MAX + 1];
    TN *tn;
    GTN_SET *live_in;
    GTN_SET *live_out;
    GTN_SET *need_reg;
...
Estimate_BB_Length(BB *bb)
{
  ...
  if (!CG_localize_tns && (BB_exit(bb) || BB_entry(bb))) {
    INT callees_needed[TI_ISA_REGCLASS_MAX + 1];
    OP *op;
    ...
    FOR_ALL_BB_OPs(bb, op) {
	  ...
	  && !TN_is_save_reg(tn = OP_opnd(op,TI_ISA_Copy_Operand(OP_code(op), TRUE)))) continue;
...
CFLOW_Layout (void)
{
  const char *phase_name = "CFLOW_layout";
  const char *prev_phase;

  if (!CFLOW_Enable || (CFLOW_Enable_Layout <= 0)) return;

#if 0
  int pu_num = Current_PU_Count();
  if ( pu_num <  CG_tmp_skip_before ||
       pu_num >  CG_tmp_skip_after  ||
       pu_num == CG_tmp_skip_equal ) {
     printf("   <<CFLOW_Layout Skip>>: skip PU %s(%d) for layout optimization\n",
             Cur_PU_Name, pu_num);
     return;
  }
#endif
  ...
  /* Find the BBs that hint pragmas indicate are never executed.  */
  never_bbs = FREQ_Find_Never_BBs(&MEM_local_nz_pool);
  if (CFLOW_Trace_Layout) {
    #pragma mips_frequency_hint NEVER
    fprintf(TFile, "\n<CFLOW_Layout> BBs 'never' executed as infered by hint pragmas: ");
    BB_SET_Print(never_bbs, TFile);
    fprintf(TFile, "\n");
  }
...
// the problem uncovered by pv661478.
  ...
  if (current_flags & CFLOW_BRANCH) {
    ...
    change = Optimize_Branches(current_flags & CFLOW_IN_CGPREP);
    if (CFLOW_Trace_Branch) {
      #pragma mips_frequency_hint NEVER
      if (change) {
  ...
  /* Enhance fall-through for if-then-else-endif pattern
   */
  flow_change |= Transform_If_Then_Else();
...
CFLOW_Initialize(void)
{
  ...
  if (Get_Trace(TP_FLOWOPT, 0xffffffff)) {
    ...
    CFLOW_Trace_Layout = Get_Trace(TP_FLOWOPT, TRACE_LAYOUT);
...
#ifdef TARG_XTENSA
  if (CFLOW_Enable_Layout > 0) {
    CFLOW_Enable_Reorder = FALSE;
    CFLOW_Enable_Freq_Order = FALSE;
  }
#endif
