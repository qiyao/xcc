 ...
 *  $Revision: 1.277 $
 *  $Date: 2000/09/28 23:25:39 $
 *  $Author: srinivas $
 *  $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/cg_dep_graph.cxx,v $
 *
 *  Description:
 *  ============
 *
...
// a positive delay.
//
static const struct dep_info_t *dep_info[CG_DEP_NUM_KIND];
static bool dep_info_initialized = false;
...
// at run-time.
//
static const struct dep_info_t dep_info_data[] = {
...
// Structure to hold list of OP_LISTs
struct op_list_list {
  OP_LIST *op_list;
  op_list_list *next;

  op_list_list (OP_LIST *ol =NULL, op_list_list *n =NULL) :
    op_list(ol), next(n) { }
};
...
BOOL CG_DEP_Verify_Mem_Deps = TRUE;		/* exported */
#ifdef TARG_IA64
...

{
  ...
  if (OP_cond((OP *) xfer_op)) {
	...
	if (TN_is_register(result_tn)) {
	  ISA_REGCLASS result_cl = TN_register_class (result_tn);
...
#define Set_ARC_omega_save(arc, val) ((arc)->omega_save = (val))

#define ARC_omega_save(arc) ((arc)->omega_save+0)
...
#define Set_ARC_is_definite(arc, val) {				\
  ARC *_arc = arc;						\
  _arc->kind_def &= ~0x0100;				\
  _arc->kind_def |= (val != 0) << 8;			\
}
...
ARC_LIST*
ARC_LIST_Find_Pred(ARC_LIST *list, OP* pred)
{
  for (; list; list = ARC_LIST_rest(list)) {
    ARC *arc = ARC_LIST_first(list);
    if (ARC_pred(arc) == pred)
      return list;
  }
  return NULL;
}
...
ARC_LIST*
ARC_LIST_Find_Succ(ARC_LIST *list, OP* succ)
{
  for (; list; list = ARC_LIST_rest(list)) {
    ARC *arc = ARC_LIST_first(list);
    if (ARC_succ(arc) == succ)
      return list;
  }
  return NULL;
}
...
REGISTER defop_get_reg_for_tn(TN *tn);

BOOL
OP_has_subset_predicate(const void *value1, const void *value2)
{
#ifdef TARG_XTENSA
  /* For each wide register defined by 'op2', check to make sure 'op1'
     defines all those registers as well. If not, return FALSE to
     indicate that at least part of the definition of 'op2' reaches
     past 'op1'. Note that we don't have to be exact here (in
     particular we don't recognize when multiple defines in 'op1' mask
     a single define in 'op2') because this is only being used to
     prune redundant dependencies. */
  OP *op1 = (OP *)value1;
  OP *op2 = (OP *)value2;
  for (INT r_idx = 0; r_idx < OP_results(op2); ++r_idx) {
    TN *result_tn = OP_result(op2, r_idx);
    REGISTER reg = defop_get_reg_for_tn(result_tn);
    if (reg != REGISTER_UNDEFINED) {
      INT width = TN_Register_Width(result_tn);
      if (width > 1)
      {
	BOOL masked = FALSE;
	
	for (INT m_idx = 0; m_idx < OP_results(op1); ++m_idx) {
	  TN *mask_tn = OP_result(op1, m_idx);
	  REGISTER mask_reg = defop_get_reg_for_tn(mask_tn);
	  if (mask_reg == REGISTER_UNDEFINED) {
	    /* Non-allocated result in 'op1' can't mask allocated
               result in 'op2'. */
	    continue;
	  }
	  
	  INT mask_width = TN_Register_Width(mask_tn);
	  if ((mask_reg <= reg) &&
	      ((mask_reg + mask_width) >= (reg + width)))
	  {
	    masked = TRUE;
	    break;
	  }
...
static TN_LIST *same_reg[REGISTER_MAX+1][TI_ISA_REGCLASS_MAX+1];
...
inline void add_reg_assignment(TN *tn)
{
  ...
  ISA_REGCLASS rclass = TN_register_class(tn);
  TN_LIST *tns;
...
static ARC *create_arc(void)
{
  ...
  if (arc) {
    /* Use first element from free list. */
    Is_True(arc->kind_def == 0xff, ("non-deleted arc on free list"));
#ifdef Is_True_On
    arc->kind_def = 0;
    arc->opnd = 0xffff;
#endif
				 ...
				 UINT8 opnd, BOOL is_definite)
{
  ...
  Set_ARC_omega_save(arc, 0xff);
  Set_ARC_latency(arc, latency);
  Set_ARC_is_dotted(arc, FALSE);
  Set_ARC_is_marked(arc, FALSE);
...
inline ARC *new_arc(CG_DEP_KIND kind, OP *pred, OP *succ, UINT8 omega,
		    UINT8 opnd, BOOL is_definite)
{
  INT16 latency = (kind == CG_DEP_REGOUT ? 1 :
		   (kind == CG_DEP_REGANTI ? 0 :
		      CG_DEP_Latency(pred, succ, kind, opnd)));
...
inline void delete_arc(ARC *arc)
{
  ...
  Is_True(arc->kind_def != 0xff, ("deleting already-deleted arc"));
#ifdef Is_True_On
...
// ---------------------------------------------------------------------
//
inline INT16 get_cycle(TOP opcode, INT16 ckind, UINT8 res, UINT8 opnd)
{
  switch ( ckind ) {
  case CYC_LOAD:
  ...
  case CYC_WRITE:
    return TI_LATENCY_Result_Available_Cycle(opcode, res);
  }
...
//
INT16 
CG_DEP_Oper_Latency(TOP pred_oper, TOP succ_oper, CG_DEP_KIND kind,
		    UINT8 res, UINT8 opnd)
{
 
  if (!dep_info_initialized) {
  ...
  cyc_pred = get_cycle(pred_oper, DEP_INFO_tail(kind), res, opnd);

  /* Get the referenced succ cycle: */
  cyc_succ = get_cycle(succ_oper, DEP_INFO_head(kind), res, opnd);
...
#ifdef TARG_XTENSA
  /* register input latencies must be greater than 0 */
  if (latency <= 0 && kind == CG_DEP_REGIN)
    latency = 1;
#endif
...
CG_DEP_Latency(OP *pred, OP *succ, CG_DEP_KIND kind, UINT8 opnd)
{
...
#ifdef TARG_XTENSA
  /* If 'kind' is CG_DEP_REGIN, then try to find the result position
     in 'pred' for the operand specified by 'opnd'. If we can't find
     it, just use a result position of 0; which duplicates the
     behavior before this change. */

  if ((kind == CG_DEP_REGIN) && (opnd < OP_opnds(succ)))
  {
    TN *tn = OP_opnd(succ, opnd);
    for (UINT i = 0; i < OP_results(pred); i++)
    {
      if (OP_result(pred, i) == tn)
      {
	res = i;
	break;
      }
  ...
  INT16 latency = CG_DEP_Oper_Latency(popcode, sopcode, kind, res, opnd);
 ...
 *
 * op_list_list defop_for_op(OP *op, UINT8 opnd, BOOL is_result)
 *   Return list of OP_LISTs registered with defop_set (if any) that
 *   defines the operand or result specified.  If the TN is dedicated
 *   or <include_ assigned_registers> is true and the TN has a
 *   register assignment, we compare the register assignments;
 *   otherwise we compare the TNs.  <is_result> is a generic variable
 *   which is set to TRUE for result.
 *
...
OP_LIST *defop_by_reg[TI_ISA_REGCLASS_MAX+1][REGISTER_MAX+1];
...
inline void defop_set(OP *op)
{
  ...
  for (i = 0; i < OP_results(op); ++i) {
    ...
    REGISTER reg = defop_get_reg_for_tn(result_tn);
    if (reg != REGISTER_UNDEFINED) {
      /* Show as defined 'reg' and adjacent registers that are also
         defined by a write to 'reg'. */
      INT width = TN_Register_Width(result_tn);
      FmtAssert(width > 0, ("unexpected register width, %d\n", width));
      
      for (INT r = reg + width - 1; r >= reg; r--) 
  ...
  if (OP_code(op) == TOP_asm) {
    ...
    FOR_ALL_ISA_REGCLASS(rc) {
      REGISTER_SET clobber = ASM_OP_clobber_set(asm_info)[rc];
      REGISTER reg;
      FOR_ALL_REGISTER_SET_members(clobber,reg) {
        defop_by_reg[rc][reg] = 
          OP_LIST_Push(op, defop_by_reg[rc][reg], &MEM_pu_pool);
      }
    }
  }
}
...
// See above for specification. 
inline op_list_list defop_for_op(OP *op, UINT8 res, BOOL is_result)
{
  ...
  if (is_result || TN_is_register(tn)) {
    if (TN_is_true_pred(tn)) {
      ...
      if (reg == REGISTER_UNDEFINED)
	return op_list_list((OP_LIST *)CG_DEP_Get_Defs(tn, defop_by_tn), NULL);
      
      /* Collect the OP_LISTs for 'reg' and adjacent registers that
         are also accessed by a reference to 'reg'. */
      INT width = TN_Register_Width(tn);
      FmtAssert(width > 0, ("unexpected register width, %d\n", width));

      op_list_list oll(defop_by_reg[TN_register_class(tn)][reg], NULL);
      for (INT r = reg + 1; r < reg + width; r++)
	oll.next = CXX_NEW(op_list_list(defop_by_reg[TN_register_class(tn)][r], oll.next),
			   &MEM_pu_pool);
      return oll;
    }
  } else {
...
inline SAME_ADDR_RESULT analyze_overlap(OP *memop1, TN *base1, TN *offset1,
					OP *memop2, TN *base2, TN *offset2,
					INT64 diff)
/* -----------------------------------------------------------------------
 ...
 */
{
  ...
  /* Check for target specific aliasing... */
  if (base1 && base2 && TN_is_symbol(base1) && TN_is_symbol(base2)) {
    FmtAssert((!offset1 || TN_has_value(offset1)) &&
	      (!offset2 || TN_has_value(offset2)),
	      ("expecting offset tn to be immediate"));

    if (Target_Analyze_Alias(TN_var(base1),
			     TN_offset(base1) +
			       ((offset1) ? TN_value(offset1) : 0),
			     size1,
			     TN_var(base2),
			     TN_offset(base2) +
			       ((offset2) ? TN_value(offset2) : 0),
			     size2))
      return OVERLAPPING;
  }
 ...
 */
{
  ...
  Is_True(base1 && base2 && TNs_Are_Equivalent(base1, base2), 
  ...
  if (offset1 && offset2 && TN_has_value(offset1) && TN_has_value(offset2)) {
    ...
    switch (analyze_overlap(memop1, base1, offset1, memop2, base2, offset2, diff)) {
    case IDENTICAL:
 ...
 */
{
  ...
  if (int0_tn==NULL)
    int0_tn = Gen_Literal_TN(0, 4);

  if (OP_unknown_addr(pred_op) || OP_unknown_addr(succ_op))
    return DONT_KNOW;
  ...
  /* for unaligned mem ops, if they are not DISTINCT then DONT_KNOW */
  bool has_unaligned_mem = OP_unalign_mem(pred_op) || OP_unalign_mem(succ_op);

  if (addr_subtract(pred_op, succ_op, pred_base, succ_base, &diff0) &&
      addr_subtract(pred_op, succ_op, pred_offset, succ_offset, &diff1)) {

    SAME_ADDR_RESULT result = analyze_overlap(pred_op, pred_base, pred_offset,
			   succ_op, succ_base, succ_offset, diff0 + diff1);

    if (has_unaligned_mem && result != DISTINCT)
      return DONT_KNOW;
    else
      return result;
  }
 ...
 */
{
  ...
  if (pred_op == NULL && succ_op == NULL) {
    ...
    if (repeat_count > 2)
      DevWarn("verify_mem: last warning reoccurred %" LLD_FMT " times "
  ...
  if (repeat_count > 0 &&
    ...
    if (repeat_count > 2)
      DevWarn("verify_mem: last warning reoccurred %" LLD_FMT " times", repeat_count);
    last_result = result;
  ...
  if (pred_op) Print_OP_No_SrcLine(pred_op);
  if (succ_op) Print_OP_No_SrcLine(succ_op);
#endif
  DevWarn("verify_mem: CG addr analysis says %s but %s says %s ALIASED "
	  "for OPs %d (WN=0x%x%s%s) and %d (WN=0x%x%s%s) "
 ...
 */
{
  ...
  char *info_src = "";
  UINT8 min_omega = 0;
  BOOL memread = OP_load(pred_op) && !OP_store(pred_op) &&
		 OP_load(succ_op) && !OP_store(succ_op);
  ...
  *definite = FALSE;
  ...
  /* In cyclic graph,
   * stream put (a store) does not alias with later load/store/stream ops
   * stream get (a load) does not alias with earlier store/stream ops
   */
  if (cyclic && OP_stream_put(pred_op) || OP_stream_get(succ_op))
      return FALSE;
  if (cyclic && OP_stream_op(pred_op) && OP_stream_op(succ_op))
      return FALSE;
  ...
  if (pred_wn == NULL || succ_wn == NULL) {
	...
	/* filter out spills that takes 2 stores to the same spill_st but
	 * to non-overlapping locations
	 */
	if (pred_base && pred_base==succ_base &&
	    CG_DEP_Address_Analyze(pred_op, succ_op)==DISTINCT)
			       ...
			       BOOL Ignore_TN_Dep) 
{
  ...
  if (call_op) {
    ...
    for (i = 0; i < OP_results(cur_op); i++) {
 	...
 	ISA_REGCLASS rclass = TN_register_class (result);
	...
	ISA_REGCLASS opnd_cl = TN_register_class (opnd_tn);
...
Add_MISC_Arcs(BB* bb)
{
  ...
  FOR_ALL_BB_OPs(bb, op) {
	...
	// Since, we do not know the exact omega values, since these are
	// not real register dependences, set the omega value conservative
	// to 1.

	if (CGTARG_Dependence_Required(prev_op, op)) {
	  ARC* arc = new_arc_with_latency(
			  CG_DEP_MISC, prev_op, op, 0, 1, 0, FALSE);

	  // set latency to extw_pseudo or memw_pseudo
	  // to 1 so the path from any
	  // predecessor of extw_pseudo or memw_pseudo
	  // to any successor has total latency 1
	  if (OP_pseudo(op)) {
	    Set_ARC_latency(arc, 1);
	  } else if (OP_store(prev_op) &&
		     (OP_code(op)==TOP_memw || OP_code(op)==TOP_extw)) {
	    // add latency from store tp memw/extw (pr13491)
	    Set_ARC_latency(arc, xt_w_stage+1);
	  }
	}
      }
    }
	...
	ARC* arc = new_arc_with_latency(CG_DEP_MISC, op, next_op, 0, 0, 0, FALSE);
	// set latency to extw_pseudo or memw_pseudo
	// to 1 so the path from any
	// predecessor of extw_pseudo or memw_pseudo
	// to any successor has total latency 1
	if (OP_pseudo(next_op)) {
	  Set_ARC_latency(arc, 1);
	} else if (OP_store(op) &&
		   (OP_code(next_op)==TOP_memw ||
		    OP_code(next_op)==TOP_extw)) {
	  // add latency from store tp memw/extw (pr13491)
	  Set_ARC_latency(arc, xt_w_stage+1);
	}
      }
  ...
  // for cyclic graph, we augment it with arcs due to barriers which
  // is represented as pseudo ops (e.g., extw_pseudo)
  // this is needed since pseudo ops are not scheduled by SWP and the
  // arcs are not used in MinDist computation
  if (cyclic) {
    FOR_ALL_BB_OPs(bb, op) {
      if (OP_pseudo(op)) {
	for (ARC_LIST* pred_arcs = OP_preds(op); pred_arcs;
			pred_arcs = ARC_LIST_rest(pred_arcs)) {
	  ARC* pred_arc = ARC_LIST_first(pred_arcs);
	  if (ARC_kind(pred_arc)==CG_DEP_MISC && !OP_pseudo(ARC_pred(pred_arc))) {
	    OP* pred_op = ARC_pred(pred_arc);
	    INT16 pred_latency = ARC_latency(pred_arc);
	    UINT16 pred_omega = ARC_omega(pred_arc);
	    for (ARC_LIST* succ_arcs = OP_succs(op); succ_arcs;
			succ_arcs = ARC_LIST_rest(succ_arcs)) {
	      ARC* succ_arc = ARC_LIST_first(succ_arcs);
	      if (ARC_kind(succ_arc)==CG_DEP_MISC && !OP_pseudo(ARC_succ(succ_arc))) {
		OP* succ_op = ARC_succ(succ_arc);
		INT16 succ_latency = ARC_latency(succ_arc);
		UINT16 succ_omega = ARC_omega(succ_arc);
		new_arc_with_latency(CG_DEP_MISC, pred_op, succ_op,
				pred_latency+succ_latency,
				pred_omega+succ_omega, 0, FALSE);
	      }
	    }
	  }
	}
 ...
 */
{
  ...
  if (OP_unalign_mem(pred) || OP_unalign_mem(succ) ||
      OP_unknown_addr(pred) || OP_unknown_addr(succ) ||
 ...
 */
{
  ...
  for (succ_idx = first_poss_succ_idx; succ_idx < num_mem_ops; succ_idx++) {
    ...
    ARC *arc;
    INT16 latency;
    CG_DEP_KIND kind = OP_store(op) ?
      (OP_store(succ) ? CG_DEP_MEMOUT : CG_DEP_MEMIN) :
      (OP_store(succ) ? CG_DEP_MEMANTI : CG_DEP_MEMREAD);
    ...
    if (get_mem_dep(op, succ, &definite, cyclic ? &omega : NULL)) {
	 ...
	 && TI_PROC_Property_Set(PROP_is_out_of_order)
	 && !definite) ? 
...
static STACKREF_KIND Memory_OP_References_Stack(OP *op)
{
...
#ifdef TARG_XTENSA
    /* Xtensa can have l32r's of constants. These don't have a symbol
       so we don't want to call WN_st, and we know they don't access
       the stack. */
    if (WN_operator(home) == OPR_INTCONST)
      return STACKREF_NO;

    /* for re-materializable TIE intrinsic op, they don't access the stack */
    if (WN_operator(home) == OPR_INTRINSIC_OP)
      return STACKREF_NO;

    /* Rematerializable INTCONST-to-TIE converts -- no access to the stack */
    if (WN_operator(home) == OPR_CVT &&
	MTYPE_is_tie(WN_rtype(home)) &&
	MTYPE_is_integral(WN_desc(home)) &&
	WN_operator(WN_kid0(home)) == OPR_INTCONST)
      return STACKREF_NO;
#endif
  ...
  if (wn) {
	...
	break;
      case OPR_CVT:
	if (MTYPE_is_tie(WN_rtype(wn)) || MTYPE_is_tie(WN_desc(wn))) {
	  // an iload/istore created from TIE mtor or rtom conversion proto
	  return STACKREF_YES;
	} else {
	...
	for (i = 0; i < num_mem_ops; i++) {
	  if (CG_DEP_Alloca_Aliases(mem_ops[i])) {
	    if (op!=mem_ops[i]) {
	      ...
	      if (cyclic)
	        new_arc(CG_DEP_MISC, op, mem_ops[i], 1, 0, FALSE);
	    }
	  }
	}
      }
...
Add_Forw_REG_Arcs(BB *bb)
{
  ...
  FOR_ALL_BB_OPs(bb, op) {
    ...
    for (i = 0; i < OP_opnds(op); i++) {
      op_list_list defop_lists_first = defop_for_op(op, i, FALSE);
      op_list_list *defop_lists = &defop_lists_first;
      if (has_assigned_reg(OP_opnd(op,i)))
	add_reg_assignment(OP_opnd(op,i));

      OP_LIST *prev_list = NULL;
      while (defop_lists) {
	OP_LIST *defop_list = defop_lists->op_list;
	while (defop_list) {
	  /*
	  ...
	  if (!OP_Shadowed_By_Prev_OPs(defop, prev_list, 
	    ...
	    // #795487: PQS doesn't work with register allocated
	    // code. It uses TN as a handle to query predicate
	    // relations and relies on single-reaching
	    // definitions. This can no longer be TRUE after register
	    // allocation, since a same register can be allocated to
	    // multiple TNs. This is a deficiency of the current PQS
	    // implementation.
	...
	defop_lists = defop_lists->next;
      }
    }

    for (i = 0; i < OP_results(op); i++) {
      op_list_list prev_defop_lists_first = defop_for_op(op, i, TRUE);
      op_list_list *prev_defop_lists = &prev_defop_lists_first;
      if (has_assigned_reg(OP_result(op,i)))
	add_reg_assignment(OP_result(op,i));

      OP_LIST *prev_list = NULL;
      while (prev_defop_lists) {
	OP_LIST *prev_defop_list = prev_defop_lists->op_list;
	...
	prev_defop_lists = prev_defop_lists->next;
      }
    } 
...
// determine if the latency need to be adjusted due to killed
// late def
static bool
need_stall_from_killed_late_def(TN* tn, OP* def_op, OP* use_op) {

  bool short_def = false;
  bool has_killed_late_def = false;
  bool has_read = false;
  for (INT j = 0; j < OP_results(def_op); j++) {
    if (OP_result(def_op, j) == tn &&
	get_cycle(OP_code(def_op), CYC_WRITE, j, 0)<= xt_w_stage) {
      // current def is a short def
      short_def = true;
      break;
    }
  }
  ...
  OP* scan = def_op;
  while (1) {
    if (OP_Defs_TN(scan, tn)) {
      for (INT j = 0; j < OP_results(scan); j++) {
	if (OP_result(scan, j) == tn &&
	    get_cycle(OP_code(scan), CYC_WRITE, j, 0)>xt_w_stage) {
	  // current def is a short def
	  has_killed_late_def = true;
	  break;
	}
      }
    }
    if (OP_Refs_TN(scan, tn)) {
      has_read;
      break;
    }
    if (scan==use_op)
    ...
    scan = OP_prev(scan);
    if (scan==NULL)
      scan = BB_last_op(OP_bb(use_op));
  }
  if (has_killed_late_def && !has_read)
    return true;
...
void Build_Cyclic_Arcs(BB *bb)
{
  ...
  for (INT use_num = 0; use_num < op_vec.size(); use_num++) {
	...
	continue;
      ...
      bool  split_pipeline_read = TI_ISA_Regclass_Has_Split_Pipe(rc);
...
#ifdef TARG_XTENSA
      if (omega<=1) {
	// force any intra-iteration and wrap-around dependences to be
	// non-definite and therefore will be included
	// this is conservative and also takes care of the fact that
	// XTENSA does not have rotating registers
	definite_dep = false;
      }
#endif
	...
	INT i;
	INT most_recent_def = -1;

	for (i = tn_defs.size()-1; i >= 0; i--) {
	  INT def_num = tn_defs[i];
	  if (omega>0 && def_num >= use_num ||
	      omega==0 && def_num < use_num) {
	    if (def_num > most_recent_def)
	      most_recent_def=def_num;
	  }
	}
	...
	for (i = tn_defs.size()-1; i >= 0; i--) {
	  ...
	  OP *def_op = op_vec[def_num];
	  if (!OP_has_disjoint_predicate_cyclic(def_op, op)) {
	    ARC* arc =
	      new_arc(CG_DEP_REGIN, def_op, op, omega, opnd, FALSE);

	    if (split_pipeline_read) {
	      UINT32 new_latency = xt_w_stage + 1;
	      UINT32 old_latency = ARC_latency(arc);
	      if (new_latency > old_latency &&
		  need_stall_from_killed_late_def(tn, def_op, op)) {
		Set_ARC_latency(arc, new_latency);
	      }
	    }
	  }
	  
	  if (OP_has_subset_predicate_cyclic(def_op, op)) 
	    break;
	}
	...
	if (!local_lr) {
	  if (!definite_dep ||
	    ...
	    for (i = tn_defs.size()-1; i >= 0; i--) {
	      ...
	      OP *def_op = op_vec[def_num];
	      ARC* arc =
	        new_arc(CG_DEP_REGIN, def_op, op,
			definite_dep ? omega : 1, opnd, FALSE);
	      if (split_pipeline_read) {
		UINT32 new_latency = xt_w_stage + 1;
		UINT32 old_latency = ARC_latency(arc);
		if (new_latency > old_latency &&
	...
	if (split_pipeline_read) {
	  for (i = tn_defs.size()-1; i >= 0; i--) {
	    ...
	    if (def_num == most_recent_def) continue;

	    OP *def_op = op_vec[def_num];
	    for (UINT j = 0; j < OP_results(def_op); j++) {
	      if (OP_result(def_op, j) == tn &&
		get_cycle(OP_code(def_op), CYC_WRITE, j, 0)> xt_w_stage) {
		// a killed late def
		// has a fake flow dep with latency from W to I stage
	        UINT8 arc_omega = (def_num>=use_num)?1:0;
	        (void)new_arc_with_latency(
		  CG_DEP_REGIN, def_op, op, xt_w_stage + 1,
		  arc_omega, opnd, FALSE);
	        break;
	      }
	    }
	  }
  ...
  for (TN_2_DEFS_VECTOR_MAP::iterator it = tn_map.begin(); it != tn_map.end(); it++) {
	...
	// Ignore same TN used twice in an OP with multiple results
	if (op1 == op2)
	  continue;
...
{
#ifdef TARG_XTENSA
  /* I've changed the proto for CGPREP_Same_Res_Opnd so this code
     needs to be updated to check each result to see if it is same_res
     as some operand. Don't bother with it now though, since this
     routine is not called. */
  FmtAssert(FALSE, ("CG_DEP_Add_Op_Same_Res_Arcs unimplemented"));
#else
  BB *bb = OP_bb(op);
...
bool 
CG_DEP_Replace_SCC_ARC_omega(ARC *arc, UINT8 new_omega)
/* -----------------------------------------------------------------------
 * See "cg_dep_graph.h" for interface specification.
 * -----------------------------------------------------------------------
 */
{
  Is_True(ARC_omega_save(arc)== 0xff, ("Illegal change to omega"));
  if (ARC_omega_save(arc)== 0xff) {
    Set_ARC_omega_save(arc, ARC_omega(arc));
    Set_ARC_omega(arc, new_omega);
    return true;
  }
  return false;
}
...
bool 
CG_DEP_Restore_SCC_ARC_omega(ARC *arc)
/* -----------------------------------------------------------------------
 * See "cg_dep_graph.h" for interface specification.
 * -----------------------------------------------------------------------
 */
{
  Is_True(ARC_omega_save(arc)!= 0xff, ("Illegal change to omega"));
  if (ARC_omega_save(arc)!= 0xff) {
    Set_ARC_omega(arc, ARC_omega_save(arc));
    Set_ARC_omega_save(arc, 0xff);
    return true;
  }
  return false;
...
Update_Entry_For_TN(
    ...
    OP              *cur_op, 
    TN_MAP          vmap, 
    void            *register_ops[TI_ISA_REGCLASS_MAX+1][REGISTER_MAX+1], 
    OP_MAP          omap, 
    BOOL            is_result)
{
  ...
  if (TN_register(vtn) != REGISTER_UNDEFINED) {
    ...
    ISA_REGCLASS rc = TN_register_class(vtn);
			     ...
			     BOOL         trace)
{
  ...
  void *reg_ops[TI_ISA_REGCLASS_MAX+1][REGISTER_MAX+1];
