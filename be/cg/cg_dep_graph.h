 ...
 *
 *  Special routines to temporarily change the omega and later restore it
 *
 *	bool CG_DEP_Replace_SCC_ARC_omega(ARC *arc, UINT8 new_omega)
 *	bool CG_DEP_Restore_SCC_ARC_omega(ARC *arc)
 *
...
#ifdef _KEEP_RCS_ID
static char *cg_dep_graph_rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/cg_dep_graph.h,v $ $Revision: 1.87 $";
#endif /* _KEEP_RCS_ID */
...
typedef struct arc {
  ...
  mUINT8        omega_save;	/* temp save location for omega */
  mUINT16       kind_def;	/* kind is LOW 8 bits,
				 * definite is next bit,
				 * dotted edge is the next bit, which tells
				 * if the edge is not always strict
				 * marked is the next bit */
  mUINT16       opnd;
...
#define ARC_saved_omega(arc)	((arc)->omega_save+0)
#define ARC_kind(arc)		((CG_DEP_KIND)((arc)->kind_def & 0xff))
#define ARC_is_definite(arc)	(((arc)->kind_def >> 8) & 1)
#define ARC_is_dotted(arc)	(((arc)->kind_def >> 9) & 1)
#define ARC_is_marked(arc)	(((arc)->kind_def >> 10) & 1)
#define ARC_opnd(arc)		((arc)->opnd+0)

#define Set_ARC_is_dotted(arc, val) {                           \
  ARC *_arc = arc;                                              \
  _arc->kind_def &= ~0x0200;                               \
  _arc->kind_def |= (val != 0) << 9;                       \
}

#define Set_ARC_is_marked(arc, val) {                           \
  ARC *_arc = arc;                                              \
  _arc->kind_def &= ~0x0400;                               \
  _arc->kind_def |= (val != 0) << 10;                       \
}
...
inline BOOL ARC_is_misc(ARC *arc)
{
  CG_DEP_KIND kind = ARC_kind(arc);
  return kind == CG_DEP_MISC;
}
...
extern ARC_LIST *ARC_LIST_Find_Pred(ARC_LIST *list, OP* pred);
extern ARC_LIST *ARC_LIST_Find_Succ(ARC_LIST *list, OP* succ);
