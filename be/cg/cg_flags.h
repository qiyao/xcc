 ...
 *
 *  BOOL CG_specialization
 *	Determine if operation specialization should be done
 *
 ...
 *
 *  INT32 CG_CU_skip_after
 *  INT32 CG_CU_skip_before
 *  INT32 CG_CU_skip_equal
 *      When CU_Skip_File_Name isn't NULL, those control which CU 
 *      (Compilation Unit) we skip optimizing, i.e. we set Opt_Level=0
 *      for that CU. CU_Skip_File_Name is set by -Wb,-fc:<counter_file>.
 *      (see be/com/be_util.cxx for detail)
 *
 *
 *  INT32 CG_skip_after
 ...
 *
 *  BOOL CG_unroll_prune_prefetches
 *     Prune prefetches for unrolled non SWP loops.
 *
 ...
 *	code bloat effects.
 *
 *  INT32 CFLOW_Enable_Layout
 *      Enable BB layout (new reordering phase) if it's greater than zero.
 *
 *  INT32 CG_xpres_bb_size
 *      Basic block size when running Xpres
 *
 ...
 *	Coalesce all loop backedges into a single backedge when forming
 *	hyperblocks from an inner loop body (default setting is
 ...
 *
 *  HB_max_instructions
 *	Maximum number of instructions allowable in a hyperblock (default 
 *	setting is architecturally dependent).
 *
 *  HB_max_sched_growth
 ...
 *	Same as above, but applies only if the frequency data for blocks
 *	was created statically.
 *
 ...
 *	Prefer side paths near the bottom of the main path when forming
 *	general candidate regions during hyperblock formation.
 *
 *  HB_min_blocks
 ...
 *
 *  HB_skip
 *	HB skip option (default OFF). The option used to conditionally
 *	skip the formation of selected HBs (used for debugging).
 *
 *  HB_skip_entry_bb
 *	HB triage option (default OFF). The option used to conditionally
 *	skip the formation of a HB whose entry BB matches HB_skip_entry_bb.
 *      Used for triaging (default OFF).
 *
...
extern BOOL CG_init_black_hole_overridden;
extern INT32 CG_maxinss;
extern INT32 CG_maxblocks;
extern BOOL CG_init_black_hole;
extern BOOL GRA_LIVE_Phase_Invoked;
extern BOOL CM_Enable;
extern BOOL CG_generate_bb_range;

extern INT32 CG_tmp_skip_after;
extern INT32 CG_tmp_skip_before;
extern INT32 CG_tmp_skip_equal;
...
extern BOOL CFLOW_opt_after_lra;
...
extern INT32 CFLOW_Enable_Layout;
...
extern BOOL CG_unroll_prune_prefetches;
...
extern BOOL LOCS_Enable_Xblock_Latency;
...
#define ALIGN_BRANCH_TARGETS      0x01
#define ALIGN_ALWAYS_AFTER_JUMP   0x02
#define ALIGN_RELAX_LONG_BRANCHES 0x04
#define ALIGN_ZERO_COST_LOOPS     0x08
#define ALIGN_IGNORE_FREQUENCY    0x10
#define DEFAULT_ALIGN_BRANCH_TARGETS 0

extern INT32 EMIT_Align_Branch_Targets;
...
extern BOOL GRA_new_conflict;

extern BOOL  HB_formation;
extern BOOL  HB_coalesce_backedges;
extern BOOL  HB_static_freq_heuristics;
extern BOOL  HB_static_freq_no_fifty_splits;
...
extern INT   HB_max_instructions;
extern const char* HB_max_sched_growth;
extern const char* HB_max_sched_growth_sf;
...
typedef enum { Alameda_Xtensa, Albany_Xtensa, Barcelona_Xtensa } Xtensa_Arch;

extern Xtensa_Arch earliest_arch;
extern Xtensa_Arch latest_arch;

extern BOOL jons_test_flags[5];
