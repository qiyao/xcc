 ...
 *    void CG_LOOP_Remove_Notations(CG_LOOP& cg_loop, BB *head, BB *tail)
 *
 *
 ...
 *
 *  BOOL Remove_Guard_Branch (BB *prolog)
...
/* ---------------------------------------------------- */
  ...
  mINT16 loh_mii;
  mINT16 scc_index;
  mUINT8 swp_wind_up_section;
  mUINT8 swp_wind_down_section;
...
#define OP_swp_wind_up_section(op) (_CG_LOOP_info(op)->swp_wind_up_section+0)
#define Set_OP_swp_wind_up_section(op, s) \
		(_CG_LOOP_info(op)->swp_wind_up_section = s)

#define OP_swp_wind_down_section(op) (_CG_LOOP_info(op)->swp_wind_down_section+0)
#define Set_OP_swp_wind_down_section(op, s) \
		(_CG_LOOP_info(op)->swp_wind_down_section = s)
...
extern INT32 CG_LOOP_skip_after;
extern INT32 CG_LOOP_skip_before;
extern INT32 CG_LOOP_skip_equal;
extern INT32 CG_LOOP_force_ifc;

extern UINT32 CG_LOOP_max_zcl_bytes;
...
inline TN *CG_LOOP_Max_Trip_Count(LOOP_DESCR *loop)
{
  LOOPINFO *info = LOOP_DESCR_loopinfo(loop);
  return info ? LOOPINFO_max_trip_count_tn(info) : NULL;
}
...
enum CG_LOOP_FLAGS {
  ...
  CG_LOOP_HAS_SAVE = 0x8,  // has a copy of prolog/body/epilog that can be restored
  CG_LOOP_HAS_BEST = 0x10,  // has a copy of the best prolog/body/epilog that can be restored
};
...
private:
  ...
  // the following fields are used to save a copy of the current state
  BOOL        unroll_fully_save;
  INT32       unroll_factor_save;
  INT32       flags_save;
  BB         *prolog_start_save;
  BB         *prolog_end_save;
  BB         *epilog_start_save;
  BB         *epilog_end_save;
  BB         *trip_count_bb_save;  // The BB contains the trip count expr
  BB         *body_save;
  BB	     *remainder_loop_save;

  // the following fields are used to save a copy of the best schedule
  BOOL        unroll_fully_best;
  INT32       unroll_factor_best;
  INT32       flags_best;
  BB         *prolog_start_best;
  BB         *prolog_end_best;
  BB         *epilog_start_best;
  BB         *epilog_end_best;
  BB         *trip_count_bb_best;  // The BB contains the trip count expr
  BB         *body_best;
  BB	     *remainder_loop_best;
  INT32	      super_swp_ii;
  INT32	      super_swp_unroll;
  BOOL	      need_super_swp;
  WN*         swp_schedule_pragma_wn;

  // the following field is used for the loop that contains multireg operand
  ISA_REGSUBCLASS  multireg_sc;
...
public:
  ...
  BOOL Has_save() const { return flags & CG_LOOP_HAS_SAVE; }
  BOOL Has_best() const { return flags & CG_LOOP_HAS_BEST; }
  ...
  BB  *Remainder_loop() const { return remainder_loop; }
  ...
  void Set_remainder_loop(BB* bb)
				  { remainder_loop = bb; }
  INT32 Unroll_factor() const     { return unroll_factor; }
  void Set_unroll_factor(INT32 n) { unroll_factor = n; }

  BOOL  Need_super_swp() const    { return need_super_swp; }
  INT32 Super_swp_ii() const      { return super_swp_ii; }
  INT32 Super_swp_unroll() const  { return super_swp_unroll; }
  WN* SWP_schedule_pragma_wn() const  { return swp_schedule_pragma_wn; }

  void Set_multireg_sc(ISA_REGSUBCLASS v) { multireg_sc = v;}
  ISA_REGSUBCLASS  Multireg_sc()          { return multireg_sc;}
  ...
  void EBO_Before_Unrolling_For_SWP();
  void EBO_After_Unrolling_For_SWP();
  void Print(FILE *fp);
  void Verify();
  void Redo_Prolog_Epilog();
  void LI_Hoisting_BB();        // loop-invariant hoisting for a loop of a single BB

  BOOL Save(BOOL is_best);
  BOOL Restore(BOOL is_best);

  CG_LOOP(LOOP_DESCR *);
  ~CG_LOOP();
};

void CG_LOOP_Remove_Notations(CG_LOOP& loop, BB *head, BB *tail);
...
extern BOOL Perform_SWP(CG_LOOP& cl, SWP_FIXUP_VECTOR& fixup,
			INT best_ii, bool is_doloop);

extern void SWP_Fixup(SWP_FIXUP& fixup);

extern BOOL Remove_Guard_Branch (BB *prolog);
