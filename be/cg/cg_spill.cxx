 ...
 *  Module: cg_spill.c
 *  $Revision: 1.89 $
 *  $Date: 2000/10/12 21:26:42 $
 *  $Author: mpm $
 *  $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/cg_spill.cxx,v $
 *
 *  Description:
 *  ============
 *
...
/* the following are mainly for TIE types and xtbools */
static LOCAL_SPILLS lra_mtype_spills[MTYPE_MAX];
static LOCAL_SPILLS swp_mtype_spills[MTYPE_MAX];
...
CGSPILL_Reset_Local_Spills (void)
{
  ...
  for (TYPE_ID i=MTYPE_XTBOOL; i<=Mtype_Last; i++) {
    LOCAL_SPILLS_Reset(&lra_mtype_spills[i]);
    LOCAL_SPILLS_Reset(&swp_mtype_spills[i]);
  }
}
...
CGSPILL_Initialize_For_PU(void)
{
  ...
  for (TYPE_ID i=MTYPE_XTBOOL; i<=Mtype_Last; i++) {
    slc = &lra_mtype_spills[i];
    LOCAL_SPILLS_mem_type(slc) = MTYPE_To_TY(i);
    LOCAL_SPILLS_free(slc) = NULL;
    LOCAL_SPILLS_used(slc) = NULL;
    slc = &swp_mtype_spills[i];
    LOCAL_SPILLS_mem_type(slc) = MTYPE_To_TY(i);
    LOCAL_SPILLS_free(slc) = NULL;
    LOCAL_SPILLS_used(slc) = NULL;
  }

  Trace_Remat = Get_Trace(TP_TEMP, TRACE_REMAT);
...
{
  Is_True( TN_register_class(tn) != ISA_REGCLASS_UNDEFINED,
...
CGSPILL_Get_TN_Spill_Location (TN *tn, CGSPILL_CLIENT client)
{
  ...
  case CGSPILL_LGRA:
	...
	mem_type = MTYPE_To_TY(mtype);
      else
	mem_type = TN_is_float(tn) || TN_is_fcc_register(tn) ?
...
CGSPILL_OP_Spill_Location (OP *op)
{
  ...
  if (spill_ids)
  {
    ...
    if (!CGSPILL_Is_Spill_Location(mem_loc))
      mem_loc = NULL;
  }
  return mem_loc;

/* the following code looks unnecessary after adding OP_spill_tn(),
   so it is commented out as of 8/2005
 */
#if 0
  if (spill_ids)
  {
    ...
    if (OP_load(op) && (OP_results(op) == 1))
      ...
      INT32 sym_idx = TI_TOP_Find_Operand_Use(OP_code(op), OU_storeval);
      if (sym_idx>=0)
        spill_tn = OP_opnd(op,sym_idx);
      else /* conservatively assume operand 0 is the stored value */
        spill_tn = OP_opnd(op,0);
    }
...
#ifdef TARG_XTENSA
    /* Using the load result tn, or the store value tn, it appears
       that 'op' is a spill. However, we still need to check that the
       symbol accessed by 'op' is a spill location. Otherwise a
       non-spill load/store of the tn will appear as a spill,
       potentially causing incorrect scheduling. */
    if (mem_loc)
    {
      ST *sav_mem_loc = mem_loc;

      /* If the offset TN is not a symbol, then we trust the TN_spill
         'mem_loc' is correct. */
      INT32 sym_idx = TI_TOP_Find_Operand_Use(OP_code(op), OU_offset);
      if (sym_idx >= 0)
      {
	TN *sym = OP_opnd(op, sym_idx);
	if (TN_is_symbol(sym))
	{
	  if (!CGSPILL_Is_Spill_Location(TN_var(sym)) ||
	      (mem_loc != TN_var(sym)))
	    mem_loc = NULL;
	} else
	  mem_loc = NULL;
	...
	mem_loc = NULL;
      ...
      /* We get a lot of false spill identification for localizing
         spill temps, so don't report those. */
#ifdef Is_True_On
      if ((mem_loc == NULL) &&
	  strncmp(ST_name(sav_mem_loc), "lcl_spill_temp", strlen("lcl_spill_temp")))
      {
	if (TFile != stdout) 
	{
	  fprintf(TFile, "TN_spill: ");
	  sav_mem_loc->Print(TFile, FALSE);
	  fprintf(TFile, "op: ");
	  Print_OP_No_SrcLine(op);
	}
	Lmt_DevWarn(3, ("TN_spill symbol differs from symbol accessed in op, assuming non-spill."));
      }
#endif
    }
#endif /* TARG_XTENSA */
		       ...
		       CGSPILL_CLIENT client)
{
  ...
  if (Can_Rematerialize_TN(tn, client))
  {
	...
	*store_cost = *restore_cost;
    ...
    case OPR_LDA: 
      {
	Exp_Tree(result, home, &OPs);
	*restore_cost = OPS_length(&OPs);
      }
      break; 
    case OPR_INTRINSIC_OP: 
      {
	/* this should work for TIE intrinsic only */
	FmtAssert(INTRN_is_tie_intrinsic(WN_intrinsic(home)),
		  ("Cannot re-materialize non-TIE intrinsic function"));
	Exp_Tree(result, home, &OPs);
	*restore_cost = OPS_length(&OPs);
      }
      break; 
    case OPR_CVT:
      {
        // This should work for INTCONST-to-TIE converts only (see opt_etable.cxx).
	FmtAssert(MTYPE_is_tie(WN_rtype(home)) && MTYPE_is_integral(WN_desc(home)) &&
		  WN_operator(WN_kid0(home)) == OPR_INTCONST,
		  ("Cannot rematerialize non-(int-to-TIE) convert"));
	Exp_Tree(result, home, &OPs);
	...
	*restore_cost += 1.0F;
    ...
    case OPR_CONST:
      ...
      *restore_cost = OPS_length(&OPs) + .25F;
      break;
    default:
      FmtAssert(0,("Unexpected rematerializable case"));
    }
  }
  else
  {
    ...
    *restore_cost = CGSPILL_DEFAULT_RESTORE_COST;
    ...
    if (MTYPE_is_tie(mtype)) {
      TIE_MACRO_p loadi_macro = tie_info->mtype_loadi_macro(mtype);
      TIE_MACRO_p storei_macro = tie_info->mtype_storei_macro(mtype);
      *store_cost += ((float)(loadi_macro->num_instructions())-1.0F);
      *restore_cost += ((float)(storei_macro->num_instructions())-1.0F);
    }
  }
			  ...
			  BB *bb)
{
  ...
  if (   Can_Rematerialize_TN(tn, client)
    ...
    case OPR_CONST:
      Exp_OP1 (opcode, tn, Gen_Symbol_TN(WN_st(home),0,0), &new_ops);
      break;
	...
	ErrMsg (EC_Unimplemented,
      ...
      Exp_OP1 (opcode, tn, const_tn, &new_ops);
      break;
     ...
     */
    CGTARG_Load_From_Memory(tn, mem_loc, &new_ops);

    /* set up restore's spill_tn field */
    OP *tmp_op;
    FOR_ALL_OPS_OPs (&new_ops, tmp_op) {
      Set_OP_spill_tn(tmp_op, tn);
    }
  }
...
#pragma mips_frequency_hint NEVER
  ...
  CGTARG_Store_To_Memory(src_tn, mem_loc, &new_ops);

  /* set up store's spill_tn field */
  OP *tmp_op;
  FOR_ALL_OPS_OPs (&new_ops, tmp_op) {
    if (OP_store(tmp_op)) {
      Set_OP_spill_tn(tmp_op, src_tn);
    }
  }
  ...
  if (first_copy_op && GRA_Spill_Around_Save_TN_Copies()) {
    ...
    for (tmp_op = OP_prev(first_copy_op); tmp_op;
	 tmp_op = OP_prev(tmp_op)) {
      if (OP_copy(tmp_op) && TN_is_save_reg(OP_opnd(tmp_op, TI_ISA_Copy_Operand(OP_code(tmp_op), TRUE)))) {
...
OP_Refs_TN_Reg( OP* op, TN* tn, TN** dep_tn )
{
  if ( TN_is_constant(tn) || TN_is_const_reg(tn) )
    ...
    ISA_REGCLASS reg_class = TN_register_class(tn);
    ...
    for ( i = OP_opnds(op) - 1; i >= 0; --i ) {
      ...
      if ( ! (TN_is_constant(opnd) || TN_is_const_reg(opnd)) ) {
        ...
        ISA_REGCLASS opnd_reg_class = TN_register_class(opnd);
