...
//
//  - tn2tn_map is a mapping from an un-allocated body TN into its allocated TN.
//
// ****************************************
...
struct SWP_REG_ASSIGNMENT {
 ...
 // 1st rotating registers (
  ...
  REGISTER_SET non_rotating_reg[TI_ISA_REGCLASS_MAX+1];

#ifndef HAS_ROTATING_REGISTERS
  REGISTER_SET multireg_boolean_reg;
#endif
  ...
  TN *Get_Non_Rotating_Register_Grantable_Local_TN(BB *bb, TN *tn);
  ...
  // return unallocatable register class if allocation failed
  ...
  // Compute the fatpoints of invariants
  // return false if not enough register is available
  // (see cg_swp_allocator.cxx for the definition of this function).
  // this assumes that the Allocate_Loop_Variants() was called first
  bool Invariants_Allocatable(const SWP_OP_vector& op_state);
  ...
  UINT Num_Unused_Non_Rotating_Register(const ISA_REGCLASS rc)
	{ return REGISTER_SET_Size(non_rotating_reg[rc]); }

#ifndef HAS_ROTATING_REGISTERS
  // Set non_rotating_reg[] for ISA_REGCLASS_branch to special-process
  // multireg operands.
  void Init_for_Multireg_oprands(ISA_REGSUBCLASS multireg_sc);
#endif

  SWP_REG_ASSIGNMENT(ISA_REGSUBCLASS multireg_sc);

  // clear all assignment to allow another round of allcoation
  void Clear();
...
enum SWP_RETURN_CODE {
  ...
  SWP_NOT_ATTEMPTED,                 // swp not tried
  SWP_PREP_ONLY,                     // disabled swp due to -SWP:prep_only
  SWP_ASM,                           // disabled swp due to asm statement
  SWP_SIMULATED_OP,                  // disabled swp due to simulated operation
  SWP_MULTIREG_OP,                   // disabled swp due to multi-reg operation
  SWP_PRIVILEGED_OP,                 // disabled swp due to privileged operation
  ...
  TOO_FEW_TRIPS,		     // swp failed: loops fully unrolled
  MOD_SCHED_FAILED,                  // swp failed: failure to schedule
  MOD_NOT_BETTER,                    // swp succeeded with non-unroll one-stage
  				     // schedule
...
extern INT swp_map_tbl_max;

// return index in SWP_OP_vector for 'op'
// return -1 if 'op' does not have a index map usually because it is
// added (as an slot-filling nop) after bundling
inline INT SWP_index(OP *op) { return (OP_map_idx(op)<swp_map_tbl_max)?
				swp_map_tbl[OP_map_idx(op)] : -1; }
...
struct SWP_OP {
  ...
  SWP_OP():op(NULL),placed(false),is_noop(false),scale((double)1.0),cycle(0),
	   trials(0),modulo_cycle(0),slot(0),issue_alignment(1) {}
  void Clear() { placed=false; cycle=0; trials=0; modulo_cycle=0; slot=0;
	  	 if (op) {
  		   Reset_OP_end_group(op); Reset_OP_bundled(op);
		   Set_OP_format_num(op, -1); Set_OP_slot_num(op, -1);
		 }
  	       }
#if 0
  SWP_OP& operator=(SWP_OP& sop) {
    direction = sop.direction;
    op = sop.op;
    placed = sop.placed;
    is_noop = sop.is_noop;
    scale=scale;
...
public:
  ...
  INT num_extern_effects_ops;	// number of ops with extern effects (TIE ports, export state)
  ...
  void Verify() const;
  void Print(FILE *fp) const;
  void Print_Modulo_Schedule(FILE *fp) const;
  bool May_Stall(INT i, INT candidate, INT cycle) const;
  bool May_Stall(INT candidate, INT cycle) const;
  SWP_OP_vector(struct bb *body, BOOL doloop, MEM_POOL *pool);
  SWP_OP_vector(SWP_OP_vector& v, MEM_POOL *pool);
};
...
#endif
  INT found_ii;
  INT Compute(const SWP_OP_vector& v, INT start, INT stop, INT ii);
public:
  INT operator()(INT i, INT j) const { return mindist[i][j]; }
  INT Found_ii() { return found_ii; }
  void Print(FILE *fp) const;
  MinDist(const SWP_OP_vector& v, INT start, INT stop, INT ii);
};



extern SWP_RETURN_CODE
Modulo_Schedule(SWP_OP_vector& v, SWP_REG_ASSIGNMENT& swp_assign,
		BB* head, BB* tail,
...
extern SWP_RETURN_CODE
Modulo_Schedule_Super(SWP_OP_vector& v, SWP_REG_ASSIGNMENT& swp_assign, BB* head, BB* tail,
		INT min_ii, INT max_ii,
		double incr_alpha, double incr_beta,
		INT max_issue_alignment,
		INT32** scheduling_order,
...
extern BOOL SWP_Emit(SWP_OP_vector& op_state, 
		     SWP_REG_ASSIGNMENT& reg_assign,
		     TN *trip_count_tn,
		     BB *head, BB *body, BB *tail,
		     bool is_doloop, bool trace);

extern BOOL SWP_Failure(BB *body, SWP_RETURN_CODE code,
			ISA_REGCLASS rc=ISA_REGCLASS_UNDEFINED);

extern void Emit_SWP_Failure_Note(SWP_RETURN_CODE code, FILE *file,
				  const char* prefix,
