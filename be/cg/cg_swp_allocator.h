...
//  $Revision: 1.9 $
//  $Date: 2000/04/06 01:45:24 $
//  $Author: mtibuild $
//  $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/cg_swp_allocator.h,v $
//
//  Revision comments:
//
...
private:
  ...
  SWP_TUNIT _late_end;   // Late End unit for split pipeline, exclusive
  ...
  bool	    _start_with_split_pipeline_def;	// used to conditionally put in
  bool	    _end_with_split_pipeline_def;	// intereference edge
  SWP_TUNIT _first_use;
...
public:
  ...
  SWP_LIFETIME():
    _start(0), _end(0), _late_end(0), _omega(0), _alpha(0), _loc(0), _tn(NULL),
    _start_with_split_pipeline_def(false), _end_with_split_pipeline_def(false),
    _first_use(-1)
  {}
  ...
  SWP_LIFETIME(SWP_TUNIT start, 
	       ...
	       INT32     alpha, 
	       TN       *tn = NULL):
    _start(start), _end(end), _late_end(end),
    _omega(omega), _alpha(alpha), _loc(InvalidLoc), _tn(tn),
    _start_with_split_pipeline_def(false), _end_with_split_pipeline_def(false),
    _first_use(-1)
  {}
  ...
  SWP_TUNIT late_end() const {return _late_end;}
  INT32     omega()    const {return _omega;}
  INT32     alpha()    const {return _alpha;}
  INT32     location() const {return _loc;}
  TN       *tn()       const {return _tn;}
  bool	    start_with_split_pipeline_def() const
  		{ return _start_with_split_pipeline_def; }
  bool	    end_with_split_pipeline_def() const
  		{ return _end_with_split_pipeline_def; }
  bool	    has_use() const { return (_first_use != -1); }
  INT32	    first_use() const { return _first_use; }

  void      set_start(INT32 start) {_start = start;}
  void      set_end(INT32 end)     {_end = end;}
  void      set_late_end(INT32 end){_late_end = end;}
  void      set_omega(INT32 omega) {_omega = omega;}
  void      set_alpha(INT32 alpha) {_alpha = alpha;}
  void      set_location(INT32 loc) {_loc = loc;}
  
  void	    set_start_with_split_pipeline_def()
  		{ _start_with_split_pipeline_def = true; }
  void	    reset_start_with_split_pipeline_def()
  		{ _start_with_split_pipeline_def = false; }
  void	    set_end_with_split_pipeline_def()
  		{ _end_with_split_pipeline_def = true; }
  void	    reset_end_with_split_pipeline_def()
  		{ _end_with_split_pipeline_def = false; }
  void	    set_first_use(INT32 first_use) { _first_use = first_use; }
...
private:
  ...
  void _clear_allocation();
  STATUS _color_fit(int r);
  STATUS _simple_fit(const INT32_VECTOR &ordered);
