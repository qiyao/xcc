...
//  Module: cg_thr.cxx
//  $Revision: 1.14 $
//  $Date: 2000/10/13 19:10:19 $
//  $Author: lesniak $
//  $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/cg_thr.cxx,v $
//
//  Description:
//  ============
//
...
CG_THR::OP_Has_Restrictions(OP *pred_op, OP *succ_op, BOOL before_regalloc)
{
  if (_thr_type & THR_DATA_SPECULATION_NO_RB) {
    if (!before_regalloc && OP_load(succ_op)) {
      INT succ_base_idx = TI_TOP_Find_Operand_Use(OP_code(succ_op), OU_base);
      if (succ_base_idx == -1)
	return TRUE;
    ...
    if (OP_load(succ_op) && 
	TI_TOP_Find_Operand_Use(OP_code(succ_op), OU_postincr) >= 0)
      return TRUE;
  }
...
CG_THR::Perform_THR()
{
  ...
  if (_thr_type & THR_DATA_SPECULATION_NO_RB) {
    if (BB_length(_bb) > 1) {
      ...
      // dotted dependences.
      Compute_OPSCH (_bb, false, _thr_map, &_thr_pool);
