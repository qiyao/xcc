				     ...
				     Dwarf_Unsigned         last_offset)
{
  ...
  if (Trace_Dwarf) {
    fprintf(TFile,
	    "New CGD_Symtab entry: %lu --> (CGD_%s,%" LLU_FMT ")\n",
	    CGD_Symtab.size() - 1,
  ...
  if (type == CGD_LABIDX) {
	...
	STR_IDX sidx = CGD_Symtab[handle].label_info.name_idx;
      fprintf(TFile,
	      "pu_idx: %d; label: %d %s; ofst: %" LLU_FMT "; base: %u\n",
...
Elf64_Word Cg_Dwarf_Translate_Symidx(Dwarf_Unsigned idx_from_sym_reloc)
{
  Is_True(CGD_Symtab.size() > idx_from_sym_reloc,
	  ("Cg_Dwarf_Translate_Symidx: Index %lu out of bounds (%lu)",
	   idx_from_sym_reloc, CGD_Symtab.size()));
  if (Trace_Dwarf) {
    fprintf(TFile, "Translating %" LLU_FMT " ", idx_from_sym_reloc);
    fflush(TFile);
    fprintf(TFile, "to %" LLU_FMT "\n", CGD_Symtab[idx_from_sym_reloc].index);
  }
...
{
  Is_True(CGD_Symtab.size() > idx_from_sym_reloc,
	  ("Cg_Dwarf_Translate_Offset: Index %lu out of bounds (%lu)",
	   idx_from_sym_reloc, CGD_Symtab.size()));
  if (Trace_Dwarf) {
    fprintf(TFile, "Translating %" LLU_FMT " ", idx_from_sym_reloc);
    fflush(TFile);
    fprintf(TFile, "through index %" LLU_FMT " ",
	    CGD_Symtab[idx_from_sym_reloc].index);
    fprintf(TFile, "to %" LLD_FMT "\n",
			  ...
			  Dwarf_Unsigned *elf_offset)
{
  ...
  if (CGD_Symtab[idx_from_sym_reloc].type == CGD_LABIDX) {
    ...
    *elf_offset = CGD_Symtab[idx_from_sym_reloc].label_info.offset;
    if (Trace_Dwarf) {
      fprintf(TFile, "to elfsym %s + 0x%" LLX_FMT "\n",
...
Cg_Dwarf_Name_From_Handle(Dwarf_Unsigned idx)
{
  Is_True(CGD_Symtab.size() > idx,
	  ("Cg_Dwarf_Name_From_Handle: Index %lu out of bounds (%lu)",
	   idx, CGD_Symtab.size()));
...
static Dwarf_P_Die get_ref_die (DST_INFO_IDX ref_idx)
{
  ...
  if (ref_die == NULL) {
    ...
    if (Trace_Dwarf) {
      fprintf (TFile,"NEW ref die for [%d,%d]: %lx, tag:%d\n",
...
put_const_attribute (DST_CONST_VALUE cval, Dwarf_Half ref_attr, Dwarf_P_Die die)
{
  switch (DST_CONST_VALUE_form(cval)) {
    ...
    case DST_FORM_DATA4:
      ...
      break;
    case DST_FORM_DATA8:
      /* do not generate FORM_DATA8 since gdb does not support
	 it on 32-bit hosts
      dwarf_add_AT_unsigned_const (dw_dbg, die, ref_attr,
	       ...
	       Dwarf_P_Die die)
{
  ...
  if (DST_IS_memdef(flag))  /* Not yet supported */ {
    ...
    */
    ...
    if (DST_SUBPROGRAM_decl_access(attr) != DW_ACCESS_public) {
       dwarf_add_AT_unsigned_const(dw_dbg, die, DW_AT_accessibility,
			           DST_SUBPROGRAM_decl_access(attr), &dw_error);
    }
    ...
    put_reference (DST_SUBPROGRAM_decl_parent(attr), DW_AT_containing_type, die);
  }
  else /* definition */ {
    char *pubname;
	...
	put_flag (DW_AT_MIPS_has_inlines, die);
    put_reference (DST_SUBPROGRAM_def_parent(attr), DW_AT_containing_type, die);
  }
}
  ...
  switch (ST_sclass(st)) {
	...
	if (base_st != SP_Sym && base_st != FP_Sym) {
		dwarf_add_expr_addr_b (expr,
				       ...
				       &dw_error);
		if (Trace_Dwarf) {
	  		fprintf (TFile,"LocExpr: symbol = %s, offset = %" LLD_FMT "\n", 
			      ST_name(base_st), ST_ofst(st) + offs); 
		}
		break;
	} else if (Debug_Level > 0 && CG_opt_level > 0) {
		ST* block_st = ST_ptr(ST_base_idx(st));

		// the following two strings to match the definitions in data_layout.cxx
		BOOL is_formal = strcmp("Formal_Arg_StkSeg", ST_name(block_st))==0;
		BOOL is_upformal = strcmp("UpFormal_Arg_StkSeg", ST_name(block_st))==0;

		if (!is_formal && !is_upformal)
		  DevWarn("Unexpected formal argument");

	  	INT reg_num = is_formal? (ST_ofst(st) >> 2) + 2 : 0xffffffff;
		BOOL not_handled = false;
		switch (reg_num) {
	  	  case 2: dwarf_add_expr_gen (expr, DW_OP_reg2, 0, 0, &dw_error);
			  break;
	  	  case 3: dwarf_add_expr_gen (expr, DW_OP_reg3, 0, 0, &dw_error);
			  break;
	  	  case 4: dwarf_add_expr_gen (expr, DW_OP_reg4, 0, 0, &dw_error);
			  break;
	  	  case 5: dwarf_add_expr_gen (expr, DW_OP_reg5, 0, 0, &dw_error);
			  break;
	  	  case 6: dwarf_add_expr_gen (expr, DW_OP_reg6, 0, 0, &dw_error);
			  break;
	  	  case 7: dwarf_add_expr_gen (expr, DW_OP_reg7, 0, 0, &dw_error);
			  break;
		  default:
			  not_handled = true;
		}
...
put_subrange_type(DST_flag flag, DST_SUBRANGE_TYPE *attr, Dwarf_P_Die die)
{
  ...
  if (DST_IS_ub_cval(flag)) {
    ...
    /* skip upper bounds for zero-length array */
    if (DST_SUBRANGE_TYPE_upper_cval(attr) != -1LL)
...
put_class_type(DST_flag flag, DST_CLASS_TYPE *attr, Dwarf_P_Die die)
{
  ...
  if (DST_IS_declaration(flag)) put_flag (DW_AT_declaration, die);
  put_reference (DST_CLASS_TYPE_inheritance(attr), DW_AT_containing_type, die);
...
put_member(DST_flag flag, DST_MEMBER *attr, Dwarf_P_Die die)
{
  ...
  if (DST_IS_declaration(flag)) {
	 ...
	 * so use decl flag. */
  	...
  	expr = dwarf_new_expr (dw_dbg, &dw_error);
#if defined (KEY) || defined (TENSILICA) || 1
  ...
  if (DST_MEMBER_access(attr) != DW_ACCESS_public) {
    dwarf_add_AT_unsigned_const(dw_dbg, die, DW_AT_accessibility,
			        DST_MEMBER_access(attr), &dw_error);
  }
}
...
put_inheritance(DST_flag flag, DST_INHERITANCE *attr, Dwarf_P_Die die)
{
  ...
  if (DST_INHERITANCE_virtuality(attr) != DW_VIRTUALITY_none) {
    dwarf_add_AT_unsigned_const(dw_dbg, die, DW_AT_virtuality,
			        DST_INHERITANCE_virtuality(attr), &dw_error);
  }
  if (DST_INHERITANCE_access(attr) != DW_ACCESS_private) {
    dwarf_add_AT_unsigned_const(dw_dbg, die, DW_AT_accessibility,
			        DST_INHERITANCE_access(attr), &dw_error);
  }
}
  ...
  BOOL visit_children)
{
  ...
  if (die == NULL) {
    ...
    if (Trace_Dwarf) {
      fprintf (TFile,"NEW die for [%d,%d]: %lx, parent: %lx, left_sibling:%lx, tag:%d\n",
    ...
    if (Trace_Dwarf) {
      fprintf (TFile,"link die for [%d,%d]: %lx, parent: %lx, left_sibling:%lx, tag:%d\n",
	idx.block_idx, idx.byte_idx, die, parent, left_sibling, tag);
    }
  }
		     ...
		     INT        high_pc)
{
  ...
  if (subprogram_def_is_inlined(PU_attr)) {
	...
	if (Trace_Dwarf) {
		fprintf (TFile,"NEW subprogram concrete instance: die: %lx, left_sibling:%lx\n",
		idie, PU_die);
	}
  ...
  /* return address */
  Dwarf_P_Expr ra_expr = dwarf_new_expr (dw_dbg, &dw_error);
  if (Target_ABI == ABI_WINDOWED)
    dwarf_add_expr_gen (ra_expr, DW_OP_regx, 
			REGISTER_machine_id (REGISTER_CLASS_ra, REGISTER_MIN+0), 
			0, &dw_error);
  else {
    /* call0 abi */
    ...
    if (ra_save_tn) {
      ...
      dwarf_add_expr_gen (ra_expr, DW_OP_fbreg, ra_offset, 0, &dw_error);
    }
    else {
      dwarf_add_expr_gen (ra_expr, DW_OP_regx, 
	  REGISTER_machine_id (TN_register_class(RA_TN), TN_register(RA_TN)), 
	  0, &dw_error);
    }      
  }
  dwarf_add_AT_location_expr (dw_dbg, PU_die, DW_AT_return_addr, ra_expr, &dw_error);
...
Print_Directives_For_All_Files(void)
{
  ...
  for (idx = DST_get_file_names(); 
    ...
    if (count == 1) {
      /* First emit a .file directive without a file number.  The assembler
	 documentation claims that this is obsolete, but GCC still generates
	 it, and it causes the assembler to create a symbol that identifies
	 the source file name.  */
      USRCPOS_filenum(usrcpos) = 0;
      CGEMIT_Prn_File_Dir_In_Asm(usrcpos, "", file_table[count].filename);
    }

    file = DST_FILE_IDX_TO_PTR(idx);
    USRCPOS_filenum(usrcpos) = count;
    path = incl_table[file_table[count].incl_index].path_name; 

    CGEMIT_Prn_File_Dir_In_Asm(usrcpos, path, file_table[count].filename);
    count++;
  }
  fputc ('\n', Asm_File);
}
...
print_source (SRCPOS srcpos)
{
  ...
  if (cur_file->max_line_printed < USRCPOS_linenum(usrcpos)) {
	...
	if (length < (sizeof(text)-2)) {
	  if (text[length-1] != '\n') { // add returns to small lines with no returns
	    text[length] = '\n';
	    text[length+1] = 0;
          }
        }

	// leave a space at the beginning of file to prevent
	// the gnu assembler from parsing this as line information
	// with line and file name info. See pr9591.
        fprintf (Asm_File, " %s%4d  %s", ASM_CMNT_LINE, i+1, text);
        while (text[sizeof(text)-2] && 
               text[sizeof(text)-2] != '\n') {  // long line
          text[sizeof(text)-2] = '\n';  
          if (fgets (text, sizeof(text), cur_file->fileptr) != NULL) {
...
void
Cg_Dwarf_Add_Line_Entry (INT code_address, SRCPOS srcpos, BOOL needed)
{
  ...
  if (needed == FALSE && (srcpos == 0 || srcpos == last_srcpos)) return;
  ...
  struct virtual_section_position {
	...
	// uncaught bug in the calling-client.
      ...
      case 1:
      	  ...
      	  break;
      case 4:
      	    t32 =  ((UINT32_unaligned *)loc)->val;
	    if (!Same_Byte_Sex) {
	      UINT32 temp;
	      char* t32_p = (char*)&t32;
	      char* temp_p = (char*)&temp;
	      for (int i=0; i<size; i++) temp_p[i]=t32_p[size-i-1];
	      t32 = temp;
	    }
	    value = t32;
      	    break;
      case 8:
      	    t64 =  ((UINT64_unaligned *)loc)->val;
	    if (!Same_Byte_Sex) {
	      UINT64 temp;
	      char* t64_p = (char*)&t64;
	      char* temp_p = (char*)&temp;
	      for (int i=0; i<size; i++) temp_p[i]=t64_p[size-i-1];
	      t64 = temp;
	    }
	    value = t64;
      	    break;
                ...
                scn_handle** scn_bufferp /* output info */)
{
  ...
  Fail_FmtAssertion("Bogus section index %" LLD_FMT "\n", scndx);
  return -1;
}
				      ...
				      BOOL           is_64bit)
{
  ...
  do {
	...
	exit (-1);
      ...
      if (ofst != 0) {
	fprintf(asm_file, " + 0x%" LLX_FMT "", (unsigned long long)ofst);
      }
      fprintf(asm_file, "\n");
...
#else
      // Because we put label expressions into the line information and
      // this can result in byte sized relocation, handle that possibility
      // here.
      char *reloc_name;
      switch(reloc_buffer[k].drd_length) {
      case 4:
	reloc_name = AS_WORD_UNALIGNED;
	break;
      case 2:
	reloc_name = AS_HALF_UNALIGNED;
      case 1:
	reloc_name = AS_BYTE;
	break;
      }
...
#ifdef TARG_XTENSA
	// This case is possible because of label expressions in
	// the line information.
      case 1:
	ofst = vsp.vsp_get_bytes(cur_byte,1);
	break;
#endif
