...
extern void Exp_OP (OPCODE opcode, TN *result, TN *op1, TN *op2, TN *op3, INT16 variant, OPS *ops);
...
/* Expand 'tree' to produce a result in 'tn'. */
extern void Exp_Tree (TN *tn, WN *tree, OPS *ops);
...
extern void Exp_Load (TYPE_ID rtype, TYPE_ID desc, TN *tgt_tn, ST *sym,
		      INT64 ofst, OPS *ops, UINT16 variant);
extern void Exp_Load (TN *tn, WN *tree, OPS *ops, UINT16 variant);

/* Generate code to store 'src_tn' into the memory location 'sym'+'ofst'. */
extern void Exp_Store (TYPE_ID mtype, TN *src_tn, ST *sym, INT64 ofst, OPS *ops, UINT16 variant);

/* Generate a copy from 'src_tn' to 'tgt_tn'. */
extern void Exp_COPY (TN *tgt_tn, TN *src_tn, OPS *ops); 

/* Generate an add : result = src1 + src2 */
extern void Expand_Add (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops);

/* Given a simulated <op> thta move/load/store br registers,
 * expand it into the sequence of instructions
 * that must be generated. They are expanded earlier than
 * the rest of the simulated instructions to allow optimization.
 * return TRUE if the expansion is done or FALSE otherwise
 */
BOOL Exp_Br_Reg_Simulated_Op (const OP *op, OPS *ops);
 ...
 * of instructions generated. (pc_value is not used in this function).
 *
 * The return value indicates if new instructions has been generated.
 */
extern void Exp_Simulated_Op (const OP *op, OPS *ops, INT pc_value);

/*
 * Given a simulated <op>, if it is TOP_load_const, expand it into 
 * the sequence of instructions <ops> that must be generated, and
 * return TRUE; otherwise, return FALSE.
 *
 * The purpose of this function is to expand TOP_load_const before IGLS
 * so l32r (TOP_load_const will be expanded into l32r) can be bundled
 * in IGLS.
 */
extern BOOL Exp_load_const (const OP *op, OPS *ops);

/* Given a simulated <op>, perform any symbol generation or other
   finalization that must occur just before assembly emission. */
extern void Finalize_Simulated_Op (OP *op);

/* For the given simulated <op>, return the number of 
 * instructions/instruction-bytes that will be generated after expansion.
 */
extern INT Simulated_Op_Real_Ops (const OP *op);
extern INT Simulated_Op_Real_Inst_Bytes (const OP *op);
...
/* generate prefetch inst; pf_flags contains prefetch flags */
extern void Exp_Prefetch (TOP opc, UINT32 pf_flags, TN *src1, TN *src2, OPS *ops);
...
/* build spadjust op */
extern void Exp_Spadjust (TN *dest, TN *size, UINT16 variant, OPS *ops);

/* Return a unique name for a symbol representing a literal. */
char *Exp_Unique_Literal_Name (void);
