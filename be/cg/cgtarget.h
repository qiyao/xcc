 ...
 *
 *	TOP CGTARG_Inter_RegClass_Copy(ISA_REGCLASS dst,
 *				       ISA_REGCLASS src,
 *				       BOOL is_double)
 ...
 *	  there isn't.
 *
 *	TOP CGTARG_Reg_To_Immed(TOP opr)
 *	  <opr> is a register format instruction, return its equivalent
 *	  immediate form if there is one, return TOP_UNDEFINED if
 *	  there isn't.
 *
 ...
 *
 *	TOP CGTARG_Invert_Branch(TOP opr)
 *	  Return the inverted version of branch <opr>. TOP_UNDEFINED
 *	  is returned if there is no inverse. The inverted version of
 *	  a branch is the one that is taken iff the original branch is
 *	  not-taken, and is not-taken iff the original branch is
 *	  taken.
 *
 *	TOP CGTARG_Same_Branch_Flipped_Operands(TOP opr)
 *	  Return a branch that behaves the same a branch <opr>, when
 *	  the operands of <opr> are switched. TOP_UNDEFINED is
 *	  returned if there is no such branch, or if <opr> has only one
 *	  operand,
 *
 ...
 *
 *      void CGTARG_Generate_Branch_Cloop(BB *prolog_bb,
 *			                  BB *loopend_bb,
 ...
 *
 *      OP *CGTARG_Generate_Updating_Op (OP *op, INT storeval_idx, INT base_idx,
 *		   		         INT offset_idx, TN *new_offset_tn)
 *        Generate an updating version of 'op' using 'storeval_idx',
 *        'base_idx', and 'offset_idx' to find its store-value, base,
 *        and offset operands ('storeval' is only meaningful if 'op'
 *        is a store). Use 'new_offset_tn' as the offset tn of
 *        the updating instruction. Return NULL if can't create an
 *        updating version of 'op'.
 *
...
/* 
 * $Revision: 3.131 $
 * $Date: 2000/10/05 21:39:57 $
 * $Author: srinivas $
 * $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/cgtarget.h,v $
 */
...
inline TOP
CGTARG_Invert_Branch(TOP opr)
{
  return TI_TOP_Invert_Branch(opr);
}

inline TOP
CGTARG_Same_Branch_Flipped_Operands(TOP opr)
{
  return TI_TOP_Same_Branch_Flipped_Operands(opr);
}

inline TOP
CGTARG_Immed_To_Reg(TOP opr)
{
  return TI_TOP_Immed_To_Reg(opr);
}

inline TOP
CGTARG_Reg_To_Immed(TOP opr)
{
  return TI_TOP_Reg_To_Immed(opr);
}
...
/* Insert_Stop_Bits is called at the end of Handle_All_Hazards
   in hb_hazars.cxx */
...
{
  extern OPCODE *CGTARG_Assoc_Base_Opr_Table;
...
{
  extern mTOP *CGTARG_Assoc_Base_Top_Table;
...
{
  extern mTOP *CGTARG_Assoc_Base_Fnc_Table;
...
inline TOP CGTARG_Inter_RegClass_Copy(ISA_REGCLASS dst,
				      ISA_REGCLASS src,
				      BOOL is_double)
{
  extern mTOP CGTARG_Inter_RegClass_Copy_Table
    [TI_ISA_REGCLASS_MAX+1][TI_ISA_REGCLASS_MAX+1][2];

  return (TOP)CGTARG_Inter_RegClass_Copy_Table[src][dst][is_double];
}


extern BOOL CGTARG_Can_Fit_Immediate_In_Entry_Instruction (INT64 immed);
extern BOOL CGTARG_Can_Fit_Immediate_In_Exit_Instruction (INT64 immed);
...
extern void CGTARG_Generate_Branch_Cloop(BB *prolog_bb, BB *loopend_bb,
					 TN *unrolled_trip_count, TN *trip_count,
					 INT32 ntimes, TN *label_tn,
					 OPS *prolog_ops, OPS *body_ops);

extern OP *CGTARG_Generate_Updating_Op (OP *op, INT storeval_idx, INT base_idx,
					INT offset_idx, TN *new_offset_tn);
...
/* Convert TNs into an array of integers for use with TI_Convert_OP */
extern BOOL Convert_TNs_To_INT(OP *op, INT *tn_num_array);
