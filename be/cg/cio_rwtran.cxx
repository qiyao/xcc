...
//  ============
//
//  CG inner loop read/write transformation module.  See "cio_rwtran.h" for
...
//
// CIO_enable_vector_rw_removal      --> activates read  elimination
// CIO_enable_cross_iter_cse_removal --> activates cicse elimination
// CIO_enable_vector_ww_removal      --> activates write elimination
// CIO_enable_vector_ww_overridden   --> sets CIO_enable_vector_ww_removal
//                                       on iff CG_opt_level > 1
// If read elimination is off, then cicse elimination also has no effect.
//
// CIO_rwtran_maxiterback            --> currently no effect
// CIO_rwtran_threshold              --> currently no effect
// CIO_rwtran_iterback_sum_limit     --> currently no effect
// CIO_rwtran_max_omega              --> ignore MEMARCs with omega over
//                                       CIO_rwtran_max_omega
//
// ======================================================================


BOOL  CIO_enable_cross_iter_cse_removal = TRUE;
BOOL  CIO_enable_vector_rw_removal      = TRUE;
BOOL  CIO_enable_vector_ww_removal      = TRUE;
BOOL  CIO_enable_vector_ww_overridden   = FALSE;

INT32 CIO_rwtran_maxiterback        = 8;
INT32 CIO_rwtran_threshold          = 70;
INT32 CIO_rwtran_iterback_sum_limit = 8;
INT32 CIO_rwtran_max_omega          = 8;
...
//
//
//   max_omega = CIO_rwtran_max_omega;
...
public:
  ...
  CIO_RWTRAN(  LOOP_DESCR *loop, MEM_POOL *mem_pool )
    : _loc_mem_pool(    mem_pool ),
      _trace_CG_DupTN(  Get_Trace( TP_CGPREP, 0x100,
				   BB_id( LOOP_DESCR_loophead( loop ) ) ) ),
      _trace_CG_RTran(  Get_Trace( TP_CGPREP, 0x200,
				   BB_id( LOOP_DESCR_loophead( loop ) ) ) ),
      _trace_CG_Chkpnt( Get_Trace( TP_CGPREP, 0x400,
				   BB_id( LOOP_DESCR_loophead( loop ) ) ) ),
...
CIO_RWTRAN::Mark_Op_For_Prolog( OP *op, const UINT8 omega )
{
  ...
  // cannot handle same res properly yet
  if ( OP_same_res( op ) )
    return NULL;
...
CIO_RWTRAN::Copy_Ops_To_Prolog()
{
  ...
  for ( op_copy_map_prolog::const_iterator op_iter = _op_prolog_map.begin();
    ...
    if ( _trace_CG_RTran ) {
      #pragma mips_frequency_hint NEVER
      fprintf( TFile, "(op 0x%lx, omega %u) ---> op 0x%lx\n\t",
				    ...
				    const UINT8 end_omega )
{
  ...
  /* Can't backpatch dedicated TNs */
  for ( INT res = 0; res < OP_results( op ); ++res ) {
    TN *tn = OP_result( op, res );
    if (TN_is_dedicated(tn))
      return FALSE;

    for ( UINT8 ix_omega = start_omega; ix_omega <= end_omega; ++ix_omega ) {
      TN *existing_non_body_tn = 
         CG_LOOP_Backpatch_Find_Non_Body_TN (CG_LOOP_prolog, new_tns[res], ix_omega);
      if (existing_non_body_tn != NULL) {
        // Conservative. Need to improve.
        return FALSE;
      }
    }
  }
...
CIO_RWTRAN::Predicate_Write( OPS *ops, OP *op, TN *tn_predicate )
{
  ...
  Is_True( !OP_unknown_addr( op ), ( "OP_unknown_addr(op) == TRUE" ) );
  ...
  FmtAssert((opnd_base != -1) && (opnd_offset != -1),
	    ("expecting memory OP to have base and offset"));
  ...
  Is_True( OP_find_opnd_use( op, OU_storeval )>=0, ( "Missing storeval" ) );
...
CIO_RWTRAN::Mark_Op_For_Epilog( OP *op, const UINT8 omega )
{
  ...
  if ( _trace_CG_RTran ) {
    #pragma mips_frequency_hint NEVER
    fprintf( TFile,
	     "<cio> Mark_Op_For_Epilog(op 0x%lx, omega %u) ---> op 0x%lx\n\t",
	     op, omega, op_epilog );
    Print_OP_No_SrcLine( op );
    fprintf( TFile, "\t" );
...
CIO_RWTRAN::Copy_Ops_To_Epilog()
{
  ...
  for ( op_copy_map_epilog::const_iterator op_iter = _op_epilog_map.begin();
	...
	if ( CGTARG_Can_Predicate() ) {
	  ...
	  INT16 variant = ( size == 4 ? V_BR_I4GE : V_BR_I8GE );
...
//
// Write_Candidate_Arc returns TRUE iff arc is a candidate for read
// elimination elimination.
//
// If particular, if:
...
CIO_RWTRAN::Read_CICSE_Candidate_Op( OP *op )
{
  ...
  // updating load should not be an candidate for read removal
  // ops with multiple results should not be a candidate for cse, either
  if ( OP_results( op ) > 1 )
    return FALSE;
...
{
  return ( OP_store( op ) && OP_results(op)==0
	   && ! ( OP_unalign_store( op ) ||
		  OP_unknown_addr( op )  ||
		  OP_unknown_memdata( op )  ||
...
CIO_RWTRAN::Read_Candidate_Arc( ARC *arc )
{
  ...
  if (!(pred != succ
	   && ARC_omega( arc ) <= CIO_rwtran_max_omega 
	   && ARC_omega( arc ) < MAX_OMEGA
	   && !(OP_unknown_addr(pred) || OP_unknown_memdata(pred) ||
		OP_unknown_addr(succ) || OP_unknown_memdata(succ))))
    return FALSE;

  TOP pred_top = OP_code(pred);
  TOP succ_top = OP_code(succ);
  INT pred_op_idx = -1;
  INT succ_op_idx = TI_TOP_Find_Result_Use(succ_top, OU_loadval);

  ISA_REGCLASS pred_rc = ISA_REGCLASS_UNDEFINED;
  ISA_REGCLASS succ_rc = TI_ISA_Valtyp_Regclass(
		TI_ISA_Op_Result(TI_ISA_Operand_Info(succ_top), succ_op_idx));

  TYPE_ID pred_mtype = MTYPE_UNKNOWN;
  TYPE_ID succ_mtype =
	TN_mtype(OP_result(succ,
			   TI_TOP_Find_Result_Use(OP_code(succ), OU_loadval)));

  if (ARC_kind( arc ) == CG_DEP_MEMIN) {
    pred_op_idx = TI_TOP_Find_Operand_Use(OP_code(pred), OU_storeval);
    pred_rc = TI_ISA_Valtyp_Regclass(
                TI_ISA_Op_Operand(TI_ISA_Operand_Info(pred_top), pred_op_idx));
    pred_mtype = TN_mtype(OP_opnd(pred, pred_op_idx));
  } else {
    pred_op_idx = TI_TOP_Find_Result_Use(OP_code(pred), OU_loadval);
    pred_rc = TI_ISA_Valtyp_Regclass(
                TI_ISA_Op_Result(TI_ISA_Operand_Info(pred_top), pred_op_idx));
    pred_mtype = TN_mtype(OP_result(pred, pred_op_idx));
  }

  return (pred_mtype==succ_mtype && pred_rc==succ_rc);
}
...
{
  if (!(ARC_kind( arc ) == CG_DEP_MEMOUT && ARC_is_definite( arc )))
    return FALSE;
  ...
  TOP pred_top = OP_code(pred);
  TOP succ_top = OP_code(succ);
  INT pred_op_idx = TI_TOP_Find_Operand_Use(pred_top, OU_storeval);
  INT succ_op_idx = TI_TOP_Find_Operand_Use(succ_top, OU_storeval);

  ISA_REGCLASS pred_rc = TI_ISA_Valtyp_Regclass(
		TI_ISA_Op_Operand(TI_ISA_Operand_Info(pred_top), pred_op_idx));
  ISA_REGCLASS succ_rc = TI_ISA_Valtyp_Regclass(
		TI_ISA_Op_Operand(TI_ISA_Operand_Info(succ_top), succ_op_idx));

  TYPE_ID pred_mtype =
	TN_mtype(OP_opnd(pred,
			 TI_TOP_Find_Operand_Use(OP_code(pred), OU_storeval)));
  TYPE_ID succ_mtype =
	TN_mtype(OP_opnd(succ,
			 TI_TOP_Find_Operand_Use(OP_code(succ), OU_storeval)));
...
struct CICSE_entry {
  ...
  BOOL   result_unique[CICSE_MAX_RESULTS];
};
...
struct CICSE_change {
  ...
  OP    *source;
  UINT8  omega;
  TN    *new_tns[CICSE_MAX_RESULTS];
  BOOL   need_copy[CICSE_MAX_RESULTS];
};


// ======================================================================
// Print_CICSE_Entries  and  Print_CICSE_Changes  print the contents of
				 ...
				 const char *message )
{
  ...
  for ( INT index = 1; index <= count; ++index ) {
    ...
    fprintf( TFile, "    op 0x%lx, source %2d, omega %u, basis %d,"
	     " match %d, lnz_opnd %d\n    opnds:",
    ...
    fprintf( TFile, "\n    results:" );
    if ( OP_results( entry.op ) > CICSE_MAX_RESULTS )
      fprintf( TFile, " OP_results(entry.op) %d > CICSE_MAX_RESULTS %d",
	       OP_results( entry.op ), CICSE_MAX_RESULTS );
    else
				 ...
				 const char *message )
{
  ...
  for ( INT index = 0; index < count; ++index ) {
    ...
    fprintf( TFile, "\n%2d: okay %d, duplicate %d, op 0x%lx, source 0x%lx,"
	     " omega %u\n  op:      ",
...
// old_tn throughout the body and the prolog and epolog backpatches of
// the current loop.
//
...
{
  // Count the number of ops in the loop body.  Also, initialize the
  // map tn_last_op, which remembers the latest OP for each result TN.
  ...
  FOR_ALL_BB_OPs_FWD( body, op ) {
    ++op_count;
    for ( INT res = OP_results( op ) - 1; res >= 0; --res )
      hTN_MAP32_Set( tn_last_op, OP_result( op, res ), op_count );
  ...
  // Allocate a table of CICSE_entry.  cicse_table[0] is not used,
  // because 0 is used to indicate constant and global operand TNs
  CICSE_entry *cicse_table
    = (CICSE_entry *) CXX_NEW_ARRAY( CICSE_entry, op_count + 1,
				     _loc_mem_pool );

  // Initialize the table entries.  entry.op must be valid for all OPs
  // in the loop (so that CICSE_lookup_op can use a binary search)
  INT index;
  op = BB_first_op( body );
  for ( index = 1; index <= op_count; ++index ) {
    ...
    if ( OP_flag1( op ) ) {
      entry.source = index;
      for ( INT opnd = OP_opnds( op ) - 1; opnd >= 0; --opnd ) {
	if ( TN_is_register(OP_opnd(op,opnd))) {
	  entry.opnd_source[opnd] = hTN_MAP32_Get( tn_last_op,
						 OP_opnd( op, opnd ) );
	  entry.opnd_omega[opnd]  = OP_omega( op, opnd );
	} else {
	  // doesn't work if just skip these, so initialize to 0
	  entry.opnd_source[opnd] = 0;
	  entry.opnd_omega[opnd]  = 0;
	}
      }
    } else
      // Ignore this OP
...
#ifdef TARG_XTENSA
    /* We need to make CICSE_entry and CICSE_change variable sized to
       allow for ops with operands > OP_MAX_FIXED_OPERANDS and/or
       results > OP_MAX_FIXED_RESULTS, as is done for
       cg_loop.h:_CG_LOOP_INFO. Until then, just abort this
       optimization if there is an OP with "too many" results. Too
       many operands are handled in "Read_Candidate_Arc". */
    if (OP_results( op ) > CICSE_MAX_RESULTS)
    {
      DevWarn( "CIO_RWTRAN::Too many OP results, need variable sized objects" );
      return FALSE;
    }
#endif
  ...
  if ( CIO_enable_cross_iter_cse_removal ) {
	...
	while ( index1 > 0 ) {
	  ...
	  // Do opnd_sources match?  (Locally constant operands have
  ...
  for ( index = 1; index <= op_count; ++index ) {
	...
	for ( INT res = OP_results( change.source ) - 1; res >= 0; --res )
      ...
      Is_True( opnd >= 0, ( "CIO_RWTRAN::CICSE_Transform can't find"
			    ...
			    TI_TOP_Name( OP_code( change.source ) ) ) );
      ...
      // tn_index may be 0 if the source op is not defined inside the loop
      if ( tn_index == 0 ||
...
CIO_RWTRAN::Read_CICSE_Write_Removal( LOOP_DESCR *loop )
{
  ...
  if ( _trace_CG_RTran ) {
    ...
    fprintf( TFile, "\n<cio> %s<cio> Read/CICSE/Write Removal:\n<cio> %s\n",
	     SBar, SBar );
    CG_LOOP_Trace_Loop( loop, "<cio> Before Read/CICSE Removal" );
    CG_DEP_Trace_Graph( body );
  }

  // Preform read and CICSE removal
  BOOL changed_loop_read = FALSE;
  if ( CIO_enable_vector_rw_removal )
  ...
  if ( CIO_enable_vector_ww_removal )
  ...
  if ( _trace_CG_RTran || _trace_CG_Chkpnt ) {
    ...
    if ( changed_loop_write )
      CG_LOOP_Trace_Loop( loop, "<cio> After Read/CICSE/Write Removal\n" );
    else
      fprintf( TFile, "<cio> After Read/CICSE/Write Removal: No Change\n" );
    fprintf( TFile, "\n" );
  }
  ...
  return ( changed_loop_read || changed_loop_write );
}
...
{
  if ( ! CIO_enable_vector_rw_removal && ! CIO_enable_vector_ww_removal )
    return FALSE;
