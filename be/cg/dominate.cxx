 ...
 * Module: dominate.c
 * $Revision: 1.24 $
 * $Date: 2000/04/06 01:47:02 $
 * $Author: mtibuild $
 * $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/dominate.cxx,v $
 *
 * Description:
 * 
...
Init_BB_Dom_Info(BB *bb, BS *dom_init)
{
  ...
  if (BB_entry(bb) || cgrin && bb == CGRIN_entry(cgrin)) {
    ...
    Set_BB_dom_set(bb, BS_Union1D(BB_dom_set(bb), BB_ID_PRE_ENTRY, NULL));
    Set_BB_dom_set(bb, BS_Union1D(BB_dom_set(bb), BB_id(bb), NULL));
  } else {
   ...
   */
  Set_BB_pdom_set(bb, BS_Union1D(BB_pdom_set(bb), BB_id(bb), NULL));
  if (BB_exit(bb) || rid && BB_REGION_Exit(bb, rid) != NO_REGION_EXIT) {
    Set_BB_pdom_set(bb, BS_Union1D(BB_pdom_set(bb), BB_ID_POST_EXIT, NULL));
  }
}

/* ====================================================================
...
#define match dom_init	/* Share this bitvector */
  ...
  for ( bb = REGION_First_BB; bb; bb = BB_next(bb) ) {
    dom_init = BS_Union1D(dom_init, BB_id(bb), NULL);
  }
  dom_init = BS_Union1D(dom_init, BB_ID_PRE_ENTRY, NULL);
  ...
  do {
	...
	src = isect;
      ...
      isect = BS_Union1D(isect, BB_id(bb), NULL);
   ...
   */
  BS* pseudo_exits = BS_Create_Empty(2+PU_BB_Count, &MEM_local_nz_pool);
  do {
    changed = FALSE;
    bb = last_bb;
    do {
  ...
  for ( BB* bb = REGION_First_BB; bb; bb = BB_next(bb) ) {
    if (!BS_MemberP(BB_pdom_set(bb),BB_ID_POST_EXIT)) {
      BOOL loop_forever = TRUE;
      BBLIST *list = BB_succs(bb);
      if (list) {
        do {
	  BB *bbp = BBLIST_item(list);
	  if (!BS_MemberP(BB_pdom_set(bbp),BB_id(bb))) {
	    loop_forever = FALSE;
	    break;
	  }
        } while (list = BBLIST_next(list));
	...
	loop_forever=FALSE;
      ...
      if (loop_forever) {
	pseudo_exits = BS_Union1D(pseudo_exits, BB_id(bb), NULL);
      }
    }
  }

  if (!BS_EmptyP(pseudo_exits))
  for ( BB* bb = REGION_First_BB; bb; bb = BB_next(bb) ) {
    if (BS_MemberP(pseudo_exits, BB_id(bb))) {
      BS* bb_pdom_set = BB_pdom_set(bb);
      (void) BS_ClearD(bb_pdom_set);
      bb_pdom_set = BS_Union1D(bb_pdom_set, BB_ID_POST_EXIT, NULL);
      bb_pdom_set = BS_Union1D(bb_pdom_set, BB_id(bb), NULL);
      Set_BB_pdom_set(bb, bb_pdom_set);
    }
  }
  ...
  do {
    ...
    do {
      ...
      if (BS_MemberP(pseudo_exits, BB_id(bb))) continue;
...
BB_REGION_Calculate_Dominators(const BB_REGION& region)
{
  ...
  do {
	...
	src = bs_tmp;
      ...
      bs_tmp = BS_Union1D(bs_tmp, BB_id(bb), NULL);
...
BB_SET_Calculate_Dominators(BB_SET *bbset, BOOL compute_dom, BOOL compute_pdom)
{
  ...
  if (compute_dom) {
	...
	bs_tmp = BS_UnionD(bs_tmp, bb_dom_set, NULL);
	if ( ! BS_EqualP(bs_tmp, bb_dom_set) ) {
	  BS_CopyD(bb_dom_set, bs_tmp, NULL);
	  changed = TRUE;
	}
  ...
  if (compute_pdom) {
	...
	bs_tmp = BS_UnionD(bs_tmp, bb_pdom_set, NULL);
	if ( ! BS_EqualP(bs_tmp, bb_pdom_set) ) {
	  BS_CopyD(bb_pdom_set, bs_tmp, NULL);
	  changed = TRUE;
	}
