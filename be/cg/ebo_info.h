...
inline void regtn_info_def(BB *current_bb, OP *current_op, ISA_REGCLASS rc, REGISTER reg)
{
  ...
  if (tninfo != NULL) {
    if (EBO_Trace_Data_Flow) {
      ...
      if (tninfo->replacement_tn != NULL) {
        fprintf(TFile,"%sASM clobbers stop propagating Copy of ",EBO_trace_pfx);
        Print_TN(tninfo->replacement_tn, FALSE);
        fprintf(TFile,"[%d] into ",
                (tninfo->replacement_tninfo != NULL) ? tninfo->replacement_tninfo->omega : 0);
        Print_TN(tninfo->local_tn,FALSE);
        fprintf(TFile,"\n");
      } else {
        fprintf(TFile,"%sASM clobbers redefine ", EBO_trace_pfx);
        Print_TN(tninfo->local_tn, FALSE);
        fprintf(TFile,"\n");
      }
    }
    if (tninfo->in_bb == current_bb) {
      tninfo->redefined_before_block_end = TRUE;
    }
  }
  EBO_TN_INFO *asm_tninfo = get_new_tninfo (current_bb, current_op, tn);
  asm_tninfo->same = tninfo;
}
