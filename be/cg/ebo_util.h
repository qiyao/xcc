                 ...
                 EBO_TN_INFO *tninfo)
{
 ...
 /* Global TN's aren't supported at low levels of optimization. */
  if ((CG_opt_level < 2) && (bb != tn_bb)) return FALSE;
...
#ifndef TARG_XTENSA
    /* We don't want this for xtensa, since the method for determining
       a spill load/store doesn't work for us (i.e. using the
       following code, we incorrectly identify non-spill loads/stores
       as being spills). */
    if (OP_load(op)) {
      spill_tn = OP_result(op,0);
    } else if (OP_store(op) &&
	       TI_TOP_Find_Operand_Use(OP_code(op), OU_storeval)>=0);
      spill_tn = OP_opnd(op,TI_TOP_Find_Operand_Use(OP_code(op), OU_storeval));
    }
#endif
  ...
  if (into_bb != outof_bb) {
    ...
    if (EBO_in_peep) {
      if (!REG_LIVE_Into_BB( TN_register_class(tn), TN_register(tn), into_bb)) {
        ISA_REGCLASS tn_class = TN_register_class(tn);
