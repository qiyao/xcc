 ...
 */
#ifdef _KEEP_RCS_ID
static const char rcs_id[] = "$Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/findloops.cxx,v $ $Revision: 1.25 $";
#endif /* _KEEP_RCS_ID */
...
LOOP_DESCR_Print_List(void)
{
  ...
  for (cloop = the_loops; cloop != NULL; cloop = LOOP_DESCR_next(cloop)) {
    ...
    i++;
    fprintf (tfile, 
	"LOOP %d  (loop head: %d, nest level: %d, # exits: %d%s",
	i,
	loophead ? BB_id(loophead) : 0, 
	LOOP_DESCR_nestlevel(cloop), 
	LOOP_DESCR_num_exits(cloop),
        Is_Frequency_Never_Loop(cloop) ? ", Freq: Never" : "");
...
LOOP_DESCR_Is_Exit_Edge(BB *bb, BB *succ)
{
  ...
  for (loop = (LOOP_DESCR *)BB_MAP_Get(LOOP_DESCR_map, bb);
	...
	/* LNO and CG disagree about the meaning of a "trip-countable"
	   loop.  LNO considers a loop trip-countable even if there's
	   an early exit that may cause the loop to execute fewer
	   times than the trip count.  CG considers a loop
	   trip-countable iff the trip count expression always gives
	   the number of trips that the loop will execute.  Since
	   LNO's definition is more broad, WOPT fills in trip counts
	   according to its definition, and it's up to CG to get rid
	   of the trip counts that aren't necessarily correct because
	   of early exits!  Within CG, the LNO's definition or trip
	   countable is maintained in LOOPINFO_max_trip_count_tn.  */
...
//		BB_id(bb), BB_id(succ), Cur_PU_Name, __LINE__, __FILE__);
	continue;
      ...
      loop_set = BB_SET_Union1D(loop_set, succ, NULL);
      ...
      Reset_Zcl_Enclosing_Loop(newloop);
      Reset_Frequency_Never_Loop(newloop);
	...
	if (LOOP_DESCR_loophead(cloop) == succ) {
	  LOOP_DESCR_bbset(newloop) = BB_SET_UnionD(LOOP_DESCR_bbset(newloop), 
  ...
  /*
     Set Frequency_Never_Loop bit. If the loop's head is a frequency-never BB
     (i.e. by #pragma mips_frequency_hint NEVER), the loop is marked as 
     Frequency_Never_Loop.
   */
  BB_SET *never_bbs = FREQ_Find_Never_BBs(pool, FALSE);
  if ((never_bbs != NULL) && !BB_SET_EmptyP(never_bbs)) {
    for (cloop = the_loops; cloop != NULL; cloop = LOOP_DESCR_next(cloop)) {
      BB *loop_head = LOOP_DESCR_loophead(cloop);
      if (BB_SET_MemberP(never_bbs, loop_head)) {
         Set_Frequency_Never_Loop(cloop);
      }
    }
  }
...
#define max_depth 128
  ...
  LOOP_DESCR* loops[max_depth];

  FmtAssert(LOOP_DESCR_nestlevel(loop)<=max_depth, ("Loop nesting too deep"));
  ...
  FOR_ALL_BB_PREDS(LOOP_DESCR_loophead(loop), preds) {
    ...
    if (BB_SET_MemberP(LOOP_DESCR_bbset(loop), pred_bb)) {
      ...
      FOR_ALL_BB_SUCCS(pred_bb, succs) {
        ...
        if (succ_bb == LOOP_DESCR_loophead(loop)) continue;
        if (BB_SET_MemberP(LOOP_DESCR_bbset(loop),  succ_bb)) 
          continue;
...
LOOP_DESCR_Can_Retarget_Loop_Entrances(LOOP_DESCR *loop)
{
  ...
  while (preds) {
    ...
    if (!BB_SET_MemberP(LOOP_DESCR_bbset(loop), pred)) {
#ifdef TARG_XTENSA
      // Can't retarget zero-cost loops
      OP *br_op = BB_branch_op(pred);
