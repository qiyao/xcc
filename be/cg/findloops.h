 ...
 * Module: findloops.h
 * $Revision: 1.18 $
 * $Date: 2000/04/06 01:47:38 $
 * $Author: mtibuild $
 * $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/findloops.h,v $
 *
 * Description:
 *
...
#define		LOOP_DESCR_Zcl_Enclosing_Loop	0x010
#define		LOOP_DESCR_Frequency_Never_Loop	0x020
...
#define		Reset_Call_Loop(loop) 	\
			(LOOP_DESCR_flags(loop) &= ~LOOP_DESCR_Call_Loop)
#define		Is_Zcl_Enclosing_Loop(loop)	\
			(LOOP_DESCR_flags(loop) & LOOP_DESCR_Zcl_Enclosing_Loop)
#define		Set_Zcl_Enclosing_Loop(loop)	\
			(LOOP_DESCR_flags(loop) |= LOOP_DESCR_Zcl_Enclosing_Loop)
#define		Reset_Zcl_Enclosing_Loop(loop) 	\
			(LOOP_DESCR_flags(loop) &= ~LOOP_DESCR_Zcl_Enclosing_Loop)
#define		Is_Frequency_Never_Loop(loop)	\
			(LOOP_DESCR_flags(loop) & LOOP_DESCR_Frequency_Never_Loop)
#define		Set_Frequency_Never_Loop(loop)	\
			(LOOP_DESCR_flags(loop) |= LOOP_DESCR_Frequency_Never_Loop)
#define		Reset_Frequency_Never_Loop(loop)	\
			(LOOP_DESCR_flags(loop) &= ~LOOP_DESCR_Frequency_Never_Loop)
