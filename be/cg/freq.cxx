 ...
 * $Author: mpm $
 * $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/freq.cxx,v $
 *
 * Description:
 *
...
static BOOL Is_Pointer(TN *tn, OP *use_op)
{
  ...
  for (;;) {
	 ...
	 */
	use_tn = OP_opnd(def_op, TI_ISA_Copy_Operand(OP_code(def_op), TRUE));
	use_op = def_op;
	continue;
      }
    }
  ...
  FOR_ALL_BB_OPs(OP_bb(use_op), op) {
    if (OP_load(op) || OP_store(op)) {
      ...
      if ((base_idx >= 0) && (OP_opnd(op, base_idx) == tn))
        return TRUE;
    }
  }
  ...
  double *s1_taken_prob)
{
  ...
  case V_BR_U8NE:
    break;
  case V_BR_ALL_BITS_SET:
  case V_BR_ALL_BITS_CLEAR:
  case V_BR_ANY_BIT_SET:
  case V_BR_ANY_BIT_CLEAR:
  case V_BR_BIT_SET:
  case V_BR_BIT_CLEAR:
...
Is_Loophead_BB(BB *branchbb, BB *succ)
{
  ...
  if (branchbb != succ && BS_MemberP(BB_pdom_set(branchbb), BB_id(succ)))
    return FALSE;
  ...
  MEM_POOL *pool)
{
  ...
  for (bb = REGION_First_BB; bb != NULL; bb = BB_next(bb)) {
	...
	*hint_bbs = BB_SET_Union1D(*hint_bbs, bb, NULL);
      }
    }
  }
}
  ...
  BB_SET *visited)
{
  ...
  if (bb == head) {
	...
	if (!BB_freq_fb_based(bb)) BB_freq(bb) = in_freq / (1.0 - PROB_LBH);
      ...
      if (!BB_freq_fb_based(bb)) BB_freq(bb) = (float)in_freq;
    }
   ...
   */
  visited = BB_SET_Union1D(visited, bb, NULL);
  to_visit = BB_SET_Difference1D(to_visit, bb);
...
Set_Reachable(BB *bb, BB_SET *reachable)
{
  ...
  reachable = BB_SET_Union1D(reachable, bb, NULL);
...
Compute_Frequencies(void)
{
  ...
  while (loops) {
     ...
     */
    to_visit = BB_SET_CopyD(to_visit, loop_bbs, NULL);
    visited = BB_SET_ClearD(visited);
   ...
   */
  to_visit = BB_SET_UniverseD(to_visit, PU_BB_Count + 2, NULL);
  visited = BB_SET_ClearD(visited);
 ...
 * Entry_BB_FB_Value
 *
 * Return the FB_FREQ entry holding feedback information for entry
 * block 'bb'. Return NULL if none found.
 *
 * ====================================================================
 */
static FB_FREQ
Entry_BB_FB_Value(BB *bb)
{
...
// Similar to FREQ_Find_Never_BBs(), but simpler !
static BB_SET *FREQ_Find_Frequent_BBs(MEM_POOL *pool, BOOL do_dom)
{
  BB_SET *pragma_bbs;
  BB_SET *frequent_bbs;
  BB *bb;
  ...
  Find_Freq_Hint_Pragmas(NULL, &pragma_bbs, pool);
  ...
  frequent_bbs = BB_SET_Create_Empty(PU_BB_Count + 2, pool);
       ...
       pragma_bb = BB_SET_Choose_Next(pragma_bbs, pragma_bb))
  {
    for (bb = REGION_First_BB; bb; bb = BB_next(bb)) {
      ...
      if (equiv_rev || equiv_fwd) {
        frequent_bbs = BB_SET_Union1D(frequent_bbs, bb, pool);
      }
    }
  }
 ...
 * ====================================================================
 */
BB_SET *FREQ_Find_Never_BBs(MEM_POOL *pool, BOOL do_dom)
{
#if 1
  BB_SET *pragma_bbs;
  BB_SET *never_bbs;
  ...
  /* 
     Treat the other branch of FREQUENT as NEVER.
     For example:
     ...
     The 'else' branch is treated as though 'else' has #pragma NEVER 
   */
  frequent_bbs = FREQ_Find_Frequent_BBs(pool, FALSE);
  if (frequent_bbs) 
  {
    if (pragma_bbs == NULL) {
      pragma_bbs = BB_SET_Create_Empty(PU_BB_Count + 2, pool);
    }

    for (bb = BB_SET_Choose(frequent_bbs);
         bb != BB_SET_CHOOSE_FAILURE;
         bb = BB_SET_Choose_Next(frequent_bbs, bb))
    {
      BB *pred = BB_Unique_Predecessor (bb);
      BB *other_branch_bb;
      if ((pred != NULL) && (BB_succs_len(pred) == 2)) {
        other_branch_bb = BB_Other_Successor(pred, bb);
        if ( (BB_Unique_Predecessor(other_branch_bb) == pred) &&
             !BB_SET_MemberP(frequent_bbs, other_branch_bb) ) {
          pragma_bbs = BB_SET_Union1D(pragma_bbs, other_branch_bb, pool);
        }
      }
  
      BB *succ = BB_Unique_Successor (bb);
      if ((succ != NULL) && (BB_preds_len(succ) == 2)) {
        other_branch_bb = BB_Other_Predecessor(succ, bb);
        if ( (BB_Unique_Successor (other_branch_bb) == succ) &&
             !BB_SET_MemberP(frequent_bbs, other_branch_bb) ) {
          pragma_bbs = BB_SET_Union1D(pragma_bbs, other_branch_bb, pool);
        }
      }
  ...
  if ( (pragma_bbs == NULL) || BB_SET_EmptyP(pragma_bbs) ) {
    if (do_dom) {
      Free_Dominators_Memory();
    }
    return NULL;
  ...
  do {
	...
	BOOL equiv_fwd =    BB_SET_MemberP(BB_pdom_set(bb), pragma_bb)
		         && BB_SET_MemberP(BB_dom_set(pragma_bb), bb);
	if (equiv_rev || equiv_fwd) {
	  never_bbs = BB_SET_Union1D(never_bbs, bb, NULL);
	}
      }
    }

    pragma_bbs = BB_SET_ClearD(pragma_bbs);
	...
	for (;;) {
	  ...
	  if (edge == NULL) {
	    pragma_bbs = BB_SET_Union1D(pragma_bbs, bb, NULL);
	    goto next_bb;
	  }
	}
      }
  ...
  do {
	...
	nevert_vvs = BB_SET_Union1D(never_bbs, bb, NULL);
 ...
 */
BB_MAP
FREQ_Denormalized_Counts(void)
{
  BB_MAP bb_exec_cnts = BB_MAPFP_Create();
  BB    *bb;

  /* This code is very similar to Normalize_BB_Frequencies. It
     performs the inverse operation to record the actual absolute
     execution counts of each block in 'bb_exec_cnts' map. */
  ...
  if (Compiling_Proper_REGION) {
    ...
    for (i = 0; i < n_entries; i++) {
      ...
      bb = entries[i];
      if (!counted[i]) {
	float entry_freq = BB_freq(bb);
	float entry_exec_cnt = entry_freq;
	if (BB_freq_fb_based(bb)) {
	  if (Cur_PU_Profile) {
	    long long exec_count = Get_Profile_Data(Cur_PU_Profile, BB_id(bb));
	    if (exec_count != -1LL)
	      entry_exec_cnt = exec_count;
	  } else {
	    FB_FREQ fb_entry = Entry_BB_FB_Value(bb);
	    if (fb_entry.Known())
	      entry_exec_cnt = FB_FREQ_Value(fb_entry);
	  }
	}
	...
	for (j = (i + 1) % n_entries; j != i; j = (j + 1) % n_entries) {
	  if (BB_SET_IntersectsP(reachable[i], reachable[j])) {
		...
		if (fb_entry.Known())
	      ...
	      entry_exec_cnt += BB_freq(entries[j]);
	    }
	  }
	}
	if (entry_freq != 0.0) {
	  /* BB *bb gets recycled now */
	  float factor = entry_exec_cnt / entry_freq;
	  FOR_ALL_BB_SET_members(reachable[i], bb) {
            Is_True(BB_freq(bb) >= 0.0F, ("Negative BB frequence found."));
	    BB_MAPFP_Set(bb_exec_cnts, bb, factor * BB_freq(bb));
	  }
	}
      }
    }
  ...
  /* Find the execution count of entry block 'bb'. Use BB_freq value
     if it is estimated. */
  Is_True(BB_freq(bb) >= 0.0F, ("Negative BB frequence found."));
  float entry_freq = (BB_freq(bb) == 0.0) ? 1.0 : BB_freq(bb);
  float entry_exec_cnt = entry_freq;
  if (BB_freq_fb_based(bb)) {
    if (Cur_PU_Profile) {
      long long exec_count = Get_Profile_Data(Cur_PU_Profile, BB_id(bb));
...
)
{
  ...
  if (!FREQ_freqs_computed && !Cur_PU_Feedback) return;

  /* new .frequency directive obviates the current block's
     frequency. */
#if 0
...
)
{
  ...
  float fall_through_freq = 0.0;
  float branch_to_freq = 0.0;

  BB * fall_through_pred = BB_Fall_Thru_Predecessor(bb);

  /* only one predecessor and it's straight line code, so we don't
     need an extra frequency directive here. (Last bb probably
     ended in a call */
  if (fall_through_pred && (BB_preds_len(bb) == 1))
      return;
  ...
  FOR_ALL_BB_PREDS(bb, edge) {
    BB *pred = BBLIST_item(edge);
    BBLIST *succ_edge = BB_Find_Succ(pred, bb);
    float freq = BBLIST_prob(succ_edge) * BB_freq(pred);
    if (fall_through_pred && (pred == fall_through_pred)) {
      Is_True(fall_through_freq == 0, ("More than one fall through predecessor"));
      fall_through_freq = freq;
    }
    else {
      branch_to_freq += freq;
    }
  ...
  if (fall_through_pred == NULL && branch_to_freq == 0.0)
    fall_through_freq = BB_freq(bb);

  Is_True (!isnan(fall_through_freq), ("fall_through_freq was a nan BB id = %d; pred BB = %d\n", BB_id(bb), BB_id(fall_through_pred)));
  Is_True (!isnan(branch_to_freq), ("branch_to_freq was a nan BB_id = %d\n", BB_id(bb)));

  Is_True ((branch_to_freq >= 0.0), ("branch_to_freq was negative (%f) BB_id = %d\n", branch_to_freq, BB_id(bb)));
#if 0
  Is_True ((fall_through_freq >= 0.0), ("fall_through_freq was negative (%f) BB_id = %d\n", fall_through_freq, BB_id(bb)));
  
#else
  if (fall_through_freq < 0)
    return;
#endif
  fprintf(file, "\t%s %#.3f %#.3f\n", AS_FREQ, 
	  fall_through_freq, branch_to_freq);
}
...
FREQ_Incorporate_Feedback(const WN* entry)
{
  ...
  for (BB_LIST *elist = Entry_BB_Head; elist; elist = BB_LIST_rest(elist)) {
    ...
    if (!BB_freq_fb_based(entry_bb)) {
      FB_FREQ fb_entry = Entry_BB_FB_Value(entry_bb);
      if (fb_entry.Known()) {
...
FREQ_Incorporate_Profile()
{
  ...
  /* This is modeled after the FREQ_Incorporate_Feedback() */
  ...
  for (bb = REGION_First_BB; bb != NULL; bb = BB_next(bb)) {
    ...
    float profile_freq = (float)exec_count;
    BB_freq(bb) = profile_freq;
    Set_BB_freq_fb_based(bb);

    LOOP_DESCR *loop;
    LOOPINFO *linfo;

    if (BB_Has_One_Succ(bb)) {
	...
	Set_EDGE_prob_fb_based(edge);
      ...
      int n_succs = 0;
      int n_computed = 0;
      EDGE *unknown_edge = NULL;
      float unknown_prob = 1.0;

      FOR_ALL_SUCC_EDGES(bb, sedge) {
	BB *succ = EDGE_succ(sedge);
	n_succs++;
	if (BB_Has_One_Pred(succ)) {
	  long long succ_exec_count = Get_Profile_Data(Cur_PU_Profile, BB_id(succ));
	  if (succ_exec_count != -1LL) {
	    EDGE_prob(sedge) = float(succ_exec_count)/float(exec_count);
	    Set_EDGE_prob_fb_based(sedge);
	    unknown_prob -= EDGE_prob(sedge);
	    n_computed++;
	  }
	  else
	    unknown_edge = sedge;
      ...
      if (n_succs == n_computed) {
      } else if (n_succs == n_computed+1) {
	EDGE_prob(unknown_edge) = unknown_prob;
	Set_EDGE_prob_fb_based(unknown_edge);
      } else if (n_succs == 2) {
        ...
        EDGE *edge1 = BB_succ_edges(bb);
        EDGE *edge2 = EDGE_next_succ(edge1);
        BB *s1 = EDGE_succ(edge1);
        BB *s2 = EDGE_succ(edge2);

	if (BB_preds_len(s1)==2 && BB_preds_len(s2)==2) {

	  BOOL s1_is_loophead = Is_Loophead_BB(bb, s1);
	  BOOL s2_is_loophead = Is_Loophead_BB(bb, s2);

	  long long s1_exec_count = Get_Profile_Data(Cur_PU_Profile, BB_id(s1));
	  long long s2_exec_count = Get_Profile_Data(Cur_PU_Profile, BB_id(s2));

	  if (s1_is_loophead == s2_is_loophead) {
	  } else if (s1_is_loophead && s1_exec_count == exec_count) {
	    EDGE_prob(edge1) = (float)(s1_exec_count-s2_exec_count)/(float)(exec_count);
	    Set_EDGE_prob_fb_based(edge1);
	    EDGE_prob(edge2) = 1.0 - EDGE_prob(edge1);
	    Set_EDGE_prob_fb_based(edge2);

	    loop = LOOP_DESCR_Find_Loop(s1);
	    linfo = loop ? LOOP_DESCR_loopinfo(loop) : NULL;
	    if (linfo) {
	      WN* wn = LOOPINFO_wn(linfo);
	      if (wn && WN_loop_trip_est(wn) > 0) {
	        WN_loop_trip_est(wn) = s1_exec_count / s2_exec_count;
	      }
	    }
	  } else if (s2_is_loophead && s2_exec_count == exec_count) {
	    EDGE_prob(edge2) = (float)(s2_exec_count-s1_exec_count)/(float)(exec_count);
	    Set_EDGE_prob_fb_based(edge2);
	    EDGE_prob(edge1) = 1.0 - EDGE_prob(edge2);
	    Set_EDGE_prob_fb_based(edge1);

	    loop = LOOP_DESCR_Find_Loop(s2);
	    linfo = loop ? LOOP_DESCR_loopinfo(loop) : NULL;
	    if (linfo) {
	      WN* wn = LOOPINFO_wn(linfo);
	      if (wn && WN_loop_trip_est(wn) > 0) {
	        WN_loop_trip_est(wn) = s2_exec_count / s1_exec_count;
	      }
	    }
	  }
	}
