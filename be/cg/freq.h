 ...
 * Module: freq.h
 * $Revision: 1.21 $
 * $Date: 2000/04/06 01:47:45 $
 * $Author: mtibuild $
 * $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/freq.h,v $
 *
 * Description:
 *
 ...
 *
 *    void FREQ_Incorporate_Profile()
 *	Incorporate CG basic block profile info (in Cur_PU_Profile)
 *	into the CG CFG;
 *
 ...
 *
 *    BB_SET *FREQ_Find_Never_BBs(MEM_POOL *pool, BOOL do_dom=TRUE)
 ...
 *
 *    BB_MAP FREQ_Denormalized_Counts(void)
 *	Return a map that for each BB gives the non-normalized execution
 *      count of the block. If frequencies are being estimated, this is
 *      identical to BB_freq, otherwise it is the absolute execution
 *      count of the block.
 *
 * ====================================================================
 * ====================================================================
...
extern void FREQ_Print_BB_Freq_Directive(
  BB *bb,
  FILE *file
);
...
extern BB_SET *FREQ_Find_Never_BBs(MEM_POOL *pool, BOOL do_dom=TRUE);

extern void FREQ_Incorporate_Feedback(const WN* entry);

extern void FREQ_Incorporate_Profile();

extern BB_MAP FREQ_Denormalized_Counts(void);
