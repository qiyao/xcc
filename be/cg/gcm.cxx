...
{
  if (BB_freq((BB *)bb1) > BB_freq((BB *)bb2))
    return 1;
  else if (BB_freq((BB *)bb1) == BB_freq((BB *)bb2))
    return 0;
  else return -1;
  //  return (BB_freq((BB *)bb1) > BB_freq((BB *)bb2));
}
...
{
  if (BBLIST_prob((BBLIST *)bl1) > BBLIST_prob((BBLIST *)bl2))
    return 1;
  else if (BBLIST_prob((BBLIST *)bl1) == BBLIST_prob((BBLIST *)bl2))
    return 0;
  else
    return -1;
  //return (BBLIST_prob((BBLIST *)bl1) > BBLIST_prob((BBLIST *)bl2));
}
...
sort_by_bb_live_in (const void *bb1, const void *bb2)
{
  ...
  if ((BB_freq((BB *)bb1) * bb1_count) > (BB_freq((BB *)bb2) * bb2_count))
    return 1;
  else if ((BB_freq((BB *)bb1) * bb1_count) == (BB_freq((BB *)bb2) * bb2_count))
    return 0;
  else 
    return -1;
  
  //return ((BB_freq((BB *)bb1) * bb1_count) > (BB_freq((BB *)bb2) * bb2_count));
}
		       ...
		       INT upper_bound)
{
  ...
  INT offset_opnd_num1 = TI_TOP_Find_Operand_Use(OP_code(mem_op1), OU_offset);
  INT offset_opnd_num2 = TI_TOP_Find_Operand_Use(OP_code(mem_op2), OU_offset);
...
inline BOOL Similar_Ptr_Offset_ok(OP *cur_op, OP *deref_op) {
  ...
  INT cur_offset_num = TI_TOP_Find_Operand_Use(OP_code(cur_op), OU_offset);
  INT deref_offset_num = TI_TOP_Find_Operand_Use(OP_code(deref_op), OU_offset);
  INT deref_base_num = TI_TOP_Find_Operand_Use (OP_code(deref_op), OU_base);
  INT cur_base_num = TI_TOP_Find_Operand_Use(OP_code(cur_op), OU_base);
  if ((cur_base_num < 0) || (deref_base_num < 0))
    return FALSE;
  ...
  if (cur_offset_num < 0 && deref_offset_num < 0) {
    ...
    if (defop && OP_iadd(defop) && kind == VAL_KNOWN) {
      ...
      TN *defop_base_tn = OP_opnd(defop, 0);
...
Similar_Ptr_Addrs_Match (OP *pred_op, OP *succ_op)
{
  if (OP_unalign_mem(pred_op) || OP_unalign_mem(succ_op) ||
      OP_unknown_addr(pred_op) || OP_unknown_addr(succ_op))
 	return TRUE;// don't know thereby have to make a conservative estimate

  INT pred_base_num = TI_TOP_Find_Operand_Use(OP_code(pred_op), OU_base);
  INT succ_base_num = TI_TOP_Find_Operand_Use(OP_code(succ_op),  OU_base);
  if ((pred_base_num == -1) || (succ_base_num == -1))
    return TRUE;
  
  TN *pred_base_tn = OP_opnd(pred_op, pred_base_num); 
  TN *succ_base_tn = OP_opnd(succ_op, succ_base_num);
  if (TN_is_constant(pred_base_tn) || TN_is_constant(succ_base_tn))
    return TRUE;
...
OP_Has_Restrictions(OP *op, BB *source_bb, BB *target_bb, mINT32 motion_type)
{
  ...
  if (motion_type & GCM_CIRC_ABOVE) {
     ...
     if (OP_memory(op)) {
       ...
       if (TI_PROC_Property_Set(PROP_has_delayed_exception) && OP_has_predicate(op)) { 
	 return FALSE;
       } else {
	 ...
	 if (base_idx != -1 && TN_is_symbol(OP_opnd(op, base_idx)) && 
	     ST_class(TN_var(OP_opnd(op, base_idx))) == CLASS_VAR && 
	     TY_kind(ST_type(TN_var(OP_opnd(op, base_idx)))) != KIND_ARRAY) 
	   return FALSE;
       }
       return TRUE;
     } 
...
Eager_Ptr_Deref_Spec(OP *deref_op, BB *dest_bb, BOOL forw)
{
  ...
  INT dbase_num = TI_TOP_Find_Operand_Use(OP_code(deref_op), OU_base);
  INT doffset_num = TI_TOP_Find_Operand_Use(OP_code(deref_op), OU_offset);
  if (dbase_num == -1)
    return FALSE;
  ...
  for (cur_op = (forw) ? BB_last_op(dest_bb) : BB_first_op(dest_bb);
    ...
    if (OP_unknown_addr(cur_op)) return FALSE;
    
    // collect all memory references in the <dest> bb 
    
    if (OP_load(cur_op) || OP_store(cur_op)) { 

      INT cbase_num = TI_TOP_Find_Operand_Use(OP_code(cur_op), OU_base);
      INT coffset_num = TI_TOP_Find_Operand_Use(OP_code(cur_op), OU_offset);
      if (cbase_num == -1)
	return FALSE;
...
Null_Ptr_Deref_Spec(OP *deref_op, BB *src, BB *dest)
{
  ...
  INT base_num = TI_TOP_Find_Operand_Use(OP_code(deref_op), OU_base);
  INT offset_num = TI_TOP_Find_Operand_Use(OP_code(deref_op), OU_offset);
  if (base_num == -1)
    return FALSE;

  TN *base_tn = OP_opnd(deref_op, base_num);
  if (TN_is_constant(base_tn))
    return FALSE;

  if ((offset_num != -1) && !TN_has_value(OP_opnd(deref_op, offset_num)))
    return FALSE;
  ...
  if (Ignore_TN_Dep) {
      ...
      if (base_reg == condition_reg && 
	  ((offset_num == -1) || TN_value(OP_opnd(deref_op, offset_num)) >= 0))
	return TRUE;
  } else {
...
Can_Inst_Be_Moved (OP *op, VECTOR succs_vector, INT succ_num)
{
  ...
  for (INT i = 0; i < OP_results(op); ++i) {
    ...
    ISA_REGCLASS result_cl = TN_register_class (result);
    REGISTER result_reg = TN_register (result);
...
{
   BB *cur_bb;
   REGISTER_SET mid_reg_defs[TI_ISA_REGCLASS_MAX+1], 
		mid_reg_uses[TI_ISA_REGCLASS_MAX+1];
   ...
   *spec_type = SPEC_NONE;
   if (motion_type & (GCM_SPEC_ABOVE | GCM_SPEC_BELOW | GCM_CIRC_ABOVE)) {
	 ...
	 if (TI_PROC_Property_Set(PROP_has_delayed_exception)) {
   ...
   if (Ignore_TN_Dep) {
     ...
     mid_gtn_defs = GTN_SET_ClearD (mid_gtn_defs);
     mid_gtn_uses = GTN_SET_ClearD (mid_gtn_uses);
   }
   ...
   FOR_ALL_BB_SET_members (*pred_bbs, cur_bb) {
     ...
     for (op = ((forw && cur_bb != src_bb) || (!forw && cur_bb == src_bb)) ?
	  ...
	  op = ((forw && cur_bb != src_bb) || (!forw && cur_bb == src_bb)) ?
       ...
       // additional target-specific barrier checking 
       if (CGTARG_Dependence_Required(op, cur_op)) return FALSE;
	 ...
	 if (Ignore_TN_Dep) {
       ...
       if (Ignore_TN_Dep && (OP_code(op) == TOP_asm)) {
         ...
         FOR_ALL_ISA_REGCLASS( rc ) {
           ...
           mid_reg_defs[rc] = REGISTER_SET_Union(mid_reg_defs[rc], asm_regs);
         }
       }
...
Update_Live_Use_Counts(BB *cur_bb, OP *cur_op, hTN_MAP *usage_map)
{
  ...
  for (INT i = 0; i < OP_opnds(cur_op); ++i) {
    ...
    if (use_opnd) {
      ...
      counter = (tn_count) ? tn_count 
                           : TYPE_MEM_POOL_ALLOC(UINT32, &MEM_local_pool);
      *counter = ++count;
			...
			VECTOR *priority_vector, VECTOR *cand_bbvector)
{
  ...
  for (INT i = 0; i < VECTOR_count(*priority_vector); i++) {
    ...
    if ((BB_freq(cand_bb) * (Cur_PU_Feedback ?
                             speculation_ratio_fb : speculation_ratio_wfb))
                > BB_freq(bb)) {


        if (!Cur_PU_Feedback && !equiv) {
  ...
  if (success) {
	...
	if (GCM_Loop_Prolog == NULL) {
	  GCM_Loop_Prolog = CG_LOOP_Gen_And_Prepend_To_Prolog(bb, loop);
          Update_REGSETs_For_NewBB (GCM_Loop_Prolog);
...
#pragma mips_frequency_hint NEVER
	...
	if (br_op) {
	  new_bb = CG_LOOP_Append_BB_To_Prolog(GCM_Loop_Prolog, bb);
          Update_REGSETs_For_NewBB (new_bb);
	  GRA_LIVE_Compute_Liveness_For_BB(new_bb);
	  Set_BB_dom_set(new_bb, BS_Create_Empty(2+PU_BB_Count+1, 
						 &gcm_loop_pool));
	  Set_BB_dom_set(new_bb, BS_Union1D(BB_dom_set(new_bb), BB_id(new_bb), &gcm_loop_pool));
	  Set_BB_dom_set(new_bb, BS_UnionD(BB_dom_set(new_bb), BB_dom_set(bb), &gcm_loop_pool));
	    
	  Set_BB_pdom_set(new_bb, BS_Create_Empty(2+PU_BB_Count+1, 
						  &gcm_loop_pool));
	  Set_BB_pdom_set(new_bb, BS_Union1D(BB_pdom_set(new_bb), BB_id(new_bb), &gcm_loop_pool));
	  Set_BB_pdom_set(new_bb, BS_UnionD(BB_pdom_set(new_bb), BB_pdom_set(bb), &gcm_loop_pool));
	  Run_Cflow_GCM = TRUE;
	}
	...
	Set_BB_dom_set(GCM_Loop_Prolog, BS_Create_Empty(2+PU_BB_Count+1, 
							&gcm_loop_pool));
	Set_BB_dom_set(GCM_Loop_Prolog, BS_Union1D(BB_dom_set(GCM_Loop_Prolog), BB_id(GCM_Loop_Prolog), &gcm_loop_pool));
	Set_BB_dom_set(GCM_Loop_Prolog, BS_UnionD(BB_dom_set(GCM_Loop_Prolog), BB_dom_set(bb), &gcm_loop_pool));

	Set_BB_pdom_set(GCM_Loop_Prolog, BS_Create_Empty(2+PU_BB_Count+1, 
							 &gcm_loop_pool));
	Set_BB_pdom_set(GCM_Loop_Prolog, BS_Union1D(BB_pdom_set(GCM_Loop_Prolog), BB_id(GCM_Loop_Prolog), &gcm_loop_pool));
	Set_BB_pdom_set(GCM_Loop_Prolog, BS_UnionD(BB_pdom_set(GCM_Loop_Prolog), BB_pdom_set(bb), &gcm_loop_pool));
...
#pragma mips_frequency_hint NEVER
	...
	for (succ_op = cand_op;
	  ...
	  if (OP_memory(succ_op)) {
	    ...
	    if ((base_opndnum != -1) && (offset_opndnum != -1) &&
		TN_has_value(OP_opnd(succ_op, offset_opndnum))) {
    ...
    if (OP_memory(cand_op)) {
      ...
      if ((base_opndnum != -1) && (offset_opndnum != -1) &&
	  TN_has_value(OP_opnd(cand_op, offset_opndnum))) {
	OP *succ_op;
...
OP_To_Move (BB *bb, BB *tgt_bb, BB_SET **pred_bbs, mINT32 motion_type, mUINT8 *spec_type)
{
  ...
  REGISTER_SET 	reg_defs[TI_ISA_REGCLASS_MAX+1], 
		reg_uses[TI_ISA_REGCLASS_MAX+1],
		failed_reg_defs[TI_ISA_REGCLASS_MAX+1], 
		failed_reg_uses[TI_ISA_REGCLASS_MAX+1];
  ...
  if (Ignore_TN_Dep) {
    ...
    failed_gtn_uses = GTN_SET_ClearD (failed_gtn_uses);
    failed_gtn_defs = GTN_SET_ClearD (failed_gtn_defs);
  }
  ...
  for (cur_op = (forw) ? BB_first_op(bb) : BB_last_op(bb); cur_op; 
    ...
    // Can't move instructions with side-effects and can't move other
    // instructions past them.
    if (OP_side_effects(cur_op)) break;
    ...
    if (Ignore_TN_Dep) {
      ...
      gtn_defs = GTN_SET_ClearD (gtn_defs);
      gtn_uses = GTN_SET_ClearD (gtn_uses);
    }
	      ...
	      BBSCH *new_cand_bbsch, BBSCH *new_bbsch)
{
  ...
  if (OP_has_result(cand_op)) {
    ...
    if (!TN_is_global_reg(result_tn)) {
      ...
      BBSCH_local_regcost(new_bbsch)[cl]--; 
    } else {
      BBSCH_global_regcost(new_cand_bbsch)++;
      BBSCH_global_regcost(new_bbsch)--;
    }
  ...
  for (INT i = 0; i < OP_opnds(cand_op); ++i) {
	...
	BBSCH_global_regcost(new_cand_bbsch)--;
	BBSCH_local_regcost(new_cand_bbsch)[cl]++;
      }
      if (!GTN_SET_MemberP(BB_live_out(bb), opnd_tn))
	BBSCH_global_regcost(new_bbsch)--;
    }
...
/* Return the number of stalls that may be introduced due to
   the dependence between the_op and preds of the_bb.
 */
static UINT32 Stalls_CausedBy_Pred_BB(BB *the_bb, OP *the_op)
{
  /* 
     If the number of the_bb's ops is over 6 (arbitrary number), 
     it is unlikely that the_op can introduce stall between the_bb
     and its pred. So, we return 0.
   */
  INT size = BB_length(the_bb);
  if ( size >= 6 ) {
    return 0;
  }
 
  INT num_tns = OP_opnds(the_op);
  bool tn_opnd_array[num_tns];
  int i;
  OP *op;

  for (i=0; i < num_tns; i++) {
    tn_opnd_array[i] = true;
  }

  /* 
     If an operand of the_op has been ref'ed/def'ed in other OPs of
     of the_bb, the possible dependence introduced by that operand
     of the_op to the predecessor BB of the_BB does not introduce
     any stalls. 
      ...
      the_bb:  
         ...
         tn1 at 2 will not be considered because tn1 of 2 will
         not introduce any stalls between the_bb and its predecessors.
   */
  for (op = OP_prev(the_op); op != NULL; op = OP_prev(op)) {
    for (i=0; i < num_tns; i++) {
      if ( OP_Defs_TN(op, OP_opnd(the_op, i)) ||
           OP_Refs_TN(op, OP_opnd(the_op, i)) ) {
        tn_opnd_array[i] = false;
      }
    }
  }

  BBLIST *bblist;
  FOR_ALL_BB_PREDS(the_bb, bblist) {
    BB *bb = BBLIST_item(bblist);
    // Only consider bb whose fall-thru is the_bb or
    // bb is loop_end bb.
    // Note: skip over empty BB (last_op == NULL)
    OP *last_op = BB_last_op(bb);
    if ( (last_op != NULL) &&
         ((OP_code(last_op) == TOP_loop_end) ||
          ((BB_next(bb) == the_bb) && !BB_loophead(the_bb))) )
    {
      bool is_defined;
      for (i=0; i < num_tns; i++) {
        if (tn_opnd_array[i]) 
        {
          ...
          FOR_ALL_BB_OPs_REV (bb, op) {
            ...
            if (OP_Defs_TN(op, OP_opnd(the_op, i))) {
              is_defined = true;
              break;
            } else if (OP_Refs_TN(op, OP_opnd(the_op, i))) {
              // This operand will not introduce any new dep !
              break;
            }
          }

          // If defining OP isn't too far away (a random magic number 9!)
          // from the bottom, then it likely introduces additional stall.
          if (is_defined && num < 9) {
            int ix = TN_Resnum_In_OP(op, OP_opnd(the_op, i));
            INT16 l = 3;
            if (ix >= 0) {
              INT16 l = TI_LATENCY_Result_Available_Cycle (OP_code(op), ix);
            }
            if ( l >= 2 ) { 
              // simply return 1
              return 1;
                  ...
                  OP *cand_op)
{
  ...
  mINT8  old_from_regcost[TI_ISA_REGCLASS_MAX+1], 
         old_to_regcost[TI_ISA_REGCLASS_MAX+1],
         new_from_regcost[TI_ISA_REGCLASS_MAX+1], 
         new_to_regcost[TI_ISA_REGCLASS_MAX+1];
  ...
  if (Cur_Gcm_Type & GCM_MINIMIZE_REGS) {
    ...
    // TODO: need to update similar ISA_REGCLASS values for global regs
    // as well.
    INT i;
  ...
  // Possible dependence upon the pred BBs of cand_bb, if so,
  // consider it
  UINT32 dep_stall = Stalls_CausedBy_Pred_BB(cand_bb, cand_op);
  ...
  float new_weighted_from = BB_freq(bb) * new_from_time;
  float old_weighted_from = BB_freq(bb) * old_from_time;
  float new_weighted_to = BB_freq(cand_bb) * (new_to_time + dep_stall);
  float old_weighted_to = BB_freq(cand_bb) * old_to_time;
  float new_weighted = new_weighted_from + new_weighted_to;
  float old_weighted = old_weighted_from + old_weighted_to;
  old_weighted = old_weighted + branch_penalty;
  old_weighted = times * old_weighted;
  BOOL worsen_schedule =
    new_weighted > old_weighted || 
  ...
  if (Cur_Gcm_Type & GCM_MINIMIZE_REGS) {
    for (INT i = TI_ISA_Regclass_First(); i <= TI_ISA_Regclass_Last() &&
  ...
  INT ncycles)
{
  ...
  cycle_set = BS_Union1D(cycle_set, icycle, NULL);
...
GCM_For_Loop (LOOP_DESCR *loop, BB_SET *processed_bbs, HBS_TYPE hb_type)
{
  ...
  FOR_ALL_BB_SET_members (loop_bbs, bb) {
    ...
    // calling GCM
      VECTOR_Sorted_Add_Element (bbvector, (void *)bb, 
				 sort_by_bb_frequency);
  ...
  for (INT i = 0; i < count; i++) {
	...
	mUINT8 spec_type;
	  ...
	  TN *old_pred_tn =
            OP_has_predicate(cand_op) ? OP_opnd(cand_op, OP_PREDICATE_OPND) : NULL;
	  ...
	  if ((Ignore_TN_Dep && BB_ALIGNED(new_cand_bbsch) && 
	      ...
	      Is_Schedule_Worse(bb, cand_bb, new_bbsch, new_cand_bbsch,
				old_bbsch, old_cand_bbsch, cand_op)) {
	    ...
	    if (spec_type != SPEC_NONE) {
	      /* reset profile bb id since the speculative move of
		 cand_op can make executed more than its home block
	      */
	      Set_OP_profile_bb_id(cand_op,0);
	    }
...
void GCM_Schedule_Region (HBS_TYPE hbs_type)
{
  ...
  for (bb = REGION_First_BB; bb != NULL; bb = BB_next(bb)) {
    ...
    if (Check_If_Should_Align_BB(bb, INST_BYTES * cur_pc, &num_of_nops) ||
	BB_entry(bb)) {
   ...
   */
  processed_bbs = BB_SET_ClearD(processed_bbs);
