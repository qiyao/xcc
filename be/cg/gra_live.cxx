 ...
 *  Module: gra_live.c
 *  $Revision: 3.104 $
 *  $Date: 2000/05/25 21:26:34 $
 *  $Author: srinivas $
 *  $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/gra_live.cxx,v $
 *
 *  Description:
 *  ============
 *
 ...
 *
 * Need to force them to be defreach_out at Handler entry BBs as well 
 * to prevent them from being converted into local TNs by GRA.  */
GTN_SET *force_live_gtns;

/* We have to force these to be live on exit from each block.  They are
 * the state registers .  If we don't force them, functions with no exits
 * will loose them */
GTN_SET *force_live_states;

/* Show trip count TNs as being live in loops. Set to FALSE after
   CGPREP since trip count tns will thereafter be referred to
   explicitly, or will not be used at all. */
BOOL GRA_LIVE_include_trip_count = TRUE;
...
Compute_Force_TNs(void)
{
  ...
  int state_register_count=0;
  FOR_ALL_ISA_REGCLASS(rc) {
    if (TI_ISA_Regclass_Is_State(rc)) {
      state_register_count+=REGISTER_CLASS_register_count(rc);
    }
  }

  force_live_states = GTN_SET_Create(state_register_count, &liveness_pool);

  FOR_ALL_ISA_REGCLASS(rc) {
    if (TI_ISA_Regclass_Is_State(rc)) {
      // force TIE states live-out initially on all blocks
      REGISTER reg;
      REGISTER_SET regset = REGISTER_CLASS_universe(rc);
      for ( reg = REGISTER_SET_Choose(regset);
	reg != REGISTER_UNDEFINED;
	reg = REGISTER_SET_Choose_Next(regset, reg)) {

	TN* state_tn = Build_Dedicated_TN(rc, reg, 0);
	GTN_UNIVERSE_Add_TN(state_tn);
	force_live_states = GTN_SET_Union1D(force_live_states, state_tn, &liveness_pool);
      }
    }
  }
  ...
  if (Find_Special_Return_Address_Symbol() == NULL && NULL!=RA_TN) {
  ...
  if (Target_ABI == ABI_CALL0) Force_Live_Add(RA_TN);

  if (Caller_FP_TN != NULL) {
    Force_Live_Add(Caller_FP_TN);
  }
}
...
GRA_LIVE_Init_BB_End(BB *bb)
{
  ...
  if (annot && GRA_LIVE_include_trip_count) {
    ...
    TN *mtn = LOOPINFO_max_trip_count_tn(info);
    if ((mtn != NULL) && (mtn != tn) && TN_is_register(mtn)) {
      GTN_UNIVERSE_Add_TN(mtn);
      TN_BB_LIST_MAP_Add(tn_live_use_bbs_map,mtn,bb);
    }
  }
...
Detect_Multiply_Defined_GTNs (GTN_SET *multiple_defined_set, MEM_POOL *pool)
{
  ...
  /* If a var is upward exposed in the entry BB and it is defined later.
     Consider it as multiple-defined TN. For example:
           foo() {
             int a;
             if (a) {
             }
     ...
     See GNAT 16862
   */
  if ( BB_bbregs(REGION_First_BB) != NULL ) {
    TN *tn;
    for (tn  = GTN_SET_Choose(BB_live_in(REGION_First_BB));
         tn != GTN_SET_CHOOSE_FAILURE;
         tn  = GTN_SET_Choose_Next(BB_live_in(REGION_First_BB), tn)) {
      if ( TN_is_global_reg(tn) && TN_MAP_Get (op_for_tn, tn) ) {
        multiple_defined_set = GTN_SET_Union1D (multiple_defined_set, tn, pool);
      }
    }
  }
...
)
{
  ...
  // force TIE states live-out initially on all blocks
  BB_live_out(bb) = GTN_SET_UnionD(BB_live_out(bb), force_live_states, &liveness_pool);
  ...
  if (BB_call(bb)) {
    ...
    BB_live_out(bb) = GTN_SET_Union1D (BB_live_out(bb), SP_TN, &liveness_pool);
    if( GP_TN != NULL ) {
      /* Do this only if the target has a gp */
      GTN_UNIVERSE_Add_TN(GP_TN);
      BB_live_out(bb) = GTN_SET_Union1D (BB_live_out(bb), GP_TN, &liveness_pool);
    }
  }
...
#ifdef TARG_XTENSA
  // Force save reg to be defreach_out to handler entry BBs
  if (BB_handler(bb)) {
    BB_defreach_out(bb) = 
      GTN_SET_UnionD(BB_defreach_out(bb), force_live_gtns, &liveness_pool);
  }
#endif
...
GRA_LIVE_Init( RID *rid )	/* RID is forced to be NULL for PU */
{
  ...
  if (rid) {
   ...
   force_live_states = GTN_SET_Create_Empty(1, &liveness_pool);
  } 
...
)
{
  ...
  if (bb == prolog_bb) {
    for (CG_LOOP_BACKPATCH *bp = prolog_bp; bp != NULL; bp = CG_LOOP_Backpatch_Next(bp)) {
      ...
      tmp_live_def = TN_SET_Difference1D(tmp_live_def,use_tn);
  ...
  FOR_ALL_BB_OPs_FWD (bb, op) {
	...
	if (multiple_defined_set && !TN_is_constant(tn) &&
	  ...
	  // ONLY need to check for defreach_in and defreach_out sets. 
          // We have to do this checking following control flow
          defreach_tn = tn;
