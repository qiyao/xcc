...
//  $Revision: 1.38 $
//  $Date: 2000/04/12 21:44:05 $
//  $Author: mtibuild $
//  $Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_bb.cxx,v $
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_bb.cxx,v $ $Revision: 1.38 $";
#endif
...
/////////////////////////////////////
// Total number of registers from regclass 'rc' local allocator
// would like for this block.
INT 
GRA_BB::Register_Girth( ISA_REGCLASS rc ) 
{
...
/////////////////////////////////////
// Total number of registers from regclass 'rc' local allocator
// could still successfully allocate this block with.
INT 
GRA_BB::Secondary_Register_Girth( ISA_REGCLASS rc ) 
{ return LRA_Secondary_Register_Request(bb,rc); }

/////////////////////////////////////
// Number of registers from regclass 'rc' local allocator
// must have for this block.
INT 
GRA_BB::Must_Register_Girth( ISA_REGCLASS rc ) 
{
  INT req = Register_Girth(rc);
  if (req <= 0)
    return 0;

  /* This block must have at least 'GRA_local_forced_max'
     registers. If this block ends in a branch, there must be at least
     as many locals as operands to the branch. The second requirement
     occurs because of how LRA spills registers, and because spills
     are not allowed in a block after the branch. */

  INT must = 0;
  OP *br = BB_branch_op(bb);
  if (br) {
    for ( INT i = OP_opnds(br) - 1; i >= 0; --i ) {
      TN *tn = OP_opnd(br, i);
      if (TN_is_register(tn) && (TN_register_class(tn) == rc) &&
	  !TN_is_dedicated(tn))
	must++;
    }
  }
  if (rc == TI_ISA_Regclass_Integer())
    return Min(req, Max(GRA_local_forced_max, must));
  else
    return Min(req, must);
}
...
{
  ISA_REGCLASS rc = lunit->Lrange()->Rc();
...
void
GRA_BB::Make_Register_Used( ISA_REGCLASS  rc, REGISTER reg)
{
...
/////////////////////////////////////
REGISTER_SET
GRA_BB::Registers_Used( ISA_REGCLASS  rc)
{
...
GBB_MGR::Initialize(void)
{
  ...
  blocks_with_asm_clobbers = BB_SET_Create_Empty(PU_BB_Count + 2,GRA_pool);
}
...
GBB_MGR::Create(BB* bb, GRA_REGION* region)
{
  ...
  if (BB_asm_clobber(bb))
    blocks_with_asm_clobbers = BB_SET_Union1D(blocks_with_asm_clobbers,bb,GRA_pool);
...
void
GRA_BB::Create_Local_LRANGEs(ISA_REGCLASS rc, INT32 count)
{
  if ( count <= 0 )
    return;
  ...
  INT second_count = Secondary_Register_Girth(rc)-Must_Register_Girth(rc);
  while ( count-- > 0 ) {
    LRANGE* new_local = 
      lrange_mgr.Create_Local(this,rc,((Secondary_Register_Girth(rc) != 0) && 
				       (count >= second_count)));
    local_lranges[rc] =
...
LRANGE*
GRA_BB::Create_Wired_LRANGE(ISA_REGCLASS  rc, REGISTER reg)
{
  LRANGE* result = lrange_mgr.Create_Local(this,rc,FALSE);
...
// <gbb> and <rc>.
INT32
GRA_BB::Global_Live_Lrange_Count(ISA_REGCLASS    rc)
{
