...
#ifdef _KEEP_RCS_ID
static char *gra_bb_rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_bb.h,v $ $Revision: 1.28 $";
#endif
#endif
...
private:
  BB*           bb;
  GRA_REGION*   region;
  LRANGE*       local_lranges[TI_ISA_REGCLASS_MAX+1];
    // Heads of internally linked (through _BB_Local_List) lists of local
    // LRANGEs by ISA_REGCLASS.
  LUNIT*        lunits[TI_ISA_REGCLASS_MAX+1];
    // Heads of internally linked (through _BB_List) list of LUNITs by
    // ISA_REGCLASS.
  INTERFERE     global_lranges[TI_ISA_REGCLASS_MAX+1];
    // All the complement LRANGEs live in the block by register class.
  REGISTER_SET  registers_used[TI_ISA_REGCLASS_MAX+1];
    // the set of registers used in this <gbb> for <rc>.
  REGISTER_SET  glue_registers_used[TI_ISA_REGCLASS_MAX+1];
  ...
  LRANGE_SET*   spill_above[TI_ISA_REGCLASS_MAX+1];
    // set of live ranges spilled at top of <gbb> for <rc>
  LRANGE_SET*   restore_below[TI_ISA_REGCLASS_MAX+1];
    // set of live ranges restored at bottom of <gbb> for <rc>
  LRANGE_LIST*  unpreferenced_wired_lranges[TI_ISA_REGCLASS_MAX+1];
  ...
  mINT8         local_lrange_count[TI_ISA_REGCLASS_MAX+1];
...
public:
  ...
  LRANGE *Local_Lranges(ISA_REGCLASS rc) { return local_lranges[rc]; }
  void Local_Lranges_Set(ISA_REGCLASS rc, LRANGE *lr) 
				{ local_lranges[rc] = lr; }
  INT32 Local_Lrange_Count(ISA_REGCLASS rc) 
				{ return local_lrange_count[rc]; }
  void Incr_Local_Lrange_Count(ISA_REGCLASS rc) 
				{ ++local_lrange_count[rc]; }
  INTERFERE Global_Lranges(ISA_REGCLASS rc) { return global_lranges[rc]; }
  ...
  LRANGE_SET* Restore_Below(ISA_REGCLASS rc) { return restore_below[rc]; }
  LRANGE_SET* Spill_Above(ISA_REGCLASS rc) { return spill_above[rc]; }
  LRANGE_LIST* Unpreferenced_Wired_LRANGEs( ISA_REGCLASS rc ) {
				    return unpreferenced_wired_lranges[rc]; }
  LRANGE_LIST* Unpreferenced_Wired_LRANGEs_Set(ISA_REGCLASS rc,
					       LRANGE_LIST *l) {
				    return unpreferenced_wired_lranges[rc] = l;}
  GRA_LOOP* Loop(void) 		{ return loop; }
  void Loop_Set(GRA_LOOP *l) 	{ loop = l; }
  REGISTER_SET Glue_Registers_Used(ISA_REGCLASS rc) {
  				  return glue_registers_used[rc]; }
  void Make_Glue_Register_Used(ISA_REGCLASS rc, REGISTER reg ) {
  ...
  // non-inlined
  INT Register_Girth( ISA_REGCLASS rc );
  INT Secondary_Register_Girth( ISA_REGCLASS rc );
  float Secondary_Cost(void) { return BB_secondary_cost(bb); }
  INT Must_Register_Girth( ISA_REGCLASS rc );
  void Create_Local_LRANGEs(ISA_REGCLASS  cl, INT32 coun);
  LRANGE* Create_Wired_LRANGE(ISA_REGCLASS  cl, REGISTER reg);
  ...
  INT32 Global_Live_Lrange_Count( ISA_REGCLASS  rc);
  ...
  void Make_Register_Used(ISA_REGCLASS rc, REGISTER reg);
  REGISTER_SET Registers_Used(ISA_REGCLASS  rc);
...
class GBB_MGR {
  ...
  BB_SET *blocks_with_asm_clobbers; // BBs with ASMs that clobber hard registers
public:
  GBB_MGR(void) {};
  ~GBB_MGR(void) {};
  ...
  BB_SET *Blocks_With_Asm_Clobbers(void){ return blocks_with_asm_clobbers; }
...
public:
  void Init(GRA_BB *gbb, ISA_REGCLASS rc) { 
