...
//  $Revision: 1.14 $
//  $Date: 2000/04/12 21:44:19 $
//  $Author: mtibuild $
//  $Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_cflow.cxx,v $
...
/////////////////////////////////////
inline BOOL
OP_Is_Moveable_Copy_From_Arg_TN(BB* bb, OP* op)
/////////////////////////////////////
//
//  Is <op> a copy from a dedicated argument TN?
//  
/////////////////////////////////////
{
  INT src_idx = CGTARG_Copy_Operand(op);
  if (src_idx == -1)
    return FALSE;

  TN* tn = OP_opnd(op, src_idx);
  if ( TN_is_dedicated(tn)) {
    ISA_REGCLASS rc = TN_register_class(tn);
    REGISTER_SET func_args = REGISTER_CLASS_function_argument(rc);
    if (REGISTER_SET_MemberP(func_args, TN_register(tn))) {
      /* 'op' is a copy from argument 'tn'. We can move it if 'tn'
	 is not defined by an earlier OP. */
      for (OP *scan = OP_prev(op); scan; scan = OP_prev(scan))
...
/////////////////////////////////////
inline BOOL
OP_Is_Moveable_Copy_To_Return_TN(BB* bb, OP* op)
/////////////////////////////////////
//
//  Is <op> a copy to a dedicated func TN?
//  
/////////////////////////////////////
{
  ...
  if ( TN_is_dedicated(tn)) {
    ISA_REGCLASS rc = TN_register_class(tn);
    REGISTER_SET func_val = REGISTER_CLASS_function_value(rc);
    if (REGISTER_SET_MemberP(func_val, TN_register(tn))) {
      /* 'op' is a copy to result 'tn'. We can move it if 'tn' is not
	 used or defined by a later OP. */
      for (OP *scan = OP_next(op); scan; scan = OP_next(scan))
	if (OP_Defs_TN(scan, tn) || OP_Refs_TN(scan, tn))
	  return FALSE;
...
//  <entry_bbs>.
//
//  For some targets we also want to move all references to argument
//  registers into the new entry block.
//  
/////////////////////////////////////
{
  OP* op;
...
#ifdef TARG_XTENSA
    /* Move any copy from an dedicated argument TN into the new entry
       block. By isolating these copies we get better preferencing
       because we don't get unnecessary interferences between the
       dedicated argument TNs and other TNs. */
    if (OP_Is_Moveable_Copy_From_Arg_TN(bb, op)) 
      BB_Move_Op_To_Start(new_entry, bb, op);
#endif
    op = prev_op;
  }
...
//  
//  For some targets we also want to move all references to return
//  registers into the new exit block.
//  
/////////////////////////////////////
{
  OP* op;
...
#ifdef TARG_XTENSA
    /* Move any copy to a dedicated return value TN into the new exit
       block. By isolating these copies we get better preferencing
       because we don't get unnecessary interferences between the
       dedicated return value TNs and other TNs. */
    if (OP_Is_Moveable_Copy_To_Return_TN(bb, op))
      BB_Move_Op_To_End(new_exit, bb, op);
#endif
    op = next_op;
  }
...
/////////////////////////////////////
{
  ...
  for ( bb = REGION_First_BB; bb != NULL; bb = next_bb ) {
    next_bb = BB_next(bb);
    if ( BB_entry(bb) && !BB_handler(bb) && !BB_exit(bb)) {
  ...
  if ( is_region == FALSE ) {
    for ( elist = Exit_BB_Head; elist; elist = BB_LIST_rest(elist) ) {
      if (!BB_entry(BB_LIST_first(elist)))
...
/////////////////////////////////////
{
  ...
  // BB_freq(new_succ) = BB_freq(bb); freq is updated in Change_Succ
  // and Change_Succ is changing freq for succ
  float old_succ_freq = BB_freq(succ);
  Change_Succ(bb, succ, new_succ);
  Link_Pred_Succ_with_Prob(new_succ, succ, 1.0);
  BB_freq(succ) = old_succ_freq;
