...
//  $Revision: 1.60 $
//  $Date: 2000/05/26 16:01:41 $
//  $Author: dew $
//  $Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_color.cxx,v $
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_color.cxx,v $ $Revision: 1.60 $";
#endif
...
static REGISTER_SET non_prefrenced_regs[TI_ISA_REGCLASS_MAX + 1];
  // Registers that noone prefers yet.

static REGISTER_SET callee_saves_used[TI_ISA_REGCLASS_MAX + 1];
  // Callee saves registers that someone has already used.  This makes it free
  // for someone else to use them.
static REGISTER_SET regs_used[TI_ISA_REGCLASS_MAX + 1]; // statistics only
...
/////////////////////////////////////
{
  ...
  UINT size = lrange->Register_Size();
  REGISTER_SET reg_range = REGISTER_SET_Range(reg, reg+size-1);

  lrange->Allocate_Register(reg);

  if ( lrange->Has_Preference() ) {
    non_prefrenced_regs[rc] =
        REGISTER_SET_Difference(non_prefrenced_regs[rc],reg_range);
  }

  if (!lrange->Has_Wired_Register() && !lrange->Tn_Is_Save_Reg()) {
    regs_used[rc] = REGISTER_SET_Union(regs_used[rc], reg_range);
    if ((REGISTER_SET_IntersectsP(REGISTER_CLASS_callee_saves(rc),reg_range)
#ifdef HAS_STACKED_REGISTERS
       || REGISTER_Is_Stacked_Local(rc, reg)
#endif
	)) 
      callee_saves_used[rc] = 
        REGISTER_SET_Union(callee_saves_used[rc],
			   REGISTER_SET_Intersection(REGISTER_CLASS_callee_saves(rc),reg_range));
  }
}

/////////////////////////////////////
static BOOL
Choose_Best_Register(REGISTER* reg, ISA_REGCLASS rc,
...
/////////////////////////////////////
{
  ...
  if (! REGISTER_Has_Stacked_Registers(rc)) {
    ...
    /* If PU is a leaf (no calls), try to use a small-numbered
       register first instead of caller_saved (large-numbered),
       this is for reducing a unnecessary register save/restore when
       window overflows.
     */
    if (!
         ((Target_ABI == ABI_WINDOWED) && (PU_Has_Calls==false))
       )
    {
...
/////////////////////////////////////
static void
Force_Color_Some_Locals( GRA_REGION* region, ISA_REGCLASS rc )
/////////////////////////////////////
//
//  The first Must_Register_Girth locals in each BB are not
//  represented by LRANGEs since we must allocate a register to each.
//  We'll just choose colors for these right before we start coloring
//  the complement live ranges.
//
/////////////////////////////////////
{
  ...
  for (iter.Init(region); ! iter.Done(); iter.Step()) {
    ...
    for ( i = gbb->Must_Register_Girth(rc); i > 0; --i ) {
      REGISTER reg;
	...
	allowed = REGISTER_SET_Union(allowed, regs_used[rc]);
      ...
      bool done = false;
      while (REGISTER_SET_EmptyP(allowed)==false && done==false) {
	BOOL non_glue_found = FALSE;
	  ...
	  || Choose_Best_Register(&reg,rc,
          ...
          if (GRA_GRANT_Local_Register(gbb,rc,reg)==false) {
	    allowed = REGISTER_SET_Difference1(allowed, reg);
	  } else {
...
/////////////////////////////////////
{
  return (lrange0->Priority() < lrange1->Priority()) ? TRUE :
	 (lrange0->Priority() > lrange1->Priority()) ? FALSE :
	 ((lrange0->Type()!=LRANGE_TYPE_LOCAL &&
	   lrange1->Type()!=LRANGE_TYPE_LOCAL)?
		(TN_number(lrange0->Tn()) < TN_number(lrange1->Tn())):FALSE);
}


/////////////////////////////////////
static void
Initialize_Priority_Queue( LRPRQ* q, GRA_REGION* region, ISA_REGCLASS cl )
/////////////////////////////////////
//
...
/////////////////////////////////////
static void
Simplify_Initialize( GRA_REGION* region, ISA_REGCLASS rc,
                                         LRANGE_CLIST*      cl,
                                         LRANGE_CLIST*      pcl,
                                         LRPRQ*             ready,
                                         LRPRQ*             not_ready )
/////////////////////////////////////
//
//  Get ready for simplification of the given <region> and ISA_REGCLASS,
...
/////////////////////////////////////
{
  ...
  for (iter.Init(region,rc); ! iter.Done(); iter.Step()) {
    ...
    if ( lr->Has_Wired_Register() ) {
      ...
      //      fprintf(TFile, "neighbors %d, reg_count %d, ", lr->Neighbors_Left(), lr->Candidate_Reg_Count());
      //      GRA_Trace_Color_LRANGE("lr",lr);
...
/////////////////////////////////////
static void
Simplify( GRA_REGION* region, ISA_REGCLASS rc, LRANGE_CLIST* cl )
/////////////////////////////////////
//
//  This is essentially Chaitin/Briggs simplification.  The live ranges in the
//  given <region> which want a register from the ISA_REGCLASS <rc> are
  ...
  for (;;) {
    ...
    for (iter.Init(lr, region); ! iter.Done(); iter.Step()) {
      ...
      if ( ! ( nlr->Listed() || nlr->Has_Wired_Register() ) &&
           nlr->Neighbors_Left_Decrement(lr) == nlr->Candidate_Reg_Count() - 1) {
...
static void
Choose_Original_Registers( GRA_REGION* region, ISA_REGCLASS rc )
/////////////////////////////////////
//
...
/////////////////////////////////////
static BOOL
Color_Region_With_Preferences( GRA_REGION *region, ISA_REGCLASS rc )
/////////////////////////////////////
//
...
/////////////////////////////////////
{
  ...
  FOR_ALL_ISA_REGCLASS_REVERSE( rc ) {
    ...
    for (iter.Init(&cl); ! iter.Done(); iter.Step()) {
      ...
      //
      // for XTENSA, do not global register allocate for any br register
      // except for the 1-bit variation
      //
