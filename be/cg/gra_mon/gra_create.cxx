...
//  $Revision: 1.88 $
//  $Date: 2000/10/13 19:10:24 $
//  $Author: lesniak $
//  $Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_create.cxx,v $
//  $Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_create.cxx,v $
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_create.cxx,v $ $Revision: 1.88 $";
#endif
#include <list>
using std::list;
...
#include "gra_cflow.h"
#include "gra_pref.h"
#include "gra_spill.h"		// for  GRA_Divide_Glue_Code()
#include "register.h"
...
/////////////////////////////////////
{
  ...
  if (succ == NULL || BB_freq(bb) == BB_freq(succ) ||
      ...
      // add the following additional condition to check for
      // unique predecessor/successor relation
      // the original freq check above is weak due
      // to rounding in frequency floating point computation
      (BB_succs_len(bb)==1 && BB_preds_len(succ)==1 &&
       BB_Unique_Successor(bb)==succ && BB_Unique_Predecessor(succ)==bb)
     ) {
    return;
  }
   ...
   */
    ...
    for (TN *tn = GTN_SET_Choose(BB_live_use(succ));
      ...
      REGISTER_SET func_val =
        REGISTER_CLASS_incoming_return_value(TN_register_class(tn));
...
/////////////////////////////////////
{
  ...
  if (BB_rotating_kernel(gbb->Bb())) {
    TN *ded_tns[TI_ISA_REGCLASS_MAX + 1][REGISTER_MAX + 1];
    ISA_REGCLASS rc;
    FOR_ALL_ISA_REGCLASS(rc) {
      for (REGISTER r = 0; r <= TI_ISA_Register_Max(); r++)
	ded_tns[rc][r] = NULL;
    }
    BB *bb = gbb->Bb();
  ...
  for (gbb_iter.Init(gra_region_mgr.Complement_Region()); ! gbb_iter.Done();
    ...
    FOR_ALL_ISA_REGCLASS( rc ) {
      if ( gbb->Register_Girth(rc) > 0 )
        gbb->Create_Local_LRANGEs(rc,gbb->Register_Girth(rc) - gbb->Must_Register_Girth(rc));
    }
  }
...
/////////////////////////////////////
{
  ...
  for ( bb = REGION_First_BB; bb != NULL; bb = BB_next(bb) ) {
	...
	lrange_mgr.Get(tn)->Add_Live_BB(gbb);
        ...
        // Do it conservatively, so we will catch the following:
        //     B1  GTN202 = ...
        //         
        //     B2  asm GTN202  : clobber regs         
        //         <GTN202 is dead at this point of B2>
        // GTN202 must not be allocated into clobbered regs
        //
        if (BB_asm_clobber(gbb->Bb()))
	  (lrange_mgr.Get(tn))->Spans_Asm_Clobbers_Set();
      }
    }
...
// For each ISA_REGCLASS, a vector indexed by register number that takes 
...
LRANGE** wired_lranges[TI_ISA_REGCLASS_MAX + 1];
...
/////////////////////////////////////
{
  ...
  FOR_ALL_ISA_REGCLASS( rc ) {
    LRANGE** vec =
      TYPE_MEM_POOL_ALLOC_N(LRANGE*,&MEM_local_nz_pool, TI_ISA_Register_Max() + 1);
    memset(vec, 0,(TI_ISA_Register_Max() + 1) * sizeof(LRANGE*));
...
/////////////////////////////////////
static void
Wired_TN_Reference( GRA_BB* gbb, ISA_REGCLASS rc, REGISTER reg , 
...
/////////////////////////////////////
static BOOL
Complement_TN_Reference( OP* op, TN* tn, GRA_BB* gbb, 
			 LUNIT** lunitp, LRANGE_LIST **wired_locals )
/////////////////////////////////////
//
...
/////////////////////////////////////
{
  ...
  INT res_width     = OP_TN_Width(op,0,TRUE);
  INT opnd_width    = OP_TN_Width(op,CGTARG_Copy_Operand(op),FALSE);
  INT width         = Min(res_width, opnd_width);
  ...
  if ( res_found && op_found && result_lr->Type() != opnd_lr->Type()
	...
	if (TN_is_dedicated(result)) {
	  ISA_REGCLASS rc = TN_register_class(result);
	  REGISTER reg = TN_register(result);
	  result = Build_Dedicated_TN(rc, reg+1, 0);
	}
	if (TN_is_dedicated(opnd)) {
	  ISA_REGCLASS rc = TN_register_class(opnd);
	  REGISTER reg = TN_register(opnd);
	  opnd = Build_Dedicated_TN(rc, reg+1, 0);
	}
	cand = gra_pref_mgr.CAND_Create(result, opnd, &MEM_local_nz_pool);
	pref_list.push_front(cand);
      }
    }
  } else {
...
/////////////////////////////////////
{
  ...
  for (iter.Init(gbb); ! iter.Done(); iter.Step()) {
	...
	GRA_Trace(1, "Renamed GTN%d to TN%d", TN_number(OP_result(xop,i)),
		  TN_number(local_rename));
	...
	GRA_Trace(1, "Renamed GTN%d to TN%d", TN_number(OP_opnd(xop,i)),
		  TN_number(local_rename));
...
/////////////////////////////////////
{
  if (TN_is_gra_homeable(op_tn) && OP_store(op) &&
      ...
      op_tn == OP_opnd(op,TI_TOP_Find_Operand_Use(OP_code(op), OU_storeval)) &&
...
/////////////////////////////////////
{
  ...
  GRA_BB_OP_FORWARD_ITER iter;
  LUNIT* lunit;
  LRANGE_LIST *wired_locals[TI_ISA_REGCLASS_MAX+1];
  hTN_MAP live_data;
  list<GRA_PREF_CAND*> pref_list;
  memset(&wired_locals, 0, (TI_ISA_REGCLASS_MAX+1) * sizeof(LRANGE_LIST*));
  ...
  for (iter.Init(gbb), op_count=1; ! iter.Done(); iter.Step(), op_count++ ) {
	...
	continue;
      INT opnd_width = OP_TN_Width(xop, i, FALSE);
      while (opnd_width-- > 0) {
	...
	if (opnd_width && TN_is_dedicated(op_tn)) {
	  ISA_REGCLASS rc = TN_register_class(op_tn);
	  REGISTER reg = TN_register(op_tn);
	  op_tn = Build_Dedicated_TN(rc, reg+1, 0);
	} else
	  break;
      }
    }
	...
	continue;
      INT res_width = OP_TN_Width(xop, i, TRUE);
      while (res_width-- > 0) {
	...
	if (res_width && TN_is_dedicated(res_tn)) {
	  ISA_REGCLASS rc = TN_register_class(res_tn);
	  REGISTER reg = TN_register(res_tn);
	  res_tn = Build_Dedicated_TN(rc, reg+1, 0);
	} else
	  break;
      }
    }
    ...
    if ( CGTARG_Is_Preference_Copy(xop) )
      ...
      // it is ok for SWP zero-cost loop instruction to reference region live-ranges
      // even it is not a glue copy op
      bool ok = false;
      if (OP_loop_start(xop)) {
	BB* next = BB_next(gbb->Bb());
	if (next) {
          RID* rid=BB_rid(next);
          if (rid && RID_type(rid) == RID_TYPE_swp && RID_level(rid) >= RL_CGSCHED)
	    ok = true;
	}
      }
...
/////////////////////////////////////
{
  ...
  if (GRA_use_old_conflict) {
	...
	interferences = GTN_SET_ClearD(interferences);
...
{
  LRANGE* lrange1;
  ISA_REGCLASS  rc  = lrange->Rc();
  LRANGE_SUBUNIVERSE* sub = region->Subuniverse(rc);
  LRANGE_SET*         set = live_lrange_sets[rc];

  if ( lrange->Type() == LRANGE_TYPE_REGION && lrange->Region() != region )
    return;
...
static LRANGE*
Add_Local_LRANGE_mapping( GRA_BB* gbb, TN*** local_lrange_map, TN* local_tn )
{
  ISA_REGCLASS rc = TN_register_class(local_tn);
  TN** class_map = local_lrange_map[rc];
  int result_index = -1;

  for (int i=0; i<gbb->Local_Lrange_Count(rc); i++) {
    if (class_map[i] = local_tn) {
      result_index = i;
      break;
    } else if (class_map[i] == NULL) {
      result_index = i;
    } 
  }
  Is_True(result_index >= 0, ("Not enough local LRANGEs"));
  class_map[result_index] = local_tn;
  LRANGE_BB_LOCAL_LIST_ITER result_iter;
  result_iter.Init(gbb->Local_Lranges(rc));
  while (result_index > 0) {
    result_iter.Step();
    result_index--;
  }
  return result_iter.Current();
}

static LRANGE*
Remove_Local_LRANGE_mapping( GRA_BB* gbb, TN*** local_lrange_map, TN* local_tn )
{
  ISA_REGCLASS rc = TN_register_class(local_tn);
  TN** class_map = local_lrange_map[rc];
  int result_index = -1;
  ...
  if (result_index == -1) {
    Add_Local_LRANGE_mapping(gbb, local_lrange_map, local_tn);
    return Remove_Local_LRANGE_mapping(gbb, local_lrange_map, local_tn);
  }
  Is_True(result_index >= 0, ("Cannot find local LRANGE mapping"));
  class_map[result_index] = NULL;
  LRANGE_BB_LOCAL_LIST_ITER result_iter;
  result_iter.Init(gbb->Local_Lranges(rc));
  while (result_index > 0) {
    result_iter.Step();
...
static void
Make_Fine_Grained_Complement_Interferences( GRA_BB* gbb )
{
  ISA_REGCLASS rc;
  GRA_BB_LIVE_OUT_LRANGE_ITER live_out_iter;
  GRA_BB_OP_BACKWARD_ITER op_iter;
  ...
  live_lrange_sets =
    TYPE_MEM_POOL_ALLOC_N(LRANGE_SET*,&MEM_local_nz_pool,
                                      TI_ISA_REGCLASS_MAX + 1);
  local_lrange_map =
    TYPE_MEM_POOL_ALLOC_N(TN**,&MEM_local_nz_pool,TI_ISA_REGCLASS_MAX + 1);
  ...
  FOR_ALL_ISA_REGCLASS( rc ) {
    ...
    live_lrange_sets[rc] =
      LRANGE_SET_Create_Empty(sub->Count(), &MEM_local_nz_pool);
    local_lrange_map[rc] =
      TYPE_MEM_POOL_ALLOC_N(TN*,&MEM_local_nz_pool,
			    gbb->Local_Lrange_Count(rc) + 1);
    memset(local_lrange_map[rc], 0,
	   (gbb->Local_Lrange_Count(rc) + 1) * sizeof(TN*));
  }
  ...
  for (op_iter.Init(gbb); ! op_iter.Done(); op_iter.Step()) {
    ...
    for ( i = OP_results(op) - 1; i >= 0; --i ) {
      ...
      LRANGE *result_lrange;
      if ( TN_Is_Allocatable(result) && TN_is_global_reg(result)) {
	if (TN_is_global_reg(result)) {
	  result_lrange = lrange_mgr.Get(result);
	} else {
	  result_lrange = Remove_Local_LRANGE_mapping(gbb, local_lrange_map, 
						      result);
	}
	if (OP_uniq_res(op) || 
	  ...
	  Remove_From_Live_Set(live_lrange_sets,region,result_lrange);
	}
      }
    }
    ...
    for ( i = OP_opnds(op) - 1; i >= 0; --i ) {
      ...
      LRANGE *opnd_lrange;
      if ( TN_Is_Allocatable(opnd) && TN_is_global_reg(opnd)) {
	if (TN_is_global_reg(opnd)) {
	  opnd_lrange = lrange_mgr.Get(opnd);
	} else {
	  opnd_lrange = Add_Local_LRANGE_mapping(gbb, local_lrange_map, opnd);
	}
	Add_To_Live_Set(live_lrange_sets,region,opnd_lrange);
      }
    }
...
static void
Build_Fine_Grained_Complement_Interference_Graph(void)
{
  GRA_REGION *region = gra_region_mgr.Complement_Region();
  GRA_REGION_GBB_ITER gbb_iter;
  GRA_REGION_RC_NL_LRANGE_ITER lrange_iter;
  ...
  for (gbb_iter.Init(region); ! gbb_iter.Done(); gbb_iter.Step()) {
    ...
    Make_Fine_Grained_Complement_Interferences(gbb);
    gbb->Create_Global_Interferences();
  }
  ...
  FOR_ALL_ISA_REGCLASS( rc ) {
	...
	for (iter1.Init(region,rc); ! iter1.Done(); iter1.Step()) {
	  ...
	  if ((lrange1->Type() == LRANGE_TYPE_COMPLEMENT) &&
	      (lrange1 > lrange0) && 
	      Check_Global_Preference(lrange0, lrange1)) {
	    lrange0->Remove_Neighbor(lrange1,region);
	    lrange1->Remove_Neighbor(lrange0,region);
	  }
	}
      }
    }
...
/////////////////////////////////////
{
  ...
  GRA_Init_Trace_Memory();
  if (GRA_new_conflict)
    Build_Fine_Grained_Complement_Interference_Graph();
  else
