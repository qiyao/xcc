...
//  $Revision: 1.25 $
//  $Date: 2000/04/12 21:45:30 $
//  $Author: mtibuild $
//  $Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_lrange.h,v $ 
...
#ifdef _KEEP_RCS_ID
static char *gra_lrange_rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_lrange.h,v $ $Revision: 1.25 $";
#endif
#endif
...
//              spilling.
  ...
  LRANGE_FLAGS_spans_asm_clobbers = 0x8000, // live across ASMs with clobbers
};
...
public:
  ...
  ISA_REGCLASS Rc(void)	{ return rc; }
  ...
  void Clear_Live_BBs(void)	{ u.c.live_bb_set = BB_SET_ClearD(u.c.live_bb_set); }
  GTN_SET* Global_Pref_Set(void){ return u.c.global_pref_set; }
  void Clear_Global_Pref_Set(void) { u.c.global_pref_set = BB_SET_ClearD(u.c.global_pref_set); }
  ...
  BOOL Spans_Asm_Clobbers(void)	{ return flags & LRANGE_FLAGS_spans_asm_clobbers; }
  void Spans_Asm_Clobbers_Set(void)	{ flags = (LR_FLAG)(flags|LRANGE_FLAGS_spans_asm_clobbers); }
  void Spans_Asm_Clobbers_Reset(void)	{ flags = (LR_FLAG)(flags&~LRANGE_FLAGS_spans_asm_clobbers); }
...
//              <head> and return the new head.
  UINT Register_Size() { return reg_size; }
  UINT Register_Alignment() { return reg_align; }
  ...
  void Add_Neighbor(LRANGE* neighbor, GRA_REGION* region );
  ...
  INT32 Neighbors_Left_Increment(LRANGE* lrange1);
  INT32 Neighbors_Left_Decrement(LRANGE* lrange1);
  REGISTER_SET Fix_Allowed_Set_For_Multi_Registers(REGISTER_SET allowed);
};
...
//              NULL if there isn't.
  ...
  void Finalize(void);
  LRANGE* Create( LRANGE_TYPE type, ISA_REGCLASS rc, size_t size );
  LRANGE* Create_Complement( TN* tn );
  LRANGE* Create_Local( GRA_BB* gbb, ISA_REGCLASS cl, BOOL secondary_local );
...
extern REGISTER_SET Register_Restrictions(LRANGE* lrange, GRA_BB* gbb);
