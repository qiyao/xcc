...
#ifdef _KEEP_RCS_ID
static char *gra_region_rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_region.h,v $ $Revision: 1.15 $";
#endif
#endif
...
friend class GRA_REGION_MGR;
  ...
  LRANGE_SUBUNIVERSE* subuniverse[TI_ISA_REGCLASS_MAX+1];
  ...
  INT32 lrange_count[TI_ISA_REGCLASS_MAX+1];
  ...
  REGISTER_SET registers_available[TI_ISA_REGCLASS_MAX+1];
...
public:
  ...
  LRANGE_SUBUNIVERSE* Subuniverse(ISA_REGCLASS r) {return subuniverse[r];}
  INT32 Lrange_Count(ISA_REGCLASS rc) { return lrange_count[rc]; }
  REGISTER_SET Registers_Used(ISA_REGCLASS rc){return registers_used[rc];}
  ...
  REGISTER_SET Registers_Available(ISA_REGCLASS rc){return registers_available[rc];}
  void Make_Register_Unavailable(REGISTER reg, ISA_REGCLASS rc) {
  ...
  BOOL Is_Loop(void)			{ return TRUE; }
		    // Is this a loop region?
                    ...
                    // from region_util.
  INT32 Callee_Saves_Registers_Available_Count(ISA_REGCLASS rc) {
...
public:
  ...
  void Init( GRA_REGION* region, ISA_REGCLASS rc) {
