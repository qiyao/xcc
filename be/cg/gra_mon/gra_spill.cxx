...
//  $Revision: 1.75 $
//  $Date: 2000/05/19 20:14:01 $
//  $Author: murthy $
//  $Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_spill.cxx,v $
...
/////////////////////////////////////
{
  ...
  for (iter.Init(gbb); ! iter.Done(); ) {
	...
	if (alias == SAME_LOCATION) {
	  ...
	  //
	  if (OP_store(op)) {
            INT st_op_num = TI_TOP_Find_Operand_Use(OP_code(op), OU_storeval);
            if (st_op_num >= 0 && OP_opnd(op, st_op_num) == orig_tn) {
              op_is_homing_store = TRUE;
              if (!need_store) {
                OP *remove_store = (prev_store != NULL ? prev_store : op);
                GRA_Trace_Home_Removal(orig_tn, gbb, remove_store);
                BB_Remove_Op(gbb->Bb(), remove_store);
              }
            } else {
              need_store = FALSE;
              prev_store = op;
...
//  regions.  On return the lists region_entry_glue_bbs and
//  region_exit_glue_bbs contain these blocks.
//
/////////////////////////////////////
{
  BB* bb;
  ...
  for ( bb = REGION_First_BB; bb != NULL; bb = BB_next(bb) ) {
	...
	if (RID_parent(BB_rid(succ_bb)) == BB_rid(bb)){
      ...
      if (BB_has_glue_copy(bb)) {

	pred_bb = BB_Fall_Thru_Predecessor(bb);
	succ_bb = BB_Fall_Thru_Successor(bb);
	RID* rid=NULL;

	if (!known_entry && succ_bb &&
	    (rid=BB_rid(succ_bb)) && RID_type(rid) == RID_TYPE_swp) {
	  Is_True(BB_succs_len(bb)==2,
		  ("More than 2 successors for region entry"));
	  region_entry_glue_bbs = BB_LIST_Push(bb,region_entry_glue_bbs,GRA_pool);
	}
	if (!known_exit && pred_bb &&
	    (rid=BB_rid(pred_bb)) && RID_type(rid) == RID_TYPE_swp) {
	  Is_True(BB_preds_len(bb)==2,
		  ("More than 2 predecessors for region exit"));
	  region_exit_glue_bbs = BB_LIST_Push(bb,region_exit_glue_bbs,GRA_pool);
	}
	Is_True(pred_bb || succ_bb, ("Missing pred/succ for glue block"));
      }
    }
  }
}
...
/////////////////////////////////////
{
  ...
  GRA_REGION* region = gbb_mgr.Get(BB_Fall_Thru_Predecessor(bb))->Region();
  BB* next_bb = BB_Fall_Thru_Successor(bb);
  TN_MAP map = TN_MAP_Create();

  OP* bb_last_op = BB_last_op(bb);

  // if the current exit block is merged with entry block to the next
  // SWP region, the loop instruction is referencing region lrange
  // of a different region and needs to be skipped
  if (next_bb) {
    GRA_REGION* next_region = gbb_mgr.Get(next_bb)->Region();
    RID* rid=next_region->Rid();
    if (rid && RID_type(rid) == RID_TYPE_swp && RID_level(rid) >= RL_CGSCHED)
      bb_last_op = OP_prev(bb_last_op);
  }

  for (op = bb_last_op; op != NULL; op = prev_op ) {
    prev_op = OP_prev(op);
	...
	if (   Complement_TN(result) && TN_register(result)
...
/////////////////////////////////////
{
  ...
  GRA_REGION* region = gbb_mgr.Get(BB_Fall_Thru_Successor(bb))->Region();

  OP* loop_op = NULL;
  OP* loop_tn_op = NULL;
  bool swp_entry= false;
  RID* rid=region->Rid();
  if (rid && RID_type(rid) == RID_TYPE_swp && RID_level(rid) >= RL_CGSCHED)
    swp_entry = true;

  // for SWP region, the trip count has been allocated and a glue copy
  // is generated
  // we need to temporarily remove the loop instruction so
  // a temp TN is not used when appending the glue copy back
  if (swp_entry) {
    OP* prev_op;
    loop_op = BB_last_op(bb);
    prev_op = OP_prev(loop_op);

    // if the trip count tn was set right before the loop instruction
    // then keep them together
    if (prev_op && OP_glue(prev_op) &&
	OP_result(prev_op,0)==OP_opnd(loop_op,0)) {
      TN* rhs = OP_opnd(prev_op,0);
      if (!TN_is_global_reg(rhs)) {
	loop_tn_op = prev_op;
	BB_Remove_Op(bb, loop_tn_op);
      }
    }
    BB_Remove_Op(bb, loop_op);
  }
  ...
  for ( op = BB_first_op(bb); op != NULL; op = next_op ) {
	...
	for ( i = OP_opnds(op) - 1; i >= 0; --i ) {
          ...
          if (    Complement_TN(operand) && TN_register(operand)
               || TN_is_dedicated(operand) && !
		  (TN_is_zero(operand) || TN_is_true_pred(operand) ||
		   TN_register_class(operand)==TI_ISA_Regclass_Special() ||
		   TI_ISA_Regclass_Is_State(TN_register_class(operand)))
          ) {

	    Is_True(REGISTER_allocatable(TN_register_class(operand), TN_register(operand)),
			("Cannot copy non-allocatable register"));
  ...
  // restore the temporarily removed loop instruction
  if (swp_entry) {
    if (loop_tn_op) {
      BB_Append_Op(bb, loop_tn_op);
    }
    BB_Append_Op(bb, loop_op);
  }
}
...
/////////////////////////////////////
{
  ...
  for ( bb_list = region_entry_glue_bbs; bb_list != NULL;
    ...
    BB* glue_bb = BB_LIST_first(bb_list);

    BB_Split_Entry_Glue(glue_bb);
  }
...
//      3. No predecessor ends in a branch requiring exactly as many
//      registers as are granted to the block, and the reload
//      targets one of those registers.
//
...
//
//  Condition 3 arises because of changes made for the xtensa
//  port. When a block ends in a branch, LRA may request just enough
//  registers to allow it to allocate. For example, if the block ends
//  in a branch with two register operands, LRA requires at least 2
//  granted registers to allocate the block. If GRA grants LRA only 2
//  registers, and we move a load into the block whose result is one
//  of those registers, then LRA will no longer be able to allocate
//  the block (I believe mips doesn't have this problem because GRA
//  always grants at least 4 registers to LRA, and no mips branch
...
#ifdef TARG_XTENSA
      /* Condition 3 described above. I think this is conservative
         because GRA_Local_Register_Grant doesn't necessarily have all
         the granted registers yet.... but I don't know how else to
         determine the grant at this point. */
      if (gbb->Must_Register_Girth(rc) <= REGISTER_SET_Size(GRA_Local_Register_Grant(bb, rc))) {
	REGISTER_SET grant = GRA_Local_Register_Grant(bb, rc);
	if (REGISTER_SET_MemberP(grant, reg))
	  return FALSE;
      }
#endif
      load_count++;
...
//         allocated region.
//	4. The allocated register for LRANGE is not used in any of the
//	   successor.
//
...
/////////////////////////////////////
{
  ...
  INT spill_count = 0;
  ISA_REGCLASS rc = lrange->Rc();
  BOOL allocated = lrange->Allocated() || lrange->Has_Wired_Register();
  ...
  for (iter.Succs_Init(lunit->Gbb()); ! iter.Done(); iter.Step()) {
    ...
    if ( ! Needs_Spill(lrange,gbb) )
      ...
      return FALSE;
	     ...
	     REGISTER_SET_MemberP(gbb->Registers_Used(rc), lrange->Reg()))
      return FALSE;
...
////////////////////////////////////////////////////////////////////////////
// Move glue copies in region entry/exit to the end/begin of the block
void
GRA_Divide_Glue_Code(void)
{
