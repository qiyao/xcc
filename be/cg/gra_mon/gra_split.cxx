...
//  $Revision: 1.68 $
//  $Date: 2000/05/19 20:14:17 $
//  $Author: murthy $
//  $Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_split.cxx,v $


#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/cg/gra_mon/RCS/gra_split.cxx,v $ $Revision: 1.68 $";
#endif
...
/////////////////////////////////////
static void
Print_Globals(GRA_BB* gbb, ISA_REGCLASS rc)
/////////////////////////////////////
//
...
/////////////////////////////////////
static void
Print_Globals_N(INT n, ISA_REGCLASS rc)
/////////////////////////////////////
//
...
/////////////////////////////////////
static REGISTER_SET
Regs_Used( TN* tn, GRA_BB* gbb, ISA_REGCLASS rc )
/////////////////////////////////////
//
...
/////////////////////////////////////
{
  ...
  // duplicate change 90921 here to exclude the register used by
  // pre-allocated trip-count tn for a zero-cost (normally SWP'ed) loop
  // it may be included because of a preferencing copy
  BB* bb = gbb->Bb();
  OP* loop_op = BB_last_op(bb);

  // check if the loop instructions pre-allocated a register
  // if yes, include that register in the used set
  // that register could be in the bb's used set already
  ...
  if (BB_asm_clobber(gbb->Bb())) {
    ...
    used = REGISTER_SET_Union(used, BB_Asm_Clobbers_Regs(gbb->Bb(), rc));
  }
  return used;
}
...
/////////////////////////////////////
{
  ...
  BOOL                found       = FALSE;
  ISA_REGCLASS  rc          = split_lrange->Rc();
  ...
  for (iter.Init(split_lrange); ! iter.Done(); iter.Step()) {
    ...
    if ( lunit->Has_Exposed_Use() || lunit->Has_Def() ) {
      ...
      allowed = split_lrange->Fix_Allowed_Set_For_Multi_Registers(allowed);
...
Avoid_Unit_Spill(GRA_BB* gbb, REGISTER_SET allowed_regs,
		 ...
		 ISA_REGCLASS rc, GRA_LOOP *maxloop)
/////////////////////////////////////
//
...
/////////////////////////////////////
{
  ...
  allowed_regs = 
    REGISTER_SET_Intersection(allowed_regs, 
			      Register_Restrictions(split_lrange, lunit->Gbb()));
  ...
  while ( GBBPRQ_Size(&gbbprq) > 0 ) {
    ...
    REGISTER_SET    gbb_allowed =
			REGISTER_SET_Intersection(allowed_regs, 
				Register_Restrictions(split_lrange, gbb));
    gbb_allowed = REGISTER_SET_Difference(gbb_allowed,regs_used);

    gbb_allowed=split_lrange->Fix_Allowed_Set_For_Multi_Registers(gbb_allowed);

    if ( REGISTER_SET_EmptyP(gbb_allowed) ||
	Avoid_Unit_Spill(gbb, allowed_regs, regs_used, &loop_allowed, rc,
			 lunit->Gbb()->Loop()) ) {

      // a SWP scheduled/allocated region cannot be a deferred border
      // which needs new register for spilling/restoring which needs
      // allocation again. So force faiure by returning 1. See pr9512.
      RID* rid=BB_rid(gbb->Bb());
      if (rid && RID_type(rid) == RID_TYPE_swp && RID_level(rid) >= RL_CGSCHED)
	return 1;

      // Similarly, a SWP prolog cannot be a boundary either since that
      // requires reload in the already allocated SWP kernel (PR11235)
      BB* bb = gbb->Bb();
      bb = BB_next(bb);
      if (bb) {
        rid=BB_rid(bb);
	...
	allowed_regs = REGISTER_SET_Difference(gbb_allowed,regs_used);
      }
...
/////////////////////////////////////
{
  ...
  if (GRA_new_conflict) {
    TN* neighbor;
    for ( neighbor = GTN_SET_Choose(result);
	  neighbor != GTN_SET_CHOOSE_FAILURE;
	  neighbor = GTN_SET_Choose_Next(result,neighbor)) {
      LRANGE* lrange1 = lrange_mgr.Get(neighbor);
      if ((lrange1 != NULL) && (lrange1->Type() == LRANGE_TYPE_COMPLEMENT) &&
	  !lrange->Interferes(lrange1))
	result = GTN_SET_Difference1D(result,neighbor);
    }
  }
...
/////////////////////////////////////
{
  ...
  if (GRA_new_conflict)
    alloc_lrange->Initialize_Region_Inteference(cregion);
  else
  ...
  for (iter.Init(split_lrange,gra_region_mgr.Complement_Region()); 
    ...
    if ( ! Interferes(deferred_lrange,deferred_interfering_gtns,neighbor) ) {
      ...
      if ( REGISTER_SET_MemberP(neighbor_used, neighbor->Reg()) )
        deferred_lrange->Neighbors_Left_Decrement(neighbor);
      else {
      ...
      neighbor->Neighbors_Left_Increment(deferred_lrange);
    ...
    if ( Interferes(alloc_lrange,alloc_interfering_gtns,neighbor) ) {
      if (GRA_new_conflict) {
	alloc_lrange->Region_Interference(neighbor,cregion);
	neighbor->Region_Interference(alloc_lrange,cregion);
      } else
...
/////////////////////////////////////
{
  ...
  for (dup_iter.Init_Following(client_iter); ; dup_iter.Step()) {
    ...
    if ( lrange->Interferes(deferred_lrange) ) {
      lrange->Neighbors_Left_Decrement(alloc_lrange);
      deferred_lrange->Neighbors_Left_Increment(lrange);
...
/////////////////////////////////////
static void
Fix_Asm_Clobbers_Info( LRANGE* lrange )
/////////////////////////////////////
//
// Check whether lrange still spans a BB that contains ASM with clobbers
//
/////////////////////////////////////
{
  // looping over the GRA_BBs that have ASM with clobbers
  class LRANGE_ASM_CLOBBERS_BB_ITER {
  private:
    BB_SET *set;
    BB *current;
  public:
    LRANGE_ASM_CLOBBERS_BB_ITER(void) {}
   ~LRANGE_ASM_CLOBBERS_BB_ITER(void) {}

    void Init(LRANGE *lrange)	{ set = lrange->Live_BB_Set(); 
                                  current = BB_SET_Intersection_Choose(set, 
					gbb_mgr.Blocks_With_Asm_Clobbers()); }
    BOOL Done(void)		{ return current == BB_SET_CHOOSE_FAILURE; }
    BB *Current(void)		{ return current; }
    void Step(void)		{ current = BB_SET_Intersection_Choose_Next(set,
					gbb_mgr.Blocks_With_Asm_Clobbers(),
  ...
  lrange->Spans_Asm_Clobbers_Reset();

  for (iter.Init(lrange); !iter.Done(); iter.Step()) {
    BB *bb = iter.Current();
    if (BB_asm_clobber(bb)) {
...
/////////////////////////////////////
{
  ...
  Fix_Asm_Clobbers_Info(deferred_lrange);
  Fix_Asm_Clobbers_Info(alloc_lrange);
