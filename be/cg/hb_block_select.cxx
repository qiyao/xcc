...
// and <hb_bbs> is the container set of BBs in the hyperblock. <cycle_found>
// flag is set to TRUE if an internal cycle is detected.
//
// =======================================================================
static void
Verify_HB(
  ...
  BOOL *cycle_found)
{
  ...
  // If an internal cycle found, return.
  if (*cycle_found) return;

  // If <bb> is not part of HB, return. Don't do further processing.
  if (!BB_SET_MemberP(hb_bbs, bb)) return;

  // No need to check if <bb> is an exit block.
  if (bb == HB_Exit(hb)) return;

  // If <bb> has been processed before, this indicates a internal cycle.
  // Set the <cycle_found> flag to TRUE and return;
  if (BB_SET_MemberP(processed_bbs, bb)) {
    *cycle_found |= TRUE;
    return;
  }
  ...
  FOR_ALL_BB_SUCCS(bb, edge) {
    ...
    if (BB_SET_MemberP(hb_bbs, succ)) {
      Verify_HB(hb, succ, processed_bbs, hb_bbs, cycle_found);
    }
  }
}
...
/////////////////////////////////////
{
  ...
  //
  if (max_priority > 0.0 &&
      ((HB_static_freq_heuristics && !Cur_PU_Feedback) || profitable_ifc)) {
    for (hbp = hb_paths.begin(); hbp != hb_paths.end(); hbp++) {
      HB_PATH* path = *hbp;
...
/////////////////////////////////////
{
  if (tolerate_increase) {
    if (HB_static_freq_heuristics && !Cur_PU_Feedback) {
      return (INT) ((float) HB_PATH_Schedule_Height(max_path) *
		    max_sched_growth_sf);
    }
    return (INT) ((float) HB_PATH_Schedule_Height(max_path) *
		  max_sched_growth);
  }
  return HB_PATH_Schedule_Height(max_path);
}
...
/////////////////////////////////////
{
  ...
  if (HB_skip && BB_id(HB_Entry(candidate)) == HB_skip_entry_bb) return FALSE;
  ...
  FOR_ALL_BB_SET_members(hb_bbs, cur_bb) {
    if (BS_MemberP(processed_bbs, BB_id(cur_bb))) continue;
    if (cycle_found) break;
    Verify_HB(candidate, cur_bb, processed_bbs, hb_bbs, &cycle_found);
  }
