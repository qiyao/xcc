 ...
 *  Module: hb_hazards.h
 *  $Revision: 1.5 $
 *  $Date: 2000/04/28 21:59:03 $
 *  $Author: srinivas $
 *  $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/hb_hazards.h,v $
 *
 *  Description:
 *  ============
 *
...
BB_last_real_op(BB *bb) {
  ...
  for (last_op = BB_last_op(bb); last_op; last_op = OP_prev(last_op)) {
    if (!OP_dummy(last_op) && !OP_pseudo(last_op)) break;
  }
  return last_op;
}
...
Last_Real_OP(OP *op)
{
  ...
  for (last_real_op = OP_prev(op); last_real_op; last_real_op = OP_prev(last_real_op)) {
    if (!OP_dummy(last_real_op) && !OP_pseudo(last_real_op)) break;
  }
...
extern void Bundle_Only_Full_Flix(BB *bb);
extern void Bundle_For_Space(BB *bb);

extern BOOL Is_Delay_Slot_Op (OP *op, BB *bb);
extern void Add_Scheduling_Note (BB *bb, void *bbsch);

extern TI_RES_RES* HB_Hazard_resource_table;
extern INT HB_Hazard_current_cycle;
