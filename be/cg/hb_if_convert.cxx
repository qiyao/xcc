			      ...
			      BB_MAP true_edges)
{
  ...
  FOR_ALL_BB_SET_members(hb_blocks, bb) {
	...
	bb_to_add = BB_SET_CopyD(bb_to_add,BB_pdom_set(bb_succ),NULL);  // better not reallocate
	...
	FOR_ALL_BB_SET_members(bb_to_add, bb_dep) {
	  ...
	  deps = BB_SET_Union1D(deps, bb, &MEM_local_pool);
	  if (true_edge) trues = BB_SET_Union1D(trues, bb, &MEM_local_pool);
	}
      }
    }
  }
...
/////////////////////////////////////
{
  ...
  if (BB_asm_clobber(bb_second)) {
    Set_BB_asm_clobber(bb_first);
  }
...
/////////////////////////////////////
static void
Predicate_Block(BB* bb, TN *pred_tn)
/////////////////////////////////////
//
  ...
  if (pred_tn && pred_tn != True_TN) {
	...
	if (TN_is_global_reg(ptn) && ptn != pred_tn) {
		if ( PQSCG_is_disjoint(pred_tn, ptn)) {
			...
			// branch will probably get removed later so ignore
		}
		else if (OP_icmp(op)) {
			// ?? cmp probably means have already taken into
			// account the multiple predicates.
		}
		else {
			DevWarn("and global predicate to current predicate");
  			OPS ops = OPS_EMPTY;
    			Exp_Generic_Pred_Calc (ptn, True_TN, COMPARE_TYPE_and, pred_tn, &ops);
  			BB_Insert_Ops(bb, op, &ops, TRUE);
		}
	}
      }
    } // for
...
// pred_tn. fall_thru_prog is the probability that the goto is executed
		    ...
		    BOOL last_block)
{
  ...
  if (pred_tn && pred_tn != True_TN) {
    ...
    Predicate_Block(fall_thru_goto,pred_tn);
		...
		list<HB_CAND_TREE*>& candidate_regions)
{
   ...
   for (idx=0; idx < block_order.size(); idx++) {
     ...
     case CASE_ALL_IN_HB:
       ...
       // predicate it. 
       BB_Remove_Branch(bb);
       hb_continuation_prob = Get_Block_Prob(bb,freq_map);
       break;
	 ...
	 fall_thru_goto = Make_Fall_Thru_Goto(bb,fall_thru,NULL,
					      1.0 - Get_Branch_Prob(bb,branch_bb,freq_map),
