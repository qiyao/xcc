...
* Module: hb_sched.h
* $Revision: 1.24 $
* $Date: 2000/06/12 21:04:14 $
* $Author: dew $
* $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/hb_sched.h,v $
*
* Description:
*
...
#define HBS_IGNORE_LATENCY		0x4000
#define HBS_LOCAL_SCHED			0x8000
...
typedef struct {
  mUINT16 num_succs;
  mUINT16 num_preds;
  mUINT16 num_unique_succs;
  mUINT16 num_unique_preds;
  mINT16  issue_alignment;
  mUINT16 estart;
  mUINT16 lstart;
  mUINT16 scycle;
  mUINT16 estart_offset;   // latency with the pred BB
  mUINT16 lstart_offset;   // latency with the succ BB
  mUINT16 dfsnum;
  mINT16  regcost;
  mUINT16 flags;
  mUINT16 min_usage;
  ...
  mUINT16 num_splits;
  mINT16 live_vals;
  mINT16 new_live_vals;
  mUINT16 live_op_mask;
  BOOL	  use_shared_resource;
} OPSCH;

#define OPSCH_num_succs(opsch)	((opsch)->num_succs)
#define OPSCH_num_preds(opsch)	((opsch)->num_preds)
#define OPSCH_num_unique_succs(opsch)	((opsch)->num_unique_succs)
#define OPSCH_num_unique_preds(opsch)	((opsch)->num_unique_preds)
#define OPSCH_issue_alignment(opsch)	((opsch)->issue_alignment)
#define OPSCH_use_shared_resource(opsch)	((opsch)->use_shared_resource)
#define OPSCH_estart(opsch)	((opsch)->estart)
#define OPSCH_lstart(opsch)	((opsch)->lstart)
#define OPSCH_scycle(opsch)	((opsch)->scycle)
#define OPSCH_estart_offset(opsch)	((opsch)->estart_offset)
#define OPSCH_lstart_offset(opsch)	((opsch)->lstart_offset)
#define OPSCH_dfsnum(opsch)	((opsch)->dfsnum)
#define OPSCH_regcost(opsch)	((opsch)->regcost)
#define OPSCH_flags(opsch)	((opsch)->flags)
#define OPSCH_min_usage(opsch)	((opsch)->min_usage)
#define OPSCH_total_usage(opsch)	((opsch)->total_usage)
#define OPSCH_inst_dist(opsch)	((opsch)->inst_dist)
#define OPSCH_cyc_dist(opsch)	((opsch)->cyc_dist)
#define OPSCH_num_splits(opsch) ((opsch)->num_splits)
#define OPSCH_live_vals(opsch)  ((opsch)->live_vals)
#define OPSCH_new_live_vals(opsch)  ((opsch)->new_live_vals)
#define OPSCH_live_op_mask(opsch)  ((opsch)->live_op_mask)
...
// The following flag identify whether an OP is root of a tree in the dep graph
#define OPSCH_TREE             0x0080
#define OPSCH_FLAG1            0x0100
...
#define OPSCH_tree(opsch)	   (OPSCH_flags(opsch) & OPSCH_TREE)
#define Set_OPSCH_tree(opsch)      (OPSCH_flags(opsch) |= OPSCH_TREE)
#define Reset_OPSCH_tree(opsch)      (OPSCH_flags(opsch) &= ~OPSCH_TREE)
#define OPSCH_flag1(opsch)	   (OPSCH_flags(opsch) & OPSCH_FLAG1)
#define Set_OPSCH_flag1(opsch)      (OPSCH_flags(opsch) |= OPSCH_FLAG1)
#define Reset_OPSCH_flag1(opsch)      (OPSCH_flags(opsch) &= ~OPSCH_FLAG1)
...
extern BOOL HB_Sched_With_Lra_Request;
extern BOOL HB_Dfs_Uses_Last_Sched_Inst;
...
private:
  ...
  INT32 	    _Cur_Regs_Avail[TI_ISA_REGCLASS_MAX+1];
  BB*               _prolog_mbb;
  BB*               _epilog_mbb;
  hTN_MAP           _regs_map;
  TI_RES_RES*       _rr_tab;
  ...
  BOOL Check_Resource_Usage(OP *op, INT cycle);
  ...
  INT32 Put_Sched_Vector_Into_BB (BB*, BBSCH*, BOOL);
...
public:
  ...
  BOOL HBS_Local_Sched(void) const { return _hbs_type & HBS_LOCAL_SCHED; }
...
extern void Compute_OPSCH(BB *bb, bool do_xblock_latency, 
                          BB_MAP value_map, MEM_POOL *pool);
