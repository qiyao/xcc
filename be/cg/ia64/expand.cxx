...
Expand_Logical_Not (TN *dest, TN *src, INT16 variant, OPS *ops)
{
  ...
  if (TN_register_class(dest) == ISA_REGISTER_CLASS_predicate) {
    ...
    */
    if (!Get_normalized_op1(variant) && CGEXP_normalize_logical)
    {
...
*/
static void
Expand_Logical_And_Or (TOP action, TN *dest, TN *src1, TN *src2, INT16 variant, OPS *ops)
{
 /*
  *  if CG_EXP_normalize is true we must normalized the operands 
  *  (if not already normalized)
  */
  ...
  if (!Get_normalized_op2(variant) && CGEXP_normalize_logical)
  {
...
Expand_SGI_Sqrt (TN *result, TN *src, TYPE_ID mtype, OPS *ops)
{
  ...
  */
  ...
  Build_OP(TOP_frsqrta, f6, p2, qp, Gen_Enum_TN(ECV_sf_s0), src, ops);

  // f4 = 0.5
  tn = Build_TN_Of_Mtype (MTYPE_I8);
  Build_OP(TOP_mov_i, tn, p2, Gen_Literal_TN(-1 + exp_bias, 4), ops);
  f4 = Build_TN_Of_Mtype (mtype);
  Build_OP(TOP_setf_exp, f4, p2, tn, ops);

  // f7 = 0.5 + epsilon
  tn = Build_TN_Of_Mtype (MTYPE_I8);
  Build_OP(TOP_mov_i, tn, p2, Gen_Literal_TN(epsilon_pow2 + exp_bias, 4), ops);
  f7 = Build_TN_Of_Mtype (mtype);
  Build_OP(TOP_setf_exp, f7, p2, tn, ops);
  tn = Build_TN_Of_Mtype (mtype);
  Build_OP(TOP_fadd, tn, p2, Gen_Enum_TN(ECV_sf_s1), f7, f4, ops);
  f7 = tn;
...
Expand_Recip_Sqrt (TN *result, TN *src, TYPE_ID mtype, OPS *ops)
{
  ...
  Build_OP(TOP_frsqrta, f2, p2, qp, Gen_Enum_TN(ECV_sf_s0), src, ops);
  ...
  for (i = 0; i < 2 + is_double; ++i) {
    ...
    f6 = tn;
    tn = (i == 1 + is_double) ? result : Build_TN_Of_Mtype (mtype);
...
Exp_Is_Large_Stack_Sym(ST* sym,  INT64 ofst)
{
  ...
  if ((base_sym == SP_Sym || base_sym == FP_Sym) &&
      !ISA_LC_Value_In_Class(base_ofst, LC_i14)) {
    return TRUE;
  }
  return FALSE;
}
...
void Expand_Const (TN *dest, TN *src, TYPE_ID mtype, OPS *ops)
{
  ...
  // load from memory
  Exp_Load (mtype, dest, TN_var(src), 0, ops, 0);
}
