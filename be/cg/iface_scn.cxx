 ...
 * Module: iface_scn.c
 * $Revision: 1.34 $
 * $Date: 2000/06/29 23:59:08 $
 * $Author: mpm $
 * $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/iface_scn.cxx,v $
 *
 * Revision history:
  ...
  WN *rwn)			/* WN of function entry/call */
{
  ...
  for (i = 0; i < parm_count; i++) {
    ...
    while (PLOC_is_nonempty(ploc)) {
    	if (Preg_Is_Dedicated_Float(PLOC_reg(ploc)))
	{
	  flt_regs |= (1 << (PLOC_reg(ploc) - first_float_preg));
    	}
   ...
   */
  flt_regs = flt_regs >>
    (Dedicated_Preg_First_Incoming_Arg(float_regclass) - first_float_preg);
