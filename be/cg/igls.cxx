...
//  Module: igls.cxx
//  $Revision: 1.34 $
//  $Date: 2000/09/28 23:25:41 $
//  $Author: srinivas $
//  $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/igls.cxx,v $
//
//  Description:
//  ============
//
...
/*
  Do a simple optimizaton within a BB after GCM is done.
*/
static void GCM_Post_Optimize()
{
  if (GCM_Enable_Cflow) {
    /* when merging happens, scheduling bits must be reset. Reseting
       is done within CFLOW.
     */
    CFLOW_Optimize(CFLOW_MERGE|CFLOW_AFTER_LRA, "CFLOW (after GCM)");
  }
  ...
  for (bb = REGION_First_BB; bb != NULL; bb = BB_next(bb)) {
    ...
    /* If optimization happens, reset scheduling
       bits so it will be re-scheduled later.
     */
    if (EBO_Post_Process_BB(bb)) {
...
IGLS_Schedule_Region (BOOL before_regalloc)
{
  ...
  if (before_regalloc) {
    ...
    if (!Get_Trace(TP_SCHED, 0x400)) 
      hbs_type = HBS_BEFORE_GRA | HBS_BEFORE_LRA | HBS_MINIMIZE_REGS;
    else
    ...
    if (TI_PROC_Property_Set(PROP_has_bundles)) hbs_type |= HBS_MINIMIZE_BUNDLES;
  ...
  if (before_regalloc) {
	...
	if (!Get_Trace(TP_SCHED, 0x4000) && (BB_length(bb)>0)) {
	  HBS_TYPE hbs_temp = HBS_BEFORE_GRA | HBS_BEFORE_LRA | 
	    HBS_CRITICAL_PATH;
	  INT32 max_sched = OP_scycle(BB_last_op(bb))+1;
	  Sched->Init(bb, hbs_temp, max_sched, NULL, NULL);
	  Sched->Schedule_BB(bb, NULL);
	}
      }
    }
	...
	if (LOCS_Enable_Scheduling) {
	  ...
	  /* use INT32_MAX instead of max_sched to allow new schedule
	     when additional instructions (e.g. spills) were added
	     and the old cycle count (i.e. max_sched) is outdated
	  */
      ...
      if (CG_opt_level >0) {
        ...
        if (xt_flix && xt_only_full_flix) {
          Bundle_Only_Full_Flix(bb);
        } else if (OPT_Flix_Space) {
          Bundle_For_Space(bb);
        }
      }
    } /* for (bb= REGION_First_BB).. */
...
void CFLOW_Layout_Reschedule (void)
{
  BB *bb;
  RID *rid;
  ...
  for (bb = REGION_First_BB; bb != NULL; bb = BB_next(bb))
  {
    ...
    if (!BB_scheduled(bb)) {
      Sched->Init(bb, HBS_LOCAL_SCHED, INT32_MAX, NULL, NULL);
      Sched->Schedule_BB(bb, NULL);
      Handle_All_Hazards (bb);
    }
  }
