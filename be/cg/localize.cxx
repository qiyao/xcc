 ...
 *  Module: localize.c
 *  $Revision: 1.68 $
 *  $Date: 2000/07/26 00:17:18 $
 *  $Author: mpm $
 *  $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/localize.cxx,v $
 *
 *  Description:
 *  ============
 *
...
Localize_Global_Param_Reg (BB *current_bb, TN *parm_tn)
{
	...
	FmtAssert(bb != NULL && BB_call(BB_prev(bb)), ("didn't find call BB after bb %d for tn %d", BB_id(current_bb), TN_number(parm_tn)));
...
Check_If_Dedicated_TN_Is_Global (TN *tn, BB *current_bb, BOOL def)
{
	...
	BOOL in_func_arg = REGISTER_SET_MemberP( 
	    REGISTER_CLASS_incoming_argument(TN_register_class(tn)), 
	    TN_register(tn));
	BOOL out_func_arg = REGISTER_SET_MemberP( 
	    REGISTER_CLASS_outgoing_argument(TN_register_class(tn)), 
	    TN_register(tn));
	BOOL in_func_retval = REGISTER_SET_MemberP( 
	    REGISTER_CLASS_incoming_return_value(TN_register_class(tn)),
	    TN_register(tn));
	BOOL out_func_retval = REGISTER_SET_MemberP( 
	    REGISTER_CLASS_outgoing_return_value(TN_register_class(tn)),
	    TN_register(tn));
        
        BOOL is_func_arg = (in_func_arg || out_func_arg);
        BOOL is_func_retval = (in_func_retval || out_func_retval);
        
        // integer registers that are not caller-saved are windowed
        BOOL is_windowed = 
          (Target_ABI == ABI_WINDOWED &&
           TN_register_class(tn) == TI_ISA_Regclass_Integer() &&
	   !REGISTER_SET_MemberP(
             REGISTER_CLASS_caller_saves(TN_register_class(tn)), 
	     TN_register(tn)));

        BOOL allocatable = REGISTER_allocatable(TN_register_class(tn),
                                                TN_register(tn));
	    ...
	    && BB_entry(current_bb) )
	{
		...
		if (!allocatable) 
			;	// this should happen only with ASM assignment
		else if (def && out_func_arg && BB_call(current_bb))
			;	// okay
		else if (def && out_func_retval && BB_exit(current_bb))
			;	// okay
#ifdef TARG_IA32
		else if (def && out_func_retval && BB_asm(current_bb))
			;	// okay
#endif
		else if (def && out_func_arg && BB_asm(current_bb))
			;	// okay
		else if (def && out_func_retval && BB_call(current_bb)
			...
			;	// okay
		else if (!def && in_func_retval && BB_entry(current_bb)
		    && RETURN_INFO_return_via_first_arg(Get_Return_Info(
		...
		else if (!def && in_func_arg && BB_entry(current_bb)) 
			;	// okay
		else if (!def && in_func_retval 
		    && BB_prev(current_bb) != NULL 
		    && BB_call(BB_prev(current_bb)))
			;	// okay
                // windowed registers are preserved across calls
		else if (!def && is_windowed
		    && BB_prev(current_bb) != NULL 
		    && BB_call(BB_prev(current_bb)))
			;	// okay
		else if (!def && in_func_retval 
    ...
    FOR_ALL_BB_OPs (bb, op) {
      ...
      for (opndnum = 0; opndnum < OP_opnds(op); opndnum++) {
        ...
        if (TN_is_dedicated(tn)) {
          // For ASM, even OP_same_res() is true, 
          //   it's possible asm does not have any result
          if (OP_same_res(op) && (OP_results(op) > 0) && tn == OP_result(op,0)) {
            /* this use is just a copy of the def */
            continue;
	  }
	  if (OP_copy(op) && (OP_results(op) > 0) && tn == OP_result(op,0)) {
