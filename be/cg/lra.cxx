 ...
 *  Module: lra.c
 *  $Revision: 1.155 $
 *  $Date: 2000/10/06 15:46:52 $
 *  $Author: dew $
 *  $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/lra.cxx,v $
 *
 *  Description:
 *  ============
 *
...
static REGISTER_SET Callee_Saved_Regs_Used[TI_ISA_REGCLASS_MAX+1];
...
static AVAIL_REGS avail_regs[TI_ISA_REGCLASS_MAX+1];

/* Set of available registers in each register class. */
static REGISTER_SET avail_set[TI_ISA_REGCLASS_MAX+1];

/* Set of excluded registers in each register class. */
static REGISTER_SET exclude_set[TI_ISA_REGCLASS_MAX+1];
...
static REGISTER last_assigned_reg[TI_ISA_REGCLASS_MAX+1];
...
static REGISTER_SET livethrough[TI_ISA_REGCLASS_MAX+1];
...
static TN *unused_tn_def[TI_ISA_REGCLASS_MAX+1];

/* remember the subclass id instead to save calling overhead */
static ISA_REGSUBCLASS regsubclass_br1;
static ISA_REGSUBCLASS regsubclass_br2;
static ISA_REGSUBCLASS regsubclass_br4;
static ISA_REGSUBCLASS regsubclass_br8;
static ISA_REGSUBCLASS regsubclass_br16;
...
typedef struct live_range {
  ...
  mINT16 first_use;     /* instruction number for first use in live range. */
  ...
  mUINT16 flags;        /* misc. flags (see definition below) */
...
#define LR_first_use(lr)        ((lr)->first_use)
...
#define LR_FAT_ALLOCATED 0x8	// LR was allocated real register by fat point
                                // calculation
#define LR_GLOBAL_LIVE   0x10   // LR is for global(s) and at least one of
                                // of those GTNs is live out of the this BB.
...
#define LR_global_live(lr)	(LR_flags(lr) & LR_GLOBAL_LIVE)
#define Set_LR_global_live(lr)	(LR_flags(lr) |= LR_GLOBAL_LIVE)
...
static INT Simple_Trip_Count;
static INT Regclass_Trip_Count[TI_ISA_REGCLASS_MAX+1];
...
static ISA_REGCLASS failing_class;
static INT failing_opnum;
static BOOL failure_avail_regs[REGISTER_MAX+1];
...
static ISA_REGCLASS trace_cl = (ISA_REGCLASS)2;
#define Calculating_Fat_Points() (failing_class != ISA_REGCLASS_UNDEFINED)
...
LRA_TN_Allocate_Register(TN *tn, REGISTER reg)
{
  ...
  //
  if (use_fat_point_regs && reg > TI_ISA_Register_Max()) {
...
Check_Allow_Reorder() {
  if ((CG_opt_level > 1) &&
      (Regclass_Trip_Count[failing_class] <= CG_opt_level) &&
...
Check_Allow_Reschedule()
{
  if (Check_Allow_Reorder() &&
      (Trip_Count-Simple_Trip_Count) == 1 &&
...
//
inline void
Clobber_Op_Info(INT opnum, ISA_REGCLASS spill_cl)
{
...
Op_Uses_TN(TN *spill_tn, INT opnum)
{
  ...
  ISA_REGCLASS cl = TN_register_class(spill_tn);
...
static void Init_Avail_Set (BB *bb)
{
  ...
  FOR_ALL_ISA_REGCLASS(cl) {
	...
	avail_set[cl] = REGISTER_SET_Union(avail_set[cl],
      ...
      if (BB_call(bb)) {
	/* Note: all this code may be unnecessary because gra_grant
	   already adds caller_save registers into the local register
	   grant. */
	OP *call_op = BB_last_op(bb);
	if ((call_op != NULL) &&
	    OP_call(call_op) &&
	    OP_cond_def(call_op)) {
	  /* If the call is conditionally executed, the return value
	     registers may already contain values that must be
	     preserved through the block. */
	  avail_set[cl] =
	    REGISTER_SET_Union (avail_set[cl],
    ...
    for (REGISTER reg = REGISTER_SET_Choose(rs);
	 ...
	 reg = REGISTER_SET_Choose_Next(rs, reg)) {
      if (REG_LIVE_Implicit_Use_Outof_BB(cl, reg, bb))
...
static REGISTER
Find_Register_Preference(OP* op, const ISA_OPERAND_INFO* opinfo, INT tnnum,
			 BOOL is_result, REGISTER old_preference)
{
  /* Since we do lra backward, return the new one as preference reg 
     whenever there is one

     (Probably it is better to keep all preference regs so LRA can choose
      the right one later.)
   */
  if (CGTARG_Is_Preference_Copy(op)) {
    TN* other_tn = NULL;
    if (is_result) {
      other_tn = OP_opnd(op, CGTARG_Copy_Operand(op));
    } else if (tnnum == CGTARG_Copy_Operand(op)) {
      other_tn = OP_result(op, 0);
    }
    if (other_tn != NULL) {
      REGISTER other_reg = LRA_TN_register(other_tn);
      if (other_reg == REGISTER_UNDEFINED) {
	LIVE_RANGE *lr = LR_For_TN (other_tn);
	if (lr != NULL)
	  other_reg = LR_prefer_reg(lr);
      }
      if ((other_reg != REGISTER_UNDEFINED) && (other_reg <= TI_ISA_Register_Max()))
	return other_reg;
    }
  }
...
Setup_Live_Ranges (BB *bb, BOOL in_lra, MEM_POOL *pool)
{
  ...
  for (opnum = 1; opnum <= BB_length(bb); opnum++) {
    ...
    for (opndnum = 0; opndnum < OP_opnds(op); opndnum++) {
      ...
      if (LR_first_use(clr) == 0) {
        LR_first_use(clr) = opnum;
      }
      if (!TN_is_local_reg(tn)) {
        if (LR_def_cnt(clr) == 0) {
          LR_exposed_use(clr) = opnum;
        }
	if (LR_last_use(clr) < opnum)
	...
	LR_prefer_reg(clr) = 
	  Find_Register_Preference(op, opinfo, opndnum, FALSE, 
				   LR_prefer_reg(clr));
      }
    }
	...
	const ISA_OPERAND_INFO *opinfo = TI_ISA_Operand_Info(OP_code(op));
	LR_prefer_reg(clr) = 
	  Find_Register_Preference(op, opinfo, resnum, TRUE, 
				   LR_prefer_reg(clr));
      }
    }
    ...
    if ( BB_bbregs(bb) != NULL ) {
      ...
      for (tn = GTN_SET_Choose(BB_live_out(bb));
        ...
        if ( TN_is_register(tn) && 
          ...
          if ( lr != NULL ) {
            Set_LR_global_live(lr);
          }
        }
      }
    } else {
      // no live info available, assume they are live
...
{
  if (!OP_load(op) ||
      TI_ISA_Op_Results(TI_ISA_Operand_Info(OP_code(op)))!=1) return FALSE;

  TN *spill_tn = OP_spill_tn(op);
  if (spill_tn == NULL) return FALSE;
  return (TN_spill(spill_tn) == spill_loc);
}
...
{
  if (!OP_store(op) ||
      TI_ISA_Op_Results(TI_ISA_Operand_Info(OP_code(op)))!=0) return FALSE;
...
static void
Mark_Reloadable_Live_Ranges (ISA_REGCLASS cl)
{
  ...
  for (LIVE_RANGE *lr = Live_Range_List; lr != NULL; lr = LR_next(lr)) 
  {
    ...
    // For now, restrict ourselves to considering only loads with 1 result.
    // E.g, an updated load will not be considered
    if (OP_results(def_op)!=1)
      continue;

    // live-range is not reloadable if we cannot determine that
    // the value is the same if we load it again
    if (OP_unknown_memdata(def_op))
      continue;
 ...
 * Register_Set_Width
 *
 * Compute the number of registers for br{1,2,4,8,16} register subclass
 * ======================================================================*/
static int
Register_Set_Width (ISA_REGSUBCLASS regsubclass)
{
	if (regsubclass == regsubclass_br1)
	  return 1;
	else if (regsubclass == regsubclass_br2)
	  return 2;
	else if (regsubclass == regsubclass_br4)
	  return 4;
	else if (regsubclass == regsubclass_br8)
	  return 8;
	else if (regsubclass == regsubclass_br16)
	  return 16;
	else
	  FmtAssert(0,("Unknown br register subclass"));
	return -1;
}
 ...
 * ======================================================================*/
static void
Add_Avail_Reg (ISA_REGCLASS regclass, REGISTER reg, INT cur_op,
	       TN* reg_tn, BOOL is_dedicated, 
	       int num_regs)
{
  // are we establishing fat points?
  if (Calculating_Fat_Points()) {
    if (fat_points[cur_op] >= FAT_POINTS_MIN+num_regs)
      fat_points[cur_op] -= num_regs;
    else fat_points[cur_op]=FAT_POINTS_MIN;
    //
    // infinite register set used when establishing fat points.
    //
    if (reg > TI_ISA_Register_Max()) return;
  }
  ...
  for (int i=0; i<num_regs; i++) {
    Is_True (!avail_regs[regclass].reg[reg+i],
	     (" LRA: Error in Add_Avail_Reg"));
    avail_regs[regclass].reg[reg+i] = TRUE;
  }
}
 ...
 * ======================================================================*/
inline void
Delete_Avail_Reg (ISA_REGCLASS regclass, REGISTER reg, INT cur_op,
		  TN* reg_tn, BOOL is_dedicated,
		  INT num_regs)
{
  // are we establishing fat points?
  if (Calculating_Fat_Points()) {
    if (fat_points[cur_op] <= FAT_POINTS_MAX-num_regs)
      fat_points[cur_op] += num_regs;
    else fat_points[cur_op]=FAT_POINTS_MAX;
    //
    // infinite register set used when establishing fat points.
    //
    if (reg > TI_ISA_Register_Max()) return;
  ...
  for (int i=0; i<num_regs; i++) {
    Is_True (!REGISTER_allocatable(regclass,reg+i) ||
	     avail_regs[regclass].reg[reg+i],
	     (" LRA: Error in Delete_Avail_Reg"));
    avail_regs[regclass].reg[reg+i] = FALSE;
  }
}
 ...
 *   4. for branch register sets (br{2,4,8,16}), make sure all elements are
 *      available, not just the first element. i.e., for br4 regsubclass,
 *      all of br4, br5, br6 and br7 have to be available to make br4
 *      available. Also make sure the register is in the right register
 *	subclass.
 * ======================================================================*/
static BOOL
Is_Reg_Available (ISA_REGCLASS regclass, 
		  ...
		  INT num_regs)
{
  ...
  if (reg != REGISTER_UNDEFINED && 
    ...
    for (int i=0; i<num_regs; i++)
      if (!avail_regs[regclass].reg[reg+i])
	return FALSE;
...
/*
 For an OP:  result_tn opc ....; where result_tn is a dedicated TN.
 Is_Non_Allocatable_Reg_Available() cannot handle it well simply by
 checking live ranges because a dedicated TN has only one live ranges
 even it is defined multiple times, so its live range conservatively
 begins from the first def to the last use. For example,
   1   a = ..       // a is a dedicated TN (using register, say r)
   2     = a
   3   ...
   4   a = ...
   5     = a
  a's live range is <1,5>, although 'a' is actually dead within 3. 

  This function is to check if 'a' is dead within 3, so register r 
  (allocated for a) can be reused within 3. 

  Given the following pattern, the function checks if tn can use
  register allocated to result_tn, if so, return true; false 
  otherwise.
...
)
{
  ...
  /* Do it if either result_tn is dedicated or op is a copy. 
     They are the cases we want to handle for now. The other
     cases should be handled already in the normal part of LRA,
     so no need to handle the other cases here.
   */
  if (!TN_is_dedicated(result_tn) &&
      !CGTARG_Is_Preference_Copy(op)) return false;

  /* make sure to use the register for result! */
  if ((LRA_TN_register(result_tn) != reg) ||
      (TN_register_class(result_tn) != regclass)) return false;

  OP *def_op = OP_VECTOR_element (Insts_Vector, LR_first_def(lr));

  if ( (def_op == op) ||   // should never happen.
       (def_op == NULL) ) {
    return false;
  }

  OP *op1;
  for (op1 = OP_next(def_op); op1 != op; op1 = OP_next(op1)) {
    if ( OP_Defs_TN(op1, result_tn) || OP_Refs_TN(op1, result_tn) ||
         OP_Defs_Reg(op1, regclass, reg) || OP_Refs_Reg(op1, regclass, reg) ) {
      return false;
    }
  }

  // check operands of OP 'op'
  if (OP_Refs_TN(op, result_tn) || OP_Refs_Reg(op, regclass, reg)) {
    return false;
  }
  return true;
}
...
)
{
  ...
  if (regclass == TI_ISA_Regclass_Branch())
    return FALSE;
  ...
  if (!REGISTER_allocatable (regclass, reg) &&
      ...
      ((LR_first_def(lr) >= LR_exposed_use(ded_lr) &&
       ...
       ((op != NULL) && 
        Is_Result_Reg_Avaliable(op, regclass, reg, lr))))  
  {
 ...
 * ======================================================================*/
static REGISTER
Get_Avail_Reg (ISA_REGCLASS regclass, 
               REGISTER_SET usable_regs, 
               LIVE_RANGE *lr, 
               REGISTER skip_reg,
	       TN* reg_tn,
	       INT num_regs)
{
  bool singleton = (reg_tn!=NULL && TN_Register_Width(reg_tn)==1);
  REGISTER next_reg = singleton? last_assigned_reg[regclass] + 1 :
				 (Check_Allow_Reschedule()? last_assigned_reg[regclass] + 1:
				  			    REGISTER_MIN);
  REGISTER reg;
  ...
  for (reg = next_reg; reg < TI_ISA_Register_Max()+1; reg++) {
    if (Is_Reg_Available(regclass, usable_regs, reg, lr, reg_tn, 
			 FALSE, num_regs) &&
	reg != skip_reg) {
      return reg;
    }
  ...
  if (Calculating_Fat_Points()) {
    reg_infinite += TN_Register_Width(reg_tn);
    return reg_infinite;
  } else {
  ...
  INT num_regs)
{
  ...
  BOOL is_dedicated = TN_is_dedicated(tn);
  ...
  /* Use register width of tn instead of num_regs, because we may ask for more
     registers than we need in order to get a block of contiguous registers. */
  Delete_Avail_Reg (regclass, reg, opnum, tn, is_dedicated, 
		    TN_Register_Width(tn));
  if (Do_LRA_Trace(Trace_LRA_Detail)) {
...
//
static BOOL
Init_Fat_Point_Calculation(ISA_REGCLASS cl, INT opnum, BB *bb, INT usage_width)
{
  //
  ...
  failing_class = cl;
  failing_opnum = opnum;
  memcpy(failure_avail_regs, avail_regs[cl].reg, (TI_ISA_Register_Max()+1)*sizeof(BOOL));
  reg_infinite = TI_ISA_Register_Max();
...
{
  fat_points = NULL;
  failing_class = ISA_REGCLASS_UNDEFINED;
...
static REGISTER_SET 
Remove_Register_Range(REGISTER_SET regset, REGISTER base, INT32 head, 
		      INT32 tail)
{
  REGISTER_SET range;
  if (base > head)
    range = REGISTER_SET_Range(base-head+1, base+tail-1);
  else
    range = REGISTER_SET_Range(REGISTER_MIN, base+tail-1);
  return REGISTER_SET_Difference(regset, range);
}
...
Usable_Registers (
  ...
  INT32        num_regs, 
  LIVE_RANGE   *lr,
  REGISTER_SET *clobber_regs = NULL   // used only for processing ASM
                                      // when there is a spill
)
{
  ISA_REGCLASS cl = TN_register_class(tn);
  REGISTER_SET usable_regs = REGISTER_CLASS_universe(cl);
  REGISTER_SET maybe_regs = REGISTER_SET_EMPTY_SET;
  bool undefined_use = false;

  INT first_op = MAX(MIN(LR_first_use(lr), LR_first_def(lr)), 1);
  INT last_op = MIN(LR_last_use(lr)+1, VECTOR_size(Insts_Vector));
  
  if (LR_def_cnt(lr)==0) {
    // limit the scope of removal of usable register since any valid
    // register can be used for a upward exposed local register
    first_op = LR_first_use(lr);
    undefined_use = true;
  }
  ...
  for (INT opnum = first_op; opnum < last_op; opnum++) {  
    ...
    OP* op = OP_VECTOR_element(Insts_Vector, opnum);
    if ( op == NULL) continue;
    if (REGISTER_SET_EmptyP(usable_regs)) {
      usable_regs = maybe_regs;
    }
    ...
    if (asm_info) {
      REGISTER_SET asm_regs = ASM_OP_clobber_set(asm_info)[cl];
      if (clobber_regs != NULL) {
        clobber_regs[opnum] = asm_regs;
      } else {
        for (REGISTER member = REGISTER_SET_Choose(asm_regs);
             member != REGISTER_UNDEFINED;
             member = REGISTER_SET_Choose_Next(asm_regs, member)) {
          usable_regs = Remove_Register_Range(usable_regs, member, num_regs, 1);
        }
      }

      for (INT resnum = 0; resnum < OP_results(op); resnum++) {
	if ((tn == OP_result(op,resnum)) && 
	    ASM_OP_result_clobber(asm_info)[resnum]) {
	  for (INT i = 0; i < OP_opnds(op); i++) {
	    if (TN_is_register(OP_opnd(op,i)) &&
		(TN_register_class(OP_opnd(op, i)) == cl) &&
		(LRA_TN_register(OP_opnd(op,i)) != REGISTER_UNDEFINED) &&
		(LRA_TN_register(OP_opnd(op,i)) <= TI_ISA_Register_Max()))
	      usable_regs = 
		Remove_Register_Range(usable_regs, 
				      LRA_TN_register(OP_opnd(op,i)), 
				      num_regs, 1);
	  }
	}
      }
	...
	if (tn == OP_opnd(op,opndnum)) {
	  for (INT i = 0; i < OP_results(op); i++) {
	    if ((TN_register_class(OP_result(op, i)) == cl) &&
		ASM_OP_result_clobber(asm_info)[i] &&
		(LRA_TN_register(OP_result(op,i)) != REGISTER_UNDEFINED) &&
		(LRA_TN_register(OP_result(op,i)) <= TI_ISA_Register_Max()))
	      usable_regs = 
		Remove_Register_Range(usable_regs, 
				      LRA_TN_register(OP_result(op,i)), 
				      num_regs, 1);
	  }
	}
      }
    ...
    /* for last use, we should not worry about the results, the exception
       being an unused def. */
    BOOL skip_result = TRUE;
    if ((opnum!=LR_last_use(lr)) || (LR_last_use(lr)==LR_first_def(lr))) 
      skip_result = FALSE;
	...
	continue;
      ...
      if (!skip_result && (LRA_TN_register(result) != REGISTER_UNDEFINED)) {
	usable_regs = Remove_Register_Range(usable_regs, 
					    LRA_TN_register(result), num_regs,
					    OP_TN_Width(op, resnum, TRUE));
      }
	...
	usable_regs = REGISTER_SET_Intersection(usable_regs, subclass_regs);
      ...
      else if (!skip_result && (REGISTER_SET_Size(subclass_regs) == 1)) {
	maybe_regs = REGISTER_SET_Union(maybe_regs,
					REGISTER_SET_Intersection(usable_regs,
								  subclass_regs));
	usable_regs = REGISTER_SET_Difference(usable_regs, subclass_regs);
      }
    }
          
    /* for first def, we should not worry about the operands */
    BOOL skip_opnd = TRUE;
    if (opnum!=LR_first_def(lr)) 
      skip_opnd = FALSE;

    // if the current LR is undefined use then it will not interfere
    // with other non-asm reads in the live-range
    if (!undefined_use || OP_code(op)==TOP_asm)
    for (INT opndnum = 0; opndnum < OP_opnds(op); opndnum++) {
	...
	continue;
      ...
      if (!skip_opnd && (LRA_TN_register(opnd) != REGISTER_UNDEFINED)) {
	usable_regs = Remove_Register_Range(usable_regs, 
					    LRA_TN_register(opnd), num_regs,
					    OP_TN_Width(op, opndnum, FALSE));
      }
      ISA_REGSUBCLASS sc = asm_info ?
	...
	usable_regs = REGISTER_SET_Intersection(usable_regs, subclass_regs);
      ...
      else if (!skip_opnd && (REGISTER_SET_Size(subclass_regs) == 1)) {
	maybe_regs = REGISTER_SET_Union(maybe_regs,
					REGISTER_SET_Intersection(usable_regs,
								  subclass_regs));
	usable_regs = REGISTER_SET_Difference(usable_regs, subclass_regs);
...
static REGISTER_SET
Fix_Usable_Registers_For_Multiregs(REGISTER_SET regset, TN *tn, LIVE_RANGE *lr)
{
  INT first_op = MAX(MIN(LR_first_use(lr), LR_first_def(lr)), 1);
  INT last_op = MIN(LR_last_use(lr)+1, VECTOR_size(Insts_Vector));
  ...
  for (INT opnum = first_op; opnum < last_op; opnum++) {  
    OP* op = OP_VECTOR_element(Insts_Vector, opnum);
    if (TI_ISA_Property_Set(PROP_has_multireg_operand, OP_code(op))) {
      const ISA_OPERAND_INFO *opinfo = TI_ISA_Operand_Info(OP_code(op));
      for (INT resnum = 0; resnum < OP_results(op); resnum++) {
	if (tn == OP_result(op,resnum)) {
	  INT tnnum2 = resnum;
	  const ISA_OPERAND_VALTYP *vtype = TI_ISA_Op_Result(opinfo, resnum);
	  if (TI_ISA_Valtyp_Is_Continuation(vtype)) {
	    while (tnnum2 > 0) {
	      tnnum2--;
	      TN *tn2 = OP_result(op, tnnum2);
	      if (!TN_is_register(tn2))
		break;
	      const ISA_OPERAND_VALTYP *vtype2 =
		TI_ISA_Op_Result(opinfo, tnnum2);
	      REGISTER reg2 = LRA_TN_register(tn2);
	      REGISTER_SET ok_set = REGISTER_SET_EMPTY_SET;
	      if (reg2 == REGISTER_UNDEFINED) {
		REGISTER_SET linked_set =
		  Usable_Registers(tn2, OP_TN_Width(op, tnnum2, TRUE), 
				   LR_For_TN(tn2));
		for (REGISTER member = REGISTER_SET_Choose(linked_set);
		     member != REGISTER_UNDEFINED;
		     member = REGISTER_SET_Choose_Next(linked_set, member)) {
		  ok_set = REGISTER_SET_Union1(ok_set, member-tnnum2+resnum);
		}
	      } else {
		REGISTER_SET regset2 = 
		  REGISTER_SUBCLASS_members(TI_ISA_Valtyp_Regsubclass(vtype2));
		if ((reg2 <= TI_ISA_Register_Max()) &&
		    REGISTER_SET_MemberP(regset2, reg2)) {
		  ok_set = REGISTER_SET_Range(reg2 - tnnum2 + resnum, 
					      reg2 - tnnum2 + resnum);
		} else {
		  ok_set = REGISTER_SET_Range(REGISTER_MIN, TI_ISA_Register_Max());
		}
	      }
	      regset = REGISTER_SET_Intersection(regset, ok_set);
	      if (!TI_ISA_Valtyp_Is_Continuation(vtype2))
		break;
	    }
	  }
	  tnnum2 = resnum;
	  while (tnnum2 < OP_results(op)-1) {
	    tnnum2++;
	    TN *tn2 = OP_result(op, tnnum2);
	    if (!TN_is_register(tn2))
	      break;
	...
	if (tn == OP_opnd(op,opndnum)) {
	  ...
	  if (TI_ISA_Valtyp_Is_Continuation(vtype)) {
	    while (tnnum2 > 0) {
	      tnnum2--;
	      TN *tn2 = OP_opnd(op, tnnum2);
	      if (!TN_is_register(tn2))
		break;
	      const ISA_OPERAND_VALTYP *vtype2 =
		TI_ISA_Op_Operand(opinfo, tnnum2);
	      REGISTER reg2 = LRA_TN_register(tn2);
	      REGISTER_SET ok_set = REGISTER_SET_EMPTY_SET;
	      if (reg2 == REGISTER_UNDEFINED) {
		REGISTER_SET linked_set = 
		  Usable_Registers(tn2, OP_TN_Width(op, tnnum2, FALSE), 
				   LR_For_TN(tn2));
		for (REGISTER member = REGISTER_SET_Choose(linked_set);
		     member != REGISTER_UNDEFINED;
		     member = REGISTER_SET_Choose_Next(linked_set, member)) {
		  ok_set = REGISTER_SET_Union1(ok_set, member-tnnum2+opndnum);
		}
	      } else {
		REGISTER_SET regset2 = 
		  REGISTER_SUBCLASS_members(TI_ISA_Valtyp_Regsubclass(vtype2));
		if ((reg2 <= TI_ISA_Register_Max()) &&
		    REGISTER_SET_MemberP(regset2, reg2)) {
		  ok_set = REGISTER_SET_Range(reg2 - tnnum2 + opndnum, 
					      reg2 - tnnum2 + opndnum);
		} else {
		  ok_set = REGISTER_SET_Range(REGISTER_MIN, TI_ISA_Register_Max());
		}
...
static INT
Find_Register_Usage_For_OP(OP* op, INT tnnum, BOOL is_result)
{
  if (!TI_ISA_Property_Set(PROP_has_multireg_operand, OP_code(op)))
    return 1;
  INT reg_usage=0;
  const ISA_OPERAND_INFO *opinfo = TI_ISA_Operand_Info(OP_code(op));
  INT max_index = (is_result ? OP_results(op) : OP_opnds(op));

  for (INT i = tnnum; i < max_index; i++) {
    const ISA_OPERAND_VALTYP *vtype = (is_result ? 
				       TI_ISA_Op_Result(opinfo, i) :
				       TI_ISA_Op_Operand(opinfo, i));
    if ((i == tnnum) || TI_ISA_Valtyp_Is_Continuation(vtype))
      reg_usage += TI_ISA_Valtyp_Num_Regs(vtype);
    else {
      break;
    }
  }
...
Find_Max_Usage_Width(BB* bb)
{
  ...
  FOR_ALL_BB_OPs_FWD (bb, op) {
    if (TI_ISA_Property_Set(PROP_has_multireg_operand, OP_code(op))) {
      INT reg_usage;
      for (INT i=0; i<OP_results(op); i++) {
	reg_usage = Find_Register_Usage_For_OP(op, i, TRUE);
	max_reg_usage = MAX(max_reg_usage, reg_usage);
      }
      for (INT i=0; i<OP_opnds(op); i++) {
	if (!TN_is_register(OP_opnd(op,i))) continue;
	reg_usage = Find_Register_Usage_For_OP(op, i, FALSE);
	max_reg_usage = MAX(max_reg_usage, reg_usage);
      }
    }
  }
  return max_reg_usage;
}


static BOOL
Assign_Registers_For_OP (OP *op, INT opnum, TN **spill_tn, BB *bb, 
			 INT *extra_fatpoint, INT usage_width)
{
  INT opndnum;
  INT resnum;
  REGISTER reg, result_reg;
  ISA_REGCLASS result_cl;
  ISA_REGSUBCLASS result_subcl;
  BOOL uniq_result;
  LIVE_RANGE *clr;
  ...
  ISA_REGCLASS *free_result_cl = TYPE_ALLOCA_N(ISA_REGCLASS, nresults);
  BOOL skip_tn = FALSE;
  INT early_clobbers=0;
  ...
  // Need to update Callee_Saved_Regs_Used[] for asm clobber
  if (CG_localize_tns && asm_info) {
    ISA_REGCLASS cl; 
    FOR_ALL_ISA_REGCLASS (cl) {
      REGISTER_SET asm_regs = ASM_OP_clobber_set(asm_info)[cl];
      if (!REGISTER_SET_EmptyP(asm_regs)) {
        REGISTER_SET callee_clobber = REGISTER_SET_Intersection (
                                        REGISTER_CLASS_callee_saves(cl),
                                        asm_regs );
        if (!REGISTER_SET_EmptyP(callee_clobber)) {
          Callee_Saved_Regs_Used[cl] = REGISTER_SET_Union (
                                         Callee_Saved_Regs_Used[cl],
                                         callee_clobber );
        }
      }
  ...
  result_cl = ISA_REGCLASS_UNDEFINED;
  result_subcl = ISA_REGSUBCLASS_UNDEFINED;
  for (resnum = 0; resnum < nresults; resnum++) {
    ...
    BOOL is_dedicated = TN_is_dedicated(result_tn);
    const ISA_OPERAND_INFO *opinfo = TI_ISA_Operand_Info(OP_code(op));
    const ISA_OPERAND_VALTYP *vtype = TI_ISA_Op_Result(opinfo, resnum);
    result_subcl = asm_info ? 
      ASM_OP_result_subclass(asm_info)[resnum] : 
      TI_ISA_Valtyp_Regsubclass(vtype);
    INT reg_usage = Find_Register_Usage_For_OP(op, resnum, TRUE);
    UINT num_regs = OP_TN_Width(op, resnum, TRUE);
    REGISTER_SET must_use;

    // skip_tn is reevaluated only for the first tn in a pair. Subsequent tns
    // in the pair use the old skip_tn value.
    if (!TI_ISA_Property_Set(PROP_has_multireg_operand, OP_code(op)) || 
	!TI_ISA_Valtyp_Is_Continuation(vtype))
      skip_tn = (reg_usage < usage_width);
	...
	failing_class == result_cl) {
      ...
      if ((result_cl == REGISTER_CLASS_sp) && (result_reg == REGISTER_sp) && CG_localize_tns) {
	  Update_Callee_Availability(bb);
      } else if ((result_reg != REGISTER_UNDEFINED) && 
		 REGISTER_allocatable (result_cl, result_reg) &&
		 (opnum == LR_last_use(clr))) {
	Delete_Avail_Reg (result_cl, result_reg, opnum, 
			  result_tn, is_dedicated, num_regs);
      }
      if (!skip_tn) {
	...
	if (result_reg == REGISTER_UNDEFINED) {
	  ...
	  if (prefer_reg == REGISTER_UNDEFINED) {
	    if (TN_is_local_reg(result_tn) && (unused_tn_def[result_cl] != NULL)) {
	      FmtAssert(result_cl!=TI_ISA_Regclass_Branch(),
			("br register class should not have unused_tn_def[]"));
		...
		if (all_results_unneeded) {
	      ...
	      continue;
	      /* Check any subsequent continuation operands for a preferred 
		 register */
	    }
	  }
	  
	  must_use = Usable_Registers(result_tn, num_regs, clr);
	  if (usage_width > 1) {
	    must_use = Fix_Usable_Registers_For_Multiregs(must_use, result_tn, 
							  clr);
	  }

	  if (prefer_reg != REGISTER_UNDEFINED &&
	      (Is_Reg_Available (result_cl, must_use, prefer_reg, clr, 
				 result_tn, is_dedicated, reg_usage) ||
		...
		*spill_tn = result_tn;
		//
		// continue going through the loop with the purpose of 
		// gathering fat point info, if appropriate.  if so, redo 
		// failing op.
		//
		if (Init_Fat_Point_Calculation(TN_register_class(*spill_tn),
					       opnum,bb,usage_width)){
	...
	// Unsatisfied register constraints
	if ((result_reg <= TI_ISA_Register_Max()) &&
	    (result_subcl != ISA_REGSUBCLASS_UNDEFINED)) {
	  REGISTER_SET regset = REGISTER_SUBCLASS_members(result_subcl);
	  if (!REGISTER_SET_MemberP(regset, result_reg)) {
	    failed_constraint = TRUE;
	  }
	}
	if ( TI_ISA_Property_Set(PROP_has_multireg_operand, OP_code(op)) &&
	     TI_ISA_Valtyp_Is_Continuation(vtype) ) {
	  REGISTER prev_reg = LRA_TN_register(OP_result(op, resnum-1));
	  if ((prev_reg != REGISTER_UNDEFINED) &&
	      (prev_reg <= TI_ISA_Register_Max()) &&
	      (result_reg != prev_reg+OP_TN_Width(op, resnum-1, TRUE))) {
	    failed_constraint = TRUE;
	  }
	}
	if (failed_constraint) {
	  if (!Calculating_Fat_Points()) {
	    *spill_tn = result_tn;
	    Init_Fat_Point_Calculation(TN_register_class(*spill_tn),
				       opnum,bb,usage_width);
	    return FALSE;
	  }
	}
 ...
 */
        ...
        free_result[resnum] = (result_reg != REGISTER_UNDEFINED);
        free_result_reg[resnum] = result_reg;
        free_result_cl [resnum] = result_cl;
	if (asm_info && ASM_OP_result_clobber(asm_info)[resnum])
	  early_clobbers++;
      }
 ...
 */
  ...
  for (resnum = 0; resnum < nresults; resnum++) {
	...
	BOOL is_dedicated = TN_is_dedicated(free_result_tn[resnum]);
        assignment_undone = TRUE;
	UINT num_regs = OP_TN_Width(op, resnum, TRUE);
	Add_Avail_Reg (free_result_cl[resnum], free_result_reg[resnum], 
		       free_opnum[resnum], 
		       free_result_tn[resnum], is_dedicated, num_regs);
  ...
  for (opndnum = 0; opndnum < OP_opnds(op); opndnum++) {
    ...
    if (TN_is_register(tn) && (LRA_TN_register(tn) != REGISTER_UNDEFINED)) {
      clr = LR_For_TN (tn);
      reg = LRA_TN_register(tn);
      ISA_REGCLASS regclass = TN_register_class(tn);
      UINT num_regs = OP_TN_Width(op, opndnum, FALSE);
      if (Calculating_Fat_Points() &&
	  failing_class != regclass) {
	continue;
      }

      if ((opnum == LR_last_use(clr)) || 
	  ((opnum <= LR_first_def(clr)) && 
	   (opnum == LR_exposed_use(clr)) ||
	   (TN_is_local_reg(tn) && (opnum < LR_exposed_use(clr))))) {
	BOOL repeated_operand = FALSE;
	if (opnum != LR_first_def(clr)) {
	  for (INT resnum = 0; resnum < OP_results(op); resnum++) 
	    if (OP_result(op, resnum) == tn)
	      repeated_operand = TRUE;
	}
	for (INT opndnum2 = 0; opndnum2 < opndnum; opndnum2++) {
	  // check if the same tn or the same reg (with different tn due to preferencing)
	  // appeared earlier and the register has been removed from the available set already
	  TN*  opnd2_tn = OP_opnd(op, opndnum2);
	  if (opnd2_tn == tn ||
	      (TN_is_register(opnd2_tn) &&
	       TN_register_class(opnd2_tn)==TN_register_class(tn) &&
	       LRA_TN_register(opnd2_tn)==reg))
	    repeated_operand = TRUE;
	}
	if (!repeated_operand)
	  Delete_Avail_Reg (TN_register_class(tn), reg, opnum, 
			    tn, TN_is_dedicated(tn), num_regs);
      }
    }
  }
  ...
  for (opndnum = 0; opndnum < OP_opnds(op); opndnum++) {
    ...
    ISA_REGCLASS regclass = TN_register_class(tn);
    ISA_REGSUBCLASS regsubclass = ISA_REGSUBCLASS_UNDEFINED;
    BOOL is_dedicated = TN_is_dedicated(tn);
    INT reg_usage = Find_Register_Usage_For_OP(op, opndnum, FALSE);
    UINT num_regs = OP_TN_Width(op, opndnum, FALSE);
    REGISTER_SET must_use = Usable_Registers(tn, num_regs, clr);
    const ISA_OPERAND_INFO *opinfo = TI_ISA_Operand_Info(OP_code(op));
    const ISA_OPERAND_VALTYP *vtype = TI_ISA_Op_Operand(opinfo, opndnum);
    REGISTER prefer_reg = 
      Find_Register_Preference(op, opinfo, opndnum, FALSE, 
			       LR_prefer_reg (clr));
    regsubclass = asm_info ? 
      ASM_OP_opnd_subclass(asm_info)[opndnum] :
      TI_ISA_Valtyp_Regsubclass(vtype);
    reg = LRA_TN_register(tn);
      
    if ((reg != REGISTER_UNDEFINED) && reg<=TI_ISA_Register_Max() &&
	!REGISTER_allocatable (regclass, reg))
      continue;
	...
	if (!skip_tn) {
	  if (usage_width > 1) {
	    must_use = Fix_Usable_Registers_For_Multiregs(must_use, tn, clr);
	  }
	  if (prefer_reg != REGISTER_UNDEFINED &&
	      (!uniq_result || (prefer_reg != result_reg)) &&
	      (Is_Reg_Available (regclass, must_use, prefer_reg, clr, 
				 tn, is_dedicated, reg_usage) ||
	       Is_Non_Allocatable_Reg_Available (regclass, must_use,
                                 prefer_reg, clr, op))) {
	    ...
	    reg = Allocate_Register (tn, uniq_result, result_cl, must_use, 
				   result_reg, opnum, reg_usage);
	    if (reg == REGISTER_UNDEFINED) {
	      if (!Calculating_Fat_Points()) {
	        *spill_tn = tn;
	        //
    ...
    if (!skip_tn) {
      ...
      // Unsatisfied register constraints
      if ((reg <= TI_ISA_Register_Max()) &&
	  (regsubclass != ISA_REGSUBCLASS_UNDEFINED)) {
	REGISTER_SET regset = REGISTER_SUBCLASS_members(regsubclass);
	if (!REGISTER_SET_MemberP(regset, reg)) {
	  failed_constraint = TRUE;
	}
      }
      if ( TI_ISA_Property_Set(PROP_has_multireg_operand, OP_code(op)) &&
	   TI_ISA_Valtyp_Is_Continuation(vtype) ) {
	REGISTER prev_reg = LRA_TN_register(OP_opnd(op, opndnum-1));
	if ((prev_reg != REGISTER_UNDEFINED) &&
	    (prev_reg <= TI_ISA_Register_Max()) &&
	    (reg != prev_reg+OP_TN_Width(op, opndnum-1, FALSE))) {
	  failed_constraint = TRUE;
	}
      }
      if (failed_constraint) {
    ...
    // register.
    if (TN_is_local_reg(tn) && (reg != REGISTER_UNDEFINED) && 
	(opnum <= LR_exposed_use(clr))) {
      BOOL repeated_operand = FALSE;
      for (INT opndnum2 = 0; opndnum2 < opndnum; opndnum2++) 
	if (OP_opnd(op, opndnum2) == tn)
	  repeated_operand = TRUE;
      if (!repeated_operand) {
	BOOL is_dedicated = TN_is_dedicated(tn);
	Add_Avail_Reg (TN_register_class(tn), reg, opnum, tn, 
		       is_dedicated, num_regs);
	if (Always_Trace(Trace_LRA) && trace_tn &&
  ...
  /* Early clobber operands mess up our fatpoint calculation. The 
     registers are not free for this op, but are free above, so we
     don't really want to change the fatpoint for this op when we
     free these operands. So, we'll return the value extra_fatpoint,
     which will be added back to the fatpoint for op, but subtracted from
     the fatpoint for the next op up. */
  if (Calculating_Fat_Points()) {
    fat_points[opnum] += early_clobbers;
    *extra_fatpoint = early_clobbers;
  }
  return TRUE;
}
 ...
 * ======================================================================*/
static BOOL
Assign_Registers (BB *bb, TN **spill_tn, BOOL *redundant_code, INT usage_width)
{
  INT extra_fatpoint=0;
  ...
  for (INT opnum = BB_length(bb); opnum > 0; opnum--) 
  {
    ...
    // walk.
    //
    if (Calculating_Fat_Points() && (opnum != BB_length(bb))) {
      fat_points[opnum] = fat_points[opnum+1]-extra_fatpoint;
    }
    if (!Assign_Registers_For_OP (op, opnum, spill_tn, bb, &extra_fatpoint, 
				  usage_width)) {
      return FALSE;
    }

    if ((usage_width > 1) && Calculating_Fat_Points())
      return TRUE;
...
Compute_Livethrough_Set (BB *bb)
{
  ...
  FOR_ALL_BB_OPs_FWD (bb, op) {
	...
	UINT num_regs = OP_TN_Width(op, opndnum, FALSE);
        livethrough[cl] = 
	  REGISTER_SET_Difference (livethrough[cl], 
				   REGISTER_SET_Range(reg, reg+num_regs-1));
      }
    }
    for (resnum = 0; resnum < OP_results(op); resnum++) {
...
Spill_Global_Register (BB *bb, SPILL_CANDIDATE *best)
{
  ...
  ISA_REGCLASS cl = (ISA_REGCLASS)best->u1.s1.spill_cl;
  ...
  INT *moveto)
{
  ...
  ISA_REGCLASS cl = TN_register_class (LR_tn(lr));
  ...
  INT fatpoint)
{
  ...
  INT use_opnum = LR_last_use(lr);
  INT multiplier = MIN(fatpoint, TN_Register_Width(LR_tn(lr)));

  if (LR_use_cnt(lr) != 0 &&
      TN_is_local_reg(LR_tn(lr)) &&
      Can_Use_Be_Moved (lr, spill_opnum, &moveto) &&
      (best->spill_kind == SPILL_NONE || 
       best->benefit < (spill_opnum - moveto)*multiplier))
  {
  ...
  if (LR_def_cnt(lr) != 0 &&
      ...
      (best->spill_kind == SPILL_NONE ||
       best->benefit < (spill_opnum - def_opnum)*multiplier))
  {
...
Update_Fat_Points (SPILL_CANDIDATE *best, INT spill_opnum)
{
  ...
  for (i = low; i <= spill_opnum; i++) {
    //
    // if spilled tn is used at this op, then thereis no reduction in its
    ...
    if (!Op_Uses_TN(LR_tn(lr), i) || (i == spill_opnum && global_lr_spill)) {
      fat_points[i]-=TN_Register_Width(LR_tn(lr));
    }
  }
}
...
static BOOL
Is_Reg_Used_As_Dedicated_TN (OP *op, ISA_REGCLASS cl, REGISTER reg)
{
  TN *tmp_tn;
  INT i;
  for (i = 0; i < OP_results(op); i++) {
    tmp_tn = OP_result(op,i);
    if (TN_is_dedicated(tmp_tn) && TN_is_register(tmp_tn)) {
      if ( (TN_register_class(tmp_tn) == cl) && (LRA_TN_register(tmp_tn) == reg) ) {
        return TRUE;
      }
    }
  }
  ...
  BOOL block_allocation_failure)
{
  ...
  BOOL spill_lr_is_for_dedicated_tn = FALSE;
  ...
  //
  if (TN_register_class(spill_tn) == TI_ISA_Regclass_Integer() 
  ...
  for (INT i = first_def; i <= last_use; i++) 
  {
    ...
    if (!is_local_tn && !spill_lr_is_for_dedicated_tn) {
      spill_lr_is_for_dedicated_tn = 
        Is_Reg_Used_As_Dedicated_TN (op, spill_cl, spill_reg);
    }
    ...
    if (at_fatpoint) {
      /*
        If spill_tn is a local and referenced in this OP or
        if spill_tn is a global and its reg is used in this OP;
        then no benefit to spill spill_tn/reg for this OP.
       */
      if ( ! (
              (is_local_tn && (OP_Refs_TN(op, spill_tn) || 
                               OP_Defs_TN(op, spill_tn)))  ||
              (!is_local_tn && (OP_Refs_Reg(op, spill_cl, spill_reg) || 
	...
	cost += 1.0;
      ...
      // guard against OP_unknown_memdata() since the values may be inconsistent
      // between register and spill_loc
      // another issue is if the spill store requires more than one stores
      // (register wider than store width) then it is hard to make sure
      // all stores of the same spill are removed (see PR14868)
      if (!OP_unknown_memdata(op) &&
	  (!reloadable && (already_spilled && Is_OP_Spill_Store (op, spill_loc)))) {
	...
	lr_spans_spill_opnum = TRUE;
      ...
      // guard against OP_unknown_memdata() since the values may be inconsistent
      // between register and spill_loc
      // another issue is if the spill restore requires more than one loads
      // (register wider than store width) then it is hard to make sure
      // all loads of the same spill are removed (see PR14868)
      if (!OP_unknown_memdata(op) &&
	  (reloadable || (already_spilled && Is_OP_Spill_Load (op, spill_loc)))) {
  ...
  if (LR_last_use(spill_lr) == VECTOR_size(Insts_Vector)) {
    ...
    benefit += cur_benefit;
    if (!def_available) {
      if ( LR_global_live(spill_lr) || spill_lr_is_for_dedicated_tn ) {
        if (pending_store) {
          cost += store_cost;
          pending_store = FALSE;
        }
        ...
        // If we are spilling global TN used in BB-ending branch OP
        // an extra copy will be inserted by Fix_Xfer_Dependences
        if (LR_exposed_use(spill_lr) == last_use &&
            OP_xfer(OP_VECTOR_element(Insts_Vector, last_use))) {
          cost += restore_cost;
        }
      } else {
        // additional load'll likely be removed, so don't count it as real load.
        cost += 0.7; // a copy is 0.5, so choose 0.7 for a load.  
      } 
    }
    else {
  ...
  benefit *= MIN(fatpoint, TN_Register_Width(spill_tn));
...
//
//  Comment out this experimental code 
//  } else if (jons_test_flags[1]) {
//    if (lr_spans_spill_opnum &&
//	benefit > 0 &&
//	(best->spill_kind == SPILL_NONE ||
//	 ((benefit * best->cost) > (cost * best->benefit))))
//      {
//	best->spill_kind = SPILL_LIVE_RANGE;
//	best->u1.spill_lr = spill_lr;
//	best->cost = cost;
...
//
    ...
    /*
       Choose the smaller of two LRs based on radio cost/benefit. 

       Let  benefit_gain = benefit - best->benefit. There are three cases:
         1. If benefit_gain = 0, choose the one with smaller cost.
         2. If benefit_gain > 0, spilling spill_lr is about the same as spilling
            the best plus another LR whose min cost is min_cost. Thus, we adjust
            the cost by adding min_cost to the best's cost before calculating 
            cost/benefit radio. 
         3. If  benefit_gain < 0, similarly to 2.

       All three conditions can use the same formula below, note that we use
       multiply rather than division.

           (cost/benefit < best_cost/best_benefit) 
         =====>
           (cost * best_benefit < best_cost * benefit)
     */
    float min_cost = store_cost + restore_cost;
    INT benefit_gain = benefit - best->benefit;
    float best_cost;
    float curr_cost;
    if (benefit_gain > 0) {
       best_cost = best->cost + min_cost;
       curr_cost = cost;
    } else if (benefit_gain == 0) {
       best_cost = best->cost;
       curr_cost = cost;
    } else {
       best_cost =  best->cost;
       curr_cost = cost + min_cost;
    }
       
    if (lr_spans_spill_opnum && (benefit > 0) &&
	(best->spill_kind == SPILL_NONE ||
         ((curr_cost * best->benefit) < (best_cost * benefit))))
      {
	best->spill_kind = SPILL_LIVE_RANGE;
	best->u1.spill_lr = spill_lr;
	best->cost = cost;
...
Add_Spill_Load_Before_Use (TN *tn, ST *spill_loc, OP *reload_op, INT use, BB *bb)
{
  ...
  if (reload_op) {
    ...
    CGSPILL_Load_From_Memory (tn, (ST*)TN_home(tn), &spill_ops, CGSPILL_LRA, bb);
  }
  else {
...
Replace_TN_References (TN *old_tn, TN *new_tn, OP *op)
{
  ...
  for (INT opndnum = 0; opndnum < OP_opnds(op); opndnum++) {
    ...
    ISA_REGCLASS old_cl = TN_register_class(old_tn);
...
TN_Pair_In_OP(OP* op, struct tn *tn_res, struct tn *tn_opnd) 
{
  ...
  for (i = 0; i < OP_results(op); i++) {
    ...
    if (!TN_is_register(tn)) continue;
    ISA_REGCLASS old_cl = TN_register_class(tn_res);
    REGISTER old_reg = LRA_TN_register(tn_res);
    if (tn_res == tn ||
        (!TN_is_local_reg(tn_res) && 
         TN_register_class(tn) == old_cl &&
         LRA_TN_register(tn) == old_reg)) {
      break; 
    }
  }
  if (i == OP_results(op)) {
  ...
  for (i = 0; i < OP_opnds(op); i++) {
    TN *tn = OP_opnd(op, i);
    if (!TN_is_register(tn)) continue;
    ISA_REGCLASS old_cl = TN_register_class(tn_opnd);
    REGISTER old_reg = LRA_TN_register(tn_opnd);
    if (tn_opnd == tn ||
        (!TN_is_local_reg(tn_opnd) && 
         TN_register_class(tn) == old_cl &&
         LRA_TN_register(tn) == old_reg)) {
      return TRUE; 
    }
...
/***********************************************************************
 *
 *      This routine assumes that the given <reg> is a result in <op>.
...
static INT 
TN_Reg_Resnum_In_OP (OP* op, ISA_REGCLASS cl, REGISTER reg) 
{
  for (INT i = 0; i < OP_results(op); i++) {
    TN *res_tn = OP_result(op,i);
    if (TN_is_register(res_tn)) {
      if (TN_register_class(res_tn) == cl && LRA_TN_register(res_tn) == reg ) {
	return i;
      }
    }
  }
  ...
  INT spill_opnum)
{
  ...
  /* 
     If is_local_tn is false, spill_lr represents a live range that is for a global TN,
     a dedicated TN, or both (in this case, one or more global TNs and a dedicated TN
     share the same register). When spilling such a live range, a dedicated TN shall be
     saved/loaded more conservatively than a global TN, because of the lack of knowledge
     about where the dedicated TN is used (only by call OP ?).  A boolean variable 
     'spill_lr_is_for_dedicated_tn' is used for this purpose.
     
     If spill_lr_is_for_dedicated_tn is TRUE, the register used for spill_lr will be 
     assumed live at the end of a BB and saved right before a xfer OP (either branch or
     call OPs) if any.  (see GNAT 13459)

        TN11 (a10) = ...
        // spill TN11....
        ....
        // restore TN11 ....
        call8 foo  // used a10 as the first param
   */
   BOOL spill_lr_is_for_dedicated_tn = FALSE;
  ...
  for (i = first_def; i <= last_use; i++) 
  {
    ...
    {
      // guard against OP_unknown_memdata() since the values maybe inconsistent
      // between register and spill_loc
      // another issue is if the spill store requires more than one stores
      // (register wider than store width) then it is hard to make sure
      // all stores of the same spill are removed (see PR14868)
      if (!OP_unknown_memdata(op) &&
	  (!reloadable && Is_OP_Spill_Store (op, spill_loc) &&
	...
	Clobber_Op_Info(i, spill_cl);
      ...
      // If there is no definition available, reload from spill location.
      // (There must be a fatpoint that forces the spill of the definition.)
      if (!def_available) {
        /*
           If pending_store is true, we must insert a spill-store
           because there is a fatpoint that splits the use from its
           def.
         */
        if (pending_store) {
	...
	if (TN_has_spill(spill_tn))
      ...
      // Never replace 'ret TN1(a0)' with 'ret TNxxx' !
      // Later, since a0 is live out, a copy 'a0 = new_tn' must be generated !
      if ( !((OP_code(op) == TOP_ret || OP_code(op) == TOP_ret_n) &&
             TN_is_dedicated(spill_tn) && 
             (TN_register_and_class(spill_tn) == CLASS_AND_REG_ra)) ) {
	...
	// if the terminating instruction has same_res then we cannot simply
	// continue since the operand has changed but the result has not and
	// they have to match so we insert a copy before the op and change
	// the operand back to the spill_tn
	TN* prev_tn = new_tn ? new_tn : spill_tn;
        if (OP_same_res(op) && TN_Pair_In_OP(op, spill_tn, prev_tn)) {
	  //
	  // very similar to the treatment of OP_same_res case below
	  // but cannot use a new_tn since we need to restore the value
	  // of spill_tn
	  //
	  BOOL copy_added = FALSE;
	  TN* new_spill_tn = spill_tn;
	  for (INT opndnum = 0; opndnum < OP_opnds(op); opndnum++) {
	    TN* op_tn = OP_opnd(op, opndnum);
	    if (op_tn == prev_tn) {

	      // fix for PR14868
	      // for sameres global register, the result TN may be different
	      // from the spill_tn but uses the same register from GRA
	      // this can result from a live-through GRA register being
	      // spilled by LRA and the value is stored at the beginning of BB
	      // with dedicated reg TN (see Spill_Global_Register()) but the
	      // restore (load) at the end of BB used a new TN but assigned the
	      // same GRA register. Using the spill TN would only make the
	      // register be the same but not the sameres operand/result TNs
	      // we choose to use the result TN since the spill_tn would be
	      // a dedicated TN
	      if (!TN_is_local_reg (spill_tn)) {
		int sameres_result_idx =
			TI_ISA_Op_Sameres_Result (OP_code(op), opndnum);
		FmtAssert(sameres_result_idx!= -1,
			     ("Missing sameres result index"));
		TN* sameres_result_tn = OP_result(op,sameres_result_idx);
		FmtAssert(LRA_TN_register(sameres_result_tn)==LRA_TN_register(spill_tn),
			     ("Mismatch global register assignments"));
		new_spill_tn = sameres_result_tn;
	      }

              if (!copy_added) {
	        OPS copy_ops = OPS_EMPTY;
	        Exp_COPY(new_spill_tn, prev_tn, &copy_ops);
	        if (Do_LRA_Trace(Trace_LRA_Spill)) {
	          fprintf (TFile,
		    "LRA_SPILL>> copy TN%d to TN%d for the last same_res OP\n", 
	  	         TN_number(prev_tn), TN_number(new_spill_tn));
	        }
	        CGSPILL_Insert_Ops_Before(bb, op, &copy_ops);
	        copy_added = TRUE;
              }
	      Set_OP_opnd(op, opndnum, new_spill_tn);
	    }
	  }
	...
	continue;
      ...
      // all loads of the same spill are removed (see PR14868)
      if (!OP_unknown_memdata(op) &&
          (LR_reloadable(spill_lr) ||
	  (already_spilled && Is_OP_Spill_Load (op, spill_loc)))) {
        // If the definition of the spill_tn is a load from the spill
        // location, we don't have to insert the store to memory. Actually
        // we can get rid of the load as well, since we will be loading 
        // from the spill location before each use of spill_tn.
	...
	if ((resnum < 0) && !TN_is_local_reg (spill_tn))
	  resnum = TN_Reg_Resnum_In_OP (op, spill_cl, spill_reg);
      }

      else if (OP_cond_def(op) && 
	       (OP_result(op, 0) == spill_tn ||
		(!TN_is_local_reg(spill_tn) && 
		 TN_register_class(OP_result(op, 0)) == spill_cl &&
		 LRA_TN_register(OP_result(op, 0)) == spill_reg))) {
        OPS copy_ops = OPS_EMPTY;
        Exp_COPY(new_tn, prev_tn, &copy_ops);
        if (Do_LRA_Trace(Trace_LRA_Spill)) {
        ...
        for (resnum = OP_results(op)-1; resnum >= 0; resnum--) {
          if (spill_tn == OP_result(op,resnum) ||
	      (!TN_is_local_reg(spill_tn) && 
	       TN_register_class(OP_result(op, resnum)) == spill_cl &&
	       LRA_TN_register(OP_result(op, resnum)) == spill_reg)) 
	    break;
        }

      }
      FmtAssert(resnum >= 0 && resnum < OP_results(op), 
		("LRA: couldn't find result for spill\n"));
  ...
  if (LR_last_use(spill_lr) == VECTOR_size(Insts_Vector) &&
    ...
    if (!def_available) {
      // If it's live out OR no live out info, insert spill-load.
      if ( LR_global_live(spill_lr) || spill_lr_is_for_dedicated_tn ) {
        if (pending_store) {
          // if not done spill-store, do it now 
          Add_Spill_Store_After_Def (new_tn, last_def, bb);
          LR_first_spill(spill_lr) = last_def + 1;
          pending_store = FALSE;
        }
        ...
        if (Do_LRA_Trace(Trace_LRA_Spill)) {
          fprintf (TFile, 
            "LRA_SPILL>>      (Following load is for a live-out GTN%d)\n",
            TN_number(spill_tn));
          fprintf (TFile, 
            "LRA_SPILL>>      (Its spill-store must've been inserted.)\n");
        }
        ...
        if (Do_LRA_Trace(Trace_LRA_Spill)) {
          fprintf (TFile, 
            "LRA_SPILL>>      (Following load needed by LRA, even GTN%d is dead)\n",
            TN_number(spill_tn));
          fprintf (TFile, 
            "LRA_SPILL>>      (EBO following LRA should be able to remove it.)\n");
        }

        /*
           Logically, we don't need a load. But the LRA needs it to
           re-use the register for this spill_tn (it is the way LRA works
           now). Later, EBO will be able to remove this unneccesary load.
         */
        Add_Spill_Load_Before_Use (spill_tn, spill_loc, reloadable_def, 
                                   LR_last_use(spill_lr), bb);
      }
...
static void
Spill_Live_Range_To_Register(BB *bb, LIVE_RANGE *spill_lr, INT fatpoint, 
			     INT spill_opnum)
{
  TN *spill_tn = LR_tn(spill_lr);
  BOOL is_local_tn = TN_is_local_reg (spill_tn);
  ...
  first_def = MIN(LR_first_use(spill_lr), LR_first_def(spill_lr));
  last_use = LR_last_use(spill_lr);
  ...
  for (i = first_def; i <= last_use; i++) 
  {
    ...
    for (INT opndnum = 0; opndnum < OP_opnds(op); opndnum++) {
      TN *opnd = OP_opnd(op, opndnum);
      const ISA_OPERAND_VALTYP *vtype = TI_ISA_Op_Operand(opinfo, opndnum);
      if (TI_ISA_Valtyp_Regsubclass(vtype) == ISA_REGSUBCLASS_UNDEFINED)
	continue;
      if ((is_local_tn && (opnd == spill_tn)) ||
	  (!is_local_tn && TN_is_register(opnd) && 
	   (TN_register_class(opnd) == spill_cl) &&
	   (LRA_TN_register(opnd) == spill_reg))) 
	{
	new_tn = Dup_TN_Even_If_Dedicated (opnd);
	Set_OP_opnd(op,opndnum,new_tn);
	Exp_COPY(new_tn, opnd, &pre_ops);
	if (OP_same_res(op)) {
	  INT resnum = Op_Sameres_Result(op, opndnum);
	  if (resnum != -1) {
	    Set_OP_result(op,resnum,new_tn);
	    Exp_COPY(opnd, new_tn, &post_ops);
	  }
	}
	if (i == spill_opnum) {
	  INT opndnum2 = opndnum+1;
	  while (opndnum2 < OP_opnds(op)) {
	    TN *opnd2 = OP_opnd(op, opndnum2);
	    const ISA_OPERAND_VALTYP *vtype2 = 
	      TI_ISA_Op_Operand(opinfo, opndnum2);
	    if (!TI_ISA_Valtyp_Is_Continuation(vtype2))
	      break;
	    new_tn = Dup_TN_Even_If_Dedicated (opnd2);
	    Set_OP_opnd(op,opndnum2,new_tn);
	    Exp_COPY(new_tn, opnd2, &pre_ops);
	    if (OP_same_res(op)) {
	      INT resnum = Op_Sameres_Result(op, opndnum2);
	      if (resnum != -1) {
		Set_OP_result(op,resnum,new_tn);
		Exp_COPY(opnd2, new_tn, &post_ops);
	      }
	    }
	    opndnum2++;
	  }
    ...
    for (INT resnum = 0; resnum < OP_results(op); resnum++) {
      ...
      if (TI_ISA_Valtyp_Regsubclass(vtype) == ISA_REGSUBCLASS_UNDEFINED)
	continue;
      if ((is_local_tn && (res == spill_tn)) ||
	  (!is_local_tn && TN_is_register(res) && 
	   (TN_register_class(res) == spill_cl) &&
	   (LRA_TN_register(res) == spill_reg)))
	{
	new_tn = Dup_TN_Even_If_Dedicated (res);
	Set_OP_result(op,resnum,new_tn);
	Exp_COPY(res, new_tn, &post_ops);
	if (i == spill_opnum) {
	  INT resnum2 = resnum+1;
	  while (resnum2 < OP_results(op)) {
	    TN *res2 = OP_result(op, resnum2);
	    const ISA_OPERAND_VALTYP *vtype2 = 
	      TI_ISA_Op_Result(opinfo, resnum2);
	    if (!TI_ISA_Valtyp_Is_Continuation(vtype2))
	      break;
	    new_tn = Dup_TN_Even_If_Dedicated (res2);
	    Set_OP_result(op,resnum2,new_tn);
	    Exp_COPY(res2, new_tn, &post_ops);
	    resnum2++;
	  }
	  resnum2 = resnum;
	  while (resnum2 > 0) {
...
static void
Update_Live_LRs_Vector (INT opnum, ISA_REGCLASS cl)
{
  if (opnum == 1) return;
  ...
  MEM_POOL *pool)
{
  ...
  Live_LRs_Vector = VECTOR_Init (BB_length(bb)+TI_ISA_Register_Max(), pool);
...
static void OP_Replace_TN(TN *old_tn, TN *new_tn, OP *op)
{
  for (INT i = 0; i < OP_opnds(op); i++) {
    TN *opnd_tn = OP_opnd(op, i);
    if (opnd_tn == old_tn) {
...
static bool OPs_Refs_TN (INT start_opnum, INT end_opnum, TN *tn)
{
   for (INT n=start_opnum; n < end_opnum; n++) {
     OP *op = OP_VECTOR_element(Insts_Vector, n);
     if (OP_Refs_TN(op, tn)) {
       return true;
     }
   }
...
/*
  When a lr cannot be allocated, try to split a live range
  accross any ASM so that we can use registers that the ASM
  clobbers. For example:
  ...
  tn = ...                          tn = ...
   ...                               ...
                                     spill-store tn to tn_loc
  asm (clobbers a2, a3, a4...)      asm (clobbers a2, a3, a4...)
   ...                               ...
                                     spill-load tn_loc to tn'
     = tn                            = tn'


  This is the last try after the normal approach cannot
  allocate a register.
 */
bool LRA_Split_Lr_Asm(BB *bb, LIVE_RANGE *lr, INT usage_width)
{
  TN *tn=LR_tn(lr);
  ISA_REGCLASS cl = TN_register_class(tn);

  const int len_asm_clobber_regs = BB_length(bb) + 1;
  REGISTER_SET *asm_clobber_regs = 
           TYPE_MEM_POOL_ALLOC_N(REGISTER_SET, &lra_pool, len_asm_clobber_regs);
  for (int i=0; i < len_asm_clobber_regs; i++) {
     asm_clobber_regs[i] = REGISTER_SET_EMPTY_SET;
  }

  /* 
     usable_regs_clobbered are registers that have NOT been
     allocated and are clobbered by TOP_asm OPs. We can still allocate
     those registers as far as they are stored before and loaded after
     those TOP_asm OPs.
   */
  REGISTER_SET usable_regs_clobbered = 
               Usable_Registers(tn, usage_width, lr, asm_clobber_regs);
  usable_regs_clobbered =
      REGISTER_SET_Intersection (usable_regs_clobbered, avail_set[cl]);

  /*
     calculate the set of clobbered register, if it is empty, return
   */
  REGISTER_SET clobbered_regs = REGISTER_SET_EMPTY_SET;
  for (int i=0; i < len_asm_clobber_regs; i++) {
    if (REGISTER_SET_EmptyP(asm_clobber_regs[i])) continue;
    clobbered_regs = REGISTER_SET_Union(clobbered_regs, asm_clobber_regs[i]);
  }
  if (REGISTER_SET_EmptyP(clobbered_regs)) {
    // nothing clobbered, no need to do anything
    return false;
  }

  REGISTER_SET usable_regs = 
     REGISTER_SET_Difference (usable_regs_clobbered, clobbered_regs);
  if ( !REGISTER_SET_EmptyP(usable_regs) ) {
    // There ARE registers available for allocation, no need to split!
    return false;
  }
  
  /* 
     Now, no available registers except those clobbered registers
     Let's split across ASM that clobbers; so the LRA will be able
     to use clobbered registers.
   */

  INT first_op = MAX(MIN(LR_first_use(lr), LR_first_def(lr)), 1);
  INT last_op = MIN(LR_last_use(lr)+1, VECTOR_size(Insts_Vector));
  bool def_available = false;
  bool val_in_memory = false;
  bool need_new_tn_for_next_ref = false;
  TN *new_tn = tn;   // new_tn is used for renaming tn.
  ST *spill_loc=TN_spill(new_tn);
  for (INT opnum = first_op; opnum < last_op; opnum++) {
    OP* op = OP_VECTOR_element(Insts_Vector, opnum);
    if ( op == NULL ) continue;

    // First, process existing spill store/load
    // If there are, prevent from inserting store/load again
    if (Is_OP_Spill_Load(op, spill_loc)) {
      if (TNs_Are_Equivalent(tn, OP_result(op, 0))) {
        def_available = true;
        val_in_memory = true;
        OP_Replace_TN (tn, new_tn, op);
        continue;
      } else {
        // When a LR of a GTN is splited by GRA, it could create the case 
        // and reach this point.
        DevWarn("The same spill loc is loaded into different TNs in BB:%d", BB_id(bb));
      }
    } else if (Is_OP_Spill_Store(op, spill_loc)) {
      val_in_memory = true;
      OP_Replace_TN (tn, new_tn, op);
      continue;
    }
      
    if (OP_Refs_TN(op, tn) || OP_Defs_TN(op, tn)) {
      if (need_new_tn_for_next_ref) {
        new_tn = Dup_TN_Even_If_Dedicated (tn);
        need_new_tn_for_next_ref = false;
      }
      if (OP_Refs_TN(op, tn)) {
        // Do spill-load before a use and use a new TN.
        if (!def_available) {
          OPS spill_ops = OPS_EMPTY;
          if (spill_loc != NULL) {
            CGSPILL_Load_From_Memory(new_tn, spill_loc, &spill_ops, CGSPILL_LRA, bb);
            CGSPILL_Insert_Ops_Before (bb, op, &spill_ops);
            if (Do_LRA_Trace(Trace_LRA_Spill)) {
              fprintf (TFile, 
                 "LRA_SPILL>>    load TN%d before %d (Split across ASM)\n",
                 TN_number(tn), opnum);
            }
          } else {
            if (Do_LRA_Trace(Trace_LRA_Spill)) {
              fprintf (TFile,
                "LRA_SPILL>>    use undefined TN%d at %d (Split across ASM)\n",
                TN_number(tn), opnum);
            }
          }

          /* Set def_available to true even if no load under spill_loc == null,
             so that it will avoid another load for the next use. 
          */
          def_available = true;
          val_in_memory = true;
        }
    ...
    // Process ASM
    if (!REGISTER_SET_EmptyP(asm_clobber_regs[opnum]) ) {
      Is_True(OP_code(op) == TOP_asm, ("LRA: OP should be ASM op"));

      // Only insert spill-store if tn is defined before AND it's not been
      // spill-stored AND it is still live at this point.
      if ( def_available && !val_in_memory && 
           OPs_Refs_TN(opnum+1, last_op, tn) ) {
        // Do spill-store before TOP_asm 
        OPS spill_ops = OPS_EMPTY;
        if (TN_has_spill(new_tn)) {
          spill_loc = TN_spill(new_tn);
        } else {
          spill_loc = CGSPILL_Get_TN_Spill_Location (new_tn, CGSPILL_LRA);
        }
        CGSPILL_Store_To_Memory (new_tn, spill_loc, &spill_ops, CGSPILL_LRA, bb);
        CGSPILL_Insert_Ops_Before(bb, op, &spill_ops);
        val_in_memory = true;

        if (Do_LRA_Trace(Trace_LRA_Spill)) {
          fprintf (TFile, 
               "LRA_SPILL>>    store TN%d before %d (Split across ASM)\n",
               TN_number(tn), opnum);
        }
      }
      def_available = false;

      // Need a new TN so we can actually split live range for
      // TN across ASM
      need_new_tn_for_next_ref = true;
    }
  }
  return true;
} 
 ...
 * ======================================================================*/
static void
Fix_LRA_Blues (BB *bb, TN *tn, HB_Schedule *Sched, INT usage_width)
{
  ...
  if (Trip_Count > MAX_TRIP_COUNT) {
    REGISTER_SET allocatable_set = REGISTER_CLASS_allocatable(cl);
    REGISTER_SET reg_set = allocatable_set;
    int width = TN_Register_Width(tn);
    if ((TI_ABI_Property_Set(ABI_PROPERTY_allocatable,cl, /*reg=*/-1)==FALSE)) {
      ErrMsg(EC_Unallocatable_Register_File, REGISTER_CLASS_name(cl));
    } else {
      BOOL enough_allocatable = FALSE;
      for ( REGISTER member = REGISTER_SET_Choose(reg_set);
	    member != REGISTER_UNDEFINED && !enough_allocatable;
	    member = REGISTER_SET_Choose_Next(reg_set,member))
      {
        REGISTER_SET range = REGISTER_SET_Range(member, member+width-1);
	if (REGISTER_SET_ContainsP(allocatable_set, range))
	  enough_allocatable = TRUE;
      }
      if (enough_allocatable)
        ...
        ErrMsg(EC_Not_Enough_Allocatable, REGISTER_CLASS_name(cl));
   }
  }
  ...
  if (Check_Allow_Reschedule()) {
    ...
    mINT8 regs_avail[TI_ISA_REGCLASS_MAX+1];
    FOR_ALL_ISA_REGCLASS(rc)
      {
	regs_avail[rc] = REGISTER_SET_Size (avail_set[rc]);
      }

    bool Sched_created = false;
    if (!Sched) {
    ...
    if (Sched_created) CXX_DELETE(Sched, &MEM_local_pool);
    return;
  }
  ...
  if (usage_width == 1) {
    ...
    // Because of register constraints, the failure point may not be the last
    // use in a live range.
    failure_point = failing_opnum;
  }
  if (Do_LRA_Trace(Trace_LRA_Spill)) {
    fprintf (TFile,"LRA_SPILL>> Out of Registers (BB:%d) trip:%d\nLRA_SPILL>>",
	     BB_id(bb), Trip_Count);
  ...
  bool try_split_lr_asm = false;

  if (failure_point !=0) {
    OP *op = OP_VECTOR_element (Insts_Vector, failure_point);
    if (!TI_PROC_Property_Set(PROP_has_branch_delay_slot) && OP_xfer(op)) {
      if (Do_LRA_Trace(Trace_LRA_Spill)) {
        fprintf(TFile,"LRA_SPILL>> can not reload registers after a branch.\n");
        Print_BB(bb);
      }

      // try one more times again before assertion
      if ( LRA_Split_Lr_Asm(bb, lr, usage_width) ) {
        return;
      }
  ...
  if (Get_Trace (TP_ALLOC, 0x20, BB_id(bb))) {
  ...
  if (usage_width > 1) {
    for (REGISTER reg = REGISTER_MIN; reg <= TI_ISA_Register_Max(); reg += usage_width) {
      block_available = TRUE;
      for (INT i=0 ; i < usage_width; i++) {
	if (!failure_avail_regs[reg+i])
    ...
    /* Single live range with conflicting constraints */
  if ((usage_width > 1) && block_available) {
    Spill_Live_Range_To_Register (bb, lr, fatpoint, failing_opnum);
    Simple_Trip_Count++;
  } else {
    /* traverse back from the instruction where register allocation 
     * failed and spill/reorder for all fatpoints above this.
     */
    Regclass_Trip_Count[failing_class]++;
    fatpoint_min = 0;
    for (opnum = failure_point; opnum > 0; ) {
      FAT_POINTS_TYPE cur_usage = fat_points[opnum];
      BOOL reordering_failure = false;
      BOOL block_allocation_failure = (usage_width > 1) && (opnum == failure_point);
      // 
	...
	if (opnum < first_fatpoint) break;
      ...
      if ((opnum < first_fatpoint) && !block_allocation_failure) break;
      
      // Check if there is a register problem at the current instruction.
      if ((fat_points[opnum] > fatpoint_min) || block_allocation_failure) {
	
	SPILL_CANDIDATE *best = TYPE_ALLOCA(SPILL_CANDIDATE);
	INT best_width;
	REGISTER reg, best_reg = REGISTER_UNDEFINED;
	best->spill_kind = SPILL_NONE;
	...
	if (Do_LRA_Trace(Trace_LRA_Spill)) {
          if (allow_reordering) {
	    fprintf (TFile,
	  	     "Analyze >> analyze reordering for OP:%d in BB:%d (fp=%d)\n",
	...
	if (block_allocation_failure) {
	  SPILL_CANDIDATE *cand = TYPE_ALLOCA(SPILL_CANDIDATE);
	  float reg_cost[TI_ISA_Register_Max()+1];
	
	  for (reg = REGISTER_MIN; reg < TI_ISA_Register_Max()+1; reg++) {
	    if (failure_avail_regs[reg])
	      reg_cost[reg] = 0;
	    else
	      reg_cost[reg] = INT32_MAX;
	  }
	  ...
	  if (!allow_reordering && !CG_localize_tns) {
	    ...
	    reg = REGISTER_SET_Choose (livethrough[cl]);
	    while (reg != REGISTER_UNDEFINED) {
	      float store_cost, restore_cost;
	      TN *spill_tn = Build_Dedicated_TN (cl, reg, 0);
	      CGSPILL_Cost_Estimate (spill_tn, NULL, &store_cost, &restore_cost, 
				     CGSPILL_LRA);
	      reg_cost[reg] = store_cost + restore_cost;     
	      reg = REGISTER_SET_Choose_Next (livethrough[cl], reg);
	    }
	  }
	  
	  for (INT i = 0; i < VECTOR_count(Live_LRs_Vector); i++) {
	    LIVE_RANGE *clr = LR_VECTOR_element (Live_LRs_Vector, i);
	    reg = LRA_TN_register(LR_tn(clr));
	    if ((reg != REGISTER_UNDEFINED) && (reg <= TI_ISA_Register_Max())) {
	      best->spill_kind = SPILL_NONE;
	      best->spill_num = opnum;
	      if (allow_reordering) {
		Analyze_Reordering (clr, best, opnum, fatpoint);
	      }
	      else {
		Analyze_Spilling_Live_Range (clr, opnum, best, fatpoint, 
					     block_allocation_failure);
	      }
	      if (best->spill_kind != SPILL_NONE) {
		INT num_regs = TN_Register_Width(LR_tn(clr));
		for (INT j = 0; j < num_regs; j++) {
		  reg_cost[reg+j] = best->cost/MIN(num_regs,usage_width);
		}
	      }
	    }
	  }
	  
	  for (reg = REGISTER_MIN; reg <= TI_ISA_Register_Max(); reg += usage_width) {
	    float bulk_cost = 0;
	    for (REGISTER reg2 = reg ; reg2 < reg+usage_width; reg2++)
	      bulk_cost += reg_cost[reg2];
	    for (INT i=0 ; i < usage_width; i++)
	      reg_cost[reg+i] = bulk_cost;
	  }
	  ...
	  if (!allow_reordering && !CG_localize_tns) {
	    ...
	    if (best->spill_kind != SPILL_NONE) {
	      best->cost = reg_cost[best_reg];
	      reg = REGISTER_SET_Choose (livethrough[cl]);
	      while (reg != REGISTER_UNDEFINED) {
		if (reg_cost[reg] < best->cost) {
		  best->cost = reg_cost[reg];
		  best->u1.s1.global_spill_reg = reg;
		}
		reg = REGISTER_SET_Choose_Next (livethrough[cl], reg);
	      }
	  ...
	  for (INT i = 0; i < VECTOR_count(Live_LRs_Vector); i++) {
	    ...
	    reg = LRA_TN_register(LR_tn(clr));
	    if ((reg != REGISTER_UNDEFINED) && (reg <= TI_ISA_Register_Max())) {
	      cand->spill_kind = SPILL_NONE;
	      cand->spill_num = opnum;
	      if (allow_reordering) {
		Analyze_Reordering (clr, cand, opnum, fatpoint);
	      }
	      else {
		Analyze_Spilling_Live_Range (clr, opnum, cand, fatpoint,
					     block_allocation_failure);
	      }
	      if ((cand->spill_kind != SPILL_NONE) && 
		  ((best->spill_kind == SPILL_NONE) ||
		   (reg_cost[reg] < best->cost))) {
		*best = *cand;
		best->cost = reg_cost[reg];
	      }
	    }
	...
	if (best->spill_kind == SPILL_MOVE_DEF || 
	    best->spill_kind == SPILL_MOVE_USE)
	  {
	    ...
	    // However, update the fat_points array and remove the relevant 
	    // live ranges from the Live_LRs_Vector so that they are not 
	    // considered again.
	    Update_Fat_Points (best, opnum);
	    best->next = transformations;
	    transformations = best;
	    best_width = TN_Register_Width(LR_tn(best->u1.s2.move_lr));
	    best_reg = LRA_TN_register(LR_tn(best->u1.s2.move_lr));
	  }
	  ...
	  fatpoint_min++;
	  livethrough[cl] = 
	    REGISTER_SET_Difference1 (livethrough[cl], 
				      best->u1.s1.global_spill_reg);
	  best_width = 1;
	  best_reg = best->u1.s1.global_spill_reg;
	}
	else if (best->spill_kind == SPILL_LIVE_RANGE) {
	  Spill_Live_Range (bb, best->u1.spill_lr, fatpoint, opnum);
	  Update_Fat_Points (best, opnum);
	  best_width = TN_Register_Width(LR_tn(best->u1.spill_lr));
	  best_reg = LRA_TN_register(LR_tn(best->u1.spill_lr));
	  ...
	  if (Do_LRA_Trace(Trace_LRA_Spill)) {
	    fprintf (TFile, "        >> reordering failed at OP:%d in BB:%d\n",
		   opnum, BB_id(bb));
	  }
	}
	
	if (block_allocation_failure && (best_reg != REGISTER_UNDEFINED)) {
	  for (INT j = 0; j < best_width; j++) {
	    failure_avail_regs[best_reg+j] = TRUE;
	  }
	  for (reg = REGISTER_MIN; reg <= TI_ISA_Register_Max(); reg += usage_width) {
	    block_available = TRUE;
	    for (INT i=0 ; i < usage_width; i++) {
	...
	opnum--;
      ...
      //
      if (!block_allocation_failure || block_available) {
  ...
  if (allow_reordering) {
    ...
    try_split_lr_asm = false;
  }

  // try split across asm if no oredering happens AND
  // some fatpoints are not reduced.
  if (try_split_lr_asm) {
    LRA_Split_Lr_Asm(bb, lr, usage_width);
  }
...
void Alloc_Regs_For_BB (BB *bb, HB_Schedule *Sched)
{
  ...
  Trip_Count = 0;
  Simple_Trip_Count = 0;
  memset(Regclass_Trip_Count, 0, (TI_ISA_REGCLASS_MAX+1)*sizeof(INT));
  Magic_Spill_Location = Local_Spill_Sym;
  max_usage_width = Find_Max_Usage_Width(bb);
  ...
  do {
    ...
    INT usage_width = max_usage_width<<1;
    do {
      usage_width >>= 1;
      Init_Avail_Regs ();
      lra_done = Assign_Registers (bb, &tn, &redundant_code, usage_width);
      if (Calculating_Fat_Points()) {
        memset(fat_points, 0, (BB_length(bb)+1)*sizeof(FAT_POINTS_TYPE));
	Init_Avail_Regs ();
	Assign_Registers (bb, NULL, &redundant_code, 1);
	lra_done = FALSE;
      }
    } while ((usage_width > 1) && lra_done);

    if (!lra_done) {
      Fix_LRA_Blues (bb, tn, Sched, usage_width);
    }
...
{
  REGISTER_SET temps[TI_ISA_REGCLASS_MAX+1];
  OP *op;
  OP *sp_adj;
...
Prolog_save_code(BB *entrybb)
{
  ...
  for (callee_num = 0; callee_num < Callee_Saved_Regs_Count; callee_num++) {
    ...
    ISA_REGCLASS cl = TN_save_rclass(tn);
...
Move_Spill_Loads_Stores (BB *bb)
{
  ...
  for (op = BB_first_op(bb); op != NULL; op = next_op) {
	...
	TN_MAP_Set (spills_map, result_tn, op);
      ...
      TN *src_tn = NULL;
      ST *spill_loc = NULL;
      if (TI_TOP_Find_Operand_Use(OP_code(op), OU_storeval)>=0) {
        src_tn = OP_opnd(op,TI_TOP_Find_Operand_Use(OP_code(op), OU_storeval));
        spill_loc = TN_spill(src_tn);
      }
      if (spill_loc && Is_OP_Spill_Store (op, spill_loc)) {
...
static REGISTER
Single_Register_Subclass (ISA_REGSUBCLASS subclass)
{
...
Preallocate_Single_Register_Subclasses (BB* bb)
{
  ...
  FOR_ALL_BB_OPs (bb, op) {
    ...
    INT i;    
    for (i = 0; i < OP_opnds(op); i++) {
      ISA_REGSUBCLASS subclass = asm_info ?
...
/* Initialize the unused TN definitions. */
  FOR_ALL_ISA_REGCLASS(cl) unused_tn_def[cl] = NULL;
  if (True_TN) unused_tn_def[TN_register_class(True_TN)] = True_TN;

  FOR_ALL_ISA_REGCLASS(cl) last_assigned_reg[cl] = REGISTER_UNDEFINED;
  HB_Schedule *Sched = NULL;
  global_spills = 0;

  regsubclass_br1 = TI_ISA_Regsubclass_Branch1();
  regsubclass_br2 = TI_ISA_Regsubclass_Branch2();
  regsubclass_br4 = TI_ISA_Regsubclass_Branch4();
  regsubclass_br8 = TI_ISA_Regsubclass_Branch8();
  regsubclass_br16 = TI_ISA_Regsubclass_Branch16();
  ...
  for (bb = REGION_First_BB; bb != NULL; bb = BB_next(bb)) 
  {
    ...
    Trace_LRA = Get_Trace (TP_ALLOC, 0x01, BB_id(bb));
    Trace_LRA_Detail = Get_Trace (TP_ALLOC, 0x02, BB_id(bb));
    Trace_LRA_Spill = Get_Trace (TP_ALLOC, 0x04, BB_id(bb));
    Trace_LRA_Entry_Exit = Get_Trace (TP_ALLOC, 0x08, BB_id(bb));
    Trace_Move_GRA_Spills = Get_Trace (TP_ALLOC, 0x10, BB_id(bb));
...
#define Reg_Request(bb,cl)      \
  (Reg_Request_Table[BB_id(bb)*TI_ISA_Num_Regclasses() + cl - TI_ISA_Regclass_First()])
static INT Reg_Table_Size;

static mINT8 *Secondary_Reg_Request_Table;
#define Secondary_Reg_Request(bb,cl)      \
  (Secondary_Reg_Request_Table[BB_id(bb)*TI_ISA_Num_Regclasses() + cl - TI_ISA_Regclass_First()])

static INT Secondary_Reg_Table_Size;
...
LRA_Init (void)
{
  ...
  Reg_Table_Size = 0;
  Secondary_Reg_Request_Table = NULL;
  Secondary_Reg_Table_Size = 0;
}
...
LRA_examine_last_op_needs (BB *bb, ISA_REGCLASS cl)
{
  ...
  OP *last_op = BB_branch_op(bb);
  if (last_op) {
    for (INT i = 0; i < OP_opnds(last_op); i++) {
      TN *tn = OP_opnd(last_op, i);
      if ((tn != NULL) &&
          !TN_is_constant(tn) &&
          !TN_is_dedicated(tn) &&
          !(LRA_TN_register(tn) != REGISTER_UNDEFINED) &&
           (TN_register_class(tn) == cl))
	min_regs+=OP_TN_Width(last_op,i,FALSE);
    }
  }

  if ((min_regs == 0) && (cl == TI_ISA_Regclass_Integer())) {
...
LRA_Estimate_Fat_Points (
  ...
  MEM_POOL* pool)
{
  mINT8 inuse[TI_ISA_REGCLASS_MAX+1];
  OP *op;
  INT i, opnum, opndnum;
  INT tot_use = 0;
  ...
  for (i = ISA_MIN_REGCLASS; i <= TI_ISA_Num_Regclasses(); i++) {
    fatpoint[i] = 0;
    inuse[i] = 0;
  }
  ...
  for (opnum = BB_length(bb); opnum > 0; opnum--) 
  {
    ...
    for (i = 0; i < OP_results(op); i++) {
      TN *result_tn = OP_result(op, i);
      ISA_REGCLASS regclass = TN_register_class(result_tn);
      ...
      if (opnum == LR_first_def(clr)) {
        if ((LR_assigned(clr) ||
	     (!regs_in_use && !TN_is_local_reg(result_tn))) &&
            (inuse[regclass] != -128))
        {
	  int num_regs = OP_TN_Width(op,i,TRUE);
          inuse[regclass]-=num_regs;
	  if (regs_in_use &&  (LR_last_use(clr) != BB_length(bb)+1) &&
	      !LR_exposed_use(clr) && (LR_last_use(clr) != LR_first_def(clr))){
	    regs_in_use[opnum] = (tot_use-=num_regs);
	  }
        }
      }
    }
    ...
    for (opndnum = 0; opndnum < OP_opnds(op); opndnum++) {
      ...
      if (TN_is_register(tn)) {
        ...
        if (!LR_assigned(clr)) {
	      ...
	      int num_regs = OP_TN_Width(op,opndnum,FALSE);
	      inuse[regclass]+=num_regs;
	      if (regs_in_use && (LR_last_use(clr) != BB_length(bb)+1) &&
	      !LR_exposed_use(clr) && (LR_last_use(clr) != LR_first_def(clr))){
		regs_in_use[opnum] = (tot_use+=num_regs);
...
LRA_Compute_Register_Request (BB *bb, MEM_POOL *pool)
{
  ...
  if (Reg_Request_Table == NULL) {
    ...
    Reg_Request_Table = 
          TYPE_PU_ALLOC_N (mINT8, (PU_BB_Count+2)*TI_ISA_Num_Regclasses());
  ...
  /* Determine the minimum number of registers that we need to avoid
     having to spill after a branch at the end of a block (any spill
     code that LRA introduces must be above a terminating branch). */

  for (INT i = ISA_MIN_REGCLASS; i <= TI_ISA_Num_Regclasses(); i++)
    fatpoint[i] = MAX(fatpoint[i], LRA_examine_last_op_needs (bb, (ISA_REGCLASS)i));

  if (Get_Trace (TP_ALLOC, 0x20, BB_id(bb))) {
    fprintf (TFile, "After branch adjust (BB:%d)\t", BB_id(bb));
    for (INT i = ISA_MIN_REGCLASS; i <= TI_ISA_Num_Regclasses(); i++) {
      fprintf (TFile, "[%d]:%d  ", i, fatpoint[i]);
    }
    fprintf (TFile, "\n");
...
void
LRA_Save_Register_Request (BB *bb)
{
  // Allocate the Secondary_Reg_Request_Table if not yet allocated.
  if (Secondary_Reg_Request_Table == NULL) {
    /* this needs to be initialized to 0, so use a zeroing pool */
    Secondary_Reg_Request_Table = 
          TYPE_PU_ALLOC_N (mINT8, (PU_BB_Count+2)*TI_ISA_Num_Regclasses());
    Secondary_Reg_Table_Size = PU_BB_Count;
  }
  Is_True (BB_id(bb) <= Secondary_Reg_Table_Size, 
	   ("incorrect BB_id: %d", BB_id(bb)));

  mINT8 *fatpoint = &Reg_Request(bb,0);
  mINT8 *fatpoint2 = &Secondary_Reg_Request(bb,0);

  for (INT i = ISA_MIN_REGCLASS; i <= TI_ISA_Num_Regclasses(); i++)
    fatpoint2[i] = fatpoint[i];
}
 ...
 * ======================================================================*/
INT
LRA_Register_Request (BB *bb,  ISA_REGCLASS cl)
{
  INT regs_needed;
...
INT
LRA_Secondary_Register_Request (BB *bb,  ISA_REGCLASS cl)
{
  INT regs_needed;

  FmtAssert (!BB_reg_alloc(bb), ("LRA_Register_Request: Invalid bb %d", BB_id(bb)));

  if (BB_id(bb) <= Secondary_Reg_Table_Size) {
    regs_needed = Secondary_Reg_Request (bb, cl);
  }
  else {
    regs_needed = 0;
  }
