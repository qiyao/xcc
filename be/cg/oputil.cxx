 ...
 * Module: oputil.c
 * $Revision: 3.134 $
 * $Date: 2000/08/31 23:36:57 $
 * $Author: mpm $
 * $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/oputil.cxx,v $
 *
 * Revision history:
...
Mk_OP(TOP opr, ...)
{
  ...
  FmtAssert(!TI_ISA_Property_Set(PROP_var_opnds, opr),
	    ("Mk_OP not allowed with variable operands"));
  ...
  if (TI_ISA_Property_Set(PROP_defs_fpu_int, opr))
    Set_TN_is_fpu_int(OP_result(op, 0));
...
Mk_VarOP(TOP opr, INT results, INT opnds, TN **res_tn, TN **opnd_tn)
{
  if (results != TOP_fixed_results(opr)) {
    FmtAssert(TI_ISA_Property_Set(PROP_var_opnds, opr) && results > TOP_fixed_results(opr),
	      ("%d is not enough results for %s", results, TI_TOP_Name(opr)));
  }
  if (opnds != TOP_fixed_opnds(opr)) {
    FmtAssert(TI_ISA_Property_Set(PROP_var_opnds, opr) && opnds > TOP_fixed_opnds(opr),
	      ("%d is not enough operands for %s", opnds, TI_TOP_Name(opr)));
  }
...
void Print_OP_No_SrcLine(const OP *op)
{
  ...
  fprintf(TFile, ":- ");
  fprintf(TFile, "%s ", TI_TOP_Name(OP_code(op)));
  if ( OP_variant(op) != 0 ) {
  ...
  if (OP_scycle(op) >= 0) {
    fprintf(TFile, " scyc:%d", OP_scycle(op));
  }

  if (OP_profile_bb_id(op) > 0) {
    fprintf(TFile, " prof_bb:%d", OP_profile_bb_id(op));
  }
...
BOOL
OP_Defs_Reg(const OP *op, ISA_REGCLASS cl, REGISTER reg)
{
  register INT num;
...
BOOL
OP_Refs_Reg(const OP *op, ISA_REGCLASS cl, REGISTER reg)
{
  register INT num;
...
/* ====================================================================
 *
 * OP_Real_Inst_Btes - How many bytes does this op really represent,
 * i.e. will be emitted.
 *
 * ==================================================================== */

INT
OP_Real_Inst_Bytes( const OP *op )
{
  if ( op == NULL || OP_dummy(op) ) {
    return 0;
  }
  else if ( OP_simulated(op) ) {
    return Simulated_Op_Real_Inst_Bytes (op);
  }
  return OP_inst_bytes(op);
}
 ...
 * OP_noop
 *
 * Return TRUE if the OP is a noop taking bundle format and slot
 * into consideration.
 *
 * ====================================================================
 */

BOOL OP_noop(const OP *op) 
{
  if (TI_ISA_Property_Set(PROP_noop, OP_code(op)))
    return true;

  if (OP_format_num(op) != -1 && OP_slot_num(op) != -1) {
    INT bundle = OP_format_num(op);
    INT slot = OP_slot_num(op);
    ISA_EXEC_UNIT_PROPERTY prop = TI_ISA_Exec_Slot_Prop (bundle, slot);
    if (TI_ISA_Noop_Top(prop)==OP_code(op))
      return true;
  }
  return FALSE;
}
...
INT32 Op_Sameres_Operand (OP *op, INT32 result_index)
{
  if (OP_code(op) == TOP_asm) {
    ASM_OP_ANNOT* asm_info = (ASM_OP_ANNOT*) OP_MAP_Get(OP_Asm_Map, op);
    for (int i=0; i<OP_opnds(op); i++) {
      char *constraint = (char*)ASM_OP_opnd_constraint(asm_info)[i];
      if (*constraint=='\0')
	return -1;
      while (strchr("=&%in", *constraint))
	constraint++;
      if (isdigit(*constraint) && (*constraint - '0' == result_index))
	return i;
    }
  } else 
    return TI_ISA_Op_Sameres_Operand(OP_code(op), result_index);
  return -1;
}

INT32 Op_Sameres_Result (OP *op, INT32 operand_index)
{
  if (OP_code(op) == TOP_asm) {
    ASM_OP_ANNOT* asm_info = (ASM_OP_ANNOT*) OP_MAP_Get(OP_Asm_Map, op);
    char *constraint = (char*)ASM_OP_opnd_constraint(asm_info)[operand_index];
    if (*constraint=='\0')
      return -1;
    while (strchr("=&%in", *constraint))
      constraint++;
    ...
    return TI_ISA_Op_Sameres_Result(OP_code(op), operand_index);
  return -1;
}

INT32 OP_Stall(OP* pred, OP* succ) {
  ...
  if (OP_extern_effects(pred) && OP_extern_effects(succ)) {
    if (TI_TOP_Has_Export_State(OP_code(pred)) &&
	TI_TOP_Has_Tie_Queue(OP_code(succ)))
      stall = MAX(1,stall);
  }
