 ...
 *
 *          Represents a register within a ISA_REGCLASS.  The
 *          ISA_REGCLASS information is not determined from the
 ...
 *
 *
 *      TYPE ISA_REGCLASS, mISA_REGCLASS (defined by targinfo)
 *
 *          Represents a register class.  Together with a REGISTER,
 *          determines a unique register.  ISA_REGCLASS's are a
 *          small numeric type and may be freely treated as such.
 *
 *
 *      TYPE ISA_REGSUBCLASS, mISA_REGSUBCLASS (defined by targinfo)
 *
 *          Represents a register subclass.  ISA_REGSUBCLASS's are a
 *          small numeric type and may be freely treated as such.
 *
 *
 *      TYPE REGISTER_SET
 ...
 *          every ISA_REGCLASS that has a member.  Use with
 *          REGISTER_CLASS_last_register (see below) to loop over the
 *          members of a ISA_REGCLASS.
 *
 ...
 *	ISA_REGCLASS REGISTER_CLASS_xxx
 *	mUINT              CLASS_AND_REG_xxx
 *
 ...
 *
 *
 *	ISA_REGCLASS Register_Class_For_Mtype(
 *	    TYPE_ID mtype
 *	)
 *
 *	    Given an MTYPE, return the corresponding register class
 *	    used to hold values of that type. Return 
 *	    ISA_REGCLASS_UNDEFINED if there is no correspondence.
 *
 *  Mapping from TNs to ISA_REGCLASSes
 *
 *      void REGISTER_CLASS_OP_Update_Mapping(
 *          struct op *op
 *      )
 *
 *          Update TN -> ISA_REGCLASS mappings for all TNs
 *          referenced in the given 'op'.
 *
 *
 *      ISA_REGCLASS TN_register_class(
 *          TN *tn
 *      )
 *
 ...
 *
 *
 *      const ISA_REGCLASS *REGISTER_CLASS_vec
 *
 ...
 *
 *
 *	ISA_REGCLASS REGISTER_SUBCLASS_register_class(
 *	    ISA_REGSUBCLASS sc
 *	)
 *
...
#ifdef _KEEP_RCS_ID
static const char register_rcs_id[] = "$Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/register.h,v $ $Revision: 3.66 $";
#endif /* _KEEP_RCS_ID */
...
/* define 32-bit structure to hold both the class and register number,
   and a union with an mUINT so that we can efficiently compare
   register class-number pairs */
typedef union class_reg_pair {
  mUINT32	class_n_reg;
  struct class_reg_struct {
    mREGISTER 	        reg;
    mISA_REGCLASS rclass;
    } class_reg;
} CLASS_REG_PAIR;
...
#define     CLASS_REG_PAIR_rclass(x)		((ISA_REGCLASS)(x).class_reg.rclass)
...
extern void Set_CLASS_REG_PAIR(CLASS_REG_PAIR& rp, ISA_REGCLASS rclass, REGISTER reg);

/* Create a CLASS_REG_PAIR from rclass/reg pair */
#define CREATE_CLASS_N_REG(rclass,reg)	((mUINT32)(((reg)<<16)|(rclass)))
...
typedef struct {
  UINT64 v[(TI_ISA_REGISTER_MAX/64)+1];
} REGISTER_SET;
...
#define MAX_REGISTER_SET_IDX		(TI_ISA_REGISTER_MAX/64)
...
#define REGISTER_MAX                ((REGISTER) (TI_ISA_REGISTER_MAX+REGISTER_MIN))
 ...
 */
typedef struct {
  mUINT32          reg_machine_id[REGISTER_MAX + 1];
  mUINT32          reg_bit_size[REGISTER_MAX + 1];
  ...
  REGISTER_SET      outgoing_return_value; // callee side
  REGISTER_SET      incoming_return_value; // caller side
  REGISTER_SET      outgoing_argument;     // caller side
  REGISTER_SET      incoming_argument;     // callee side
...
extern REGISTER_CLIENT_CONST ISA_REGCLASS
REGISTER_CLASS_vec[TI_ISA_REGCLASS_MAX + 1];

/* Cached information about each ISA_REGCLASS:
 */
extern REGISTER_CLIENT_CONST REGISTER_CLASS_INFO
REGISTER_CLASS_info[TI_ISA_REGCLASS_MAX + 1];

/* Accessing cached information about a ISA_REGCLASS
 */
...
#define REGISTER_CLASS_outgoing_return_value(x)		\
			(REGISTER_CLASS_info[x].outgoing_return_value)
#define REGISTER_CLASS_incoming_return_value(x)		\
			(REGISTER_CLASS_info[x].incoming_return_value)
#define REGISTER_CLASS_outgoing_argument(x)		\
			(REGISTER_CLASS_info[x].outgoing_argument)
#define REGISTER_CLASS_incoming_argument(x)		\
			(REGISTER_CLASS_info[x].incoming_argument)
#define REGISTER_CLASS_function_value(x) REGISTER_CLASS_outgoing_return_value(x)
#define REGISTER_CLASS_function_argument(x) REGISTER_CLASS_incoming_argument(x)
...
/* Cached information about each ISA_REGSUBCLASS:
 */
extern REGISTER_CLIENT_CONST REGISTER_SUBCLASS_INFO*
REGISTER_SUBCLASS_info;

/* Accessing cached information about a ISA_REGSUBCLASS
 */
...
extern CLASS_REG_PAIR		CLASS_REG_PAIR_lbeg;
#define REGISTER_lbeg		CLASS_REG_PAIR_reg(CLASS_REG_PAIR_lbeg)
#define REGISTER_CLASS_lbeg	CLASS_REG_PAIR_rclass(CLASS_REG_PAIR_lbeg)
#define CLASS_AND_REG_lbeg	CLASS_REG_PAIR_class_n_reg(CLASS_REG_PAIR_lbeg)

extern CLASS_REG_PAIR		CLASS_REG_PAIR_lend;
#define REGISTER_lend		CLASS_REG_PAIR_reg(CLASS_REG_PAIR_lend)
#define REGISTER_CLASS_lend	CLASS_REG_PAIR_rclass(CLASS_REG_PAIR_lend)
#define CLASS_AND_REG_lend	CLASS_REG_PAIR_class_n_reg(CLASS_REG_PAIR_lend)

extern CLASS_REG_PAIR		CLASS_REG_PAIR_lcount;
#define REGISTER_lcount		CLASS_REG_PAIR_reg(CLASS_REG_PAIR_lcount)
#define REGISTER_CLASS_lcount	CLASS_REG_PAIR_rclass(CLASS_REG_PAIR_lcount)
#define CLASS_AND_REG_lcount	CLASS_REG_PAIR_class_n_reg(CLASS_REG_PAIR_lcount)

extern CLASS_REG_PAIR		CLASS_REG_PAIR_sar;
#define REGISTER_sar		CLASS_REG_PAIR_reg(CLASS_REG_PAIR_sar)
#define REGISTER_CLASS_sar	CLASS_REG_PAIR_rclass(CLASS_REG_PAIR_sar)
#define CLASS_AND_REG_sar	CLASS_REG_PAIR_class_n_reg(CLASS_REG_PAIR_sar)

extern CLASS_REG_PAIR		CLASS_REG_PAIR_br;
#define REGISTER_br		CLASS_REG_PAIR_reg(CLASS_REG_PAIR_br)
#define REGISTER_CLASS_br	CLASS_REG_PAIR_rclass(CLASS_REG_PAIR_br)
#define CLASS_AND_REG_br	CLASS_REG_PAIR_class_n_reg(CLASS_REG_PAIR_br)
...
extern CLASS_REG_PAIR		CLASS_REG_PAIR_acc_lo;
#define REGISTER_acc_lo		CLASS_REG_PAIR_reg(CLASS_REG_PAIR_acc_lo)
#define REGISTER_CLASS_acc_lo	CLASS_REG_PAIR_rclass(CLASS_REG_PAIR_acc_lo)
#define CLASS_AND_REG_acc_lo	CLASS_REG_PAIR_class_n_reg(CLASS_REG_PAIR_acc_lo)

extern CLASS_REG_PAIR		CLASS_REG_PAIR_acc_hi;
#define REGISTER_acc_hi		CLASS_REG_PAIR_reg(CLASS_REG_PAIR_acc_hi)
#define REGISTER_CLASS_acc_hi	CLASS_REG_PAIR_rclass(CLASS_REG_PAIR_acc_hi)
#define CLASS_AND_REG_acc_hi	CLASS_REG_PAIR_class_n_reg(CLASS_REG_PAIR_acc_hi)
  ...
  if (reg != REGISTER_UNDEFINED)
    {
      ...
      Is_True((UINT)bit <= TI_ISA_Register_Max(),
...
inline ISA_REGCLASS Register_Class_For_Mtype(TYPE_ID mtype)
{
  extern mISA_REGCLASS Mtype_RegClass_Map[MTYPE_MAX];
  return   (ISA_REGCLASS)Mtype_RegClass_Map[mtype];
}

extern void Init_Mtype_RegClass_Map(void);

// what is the max width when a dedicated register is used
// e.g. b4 can be used for either xtbool or xtbool4
// so the max width is 4
extern INT Max_Dedicated_Boolean_Register_Width(REGISTER reg);

// what is the width of register for the given mtype
// e.g. MTYPE_XTBOOL4 requires 4 boolean register
extern INT Mtype_Register_Width(TYPE_ID mtype);
