 ...
 * $Author: mpm $
 * $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/tn.h,v $
 *
 * Description:
 *
 ...
 *	   TN_is_asm_reg(tn)
 *	     This macro indicates that the register TN corresponds to a 
 *           register asm.  We should limit optimizations on these TNs.
 *
 *	   TN_is_float(tn)
 *	     TN is for a floating point register.
 *
 *	   TN_is_xtbool(tn)
 *	     TN is for a boolean register.
 *
 *	   TN_is_tie(tn)
 *	     TN is for a TIE register.
 *
 *	   TN_is_fpu_int(tn)
...
struct tn {
  ...
  union {
    ...
    ISA_ENUMCLASS_VALUE ecv;	/* Enum constant */
  ...
  /* offset 16 */
  TYPE_ID	mtype;		/* type of the TN */
				/* this is needed to determine type-dependent
				   operations such as spilling TIE type
				   register values */
				/* as a sanity check, the mtype and the
				   register class should be consistent */
};
...
#define TN_ASM_REG 0x4000  /* TN is an ASM register */
...
#define     TN_mtype(t)		(CAN_USE_TN(t)->mtype)
#define Set_TN_mtype(t,x)	(CAN_USE_TN(t)->mtype = (x))
...
#ifdef TARG_XTENSA
// mtype is an explicit field in TN
#define       TN_is_float(x)    (MTYPE_is_float(TN_mtype(x)))
#define       TN_is_xtbool(x)   (MTYPE_is_xtbool(TN_mtype(x)))
#define       TN_is_tie(x)      (MTYPE_is_tie(TN_mtype(x)))
#else
...
#define      TN_is_asm_reg(r)	(TN_flags(r) &   TN_ASM_REG)
#define  Set_TN_is_asm_reg(r)	(TN_flags(r) |=  TN_ASM_REG)
#define Reset_TN_is_asm_reg(r) (TN_flags(r)&= ~TN_ASM_REG)
...
extern  TN *Build_Dedicated_TN ( ISA_REGCLASS rclass, REGISTER reg, INT size);
...
inline TN *Build_TN_Like(TN *tn)
{
  ...
  if (!TN_is_register(tn)) {
	...
	new_tn = Gen_Typed_Register_TN (MTYPE_I4, TN_size(tn));
  }
  else {
  	new_tn = Gen_Typed_Register_TN ( TN_mtype(tn), TN_size(tn) );
  }
...
{
  return Gen_Typed_Register_TN (mtype, MTYPE_RegisterSize(mtype) );
}
...
// literals always have MTYPE_I4 type
...
// the enum TN is assumed to be MTYPE_I4
extern TN *Gen_Enum_TN (ISA_ENUMCLASS_VALUE ecv);

// the symbol TN always has Pointer_type
extern  TN *Gen_Symbol_TN ( ST *s, INT64 offset, INT32 relocs);
// the label and tag TN is assumed to be Pointer_type
extern  TN *Gen_Label_TN ( LABEL_IDX lab, INT64 offset );
extern  TN *Gen_Tag_TN ( LABEL_IDX tag);
// the adjusted TN inherit mtype from the old TN
 ...
 */
extern INT TN_Register_Width(TN* tn);
extern INT TN_Register_Align(TN* tn);
extern INT OP_TN_Width( struct op* op, int tn_num, BOOL is_result);
extern BOOL Imm_Or_Stack_Sym_Offset (TN *tn, INT64 *val);
