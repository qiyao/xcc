 ...
 *  Module: tn_map.cxx
 *  $Revision: 1.17 $
 *  $Date: 2000/04/07 01:11:57 $
 *  $Author: mtibuild $
 *  $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/tn_map.cxx,v $
 *
 *  Description:
 *  ============
 *
...
{
  FmtAssert((TN_number(tn) <= Last_TN),
...
// Arrays of size TN_MAP_HASH look up the hashed TN values
...
struct htn_map_iterator {
  hTN_MAP map;
  INT index;
  HASH_ENTRY *entry;
};

#define hTN_MAP_ITERATOR_map(iter)	((iter)->map)
#define hTN_MAP_ITERATOR_index(iter)	((iter)->index)
#define hTN_MAP_ITERATOR_entry(iter)	((iter)->entry)

hTN_MAP_ITERATOR htn_map_iterator_create (hTN_MAP map)
{
  hTN_MAP_ITERATOR iter = 
     TYPE_MEM_POOL_ALLOC (struct htn_map_iterator, hTN_MAP_pool(map));
  hTN_MAP_ITERATOR_map(iter) = map;
  hTN_MAP_ITERATOR_index(iter) = 0;
  hTN_MAP_ITERATOR_entry(iter) = hTN_MAP_entry(map, hTN_MAP_ITERATOR_index(iter));
  if (hTN_MAP_ITERATOR_entry(iter) == NULL) {
...
BOOL is_htn_map_iterator_empty(hTN_MAP_ITERATOR iter)
{
  return (hTN_MAP_ITERATOR_index(iter) == TN_MAP_HASH);
}

TN *get_htn_map_iterator_tn (hTN_MAP_ITERATOR iter) 
{
  HASH_ENTRY *map_entry = hTN_MAP_ITERATOR_entry(iter);
  return (map_entry == NULL) ? NULL : HASH_ENTRY_tn(map_entry);
}

void *get_htn_map_iterator_value (hTN_MAP_ITERATOR iter)
{
  HASH_ENTRY *map_entry = hTN_MAP_ITERATOR_entry(iter);
  return (map_entry == NULL) ? NULL : HASH_ENTRY_value(map_entry);
}

void htn_map_iterator_next(hTN_MAP_ITERATOR iter)
{
  HASH_ENTRY *map_entry = hTN_MAP_ITERATOR_entry(iter);
  if (map_entry) {
    HASH_ENTRY *next = HASH_ENTRY_next(map_entry);
    if (next) {
      hTN_MAP_ITERATOR_entry(iter) = next;
      return;
    }
  }
  INT index;
  for ( index = hTN_MAP_ITERATOR_index(iter) + 1;
        index < TN_MAP_HASH;
        index++) {
    map_entry = hTN_MAP_entry(hTN_MAP_ITERATOR_map(iter), index);
    if (map_entry) {
      hTN_MAP_ITERATOR_index(iter) = index;
      hTN_MAP_ITERATOR_entry(iter) = map_entry;
      return;
    }
  }
  hTN_MAP_ITERATOR_index(iter) = TN_MAP_HASH;
  hTN_MAP_ITERATOR_entry(iter) = NULL;
  return;
}
