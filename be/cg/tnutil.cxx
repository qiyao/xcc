 ...
 */
static TN *ded_tns[TI_ISA_REGCLASS_MAX + 1][REGISTER_MAX + 1];
 ...
 */
static TN *
Create_Dedicated_TN (ISA_REGCLASS rclass, REGISTER reg)
{
  ...
  /* What type to assign to a dedicated register? Perhaps a type that
     spans the entire register. That's what we want to do for the
     integer registers, since we want to call Build_TN_Like(<dedicated
     register>), and that requires that the dedicated register have a
     type. For other register classes, let's just pick any type that bases
     on the register class for now. */
  if (rclass == TI_ISA_Regclass_Integer())
    Set_TN_mtype(tn, MTYPE_U4);
  else if (rclass == TI_ISA_Regclass_Branch())
    Set_TN_mtype(tn, MTYPE_XTBOOL);
  else {
    TYPE_ID mtype = TI_ISA_Mtype_For_Regclass(rclass);
    Set_TN_mtype(tn, mtype);
  }

  /* TENSILICA: don't ever want to mark a tn as being a float since
     that triggers lots of fp specific register use, etc; and our fp
     could be in the ar registers. I'm not sure if this is best
     solution... */
#ifndef TARG_XTENSA
  if (rclass == TI_ISA_Regclass_Float())
  {
    Set_TN_is_float(tn);
  }
#endif
...
Init_Dedicated_TNs (void)
{
  ...
  if( REGISTER_CLASS_gp != ISA_REGCLASS_UNDEFINED )
  {
 ...
 */
TN *
Build_Dedicated_TN (ISA_REGCLASS rclass, REGISTER reg, INT size)
{
...
{
  ISA_REGCLASS rclass = Register_Class_For_Mtype (mtype);
  TN* tn;

  Is_True(rclass != ISA_REGCLASS_UNDEFINED,
       ...
       (REGISTER_CLASS_register_count(rclass) == 1))
  {
	...
	/* TENSILICA: don't ever want to mark a tn as being a float
	   since that triggers lots of fp specific register use, etc;
	   and our fp could be in the ar registers. I'm not sure if
...
extern TN* Gen_State_Register_TN (const char* state_name)
{
  ISA_REGSUBCLASS sc = TI_ISA_Regsubclass_State(state_name);
  const ISA_REGSUBCLASS_INFO* sc_info = TI_ISA_Regsubclass_Info(sc);
  ISA_REGCLASS rc = TI_ISA_Regsubclass_Class(sc_info);
  FmtAssert(TI_ISA_Regsubclass_Count(sc_info)==1,
			("Register subclass info error"));

  INT reg=TI_ISA_Regsubclass_Member(sc_info,0)+REGISTER_MIN;
  TN* tn = Build_Dedicated_TN(rc, reg, 4);
...
Gen_Unique_Literal_TN (INT64 ivalue, INT size)
{
  Is_True(size != 4 || (ivalue >= -2147483648LL && ivalue <= 4294967295LL),
	  ("Gen_Literal_TN: 4-byte literal 0x%016"LLX_FMT" is out-of-range", ivalue));
...
TN *
Gen_Enum_TN (ISA_ENUMCLASS_VALUE ecv)
{
	TN *tn = Gen_TN ();
	Set_TN_size(tn, 2);
	Set_TN_is_constant(tn);
...
sPrint_TN ( TN *tn, BOOL verbose, char *buf )
{
  ...
  if (TN_is_constant(tn)) {
    if ( TN_has_value(tn)) {
      buf += sprintf ( buf, "(0x%" LLX_FMT ")", TN_value(tn) );
      if (TN_size(tn) == 4 && 
	  TN_value(tn) >> 32 != 0 &&
	  TN_value(tn) >> 31 != -1)
	buf += sprintf ( buf, "!!! TN_value=0x%" LLX_FMT " is too big to fit in a word",
			 TN_value(tn));
    }
    else if (TN_is_enum(tn)) {
      buf += sprintf ( buf, "(enum:%s)", TI_ISA_ECV_Name(TN_enum(tn)) );
    }
    else if ( TN_is_label(tn) ) {
      LABEL_IDX lab = TN_label(tn);
      const char *name = LABEL_name(lab);
      INT64 offset = TN_offset(tn);
      const char* hi_lo = ((TN_relocs(tn)==TN_RELOC_NONE)? "":
			   (TN_relocs(tn)==TN_RELOC_LOW16)? "#lo":
			   (TN_relocs(tn)==TN_RELOC_HIGH16)? "#hi": "");
      if ( offset == 0 ) {
	buf += sprintf ( buf, "(lab%s:%s)", hi_lo, name );
      }
      else {
	buf += sprintf ( buf, "(lab%s:%s+%" LLD_FMT ")", hi_lo,  name, offset );
      }
    } 
	...
	ST *base_sym;
	INT64 base_ofst;
	Base_Symbol_And_Offset( var, &base_sym, &base_ofst);
	if (base_sym == var)
	  buf += sprintf ( buf, ":%s%+"LLD_FMT")", ST_name(var), TN_offset(tn) );
	else
	  buf += sprintf ( buf, ":%s[%s%+"LLD_FMT"]%+"LLD_FMT")",
			   ST_name(var), ST_name(base_sym), base_ofst, TN_offset(tn) );
      }
    } 
    else {
    ...
    if (TI_ISA_Regclass_Is_State(TN_register_class(tn))) {
      buf += sprintf (buf, "(%s)", 
		REGISTER_name(TN_register_class(tn), TN_register(tn)));
    } else if (TN_register_class(tn) == TI_ISA_Regclass_Special()) {
      buf += sprintf (buf, "(%s)", 
		TI_ISA_Regsubclass_Special_Name(TN_register(tn)-REGISTER_MIN));
	...
	if (reaching_def) {
	  FOR_ALL_BB_PREDS(bb, edge) {
	    ...
	    BOOL live_out = REG_LIVE_Outof_BB(rc, reg, cur_bb);
            ...
            /* Ignore self predecessor if 'bb' doesn't define 'tn'. */
            if (live_out && cur_bb == bb && !BB_Defs_TN(bb, tn))
              continue;
...
)
{
  ...
  do {
	...
	tn = OP_opnd(def_op, TI_ISA_Copy_Operand(OP_code(def_op), TRUE));
	use_op = def_op;
	continue;
      } else if (OP_code(def_op) == TOP_movi) {
        tn =  OP_opnd(def_op, 0);
        Is_True(TN_has_value(tn), 
                ("TN_Value_At_Op: TOP_movi does not have value"));
        *val = TN_value(tn);
        return TRUE;
      }
    }
  ...
  INT64 *maxval )	/* Result: Maximum value in range */
{
   ...
   */
  if ( ! TN_is_constant(tn) ) {
    *minval = (INT64)HUGE_MIN;
    *maxval = (INT64)HUGE_MAX;
    return FALSE;
  }
  ...
  INT32	disp)		/* Displacement from symbolic value */
{
  ...
  return Evaluate_Operand_Range ( tn1, disp, &lbound, &hbound )
	   && TI_TOP_Can_Have_Immediate ( lbound, opcode )
	   && TI_TOP_Can_Have_Immediate ( hbound, opcode );
}

/* ====================================================================
 * returns the number of registers based on the TN mtype info
 * or the max width if a dedicated boolean register TN
 * or 1 if dedicated register TN of any other type
 * or 0 if not a register TN
 * ====================================================================
 */
INT
TN_Register_Width(TN* tn)
{
  if (TN_is_register(tn)==FALSE)
    return 0;

  if (TN_is_dedicated(tn)) {
    ISA_REGCLASS rc = TN_register_class(tn);
    if (rc==TI_ISA_Regclass_Branch()) {
      return Max_Dedicated_Boolean_Register_Width(TN_register(tn));
    } else {
      return 1;
    }
  } else {
    return TI_ISA_Regsize_For_Mtype(TN_mtype(tn));
#if 0
    ISA_REGCLASS rc = Register_Class_For_Mtype(TN_mtype(tn));
    int tn_size = MTYPE_bit_size(TN_mtype(tn));
    int reg_size = TI_ISA_Regclass_Bit_Size(TI_ISA_Regclass_Info(rc));
    return (tn_size+reg_size-1)/reg_size;
#endif
  }
}

INT
TN_Register_Align(TN* tn)
{
  if (TN_is_register(tn)==FALSE)
    return 1;
  ...
  if (TN_is_dedicated(tn)) {
    ...
    int align;
    if (rc==TI_ISA_Regclass_Branch())
      align = MTYPE_bit_size(TN_mtype(tn));
    else
      align = MTYPE_alignment(TN_mtype(tn))*8;
    int reg_size = TI_ISA_Regclass_Bit_Size(TI_ISA_Regclass_Info(rc));
    return (align+reg_size-1)/reg_size;
#endif
  }
}

INT OP_TN_Width( OP* op, int tn_num, BOOL is_result)
{
  TN* tn = (is_result ? OP_result(op,tn_num) : OP_opnd(op,tn_num));
  if (TN_is_dedicated(tn) && (OP_code(op) != TOP_asm)) {
#if 1
    const ISA_OPERAND_INFO *opinfo = TI_ISA_Operand_Info(OP_code(op));
    const ISA_OPERAND_VALTYP *vtype = (is_result ?
				       TI_ISA_Op_Result(opinfo, tn_num) :
				       TI_ISA_Op_Operand(opinfo, tn_num));
    return TI_ISA_Valtyp_Num_Regs(vtype);
#else
    ISA_REGCLASS rc = (is_result ? 
		       OP_result_reg_class(op,tn_num) :
		       OP_opnd_reg_class(op,tn_num));
    ISA_REGSUBCLASS sc = (is_result ? 
			  OP_result_reg_subclass(op,tn_num) :
			  OP_opnd_reg_subclass(op,tn_num));
    if (rc == ISA_REGCLASS_branch && sc != ISA_REGSUBCLASS_UNDEFINED) {
      return REGISTER_CLASS_register_count(rc) 
	/ REGISTER_SET_Size(REGISTER_SUBCLASS_members(sc));
    } else
      return 1;
#endif
  } else
    return TN_Register_Width(tn);
}

BOOL Imm_Or_Stack_Sym_Offset (TN *tn, INT64 *val)
{
  if (TN_has_value(tn))
  {
    *val = TN_value(tn);
  ...
  else if (TN_is_symbol(tn))
  {
    ...
    Base_Symbol_And_Offset(TN_var(tn), &base_st, &base_ofst);
    if ((base_st == SP_Sym) || (base_st == FP_Sym))
    {
      *val = base_ofst + TN_offset(tn);
      return TRUE;
    }
  }
...
/**********************************************/
/* All routines declared in tn_targ.h         */
/* under common/targ_info/static/be/cg/xtensa */
/* are defined here.                          */
/**********************************************/
...
cpenable_tn()
{
  static TN *cpenable_tn = NULL;
  if (cpenable_tn == NULL) {
    cpenable_tn = Gen_State_Register_TN("CPENABLE");
  }
  return cpenable_tn;
}

/* determine if 'top' has CPENABLE state operand and return CPENABLE TN */

TN *
cpenable_tn(TOP top)
{
  const ISA_OPERAND_INFO *operands_info = TI_ISA_Operand_Info (top);
  int op_index = TI_ISA_Op_Operands(operands_info)-1;
  const ISA_OPERAND_VALTYP* operand_value_type =
        TI_ISA_Op_Operand(operands_info, op_index);
  if (TI_ISA_Valtyp_Is_Register(operand_value_type)) {
    ISA_REGSUBCLASS sc = TI_ISA_Valtyp_Regsubclass(operand_value_type);
    if (sc != ISA_REGSUBCLASS_UNDEFINED) {
      const ISA_REGSUBCLASS_INFO* sc_info = TI_ISA_Regsubclass_Info(sc);
      if (!strcmp(TI_ISA_Regsubclass_Name(sc_info), "CPENABLE"))
        return cpenable_tn();
    }
  }
  return NULL;
