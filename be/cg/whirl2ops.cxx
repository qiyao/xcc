 ...
 * $Revision: 1.539 $
 * $Date: 2000/10/30 19:56:41 $
 * $Author: lesniak $
 * $Source: /isms/cmplrs.src/osprey1.0/be/cg/RCS/whirl2ops.cxx,v $
 *
 * Description:
 *
...
#include "cgtarget.h"
#include "cg_swp.h"	/* for SWP_RETURN_CODE */
#include "cg_spill.h"
...
static DYN_ARRAY<WN *>* last_loop_pragmas=NULL;
...
/* This is an external function we provide for the Instruction selector. */
extern void End_Basic_Block( WN *branch )
{
...
Set_OP_To_WN_Map(WN *wn)
{
  ...
  for ( ; op != NULL; op = OP_next(op)) {
    ...
    if (OP_memory(op) && WN_Is_Volatile_Mem(wn)) {
      ...
      if (needs_memw && (!prev_op || OP_code(prev_op)!=TOP_memw)) {
	OP* new_op = Mk_OP(TOP_memw);
	OP_srcpos(new_op) = OP_srcpos(op);
	OPS_Insert_Op(&New_OPs, op, new_op, TRUE /*before*/);
      }
    }
    if (OP_prefetch(op)) {
	...
	WN_to_OP_map = WN_MAP_Create(&MEM_phase_pool);
      ...
      if (Trace_WhirlToOp) {
	fprintf (TFile, "Set_OP_To_WN_Map: op ");
	Print_OP(op);
	fprintf(TFile, "whirl node ");
	fdump_wn(TFile, wn);
      }
    }
  }
...
void Set_Last_Mem_OP( OPS *o )
{
  Last_Mem_OP = OPS_last(o);
}
...
Preg_Is_Rematerializable(PREG_NUM preg, BOOL *gra_homeable)
{
...
#ifdef TARG_XTENSA
  /* When we split an F8 into two F4's (or I4's) we replace one preg
     with two (actually, I think we use the existing preg and add one
     more). We should really probably update the original preg's home
     so that it does not point to a double, but instead points to the
     half of the double that is now being represented by the preg. And
     the other preg should show the home as the other half. Since we
     don't do that, we just disallow the use of the home location by
     showing the preg as not being rematerializable. */
  if (MTYPE_is_double(rtype))
    return NULL;

  /* similar to the problem describe above, A TIE preg initialized with
   * long long or double cannot be rematerialized by ISEL directly (see
   * pr9370).
   */
  if (MTYPE_is_tie(rtype) && OPCODE_operator(opc)==OPR_CVT) {
    TYPE_ID desc = OPCODE_desc(opc);
    if (MTYPE_is_double(desc) || MTYPE_is_longlong(desc))
      return NULL;
  }
#endif
  ...
  if (WN_operator(home) == OPR_LDID) {
    ...
    // for TIE or xtbool converting ldid, we could simply return NULL
    // and disable homing

    if (WN_desc(home) != WN_rtype(home) &&
        ((MTYPE_is_tie(WN_desc(home)) || MTYPE_is_tie(WN_rtype(home))) ||
         (MTYPE_is_xtbool(WN_desc(home)) || MTYPE_is_xtbool(WN_rtype(home))))) {
        return NULL;
    }
	...
	ST_class(sym) == CLASS_VAR && ST_sclass(sym)!=SCLASS_FORMAL) {
      if (ST_gprel(basesym) ||
	  (ST_is_split_common(basesym) && ST_gprel(ST_full(basesym)))
	  || basesym == SP_Sym || basesym == FP_Sym) {
	if (gra_homeable)
	  *gra_homeable = TRUE;
	return home;
      }
    ...
    return NULL;
  } else if (WN_operator(home) == OPR_INTRINSIC_OP &&
	     INTRN_is_tie_intrinsic(WN_intrinsic(home))) {
    TIE_MACRO_ID tie_macro_id = Intrinsic_To_Tie_Macro_Id(WN_intrinsic(home));
    TIE_MACRO_p tie_macro = tie_info->tie_macro(tie_macro_id);
    UINT kid_count = WN_kid_count(home);
    if (tie_macro->is_whirl_intrinsic_op() && kid_count==1 &&
	WN_operator(WN_kid0(WN_kid0(home)))==OPR_INTCONST) {
	return home;
    }
...
{
  PREG_NUM new_max_preg_to_tn_index;
  if (preg_num < max_preg_to_tn_index) return;
  	
  new_max_preg_to_tn_index = Get_Preg_Num (PREG_Table_Size(CURRENT_SYMTAB))+10;
  PREG_To_TN_Array = TYPE_MEM_POOL_REALLOC_N( TN *, &MEM_pu_pool,
	 PREG_To_TN_Array, max_preg_to_tn_index,
	 new_max_preg_to_tn_index + 10);
  PREG_To_TN_Mtype = TYPE_MEM_POOL_REALLOC_N( TYPE_ID, &MEM_pu_pool,
	 PREG_To_TN_Mtype, max_preg_to_tn_index,
	 new_max_preg_to_tn_index + 10);
  max_preg_to_tn_index = new_max_preg_to_tn_index;
}
...
PREG_To_TN (ST *preg_st, PREG_NUM preg_num)
{
  ...
  if (tn == NULL)
  {
	...
	if (TI_ABI_Property_Set(ABI_PROPERTY_stacked, 
				rclass,
...
#ifdef EMULATE_LONGLONG
        // only on IA-32
        if (reg == Dedicated_Preg_First_Incoming_Ret(TI_ISA_Regclass_Integer())) {
          // dedicated and eax
    ...
    else if (MTYPE_is_tie_packed(TY_mtype(ST_type(preg_st)))) {

      /* create a set of TNs for this PREG of packed mtype */
      TYPE_ID output_mtype = TY_mtype(ST_type(preg_st));
      TN* first_tn = NULL;
      for (int i=0; i<tie_info->num_scalar_mtypes(output_mtype); i++) {
        TYPE_ID mtype = tie_info->get_scalar_mtype(output_mtype,i);
        tn = Build_TN_Of_Mtype (mtype);
	if (i==0) first_tn = tn;
	PREG_NUM preg2_num = preg_num + i;
	Realloc_Preg_To_TN_Arrays (preg2_num);
	TN_MAP_Set( TN_To_PREG_Map, tn, (void *)preg2_num );
	PREG_To_TN_Array[preg2_num] = tn;
    ...
    Realloc_Preg_To_TN_Arrays (preg_num);
...
TN_To_Assigned_PREG (TN *tn)
{
  ...
  i += Dedicated_Preg_First(TN_register_class(tn));
...
Handle_Call_Site (WN *call, OPERATOR call_opr)
{
  ...
  Handle_Call_Site_Extras( call, call_opr );
}

void
Handle_Call_Site_Extras(WN *call, OPERATOR call_opr)
{
  CALLINFO *call_info;
  ST *call_st = (call_opr != OPR_ICALL) ? WN_st(call) : NULL;
  ...
  if ( GP_TN!=NULL && (Is_Caller_Save_GP && !Constant_GP 
	&& (call_st == NULL || ST_export(call_st) == EXPORT_PREEMPTIBLE)))
  {
	...
	if (caller_gp_tn == NULL) {
		caller_gp_tn = Gen_Typed_Register_TN ( Pointer_type, 
			Pointer_Size);
...
/*
**  we have to check both ty alignment and the offset alignment
*/
static INT16 Compute_Variant(OPCODE opcode, BOOL targ_unaligned, INT required_alignment, WN_OFFSET offset, INT ty_align)
{
  INT	variant = 0;
  INT	align = ty_align;

  /* Temporary workaround until I can make sure everything is working. */
  Is_True ((required_alignment != 0), ("bad value 0 for required_align"));
  ...
  if (offset)
  {
    ...
    INT offset_lsb = offset & (-offset);

    /* handle corner case when offset is 0x80000000 (pr10500) */
    if (offset_lsb<0)
      offset_lsb = 0;

    if (offset_lsb)
      align = MIN(ty_align, offset_lsb);
  }
  if (align < required_alignment)
  {
    if (Warn_Unaligned) ErrMsgSrcpos(EC_Bad_Align, current_srcpos);
    if (targ_unaligned) return variant;
    Set_alignment(variant, align);
   /*
    *	TODO
    *	When we have ST information we may be able to compute an
    *	offset(say mod 16) that will give us further information
    */
    Set_align_offset_unknown(variant);
  }
  return variant;
}
...
*/
static INT Variant_Is_Volatile(INT variant, INT align)
{
...
INT16 Variant_LDID(WN *node)
{
  INT		variant;
  INT		align;

  align = ST_alignment(WN_st(node));

  variant = Compute_Variant(WN_opcode(node),
			    xt_unaligned_loads,  
			    	MTYPE_alignment(WN_desc(node)),
			    WN_load_offset(node),
			    align);

  if ((variant)				   &&
      (WN_class(node)==CLASS_VAR)   &&
      (TY_is_volatile(ST_type(WN_st(node)))))
  {
    return Variant_Is_Volatile(variant, align);
  }
  return variant;
}

INT16 Variant_ISTORE(WN *node)
{
  INT		variant;
  TY_IDX	ty = WN_ty(node);
  ...
  variant = Compute_Variant(WN_opcode(node),
			    xt_unaligned_stores, MTYPE_alignment(WN_desc(node)),
			    WN_store_offset(node),
   			    TY_align(ty));

  if (variant && TY_is_volatile(ty))
  {
    return Variant_Is_Volatile(variant, TY_align(ty));
  }
  return variant;
}

INT16 Variant_STID(WN *node)
{
  INT		variant;
  INT		align;
...
INT16 Variant_ILOAD(WN *node)
{
  INT		variant;
  TY_IDX	ty = WN_load_addr_ty(node);
  ...
  variant = Compute_Variant(WN_opcode(node),
			    xt_unaligned_loads, MTYPE_alignment(WN_desc(node)),
			    WN_load_offset(node),
   			    TY_align(ty));

  if (variant && 
     (TY_is_volatile(ty) || TY_is_volatile(WN_ty(node))))
  {
    return Variant_Is_Volatile(variant, TY_align(ty));
  }
  return variant;
...
static TN *
Handle_LDA (WN *lda, WN *parent, TN *result)
{
...
Handle_LDID (WN *ldid, TN *result, OPCODE opcode)
{
  ...
  else
  {
    ...
    variant = Variant_LDID(ldid);

    if (result == NULL) result = Allocate_Result_TN (ldid, NULL);
    Last_Mem_OP = OPS_last(&New_OPs);

    if (WN_opcode(ldid) == opcode)
      Exp_Load(result, ldid, &New_OPs, variant);
    else
      Exp_Load(OPCODE_rtype(opcode), OPCODE_desc(opcode),
	       result, WN_st(ldid), WN_load_offset(ldid), 
	       &New_OPs, variant);

    Set_OP_To_WN_Map(ldid);
  }
  if (asm_reg) {
      Set_TN_is_asm_reg(result);
  }
  return result;
}
...
Handle_LDBITS (WN *ldbits, TN *result, OPCODE opcode)
{
  ...
  {
    INT16 variant = Variant_LDID(ldbits);

    src_tn = Allocate_Result_TN (ldbits, NULL);
    Last_Mem_OP = OPS_last(&New_OPs);

    if (WN_opcode(ldbits) == opcode)
      Exp_Load(src_tn, ldbits, &New_OPs, variant);
    else
      Exp_Load(OPCODE_rtype(opcode), OPCODE_desc(opcode),
	       src_tn, WN_st(ldbits), WN_load_offset(ldbits), 
	       &New_OPs, variant);
...
{
  INT16 variant = Variant_ILOAD(ildbits);
  ...
  if (WN_operator_is(kid0, OPR_LDA)) {
    ...
    Exp_Load(OPCODE_rtype(opcode), OPCODE_desc(opcode),
	     src_tn, st, WN_offset(ildbits) + WN_lda_offset(kid0),
	     &New_OPs, variant);
  }
  else {
...
Handle_STID (WN *stid, OPCODE opcode)
{
  ...
  if (WN_class(stid) == CLASS_PREG) {
    ...
    INT16 variant = Variant_STID(stid);
    Last_Mem_OP = OPS_last(&New_OPs);
    Exp_Store (OPCODE_desc(opcode), Expand_Expr (WN_kid0(stid), stid, NULL),
	       WN_st(stid), WN_store_offset(stid),
	       &New_OPs, variant);
...
{
  INT16 variant  = Variant_ISTORE(istore);
  ...
  if (WN_operator_is(kid1, OPR_LDA)) {
    ...
    Exp_Store (OPCODE_desc(opcode), kid0_tn,
	       st, WN_offset(istore) + WN_lda_offset(kid1),
	       &New_OPs, variant);
  }
  else {
...
{
  INT16 variant  = Variant_ISTORE(istbits);
  ...
  if (WN_operator_is(kid1, OPR_LDA)) {
    ...
    Exp_Load(rtype, desc, field_tn, st, 
	     WN_store_offset(istbits) + WN_lda_offset(kid1),
	     &New_OPs, variant);
...
Handle_SELECT(WN *select, TN *result, OPCODE opcode)
{
  ...
  */
  ...
  if (Check_Select_Expansion (WN_opcode(compare)) || (variant == V_BR_NONE)) {
	...
	Exp_OP3v (opcode, result, cond, trueop, falseop, 0, &New_OPs);
  } 
  else {
...
Handle_LAND_LIOR(WN *expr, WN *parent, TN *result)
{
  ...
  variant = 0;
  if (OPCODE_is_boolean(WN_opcode(WN_kid0(expr)))) Set_normalized_op1(variant);
  if (OPCODE_is_boolean(WN_opcode(WN_kid1(expr)))) Set_normalized_op2(variant);
...
Handle_LNOT(WN *expr, WN *parent, TN *result)
{
  ...
  variant = 0;
  if (OPCODE_is_boolean(WN_opcode(kid0))) Set_normalized_op1(variant);
...
Handle_ALLOCA (WN *tree, TN *result)
{
  ...
  if ( ! TN_is_zero(tsize)) {
	Exp_Spadjust (SP_TN, tsize, V_ADJUST_MINUS, &New_OPs);
...
Handle_DEALLOCA (WN *tree)
{
  ...
  Exp_Spadjust (SP_TN, val, V_ADJUST_PLUS, &New_OPs);
...
Expand_Expr (WN *expr, WN *parent, TN *result)
{
  ...
  INT num_opnds;
  INT i;
  TN *opnd_tn[TI_ISA_Operand_Max()]; // we shouldn't be handling variable OPs
  TN *const_tn;
  TOP top;
  ...
  if (TI_ISA_Property_Set(PROP_noop, top)
  ...
  if (top != TOP_UNDEFINED) {
    num_opnds =   TI_ISA_Op_Operands(TI_ISA_Operand_Info(top))
		- (TI_ISA_Property_Set(PROP_predicated, top) != 0);
  } else {
    num_opnds = OPCODE_nkids(opcode);
  }
  FmtAssert(num_opnds <= TI_ISA_Operand_Max(), ("too many operands (%d)", num_opnds));
  ...
  case OPR_LDA:
    return Handle_LDA(expr, parent, result);
...
//      break;
  ...
  case OPR_PREFETCH:
  case OPR_PREFETCHX:
    if ( ! Prefetch_Kind_Enabled( expr ) ) return NULL;
    Last_Mem_OP = OPS_last(&New_OPs);
    Exp_Prefetch (top, WN_prefetch_flag(expr),
  ...
  if (top != TOP_UNDEFINED) {
    ...
    if (TI_ISA_Property_Set(PROP_predicated, top)) {
 ...
 *  label_is_external 
 *
 *  The second argument should be a label.
 ...
 */
static 
BOOL label_is_external ( INT *num, LABEL_IDX label, BB *bb )
{
  INT32 label_number;
  BB *target_bb;
  RID *rid = BB_rid( bb );
 ...
 */
static 
BOOL wn_label_is_external ( INT *num, WN *wn, BB *bb )
{
  INT32 label_number;
  LABEL_IDX label;
  BB *target_bb;
  ...
  return label_is_external(num, WN_label_number(wn), bb);
}
...
BOOL Has_External_Branch_Target( BB *bb )
{
  ...
  case OPC_I4INTRINSIC_CALL:
    {
      INTRINSIC intrn = WN_intrinsic(branch_wn);
      FmtAssert(INTRN_is_tie_intrinsic(intrn),("Expecting TIE branch"));
      TIE_MACRO_p tie_macro =
		tie_info->tie_macro(Intrinsic_To_Tie_Macro_Id(intrn));
      FmtAssert(tie_macro->is_conditional_branch(),("Expecting TIE branch"));
      for (int i=1; i<=tie_macro->num_labels(); i++) {
	int proto_index = tie_macro->label_proto_index(i);
	WN* arg_wn = WN_kid(branch_wn, proto_index);
	LABEL_IDX label = WN_const_val(WN_kid0(arg_wn));
	FmtAssert(label!=0,("Expect a valid label"));
	if (label_is_external( &j, label, bb ))
	  return TRUE;
      }
    }
    return FALSE;
  ...
  case OPC_FALSEBR:
  case OPC_GOTO:
    return wn_label_is_external( &j, branch_wn, bb );
  case OPC_REGION_EXIT:
    return TRUE;
  case OPC_COMPGOTO:
    wn = WN_first( WN_kid1( branch_wn ) );
    for ( i = 0; i < WN_num_entries( branch_wn ); i++ ) {
      if ( wn_label_is_external( &j, wn, bb ) )
	return TRUE;
      wn = WN_next( wn );
    }
    if ( WN_kid_count( branch_wn ) == 3 ) {
      if ( wn_label_is_external( &j, WN_kid( branch_wn, 2 ), bb ) )
	return TRUE;
    }
    return FALSE;
  default:
...
BOOL Has_External_Fallthru( BB *bb )
{
  ...
  if ( ( branch_wn == NULL )
       || ( WN_opcode( branch_wn ) == OPC_TRUEBR ||
	    ...
	    WN_opcode( branch_wn ) == OPC_I4INTRINSIC_CALL ) ) {
    /* the OPC_I4INTRINSIC_CALL case is for TIE branch */
  ...
  for (bb = REGION_First_BB; bb != NULL; bb = BB_next(bb)) {
	...
	if ( br_op == NULL ) {
	  ...
	  if (!OP_tie(br_op)) {
	    ...
	    FmtAssert(OP_cond(br_op), ("TIE branch should be conditional"));
	    for (int i=0; i<OP_opnds(br_op); i++) {
	      target_tn = OP_opnd(br_op, i);
	      if (TN_is_label(target_tn)) {
    ...
    if (branch_wn != NULL) {
      switch (WN_opcode(branch_wn)) {
      case OPC_I4INTRINSIC_CALL:
	/* check to make sure the intrinsic call is a TIE branch */
	/* otherwise it is almost the same as TRUEBR except for potentially
	   multiple targets
	*/
	{
	  INTRINSIC intrn = WN_intrinsic(branch_wn);
	 ...
	 */
	{
	  ...
	  if ( br_op == NULL || OP_cond( br_op ) ) {
	    if ( ! Has_External_Fallthru( bb ) ) {
	      ...
	      if (WN_opcode(branch_wn)!=OPC_I4INTRINSIC_CALL) {
	        wn_label_is_external( &num, branch_wn, bb );
		...
		for (int i=1; i<=tie_macro->num_labels(); i++) {
		  ...
		  LABEL_IDX label = WN_const_val(WN_kid0(arg_wn));
		  FmtAssert(label !=0,("Expect a valid label"));
		  label_is_external( &num, label, bb );
		  CGRIN_exit_i( RID_cginfo( rid ), num) = bb;
		  CGRIN_exit_label_i( RID_cginfo( rid ), num) = 0;
		}
	      }
	...
	/* fall through and link the branch target. */
      ...
      case OPC_REGION_EXIT:
	if (WN_opcode(branch_wn)!=OPC_I4INTRINSIC_CALL) {
	  if ( ! wn_label_is_external( &num, branch_wn, bb ) ) {
	    /*internal label*/
	  ...
	  for (int i=1; i<=tie_macro->num_labels(); i++) {
	    ...
	    LABEL_IDX label = WN_const_val(WN_kid0(arg_wn));
	    FmtAssert(label !=0,("Expect a valid label"));
	    if ( ! label_is_external( &num, label, bb ) ) {
	      /*internal label*/
	      Link_BBs(bb, label);
	    } else if ( rid ) {
	      WN *new_exit;
	...
	for (i = 0; i < WN_num_entries(branch_wn); i++) {
	  if ( ! wn_label_is_external( &num, wn, bb ) ) {
	...
	if (WN_kid_count(branch_wn) == 3) {
	  if ( ! wn_label_is_external( &num, WN_kid( branch_wn, 2 ), bb ) )
	    /* default case */
	    Link_BBs (bb, Get_WN_Label(WN_kid(branch_wn,2)));
	  else if ( rid ) {
	    CGRIN_exit_i( RID_cginfo( rid ), num ) = bb;
...
/*
  DFS_Find_Call_or_Exit() finds out the first unique BB that is 
  an EXIT/CALL/ASM/REGION and that post-dominates "start". 
  If such a BB exits, it returns true and 'result' will point 
  to this BB; otherwise, it returns false.  Note if a BB that
  is an EXIT/CALL/ASM/REGION DOES NOT post dominate 'start', 
  it will return false.

  It works by depth-first searching the control flow from
  'start'.  If 'start' is such a BB, set 'result' to 'start'
  and return true; otherwise, recursively invoke 
  DFS_Find_Call_or_Exit() for each of start's successors. 
  If all successors return true and their results are the same,
  it returns true; otherwise, set 'result' to NULL and return false.
  Note that when DFS_Find_Call_or_Exit() returns true with 
  result == NULL, it indicates that this search reaches a node 
  that has been searched already. Any EXIT/CALL/ASM/REGION node
  reachable from this already-searched node has already been found,
  so no need to search it again.  This function does a special handling
  for returning true with result=NULL.
 */
static bool DFS_Find_Call_or_Exit (
   BB *start,    // input
   BB **result   // output
)
{
  ...
  *result = NULL;
  bb = start;
  /* If visited, stop searching and return true */
  if (BB_local_flag1(bb)) return true;

  // check 'start' first
  Set_BB_local_flag1(bb);
  rid = BB_rid(bb);
  if ( (rid && RID_level(rid) >= RL_CGSCHED) ||
       BB_call(bb) || BB_exit(bb) || BB_asm(bb) ) {
    *result = bb;
    return true;
  }
  ...
  BBLIST *succ_list;
  bool success = true;
  FOR_ALL_BB_SUCCS (start, succ_list) {
    bb = BBLIST_item(succ_list);
    if (!DFS_Find_Call_or_Exit(bb, &tmp)) {
      success = false;
      break;
    } else {
      if (target_bb == NULL) {  // not found yet
        target_bb = tmp;
      } else if ((tmp != NULL) && (target_bb != tmp)) {
        // found two, neither of them pdom start, bail out.
        success = false;
        break;
      }
    }
 ...
 *  post-dominating successors which contains a CALL (or an asm) or an Exit.
 *  If a successor containing CALL/EXIT/ASM does not post-dominate 'start',
 *  it means there must be another successor that contains CALL/EXIT/ASM, 
 *  which violates the assumption that a dedicated TN should have unique 
 *  use, thus unique successor that contain CALL/EXIT (see comments right
 *  before Localize_or_Replace_Dedicated_TNs()).
 *
 * =======================================================================
 */
static BB *
BB_Reaches_Call_or_Exit( BB *start )
{
  ...
  BB *bb;
  FOR_ALL_BBs (REGION_First_BB, bb) {
    Reset_BB_local_flag1(bb);
  }

  BB *expected_bb;
  if (DFS_Find_Call_or_Exit(start, &expected_bb)) {
    return expected_bb;
  } else {
    return NULL;
  }
 ...
 *  should be associated with a CALL, an ENTRY, an EXIT, or a previously 
 *  compiled REGION. The references associated with a previously compiled REGION
 *  are eliminated by replacing them with references to previously allocated,
 *  but non-dedicated TNs.  The references associated with CALLs and ENTRYs
 ...
 *  one implicit use, either a CALL or an EXIT or a previously compiled REGION.
...
Localize_or_Replace_Dedicated_TNs(void)
{
  ...
  for (bb = REGION_First_BB; bb != NULL; bb = BB_next(bb)) {
	...
	if ( non_region_use_bb == bb )
	  ...
	  // which have multiple defs and use all of the same tn.
	  // There are two case:
...
Handle_CONDBR (WN *branch)
{
  ...
  FmtAssert(WN_opcode(branch)!=OPC_I4INTRINSIC_CALL, ("Invalid opcode"));
...
{
  TN *target_tn = Expand_Expr (WN_kid0(branch), NULL, NULL);
  Exp_Indirect_Branch (target_tn, &New_OPs);
  Handle_XGOTO_Extras(branch);
}


void
Handle_XGOTO_Extras (WN *branch)
{
  ...
  /* make sure st is allocated */
  if (!ST_is_switchjump(st)) {
    Allocate_Object(st);
  }
  INITO_IDX ino = New_INITO(st);
  INITO_IDX prev_inv = INITV_IDX_ZERO;

  WN *wn = WN_first(WN_kid1(branch));	/* first goto */
  for (UINT i = 0; i < WN_num_entries(branch); i++) {
    FmtAssert ((wn && WN_opcode(wn) == OPC_GOTO),
	       ("XGOTO block doesn't have goto's? (%d)", WN_opcode(wn)));
    LABEL_IDX lab = WN_label_number(wn);
    INITO_IDX inv = New_INITV();
#ifdef TARG_XTENSA
    if (!xt_switchjump)
      INITV_Init_Symdiff(inv, lab, Get_Current_PU_ST(), 0);
    else
#endif
...
static void Handle_Entry (WN *entry) 
{
  ...
  if (WN_Label_Is_Handler_Begin(entry)) {
	...
	static const char *hprefix = ".L_Handler";
  	char *name = (char *) alloca (strlen(hprefix) + 1 + 16 + 1 + 16 + 1);
	sprintf(name, "%s%s%d%s%d", hprefix,
		Label_Name_Separator, Current_PU_Count(), 
		Label_Name_Separator, Get_WN_Label(entry));

	ST_Init (entry_st, Save_Str (name),
...
//
void
Handle_ASM (const WN* asm_wn, asm_input_callback *callback)
{
  // 'result' and 'opnd' below have a fixed size as well as
  // the arrays in ASM_OP_ANNOT. Define here so we can sanity check.
  enum { MAX_OPNDS = TI_ISA_ASM_OPERANDS_MAX, MAX_RESULTS = TI_ISA_ASM_RESULTS_MAX };
  ...
  ISA_REGSUBCLASS opnd_sc[TI_ISA_Operand_Max()];
  ...
  for (WN* out_pragma = WN_first(asm_output_constraints);
    ...
    const char* constraint = strdup(WN_pragma_asm_constraint(out_pragma));
    ...
    TYPE_ID preg_rtype = MTYPE_UNKNOWN;
    if (load==NULL) {
      preg_rtype = ST_mtype(WN_st(out_pragma));
    }

    TN* tn = CGTARG_TN_For_Asm_Operand(constraint, load, pref_tn, &subclass, asm_wn, preg_rtype);
  ...
  for (INT kid = 2; kid < WN_kid_count(asm_wn); ++kid) {
    ...
    const char* constraint = strdup(WN_asm_input_constraint(asm_input));
    ...
    if (OPERATOR_has_sym(WN_operator(load))) {
      ...
      if (ST_assigned_to_dedicated_preg(pref_st)) {
        pref_tn = PREG_To_TN(MTYPE_To_PREG(ST_mtype(pref_st)),
                             Find_PREG_For_Symbol(pref_st));
        Set_TN_is_asm_reg(pref_tn);
      }
    }
    ISA_REGSUBCLASS subclass = ISA_REGSUBCLASS_UNDEFINED;

    TN* tn = CGTARG_TN_For_Asm_Operand(constraint, load, pref_tn, &subclass, asm_wn, MTYPE_UNKNOWN);
...
#ifdef TARG_XTENSA
      /* We want to use the code selector to generate the ops for th
         input parameters. */
      if (callback)
	{
	  (*callback)(load, tn, &New_OPs);
	}
      else
#endif
...
// Return the set of registers found in ASM clobber lists in BB
REGISTER_SET
BB_Asm_Clobbers_Regs(BB* bb, ISA_REGCLASS rc)
{
  REGISTER_SET clobber = REGISTER_SET_EMPTY_SET;
  OP *op;
  FOR_ALL_BB_OPs(bb, op) {
    if (OP_code(op) == TOP_asm) {
      ASM_OP_ANNOT* asm_info = (ASM_OP_ANNOT*) OP_MAP_Get(OP_Asm_Map, op);
      clobber = REGISTER_SET_Union(clobber, ASM_OP_clobber_set(asm_info)[rc]);
    }
  }
  return clobber;
}
...
*/
  ...
  case OPC_LABEL:
	...
	if (WN_operator_is(trip_wn, OPR_INTCONST)) {
      ...
      LOOPINFO_max_trip_count_tn(info) = trip_tn;
      if (!Cur_PU_Feedback && WN_loop_trip_est(loop_info) == 0)
    ...
    if (info) {
      // initialize swp failure code to not attempted
      // will be replaced by real failure code once swp is performed
      LOOPINFO_swp_failure_code(info) = SWP_NOT_ATTEMPTED;
      BB_Add_Annotation(bb, ANNOT_LOOPINFO, info);
      if (last_loop_pragmas && last_loop_pragmas->Elements()>0) {
	for (int i=0; i<last_loop_pragmas->Elements(); i++) {
	  BB_Add_Annotation(bb, ANNOT_PRAGMA, last_loop_pragmas->Get(i));
	}
	last_loop_pragmas->Resetidx();
      }
    }
    break;
  case OPC_ALTENTRY:
  ...
  case OPC_XPRAGMA:
    if (WN_pragmas[WN_pragma(stmt)].users & PUSER_CG) {
	  ...
	  WN_pragma(stmt) == WN_PRAGMA_SUPER_SWP ||
	  WN_pragma(stmt) == WN_PRAGMA_SWP_SCHEDULE) {
	last_loop_pragmas->AddElement(stmt);
      } else
...
/*
   Return a TN_LIST of dedicated TNs that are 
     1) incoming parameters via registers if node_wn is ENTRY; or
     2) outgoing parameters via registers if node_wn is CALL.

   Note that the 'incoming' and 'outgoing' is from the view of current PU.
*/
static TN_LIST *Get_Dedicated_Param_TNs ( TY_IDX node_ty, WN *node_wn )
{

  TN_LIST *tnl=0;
  BOOL func_entry = WN_operator (node_wn) == OPR_FUNC_ENTRY;
  PLOC ploc;
  INT parm_count;
  if (func_entry) {
    parm_count = WN_num_formals(node_wn);
    ploc = Setup_Input_Parameter_Locations (node_ty);
  } else {
    parm_count = WN_num_actuals(node_wn);
    ploc = Setup_Output_Parameter_Locations (node_ty);
  }

  for (INT i = 0; i < parm_count; i++)
  {
    TY_IDX parm_ty = TY_Of_Parameter(WN_actual(node_wn,i));
    if (func_entry) {
    ...
    while (PLOC_is_nonempty(ploc))
    {
      ...
      ISA_REGCLASS cl;
      REGISTER reg;
      FmtAssert(CGTARG_Preg_Register_And_Class(PLOC_reg(ploc), &cl, &reg),
                ("Don't know how to handle PREG%d", PLOC_reg(ploc)));
      TN *tn = Build_Dedicated_TN (cl, reg, 0);
      tnl = TN_LIST_Push(tn, tnl, &MEM_pu_pool);
...
/*
   Return a TN_LIST of dedicated TNs that are 
     1) outgoing return value via registers if node_wn is ENTRY(or zero); or
     2) incoming return value via registers if node_wn is CALL.

   Note that the 'incoming' and 'outgoing' is from the view of current PU.
*/
static TN_LIST *Get_Dedicated_Return_TNs ( TY_IDX node_ty, WN * = 0 );
static TN_LIST *Get_Dedicated_Return_TNs ( TY_IDX node_ty, WN *node_wn )
{
  TN_LIST *tnl=0;
  BOOL func_entry = (node_wn == 0) ? OPR_FUNC_ENTRY 
                                   : WN_operator (node_wn) == OPR_FUNC_ENTRY;
  Return_Info_Dir dir = func_entry ? Return_Info_Outgoing : Return_Info_Incoming;
  PREG_NUM retpreg[MAX_NUMBER_OF_REGISTERS_FOR_RETURN];

  if (WHIRL_Return_Info_On) {

    RETURN_INFO return_info = Get_Return_Info (TY_ret_type(node_ty),
                                               No_Simulated,
                                               dir);
    ...
    for (i = 0; i < RETURN_INFO_count(return_info); i++) {
      ...
      FmtAssert(CGTARG_Preg_Register_And_Class(retpreg[i], &cl, &reg),
                ("Don't know how to handle PREG%d", retpreg[i]));
      TN *tn = Build_Dedicated_TN (cl, reg, 0);
      tnl = TN_LIST_Push(tn, tnl, &MEM_pu_pool);
    }
  } else {
    Fail_FmtAssertion ( ("WHIRL_Return_Info_On should be TRUE"));
  }
...
/*
   Update annotations for ENTRY/EXIT/CALL. This records dedicated TNs for
   Entry's incoming register parameters, for Exit's outgoing register return-value,
   and for Call's outgoing register actuals and incoming register return-value.

   Note that 'incoming/outgoing' is from the point of view of the current PU.
 */
static void
Update_Annotations()
{
  ...
  TN *tn;
  TN_LIST *tns_in, *tns_out;
  ISA_REGCLASS rclass = TI_ISA_Regclass_Integer();
  ANNOTATION *ant;
  ...
  for (bb = REGION_First_BB; bb != NULL; bb = BB_next(bb))
  {
    if (BB_entry(bb))
    {
      ...
      if (my_ant == 0) continue;
      
      ENTRYINFO_in_params(my_ant) = Get_Dedicated_Param_TNs (
                                  ST_pu_type(pu_st), ENTRYINFO_entry_wn(my_ant));
      entry_wn = ENTRYINFO_entry_wn(my_ant);
    } else if (BB_exit(bb))
    {
      ant = ANNOT_Get(BB_annotations(bb), ANNOT_EXITINFO);
      EXITINFO *my_ant = ANNOT_exitinfo(ant);
      if (my_ant == 0) continue;

      // without passing entry_wn (or entry_wn is zero), the call still works
      EXITINFO_ret_value(my_ant) = Get_Dedicated_Return_TNs (ST_pu_type(pu_st), entry_wn);
    } else if (BB_call(bb))
    {
      ...
      WN *call_wn = CALLINFO_call_wn(my_ant);
      ...
      CALLINFO_actual_params(my_ant) = Get_Dedicated_Param_TNs (call_ty, call_wn);
      CALLINFO_ret_value(my_ant) = Get_Dedicated_Return_TNs(call_ty, call_wn);
    }
  }
}
...
convert_stmt_list_to_OPs(WN *stmt)
{
  ...
  for( ; stmt; stmt = next_stmt ) 
  {
	...
	if (INTRN_is_tie_intrinsic(WN_intrinsic(stmt))) {
          /* let the isel handle tie intrinsics */
...
Convert_WHIRL_To_OPs (WN *tree)
{
  ...
  /* replace dedicated TNs involved in REGION interface with the 
...
void Whirl2ops_Initialize(struct ALIAS_MANAGER *alias_mgr)
{
  ...
  last_loop_pragmas = new DYN_ARRAY<WN*>(&MEM_phase_pool);
...
void Whirl2ops_Finalize(void)
{
  ...
  if (last_loop_pragmas && last_loop_pragmas->Elements()>0 &&
      CG_opt_level > 1) {
    for (int i=0; i<last_loop_pragmas->Elements(); i++) {
      WN* pragma = last_loop_pragmas->Get(i);
      if (!WN_pragma_compiler_generated(pragma)) {
	ErrMsgSrcpos(EC_LNO_Bad_Pragma_String, WN_Get_Linenum(pragma),
		     WN_pragmas[WN_pragma(pragma)].name,
		     "not followed by a loop, ignored");
      }
    }
    last_loop_pragmas->Resetidx();
    delete last_loop_pragmas;
    last_loop_pragmas=NULL;
  }
