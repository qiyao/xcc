...
//=============================================================================
// The following functions are defined for external instruction selector.
//=============================================================================
//
// Get or create label name from branch node.
//
LABEL_IDX Get_WN_Label( WN *wn);

// End a basic block, should be called after generate code for a branch.
void End_Basic_Block(WN *branch);
BB * Start_New_Basic_Block(void);
BB * Get_Cur_BB(void);

// For Generation of memory operations:  Whirl2ops maintains a mapping between
// memory OPS and their corresponding WN node.  Later phases will use the
// mapping to remove redundant stores.  Two functions exists for maintaining
// the mappings.
...
// Usage:
//      Set_Last_Mem_OP( ops );
//      Build_OP( ...., ops ); /* Generate memory op */
//      Set_OP_To_WN_Map( tree );
//
// Set_OP_To_WN_Map will scan for all memory ops following Last_Mem_OP to
// the end of "ops".  Note that it is safe to call Set_OP_To_WN_Map even
// if you didn't generate any memory op between the call to Set_Last_Mem_OP
// and Set_OP_To_WN_Map.  So you won't have to predicate the calls to either
//

// Handle extra processing associate with a call.
extern void Handle_Call_Site_Extras(WN *call, OPERATOR call_opr);

// Handle extra processing associate with an indirect jump.
extern void Handle_XGOTO_Extras (WN *branch);

// Handle processing associate with an asm statement.
typedef void (asm_input_callback)(WN *wn, TN *result, OPS *ops);
extern void Handle_ASM (const WN* asm_wn, asm_input_callback *callback =NULL);

extern INT16 Variant_LDID(WN *node);
extern INT16 Variant_STID(WN *node);
extern INT16 Variant_ILOAD(WN *node);
extern INT16 Variant_ISTORE(WN *node);
