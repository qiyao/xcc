...
extern DLL_SHARED BE_SCOPE *Be_scope_tab;
...
extern DLL_SHARED BE_SYMBOL_TABLE	Be_symbol_table;
...
extern DLL_SHARED BE_PREG_TAB Be_preg_tab;
...
Create_Preg(TYPE_ID mtype, char *name, WN *home)
{
  ...
  Be_preg_tab[Get_Preg_Idx(retval)].Set_home_location(home);
  return retval;
}
...
{
  const UINT idx = Get_Preg_Idx(preg);
...
{
  const UINT idx = Get_Preg_Idx(preg);
  if (idx < Be_preg_tab.Size()) Be_preg_tab[idx].Set_home_location(home);
}
