...
#include "data_layout.h"		// Find_Section_Name_For_ST
#include "targ_sim.h"
#include "tie.h"
...
// Is this PREG an ASM output operand?
// NOTE: This must be in sync with front-end numbering (wfe_stmt.cxx)
static inline BOOL
Preg_Is_Asm_Output(PREG_NUM preg)
{
  static PREG_NUM first_asm_preg = -2;
  static bool first_asm_preg_initialized = false;

  if (first_asm_preg_initialized==false) {
    if (tie_info) {
      first_asm_preg = (PREG_NUM)(-(tie_info->max_num_output()+1));
    }
    first_asm_preg_initialized=true;
  }
  return preg <= first_asm_preg;
...
IPO_CLONE::Fix_ST (WN* cloned_wn, WN* wn)
{
  ...
  if (WN_operator(wn) == OPR_PRAGMA &&
    ...
    WN_set_pragma_asm_copyout_preg(cloned_wn,
                                   WN_pragma_asm_copyout_preg(cloned_wn)
                                   + _asm_neg_preg_adjustment);
  }
  ...
  if (ST_class(st) == CLASS_PREG) {
	...
	WN_offset(cloned_wn) += _sym->Get_cloned_preg_last_idx();
      if (Preg_Is_Asm_Output(offst))
	WN_offset(cloned_wn) += _asm_neg_preg_adjustment;
    }
...
IPO_SYMTAB::fix_base<ST>::operator () (UINT idx, ST* old_st) const
{
	...
	if (ST_base_idx (old_st) != ST_st_idx (old_st)) {
        ...
        // also promote named section ST_ATTR
        if (ST_sclass(old_st)==SCLASS_PSTATIC && ST_has_named_section(old_st)){
          ST_ATTR_IDX st_attr_idx;
          ST_ATTR&    st_attr = New_ST_ATTR(GLOBAL_SYMTAB, st_attr_idx);
          ST_ATTR_Init(st_attr, ST_st_idx(copy_st), 
                       ST_ATTR_SECTION_NAME, Find_Section_Name_For_ST(old_st));
        }
	break;
    default:
	break;
