...
public:
  ...
  T&      Get(mUINT32 idx) const { return (*this)[idx]; } // to access the array by idx
  void    Set(mUINT32 idx, const T& val) { (*this)[idx]=val; }
...
//for compatability with stl vectors:
  mUINT32 size(void)  const              { return _size; }
  bool    empty(void) const              { return _size == 0; }
  void    reserve(mUINT32 size)          { Force_Alloc_array(size); }
  void    reserve_and_bzero(mUINT32 size)
  { 
    Force_Alloc_array(size); 
    Bzero_array();
  }
  void    push_back(const T& val)        { _array[++_lastidx] = val; }
...
DYN_ARRAY<T>::Alloc_array(mUINT32 arr_size)
{
  ...
  if ( _array == NULL ) fprintf(stderr, "Out of memory\n");
#else
