...
extern DLL_SHARED BOOL xt_rw_str;	/* in config_targ_options.h */
...
#define DEFAULT_TEMP_SPACE_BYTES 64
...
// #endif
// static INT16 stack_align = DEFAULT_STACK_ALIGNMENT;  now a compiler option: xt_stack_alignment
...
{
  return (SCLASS_AUTO <= ST_sclass(sym) && ST_sclass(sym) <= SCLASS_FORMAL);
}
...
static ST *Formal_Sym(ST *sym, BOOL onstack)
{
  ...
  */
  ...
  if ( Trace_Frame )
  {
    fprintf(TFile, "<lay> SCLASS_FORMAL (%s, base %s, onstack= %d, preg= %d) allocated\n",
...
BOOL
ST_Needs_Alignment (ST *st) {
  // For now, this function requires alignment only for static arrays
  if (!Align_Arrays ||
...
void PIF_Safe_Align_ST (ST *st) {
  // Although we can get the current PIF through libti, it's safer
  // to always align at 16-byte boundaries.
  UINT align = 16;
  TY_IDX ty = ST_type (st);

  // Don't make alignment larger than the object size.
  if (TY_size(ty) < align)
    {
      align = TY_size(ty);
      // Round the object size up to the nearest power of two and use this
      // as the alignment.
      while (!IS_POW2(align))
	align++;
    }

  if (TY_align(ty) < align) {
...
{
  ST *new_blk = New_ST_Block(name, FALSE, SCLASS_UNKNOWN, xt_stack_alignment, 0);
...
Allocate_Space(ST *base, ST *blk, INT32 lpad, INT32 rpad, INT64 maxsize)
{
  ...
  if ((base == SP_Sym && Frame_Has_Calls) || base == FP_Sym) {
	...
	Set_ST_ofst(blk, ST_ofst(blk) + Stack_Offset_Adjustment_For_PU());
  }
  if ( Trace_Frame ) {
    if (ST_class(blk) == CLASS_CONST)
	fprintf ( TFile, "<lay> Allocate <constant: %s>", Targ_Print(NULL,STC_val(blk)));
    else
    	fprintf ( TFile, "<lay> Allocate %s", ST_name(blk));
    fprintf (TFile, " to %s: offset = %" LLD_FMT ", size = %" LLD_FMT "\n",
...
Assign_Object_To_Frame_Segment ( ST *sym, SF_SEGMENT seg, INT64 offset)
{
	if (Trace_Frame) {
		fprintf ( TFile,
			"<lay> Assigning symbol %s to segment %s at offset %" LLD_FMT "\n",
  ...
  FOREACH_SYMBOL (GLOBAL_SYMTAB,st,i) {
    ...
    if (STB_section(st) && ST_name_idx(st) == name) {
      if (STB_section_idx(st) != sec &&
          (STB_section_idx(st) == _SEC_TEXT || sec == _SEC_TEXT)) {
...
static bool
Align_In_Section (const char *sec_name)
{
  // This is a list of special sections that we do not align in. The reason for
  // that is because the compiler and the linker assume that things placed in these
  // sections from different object files will be laid out in a contiguous block.
  static const char *special_sections_prefix[] = {
    ".ctors",
    ".dtors",
    ".xt_except",
    NULL
  ...
  for (int i = 0; special_sections_prefix[i]; i++)
    {
      const char *special_sec = special_sections_prefix[i];
      if (!strncmp(sec_name, special_sec, strlen(special_sec)))
	return false;
    }
...
Assign_ST_To_Named_Section (ST *st, STR_IDX name)
{
	...
	// Make sure the symbol has the required alignment
	if (ST_Needs_Alignment(st) && Align_In_Section(Index_To_Str(name)))
	  PIF_Safe_Align_ST(st);
	...
	if (ST_class(st) == CLASS_FUNC) {
		...
		case SCLASS_PSTATIC:
			    ...
			    if (Gen_PIC_Shared)
			      /* TODO: We should distinguish variables with
				 initializers that require no relocations.
				 In the longer term, we should also make use
				 of .data.rel, .data.rel.ro and
				 .data.rel.ro.local.  */
			      sec = _SEC_DATA;
			    else
			      sec = _SEC_RDATA;
		...
		case SCLASS_EH_REGION:
			...
			break;
		case SCLASS_EXTERN:
			// ignore section attribute on extern declaration
			return;
		default:
			FmtAssert(FALSE,
		...
		if (sec == _SEC_BSS && STB_section_idx(newblk) != _SEC_BSS) {
		        DevWarn("change bss symbol %s to be initialized to 0", ST_name(st));
		...
		ST_Block_Merge (newblk, st, 0, 0, SEC_max_sec_size(sec));
		 ...
		 (ST_sclass(st) == SCLASS_FSTATIC || 
		  ST_sclass(st) == SCLASS_MERGE_STRING)) {
	  Set_ST_sclass(st, SCLASS_FSTATIC);
	  int align = ST_alignment(st);
	  // put c++ type name string from type_info function in
	  // "linkonce.r" section
	  SECTION_IDX  sec = _SEC_RDATA;
	  newblk = Get_Section_ST_With_Given_Name (sec,
						   SCLASS_UNKNOWN, name);
	  int base_align = ST_alignment(newblk);
	  if (base_align < align)
	    Set_STB_align(newblk, align);
	  Set_ST_base(st, newblk);
          ST_Block_Merge (newblk, st, 0, 0, SEC_max_sec_size(sec));
	}
	else
...
struct Assign_Section_Names_If_Inited
{
...
struct Assign_Section_Names_If_UnInited
{
    inline void operator() (UINT32, ST_ATTR *st_attr) const {
	ST *st;
	STR_IDX name;
...
// return section name for corresponding ST via st_attr table
struct find_st_attr_ro_secname {
        ST_IDX st;
        find_st_attr_ro_secname (const ST *s) : st (ST_st_idx (s)) {}
...
STR_IDX
Find_RO_Section_Name_For_ST (const ST *st)
{
    ST_IDX idx = ST_st_idx (st);
    ST_ATTR_IDX d;

    d = For_all_until (St_Attr_Table, ST_IDX_level (idx),
                          find_st_attr_ro_secname(st));
    FmtAssert(d != 0, ("didn't find section name for ST %s", ST_name(st)));
...
Max_Arg_Area_Bytes(WN *node)
{
  ...
  case OPR_INTRINSIC_CALL:
    ...
    if (!INTRN_is_tie_intrinsic(WN_intrinsic(node)))
      Frame_Has_Calls = TRUE;
    break;
#ifdef TARG_IA32
  case OPR_IO:
...
static TY_IDX Formal_ST_type(ST *sym)
{
  if (ST_sclass(sym) == SCLASS_FORMAL_REF)
  {
    return Make_Pointer_Type(ST_type(sym));
  }
  return Promoted_Parm_Type(sym);
}
...
#ifdef TARG_XTENSA
/* Record the symbol representing the first upformal argument. */
ST *first_upformal_sym = NULL;
#endif
...
{
  const PREG_NUM iarg = Dedicated_Preg_First_Incoming_Arg(TI_ISA_Regclass_Integer());

  Is_True(PLOC_reg(ploc) < (iarg+MAX_NUMBER_OF_REGISTER_PARAMETERS),
	  ("Get_Vararg_Symbol:  ploc %d out of range", PLOC_reg(ploc)));

  return vararg_symbols[PLOC_reg(ploc)-iarg];
}
  ...
  */
  for (i = 0; i < WN_num_formals(pu); i++)
  {
    ...
    Allocate_Entry_Formal (sym, PLOC_on_stack(ploc),
			   Preg_Is_Dedicated_Incoming_Arg(PLOC_reg(ploc)));

#ifdef TARG_XTENSA
    if ((first_upformal_sym == NULL) && PLOC_on_stack(ploc))
      first_upformal_sym = sym;
#endif
  }
  ...
  if ( PU_has_altentry(Get_Current_PU()))
  {
	...
	for (i = 0; i < WN_kid_count(tree); i++)
	{
	  ...
	  sym =  Formal_Sym(sym, PLOC_on_stack(ploc) || varargs);
    	  ...
    	  Allocate_Entry_Formal (sym, TRUE,
				 Preg_Is_Dedicated_Incoming_Arg(PLOC_reg(ploc)));
	}
      }
    }
  ...
  if (varargs) {
#ifdef TARG_XTENSA
    if (WN_num_formals(pu)) {  /* Can only use varargs if at least one formal*/
#endif
	/*
	...
	while (!PLOC_on_stack(ploc)) {
		...
		vararg_symbols[PLOC_reg(ploc)-iarg] = sym;
 		Allocate_Entry_Formal(sym, PLOC_on_stack(ploc),
				      Preg_Is_Dedicated_Incoming_Arg(PLOC_reg(ploc)));
		/*
...
#ifdef TARG_XTENSA
  /* We need the symbol representing the first upformal
     argument. If we have enough actual arguments, then we may
     have already found it in the formals list. Otherwise we
     create an upformal symbol to represent the potential vararg
     in the upformal area. */

  if (first_upformal_sym == NULL) {
    first_upformal_sym = New_ST();
    ST_Init(first_upformal_sym,
	    Save_Str("__first_upformal"),
	    CLASS_VAR, SCLASS_FORMAL,
	    EXPORT_LOCAL, MTYPE_To_TY(MTYPE_U4));

    Add_Object_To_Frame_Segment(first_upformal_sym, SFSEG_UPFORMAL, TRUE);
  }
#endif
}
  ...
  if (TY_is_varargs (pu_ty)) {
	...
	*upformal_size = 0;
	...
	for (i = 0; i < WN_num_formals(pu_tree); i++) {
            ST *sym = WN_st(WN_formal(pu_tree, i));
	    ploc = Get_Input_Parameter_Location (Formal_ST_type(sym));
	}
...
Calc_Local_Area (void)
{
	...
	case SCLASS_AUTO:
		...
		break;
	    if (Trace_Frame)
		fprintf (TFile, "<lay> local %s has size %" LLD_FMT "\n", ST_name(&st),
			 ST_size(&st));
...
{
  SF_Maxsize ( SFSEG_UPFORMAL ) = ROUNDUP(upformal_size, xt_stack_alignment);
  SF_Maxsize ( SFSEG_FORMAL ) = ROUNDUP(formal_size, xt_stack_alignment);
}
...
Init_Frame_For_PU (INT32 actual_size)
{
   ...
   */
  SF_Maxsize ( SFSEG_ACTUAL ) = ROUNDUP(actual_size, xt_stack_alignment);
...
Merge_Fixed_Stack_Frame(ST *SP_baseST, ST *FP_baseST)
{
  ...
  case SMODEL_LARGE:
    ...
    Set_ST_ofst (SF_Block(SFSEG_UPFORMAL), Stack_Offset_Adjustment_For_PU());
    MERGE_SEGMENT(FP_baseST, SFSEG_FORMAL, MAX_LARGE_FRAME_OFFSET);
    break;

  case SMODEL_DYNAMIC:
#ifdef TARG_XTENSA
    MERGE_SEGMENT(FP_baseST, SFSEG_ACTUAL, Max_Small_Frame_Offset);
    MERGE_SEGMENT(FP_baseST, SFSEG_FTEMP, Max_Small_Frame_Offset);
    /* attach formals and upformal to FP, but don't assign offsets till finalize */
...
Initialize_Stack_Frame (WN *PU_tree)
{
  ...
  if (Trace_Frame) {
	fprintf(TFile, "<lay> locals' size = %" LLD_FMT "\n", frame_size);
  ...
  if (Trace_Frame) {
	fprintf(TFile, "<lay> guess frame_size = %" LLD_FMT "\n", frame_size);
	fprintf(TFile, "<lay> stack model = %d\n", Current_PU_Stack_Model);
  }

  /* stack grows down; sp objects grow up from sp; fp objects grow up
     from fp */
  stack_direction = DECREMENT;
  SP_Sym = Create_Base_Reg (".SP", INCREMENT);
  FP_Sym = Create_Base_Reg (".FP", INCREMENT);
...
Calculate_Stack_Frame_Sizes (WN *PU_tree)
{
  ...
  if (Trace_Frame && actual_size != Current_PU_Actual_Size) {
	fprintf(TFile, "<lay> actual_size was %d, now is %d\n",
		Current_PU_Actual_Size, actual_size);
  }
...
Trace_Stack_Segments( char *msg, ST *SP_baseST, ST *FP_baseST )
{
  ...
  fprintf(TFile, "<lay> Stack Segment after \"%s\"\n", msg);
  ...
  for ( s = SFSEG_FIRST; s <= SFSEG_LAST; s = (SF_SEGMENT) (s + 1) ) {
    ...
    fprintf(TFile, "%s \t%" LLD_FMT "\t%s\t%" LLD_FMT "\t%d\t%" LLD_FMT "\n",
	    SF_Name(s), offset, basename, size, align, SF_Maxsize(s));
  }
  if (SP_baseST != NULL)
	fprintf(TFile, "%s \t\t%" LLD_FMT "\t\t%" LLD_FMT "\t%d\n",
...
INT64 Finalize_Stack_Frame (void)
{
...
#ifdef TARG_XTENSA
  UINT extra_stack_bytes = Stack_Size_Adjustment(Frame_Has_Calls);
#else
  UINT extra_stack_bytes = 0;
#endif
  ...
  case SMODEL_SMALL:
...
#ifdef TARG_XTENSA
    /* Why do we need to allocate space for the formals if there are
       upformals? */
	...
	extra_stack_bytes + (Frame_Has_Calls ? Stack_Offset_Adjustment_For_PU() : 0), 0);
    break;
  ...
  case SMODEL_DYNAMIC:
#ifdef TARG_XTENSA
    /* For xtensa we do the same as for SMODEL_SMALL, except we use FP
       instead of SP. */
    if (ST_is_not_used(SF_Block(SFSEG_FORMAL)) ) {
	if (Trace_Frame) fprintf(TFile, "<lay> formals not used\n");
    }
    else {
	MERGE_SEGMENT(FP_Sym, SFSEG_FORMAL, Max_Small_Frame_Offset);
    }
    Frame_Size = STB_size(FP_Sym);
    Assign_Offset(SF_Block(SFSEG_UPFORMAL), FP_Sym,
	extra_stack_bytes + (Frame_Has_Calls ? Stack_Offset_Adjustment_For_PU() : 0), 0);
#else
    Frame_Size = STB_size(SP_Sym) + STB_size(FP_Sym);
#endif
  ...
  Frame_Size = ROUNDUP(Frame_Size + extra_stack_bytes, xt_stack_alignment);
  ...
  if (!Frame_Has_Calls) {
	Frame_Size = MAX(0, Frame_Size - Stack_Offset_Adjustment_For_PU());
  }

  if ( Trace_Frame ) {
    Trace_Stack_Segments ( "Finalize_Stack_Frame", SP_Sym, FP_Sym);
    fprintf(TFile, "<lay> final frame_size = %" LLD_FMT "\n", Frame_Size);
  }
#ifdef TARG_XTENSA
  /* we have only one stack frame model, so all the gymnastics for t
     the others are wasted */
  if (Frame_Size > UINT32_MAX)
	ErrMsg (EC_LAY_stack_limit, Frame_Size, UINT32_MAX);
#else
  if (Current_PU_Stack_Model == SMODEL_SMALL
...
Allocate_Object ( ST *st )
{
  ...
  if (ST_sclass(st) == SCLASS_FORMAL) {
    /* Search through the base objects to find the block 'st' is
       assigned to. If the block is SFSEG_FORMAL, then we need to mark
       the formals as used so that they will be allocated. */
    ST *last_base_scan = st;
    ST *base_scan = ST_base(last_base_scan);
    while (base_scan && (base_scan != last_base_scan)) {
      if (base_scan == SF_Block(SFSEG_FORMAL)) {
	if (ST_is_not_used(SF_Block(SFSEG_FORMAL)) && Get_Trace(TP_DATALAYOUT, 1))
	  fprintf(TFile, "<lay> Marking formals as used\n");
	...
	break;
      ...
      last_base_scan = base_scan;
      base_scan = ST_base(base_scan);
    }
  }
#else
  if (ST_sclass(st) == SCLASS_FORMAL) {
  ...
  case SCLASS_LITERAL_POOL: {
    /* Xtensa only:  The symbol is allocated to the per-function literal section */
    FmtAssert( ST_class(base_st)==CLASS_VAR || ST_class(base_st)==CLASS_CONST,
	       ("Illegal symbol class for allocation to literal pool"));
    switch( ST_type( base_st ) ) {
    case MTYPE_F4:
    case MTYPE_I4:
    case MTYPE_U4:
      Fail_FmtAssertion("Illegal type for allocation to literal pool");
      break;
    }
    Allocate_Object_To_Section ( base_st, _SEC_LITERAL_POOL, Adjusted_Alignment(base_st));
    break;
  }
  ...
  case SCLASS_FSTATIC :
    if (ST_is_thread_private(st)) {
      ...
      if (ST_is_constant(st) && !(Is_String_Literal(st) && xt_rw_str)) {
	ST * PU_st = Get_Current_PU_ST();
	if (PU_st && ST_has_named_ro_section(PU_st)) {
	  STR_IDX name = Find_RO_Section_Name_For_ST (Get_Current_PU_ST());
	  Assign_ST_To_Named_Section (st, name);
	  return;
	}
	if (Gen_PIC_Shared)
	  /* TODO: We should distinguish variables with initializers that
	     require no relocations.  In the longer term, we should also make
	     use of .data.rel, .data.rel.ro and .data.rel.ro.local.  */
	  sec = _SEC_DATA;
	else
	  sec = _SEC_RDATA;
      }
  ...
  case SCLASS_DGLOBAL :
    if (ST_is_thread_private(st)) sec = _SEC_LDATA;
      ...
      if (Gen_PIC_Shared)
	/* TODO: We should distinguish variables with initializers that require
	   no relocations.  In the longer term, we should also make use of
	   .data.rel, .data.rel.ro and .data.rel.ro.local.  */
	sec = _SEC_DATA;
      else
	sec = _SEC_RDATA;
    }
...
Allocate_File_Statics (void)
{
  ...
  if ( ST_ATTR_Table_Size (GLOBAL_SYMTAB)) {
    /* We must separate these so that inited objects are layed out
       in a section prior to uninited ones. This matches what
       code gen and data layout expect. See prs 8884 and 8885 */
    For_all (St_Attr_Table, GLOBAL_SYMTAB,
	     Assign_Section_Names_If_Inited());
    For_all (St_Attr_Table, GLOBAL_SYMTAB,
        Assign_Section_Names_If_UnInited());
  }
...
DevWarn("found commons with same name %s; merge together %d->%d", ST_name(st), ST_index(st), ST_index(*cit));
  ...
  // We make 2 passes through the symbols in order to try to layout the aligned arrays
...
/* Pad symbols that are within +/- PAD_RANGE of the dcache size.  */
#define PAD_RANGE 0.05
...
struct pad_global_arrays
{
	...
	if ((ST_sclass(st) == SCLASS_UGLOBAL) && (ST_class(st) == CLASS_VAR)) {
	  ...
	  if (!ST_is_not_used(st) &&
	    ...
	    INT size = TY_size (ty);
	    INT set_size = xt_dcache_bytes;
	    INT pad_size = 0;

	    if (size > ((1.0 - PAD_RANGE) * set_size)) {
	      const INT mod = size % set_size;
	      const INT range = (int)(set_size * PAD_RANGE);
	      if (mod < range)
		pad_size = range - mod;
	      else if ((set_size - mod) < range)
		pad_size = range + (set_size - mod);
	    }
		...
		if (Get_Trace(TP_TEMP, 0x2000))
		  fprintf(TFile, "<cg-pad> range %.2f%%, symbol %s from %d bytes to %d bytes\n",
			  PAD_RANGE * 100.0, ST_name(st), size-pad_size, size);
...
Pad_Global_Arrays()
{
  if (xt_dcache_bytes > 0)
    {
      if (Get_Trace(TP_TEMP, 0x2000))
	fprintf(TFile, "<cg-pad> padding symbols\n");
...
extern void
Clear_Literal_Pool(void)
{
  ST *sy = SEC_block(_SEC_LITERAL_POOL);
  if( sy ) {
    Set_STB_size(sy, 0 );
  }
}
