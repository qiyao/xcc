...
extern DLL_SHARED ST *Local_Spill_Sym;	/* reserved close location for spill */
...
extern DLL_SHARED STACK_MODEL Current_PU_Stack_Model;    /* stack model of current pu */

extern DLL_SHARED STACK_DIR Stack_Direction(void);
...
/* 'st' must have a section name attribute. Return the name specified
    by the attribute. */
extern STR_IDX Find_Section_Name_For_ST (const ST *st);

/* 'st' must have an ro section name attribute. Return the name specified
    by the attribute. */
extern STR_IDX Find_RO_Section_Name_For_ST (const ST *st);
...
extern DLL_SHARED INT32 Current_PU_Actual_Size;	
