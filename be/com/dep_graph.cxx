...
#pragma hdrstop
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/com/RCS/dep_graph.cxx,v $ $Revision: 1.126 $";
#endif /* _KEEP_RCS_ID */
...
{
    if (Current_Dep_Graph)
	Current_Dep_Graph->PreserveMap(orig, wn);
}
...
void ARRAY_DIRECTED_GRAPH16::Erase_LNO_Graph()
{
...
#ifdef LNO
void ARRAY_DIRECTED_GRAPH16::Print_LNO(FILE *fp)
{
  VINDEX16 i;
  EINDEX16 e;
  if (_type==DEPV_ARRAY_ARRAY_GRAPH) {
  ...
  for (i=1; i<_v.Lastidx()+1; i++) {
   if (!_v[i].Is_Free()) {
	...
	if (is_load) {
      ...
      fprintf(fp,"Vertex %d for Wn = 0x%lx",i,_v[i].Wn);
	...
	CALL_STACK *calls, BOOL skip_bad) 
{
  ...
  if (WN_opcode(wn) == OPC_DO_LOOP) {
	...
	WN_st(wn)!=Tie_Output_Volatile_Preg) {
      Set_Bad_Mem(wn);
      return 1;
...
void SCALAR_STACK::Add_Scalar(WN *wn_call, SYMBOL* symbol, UINT snumber)
{
  Is_True(WN_operator(wn_call) == OPR_CALL
      || WN_operator(wn_call) == OPR_INTRINSIC_CALL
