...
//		Not to be used inside the LNO dll
//
//	    void Erase_LNO_Graph()
//
//		Remove all edge edges and vertices from the graph
//		Version to be used inside the LNO dll
//
...
#ifdef _KEEP_RCS_ID
static char *dep_graph_rcs_id = dep_graph_INCLUDED "$Revision: 1.49 $";
#endif /* _KEEP_RCS_ID */
...
extern "C" {
  ...
  void LNOPreserveMap(WN *orig, WN *wn);
...
public:
  ...
  void PreserveMap(WN *orig, WN *wn)
  {
    VINDEX16	origV;
    Is_True((Get_Vertex(wn)==0),("Dep_PreserveMapPair(): unexpected map"));

    if (origV = Get_Vertex(orig))
    {
      VINDEX16	v = Get_Vertex(wn);
...
extern DLL_SHARED ARRAY_DIRECTED_GRAPH16 *Current_Dep_Graph;
...
public:
  ...
  void Print(FILE *fp) {
    fprintf(fp,"Wn, Statment_Number = 0x%lx %d\n",
	Wn,Statement_Number); 
  }
  ~REFERENCE_NODE(); 
};
