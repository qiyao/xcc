...
// Module: fb_cfg.h
// $Revision: 1.22 $
// $Date: 2000/05/19 20:19:24 $
// $Author: murthy $
// $Source: /isms/cmplrs.src/osprey1.0/be/com/RCS/fb_cfg.h,v $
//
// Description:
//
// fb_cfg.h and fb_cfg.cxx implement a control flow graph for edge
// frequency data.  The CFG can can propagate known frequencies to
// compute unknown values. The CFG _MUST_ contain no critical edges.
//
// ====================================================================
// ====================================================================
...
typedef std::vector<FB_NODEX> NODE_VECTOR;

struct FB_NODE {

  NODE_VECTOR preds; // Predecessors
  NODE_VECTOR succs; // Successors
  ...
  void Print( FILE *fp, FB_NODEX nx ) const {
    ...
    fprintf( fp, "node %d: node_type %s, source 0x%lx, in_out_same %c,\n",
	     nx, buffer, source, ( in_out_same ? 'Y' : 'N' ) );
    fprintf( fp, "  one_edge_preds %c, one_edge_succs %c\n", 
             ( one_edge_preds ? 'Y' : 'N' ), ( one_edge_succs ? 'Y' : 'N' ) );
...
private:
  ...
  __gnu_cxx::hash_map< LABEL_IDX, FB_NODEX, __gnu_cxx::hash<LABEL_IDX>, __gnu_cxx::equal_to<LABEL_IDX>,
    mempool_allocator< __gnu_cxx::pair<LABEL_IDX,FB_NODEX> > > _lblx_to_nx;
...
public:
  ...
  FB_CFG() :
    ...
    _lblx_to_nx( 100, __gnu_cxx::hash<LABEL_IDX>(),
		 ...
		 mempool_allocator< __gnu_cxx::pair<LABEL_IDX,FB_NODEX> >(&_m) ),
