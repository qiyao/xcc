...
#define TP_CG_FEEDBACK_DRAW    0x1000


// ====================================================================
// Convenience Macros
// ====================================================================
#define FB_Map WN_MAP_FEEDBACK
...
public:
  ...
  void    Annot_cycle_count ( FB_FREQ freq ) { _cycle_count = freq; }
  FB_FREQ Query_cycle_count ( ) const { return _cycle_count ; }
  ...
  void FB_lower_circuit_to_cselect ( WN *wn_circuit, WN *wn_cselect);
  ...
  void FB_peel_loop      ( WN *wn_loop_orig, WN *wn_loop_peeled, INT64 peeled_iterations, bool pre_peel );  
  void FB_unroll_loop    ( WN *wn_loop, INT unroll_factor, bool jamming=false );  
  void FB_hoist_outside_loop    ( WN *wn_loop, WN *wn );  
  void FB_tile_loop(WN *in_loop, WN *tile_loop, INT factor);
  void FB_adjust_loop ( WN *in_loop, INT64 iter_count);
  void FB_wind_down_loop( WN *winddown, INT new_trip);

  /* new_child has just been inserted under parent, (probably
     cloned from somewhere and moved here.)
     scale its frequencies to match the new number of entries.
     We can't ever be 100% accurate though, because this is new code,
     so we guess that it will be entered the parent number of times
     and that the relative number of times each path is taken will 
     be the same.

     This will also have the effect of marking every frequency as a guess
     which is a good thing here.

  */
  void FB_adjust_frequency( WN* parent, WN*new_child);
  ...
  void FB_lower_mstore_to_call ( WN *wn_mstore, WN *wn_call );
...
extern DLL_SHARED FEEDBACK *Cur_PU_Feedback;
extern DLL_SHARED void *Cur_PU_Profile;
