...
* $Revision: 1.123 $
* $Date: 2000/05/19 20:20:15 $
* $Author: murthy $
* $Source: /isms/cmplrs.src/osprey1.0/be/com/RCS/opt_alias_mgr.cxx,v $
*
* Revision history:
...
static char *rcs_id = 	opt_alias_mgr_CXX"$Revision: 1.123 $";
#endif /* _KEEP_RCS_ID */
...
ALIAS_MANAGER::ALIAS_MANAGER(void)
{
  ...
  _mtype_alias_class_map = CXX_NEW(MTYPE_ALIAS_CLASS_MAP(&_mem_pool, 11), &_mem_pool);
  _last_mtype_alias_class = 0;
  
  /* Assign each scalar and its corresponding vector memory type to the
     same type alias class so that they alias in regard to the ANSI type-based
     aliasing rules. */
  if (LNO_SIMD_Level != 0) {
    ISA_SIMD_TYPE_INFO isi = NULL;
    FOR_ALL_ISA_SIMD_TYPE_INFO(isi) {
      TYPE_ID base_type = TI_ISA_Simd_Base_Mtype(isi);
      TYPE_ID vmem_type = TI_ISA_Simd_Vector_Memory_Mtype(isi);
      Join_mtype_alias_classes(base_type, vmem_type);
    }
  }
  
  Set_pu_context(ac);
  _rule = CXX_NEW(ALIAS_RULE(this, ac), &_mem_pool);
...
//
// Find the 'wn's access high level type 'ty'. If 'wn' is a structure
// field access, determine the field offset and size if possible and return
// TRUE. Return FALSE otherwise.
//
static BOOL
Get_access_info (const WN *wn, TY_IDX &ty, UINT64 &offset, UINT64 &size)
{
  OPERATOR opr = WN_operator(wn);
  ty = (TY_IDX)NULL;
  ...
  if (opr != OPR_STBITS && opr != OPR_LDBITS &&
      opr != OPR_STID && opr != OPR_LDID &&
  ...
  UINT cur_field_id = 0;
  FLD_HANDLE fld = FLD_And_Offset_From_Field_Id(ty, field_id, cur_field_id, offset);
  Is_True(!fld.Is_Null(), ("Invalid struct field info"));
...
//
// Check if wn1 and wn2 access fields in the same struct type and the two
// fields are disjoint or possibly overlap.
//
enum SF_INFO { SF_NONE, SF_DISJOINT, SF_OVERLAP };
static SF_INFO
Disjoint_struct_fields (const WN *wn1, const WN *wn2)
{
  TY_IDX ty1 = (TY_IDX)NULL;
  TY_IDX ty2 = (TY_IDX)NULL;
  UINT64 ofst1 = 0, size1 = 0;
  UINT64 ofst2 = 0, size2 = 0;
  
  BOOL aok1 = Get_access_info(wn1, ty1, ofst1, size1);
  BOOL aok2 = Get_access_info(wn2, ty2, ofst2, size2);
  
  /* Both accesses must be to the same structure. */
  if (!ty1 || !ty2 || ty1 != ty2 || TY_kind(ty1) != KIND_STRUCT)
    return SF_NONE;
  
  /* Be conservative if unable to find the field offset and size. */
  if (!aok1 || !aok2)
    return SF_OVERLAP;
  
  if ((ofst1 + size1 <= ofst2) || (ofst2 + size2 <= ofst1))
    return SF_DISJOINT;
...
ALIAS_RESULT Aliased(const ALIAS_MANAGER *am, WN *wn1, WN *wn2) 
{
  ...
  if (OPERATOR_is_store(WN_operator(wn1)) && OPERATOR_is_load(WN_operator(wn2))) {
    SF_INFO sfi = Alias_Pointer_Types ? Disjoint_struct_fields(wn1, wn2) : SF_NONE;
    if (sfi == SF_DISJOINT)
      return NOT_ALIASED;

    /* Disable ANSI type checking if the accesses point to overlapping
       struct locations. */
    TY_IDX ty1 = (TY_IDX)NULL;
    TY_IDX ty2 = (TY_IDX)NULL;
    if (sfi != SF_OVERLAP) {
      ty1 = WN_object_ty(wn1);
      ty2 = WN_object_ty(wn2);
    }

    if (am->Rule()->Aliased_Memop(pt1, pt2, ty1, ty2)) 
      return POSSIBLY_ALIASED;
  } else {
...
ALIAS_RESULT Aliased_with_region(const ALIAS_MANAGER *am, const WN *wn, const WN *region_or_call, READ_WRITE how)
{
  ...
  if (OPERATOR_is_call(WN_operator(region_or_call))) {
    ...
    READ_WRITE how_aliased = am->Rule()->Aliased_with_Call(region_or_call, pt);
...
{
  if (Alias_Pointer_Types && Disjoint_struct_fields(wn1, wn2) == SF_DISJOINT) { 
    return NOT_ALIASED;
  }
...
void
ALIAS_MANAGER::Join_mtype_alias_classes (TYPE_ID mtype1, TYPE_ID mtype2)
{
  UINT32 ac1 = Mtype_alias_class(mtype1);
  UINT32 ac2 = Mtype_alias_class(mtype2);
  
  if (_trace) {
    fprintf(TFile,
            "Joining mtype alias classes of %s (current %u) and %s (current %u)...\n",
            MTYPE_name(mtype1), ac1, MTYPE_name(mtype2), ac2);
  }

  /* If no class assigned yet to either type, create a new one. */
  if (ac1 == 0 && ac2 == 0) {
    _last_mtype_alias_class++;
    _mtype_alias_class_map->insert(mtype1, _last_mtype_alias_class);
    _mtype_alias_class_map->insert(mtype2, _last_mtype_alias_class);

    if (_trace) {
      fprintf(TFile, ".. created new mtype alias class %u.\n", _last_mtype_alias_class);
    }

    return;
  }

  /* If only one type has an assigned class, assign the other type to the
     same class. */
  if (ac1 == 0 && ac2 != 0) {
    _mtype_alias_class_map->insert(mtype1, ac2);
    
    if (_trace) {
      fprintf(TFile, ".. assigned %s to alias class %u.\n",
              MTYPE_name(mtype1), ac2);
    }
  ...
  if (ac1 != 0 && ac2 == 0) {
    _mtype_alias_class_map->insert(mtype2, ac1);

    if (_trace) {
      fprintf(TFile, ".. assigned %s to alias class %u.\n",
              MTYPE_name(mtype2), ac1);
    }
  ...
  Is_True(ac1 != 0 && ac2 != 0,
          ("Unexpected mtype alias classes (%d, %d).", ac1, ac2));
  
  if (ac1 == ac2) {
    if (_trace) {
      fprintf(TFile, ".. already in the same alias class %u.\n", ac1);
    }
    
    return;
  }

  /* Reassign all types that are currently in mtype2's class to
     the class of mtype1. */
  MTYPE_ALIAS_CLASS_MAP::keyValuePair *scan;
  MTYPE_ALIAS_CLASS_MAP::iter iter(_mtype_alias_class_map);
      ...
      if (scan->value() == ac2)
        {
          ...
          if (_trace) {
            fprintf(TFile, ".. reassigned %s from alias class %u to %u.\n",
                    MTYPE_name(scan->key()), ac2, ac1);
          }
        }
    }
  
  Is_True(Mtype_alias_class(mtype2) == ac1,
          ("Mtype alias class update didn't work."));
}


UINT32
ALIAS_MANAGER::Mtype_alias_class (TYPE_ID mtype) const
{
  UINT32 alias_class = 0;
  _mtype_alias_class_map->find(mtype, alias_class);
  return alias_class;
}


static WN *
find_access_base (WN *wn)
{
  WN *base = NULL;
  ...
  switch (oper)
    ...
    default:
      for (INT i = 0; i < WN_kid_count(wn); i++)
        {
          base = find_access_base(WN_kid(wn, i));
          if (base)
            break;
        }
      break;
...
/* Look for
   OPR_STID (pointer) <- OPR_LDID (pointer), or
   OPR_STID (pointer) <- OPR_LDA (array). */
static bool
get_stid_ptr_typecast_mtypes (WN *wn, TYPE_ID &mtype1, TYPE_ID &mtype2)
{
  Is_True(WN_operator(wn) == OPR_STID,
          ("Unexpected operator %s", OPERATOR_name(WN_operator(wn))));
  
  mtype1 = MTYPE_UNKNOWN;
  mtype2 = MTYPE_UNKNOWN;
  ...
  WN *kid = WN_kid0(wn);
  OPERATOR kid_oper = WN_operator(kid);
  if (kid_oper != OPR_LDA && kid_oper != OPR_LDID)
    return false;
  
  TY_IDX ty2 = WN_ty(kid);
  switch (kid_oper) {
  case OPR_LDA:
    if (TY_kind(ty2) == KIND_POINTER)
      ty2 = TY_pointed(ty2);
    
    if (TY_kind(ty2) != KIND_ARRAY && TY_kind(ty2) != KIND_SCALAR)
      return false;
  ...
  case OPR_LDID:
    ...
    while (TY_kind(ty1) == KIND_POINTER &&
           TY_kind(ty2) == KIND_POINTER) {
      ty1 = TY_pointed(ty1);
      ty2 = TY_pointed(ty2);
    }
  ...
  default:
    FmtAssert(0, ("Unexpected kid operator %s", OPERATOR_name(kid_oper)));
    break;
  }
  
  if (TY_kind(ty1) != KIND_SCALAR ||
      TY_kind(ty2) != KIND_SCALAR)
    return false;
...
/* 'to_ty' is the high-level pointer type of the load/store access.
   'base' is the base LDID/LDA. Use the two to determine any pointer
   typecasting between 'mtype1' and 'mtype2'. */
static bool
get_base_ptr_typecast_mtypes (TY_IDX to_ty, WN *base,
                              TYPE_ID &mtype1, TYPE_ID &mtype2)
{
  mtype1 = MTYPE_UNKNOWN;
  mtype2 = MTYPE_UNKNOWN;
  ...
  if (base_oper != OPR_LDA && base_oper != OPR_LDID)
    return false;

  if (TY_kind(to_ty) != KIND_POINTER)
    return false;

  to_ty = TY_pointed(to_ty);
  if (TY_kind(to_ty) != KIND_SCALAR)
    return false;

  mtype1 = TY_mtype(to_ty);
  
  TY_IDX ty = WN_ty(base);
  switch (base_oper)
    {
    case OPR_LDA:
    ...
    default:
      FmtAssert(0, ("Unexpected base operator."));
      break;
    }
...
void
ALIAS_MANAGER::Collect_typecast_alias_classes (WN *wn)
{
  OPERATOR oper = WN_operator(wn);
  if (oper == OPR_BLOCK)
    {
      for (WN *kid = WN_first(wn); kid != NULL; kid = WN_next(kid))
        {
          Collect_typecast_alias_classes(kid);
        }
  ...
  for (INT i = 0; i < WN_kid_count(wn); i++)
    {
      Collect_typecast_alias_classes(WN_kid(wn, i));
    }

  TYPE_ID mtype1 = MTYPE_UNKNOWN;
  TYPE_ID mtype2 = MTYPE_UNKNOWN;
  ...
  case OPR_STID:
    /* Look for
       mtype2 *p2;
       mtype1 *p1 = (mtype1 *)p2; */
    if (!get_stid_ptr_typecast_mtypes(wn, mtype1, mtype2))
      return;
    break;
  ...
  case OPR_ILOAD:
    /* Look for
       mtype2 *p2;
       t = *(mtype1 *)p2; */
    if (!get_base_ptr_typecast_mtypes(WN_load_addr_ty(wn),
                                      find_access_base(WN_kid0(wn)),
                                      mtype1, mtype2))
      return;
    break;
  ...
  case OPR_ISTORE:
    /* Look for
       mtype2 *p2;
       *(mtype1 *)p2 = t; */
    if (!get_base_ptr_typecast_mtypes(WN_ty(wn),
                                      find_access_base(WN_kid1(wn)),
                                      mtype1, mtype2))
      return;
    break;
  ...
  Join_mtype_alias_classes(mtype1, mtype2);
}


void
Join_mtype_alias_classes (ALIAS_MANAGER *am, TYPE_ID mtype1, TYPE_ID mtype2)
{
  am->Join_mtype_alias_classes(mtype1, mtype2);
}


UINT32
Mtype_alias_class (const ALIAS_MANAGER *am, TYPE_ID mtype)
{
  return am->Mtype_alias_class(mtype);
}
