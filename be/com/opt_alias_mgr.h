...
#ifdef _KEEP_RCS_ID
static char *opt_alias_mgrrcs_id = 	opt_alias_mgr_INCLUDED"$Revision: 1.28 $";
#endif /* _KEEP_RCS_ID */
...
class ALIAS_MANAGER {
 public:
  typedef UTL_Map<TYPE_ID, UINT32, UTL_Map_IntHash> MTYPE_ALIAS_CLASS_MAP;
...
private:
  ...
  MTYPE_ALIAS_CLASS_MAP *_mtype_alias_class_map;
  UINT32                 _last_mtype_alias_class;
...
public:
  ...
  /* Infrastructure to allow certain classes of types to alias in regard
     to the standard ANSI type-based alias rules. Each type may be assigned
     an integer type alias class id. If 2 types are assigned to the same non-zero
     class id, the 2 types alias. */
  MTYPE_ALIAS_CLASS_MAP *Mtype_alias_class_map (void) const { return _mtype_alias_class_map; }
  void Join_mtype_alias_classes (TYPE_ID mtype1, TYPE_ID mtype2);
  UINT32 Mtype_alias_class (TYPE_ID mtype) const;

  void Collect_typecast_alias_classes (WN *wn);
};
