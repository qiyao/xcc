...
#ifdef _KEEP_RCS_ID
static char *opt_alias_rulercs_id = 	opt_alias_rule_INCLUDED"$Revision: 1.26 $";
#endif /* _KEEP_RCS_ID */
...
*
*   A.5: (Call_rule)
*     A procedure call does not affect local automatic variables that
*     are not addr_taken_and_saved. Note that local static variables
*     may be affected if we cannot prove that the call avoids
*     recursively invoking the current PU.
*
*   A.6: (Qual_rule)
...
public:
  ...
  ALIAS_RULE (ALIAS_MANAGER *mgr, ALIAS_CONTEXT ac) :
    _alias_mgr(mgr), _context(ac) { }
  ...
  //
  READ_WRITE Aliased_with_Call(const WN *, const POINTS_TO *) const;
