...
BOOL POINTS_TO::Overlap(const POINTS_TO *p) const
{
  ...
  if (Target_Analyze_Alias(this->Base(), this->Byte_Ofst(), this->Byte_Size(),
			   p->Base(), p->Byte_Ofst(), p->Byte_Size()))
    return TRUE;
...
//	Set_const();   // make sure the addr is not modified by calls.
    ...
    case SCLASS_REG:
      ...
      Set_safe_to_speculate();
      Set_local();
      if (Preg_Is_Dedicated(byte_ofst))
	Set_dedicated();
      break;
    case SCLASS_TEXT:
  ...
  if (Base_is_fixed()) {
    ...
    if (wn) {
	  ...
	  TIE_MACRO_p tie_macro = tie_info->tie_macro(tie_macro_id);
	    ...
	    object_size = tie_macro->simple_addr_load_store_size();
	    if (object_size == 0)
	      size = 0;
	  }
	}
      } else {
	object_size = WN_object_size(wn);
      }
    }
    ...
    Reset_safe_to_speculate();
    if (Ofst_kind() == OFST_IS_FIXED && Is_pointer() && wn != NULL && object_size != -1) {
      Set_byte_size(Byte_Size() + object_size);
    } else
      Set_ofst_kind(OFST_IS_UNKNOWN);
  }
...
void POINTS_TO::Print(FILE *fp) const
{
  ...
  if (pr_ofst) {
    ...
    case OFST_IS_INVALID:
      ...
      break;
    case OFST_IS_FIXED:
      fprintf(fp, "byte ofst is %" LLD_FMT ", ", Byte_Ofst());
      if (Bit_Size() != 0)
  ...
  if (pr_size) {
    fprintf(fp, "byte size is %" LLD_FMT ", ", Byte_Size());
    if (Bit_Size() != 0)
