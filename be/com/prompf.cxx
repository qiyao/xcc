...
static const char* Whirl_Symbol_Type(WN* wn)
{
  ...
  if (ST_class (st) != CLASS_PREG)
      return ST_name (st);
  else if (!Preg_Is_Dedicated(WN_offset(wn_symbol)))
...
{ 
  ...
  if (map_id != 0) {
    ...
    DevWarn("Missing Prompf Id for 0x%x %s", wn_tree, 
...
{ 
  ...
  if (map_id != 0) {
    ...
    PM_ASSERT(Prompf_Id_Type(wn_tree, wn_region, &is_first) == MPID_UNKNOWN, 
      (fp, "PROMPF_INFO: Missing Prompf Id for 0x%lx %s", 
