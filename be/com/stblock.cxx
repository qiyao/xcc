...
Adjusted_Alignment(ST *sym)
{
  ...
  INT32  align;
  TY_IDX ty_idx = Promoted_Parm_Type(sym);
...
INT32 ST_alignment(ST *sym)
{
  ...
  if (ST_pu_defined(sym))
  {
    if (Is_Allocated(sym))
    {
      ...
      Is_True(Assembly || (basealign>=align), ("sym has align > than base"));
...
Copy_ST_Block (ST *orig_blk)
{
  ST *new_blk = New_ST_Block (ST_name_idx(orig_blk),
	...
	ST_sclass(orig_blk), STB_align(orig_blk), 0);
...
ST *
Find_Return_Addr_Symbol (SYMTAB_IDX stab)
{
    ST_IDX st_idx = For_all_until (St_Table, stab, is_return_addr_sym ());

    if (st_idx == 0)
	return NULL;
    else
	return &St_Table[st_idx];
} // Find_Return_Addr_Symbol
