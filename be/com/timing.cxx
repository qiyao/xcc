 ...
 * Module: timing.c
 * $Revision: 3.33 $
 * $Date: 2000/04/06 02:00:17 $
 * $Author: mtibuild $
 * $Source: /isms/cmplrs.src/osprey1.0/be/com/RCS/timing.cxx,v $
 *
 * Revision history:
...
Initialize_Timing ( BOOL enable )
{
  ...
  if ( Enabled = enable ) {
    ...
    Timer ( T_Prep_Comp ) =
	    Resource_Alloc ( "  Scheduling Preparation", NULL );
    Timer ( T_Prep_CU ) =
	    Resource_Alloc ( "  Scheduling Preparation", Timer(T_Prep_Comp) );
    Timer ( T_Freq_Comp ) =
    ...
    Timer ( T_Peep_Comp ) =
	    Resource_Alloc ( "  Peephole Optimization", NULL );
    Timer ( T_Peep_CU ) =
	    Resource_Alloc ( "  Peephole Optimization", Timer(T_Peep_Comp));
    Timer ( T_Emit_Comp ) =
...
Report_CG_Region_Timing (FILE *file, char *name)
{
  ...
  Report_Delta_Time ( file, T_Prep_CU );
  ...
  Report_Delta_Time ( file, T_Peep_CU );
  ...
  char *source )
{
  if ( Enabled ) {
	...
	Report_Delta_Time ( file, T_Prep_Comp );
	...
	Report_Delta_Time ( file, T_Peep_Comp );
