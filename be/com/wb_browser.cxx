...
#ifndef __GNUC__ 
/* gyrations for different name manglers */
...
#elif _WIN32
#pragma weak _Print__17DO_LOOP_INFO_BASEP6_iobufi
#pragma weak _Print__7IF_INFOP6_iobuf
#pragma weak _Print__C12ACCESS_ARRAYP6_iobufi
#pragma weak _Print__C13ACCESS_VECTORP6_iobufii
#pragma weak _Summary__10WB_BROWSERP6_iobuf
#else
...
#else
    DevWarn("Windows has trouble with ACCESS_ARRAYS!\n");
#endif
...
#else
    DevWarn("Windows has trouble with access arrays\n");
#endif
...
{ 
  if (OPCODE_is_load(WN_opcode(wn)) || OPCODE_is_store(WN_opcode(wn))) {
    ...
    switch (red_type) {
    case RED_ADD:
      fprintf(fp, "[%d] 0x%lx RED_ADD ", Carray().Next_Index(), wn);
      break;
    case RED_MPY:
      fprintf(fp, "[%d] 0x%lx RED_MPY ", Carray().Next_Index(), wn);
      break;
    case RED_MIN:
      fprintf(fp, "[%d] 0x%lx RED_MIN ", Carray().Next_Index(), wn);
      break;
    case RED_MAX:
      fprintf(fp, "[%d] 0x%lx RED_MAX ", Carray().Next_Index(), wn);
      break;
    }
    switch (red_type) {
    case RED_ADD:
		            ...
		            INT increment)
{
  ...
  case OPC_DO_WHILE:
    Dump_Spaces(fp, spaces);
    fprintf(fp, "[%d] 0x%lx DO_WHILE_LOOP \n", Carray().Next_Index(), wn);
    Carray().Enter_This_Node(wn);
    Loops_Walk(WN_while_body(wn), fp, spaces + increment, increment);
    break;
  case OPC_WHILE_DO:
    Dump_Spaces(fp, spaces);
    fprintf(fp, "[%d] 0x%lx WHILE_DO_LOOP \n", Carray().Next_Index(), wn);
    Carray().Enter_This_Node(wn);
    Loops_Walk(WN_while_body(wn), fp, spaces + increment, increment);
    break;
  case OPC_REGION:
...
{ 
  ...
  case '~': 
    ...
    Summary(stdout);
#else
    DevWarn("Summary trouble on Windows\n");
#endif
    break;
  default: 
