                                    ...
                                    INT common_shape_index)
{
  ...
  if (Is_kind_scalar()) {
    fprintf(fp, "Scalar ELEMENT_SIZE(%d) OFFSET(%" LLD_FMT ") ",
                             ...
                             INT value_index)
{
  ...
  if (Is_unknown())
    ...
    fprintf(fp, "(%" LLD_FMT ") ", Get_int_const_value());
  ...
  if (Has_const_operand()) {
    if (Get_kid() == 0) {
      ...
      if (OPCODE_nkids(Get_opcode()) == 2)
        fprintf(fp, "%" LLD_FMT " ", Get_const_value());
    } else {
      fprintf(fp, "%" LLD_FMT " ", Get_const_value());
      Node(fp, 0);
    }
                                ...
                                INT feedback_index)
{
  ...
  fprintf(fp, "CYCLE(%" LLD_FMT ") FREQ(%d) BB_COUNT(%d) STMT_COUNT(%d) ",
