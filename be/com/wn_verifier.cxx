...
class WN_Verifier{
 ...
 protected:
    ...
    BOOL     Exception_Info_Preserved(WN *wn, WN *parent_wn);
  ...
  public:
             WN_Verifier(WN *wn, Mtype_Return_Level rv);
            ~WN_Verifier();
...
WN_Verifier::WN_Verifier(WN *wn, Mtype_Return_Level rv)
{
    ...
    switch(OPCODE_operator(op))
    {
      ...
      case OPR_PICCALL:
	    ...
	    // Also check the ST pointer
            ...
            _is_tree_OK &= ST_is_not_NULL(wn,op);
#ifdef TARG_XTENSA
	    // Check is __exception_info returned by call to
	    // __start_cp_handler is preserved and not promoted to a
	    // PREG.
	    _is_tree_OK &= Exception_Info_Preserved(wn,parent_wn);
#endif
            break;
      case OPR_ICALL:
...
{      
  ...
  for (UINT i = 0; i < RETURN_INFO_count(return_info); i++)
    {
...
BOOL WN_Verifier::CALL_parent_LDID(WN *wn, WN *parent_wn)
{
  ...
  if (opr == OPR_LDID && ST_class(WN_st(wn)) == CLASS_PREG &&
      Preg_Is_Dedicated_Incoming_Ret(WN_offset(wn)))
    { 
	...
	slink_used = TRUE; // since we don't know, cannot assert
      ...
      // The load may be removed from the call by several STIDs that
      // are also storing return values (should we do a better check
      // here to make sure the kid of the STID is an LDID of a
      // dedicated preg??). */
      while ((temp_wn !=NULL) && (WN_operator(temp_wn) == OPR_STID))
...
#ifdef TARG_XTENSA
/*------------------------------------------------------------------
 * This procedure checks that the result of each call to
 * __start_cp_handler is called __exception_info and that the
 * __exception_info STID is neither stored nor promoted to a
 * PREG. This is required because the exception-handling runtime
 * expects to be able to access __exception_info from the stack.
 * *------------------------------------------------------------------*/
BOOL WN_Verifier::Exception_Info_Preserved (WN *wn, WN *parent_wn)
{
  ST *callee_st = WN_st(wn);

  /* We only check direct calls to __start_cp_handler. */
  if ((WN_operator(wn) != OPR_CALL) ||
      strcmp(ST_name(callee_st), "__start_cp_handler"))
    return TRUE;

  /* If the call has no successor, then we assume that it is in a
     comma block that will later be removed. So we skip this check for
     now... */
  WN *stid = WN_next(wn);
  if (!stid)
    return TRUE;

  /* If there is no exception region between the call to
     __start_cp_handler and the call to __cp_pop_exception, then we
     allow __exception_info to not be stored since there is nothing
     that can cause an exception, and thus no reason for the runtime
     to require access to __exception_info. */
  {
    WN *scan = stid;
    while (scan)
    {
      if (WN_operator(scan) == OPR_CALL)
      {
	if (!strcmp(ST_name(WN_st(scan)), "__cp_pop_exception"))
	  return TRUE;

	break;
      }
      else if (WN_operator(scan) == OPR_REGION)
	break;
  ...
  /* The next statement should be an STID directly into
     __exception_info, or into a PREG that is later stored to
     __exception_info. */
  if (WN_operator(stid) == OPR_STID)
  {
    /* Make sure the source for __exception_info is the return value
       of the call to __start_cp_handler. */
    WN *ldid = WN_kid0(stid);
    if (!ldid ||
	(WN_operator(ldid) != OPR_LDID) ||
	(ST_class(WN_st(ldid)) != CLASS_PREG) ||
	((WN_st(ldid) != Return_Val_Preg) &&
	 !Preg_Is_Dedicated_Incoming_Ret(WN_offset(ldid))))
    {
      Is_True(FALSE, ("WN_verifier Error (Exception_Info_Preserved): Value stored to "
		      "__exception_info is not return of __start_cp_handler"));
      return FALSE;
    }

    /* If 'stid' stores to a PREG, then search forward in the block
       for an STID that sources the PREG. It may be a copy to another
       PREG, we keep searching until we find a non-PREG stid. */
    while (stid && (ST_class(WN_st(stid)) == CLASS_PREG))
    {
      PREG_NUM search_preg = WN_offset(stid);
      stid = WN_next(stid);
	...
	if (WN_operator(stid) == OPR_STID)
        {
	  WN *preg_ldid = WN_kid0(stid);
	  if (preg_ldid &&
	      (WN_operator(preg_ldid) == OPR_LDID) &&
	      (ST_class(WN_st(preg_ldid)) == CLASS_PREG) &&
	      (WN_offset(preg_ldid) == search_preg))
	    break;
	}
    ...
    if (stid)
    {
      ST *stid_st = WN_st(stid);
      if (stid_st &&
	  (ST_class(stid_st) != CLASS_PREG) &&
	  !strcmp(ST_name(stid_st), "__exception_info"))
	return TRUE;
    }
  }

  Is_True(FALSE, ("WN_verifier Error (Exception_Info_Preserved): Result of "
		  "__start_cp_handler not stored to __exception_info"));
...
BOOL WN_Verifier::STID_check_st_class(WN *wn)
{
  ...
  if (opr == OPR_STID) {
    ...
    if ( (ST_class(st) == CLASS_PREG) && 
         (Preg_Is_Dedicated_Outgoing_Ret(WN_offset(wn))) )
    {
       WN *temp_wn = WN_next(wn);
       
       // There could be several STIDs following one another
       // before actual return statement. So skip any STIDs to dedicated pregs.
       while ((temp_wn != NULL) && 
	      (WN_operator(temp_wn) == OPR_STID) &&
	      (ST_class(WN_st(temp_wn)) == CLASS_PREG ) &&
	      (Preg_Is_Dedicated_Outgoing_Ret(WN_offset(temp_wn)))  )
       {
...
WN_Verifier::Field_id_valid (WN* wn)
{
	...
	if (TY_kind(*ty) != KIND_STRUCT) {
	  ...
	  if ((WN_desc(wn) != MTYPE_M) &&
	    ...
	    //	    Is_True(FALSE, ("field_id and descriptor type are inconsistent"));
	    DevWarn("SHOULD BE Is_True:: field_id and descriptor type are inconsistent.\n"
		    "field_id = %d, desc = %d, desc_size = %d, ty_size = %d",
		    WN_field_id(wn), WN_desc(wn), MTYPE_byte_size(WN_desc(wn)), TY_size(*ty));
	  }
#endif
	}
	break;
...
WN_verifier(WN *wn, BOOL no_sim)
{
  ...
  WN_Verifier wnv(wn, (no_sim) ? No_Simulated : Complex_Not_Simulated);
