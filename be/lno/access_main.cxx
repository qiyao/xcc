...
return_point:
  ...
  if (dli->Est_Num_Iterations == -1) {
    ...
    if (Cur_PU_Feedback && dli->Num_Iterations_Symbolic) {
      ...
      INT32 freq_loop_header = (INT32) info_loop.freq_positive.Value();
      INT32 freq_loop_body   = (INT32) info_loop.freq_iterate.Value();
      
      if (LNO_Verbose) {
	fprintf(stdout, "Header executed  %f\n", freq_loop_header);
	fprintf(stdout, "Body executed  %f\n", freq_loop_body);
      }
	...
	if (LNO_Verbose)
	  fprintf(stdout, "Iteration counts from profile %" LLD_FMT "\n", dli->Est_Num_Iterations);
      }
    }
  }
}
...
extern void LNO_Build_If_Access(WN *wn, DOLOOP_STACK *stack)
{
  ...
  if (Cur_PU_Feedback && info->Freq_True<0 && info->Freq_False<0) {
    FB_Info_Branch fb_info = Cur_PU_Feedback->Query_branch(wn);
    float total = fb_info.freq_taken.Value() + 
      fb_info.freq_not_taken.Value();
    if (total > 0.0) {
      info->Freq_True = fb_info.freq_taken.Value()/total;
      info->Freq_False = fb_info.freq_not_taken.Value()/total;
    }
    if (LNO_Verbose) {
