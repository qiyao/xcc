...
{ 
  ...
  if (ccount + ll_size < tcount) {
    INT new_char_count = sprintf(buf + ccount, "%" LLD_FMT "", value);
    return ccount + new_char_count;
  } 
  for (INT i = 0; i < ccount; i++)
    sprintf(buf + i, "%c", '&');
			 ...
			 BOOL print_brackets) const
{
  ...
  for (INT32 i = 0; i < Nest_Depth(); i++) {
	...
	new_ccount = snprintfs(bf, new_ccount, MAX_TLOG_CHARS, "*idx");
				        ...
				        BOOL is_bound) const
{
  ...
  if (!is_bound) {
    if (Const_Offset != 0) {
      fprintf(fp,"%" LLD_FMT " ",Const_Offset);
      seen_something = TRUE;
    }
  }
...
SUMPROD_LIST *Add(SUMPROD_LIST *sl1, SUMPROD_LIST *sl2,
		  MEM_POOL *mem_pool) {
  SUMPROD_LIST *res = CXX_NEW(SUMPROD_LIST,mem_pool);
  if (sl1)
    res->Init(sl1,mem_pool);
  if (!sl2)
    return res;
  
  SUMPROD_ITER iter2(sl2);
  // for each node in list 2 ...
  for (SUMPROD_NODE *node2 = iter2.First();
       !iter2.Is_Empty(); node2 = iter2.Next()) {
    SUMPROD_ITER iterr(res);
    SUMPROD_NODE *noder=NULL;
    // ... try to find it in the result list
    for (noder = iterr.First();
	 !iterr.Is_Empty(); noder=iterr.Next()) {
      if (*(noder->Prod_List)==*(node2->Prod_List)) {
	break;
      }
    }
    if (noder) {
      noder->Coeff += node2->Coeff;
      if (noder->Coeff==0) {
	res->Remove_node(noder);
	CXX_DELETE(noder,mem_pool);
      }
    } else {
      SUMPROD_NODE *n = CXX_NEW(SUMPROD_NODE(node2,mem_pool),mem_pool);
      res->Append(n);
    }
  }
...
SUMPROD_LIST *Subtract(SUMPROD_LIST *sl1, SUMPROD_LIST *sl2,
		       MEM_POOL *mem_pool) {
  SUMPROD_LIST *res = CXX_NEW(SUMPROD_LIST,mem_pool);
  if (sl1)
    res->Init(sl1,mem_pool);
...
char* SYMBOL::Name(char* buf, INT bufsz) const
{
  ...
  if (u._st == NULL) {
    ...
    const char* name;
    BOOL        use_woff = TRUE;
    if (!Preg_Is_Dedicated(WN_Offset())) {
    ...
    if (ST_Base()) {
      ...
      if (name == NULL || name[0] == '\0') {
        ...
        sprintf(name, "$noname0x%lx", ST_Base());
      }
    }
    else
      name = "$nobase";
    ...
    if (slow || ST_Base() != u._st)
      sprintf(goalname, "%s(%s.%" LLD_FMT ".%d)", true_name, 
...
INT ACCESS_ARRAY::Delinearize(DOLOOP_STACK *stack, INT dim, WN *wn)
{
  ...
  for (const SYMBOL_NODE *node = iter.First(); !iter.Is_Empty();
	...
	this_one_good &= node2->Prod_List->Contains(delin_symbol);
      ...
      if (this_one_good && av->Can_Delinearize(wn, dim, delin_symbol, stack)) {
...
/*--------------------------------------------------------------------------*
 * check to see if 'wn' is defined as wn = delin_symbol >> const            *
 * return the 'wn = delin_symbol >> const' node if this is true.            *
 *--------------------------------------------------------------------------*/
extern
WN *Is_Fraction_Of_Symbol(WN *wn, DEF_LIST *def_delin, 
			  const SYMBOL *delin_symbol)
{
  if (wn == NULL) { return NULL; }

  Is_True(WN_operator(wn) == OPR_LDID, 
	  ("Is_Fraction_Of_Symbol: input is not a LDID"));

  DEF_LIST *def = Du_Mgr->Ud_Get_Def(wn);
  if (!def || def->Incomplete()) {
    return NULL;
  }

  DEF_LIST_ITER iter(def);
  DU_NODE *du_node = iter.First();
    
  /* restricted to single def for now */
  if (!du_node || iter.Next()) return NULL;

  /* check y = delin >> constant */
  WN *def_wn = du_node->Wn();
  if (WN_operator(def_wn) == OPR_STID &&
      WN_operator(WN_kid0(def_wn)) == OPR_ASHR &&
      WN_operator(WN_kid1(WN_kid0(def_wn))) == OPR_INTCONST &&
      SYMBOL(WN_kid0(WN_kid0(def_wn))) == *delin_symbol) {
      
    /* same single def as the 'def_delin' */
    WN *wn_y_delin = WN_kid0(WN_kid0(def_wn));
    DEF_LIST *def1 = Du_Mgr->Ud_Get_Def(wn_y_delin);
    if (!def1 || def1->Incomplete()) {
      return NULL;
    }

    DEF_LIST_ITER iter1(def1);
    DEF_LIST_ITER iter2(def_delin);
    for (const DU_NODE *node=iter1.First(); !iter1.Is_Empty(); 
	 node=iter1.Next()){
      const WN *def = node->Wn();
      if (!def_delin->Contains(def)) {
	return NULL;
      }
    }
    for (const DU_NODE *node=iter2.First(); !iter2.Is_Empty(); 
	 node=iter2.Next()){
      const WN *def = node->Wn();
      if (!def1->Contains(def)) {
	return NULL;
      }
    }
    /* pass all the test */
...
/*--------------------------------------------------------------------------*
 * Find from symbols and loop bounds in 'av' where the symbol or bound is   *
 * defined by 'wn = delin_symbol >> const                                   *
 * return the STID node it this is true.                                    *
 *--------------------------------------------------------------------------*/
extern 
WN* Find_Delinear_Relation_Wn(WN *wn, INT dim, const SYMBOL *delin_symbol, 
			      ACCESS_VECTOR *av, INT nest_depth, 
			      const DOLOOP_STACK *stack)
{
  /* there are two kinds of input for this function.
     when called with dim>1, 'wn' is an array node, get the wn_delin from
     the array subscript for dimension 'dim'.
     otherwise, 'wn' is the 'wn_delin' */
  WN* wn_delin = wn;
  WN* array_index = NULL;
  if (dim >= 0) {
    Is_True(WN_operator(wn) == OPR_ARRAY,
	    ("Find_Delinear_Relation_Wn: called on a non-array"));

    // check for single 'def' of wn_delin
    array_index = WN_array_index(wn, dim);
    wn_delin    = Find_First_Ldid_For_Symbol(array_index, delin_symbol);
  ...
  if (dim >= 0) {
    /* search for y = delin >> constant */
    INTSYMB_ITER iter_sym(av->Lin_Symb);
    for (INTSYMB_NODE *node=iter_sym.First();!iter_sym.Is_Empty(); 
	 node=iter_sym.Next()) {
      if (node->Symbol != *delin_symbol) {
	// find the wn corresponding to the symbol in the access vector
	WN *wn = Find_First_Ldid_For_Symbol(array_index,&node->Symbol);
	WN *def_wn = Is_Fraction_Of_Symbol(wn, def_delin, delin_symbol);
	if (def_wn) {
	  return def_wn;
	}
      }
  ...
  /* There exist no such symbol directly in the subscript,
     now, search the loop coefficient for loop bounds  */
  for (INT i = 0; i < nest_depth; i++) {
    if (av->Loop_Coeff(i) != 0) {
      WN *cur_l = stack->Bottom_nth(i);
      for (LWN_ITER* wniter = LWN_WALK_TreeIter(WN_end(cur_l));
	   wniter != NULL; wniter = LWN_WALK_TreeNext(wniter)) {
	WN *wn = wniter->wn;
	...
	for (LWN_ITER* wniter = LWN_WALK_TreeIter(WN_kid0(WN_start(cur_l)));
	     wniter != NULL; wniter = LWN_WALK_TreeNext(wniter)) {
	  WN *wn = wniter->wn;
	  if (WN_operator(wn) == OPR_LDID) {
	    WN *def_wn = Is_Fraction_Of_Symbol(wn, def_delin, delin_symbol);
...
/*---------------------------------------------------------------------------*
 * Generate an ACCESS_VECTOR for (1<<const) * y <= delin_symbol              *
 *---------------------------------------------------------------------------*/
extern ACCESS_VECTOR* 
Gen_Shift_Right_Constraint(WN *shift_wn, const SYMBOL *delin_symbol,
			   INT nest_depth)
{
  SYMBOL symbol_y(shift_wn);
  INT    shift = WN_const_val(WN_kid1(WN_kid0(shift_wn)));
  //  add (1<<const) * y <= delin_symbol condition
  ...
  INTSYMB_NODE *y_node =
    CXX_NEW(INTSYMB_NODE(symbol_y, 1<<shift), &LNO_local_pool);
  tmp->Lin_Symb->Prepend(y_node);
  return tmp;
}
...
BOOL ACCESS_VECTOR::Can_Delinearize(WN *wn, const INT dim, 
				    const SYMBOL *delin_symbol,
				    DOLOOP_STACK *stack)
{
  ...
  if (Is_Consistent_Condition(tmp,wn)) {
    ...
    // if there is a symbol y = delin_symbol >> const
    // check:
    //  if (Is_Consistent_Condition(y < 0  && delin_sym < 0 && 'tmp', wn)) {
    //      return FALSE;
    //  if (Is_Consistent_Condition((1<<const) * y <= delin_symbol && 'tmp',
    //      wn))
    //      return FALSE;
    //  /* else */
    //      return TURE;
    ...
    /* find the shift wn */
    WN *shift_wn = Find_Delinear_Relation_Wn(wn, dim, delin_symbol, tmp,
					     Nest_Depth(), stack);
    /* found a y = delin_sym >> shift */
    if (shift_wn) {
      SYMBOL symbol_y(shift_wn);
      INT    shift  = WN_const_val(WN_kid1(WN_kid0(shift_wn)));
	...
	CXX_NEW(INTSYMB_NODE(symbol_y, 1), &LNO_local_pool);
      tmp1->Lin_Symb->Prepend(y_node);

      // is it possible that tmp1 < 0 (tmp1 <= -1)
      tmp1->Const_Offset = -1;
      ...
      delin_node = CXX_NEW(INTSYMB_NODE(*delin_symbol, 1), &LNO_local_pool);
      tmp2->Lin_Symb->Prepend(delin_node);

      // is it possible that tmp2 < 0 (tmp2 <= -1)
      tmp2->Const_Offset = -1;
#if 0
      fprintf(stderr,"\nCan_Delinearize: Found y = delim >> shift\n");
      fprintf(stderr,"Test condition\n");
      tmp->Print(stderr, 1, 1);
      fprintf(stderr,"\nExtra constraint for y\n");
      tmp1->Print(stderr, 1, 1);
      fprintf(stderr,"\nExtra constraint for delim\n");
      tmp2->Print(stderr, 1, 1);
      fprintf(stderr,"\n");
#endif
      if (Is_Consistent_Condition(tmp, wn, tmp1, tmp2)) {
#if 0
	...
	return FALSE;
      ...
      //      return FALSE;
      ACCESS_VECTOR *tmp3 = 
	Gen_Shift_Right_Constraint(shift_wn, delin_symbol, Nest_Depth());
#if 0
      fprintf(stderr, ">0 test succeed\n");
      fprintf(stderr, "%d * y <= delin for constant %d\n", 1<<shift, shift);
      tmp3->Print(stderr, 1, 1);
      fprintf(stderr, "\n");
#endif

      // is it possible that tmp3 <= 0:  ((1<<shift) * y - delin_symbol) <= 0
      if (!Is_Consistent_Condition(tmp, wn, tmp3)) {
#if 0
	fprintf(stderr, "additional test succeed\n\n");
#endif
	MEM_POOL_Pop(&LNO_local_pool);
	return TRUE;
      }
#if 0
      fprintf(stderr, "additional test failed\n\n");
#endif
    }
				...
				BOOL allow_nonlin)
{
  ...
  if (WN_operator(wn) == OPR_ADD) {
    ...
    if (WN_operator(WN_kid(wn,0)) == OPR_INTCONST) {
      ...
      // change coeff * x * (y<<const) to
      // (coeff << const) * x * y
      WN *kid0 = WN_kid0(wn);
      WN *kid1 = WN_kid1(wn);
      if (WN_operator(kid1) == OPR_SHL && 
	  WN_operator(WN_kid1(kid1)) == OPR_INTCONST) {
	WN *new_wn = LWN_CreateExp2(WN_opcode(wn), LWN_Copy_Tree(kid0),
				    LWN_Copy_Tree(WN_kid0(kid1)));
	Add_Sum(new_wn, coeff<<WN_const_val(WN_kid1(kid1)),stack,
		allow_nonlin);
	LWN_Delete_Tree(new_wn);
      } else if (WN_operator(kid0) == OPR_SHL && 
		 WN_operator(WN_kid1(kid0)) == OPR_INTCONST) {
	WN *new_wn = LWN_CreateExp2(WN_opcode(wn), 
				    LWN_Copy_Tree(WN_kid0(kid0)), kid1);
	Add_Sum(new_wn, coeff<<WN_const_val(WN_kid0(kid0)),stack,
		allow_nonlin);
	LWN_Delete_Tree(new_wn);
...
ACCESS_VECTOR *Subtract(ACCESS_VECTOR *v1, ACCESS_VECTOR *v2,
			MEM_POOL *mem_pool, BOOL use_max_depth)
{
  Is_True(v1 && v2, ("Access vector subtraction requires non-nil operands"));
  
  BOOL too_messy = v1->Too_Messy || v2->Too_Messy;
  ACCESS_VECTOR *v_to_free = NULL;
  
  if (v1->_nest_depth != v2->_nest_depth) {
    if (!use_max_depth)
      return NULL;
    if (!too_messy) { // use the MAX nest depth
      if (v1->_nest_depth<v2->_nest_depth) {
	v1 = CXX_NEW(ACCESS_VECTOR(v1,mem_pool),mem_pool);
	v1->Set_Nest_Depth(v2->_nest_depth);
	v_to_free = v1;
      } else {
	Is_True(v1->_nest_depth>v2->_nest_depth,("Wrong nest depth"));
	v2 = CXX_NEW(ACCESS_VECTOR(v2,mem_pool),mem_pool);
	v2->Set_Nest_Depth(v1->_nest_depth);
	v_to_free = v2;
      }
    }
  }
  ...
  rv->Too_Messy = too_messy;
  if (rv->Too_Messy)
    return rv;

  Is_True(v1->_nest_depth==v2->_nest_depth,("Bad nest depth set up code."));
  ...
  rv->Non_Lin_Symb = Subtract(v1->Non_Lin_Symb, v2->Non_Lin_Symb, rv->_mem_pool);
  
  // free up memory if necessary
  if (v_to_free) {
    CXX_DELETE(v_to_free,mem_pool);
  }
...
ACCESS_VECTOR *Add(ACCESS_VECTOR *v1, ACCESS_VECTOR *v2,
		   MEM_POOL *mem_pool, BOOL use_max_depth)
{
  Is_True(v1 && v2, ("Access vector addition requires non-nil operands"));
  ...
  rv->Non_Lin_Symb = Add(v1->Non_Lin_Symb, v2->Non_Lin_Symb, rv->_mem_pool);
