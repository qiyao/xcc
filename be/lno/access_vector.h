...
//				  MEM_POOL *mem_pool)
//
//		Return sl1 - sl2, store the new list in mem_pool
//		Either INPUT list might be NULL.
//
//    SUMPROD_LIST *Add(SUMPROD_LIST *sl1, SUMPROD_LIST *sl2,
//				  MEM_POOL *mem_pool)
//
//		Like Subtract, but sl1 + sl2
//
...
#ifdef _KEEP_RCS_ID
static char *access_vector_rcs_id = access_vector_INCLUDED "$Revision: 1.87 $";
#endif /* _KEEP_RCS_ID */
...
public:
  ...
  INT Negate_Me() ;
  void Merge(SUMPROD_LIST *sl);
  friend SUMPROD_LIST *Add(SUMPROD_LIST *sl1, SUMPROD_LIST *sl2,
			   MEM_POOL *mem_pool);
  friend SUMPROD_LIST *Subtract(SUMPROD_LIST *sl1, SUMPROD_LIST *sl2,
				MEM_POOL *mem_pool);
  void Print(FILE* fp) const;
  INT Print(char* bf, INT ccount) const;
...
class ACCESS_VECTOR {
  ...
  BOOL    _power_of_2; //   Delinearized_Symbol is power of 2
public:
  ...
  SYMBOL *Delinearized_Symbol;
  BOOL Is_Power_Of_2_Delin(void) const { return _power_of_2; }
  void Set_Power_Of_2_Delin(void) { _power_of_2 = TRUE; }
...
#ifdef LNO
  BOOL Can_Delinearize(WN *wn, INT dim, const SYMBOL *delin_symbol,
		       DOLOOP_STACK *stack);
#endif
...
friend ACCESS_VECTOR *Subtract(ACCESS_VECTOR *, ACCESS_VECTOR *,
			       MEM_POOL *mem_pool, BOOL use_max_depth = FALSE);
 ...
 *--------------------------------------------------------------------------*/
extern WN* Find_Delinear_Relation_Wn(WN *wn, INT dim, const 
				     SYMBOL *delin_symbol, 
 ...
 *---------------------------------------------------------------------------*/
extern ACCESS_VECTOR* Gen_Shift_Right_Constraint
(WN *shift_wn, const SYMBOL *delin_symbol, INT nest_depth);

class DEF_LIST;
extern WN *Is_Fraction_Of_Symbol(WN *wn, DEF_LIST *def_delin, const SYMBOL *delin_symbol);
#endif
