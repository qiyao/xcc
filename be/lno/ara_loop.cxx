...
void ARA_LOOP_INFO::Reduction_List(REDUCTION_LIST *rlist)
{
  ...
  for (i = 0; i < _reduction.Elements(); i++) {
    ...
    if (WN_operator(wn) == OPR_ISTORE) {
      prag = WN_CreateXpragma(WN_PRAGMA_REDUCTION, (ST_IDX) 0, 1);
...
ARA_LOOP_INFO::Generate_Parallel_Pragma()
{
  ...
  if (Is_Parallel()) {
    ...
    // Create the pragmas
    WN *prag = WN_CreatePragma(WN_PRAGMA_PARALLEL_DO, (ST_IDX) NULL, 0, 1);
    ...
    if (if_cond != NULL) {
      prag = WN_CreateXpragma(WN_PRAGMA_IF, (ST_IDX) 0, 1);
    ...
    if (dli->Is_Doacross) {
      prag = WN_CreatePragma(WN_PRAGMA_SYNC_DOACROSS, (ST_IDX) NULL, 0, 0);
      WN_set_pragma_compiler_generated(prag);
      WN_Set_Linenum(prag, WN_Get_Linenum(_loop));
...
ARA_LOOP_INFO::Generate_Copyout_Loop()
{
  ...
  WN *prag = WN_CreatePragma(WN_PRAGMA_PARALLEL_BEGIN, (ST_IDX) NULL, 0, 0);
  WN_set_pragma_compiler_generated(prag);
  WN_Set_Linenum(prag, WN_Get_Linenum(_loop));
  LWN_Insert_Block_Before(WN_region_pragmas(region),NULL,prag);
  if (Prompf_Info != NULL && Prompf_Info->Is_Enabled())  
  ...
  // Create the pragmas
  prag = WN_CreatePragma(WN_PRAGMA_PDO_BEGIN, (ST_IDX) NULL, 0, 0);
  if (Prompf_Info != NULL && Prompf_Info->Is_Enabled()) {
    INT loop_id = WN_MAP32_Get(Prompf_Id_Map, do_loop);
    WN_MAP32_Set(Prompf_Id_Map, prag, loop_id);
  }
  ...
  if (!_reduction.Elements() && !Index_Variable_Live_At_Exit(_loop) &&
    ...
    prag = WN_CreatePragma(WN_PRAGMA_NOWAIT, (ST_IDX) NULL,0,0);
    WN_set_pragma_compiler_generated(prag);
    WN_Set_Linenum(prag, WN_Get_Linenum(_loop));
    LWN_Insert_Block_Before(WN_region_pragmas(region), NULL, prag);
  } 
