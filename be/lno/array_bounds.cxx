...
{
  if (LNO_Verbose) {
     fprintf(stdout, " HMB: Replacing Messy Bound for Loop 0x%lx\n",
       wn_hoist_loop);
     fprintf(TFile, " HMB: Replacing Messy Bound for Loop 0x%lx\n",
       wn_hoist_loop);
  }
  DU_MANAGER* du = Du_Mgr;
  ARRAY_DIRECTED_GRAPH16* dg = Array_Dependence_Graph;
...
{ 
  ...
  return WN_kid_count(wn_tree) > 0 
    && opr != OPR_ADD && opr != OPR_SUB && opr != OPR_MPY && opr != OPR_NEG
    && !(opr == OPR_SHL && WN_operator(WN_kid1(wn_tree)) == OPR_INTCONST
	 && Contains_Index_Variable(WN_kid0(wn_tree)))
    && (!need_safe || Statically_Safe_Exp(wn_tree))
    /* && !Contains_Index_Variable(wn_tree) */
    && !Contains_Vertex(wn_tree);
} 
...
static SNL_REGION HMB_Hoist_Messy_Bounds(WN* wn_snl,
					 STACK<WN*>* stk_bounds, 
					 LS_IN_LOOP* loop_ls)
{
  ARRAY_DIRECTED_GRAPH16* dg = Array_Dependence_Graph; 
  ...
  if (i > inner_depth && stk_safe_messy.Elements() == 0)
    return SNL_REGION(wn_snl, wn_snl);

  // Indicate that SNL is being transformed.
  if (LNO_Verbose) { 
    fprintf(stdout, "HMB: Transforming SNL %s at 0x%lx\n", 
      WB_Whirl_Symbol(wn_snl), wn_snl);
    fprintf(TFile, "HMB: Transforming SNL %s at 0x%lx\n", 
      WB_Whirl_Symbol(wn_snl), wn_snl);
  } 
  ...
  return SNL_REGION(wn_version[0], wn_version[version_count-1]);
}
...
static void SNL_Hoist_Messy_Bounds(WN* wn_snl)
{
  ...
  SNL_REGION region = HMB_Hoist_Messy_Bounds(wn_max, stk_bounds, loop_ls); 

  // if transformed, update dependence
  if (region.First != wn_max) {
    // rebuild dependence graph for the region
    DOLOOP_STACK *dostack = CXX_NEW(DOLOOP_STACK(&LNO_local_pool),
				    &LNO_local_pool);

    Build_Doloop_Stack(LWN_Get_Parent(region.First), dostack);

    if (!dg->Build_Region(region.First, region.Last, dostack, TRUE,TRUE)) {
      DevWarn("Array dependence graph overflowed in SNL_Hoist_Messy_Bounds");
      if (region.First != region.Last) {
	LNO_Erase_Dg_From_Here_In(LWN_Get_Parent(region.First), dg);
      } else {
	LNO_Erase_Dg_From_Here_In(region.First, dg);
      }
    }
  }
}
