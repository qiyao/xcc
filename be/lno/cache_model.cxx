...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = cache_model_CXX "$Revision: 1.138 $";
#endif /* _KEEP_RCS_ID */
...
void FORMULA::Print(FILE* f, FORMULA_OP parent) const
{
  ...
  switch (_fop) {
   ...
   case FORMULA_MIN:
    ...
    fprintf(f, _fop == FORMULA_MAX ? "Max(" : "Min(");
    _kids.Left->Print(f);
    fprintf(f, ",");
    _kids.Right->Print(f);
    fprintf(f, ")");
          ...
          if (WN_operator(wn_leading_dim) == OPR_INTCONST) {
            ...
            if (leading >= f/2) {
              ...
              if ((diff-2)*128 < f) { // within 2 of f, or 3 if f>128, or 4 ...
                ...
                Rglist.Stride_Loop = -1;
                if (Debug_Cache_Model >= 3) {
                  fprintf(TFile, "pwr2 hack (spatial locality cs=%" LLD_FMT "): ",
                          Cur_Mhd->Size);
                  Print(TFile);
                  fprintf(TFile, "\n");
                }
...
FORMULA* Formula_For_Nk(INT        k,
			...
			INT*       const_answer)
{
  FmtAssert(outersz == -1 || outersz > 0, ("Bad outersz %d", outersz));
    ...
    for (ncnn = 0; ncnn < nc_nloops; ncnn++) {
      ...
      if (d <= dim) {       // jn is used in a problematic loop.
        ...
        if (ncn < ncnn || approx_inner_iters[jn] > 1) {
          if (Debug_Cache_Model >= 3) {
            fprintf(TFile, "pwr2 hack group locality, cs=%" LLD_FMT ": won't insert ",
                    Cur_Mhd->Size);
            n->Print(TFile);
            fprintf(TFile, "into ");
            rg->Print(TFile);
                    ...
                    INT               middle_loop_no)
{
  ...
  if (Debug_Cache_Model >= 3) {
    fprintf(TFile,
            "Compute_Footprint <tlb=%d>: v_first=%d outersz=%" LLD_FMT " loops=(",
            using_tlb, v_first, outersz);
    INT i;
    for (i = 0; i < nloops; i++)
      fprintf(TFile, i == 0 ? "%d" : ",%d", loops[i]);
    fprintf(TFile, ")\n");
  }
		       ...
		       INT64 		       u_nest_number)
{
  ...
  if (Debug_Cache_Model) {
    ...
    for (i = 0; i < nloops - 1; i++) 
      fprintf(TFile, "%d,", loops[i]);
    fprintf(TFile, "%d}: #%" LLD_FMT " ", loops[nloops - 1], u_nest_number); 
    fprintf(TFile, "+++++\n"); 
  } 
  ...
  if (Debug_Cache_Model) 
    fprintf(TFile, "USING MIDDLE LOOP ESTIMATE OF %" LLD_FMT " ITERATIONS\n", v1); 
  ...
  if (Debug_Cache_Model) {
    fprintf(TFile, "++++ RESULTS FOR NEST MODEL #%" LLD_FMT "\n", u_nest_number); 
...

  {
    ...
    // choosing reasonable first guesses.
    {
      ...
      for (l = 2; l <= ntiles+ninners; l++) {
        ...
        Nominal_Blocksize[l] = (INT64)MIN(double(1 << (i-1)), (MAX_BLOCKSIZE>>1));
      }
    }
  ...
  *stripdepth = depth+1;
  ...
  for (i = 0; i <= depth; i++) {
    ...
    if (exact_iters != -1 && est != exact_iters) {
      DevWarn("Est_Num_Iterations=%" LLD_FMT ", expected %" LLD_FMT "",
              dli->Est_Num_Iterations, exact_iters);
      est = exact_iters;
    }
    est_iters[i] = (est + unrolls[i] - 1) / unrolls[i];
    Is_True(est_iters[i] >= 0, ("Bug: est_iters=%" LLD_FMT "", est_iters[i]));
    if (est_iters[i] < 2)
      est_iters[i] = 1;
  ...
  for (i = 0; i <= depth; i++) {
    ...
    INT sz = (mhd_level >= 0) ? required_blocksize[i*MHD_MAX_LEVELS + mhd_level] : 1;
    if (sz > 1) {
