				   ...
				   LABEL_LIST* label_list)
{
  ...
  if (opcode == OPC_BLOCK) {
    ...
    if (uses && !uses->Incomplete() && uses->Is_Empty()) { //dead
      if (!Contains_Volatile_Refs(wn)) { // ok to remove
	  ...
	  if (!const_trip && (WN_kid_count(loop_info) == 2)) { // delete trip
	    WN *new_loop_info = 
	      LWN_CreateLoopInfo(LWN_Copy_Tree(WN_kid0(loop_info)),
				 NULL,0,0,0,WN_loop_at_region_id(loop_info));
	    ...
	    WN *new_loop_info =
	      LWN_CreateLoopInfo(LWN_Copy_Tree(WN_kid0(loop_info)),
				 trip_count,0,0,0,WN_loop_at_region_id(loop_info));
	    LWN_Delete_Tree(loop_info);
	    WN_set_do_loop_info(wn,new_loop_info);
	    LWN_Set_Parent(new_loop_info,wn);
	    loop_info = new_loop_info;
...
// Given a loop with a step size of 1, a lb >= 0 and an upper bound 
// (i <= divceil(n,const>0)), replace upper bound with i <= (n+const-1)/const
static void Remove_Div_Ceil(WN *loop)
{
  INT64  stepsz = Step_Size(loop);
  if (stepsz != 1) return;
  WN *lower_stid = WN_start(loop);
  if (OPCODE_desc(WN_opcode(lower_stid)) != MTYPE_I4) {
    return;
  }
  SYMBOL sl = SYMBOL(lower_stid);
  WN *test = WN_end(loop);
  if ((WN_operator(test) != OPR_LT) && (WN_operator(test) != OPR_LE)) {
    return;
  }
  WN *divceil = WN_kid1(test);
  WN *divceil_parent=test;
  INT kidno=1;
  if (WN_operator(divceil) == OPR_ADD) {
    WN *tmp = WN_kid1(divceil);
    if (WN_operator(tmp) != OPR_INTCONST ||
        WN_const_val(tmp) > 1) {
      return;
    }
    divceil_parent = divceil;
    kidno = 0;
    divceil = WN_kid0(divceil);
  }
  if (WN_operator(divceil) != OPR_INTRINSIC_OP) {
    return;
  }
  if (WN_intrinsic(divceil)  != INTRN_I4DIVCEIL) {
    return;
  }
  if (WN_operator(WN_kid0(test)) != OPR_LDID) {
    return;
  }
  if (SYMBOL(WN_kid0(test)) != lower_stid) {
    return;
  }
  WN *const_kid = WN_kid0(WN_kid1(divceil));
  if (WN_operator(const_kid) != OPR_INTCONST) {
    return;
  }
  if (WN_const_val(const_kid) <= 1) {
    return;
  }

  //Do the transformation
  WN *new_ldid = LWN_Copy_Tree(WN_kid0(WN_kid0(divceil)));
  LWN_Copy_Def_Use(WN_kid0(WN_kid0(divceil)),new_ldid,Du_Mgr);

  OPCODE addop = OPCODE_make_op(OPR_ADD, MTYPE_I4, MTYPE_V);
  WN *add = WN_CreateExp2(addop,new_ldid,
			        LWN_Make_Icon(MTYPE_I4,WN_const_val(const_kid)-1));

  OPCODE divop = OPCODE_make_op(OPR_DIV, MTYPE_I4, MTYPE_V);
  WN *div = LWN_CreateExp2(divop, add,
			        LWN_Make_Icon(MTYPE_I4,WN_const_val(const_kid)));
  LWN_Delete_Tree(divceil);
  WN_kid(divceil_parent,kidno) = div;
  LWN_Parentize(test);
}
...
extern WN* Guard_A_Do(WN *do_wn, bool move_higher)
{
  ...
  /* Some of the 'loop_stmt's in the end condition may point to the current
     loop (do_wn). Clear all of them to be on the safe-side. */
  LWN_Clear_Loop_Stmt(test, Du_Mgr);
  
  if (Array_Dependence_Graph) {
    if (!Array_Dependence_Graph->Add_Deps_To_Copy_Block(WN_end(do_wn),test,FALSE)) {
  ...
  WN_Set_Linenum(new_if,WN_Get_Linenum(do_wn));
    ...
    if (if_wn && (WN_opcode(if_wn) == OPC_IF) && (if_wn == WN_last(block_wn))) {
	  ...
	  if (non_const_loops != -1) {
	    ...
	    if (new_pos != if_wn) {  // we can move it higher
	      ...
	      LWN_Copy_Frequency_Tree(if_wn,new_pos);
...
BOOL Is_Consistent_Condition(ACCESS_VECTOR *av, WN *expr,
			     ACCESS_VECTOR *extra1, ACCESS_VECTOR *extra2)
{
  ...
  if (extra1 && (extra1->Too_Messy || extra1->Contains_Non_Lin_Symb())) 
    return TRUE;
  if (extra2 && (extra1->Too_Messy || extra1->Contains_Non_Lin_Symb())) 
    return TRUE;
  ...
  MEM_POOL_Push(&LNO_local_pool);
  {
    ...
    if (extra1) {
      info.Add_Access(extra1,expr,statement);
    }
    if (extra2) {
      info.Add_Access(extra2,expr,statement);
    }
    if (info.Bounds().Is_Consistent()) {
      answer = TRUE;
    } else {
...
static void STD_Canonicalize_Upper_Bound(WN* wn_loop)
{
  ...
  if (WN_operator(WN_end(wn_loop)) == OPR_LT) {
    if (COND_Do_Info(wn_loop, &LNO_local_pool) != COND_DO_AT_LEAST_ONCE) {
      ...
      WN *enclosed = Enclosing_Loop(LWN_Get_Parent(wn_loop));
      if (enclosed!=NULL && WN_operator(enclosed)==OPR_DO_LOOP) {
        SIMD_Msg(AT_MSG_SIMD_UNSIGNED_LOOP_UPPER_BOUND, wn_loop);
      }
    }
...
{ 
  ...
  if (WN_opcode(wn_tree) == OPC_BLOCK) {
    WN *wn_next = WN_first(wn_tree);
    while (wn_next)
      {
        /* When a loop is canonicalized, its statement may be wrapped into
           an if-guard, i.e., its next statement pointer would become NULL.
           Therefore, obtain the next statement before processing the current
           one. */
        WN *wn = wn_next;
...
extern void Update_Guarded_Do_FB(WN *if_wn, WN *do_wn, FEEDBACK *feedback)
{
  ...
  FB_Info_Branch if_freq;
  WN * parent = LWN_Get_Parent( do_wn );

  while ( parent && WN_operator( parent ) == OPR_BLOCK)
    parent = LWN_Get_Parent( parent );

  if (!parent) {
    DevWarn("No valid parent found for guarding a do");
    return;
  }

  if ( Wn_Is_Inside( if_wn, parent ) ) {
    if_freq.freq_taken = loop_freq.freq_positive;
    if_freq.freq_not_taken = loop_freq.freq_zero;
  }
  else {
    /* because the guard may have been moved past other flow control 
       (eembc_consumer_noTie_filters01 at -O3), we have to fix up
       the feedback by entries * (taken / total), rather than
       the absolute values
       
       FIXME: this has the potential to be wrong when the parent
       frequency total out is divided among two edges. This happens 
       rarely in practice though. And the current scheme is
       much better than just relying on the loop frequency.
    */
  
    FB_FREQ in  = feedback->Query_total_out(parent);
    float exit_freq = loop_freq.freq_exit.Value();
    if (exit_freq == 0.0)
      exit_freq = 1.0;
  
    if_freq.freq_taken = in * (loop_freq.freq_positive / exit_freq);
    if_freq.freq_not_taken = in * (loop_freq.freq_zero / exit_freq);
  }
