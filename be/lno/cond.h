...
***
***	void Update_Guarded_Do_FB(WN *if_wn, WN *do_wn, 
***                               FEEDBACK *feedback, speculated)
***
***	  Update feedback to reflect the fact that if_wn guards do_wn.
***       If speculated, then the guard doesn't appear immediately before the
***       loop
**/

/** $Revision: 1.18 $
*** $Date: 2000/04/06 02:04:27 $
*** $Author: mtibuild $
...
#ifdef _KEEP_RCS_ID
static char *cond_rcs_id = cond_INCLUDED "$Revision: 1.18 $";
#endif /* _KEEP_RCS_ID */
...
BOOL Hoist_Conditionals(WN *func_nd);
BOOL Is_Consistent_Condition(ACCESS_VECTOR *av, WN *expr, 
			     ACCESS_VECTOR *e1=NULL, ACCESS_VECTOR *e2=NULL);
extern WN* Guard_A_Do(WN *do_wn, bool move_higher=true); 
