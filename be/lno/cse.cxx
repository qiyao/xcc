...
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/cse.cxx,v $ $Revision: 1.28 $";
...
static void
schedule_delete (WN *wn)
{
  //  LWN_Set_Parent(wn, NULL);
  delete_stack->Push(wn);
}
...
static void Transform_Code(STACK<WN_PAIR_EC> *cse_stack, WN *loop, BOOL all_invariant)
{
  ...
  if (!all_invariant) {
	...
	add = LWN_CreateExp1(op,sum1);
      ...
      /* SCA: FIXME: these don't seem to have any effect */
  ...
  LWN_Copy_Frequency_Tree(wn1, stid);
