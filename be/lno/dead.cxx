...
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/dead.cxx,v $ $Revision: 1.15 $";
		...
		ARRAY_DIRECTED_GRAPH16 *dep_graph)
{
  ...
  if (!store) return;  // give up on dead store elimination when when the acess array is not on 
  		       // the address (this can happen if the address has an add in it
  ...
  for (e = dep_graph->Get_Out_Edge(v); e; e=next_e) {
    ...
    if (OPCODE_is_store(opcode) && (store2_wn != store_wn)) {
	  ...
	  WN_MAP_Get(LNO_Info_Map,WN_kid1(store2_wn));
        if (store2 && Equivalent_Access_Arrays(store,store2,store_wn,store2_wn) &&
