...
{ 
  ...
  name = ST_class(WN_st(wn_symbol)) != CLASS_PREG
    ? ST_name(WN_st(wn_symbol)) :
      !Preg_Is_Dedicated(WN_offset(wn_symbol))
...
{ 
  ...
  if (WN_operator(wn) == OPR_INTCONST) {
    fprintf(stdout, "%" LLD_FMT " ", WN_const_val(wn)); 
  } else { 
...
static BOOL Load_Loop(WN** wn_loop) 
{
  load_buffer(); 
  scan_blanks_and_tabs(); 
  sscanf(buffer + buffer_start, "0x%lx", wn_loop); 
  buffer_start += 2;
...
static void Load_mINT64(mINT64* value) 
{
  load_buffer(); 
  scan_blanks_and_tabs(); 
  sscanf(buffer + buffer_start, "%" LLD_FMT "", value); 
  skip_to_separator(SKIP_NUMERIC); 
}
		       ...
		       INT cc)
{
  ...
  switch(opr) {
   case OPR_CONST:
    ...
    break;
   case OPR_INTCONST:
    cc += sprintf(buffer + cc, "%" LLD_FMT "", WN_const_val(wn));
    break;
   case OPR_LDID:
...
static void WB_set_node() 
{
  WN* node;
  scan_blanks_and_tabs();  
  (void) sscanf(buffer + buffer_start, "0x%lx", &node);
  buffer_start += 2;
  skip_to_separator(SKIP_HEX); 
  cnode = node; 
  print_this_node(); 
...
static void WB_defs_loop()
{
  ...
  for (; itr != NULL; itr = LWN_WALK_TreeNext(itr)) {
    ...
    fprintf(stdout, "    Loop Statement: 0x%lx\n", def_list->Loop_stmt());
...
{ 
  ...
  if (opc == OPC_DO_LOOP || opc == OPC_IF || opr == OPR_REGION 
    ...
    if (opr == OPR_DO_LOOP && Get_Do_Loop_Info(cnode, TRUE) != NULL) {
      ...
      fprintf(stdout, "Outer Tile is 0x%lx\n", wn_outer_tile); 
    }
  } else {
    Error_Cleanup(); 
  }  
...
static void dump_reductions(WN* wn, REDUCTION_MANAGER* rm)
{
  if (WN_opcode(wn) == OPC_BLOCK) {  
    ...
    if (OPCODE_is_load(WN_opcode(wn)) || OPCODE_is_store(WN_opcode(wn))) {
      ...
      switch (red_type) { 
      case RED_ADD:
	fprintf(stdout, "[%d] 0x%lx RED_ADD ", reduction_count, wn);  
        break; 
      case RED_MPY:
	fprintf(stdout, "[%d] 0x%lx RED_MPY ", reduction_count, wn);  
        break; 
      case RED_MIN:
	fprintf(stdout, "[%d] 0x%lx RED_MIN ", reduction_count, wn);  
        break; 
      case RED_MAX:
	fprintf(stdout, "[%d] 0x%lx RED_MAX ", reduction_count, wn);  
        break; 
      }
      switch (red_type) { 
      case RED_ADD:
...
extern void dump_loops(WN* wn, FILE* fp, INT spaces, INT increment)
{
  ...
  case OPC_DO_LOOP: { 
	...
	const char *name = WB_Whirl_Symbol(wn);
	if (Prompf_Id_Map != WN_MAP_UNDEFINED) { 
	  fprintf(fp, "[%d] 0x%lx DOLOOP (%d) <%d> %s\n", loop_count, wn,  
	    (INT) WN_linenum(wn), WN_MAP32_Get(Prompf_Id_Map, wn), name);
	} else { 
	  fprintf(fp, "[%d] 0x%lx DOLOOP (%d) %s\n", loop_count, wn,  
	    (INT) WN_linenum(wn), name);
	} 
      } else {  
	const char *name = WB_Whirl_Symbol(wn); 
	if (Prompf_Info != NULL) {
	  fprintf(fp, "[%d] 0x%lx DOLOOP %d (%d) <%d> %s\n", loop_count, wn,  
  ...
  case OPC_FUNC_ENTRY: 
    dump_spaces(fp, spaces);
    if (Prompf_Info != NULL) { 
      fprintf(fp, "[%d] 0x%lx FUNC_ENTRY <%d> %s\n", loop_count, wn, 
	WN_MAP32_Get(Prompf_Id_Map, wn), WB_Whirl_Symbol(wn));
    } else { 
      fprintf(fp, "[%d] 0x%lx FUNC_ENTRY %s\n", loop_count, wn, 
	WB_Whirl_Symbol(wn));
    } 
    if (loop_count < MAX_SAVED_NODES) 
      carray[loop_count++] = wn; 
  ...
  case OPC_IF:
    if (fancy < 3) { 
      ...
      dump_spaces(fp, spaces); 
      fprintf(fp, "[%d] 0x%lx IF ([%d] 0x%lx) THEN [%d] 0x%lx\n", 
  ...
  case OPC_WHILE_DO:
    dump_spaces(fp, spaces);
    fprintf(fp, "[%d] 0x%lx WHILE_DO_LOOP (%d) \n", loop_count, wn,
      (INT) WN_linenum(wn));
    if (loop_count < MAX_SAVED_NODES)
      carray[loop_count++] = wn;
    dump_loops(WN_while_body(wn), fp, spaces + increment, increment);
  ...
  case OPC_REGION:
    if (fancy >= 3) {
      dump_spaces(fp, spaces);
      if (Prompf_Info != NULL) { 
        fprintf(fp, "[%d] 0x%lx REGION <%d>\n", loop_count, wn, 
          WN_MAP32_Get(Prompf_Id_Map, wn));
      } else { 
        fprintf(fp, "[%d] 0x%lx REGION \n", loop_count, wn);
      } 
      if (loop_count < MAX_SAVED_NODES)
	carray[loop_count++] = wn;
      for (INT i = 0; i < WN_kid_count(wn); i++) 
  ...
  case OPC_DO_LOOP:
    {
	  ...
	  fprintf(fp, " (EXACT %" LLD_FMT ")", iteration_count); 
	  ...
	  upper_bound = dli->Est_Max_Iterations_Index;  
        if (upper_bound != -1LL)   
	  fprintf(fp, " (UPPER_BOUND %" LLD_FMT ")", upper_bound);
        if (dli->Num_Iterations_Profile)
	  fprintf(fp, " (FEEDBACK %" LLD_FMT ")", dli->Est_Num_Iterations);
        else if (iteration_count == -1LL && dli->Est_Num_Iterations != -1LL) 
	  fprintf(fp, " (ESTIMATE %" LLD_FMT ")", dli->Est_Num_Iterations); 
...
static void WBT_FORMULA_EVALUATE()
{
  ...
  for (INT i = 0; i < count; i++) {
    ...
    case 2: 
      ...
      fprintf(stdout, "Value is %" LLD_FMT "\n", vars_int64[i]);   
      break; 
    } 
  }
}
...
static BOOL WBTR_Pre_Loop_Peeling()
{
  ...
  fprintf(stdout, "Peeling %d iterations from the front of loop 0x%lx\n",   
    iter_count, wn_loop);   
...
static BOOL WBTR_Post_Loop_Peeling()
{
  ...
  fprintf(stdout, "Do you want to preserve the loop index [Y]? "); 
  Load_Boolean(&preserve_loop_index, TRUE, TRUE); 
  fprintf(stdout, "Peeling %d iterations from the back of loop 0x%lx\n", 
    iter_count, wn_loop);   
...
static BOOL WBTR_Loop_Reversal()
{
  ...
  if (legality_check) { 
    if (!RV_Is_Legal(wn_loop)) {
      fprintf(stdout, "Reversal of loop 0x%lx is NOT legal.\n", wn_loop); 
      Error_Cleanup(); 
      return FALSE;
    }
    fprintf(stdout, "Reversal of loop 0x%lx is legal.\n", wn_loop); 
  }
  fprintf(stdout, "Reversing loop 0x%lx\n", wn_loop); 
...
static BOOL WBTR_Loop_Permutation()
{
  ...
  if (legality_check) { 
    ...
    if (wn_outer != wn_safe_outer) {
      fprintf(stdout, "Safest outermost loop is 0x%lx.\n", wn_safe_outer); 
      Error_Cleanup(); 
      return FALSE; 
    } 
...
static BOOL WBTR_Loop_Tiling()
{
  ...
  if (legality_check) { 
    ...
    if (wn_loop != wn_safe_loop) {
      fprintf(stdout, "Loop 0x%lx was not safe for tiling\n", wn_loop); 
      Error_Cleanup(); 
      return FALSE; 
    } 
    if (Step_Size(wn_loop) != 1) { 
      fprintf(stdout, "Loop 0x%lx does not have unity step\n", wn_loop); 
      Error_Cleanup(); 
      return FALSE; 
    } 
  } 
