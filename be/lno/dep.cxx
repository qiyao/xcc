...
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/dep.cxx,v $ $Revision: 1.93 $";
...
BOOL
DEPV_ARRAY::Dependence_Carried_By_Loop (INT depth)
{
  INT num_dim = Num_Dim();
  INT num_unused = Num_Unused_Dim();

  if (depth >= (num_unused + num_dim))
    return FALSE;
  
  /* Be conservative for outer loops. */
  if (depth < num_unused)
    return TRUE;
  
  INT depth_level = depth - num_unused;
  
  for (INT i = 0; i < Num_Vec(); i++) {
    DEPV *depv = Depv(i);

    bool outer_pos = false;
    ...
    /* The dependence is not carried by the 'depth' loop
       if there's an outer (+) dependence. Go to the
       next dependence vector. */
    if (outer_pos)
      continue;
    
    /* If the dependence carried by the 'depth' loop
       is not (=), then the loop carries a dependence. */
    DIRECTION dir = DEP_Direction(DEPV_Dep(depv, depth_level));
    if (dir != DIR_EQ)
      return TRUE;
  }
...
INT
DEPV_ARRAY::Loop_Carrying_Dependence() {
  INT num_dim = Num_Dim();
  INT num_unused = Num_Unused_Dim();
  INT result = -1; 
  for (INT i=0; i<Num_Vec(); i++) {
...
void DEPV_LIST::Append(DEPV_LIST *list) 
{
  DEPV_ITER iter(list);
...
INT DEPV_LIST::First_Non_Star() const
{
  MEM_POOL_Push(&LNO_local_pool);

  DEPV_CONST_ITER iter(this);
  DEPV_NODE *node = (DEPV_NODE *) iter.First();
  DEPV *depv_union = DEPV_Copy(&LNO_local_pool, node->Depv, _dv_dim);
  node = (DEPV_NODE *) iter.Next();
  while (!iter.Is_Empty()) { 
    for (INT i=0; i<_dv_dim; i++)  {
      DEP dep1 = DEPV_Dep(depv_union,i);
      DEP dep2 = DEPV_Dep(node->Depv,i);
      DEPV_Dep(depv_union,i) = DEP_UnionDirection(dep1, DEP_Direction(dep2)); 
    }
    node = (DEPV_NODE *) iter.Next();
  }
  for (INT i=0; i<_dv_dim; i++)  {
    if (DEP_Direction(DEPV_Dep(depv_union,i)) != DIR_STAR) {
      MEM_POOL_Pop(&LNO_local_pool);
      return i;
    }
  }
		     ...
		     INT use_num_bad_outer)
{
  ...
  SYMBOL_STACK *symbol_stack = CXX_NEW(SYMBOL_STACK(&LNO_local_pool), 
	&LNO_local_pool); 
  _num_bad_outer_min=INT32_MAX;
  _num_bad_outer_max=0;
  DEPV_LIST *tmp_result = CXX_NEW(DEPV_LIST(result->_dv_dim, 
					    result->_num_unused_dim,
					    &LNO_local_pool),&LNO_local_pool);
  ...
  if (dr == DEP_INDEPENDENT) {
    ...
    SIMD_Msg(AT_MSG_SIMD_ARRAY_ALIAS, ref1, SIMD_Base_Name_Msg(ref1),
             SIMD_Base_Name_Msg(ref2), (INT)Srcpos_To_Line(LWN_Get_Linenum(ref2)));
    tmp_result->Append(CXX_NEW(DEPV_NODE(DEPV_CreateStar(pool,dv_dim)),pool));
    if (debug >= 2) {
...

  {
   ...
   // the results with !='s
   //
   // If we have something like a[n][i] where 'n' varies unpredictably
   //   we have a choice between throwing out the first dimension and 
   //   throwing out some loops (ideally we would use the first dimension
   //   in all diretion vectors that have ='s in the bad loops, but that's
   //   too hard).  We used to throw out the dimension.  Now, we throw
   //   out the dimension, but if it turns out that the bad loops all end
   //   up with *'s anyway, we rerun and throw out the loops by setting
   //   use_num_bad_outer
   INT num_bad_outer;
   if (!use_num_bad_outer) {
    if (a1) {
     _num_bad_outer_min =  a1->Dim(0)->Non_Const_Loops();
     _num_bad_outer_max =  a1->Dim(0)->Non_Const_Loops();
     for (INT i=1; i<num_vec1; i++) {
       _num_bad_outer_min=MIN(_num_bad_outer_min,a1->Dim(i)->Non_Const_Loops());
       _num_bad_outer_max=MAX(_num_bad_outer_max,a1->Dim(i)->Non_Const_Loops());
     }
    } else {
     REGION_ITER iter(&ara_ref1->Image());
     ...
     for (const REGION *node=first; !iter.Is_Empty(); node = iter.Next()) {
       ...
       _num_bad_outer_min=MIN(_num_bad_outer_min,lo->_ac_v->Non_Const_Loops());
       _num_bad_outer_max=MAX(_num_bad_outer_max,lo->_ac_v->Non_Const_Loops());
       CON_PAIR *up = axle->up;
       FmtAssert(up != NULL, ("Compute: axle->hi is NULL"));
       if (up) {
         _num_bad_outer_min = 
             MIN(_num_bad_outer_min,up->_ac_v->Non_Const_Loops());
         _num_bad_outer_max = 
	     MAX(_num_bad_outer_max,up->_ac_v->Non_Const_Loops());
       }
     }
    }

    if (a2) {
     _num_bad_outer_min=MIN(_num_bad_outer_min,a2->Dim(0)->Non_Const_Loops());
     _num_bad_outer_max=MAX(_num_bad_outer_max,a2->Dim(0)->Non_Const_Loops());
     for (INT i=1; i<num_vec2; i++) {
       _num_bad_outer_min=MIN(_num_bad_outer_min,a2->Dim(i)->Non_Const_Loops());
       _num_bad_outer_max=MAX(_num_bad_outer_max,a2->Dim(i)->Non_Const_Loops());
     }
    } else {
     REGION_ITER iter(&ara_ref2->Image());
     const REGION *first = iter.First();
     for (const REGION *node=first; !iter.Is_Empty(); node = iter.Next()) {
       ...
       CON_PAIR *lo = axle->lo;
       _num_bad_outer_min=MIN(_num_bad_outer_min,lo->_ac_v->Non_Const_Loops());
       _num_bad_outer_max=MAX(num_bad_outer,lo->_ac_v->Non_Const_Loops());
       CON_PAIR *up = axle->up;
       if (up) {
         _num_bad_outer_min=MIN(_num_bad_outer_min,up->_ac_v->Non_Const_Loops());
         _num_bad_outer_max=MAX(num_bad_outer,up->_ac_v->Non_Const_Loops());
       }
     }
    }
 
    _num_bad_outer_min -= (common_nest-dv_dim);
    _num_bad_outer_min = MAX(_num_bad_outer_min,0);

    _num_bad_outer_max -= (common_nest-dv_dim);
    _num_bad_outer_max = MAX(_num_bad_outer_max,0);

    num_bad_outer = _num_bad_outer_min;
   } else {
    _num_bad_outer_min = use_num_bad_outer;
    _num_bad_outer_max = use_num_bad_outer;
    num_bad_outer = use_num_bad_outer;
   }

   if (num_bad_outer >= dv_dim) {
     tmp_result->Append(CXX_NEW(DEPV_NODE(
   ...
   if (dr == DEP_INDEPENDENT) {
     ...
     tmp_result->Append(NULL,num_bad_outer);
     goto return_point;
   } else if (dr == DEP_DEPENDENT) {
     tmp_result->Append(CXX_NEW(DEPV_NODE(
	 DEPV_CreateStar(pool,dv_dim)),pool));
   ...
   if (use_bounds) {
     Compute_Dep_Vectors(soe,is_used,map_used,init_distance,tmp_result,
   ...
   // trying differet dependence vectors)
     ...
     if (num_bad_outer == 0) {
       tmp_result->Append(
	 CXX_NEW(DEPV_NODE(DEPV_Copy(_pool,init_distance,tmp_result->_dv_dim)),
			 _pool));
     } else {
       tmp_result->Append(DEPV_Copy(&LNO_local_pool,init_distance,
     	 dv_dim-num_bad_outer),num_bad_outer);
     }
   }
   tmp_result->Normalize_Step(&_step1[common_nest-dv_dim]);
  }

 return_point:  
  if (dr != DEP_INDEPENDENT &&
      !use_num_bad_outer && _num_bad_outer_max && 
      _num_bad_outer_max < dv_dim &&
      _num_bad_outer_max != _num_bad_outer_min) {
    INT first_non_star = tmp_result->First_Non_Star();
    if (first_non_star > _num_bad_outer_min) {
      Compute(result, ref1,ara_ref1, ref2, ara_ref2, common_nest,
		dv_dim, use_bounds, pool, s1, s2, 
		MIN(first_non_star, _num_bad_outer_max));
	...
	mUINT16 common_nest, mUINT16 dv_dim,const DOLOOP_STACK *s1) const
{
  ...
  if (av1->Lin_Symb) {
    ...
    for (INTSYMB_NODE *node=iter.First();!iter.Is_Empty(); node=iter.Next()) {
      ...
      WN *wn1 = Find_First_Ldid_For_Symbol(array1_index,&node->Symbol);
      WN *wn2 = Find_First_Ldid_For_Symbol(array2_index,&node->Symbol);
...
extern WN *Find_First_Ldid_For_Symbol(WN *wn, const SYMBOL *symb)
{
  ...
  for (INT kidno=0; kidno<WN_kid_count(wn); kidno++) {
    WN *tmp =  Find_First_Ldid_For_Symbol(WN_kid(wn,kidno),symb);
    if (tmp) {
      return tmp;
    }
  }
...
static TYPE_ID
Element_Type (TY_IDX ty_idx) {
  if (TY_kind(ty_idx)==KIND_ARRAY) {
    return Element_Type(TY_AR_etype(ty_idx));
  }
...
static POINTS_TO* Points_To_For_Array_Star(ARA_REF *ara_ref1)
{
  ...
  TYPE_ID wtype = Element_Type(TY_pointed(ST_type(st1)));
