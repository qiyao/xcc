...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/ff_utils.cxx,v $ $Revision: 1.122 $";
#endif /* _KEEP_RCS_ID */
...
extern BOOL scalar_rename(WN* ref, HASH_TABLE<WN*,INT>* checked) {
  ...
  for (i=0; can_rename && i<equivalence_class->Elements(); i++) {
	...
	can_rename = FALSE;
      else if (WN_ty(scalar_ref) != MTYPE_To_TY(desc_type))
        can_rename = FALSE;
	...
	// while memory does 
	  ...
	  if ((WN_opcode(LWN_Get_Parent(scalar_ref)) != OPC_DO_LOOP) ||
	    ...
	    cvtl_desc=Mtype_TransferSign(desc,cvtl_desc);
            ...
            WN *cvtl = LWN_CreateCvtl(cvtl_o, Size, WN_kid0(scalar_ref));
