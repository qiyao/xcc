...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/fis_gthr.cxx,v $ $Revision: 1.78 $";
#endif /* _KEEP_RCS_ID */
...

{
  ...
  ST* symbol_st = symbol[0]->_scalar.St();
  INT sz0 = TY_size(ST_type(symbol_st));
...
//		 (void*) (unique_gs_id-total_var+each_var+1));
      ...
      LWN_Copy_Frequency_Tree(WN_do_body(use_loop), store);
        ...
        WN2VINDEX*       stmt_to_vertex) // map stmt to vertex in dpg
{
  ...
  for (i=0; i<cond_scc->Elements(); ++i) {
    ...
    if (LNO_Verbose) 
      fprintf(stdout, "Size estimate is %" LLD_FMT "\n", size_est[ind_new_scc]);

    BOOL has_bad_type = FALSE;
    // Determine exposed scalar uses
  ...
  for (i=1; i<num_loop; ++i){
    ...
    LWN_Copy_Frequency_Tree(then_body, new_store);
    ...
    LWN_Copy_Frequency(old_value, cons_0);
    ...
    LWN_Copy_Frequency_Tree(then_body, new_end);
