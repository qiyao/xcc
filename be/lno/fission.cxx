...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/fission.cxx,v $ $Revision: 1.98 $";
#endif /* _KEEP_RCS_ID */
...
Separate(WN* in_loop,WN* in_stmt, UINT8 level, WN** new_loop, BOOL create_empty_loop)
{
  ...
  *new_loop =LWN_CreateDO (
  ...
  if (Cur_PU_Feedback) {
    Cur_PU_Feedback->FB_duplicate_node(wn1, wn2);
  }
  ...
  BOOL rename_loop_var)
{
  ...
  for (i=total_loops-1; i>0; i--) {
    ...
    for (INT j=fission_level-2; j>=0; j--) {
      ...
      outer_loop_info->Has_Non_Intrinsic_Calls = new_loop_info->Has_Non_Intrinsic_Calls;
...
DYN_ARRAY<FF_STMT_LIST>& loop, MEM_POOL* pool)
{
  ...
  for (stmt = WN_first(WN_do_body(in_loop));
    ...
    WN_MAP_Set(dep_graph_map, stmt, (void *)(INTPTR)v);
