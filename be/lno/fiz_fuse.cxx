...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/fiz_fuse.cxx,v $ $Revision: 1.78 $";
#endif /* _KEEP_RCS_ID */
  ...
  for (INT i=0; i<total_snl; i++) {
    ...
    if (type==Not_Inner || type==Inner) {
      for (INT j=0; j<depth; j++) {
        ...
        if (old_index) {
          DevWarn("Duplicate found for loop <0x%lx> in SNL info (%d) and (%d), fixed",
            wn, old_index, i);
          if (j==0)
            _snl_info[i]._type=Invalid;
          else {
...
void FIZ_FUSE_INFO::Build(WN* root, BOOL all_loops, BOOL verbose) {
