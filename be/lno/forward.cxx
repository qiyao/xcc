...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/forward.cxx,v $ $Revision: 1.71 $";
#endif /* _KEEP_RCS_ID */
...
// FUNCTION: Returns TRUE if 'wn_use' is nested inside an IF (other than the test)
//    within  'wn_stop', FALSE otherwise.
//-----------------------------------------------------------------------
			    ...
			    WN* wn_stop) 
{
  WN *wn;
  WN *child=NULL;
  for (wn = wn_use; wn != NULL && wn != wn_stop; wn = LWN_Get_Parent(wn)) { 
    if (WN_opcode(wn) == OPC_IF) { 
      if (child != WN_if_test(wn)) {
        return TRUE;
      }
    }
    child = wn;
			  ...
			  LS_IN_LOOP* loop_ls) 
{
  ...
  for (unode = iter.First(); !iter.Is_Empty(); unode = unnode_next) {
    ...
    if (WN_rtype(WN_kid0(wn_orig)) != WN_desc(wn_orig)) {
       WN *use_parent = LWN_Get_Parent(use);
       int i=0; 
       while (WN_kid(use_parent,i) != use) i++;
       WN *new_use = LWN_Int_Type_Conversion(use,WN_desc(wn_orig));
       WN_kid(use_parent,i) = new_use; 
       LWN_Set_Parent(new_use,use_parent);
    }
...
// Is there any statement between use and store
static BOOL Intervening_Store(WN *store, WN *use)
{
	WN *use_statement = LWN_Get_Statement(use);
	BOOL first_inside_if=TRUE;
	while (first_inside_if) {
          if (WN_prev(use_statement)) {
            first_inside_if = FALSE;
	  } else {
            WN *parent = LWN_Get_Parent(use_statement);
	    if (parent && WN_opcode(parent) == OPC_IF) {
                use_statement=parent;
	    } else if (parent && WN_opcode(parent) == OPC_BLOCK &&
			    WN_opcode(LWN_Get_Parent(parent)) == OPC_IF){
		use_statement = LWN_Get_Parent(parent);
	    } else {
              first_inside_if = FALSE;
	    }
	  }
	}
	return WN_prev(use_statement) != store;
}
			  ...
			  LS_IN_LOOP* loop_ls)
{
  ...
  for (unode = iter.First(); !iter.Is_Empty(); unode = iter.Next()) { 
    WN *use = unode->Wn(); 
    intervening_store |= Intervening_Store(wn_orig,use);
    if (WN_operator(use) != OPR_LDID) {
      return FALSE; 
    }
    if (Contains_MP) {
...
static WN *Inside_If_Condition(WN *wn)
{
  WN *prev=wn;
  while (wn && !OPCODE_is_scf(WN_opcode(wn))) {
    prev = wn;
    wn = LWN_Get_Parent(wn);
  }
  if (wn && WN_opcode(wn) == OPC_IF && prev == WN_if_test(wn)) {
    return wn;
  }
  return NULL;
}

static BOOL Is_Loop_Bound(WN *wn) 
{
  WN *tmp = wn;
  while (wn && !OPCODE_is_scf(WN_opcode(wn))) {
    tmp = wn;
  ...
  if (WN_opcode(wn) == OPC_DO_LOOP) {
    if (tmp == WN_start(wn) || tmp == WN_end(wn)) {
      return TRUE;
    }
  }
  return FALSE;
...
// Does the expression contain only operators that can't damamage a loop bound
static BOOL Good_For_Loop_Bound(WN *wn)
{
  for (INT i=0; i<WN_kid_count(wn); i++) {
    if (!Good_For_Loop_Bound(WN_kid(wn,i))) {
      return FALSE;
    }
  }
  switch (WN_operator(wn)) {
...
//   not sure.
//   Also return FALSE if we would otherwise substitute into a loop bound
//   an expression that can not be analyzed
//-----------------------------------------------------------------------
                                 ...
                                 DU_MANAGER* du)
{
  ...
  WN* wn_first = wn_def;
  WN* wn_last;
  WN *tmp = Inside_If_Condition(wn_ldid);
  if (!tmp || !WN_prev(tmp)) {
    wn_last = Find_Sibling_Containing(wn_first, wn_ldid);
  } else { // if wn_ldid is the condition of an IF, that IF can't modify it, so only look
	   // up to just before the IF
    wn_last = Find_Sibling_Containing(wn_first, WN_prev(tmp));
  }
  if (wn_last == NULL) 
    return NULL; 
  ...
  if (Is_Loop_Bound(wn_ldid)) {
    if (!Good_For_Loop_Bound(WN_kid0(wn_def))) {
      return NULL;
    }
  }
  return wn_def;
...
extern BOOL Forward_Substitute_Ldids(WN* wn_exp,
                                     DU_MANAGER* du)
{
  ...
  if (WN_operator(wn_exp) == OPR_LDID) {
    ...
    if (wn_def != NULL) {
      if (LNO_Verbose) { 
	fprintf(stdout, "FS: Forward substituting %s at 0x%lx\n",
	  WB_Whirl_Symbol(wn_def), wn_def);
	fprintf(TFile, "FS: Forward substituting %s at 0x%lx\n",
	  WB_Whirl_Symbol(wn_def), wn_def);
      } 
	...
	if (WN_kid(wn_parent, k) == wn_exp)
      ...
      TYPE_ID desc = WN_desc(wn_def);
      if (desc != Promote_Type(desc) &&
           desc != WN_rtype(wn_copy)) { // store had implicit cvtl
	INT size = MTYPE_bit_size(desc);
	TYPE_ID cvtl_desc=WN_rtype(wn_copy);
	OPCODE cvtl_o = OPCODE_make_op(OPR_CVTL,cvtl_desc,MTYPE_V);
        wn_copy  = LWN_CreateCvtl(cvtl_o,size,wn_copy);
        WN_kid(wn_parent,k)  = wn_copy;
	LWN_Set_Parent(wn_copy,wn_parent);
      }
  ...
  BOOL result=FALSE;
  for (INT i = 0; i < WN_kid_count(wn_exp); i++)
    result |= Forward_Substitute_Ldids(WN_kid(wn_exp, i), du);
  return result;
}
