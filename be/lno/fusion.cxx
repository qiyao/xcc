...
BOOL unrolled, BOOL preserve_loop_index)
{
  ...
  if (unrolled==FALSE) {
    ...
    if (Cur_PU_Feedback) {
      Cur_PU_Feedback->FB_peel_loop(in_loop, new_loop, iter_count, false);
    }
...
extern SX_INFO* Pre_loop_peeling_simd(WN* in_loop, WN *peel,
				      SX_INFO *pinfo, 
				      WN* permloop[], 
				      ...
				      INT peel_count)
{
  ...
  Is_True(WN_operator(peel) == OPR_STID, ("Unexpected peeping type"));
  WN *iter_ldid = WN_LdidPreg(WN_desc(peel), WN_offset(peel));
  Du_Mgr->Add_Def_Use(peel, iter_ldid);

  Is_True(WN_first(WN_do_body(in_loop))!=NULL, ("Peel empty loop"));

  // abort if bound is not 'i<=n'
  // Upper_Bound_Standardize(WN_end(in_loop));
  if (loop_var_is_live_on_exit(in_loop)) {
    Finalize_Index_Variable(in_loop,FALSE);
    scalar_rename(WN_start(in_loop));
  }
...

  {
    ...
    Unrolled_DU_Update(new_iter, 2, Do_Loop_Depth(in_loop));
    if (red_manager)
      red_manager->Unroll_Update(new_iter,2);
    WN* last_stmt=WN_last(new_iter[0]);
    LWN_Insert_Block_Before(new_iter[0], NULL, new_iter[1]);
    DYN_ARRAY<FF_STMT_LIST> loop(&LNO_local_pool);
    loop.Newidx(); loop[0].Init(NULL);
    ...
    Separate_And_Update(in_loop, loop, 1);
    new_loop=WN_next(in_loop);

    if (Cur_PU_Feedback) {
      Cur_PU_Feedback->FB_peel_loop(in_loop, new_loop, peel_count, true);
    }
    ...
    /* update pinfo */
    loop_map = Make_Loop_Mapping(in_loop, new_loop, pool);
    new_pinfo = CXX_NEW(SX_INFO(*pinfo, in_loop, loop_map, pool),
			pool);

    if (permloop) {
      /* update the permloop to point to the new loops */
      for (INT pi = 0; pi < num_loops; pi++) {
	WN* old_inner = permloop[pi];
	WN* new_inner = loop_map->Find(old_inner);
	if (new_inner != NULL) {
	  permloop[pi] = new_inner;
	}
      }
    }
    
    Is_True(Step_Size(new_loop) == 1, ("Wrong step size"));
    WN_kid0(WN_start(new_loop))=
      LWN_CreateExp2(				// m+(iter_ldid)
        OPCODE_make_op(OPR_ADD, index_type, MTYPE_V),
        WN_kid0(WN_start(new_loop)),
	iter_ldid);

    wn=LWN_Copy_Tree(WN_kid0(WN_start(in_loop)),TRUE,LNO_Info_Map);
    if (!Array_Dependence_Graph->
      Add_Deps_To_Copy_Block(WN_kid0(WN_start(in_loop)), wn, FALSE))
      LNO_Erase_Dg_From_Here_In(WN_kid0(WN_start(in_loop)),
        Array_Dependence_Graph);

    LWN_Copy_Def_Use(WN_kid0(WN_start(in_loop)),wn, Du_Mgr);
    /* make a copy of the peel amount */
    WN* iter_copy = LWN_Copy_Tree(iter_ldid, TRUE, LNO_Info_Map);
    LWN_Copy_Def_Use(iter_ldid, iter_copy, Du_Mgr);

    WN* sub_1 = 
      LWN_CreateExp2(OPCODE_make_op(OPR_SUB, index_type, MTYPE_V),
		     iter_copy, LWN_Make_Icon(index_type, 1));

    WN_kid1(WN_end(in_loop))=
      LWN_CreateExp2(				// min(lb+(iter_ldid-1),ub)
        OPCODE_make_op(OPR_MIN, index_type, MTYPE_V),
        LWN_CreateExp2(
          OPCODE_make_op(OPR_ADD, index_type, MTYPE_V), wn,  sub_1),
        WN_kid1(WN_end(in_loop)));
    ...
    LNO_Build_Access(new_loop, loop_stack, &LNO_default_pool);
    LNO_Build_Do_Access(in_loop, loop_stack);
    LNO_Build_Do_Access(new_loop, loop_stack);

    if (!adg->Build_Region(in_loop,new_loop,loop_stack,TRUE,TRUE)) {
      DevWarn("Array dependence graph overflowed in Pre_loop_peeling()");
      LNO_Erase_Dg_From_Here_In(LWN_Get_Parent(new_loop), adg);  
    } 

  }
  MEM_POOL_Pop(&LNO_local_pool);

  SRCPOS srcpos=WN_Get_Linenum(new_loop);
  if (LNO_Verbose)
    pre_peeling_verbose_info(srcpos, 0);
  if (LNO_Analysis)
    pre_peeling_analysis_info(srcpos, 0);
  if (LNO_Tlog)
    pre_peeling_tlog_info(new_loop, 0);
...
*/
  ...
  for (i=0; i<fusion_level; i++) {
    ...
    LWN_Copy_Frequency_Tree(start2, new_start2);
    ...
    LWN_Copy_Frequency_Tree(ldid_wn, wn);
    ...
    LWN_Copy_Frequency_Tree(new_start2, WN_kid0(new_start2));
    ...
    if (kid0_symbol==new_loop_var_symbol[i]) {
      WN_kid1(old_end2)=
        ...
        LWN_Make_Icon(Promote_Type(index_type), offset[i]));
      LWN_Copy_Frequency_Tree(old_end2, WN_kid1(old_end2));
      ...
      WN_kid0(old_end2)=
        ...
        WN_kid0(old_end2),
        LWN_Make_Icon(Promote_Type(index_type), offset[i]));
      LWN_Copy_Frequency_Tree(old_end2, WN_kid0(old_end2));
  ...
  BOOL has_non_intrinsic_calls =
    loop_info1->Has_Non_Intrinsic_Calls || loop_info2->Has_Non_Intrinsic_Calls;
  ...
  if (!inner_loop_removed) {
    ...
    loop_info1->Has_Non_Intrinsic_Calls = has_non_intrinsic_calls;
