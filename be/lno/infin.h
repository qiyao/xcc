...
#ifdef _KEEP_RCS_ID
static char *infin_rcs_id = infin_INCLUDED "$Revision: 1.10 $";
#endif /* _KEEP_RCS_ID */
...
class INT32_INFIN {
  INT32 _val;
  enum {LNO_INFINITE = 2147483647,LNO_NEGINFINITE = -2147483647-1};
public:

  INT32_INFIN(const INT32 i) { _val = i; }
  INT32_INFIN() { _val = 0; }
  INT32_INFIN(const INT32_INFIN& i) { _val = i._val; }
  BOOL Is_Infinite() const { return(_val == LNO_INFINITE); }
  void Set_Infinite() { _val = LNO_INFINITE; }
  BOOL Is_Neg_Infinite() const { return(_val == LNO_NEGINFINITE); }
  void Set_Neg_Infinite() { _val = LNO_NEGINFINITE; }
...
inline INT32_INFIN operator +(const INT32_INFIN a, const INT32_INFIN b) 
{
  ...
  if (a.Is_Infinite()  || b.Is_Infinite()) {
    return INT32_INFIN::LNO_INFINITE; 
  } else if (a.Is_Neg_Infinite()  || b.Is_Neg_Infinite()) {
    return INT32_INFIN::LNO_NEGINFINITE; 
  } else {
...
inline INT32_INFIN operator *(const INT32_INFIN a, const INT32_INFIN b) 
{
  ...
  if (prod >= (INT64) INT32_INFIN::LNO_INFINITE) {
    return INT32_INFIN::LNO_INFINITE;
  } else if (prod <= (INT64) INT32_INFIN::LNO_NEGINFINITE) {
    return INT32_INFIN::LNO_NEGINFINITE;
  } else {
    return((INT32) prod);
  }
