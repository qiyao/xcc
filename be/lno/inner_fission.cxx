...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/inner_fission.cxx,v $ $Revision: 1.107 $";
#endif /* _KEEP_RCS_ID */
...
/* sun wants its long long constants explicitly declared to be so:
   (9223372036854775807LL) instead of (9223372036854775807), and because
   of the elf.h and dwarf mess we aren't picking up sun's limits.h
*/
...
static void reg_used_if_merged(
  ...
  // hash table which maps a statement in one of the two sccs to 1 
  WN2INT *stmt_to_loop=
  ...
  WN *body = WN_do_body(parent_loop);
  ...
  for (FF_STMT_NODE* ref_node=r_iter.First(); !r_iter.Is_Empty();
      ...
      if (stmt_to_loop->Find(stmt0)) {
        WN* wn_eq_loop = NULL; 
          STACK<WN*>* equivalence_class=
             Scalar_Equivalence_Class(ref, Du_Mgr, &INNER_FISSION_default_pool,
               TRUE, &wn_eq_loop);
        BOOL need_expansion=FALSE;
        for (INT i=0; i<equivalence_class->Elements() && !need_expansion; i++) {
          WN* ref1=equivalence_class->Bottom_nth(i);
          WN* stmt1=Find_Stmt_Under(ref1,body);
	  if (!stmt_to_loop->Find(stmt1)) {
            need_expansion=TRUE;
          }
        }
        if (need_expansion) {
          if (!se_needed->Find(ref)) {
	    se_needed->Enter(ref,1);
          }
          for (INT i=0; i<equivalence_class->Elements(); i++) {
              WN* ref1=equivalence_class->Bottom_nth(i);
              if (!se_needed->Find(ref1)) {
	        se_needed->Enter(ref1,1);
              }
          }
        }
      }
  ...
  double orig_cycles;
  merge_reg_model.Evaluate( parent_loop,se_needed,NULL,&orig_cycles,regs);
}
	...
	)
{
  ...
  LNO_REGCLASS_INFO *regclass_info = 
	CXX_NEW(LNO_REGCLASS_INFO(&INNER_FISSION_default_pool,NULL),
			&INNER_FISSION_default_pool);
  LNO_REGS *regs = CXX_NEW(LNO_REGS(regclass_info,&INNER_FISSION_default_pool),
		  &INNER_FISSION_default_pool);
  ...
  while (tail<head) {
    ...
    reg_model[seed_scc_id].Calculate_Register_Usage(loop,regs);
    BOOL can_allocate=true;
    for (LNO_REGS_IDX idx = LNO_REGS_IDX_UNDEFINED; idx<regs->Regs_Count(); idx++) {
      if (regs->Count_Of_Idx(idx) > regs->Limit_Of_Idx(idx)) {
        can_allocate = false;
      }
    }
    if (can_allocate) {
      ...
      do {
        ...
        for (j=tail; j<head; j++) {
          ...
          reg_used_if_merged(
              &scc[seed_scc_id], &scc[scc_queue[j]], loop,regs,expandable_ref_list);
	  BOOL can_allocate=true;
          for (LNO_REGS_IDX idx = LNO_REGS_IDX_UNDEFINED; idx<regs->Regs_Count(); idx++) {
            if (regs->Count_Of_Idx(idx) > regs->Limit_Of_Idx(idx)) {
              can_allocate = false;
          ...
          if (can_allocate) {
            ...
            if (LNO_Test_Dump)
              printf(" but uses too many registers.\n");
            else;
        }
  ...
  if (LNO_Run_P3==TWO_PASS) {
    ...
    for (INT j=seed_scc->Lastidx()-1; j>=0; j--) {
      ...
      reg_used_if_merged( &scc[scc1], &scc[scc2], loop, regs,expandable_ref_list);
      BOOL can_allocate=true;
      for (LNO_REGS_IDX idx = LNO_REGS_IDX_UNDEFINED; idx<regs->Regs_Count(); idx++) {
        if (regs->Count_Of_Idx(idx) > regs->Limit_Of_Idx(idx)) {
          can_allocate = false;
	...
	FF_STMT_LIST& expandable_ref_list)
{
  ...
  for (i=0; i<total_loops; i++) {
    ...
    double cycles;
    UINT seed_scc=(*new_loops)[i];
    reg_model[seed_scc].Evaluate(loop,se_needed,NULL,&cycles,regs);
        ...
        DYN_ARRAY<UINT>* line_number)
{
  ...
  for (i=0; i<total_loops; i++) {
    ...
    reg_model[seed_scc].Calculate_Register_Usage(loop,regs);
    BOOL can_allocate=true;
    for (LNO_REGS_IDX idx = LNO_REGS_IDX_UNDEFINED; idx<regs->Regs_Count(); idx++) {
      if (regs->Count_Of_Idx(idx) > regs->Limit_Of_Idx(idx)) {
        can_allocate = false;
    ...
    if (LNO_Verbose) {
      ...
      sprintf(message+strlen(message),
             " has %d stmts and can_allocate is %d \n", 
		  total_stmt, can_allocate);
      inner_fission_verbose_info(srcpos, message);
    }
    if (LNO_Tlog) {
      char message[256];
      sprintf(message, "%d (%d) ", total_stmt, can_allocate);
  ...
  if (LNO_Analysis) {
    ...
    if (fission_ok) {
      fprintf(LNO_Analysis, "( LNO_Inner_Loop_Fission_Success (%d ) %d ",
      ...
      fprintf(LNO_Analysis, "( LNO_Inner_Loop_Fission_Failure (%s %d) ",
...
BOOL known_to_have_register_allocation_problem)
{
  ...
  orig_reg_model->Evaluate( innerloop,se_needed,NULL,&orig_cycles,regs);
  if (LNO_Verbose)
    printf("Before inner_fission: %f cycles", orig_cycles);
  ...
  if (!known_to_have_register_allocation_problem) {
    ...
    test_reg_model->Calculate_Register_Usage(innerloop, regs);
    if (LNO_Verbose) {
      char message[256];
      sprintf(message, "Model estimates ");
      for (LNO_REGS_IDX idx = LNO_REGS_IDX_UNDEFINED; idx<regs->Regs_Count(); idx++) {
        if (regs->Count_Of_Idx(idx) > 0) {
          sprintf(message, "(%d/%d) ",regs->Count_Of_Idx(idx), regs->Limit_Of_Idx(idx));
        }
      }
      inner_fission_verbose_info(srcpos, message);
    }
  ...
  if (LNO_Verbose) {
    ...
    for (LNO_REGS_IDX idx = LNO_REGS_IDX_UNDEFINED; idx<regs->Regs_Count(); idx++) {
      if (regs->Count_Of_Idx(idx) > 0) {
        printf(" (%d/%d) ",regs->Count_Of_Idx(idx), regs->Limit_Of_Idx(idx));
      }
    }
    inner_fission_verbose_info(srcpos, message);
  }
  ...
  if (LNO_Test_Dump)
    for (i=1; i<=total_scc; i++) {
      ...
      INT fpr, intr,tlb;
      reg_model[i].Calculate_Register_Usage(innerloop,regs);
      printf("has %d stmts and use the following regs ", j);
      for (LNO_REGS_IDX idx = LNO_REGS_IDX_UNDEFINED; idx<regs->Regs_Count(); idx++) {
        if (regs->Count_Of_Idx(idx) > 0) {
          printf(" (%d/%d) ",regs->Count_Of_Idx(idx), regs->Limit_Of_Idx(idx));
        }
  ...
  if (((DO_LOOP_INFO*)Get_Do_Loop_Info(innerloop))->Aggressive_Inner_Fission) {
    ...
    new_loops=merge_scc_to_form_new_loop(
        ...
        Expandable_Scalar_Set,innerloop, ac_g,expandable_ref_list);
  }
    ...
    if (LNO_Verbose) {
      ...
      for (i=0; i<new_loops->Elements(); i++) {
        ...
        tmp_reg_model->Evaluate(
	      loop_wn,se_needed,NULL,&tmp_cycles,regs);
        printf("(%f cycles , regs) ", tmp_cycles);
        for (LNO_REGS_IDX idx = LNO_REGS_IDX_UNDEFINED; idx<regs->Regs_Count(); idx++) {
          if (regs->Count_Of_Idx(idx) > 0) {
            printf(" (%d/%d) ",regs->Count_Of_Idx(idx), regs->Limit_Of_Idx(idx));
          }
