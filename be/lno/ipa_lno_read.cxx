...
static void IPA_LNO_Read_Sections(IPA_LNO_READ_FILE* IPA_LNO_File)
{
  ...
  IPA_LNO_SUMMARY_PROCEDURE* procedure_array
    = (IPA_LNO_SUMMARY_PROCEDURE*) IPA_LNO_File->Section_Address(IPA_PROCEDURE); 
  INT procedure_size = IPA_LNO_File->Section_Size(IPA_PROCEDURE);
...
//-----------------------------------------------------------------------
// NAME: Scalars_In_Expr_Ok
// FUNCTION: Return TRUE if all of the nodes with symbols in the tree 
//   rooted at 'wn_expr' have symbols which are of KIND_SCALAR.  It
//   skips through OPR_ILOAD nodes. Return FALSE otherwise. 
//-----------------------------------------------------------------------

extern BOOL
Scalars_In_Expr_Ok(WN* wn_expr) {
  if (WN_operator(wn_expr)==OPR_ILOAD) {
    return TRUE;
  }
  ...
  for (INT i = 0; i < WN_kid_count(wn_expr); i++) 
    if (!Scalars_In_Expr_Ok(WN_kid(wn_expr, i)))
      return FALSE; 
  return TRUE; 
} 
...
//   rather than arrays (except OPR_ILOADs which we skip over (dp))
//-----------------------------------------------------------------------
...
{ 
  ...
  if (formal_number < 0 || formal_number >= WN_kid_count(wn_call)) { 
    if (Get_Trace(TP_LNOPT2, TT_CALL_INFO)) 
      fprintf(stdout, 
	"SHAPE: Formal/Actual counts do not match for call 0x%lx\n",
	wn_call);
    return FALSE; 
  } 
  WN* wn_parm = WN_kid(wn_call, formal_number);
    ...
    if (WN_operator(wn_def) == OPR_STID) {
	  ...
	  fprintf(stdout, "DU CLIP: From %s to %s\n", 
        ...
        Update_Loop_Stmt(wn_call);
	       ...
	       WN_operator(wn_def)==OPR_INTRINSIC_CALL) {
    ...
    if (WN_operator(wn_use) == OPR_LDID) {
	  ...
	  fprintf(stdout, "DU CLIP: From %s to %s\n", 
        ...
        Update_Loop_Stmt(wn_use);
	       ...
	       WN_operator(wn_use) == OPR_INTRINSIC_CALL) {
...
//   FALSE otherwise. 
// We now allow reading of state to be treated as pure, 
// since nothing lno handles can 
// write state, this is always fine.
//-----------------------------------------------------------------------

static BOOL Treat_As_Pure(WN* wn_call)
{
  if (WN_operator(wn_call) == OPR_INTRINSIC_CALL) {
    INTRINSIC intr = WN_intrinsic(wn_call);
    if (INTRN_is_tie_intrinsic(intr)) {
      TIE_MACRO_p tie = tie_info->tie_macro(Intrinsic_To_Tie_Macro_Id(intr));
      if (!tie->reads_memory() && !tie->writes_memory() && !tie->has_side_effect()) {
	return TRUE;
      }
    }
    return FALSE;
...
// Is this a TIE intrinsic that doesn't touch memory
static BOOL No_Memory_TIE(WN* wn_call)
{
  if (WN_operator(wn_call) == OPR_INTRINSIC_CALL) {
    INTRINSIC intr = WN_intrinsic(wn_call);
    if (INTRN_is_tie_intrinsic(intr)) {
      TIE_MACRO_p tie = tie_info->tie_macro(Intrinsic_To_Tie_Macro_Id(intr));
      if (!tie->reads_memory() && !tie->writes_memory()) {
	return TRUE;
      }
    }
    return FALSE;
...
// FUNCTION: Annotate the call 'wn_call' with information about scalars, 
//   given that either it has no side effects or it doesn't touch memory
//-----------------------------------------------------------------------

static BOOL NSE_Annotate_Scalar_Call(WN* wn_call)
{
  ARA_LOOP_INFO *ali = NULL;
  if (!Treat_As_Pure(wn_call) && !No_Memory_TIE(wn_call))  { 
    return FALSE; 
  } 
  ali = CXX_NEW(ARA_LOOP_INFO(wn_call, NULL, TRUE), &ARA_memory_pool);
  for (INT i = 0; i < WN_kid_count(wn_call); i++) { 
    WN* wn_parm = WN_kid(wn_call, i);
    if (WN_operator(wn_parm) != OPR_PARM ||
	!WN_Parm_By_Value(wn_parm))
      return FALSE; 
    WN* wn_arg = WN_kid0(wn_parm);
    OPERATOR oper = WN_operator(wn_arg);
    switch (oper) { 
#ifdef __TIE_CALLS_ONLY__
    case OPR_ARRAY:
      // bail out if an address is passed down to a call
      return FALSE;
#endif // __TIE_CALLS_ONLY__
    case OPR_LDID:
    case OPR_LDA: {  
      ...
      // bail out if an address is passed down to a call
      TY_IDX ty_idx = ST_type(st);
      if ((oper == OPR_LDID && TY_kind(ty_idx)==KIND_POINTER) ||
	  oper == OPR_LDA) {
	return FALSE;
      }
#endif // __TIE_CALLS_ONLY
      if (!Is_Kind_Array(st)) { 
	SYMBOL sym_scalar(st, (WN_OFFSET) 0, TY_mtype(ST_type(st)));
	WN* wn_single = Single_Definition_Temp(wn_arg); 
	if (wn_single != NULL && Scalars_In_Expr_Ok(wn_single))
	  Add_Scalars_In_Expr(wn_single, &ali->SCALAR_USE());
...
static bool
Add_Arrays_In_Expr(WN *wn_arg, ARA_LOOP_INFO *ali, WN *wn_call) {
  ...
  case OPR_LDA: { 
	...
	TY_pointed(ST_type(st)) : ST_type(st);
      SYMBOL sym_array(st, (WN_OFFSET) 0, TY_mtype(tid_base));
      PROJECTED_REGION* pr_array =
	CXX_NEW(PROJECTED_REGION(MESSY_REGION,
				 0, TY_AR_ndims(tid_base),
				 &ARA_memory_pool),
		&ARA_memory_pool);
  ...
  case OPR_ILOAD: { 
    ...
    PROJECTED_REGION* pr_array =
      Projected_Region_From_Access_Array(aa, &ARA_memory_pool, NULL);
    if (pr_array == NULL) { 
      return false; 
    } 
    pr_array->Fill_Out();
  ...
  default:
    for (INT kid_idx=0; kid_idx<WN_kid_count(wn_arg); kid_idx++) {
      if (!Add_Arrays_In_Expr(WN_kid(wn_arg,kid_idx),ali,wn_call)) {
	return false;
      }
    }
    break;
...
static BOOL first_tie_var=1;
static ST *fake_global;

//-----------------------------------------------------------------------
// NAME: NSE_Annotate_Array_Call
// FUNCTION: Annotate the call 'wn_call' with information about arrays,
//   given that it has no side effects or only writes TIE state.
//-----------------------------------------------------------------------
...
{ 
  ...
  if (!Treat_As_Pure(wn_call)) {
    // this function writes state, put in a read/write to a special global var 
    // to represent all state
    if (first_tie_var) {
      first_tie_var = FALSE;
      fake_global = New_ST (GLOBAL_SYMTAB);
      ST_Init (fake_global, Save_Str ( "__tie_state" ), CLASS_VAR,
             SCLASS_EXTERN, EXPORT_PREEMPTIBLE, Be_Type_Tbl (MTYPE_I4));
    }
    SYMBOL sym_array(fake_global, (WN_OFFSET) 0, TY_mtype(ST_type(fake_global)));
    REGION* rg_array = CXX_NEW(REGION(0, 0), &LNO_default_pool);
    rg_array->_type = ARA_TOO_MESSY;
    ARA_REF* ref_new = CXX_NEW(ARA_REF(&sym_array, rg_array, ali, TRUE), &LNO_default_pool);
    ali->Add_May_Def(ref_new);
    ali->Add_Use(ref_new);
    return TRUE;
  }
  for (INT i = 0; i < WN_kid_count(wn_call); i++) { 
    ...
    if (WN_Parm_By_Value(wn_parm)) {
      // there is no need to add any arrays to the ARA info
      // if the parameter is by value for dependence analysis purposes
      continue;
    }
    if (!Add_Arrays_In_Expr(WN_kid0(wn_parm),ali,wn_call)) {
      return FALSE;
    }
  } 
  if (Get_Trace(TP_LNOPT2, TT_CALL_INFO)) {
					  ...
					  STACK<ST_IDX>* st_pstatic)
{
  if (WN_operator(wn_tree)==OPR_CALL ||
      WN_operator(wn_tree)==OPR_INTRINSIC_CALL)
  ...
  for (; itr != NULL; itr = LWN_WALK_TreeNext(itr)) {
	...
	dli->Has_Unsummarized_Calls = TRUE;
        if (WN_operator(wn) == OPR_INTRINSIC_CALL) {
          ...
          if (INTRN_is_tie_intrinsic(intr)) {
            SIMD_Msg(AT_MSG_SIMD_BAD_TIE_OP, wn,
                     tie_info->macro_demangled_name(Intrinsic_To_Tie_Macro_Id(intr)));
          }
        }
      }
