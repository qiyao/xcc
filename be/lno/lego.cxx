...
extern DLL_SHARED BOOL Run_Dsm_Check;
...
extern void Lego_PU_Init (void) {
  ...
  da_count = 0;
  char name[64];
  sprintf (name, "_da_count_PUnum_%" LLD_FMT "", pu_count);
...
extern void Lego_PU_Fini (void) {
   ...
   * in this ST.
   */
  Irb_Init_Integer (8, da_count, 1, pu_ino, 0);
  if (Verbose_Lego) {
...
extern void Lego_File_Fini (void) {
  ...
  Irb_Init_Integer (8, DSM_VERSION_NUMBER, 1, version_number_ino, 0);
  Irb_Init_Integer (8, pu_count, 1, file_ino, 0);
  if (Verbose_Lego)
    printf ("Done Lego_File_Fini: File had %" LLD_FMT " PUs\n", pu_count);
