...
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/lno_split.cxx,v $ $Revision: 1.40 $";
...
// Too many is at least registers -2
// All stores in the region must be descendents of a do loop
//
// Do nothing if we have less than 8 registers (in that case model will
...
static BOOL Need_To_Split(WN *region)
{
  ...
  reg_model->Calculate_Register_Usage(Find_Inner(region),regs);
  ...
  for (LNO_REGS_IDX idx = LNO_REGS_IDX_UNDEFINED; idx<regs->Regs_Count(); idx++) {
    if (regs->Count_Of_Idx(idx) >= 6) { // only split for regfiles of size at least 8
      if (regs->Count_Of_Idx(idx) >= (regs->Limit_Of_Idx(idx)-2)) {
