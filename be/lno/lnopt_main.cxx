 ...
 * Module: lnopt_main.c
 * $Revision: 1.425 $
 * $Date: 2000/05/19 20:22:37 $
 * $Author: murthy $
 * $Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/lnopt_main.cxx,v $
 *
 * Revision history:
...
extern DLL_SHARED BOOL Run_lno;
			...
			ALIAS_MANAGER *alias_mgr)
{
  ...
  SIMD_Msg(AT_MSG_SIMD_PROC_BEGIN, func_nd, Cur_PU_Name);
  
  // early exit test
  if ( !Du_Built(du_mgr) ) {
    SIMD_Msg(AT_MSG_SIMD_NO_LOOP, func_nd);
  ...
  FmtAssert(Safe_Spec_Map != -1,("Ran out of mappings in Lnoptimizer"));
...

  {
    ...
    if ((LNO_Call_info || Run_autopar) && LNO_enabled) {
      // annotate calls
    ...
    /* Convert ifs to min/max. */
    if (!LNO_skip && LNO_enabled) {
      If_MinMax(func_nd);
    }
    ...
    if ((LNO_Opt == 0 || LNO_skip) && !(Run_autopar && LNO_Run_AP > 0)) {
      ...
      Stop_Timer ( T_LNOBuildDep_CU );
      if (!graph_is_ok) {
	SIMD_Msg(AT_MSG_SIMD_BAD_DGRAPH, func_nd);
        goto return_point;
      }
    }
    ...
    if (Get_Trace(TP_LNOPT,TT_LNO_DEP2) || 
      ...
      fprintf(TFile, "%sLNO dependence graph (after transformation)\n%s",
                      DBar, DBar);
      Array_Dependence_Graph->Print_LNO(TFile);
    }
    ...
    Find_Mul16(func_nd,du_mgr,Alias_Mgr); 
    looked_for_mul16 = true;
  
    if (Get_Trace(TP_LNOPT,TT_LNO_DEP2) || 
        Get_Trace(TP_LNOPT,TT_LNO_DEP)) {
      fprintf(TFile, "%sLNO dep graph for CG, after LNO\n%s", DBar, DBar);
      Current_Dep_Graph->Print_LNO(TFile);
      fprintf(TFile, DBar);
    }
...
void DO_LOOP_INFO::Print(FILE *fp, INT indentation)
{
  ...
  if (Has_Non_Intrinsic_Calls) fprintf(fp,"%sIt has non-intrinsic calls \n", buf);
  ...
  if (SIMD_If_Convert_Directive) fprintf(fp,"%sSIMD_If_Convert_Directive is 1 \n",buf);
  if (Work_Estimate != 0)
  ...
  if (Step) {
    ...
    fprintf(fp, "<null>\n");
  fprintf(fp,"%sWe estimate this loop has %" LLD_FMT " iterations.  %s\n",
	  buf, Est_Num_Iterations, Num_Iterations_Symbolic?"<symbolic>":"");
  if (Est_Max_Iterations_Index != -1) 
    fprintf(fp,"%sThis loop has at most %" LLD_FMT " iterations due to array bounds.\n",
                      ...
                      BOOL do_inner_fission)
{
  ...
  if (do_fiz_fuse) {
    ...
    LNO_Check_Graph(Array_Dependence_Graph);    
    if (Cur_PU_Feedback)
      Cur_PU_Feedback->Verify("After fiz_fuse");
    if (LNO_Verbose) {
...
//  Hoist_Outer_Invar(func_nd);
  ...
  if (do_phase25 && LNO_Fission!=0 && LNO_Gather_Scatter!=0) {
    ...
    if (!Get_Trace(TP_LNOPT, TT_LNO_NORENAME) && (Simd_Target == SIMD_TARG_UNKNOWN)) {
...
DO_LOOP_INFO::DO_LOOP_INFO(MEM_POOL *pool, ACCESS_ARRAY *lb, ACCESS_ARRAY *ub,
			   ACCESS_VECTOR *step, BOOL has_calls, BOOL has_non_intrinsic_calls,
			   BOOL has_unsummarized_calls,
    ...
    Step = step;
    Has_Calls = has_calls;
    Has_Non_Intrinsic_Calls = has_non_intrinsic_calls;
    ...
    SIMD_If_Convert_Directive = FALSE;
    No_Fission = FALSE;
    No_Fusion = FALSE; 
...
DO_LOOP_INFO::DO_LOOP_INFO(DO_LOOP_INFO *dli, MEM_POOL *pool) {
    ...
    Has_Non_Intrinsic_Calls = dli->Has_Non_Intrinsic_Calls;
    ...
    SIMD_If_Convert_Directive = dli->SIMD_If_Convert_Directive;
...
{ 
  ...
  if (opr == OPR_ILOAD || opr == OPR_ISTORE) { 
	...
	error_count++;
	fprintf(stdout, "0x%lx missing vertex\n", wn_tree);
      } 
    } 
  } 
...
{  
  ...
  for (; itr != NULL; itr = LWN_WALK_TreeNext(itr)) {
    ...
    if (wnn == NULL) { 
      error_count++; 
      fprintf(stdout, "0x%lx bad loop stmt 0x%lx\n", wn, wn_loop);
    } 
  }
  return error_count;
}  
