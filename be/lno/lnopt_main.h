...
//
//	mBOOL Has_Non_Intrinsic_Calls
//
//		Any non-intrinsic calls inside this do loop
//
...
//
//	mBOOL SIMD_If_Convert_Directive
//
//		Is there a SIMD if-convert directive on this loop.
//		If true, aggressively if-convert in the loop by
//              performing speculative loads and stores.
//
//	mBOOL No_Fission
//
...
//
//  BOOL Do_Loop_Has_Non_Intrinsic_Calls (WN *wn)
//
//		Does this do loop have any non-intrinsic calls
//
...
// Invariant expression routines and definitions (in oinvar.cxx)

typedef HASH_TABLE<WN *, BIT_VECTOR *> INVAR_TABLE;
extern void Mark_Invar(WN *region, INT num_loops, DOLOOP_STACK *do_stack,
		       INVAR_TABLE *htable, MEM_POOL *pool, bool outer_only,
		       bool all_expr = false);
extern BOOL Sort_Invar_Expressions(WN *wn, INVAR_TABLE *htable);
extern void Hoist_Outer_Invar(WN *wn_inner, INT num_loops, INT outer_reg_tile,
			      BOOL can_tile);
...
inline BOOL Do_Loop_Has_Non_Intrinsic_Calls (WN *wn)
{
  DO_LOOP_INFO *dli = Get_Do_Loop_Info(wn);
  return(dli && dli->Has_Non_Intrinsic_Calls);
}
...
inline BOOL Do_Loop_SIMD_If_Convert_Directive(WN *wn)
{
  DO_LOOP_INFO *dli = Get_Do_Loop_Info(wn);
  return(dli->SIMD_If_Convert_Directive);
}
...
#define  TT_LNO_SIMD_DEBUG              0x00001000
...
#define TT_LNO_PREFETCH        	        0x20000000
#define TT_LNO_PREFETCH_DEBUG		0x40000000
