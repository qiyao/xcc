...
extern bool
Is_Tie_Output_Parm(WN *wn)
{
  WN *p = LWN_Get_Parent(wn);
  WN *gp = (p) ? LWN_Get_Parent(p) : NULL;
  
  if (gp && (WN_operator(gp) == OPR_INTRINSIC_CALL ||
	     WN_operator(gp) == OPR_INTRINSIC_OP)) {
    INTRINSIC intrn_id = WN_intrinsic(gp);
    if (INTRN_is_tie_intrinsic(intrn_id)) {
      TIE_MACRO_ID macro_id = Intrinsic_To_Tie_Macro_Id(intrn_id);
      if (macro_id != TIE_INVALID_ID) {
	TIE_MACRO_p tie_macro = tie_info->tie_macro(macro_id);
	for (INT i = 0; i < WN_kid_count(gp); i++) {
	  if (WN_kid(gp, i) == p) {
	    INT proto_index = tie_macro->whirl_to_proto_index(i);
	    return (tie_macro->proto_is_out(proto_index));
	  }
	}
      }
    }
                        ...
                        WN** list, WN* parent, BOOL recursive)
{
  ...
  if (fancy > 3 && parent != LWN_Get_Parent(wn)) {
    fprintf(f, "%%%%%%%% parent=%lx, real parent=%lx %%%%%%%%",
  ...
  if (fancy) {
	...
	break;
      ...
      break;
     case OPR_INTCONST:
      fprintf(f, "%" LLD_FMT "", WN_const_val(wn));
      break;
     case OPR_LDID:
	  ...
	  fprintf(f, " <<incomplete DU defs>>");
          ...
          if (stmt == NULL)
            ...
            if (WN_opcode(stmt)==OPC_DO_LOOP) {
              fprintf(f, "<<loop_stmt=%s(0x%lx)>>",
                    SYMBOL(WN_index(stmt)).Name(), stmt);
            } else {
              fprintf(f, "<<loop_stmt=%%%%(0x%lx)>>", stmt);
            }
        }
      }
      break;
	     ...
	     BOOL recursive)
{
  ...
  if (OPCODE_has_next_prev(WN_opcode(wn))) {
    ...
    if (line == 0)
      DevWarn("Missing line number for wn=0x%lx (opr=%s)",
                    ...
                    WN* alias_wn, WN* ancestor)
{
  ...
  if (op == OPC_BLOCK) {
	...
	// when replacing a short or char variable with a register var
	// insert necessary cvtl for stores (PR11214)
        if (opr == OPR_STID &&
	    ST_class(symnew.St()) == CLASS_PREG &&
	    WN_desc(wn) != ST_mtype(symnew.St())) {
	  if (LWN_Get_Parent(wn) && WN_operator(LWN_Get_Parent(wn)) == OPR_DO_LOOP &&
		WN_step(LWN_Get_Parent(wn)) == wn) {
		// PR14099:  Not allowed to have a cvtl inside a step
	     ;
          } else {
	    TYPE_ID old_desc = WN_desc(wn);
	    TYPE_ID new_desc = ST_mtype(symnew.St());
	    WN_set_desc(wn, new_desc);
	    WN* kid = WN_kid0(wn);
	    kid = LWN_Integer_Casts(kid, old_desc, new_desc);
	    WN_kid0(wn) = kid;
  ...
  if (op == OPC_BLOCK) {
    ...
    if (OPCODE_has_sym(op)) {
      for (INT i = 0; i < count; i++) {
	    ...
	    // insert necessary cvtl for stores (PR11214)
	    if (opr == OPR_STID &&
	      ST_class(snew[i].St()) == CLASS_PREG &&
	      WN_desc(wn) != ST_mtype(snew[i].St())) {
	      TYPE_ID old_desc = WN_desc(wn);
	      TYPE_ID new_desc = ST_mtype(snew[i].St());
	      WN_set_desc(wn, new_desc);
	      WN* kid = WN_kid0(wn);
	      kid = LWN_Integer_Casts(kid, old_desc, new_desc);
	      WN_kid0(wn) = kid;
  ...
  if (opr == OPR_BLOCK) {
    ...
    if (opr == OPR_LDID) {
      ...
      FmtAssert(stok || SYMBOL(wn) != sym,
                ("Writing to <%s,%d> in Add_To_Symbol()", sym.Name()));
    }
  }
...
WN* Replace_Wnexp_With_Exp_Copy(WN* wn, WN* expr, DU_MANAGER* du, 
  BOOL* added_cvt, ARRAY_DIRECTED_GRAPH16 *dep_graph, BOOL delete_wn)
{
  ...
  LWN_Copy_Frequency_Tree(expr, copy);
  if (du != NULL)
					  ...
					  DU_MANAGER* du)
{
  ...
  LWN_Copy_Frequency_Tree(array_store, wn_store);
  if (du != NULL) 
...
{
  FmtAssert(type != MTYPE_M, ("Create_Preg_Symbol: No PREG for MTYPE_M"));
#ifdef _NEW_SYMTAB
  ...
  if (opcode == OPC_BLOCK) {
    ...
    switch (opr) {
	  ...
	  if (opr == OPR_LDID) DevWarn("WARNING NO DEF LIST ");
	    ...
	    DevWarn("WARNING loop_stmt is not a DO_LOOP (0x%lx,ls=0x%lx)",
                    wn, loop_stmt);
...
BOOL Ldid_Is_Power_Of_2(WN *wn, WN *&def_wn)
{
  Is_True(WN_operator(wn) == OPR_LDID, ("Ldid_Is_Power_Of_2: not a LDID"));
  BOOL is_power_of_2 = TRUE;
  BOOL seen_def = FALSE;
  def_wn = NULL;

  DEF_LIST *def_ll = Du_Mgr->Ud_Get_Def(wn);
  if (def_ll && !def_ll->Incomplete()) {
    DEF_LIST_ITER iter(def_ll);
    if (iter.Is_Empty()) {
      is_power_of_2 = FALSE;
    }
    for (const DU_NODE *node=iter.First(); !iter.Is_Empty(); 
	 node = iter.Next()) {
      WN *def = node->Wn();
      if (WN_operator(def) == OPR_STID &&
	  WN_operator(WN_kid0(def)) == OPR_SHL &&
	  WN_operator(WN_kid0(WN_kid0(def))) == OPR_INTCONST &&
	  WN_const_val(WN_kid0(WN_kid0(def))) == 1) {
	// do nothing
	if (!seen_def) {
	  seen_def = TRUE;
	  def_wn = def;
  ...
  if (is_power_of_2 == FALSE) {
    fprintf(stderr, "Ldid_Is_Power_Of_2: FALSE\n");
  } else {
    fprintf(stderr, "Ldid_Is_Power_Of_2: TRUE\n");
  }
#endif
...
extern BOOL Is_Consistent_Condition(ACCESS_VECTOR*, WN*, 
				    ACCESS_VECTOR *e1=NULL,
				    ACCESS_VECTOR *e2=NULL);

static bool
Term_Multiple_Of (WN *term, INT64 denom, bool do_divide)
{
  if (denom == 0)
    return false;
  
  OPERATOR oper = WN_operator(term);
  switch (oper)
    {
    case OPR_MPY:
      if (Term_Multiple_Of(WN_kid0(term), denom, do_divide))
        return true;
      
      if (Term_Multiple_Of(WN_kid1(term), denom, do_divide))
        return true;

      break;
      
    case OPR_INTCONST:
      if ((WN_const_val(term) % denom) == 0)
        {
          if (do_divide)
            WN_const_val(term) /= denom;
...
static bool
Expr_Multiple_Of (WN *expr, INT64 denom, bool do_divide)
{
  bool multiple = true;
  ...
  if (oper == OPR_ADD ||
      ...
      oper == OPR_NEG)
    {
      for (int k = 0; k < WN_kid_count(expr); k++)
        {
          if (!Expr_Multiple_Of(WN_kid(expr, k), denom, do_divide))
            {
              multiple = false;
              break;
            }
    ...
    {
      multiple = Term_Multiple_Of(expr, denom, do_divide);
    }

  Is_True(multiple || !do_divide,
          ("Term is not a multiple of the divisor"));

  return multiple;
}


/* If (nom % denom) == 0, nom /= denom and return true.
   Otherwise, return false. The routine currently works only if
   'denom' is INTCONST. */
static bool
Divide_Multiple (WN *nom, WN *denom)
{
  if (WN_operator(denom) != OPR_INTCONST)
    return false;
  
  /* First, check if the expression is a multiple of 'denom'. */
  if (!Expr_Multiple_Of(nom, WN_const_val(denom), /* do_divide */ false))
    return false;
  
  /* Then, perform the division. */
  Expr_Multiple_Of(nom, WN_const_val(denom), /* do_divide */ true);
  return true;
}


static
BOOL Solve_For_Power_Of_2(WN *wn_top, WN *wn_l, WN *wn_r,
			  WN *wn_ll, WN *wn_lr, const SYMBOL &sym)
{
  /* if wn_top = lr * index < rl - rr 
       and   (1) rr >= 0
       and   (2) 2*rr < lr            
       and   (3) rl % lr = 0 or (rl/lr <= 0) && 0 <= index

       then we can replace wn_top with 
       index < rl/lr                 */
  Is_True(WN_kid0(wn_top) == wn_l && WN_kid1(wn_top) == wn_r &&
	  WN_operator(wn_l) == OPR_MPY && WN_operator(wn_ll) == OPR_LDID &&
	  SYMBOL(wn_ll) == sym && WN_operator(wn_lr) == OPR_LDID,
	  ("Solve_For_Power_of_2: unexpected inputs"));
  
  if (WN_operator(wn_top) != OPR_LT || WN_operator(wn_r) != OPR_SUB) {
    return FALSE;
  }
  
  WN *wn_rl = WN_kid0(wn_r);
  WN *wn_rr = WN_kid1(wn_r);
  
  if (WN_operator(wn_rl) != OPR_LDID || WN_operator(wn_rr) != OPR_LDID) {
    return FALSE;
  }

  MEM_POOL_Push(&LNO_local_pool);
  DOLOOP_STACK stack(&LNO_local_pool);
  Build_Doloop_Stack(wn_top, &stack);
  
  INT nest_depth = stack.Elements();
  if (nest_depth == 0) {
    /* no chance to prove 'll' and 'rr' > 0 */
    MEM_POOL_Pop(&LNO_local_pool);
    return FALSE;
  }
  ...
  ACCESS_VECTOR *tmp1 = CXX_NEW(ACCESS_VECTOR(nest_depth, &LNO_local_pool),
				&LNO_local_pool);
  tmp1->Too_Messy = FALSE;
  tmp1->Const_Offset = -1;

  BOOL rr_is_index = FALSE;
  for (INT i = 0; i < nest_depth; i++) {
    if (sym_rr == SYMBOL(WN_index(stack.Bottom_nth(i)))) {
      rr_is_index = TRUE;
      tmp1->Set_Loop_Coeff(i, 1);
      break;
    }
  }
  if (!rr_is_index) {
    tmp1->Lin_Symb = CXX_NEW(INTSYMB_LIST, &LNO_local_pool);
    INTSYMB_NODE *rr_node = CXX_NEW(INTSYMB_NODE(sym_rr, 1), &LNO_local_pool);
    tmp1->Lin_Symb->Prepend(rr_node);
  }

  // (1) tmp1 contains rr <= -1
  // is it possible rr <= -1?
  if (Is_Consistent_Condition(tmp1, wn_top)) {
#if 0
    fprintf(stderr,"Solve_For_Power_Of_2: rr >= 0 failed\n");
#endif
    MEM_POOL_Pop(&LNO_local_pool);
    return FALSE;
  }
#if 0
  fprintf(stderr,"Solve_For_Power_Of_2: rr >= 0 succeed\n");
#endif

  // (2) 2*rr < lr => 2*rr + 1 <= lr => 2*rr - lr <= -1
  ACCESS_VECTOR *tmp2 = CXX_NEW(ACCESS_VECTOR(nest_depth, &LNO_local_pool),
				&LNO_local_pool);
  tmp2->Too_Messy = FALSE;
  tmp2->Const_Offset = -1;

  BOOL   lr_is_index = FALSE;
  rr_is_index = FALSE;
  
  for (INT i = 0; i < nest_depth; i++) {
    if (sym_lr == SYMBOL(WN_index(stack.Bottom_nth(i)))) {
      lr_is_index = TRUE;
      tmp2->Set_Loop_Coeff(i, -1);
    }
    if (sym_rr == SYMBOL(WN_index(stack.Bottom_nth(i)))) {
      rr_is_index = TRUE;
      tmp2->Set_Loop_Coeff(i, 2);
    }
  }

  if (!lr_is_index) {
    tmp2->Lin_Symb = CXX_NEW(INTSYMB_LIST, &LNO_local_pool);
    INTSYMB_NODE *lr_node = CXX_NEW(INTSYMB_NODE(sym_lr, -1), &LNO_local_pool);
    tmp2->Lin_Symb->Prepend(lr_node);
  }
  if (!rr_is_index) {
    if (tmp2->Lin_Symb == NULL) {
      tmp2->Lin_Symb = CXX_NEW(INTSYMB_LIST, &LNO_local_pool);
    }
    INTSYMB_NODE *rr_node = CXX_NEW(INTSYMB_NODE(sym_rr, 2), &LNO_local_pool);
    tmp2->Lin_Symb->Prepend(rr_node);
  }
  tmp2->Negate_Me();

  if (Is_Consistent_Condition(tmp2, wn_top)) {
#if 0
    fprintf(stderr,"Solve_For_Power_Of_2: simple 2*rr < lr failed");
#endif
    
    // add new constraints to handle dividing by power of 2
    // if there is a symbol y = sym_lr >> const
    // check:
    //  if (Is_Consistent_Condition((1<<const) * y <= sym_lr && 'tmp2',
    //      wn))
    //      return FALSE;
    //  /* else */
    //      return TURE;
    ...
    /* find the shift wn */
    WN *shift_wn = Find_Delinear_Relation_Wn(wn_lr, -1, &sym_lr, tmp2,
					     nest_depth, &stack);
    /* found a y = sym_lr >> shift */
    if (shift_wn) {
#if 0
      fprintf(stderr,"Solve_For_Power_Of_2: Found y = sym_lr >> shift\n");
#endif
      SYMBOL symbol_y(shift_wn);
      INT    shift  = WN_const_val(WN_kid1(WN_kid0(shift_wn)));
      
      //  if (Is_Consistent_Condition(y < 0 && sym_lr < 0, wn)) {
      //      return FALSE;
      //  else 
      //      sym_lr > 0 && y > 0
      ACCESS_VECTOR *tmp6 =CXX_NEW(ACCESS_VECTOR(nest_depth, &LNO_local_pool),
				   &LNO_local_pool);
      tmp6->Too_Messy = FALSE;
      tmp6->Lin_Symb = CXX_NEW(INTSYMB_LIST,&LNO_local_pool);
      INTSYMB_NODE *y_node = 
	CXX_NEW(INTSYMB_NODE(symbol_y, 1), &LNO_local_pool);
      tmp6->Lin_Symb->Prepend(y_node);

      // is it possible that tmp6 < 0 (tmp6 <= -1)
      tmp6->Const_Offset = -1;

      // sym_lr < 0
      ACCESS_VECTOR *tmp7 =CXX_NEW(ACCESS_VECTOR(nest_depth, &LNO_local_pool),
				   &LNO_local_pool);
      tmp7->Too_Messy = FALSE;
      tmp7->Lin_Symb = CXX_NEW(INTSYMB_LIST,&LNO_local_pool);
      INTSYMB_NODE *lr_node = 
	CXX_NEW(INTSYMB_NODE(sym_lr, 1), &LNO_local_pool);
      tmp7->Lin_Symb->Prepend(lr_node);

      // is it possible that tmp7 < 0 (tmp7 <= -1)
      tmp7->Const_Offset = -1;

#if 0
      fprintf(stderr,"\nSolve_For_Power_Of_2: Found y = sym_lr >> shift\n");
      fprintf(stderr,"\nExtra constraint for y\n");
      tmp6->Print(stderr, 1, 1);
      fprintf(stderr,"\nExtra constraint for sym_lr\n");
      tmp7->Print(stderr, 1, 1);
      fprintf(stderr,"\n");
#endif

      if (Is_Consistent_Condition(tmp6, wn_top, tmp7)) {
#if 0
	fprintf(stderr, ">0 test failed\n");
#endif
	MEM_POOL_Pop(&LNO_local_pool);
	return FALSE;
      }
      ...
      //      return FALSE;
      ACCESS_VECTOR *tmp3 = 
	Gen_Shift_Right_Constraint(shift_wn, &sym_lr, nest_depth);
#if 0
      fprintf(stderr, "%d * y <= sym_lr for constant %d\n", 1<<shift, shift);
      tmp3->Print(stderr, 1, 1);
      fprintf(stderr, "\n");
#endif

      // is it possible that tmp3 <= 0:  ((1<<shift) * y - sym_lr) <= 0
      if (Is_Consistent_Condition(tmp2, wn_top, tmp3)) {
#if 0
	fprintf(stderr,"Solve_For_Power_Of_2: additional test 2*rr<lr failed\n");
#endif
	MEM_POOL_Pop(&LNO_local_pool);
	return FALSE;
      }
#if 0
      fprintf(stderr, "Solve_For_Power_Of_2: additional test 2*rr<lr succeed\n");
#endif
    }
  } else {
#if 0
    fprintf(stderr,"Solve_For_Power_Of_2: simple 2*rr<lr succeed\n");
#endif
  }
    
  // (3) rl % lr = 0 or (rl/lr <= 0) && (0 <= index <= rl)
  WN *lr_def = NULL;
  BOOL lr_is_power_of_2 = Ldid_Is_Power_Of_2(wn_lr, lr_def);
  if (!lr_is_power_of_2) {
    MEM_POOL_Pop(&LNO_local_pool);
    return FALSE;
  }
  ...
  WN *rl_def = NULL;
  BOOL rl_is_power_of_2 = Ldid_Is_Power_Of_2(wn_rl, rl_def);
  if (!rl_is_power_of_2) {
    MEM_POOL_Pop(&LNO_local_pool);
    return FALSE;
  }

  /* now they must rl % lr = 0 or rl/lr < 0
     check for index > 0                          */
  WN* wn_loop = LWN_Get_Parent(wn_top);
  Is_True(WN_operator(wn_loop) == OPR_DO_LOOP,
	  ("Solve_For_Power_Of_2: Unexpecting DO_LOOP"));
  WN *wn_start = WN_start(wn_loop);
  WN *wn_init = WN_kid0(wn_start);
  WN *wn_stride = Loop_Step(wn_loop);

  if (!(WN_operator(wn_stride) == OPR_INTCONST &&
	WN_const_val(wn_stride) > 0 &&
	WN_operator(wn_init) == OPR_INTCONST &&
	WN_const_val(wn_init) >= 0)) {
#if 0
    fprintf(stderr, "Solve_For_Power_Of_2: index > 0 failed\n");
#endif
    MEM_POOL_Pop(&LNO_local_pool);
    return FALSE;
  }
...
#if 0
  fprintf(stderr, "Init %d, Stride %d\n", WN_const_val(wn_init), 
	  WN_const_val(wn_stride));
  fprintf(stderr, "Solve_For_Power_Of_2: index > 0 succeed\n");
#endif

  if (lr_def && rl_def) {
    /* both has single definition:
       lr_def = (1<<x);
       ...
       rl_def = (1<<y);
       ...
       and the upbound to index < (1<<(t_y-t_x))
       in order to avoid the DIV
    */
    Is_True(WN_operator(lr_def) == OPR_STID &&
	    WN_operator(WN_kid0(lr_def)) == OPR_SHL &&
	    WN_operator(WN_kid0(WN_kid0(lr_def))) == OPR_INTCONST &&
	    WN_const_val(WN_kid0(WN_kid0(lr_def))) == 1 &&
	    WN_operator(rl_def) == OPR_STID &&
	    WN_operator(WN_kid0(rl_def)) == OPR_SHL &&
	    WN_operator(WN_kid0(WN_kid0(rl_def))) == OPR_INTCONST &&
	    WN_const_val(WN_kid0(WN_kid0(rl_def))) == 1,
	    ("Solve_For_Power_Of_2: rl/lr definition is not power of 2"));
    
    PREG_NUM preg_1 = Create_Preg(MTYPE_I4, "_$shift_1");
    PREG_NUM preg_2 = Create_Preg(MTYPE_I4, "_$shift_2");

    WN *wn_amount_1 = WN_kid1(WN_kid0(lr_def));
    WN *copy_1 = LWN_Copy_Tree(wn_amount_1, TRUE, LNO_Info_Map);
    LWN_Copy_Frequency_Tree(wn_amount_1, copy_1);
    LWN_Copy_Def_Use(wn_amount_1, copy_1, Du_Mgr);
    WN *stid_1 = WN_StidPreg(MTYPE_I4, preg_1, copy_1);
    LWN_Set_Parent(copy_1, stid_1);
    LWN_Insert_Block_After(LWN_Get_Parent(lr_def), lr_def, stid_1);

    WN *wn_amount_2 = WN_kid1(WN_kid0(rl_def));
    WN *copy_2 = LWN_Copy_Tree(wn_amount_2, TRUE, LNO_Info_Map);
    LWN_Copy_Frequency_Tree(wn_amount_2, copy_2);
    LWN_Copy_Def_Use(wn_amount_2, copy_2, Du_Mgr);
    WN *stid_2 = WN_StidPreg(MTYPE_I4, preg_2, copy_2);
    LWN_Set_Parent(copy_2, stid_2);
    LWN_Insert_Block_After(LWN_Get_Parent(rl_def), rl_def, stid_2);

    WN *ldid_1 = WN_LdidPreg(MTYPE_I4, preg_1);
    WN *ldid_2 = WN_LdidPreg(MTYPE_I4, preg_2);
    Du_Mgr->Add_Def_Use(stid_1, ldid_1);
    Du_Mgr->Add_Def_Use(stid_2, ldid_2);

    /* ldid_2 - ldid_1 */
    OPCODE subop = OPCODE_make_op(OPR_SUB, MTYPE_I4, MTYPE_V);
    WN *sub      = LWN_CreateExp2(subop, ldid_2, ldid_1);
    OPCODE shlop = OPCODE_make_op(OPR_SHL, WN_rtype(WN_kid0(rl_def)), MTYPE_V);
    WN *const_1  = WN_CreateIntconst(OPR_INTCONST, MTYPE_I4, MTYPE_V, 1);
    WN *shl      = LWN_CreateExp2(shlop, const_1, sub);

    WN_kid0(wn_top) = wn_ll;
    WN_kid1(wn_top) = shl;
    LWN_Parentize_One_Level(wn_top);
    
    /* remove wn_rl, wn_lr */
    Du_Mgr->Remove_Use_From_System(wn_rl);
    LWN_Delete_Tree(wn_rl);
    Du_Mgr->Remove_Use_From_System(wn_lr);
    LWN_Delete_Tree(wn_lr);
  } else {
    /* everything passes change wn_top to "index < rl/lr */
    OPCODE divop = OPCODE_make_op(OPR_DIV, WN_desc(wn_top), MTYPE_V);
    WN *div = LWN_CreateExp2(divop, wn_rl, wn_lr);
    WN_kid0(wn_top) = wn_ll;
    WN_kid1(wn_top) = div;
    LWN_Parentize_One_Level(wn_top);
  }
  ...
  /* remove wn_rr */
  Du_Mgr->Remove_Use_From_System(wn_rr);
  LWN_Delete_Tree(wn_rr);
  ...
  while (1) {
	...
	if (v < 0) {
	  ...
	  r = WN_CreateExp1(negop, r);
        ...
        /* Try to avoid a divide operation if 'r' is a multiple of 'lr'. */
        if (Divide_Multiple(r, lr))
          {
            WN_Delete(lr);
          }
        else
	...
	WN_Delete(l);
	l = ll;
      } else if (WN_operator(lr) == OPR_LDID &&
		 WN_operator(ll) == OPR_LDID) {
	WN_kid0(wn_top) = l;
	WN_kid1(wn_top) = r;
	LWN_Parentize(wn_top);
	return Solve_For_Power_Of_2(wn_top, l, r, ll, lr, sym);
      } else {
	break;
      }
    }
...
extern BOOL Upper_Bound_Standardize(WN* ub, BOOL ok_to_fail)
{
  ...
  if (opr == OPR_LT) {
    if (MTYPE_is_integral(OPCODE_desc(opc))) {
      ...
      WN_kid1(ub) = ub1;
      LWN_Copy_Frequency_Tree(ub, ub1);
...
static BOOL LNO_Check_Du_Check(HASH_TABLE<WN*,WN*>* ht)
{
  ...
  while (it.Step(&copy, &orig)) {
	...
	rval = FALSE;
	printf("DU check: Bad deflist do: use=0x%lx[0x%lx] do=0x%lx[0x%lx]\n",
	...
	if (ok == FALSE) {
	  rval = FALSE;
	  printf("DU check: Inadequate defs: use=0x%lx[0x%lx] def=0x%lx[0x%lx]\n",
...
//      FmtAssert(OPCODE_is_call(opc) || ul_len, ("Missing use list in copy"));
	...
	if (ok == FALSE) {
	  rval = FALSE;
	  printf("DU check: Inadequate uses: def=0x%lx[0x%lx] use=0x%lx[0x%lx]\n",
		 orig, copy, duorig, du->Wn());
	}
      }
    }
  ...
  for (si=0; si<write_stack->Elements(); si++) {
    ...
    for (DU_NODE* u_node=u_iter.First(); !u_iter.Is_Empty();
      ...
      if (bit!=0)
        ...
        fprintf(fp,"WARNING: %s %d [0x%lx]", 
	  OPERATOR_name(opr), WN_map_id(write), write);
        Dump_WN(write,fp,fancy,2,2,NULL,NULL,NULL,FALSE);
        fprintf(fp,"has a non-matching DU relation with node: %d [0x%lx]\n",
                WN_map_id(use), use);
  ...
  if (pass==0) {
    ...
    if (Du_Mgr->Ud_Get_Def(wn)) {
      ...
      if (loop) {
	    ...
	    OPERATOR_name(opr), WN_map_id(wn), wn);
          Dump_WN(wn,fp,fancy,2,2,NULL,NULL,NULL,FALSE);
          fprintf(fp,"has a non-loop node as loop_stmt: %d (0x%lx 0x%lx)\n",
      ...
      for (DU_NODE *def_node=(DU_NODE *)d_iter.First();
        ...
        if (ldid_in_do_loop_head) {
	      ...
	      OPERATOR_name(opr), WN_map_id(wn), wn);
            Dump_WN(wn,fp,fancy,2,2,NULL,NULL,NULL,FALSE);
            fprintf(fp, 
              "is ldid in loop head but has def out of loop head: %d [0x%lx]\n",
              WN_map_id(def1), def1);
          }
        }
    ...
    if (Du_Mgr->Du_Get_Use(wn)) {
      ...
      for (DU_NODE *use_node=(DU_NODE *)u_iter.First();
        ...
        if (!h_table->Find(use1)) {
          fprintf(fp,"WARNING: Def %d [0x%lx]", WN_map_id(wn), wn);
          Dump_WN(wn,fp,fancy,2,2,NULL,NULL,NULL,FALSE);
          fprintf(fp,"has a use outside the tree: %d [0x%lx]\n",
                      WN_map_id(use1), use1);
       ...
       !((ST_class(WN_st(wn)) ==  CLASS_PREG) && 
		Preg_Is_Dedicated(WN_offset(wn)))) {
       fprintf(fp,"WARNING: %s %d [0x%lx]", OPERATOR_name(opr), 
	 WN_map_id(wn), wn);
       Dump_WN(wn,fp,fancy,2,2,NULL,NULL,NULL,FALSE);
  ...
  if (opr == OPR_BLOCK) {
    ...
    if (opr == OPR_LDID) {
      ...
      for (INT i = symbols.Elements()-1; i >= 0; i--) {
	    ...
	    if (!dl->Incomplete()) {
              DevWarn("Bad loop stmt 0x%lx for induction variable %s <fixed>",
...
void FB_Sanity_Check(WN *wn)
{
  if (Cur_PU_Feedback && LNO_Test_Dump) {
    ...
    while (wniter) {
      ...
      freq = WN_MAP32_Get(FB_Map, cur);
      if (freq == 0) {
	DevWarn("? Missing frequency count for wn=0x%lx (opr=%s)",
		wn, OPERATOR_name(WN_operator(wn)));
      }
    }
  }
...
extern void MP_Sanity_Check_Func(WN *wn)
{
  ...
  if (opcode == OPC_BLOCK) {
	...
	("MP Do loop with a non-region grandparent 0x%lx",wn));
    }
    for (INT i = 0; i < WN_kid_count(wn); i++) {
...
//-----------------------------------------------------------------------
// NAME: Contains_Volatile_Refs
// FUNCTION: Returns 'true' when 'wn_tree' contains a reference to a volatile
//   location, 'false' otherwise.
//-----------------------------------------------------------------------

extern bool
Contains_Volatile_Refs (WN* wn_tree) {
  OPCODE opc = WN_opcode(wn_tree);
  
  if (OPCODE_is_store(opc) || OPCODE_is_load(opc)) {
    if (OPCODE_has_1ty(opc) && TY_is_volatile(WN_ty(wn_tree))) {
      return true;
    }
    
    if (OPCODE_has_2ty(opc) &&
	(TY_is_volatile(WN_ty(wn_tree)) ||
	 TY_is_volatile(WN_load_addr_ty(wn_tree)))) {
      return true;
    }
  }
  ...
  if (WN_opcode(wn_tree) == OPC_BLOCK) {
    for (WN *wn = WN_first(wn_tree); wn != NULL; wn = WN_next(wn)) {
      if (Contains_Volatile_Refs(wn)) {
        return true;
      }
    }
  } else {
    for (INT i = 0; i < WN_kid_count(wn_tree); i++) {
      if (Contains_Volatile_Refs(WN_kid(wn_tree, i))) {
	return true;
      }
    }
  }
...
extern INT64 WN_Whirl_Linenum(WN* wn_ref)
{
  WN *wn;
...
/* Returns 'exp' s.t. 2 ^ (exp - 1) < n <= 2 ^ exp */
INT
LNO_Min_Pow_Of_Two_Exp (INT n)
{
  INT pow = 0;
  while ((pow < sizeof(INT) * 8) && ((1 << pow) < n))
    pow++;
