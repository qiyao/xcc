...
***         Delete 'wn' if delete_wn is not FALSE  
***         
...
***
***	bool Contains_Volatile_Refs (WN* wn_tree)
***
***	  Returns 'true' when 'wn_tree' contains a reference to a volatile
...
***	  'nloops'.
***
***	INT64 WN_Whirl_Linenum(WN* wn_ref)
***
...
extern WN* Replace_Wnexp_With_Exp_Copy(WN* wn, WN* expr, DU_MANAGER* =NULL,
  BOOL* =NULL,ARRAY_DIRECTED_GRAPH16 *dep_graph=NULL, BOOL delete_wn=TRUE);
...
extern void Du_Sanity_Check(WN* wn, FILE* fp=stderr, UINT fancy=2);
...
extern INT64 WN_Whirl_Linenum(WN* wn); 
...
extern bool Contains_Volatile_Refs (WN* wn_tree);

extern BOOL Ldid_Is_Power_Of_2(WN *wn, WN *&def_wn);
extern bool Is_Tie_Output_Parm(WN *wn);


/* Returns 'exp' s.t. 2 ^ (exp - 1) < n <= 2 ^ exp */
extern INT LNO_Min_Pow_Of_Two_Exp (INT n);
#define LNO_IS_POWER_OF_TWO(n) (((n) & ((n) - 1)) == 0)
