...
extern WN* LWN_Get_Next_Stmt_Node (const WN* wn) {
    ...
    while (1) {
      ...
      if (ret_wn) {
        ...
        Is_True (!(OPCODE_is_expression(WN_opcode(ret_wn))),
                 ("GetNextStmtNode: 0x%lx trying to return an expression\n", ret_wn));
        if (OPCODE_is_expression(WN_opcode(ret_wn))) {
          printf ("GetNextStmtNode: 0x%lx trying to return an expression\n", ret_wn);
          exit (1);
        }
      }
      else break;
  ...
  Is_True (((!ret_wn) || OPCODE_is_stmt(WN_opcode(ret_wn))),
           ("GetNextStmtNode: returning a non-stmt 0x%lx\n", ret_wn));
  return ret_wn;
} /* GetNextStmtNode */
...
extern WN* LWN_Get_Next_SCF_Node (const WN* wn) {
  ...
  Is_True (((!ret_wn) || (OPCODE_is_scf(WN_opcode(ret_wn)))), 
           ("GetNextSCFNode: trying to return a non-SCF 0x%lx\n", ret_wn));
  return ret_wn;
} /* GetNextSCFNode */
...
void LWN_Insert_Block_Before (WN *block, WN *wn, WN *in)
{
  ...
  Is_True(!wn || OPCODE_is_stmt(WN_opcode(wn)) || OPCODE_is_scf(WN_opcode(wn)),
	  ("LWN_Insert_Block_Before: insert node 0x%lx not a stmt/scf\n", wn));
  Is_True((in != NULL),
...
void LWN_Insert_Block_After (WN *block, WN *wn, WN *in)
{
  ...
  Is_True(!wn || OPCODE_is_stmt(WN_opcode(wn)) || OPCODE_is_scf(WN_opcode(wn)),
	  ("LWN_Insert_Block_After: insert node 0x%lx not a stmt/scf\n", wn));
  Is_True((in != NULL),
 ...
 * NON LNO Version
 */
extern void LWN_Delete_From_Block_Non_LNO(WN *block, WN* wn) {
  WN *node;
  WN* parent_wn;
...
static WN* LWN_Copy_Tree_R(WN *,BOOL,WN_MAP,BOOL,WN_MAP,STACK_OF_WN *,
			   BOOL); 
#else
static WN* LWN_Copy_Tree_R_Non_LNO(WN *,BOOL,WN_MAP,BOOL,WN_MAP,STACK_OF_WN *,
			   BOOL); 
#endif
...
/* copy a tree */
/* if copy_access is TRUE, create copies of all the access vectors */
extern WN* LWN_Copy_Tree_Non_LNO(WN *wn, BOOL copy_access, WN_MAP access_map,
                         BOOL copy_version, WN_MAP version_map,
			 BOOL copy_all_nodes) {
  BOOL simp_state_save = WN_Simplifier_Enable(FALSE);
  STACK_OF_WN *region_stack;
  region_stack=NULL;
  WN *result = 
    LWN_Copy_Tree_R_Non_LNO(wn,copy_access,access_map,copy_version,version_map,
                    region_stack, copy_all_nodes);
  WN_Simplifier_Enable(simp_state_save);
  return result;
}
...
static  WN* LWN_Copy_Tree_R(WN *wn, BOOL copy_access, WN_MAP access_map,
  ...
  if (wn) {
    ...
    }
    {
      ...
      if (RR_Map && oper == OPR_ARRAY && Get_RR_Map(wn)) {
    ...
    if (Cur_PU_Feedback) {
      Cur_PU_Feedback->FB_duplicate_node(wn, ret_wn);
    }
...
#else // LNO 
static  WN* LWN_Copy_Tree_R_Non_LNO(WN *wn, BOOL copy_access, WN_MAP access_map,
			    BOOL copy_version, WN_MAP version_map,
			    STACK_OF_WN *region_stack,
			    BOOL copy_all_nodes) {
  WN* ret_wn = NULL;
  ...
  if (wn) {
    ...
    Is_True(copy_access == FALSE,
	    ("Compile with -DLNO to copy access vectors in LWN_Copy_Tree_R_Non_LNO"));

    if (WN_opcode(wn) == OPC_BLOCK) {
      WN_first(ret_wn) = WN_last(ret_wn) = NULL;
      kid = WN_first (wn);
      if (kid) {
        ret_kid = LWN_Copy_Tree_R_Non_LNO (kid,
				   copy_access, access_map,
				   copy_version, version_map,
				   region_stack,
				   copy_all_nodes);
      ...
      for (kidno=0; kidno<WN_kid_count(wn); kidno++) {
        if (WN_kid (wn, kidno)) { 
          ret_kid = LWN_Copy_Tree_R_Non_LNO (WN_kid (wn, kidno), 
                                     copy_access, access_map,
                                     copy_version, version_map,
				     region_stack,
				     copy_all_nodes);
...
void LWN_Copy_Def_Use(WN* from_exp, WN* to_exp, DU_MANAGER* du)
{
  ...
  FmtAssert(fop == top && 
	    (OPCODE_is_expression(fop) || OPCODE_is_call(fop)),
	    ("Opcodes unequal LWN_Copy_Def_Use(%d,%d) or not expr/call", fop, top));
...
void LWN_Copy_Def_Use_For_Def(WN* from_node, WN* to_node, DU_MANAGER* du)
{ 
  FmtAssert(WN_operator(from_node) == WN_operator(to_node) &&
	    WN_operator(from_node) == OPR_STID, 
	    ("LWN_Copy_Def_Use_For_Def: Expecting STID"));

  USE_LIST* uselist = du->Du_Get_Use(from_node);
  if (uselist) {
    USE_LIST_ITER iter(uselist);
    for (DU_NODE* n = iter.First(); !iter.Is_Empty(); n = iter.Next()) {
      WN* use = n->Wn();
      du->Add_Def_Use(to_node, use);
    }
    USE_LIST* uselist2 = du->Du_Get_Use(to_node);
    if (uselist2 == NULL) {
      du->Create_Use_List(to_node);
      uselist2 = du->Du_Get_Use(to_node);
    } 
    if (uselist->Incomplete()) 
      uselist2->Set_Incomplete();
  }
} 


void
LWN_Clear_Loop_Stmt (WN *wn, DU_MANAGER *du)
{
  if (WN_operator(wn) == OPR_BLOCK) {
    for (WN *kid = WN_first(wn); kid; kid = WN_next(kid)) {
      LWN_Clear_Loop_Stmt(kid, du);
    }
  ...
  for (INT k = 0; k < WN_kid_count(wn); k++) {
    LWN_Clear_Loop_Stmt(WN_kid(wn, k), du);
  }
  
  DEF_LIST* deflist = du->Ud_Get_Def(wn);
  if (deflist) {
    ...
    if (WN_opcode(wn) == OPC_BLOCK) {
      ...
      while (kid) {
        Is_True ((LWN_Get_Parent (kid) == wn),
                 ("CheckParentize Error: kid 0x%lx, parent 0x%lx\n", kid, wn));
      ...
      for (kidno=0; kidno<WN_kid_count(wn); kidno++) {
        ...
        if (kid) { 
          Is_True ((LWN_Get_Parent (kid) == wn),
                   ("LWN_Check_Parentize(): kid 0x%lx (%s), parent 0x%lx (%s)\n",
...
void LWN_Delete_Tree_Non_LNO(WN* wn)
{
  if (wn == NULL)
    return;
  ...
  if (WN_opcode(wn) == OPC_BLOCK) {
    ...
    for (INT i = 0; i < WN_kid_count(wn); i++) {
      LWN_Delete_Tree_Non_LNO(WN_kid(wn,i));
    }
  }
  ...
  if (parent && WN_operator(parent) == OPR_BLOCK) {
    LWN_Delete_From_Block_Non_LNO(parent,wn);
  } else {
    WN_Delete(wn);
  }
}
...
WN *LWN_CreateDO(WN *index, WN *start, WN *end, WN *step,
		       WN *body)
{
  WN *loop_info = LWN_CreateLoopInfo(LWN_Copy_Tree(index),NULL,0,0,0,AT_REGION_ID_UNKNOWN);
...
WN *LWN_CreateLoopInfo(WN *induction, WN *trip, UINT16 trip_est,
				UINT16 depth, INT32 flags, UINT16 at_region_id)
{
  WN *wn = WN_CreateLoopInfo(induction, trip, trip_est,depth, flags, at_region_id);
...
#else
WN *LWN_CreateStid_Non_LNO(OPCODE opc, WN *orig_op, WN *value)
{
  Is_True((WN_operator(orig_op) == OPR_LDID) ||
          (WN_operator(orig_op) == OPR_STID),
	  ("Illegal orig_op in LWN_Create_Stid"));
...
WN *
LWN_Create_Intrinsic (OPCODE opc, INTRINSIC intrinsic,
		      INT32 n, WN *kids[]) {
  WN *wn = WN_Create_Intrinsic(opc, intrinsic, n, kids);
  if (Cur_PU_Feedback)
    Cur_PU_Feedback->FB_set_in_out_same_node(wn);
  LWN_Parentize_One_Level(wn);
  return wn;
}
...
WN *
LWN_CreateOutpart (WN *intr_out, INT part, TYPE_ID rtype) {
  WN *wn = WN_CreateOutpart(intr_out, part, rtype);
  LWN_Parentize_One_Level(wn);
  return wn;
}
...
// Return signed integer type of size 'bit_size'
extern TYPE_ID Bit_Size_To_Int(UINT bit_size) {
  switch (bit_size) {
...
void DBX_WN_Map (WN_MAP map, WN* wn) {
  ...
  printf ("Map = 0x%lx (%ld)\n", WN_MAP_Get(map, wn), WN_MAP_Get(map, wn));
}


//
// Find index 'i' such that WN_kid(wn, i) == kid. Return -1 if no such index.
//
INT
LWN_Kid_Index (const WN *wn, const WN *kid)
{
  if (WN_operator(wn) == OPR_BLOCK)
    return -1;
...
WN*
LWN_Simplify_Tree(WN* wn)
{
  WN *wn_parent = LWN_Get_Parent(wn); 
  INT kid_idx = wn_parent ? LWN_Kid_Index(wn_parent, wn) : -1;
  
  WN *wn_new = WN_Simplify_Tree(wn);
  
  if (wn_parent != NULL) {
    if (kid_idx < 0) {
      FmtAssert(WN_operator(wn_parent) == OPR_BLOCK && wn_new == wn,
		("No support for simplified statement in a block"));
    } else {
      WN_kid(wn_parent, kid_idx) = wn_new;
