 ...
 *   (block) node.
 *
 * extern void LWN_Delete_From_Block_Non_LNO(
 *     WN *block, 
 *     WN* wn, 
 *     );
 *   Delete the node 'wn' assuming that 'wn' is the child of a BLOCK
 *   (block) node.
 *   Version to be called from outside of LNO dll
 * 
 * 
 ...
 *   Copies the parent annotations. This is strictly a tree copy.
 *   If copy_access=TRUE, also copies the access arrays
 *   Version to be called outside of LNO dll
 *
 ...
 *
 *  TYPE_ID Bit_Size_To_Int(UINT bit_size)
 *
 ...
 *
 * extern void LWN_Copy_Def_Use_For_Def(WN* from_exp, WN* to_exp, DU_MANAGER*);
 *
 *   Copies the def/use information in from to the corresponding nodes
 *   in to.  This is special case code only where both are STID.
 *
 * extern void LWN_Clear_Loop_Stmt (WN *wn, DU_MANAGER *du)
 *
 *   Sets the 'loop_stmt' of all DEF_LIST's in wn to NULL.
 *
 ...
 *   Given a tree, walk one level and set the parent pointers
 *   Note: the pointer for the node wn itself is NOT changed, but only for
 *      its kids
 *
 ...
 *
 * extern void LWN_Delete_Tree_Non_LNO(WN *wn);
 *   Delete the node and all its descendants.
 *   Non-lno version
 *
 ...
 *
 * extern WN *LWN_CreateStid_Non_LNO(OPCODE opc, WN *orig_op, WN *value)
 *   Create an Stid to the same location as orig_op.  While you can also
 *   use the mimicking LWN_CreateStid, using this version will give you
 *   more accurate alias information.
 *   Version to be used outside of LNO dll
 *  
 ...
 *
 * inline void LWN_Copy_Frequency(const WN* wn_origl, const WN* wn_dupli);
 *    
 ...
 *
 * extern void LWN_Copy_Frequency_Tree(const WN* wn_origl, const WN *wn_dupli);
 *
...
/* to be implemented */
inline void LWN_Set_Frequency(const WN* wn, const INT64 value) 
{
  //as of Feb 2003, this is only called from cache tiling
  //code inside snl_gen and snl_inv.cxx , so when we get that 
  //working, we will need to update this.
  FmtAssert(0, ("LWN_Set_Frequency unimplemented"));
}

inline void LWN_Copy_Frequency(WN* wn_origl, WN* wn_dupli) 
{ 
  if (Cur_PU_Feedback)
    Cur_PU_Feedback->FB_duplicate_node(wn_origl, wn_dupli);
}

inline void LWN_Scale_Frequency(WN* wn, float scale) 
{
  if (Cur_PU_Feedback)
    Cur_PU_Feedback->FB_scale_node(wn, FB_FREQ(scale, false));
}

inline void LWN_Copy_Frequency_Tree(WN* wn_origl, WN * wn_dupli) 
{
  if (Cur_PU_Feedback) {
    Cur_PU_Feedback->FB_duplicate(wn_origl, wn_dupli);
  }
}

inline void LWN_Scale_Frequency_Tree(WN* wn, const float scale) 
{
  if (Cur_PU_Feedback) {
    Cur_PU_Feedback->FB_scale(wn, FB_FREQ(scale, false));
  }
}

inline void
LWN_Adjust_Frequency(WN* parent, WN* inserted)
{
  if ( Cur_PU_Feedback )
    Cur_PU_Feedback->FB_adjust_frequency(parent, inserted);
}
...
extern WN* LWN_Copy_Tree_Non_LNO(
       WN *wn, 
       BOOL copy_access=FALSE,
       WN_MAP access_map=0,
       BOOL copy_version=FALSE,
...
extern INT LWN_Kid_Index (const WN *wn, const WN *kid);
...
extern void LWN_Copy_Def_Use_For_Def(WN*, WN*, DU_MANAGER*);
extern void LWN_Clear_Loop_Stmt (WN *wn, DU_MANAGER *du);
extern void LWN_Delete_DU(WN *wn);
extern void LWN_Delete_LNO_dep_graph(WN *wn);
extern void LWN_Delete_CG_dep_graph(WN *wn);
...
extern WN *LWN_CreateParm(TYPE_ID rtype, WN *parm_node, TY_IDX ty, UINT32 flag);

extern WN *LWN_Create_Intrinsic (OPCODE opc, INTRINSIC intrinsic,
				 INT32 n, WN *kids[]);

extern WN *LWN_CreateOutpart (WN *intr_out, INT part, TYPE_ID rtype);
