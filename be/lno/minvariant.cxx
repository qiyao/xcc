...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/minvariant.cxx,v $ $Revision: 1.67 $";
#endif
...
BOOL MIR_REFLIST::Same(ST* base, ACCESS_ARRAY* aa, WN* array)
{
  return Same_Base(base, Base) &&
    *aa == *Aa &&
    ...
    == DEP_CONTINUE &&
    (WN_desc(LWN_Get_Parent(Wnlist[0])) ==  WN_desc(LWN_Get_Parent(array))) 
    &&
    /* same rtype or one of them is store */
    (WN_rtype(LWN_Get_Parent(Wnlist[0])) == WN_rtype(LWN_Get_Parent(array))
     || WN_rtype(LWN_Get_Parent(Wnlist[0])) == MTYPE_V 
     || WN_rtype(LWN_Get_Parent(array)) == MTYPE_V);
}
...
void MIR_REFLIST::Print(FILE* f) const
{
  ...
  for (INT j = 0; j < Wnlist.Elements(); j++) {
    ...
    fprintf(f, " <0x%lx,0x%lx>", parent, wn);
  }
			...
			BOOL 			 messy_only)
{
  ...
  */
  ...
  if (type == Promote_Type(type)) {
     ...
     newsym = Create_Local_Symbol(newname, type);
     used_local = true;
  }
  ...
  if (j < p->Wnlist.Elements()) {
    ...
    if (used_local) Create_local_alias(Alias_Mgr, read_tree);
    LWN_Copy_Linenumber(LWN_Get_Statement(the_load),read_tree);
    //LWN_Copy_Frequency_Tree(loop, read_tree);
  ...
  if (j < p->Wnlist.Elements()) {
    ...
    WN*         ldid = WN_CreateLdid(op, newsym.WN_Offset(), 
				newsym.St(), ty);
    if (used_local) Create_local_alias(Alias_Mgr, ldid);
    ...
    //LWN_Copy_Frequency_Tree(loop, write_tree);
    if (gdepth > 0) {
  ...
  for (j = 0; j < p->Wnlist.Elements(); j++) {
    ...
    for (e = dg->Get_Out_Edge(v); e; e = dg->Get_Next_Out_Edge(e)) {
      ...
      if ((components <= gdepth) != (prnt == NULL)) {
        fprintf(TFile, "Disaster: %d %d 0x%lx:", components, gdepth, prnt);
        fprintf(TFile, " v=%d 0x%lx v2=%d 0x%lx e=%d\n",
    ...
    if (is_write) {
      ...
      if (used_local) Create_local_alias(Alias_Mgr, newstid);
      LWN_Copy_Linenumber(oldistore,newstid);
      LWN_Copy_Frequency_Tree(oldistore, newstid);
      ...
      WN*     newldid = WN_CreateLdid(op, newsym.WN_Offset(), 
			   newsym.St(), ty);
      if (used_local) Create_local_alias(Alias_Mgr, newldid);
      WN*     p_oldiload = LWN_Get_Parent(oldiload);
      LWN_Copy_Frequency_Tree(oldiload, newldid);
                              ...
                              ARRAY_DIRECTED_GRAPH16* dg)
{
  ...
  for (INT j = 0; j < p->Wnlist.Elements(); j++) {
    ...
    FmtAssert(np, ("Couldn't find parent of 0x%lx(opc=%d)!", pp, WN_opcode(pp)));    if (!for_sure_not) {
      stk_good.Push(wn);
    }
  }
  ...
  // Step four, disqualify structure loads/stores
  // Structure loads/stores currently assert because we try to create
  // a structure preg.  In normal structure code this doesn't come up, so it's
  // not worth fixing for now.
  for (i = 0; i < elts; i++) { 
    WN *load_store = LWN_Get_Parent(p->Wnlist[i]);
    if (WN_desc(load_store) == MTYPE_M || WN_rtype(load_store) == MTYPE_M) {
	    return FALSE;
    }
  }
                                      ...
                                      BOOL      duplicates_okay)
{
  ...
  // don't hoist converting loads
  if (OPCODE_is_load(parent_op)) {
    if (OPCODE_rtype(parent_op) != OPCODE_desc(parent_op)) {
      if (Promote_Type(OPCODE_desc(parent_op)) != OPCODE_rtype(parent_op)) {	
        return;
      }
    }
  }
                                     ...
                                     MEM_POOL* pool)
{
  ...
  if (opr == OPR_BLOCK) {
    ...
    FmtAssert(Get_Do_Loop_Info(wn)->Has_Calls &&
	      Get_Do_Loop_Info(wn)->Has_Non_Intrinsic_Calls,
	      ("IO error in minvariant"));
#endif
  }
  else {
    for (INT i = 0; i < WN_kid_count(wn); i++)
...
extern void Minvariant_Removal_Loop(WN *loop, ARRAY_DIRECTED_GRAPH16 *dg)
{
  if (mir_local_pool_initialized == FALSE) {
    mir_local_pool_initialized = TRUE;
    MEM_POOL_Initialize(&MIR_local_pool, "MIR_local_pool", FALSE);
  ...
  if (WN_opcode(loop)==OPC_DO_LOOP) {
    BOOL messy_only = FALSE;
    MEM_POOL_Push_Freeze(&MIR_local_pool);
    DYN_ARRAY<MIR_REFLIST*>* lists =
      MIR_Build_Loop_List(loop, &MIR_local_pool);
    DO_LOOP_INFO* dli = Get_Do_Loop_Info(loop);
    if (!dli->Has_Bad_Mem && !dli->Has_Gotos)
      MIR_Try_Hoist(loop, *lists, dg, &MIR_local_pool, messy_only);
    MIR_Go_Inside(WN_do_body(loop), *lists, dg, messy_only);
    while (lists->Elements() > 0)
      lists->Decidx();
    CXX_DELETE(lists, &MIR_local_pool);
    MEM_POOL_Pop_Unfreeze(&MIR_local_pool);
