...
//
//	    INT Old_To_New_Order(INT old_i)
//
//		Maps the loop with original number old_i to its new position
//              after a permutation. Old_To_New_Order(0) is the new depth of the
//              original outermost loop.
//
//	    INT Old_To_New_Order(INT old_i, INT new_inner)
//
//		Maps the loop with original number old_i to its new position
//              after a permutation with a new inner loop 'new_inner'. Old_To_New_Order(0,_) is
//              the new depth of the original outermost loop.
//
...
//		class SX_INFO *pi, 
//		INT SNL_Depth, INVAR_TABLE *invar_table)
//
...
//
//	    void Enter(SYMBOL *symbol, BOOL is_store, INT weight, BOOL is_reduction)
//
//		If this symbol isn't in the tree, enter it and set
//		its weight.  Either way, if is_store is TRUE, set the
//		node's is_store and if is_reduction is TRUE, set the node's 
//		is_reduction
//
...
#define MD_RCS_ID
#ifdef _KEEP_RCS_ID
static char *model_rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/model.h,v $ $Revision: 1.62 $";
#endif /* _KEEP_RCS_ID */
#endif
...
class SIMD_LOOP;     // declaration in simd.h
class LOOP_MODEL;
class REGISTER_MODEL;
class ARRAY_REF;
class ARRAY_REF_LIST;
class LAT_DIRECTED_GRAPH16;
class REG_REF_COUNT;
class LNO_REGCLASS_INFO;
class LNO_REGS;
class SYMBOL_TREE;
...
// build a stack of DO_LOOP_INFO structures
extern void Build_DLI_Stack(WN *wn, DLI_STACK *stack);

typedef INT LNO_REGS_IDX;

#define LNO_REGS_IDX_UNDEFINED (LNO_REGS_IDX)0
#define LNO_REGS_IDX_FIRST (LNO_REGS_IDX)1

//
// LNO_REGS
//
// A counter for each type of register. This class uses LNO_REGCLASS_INFO
// to dynamically match types to register classes and maintain counts
//
class LNO_REGS {

private:
  MEM_POOL *_pool;
  const LNO_REGCLASS_INFO *_regclass_info;
  LNO_REGS_IDX _regs_count;
  INT *_cnt;
  INT *_limit;
  
public:
  LNO_REGS(const LNO_REGCLASS_INFO *regclass_info, MEM_POOL *pool);
  LNO_REGS(const LNO_REGS *regs, MEM_POOL *pool);
  ~LNO_REGS();
  
  MEM_POOL *Pool() { return _pool; }
  const LNO_REGCLASS_INFO *Regclass_Info() const { return _regclass_info; }
  
  LNO_REGS &operator = (const LNO_REGS &regs);
  
  void Clear();
  
  void Add(const LNO_REGS *regs);
  void Add(const LNO_REGS &regs) { Add(&regs); }
  
  void Sub(const LNO_REGS *regs);
  void Sub(const LNO_REGS &regs) { Sub(&regs); }
  
  INT Total();
  
  INT &Count_Of_Idx(LNO_REGS_IDX idx);
  INT Count_Of_Idx(LNO_REGS_IDX idx) const;

  INT &Limit_Of_Idx(LNO_REGS_IDX idx) ;
  INT Limit_Of_Idx(LNO_REGS_IDX idx) const;
  
  INT &Count_Of_Type(TYPE_ID type);
  INT Count_Of_Type(TYPE_ID type) const;
  
  // increments the usage for 'type' by 'uses' multiplied
  // by the number of registers that 'type' occupies
  void Inc_Type_Usage(TYPE_ID type, INT uses=1);
  
  INT &Count_Of_At_Ty_Id(AT_TY_ID ty_id);
  INT Count_Of_At_Ty_Id(AT_TY_ID ty_id) const;
  
  INT &Count_Of_Int() { return Count_Of_Type(MTYPE_I4); }
  INT Count_Of_Int() const { return Count_Of_Type(MTYPE_I4); }
  INT &Count_Of_Float() { return Count_Of_Type(MTYPE_F4); }
  INT Count_Of_Float() const { return Count_Of_Type(MTYPE_F4); }
  
  LNO_REGS_IDX Regs_Count() const { return _regs_count; }
  
  void Print(FILE *f=stderr, INT indent=0, bool verbose=false);
};
...
private:
  ...
  WN *_wn; // model inner loop (original)
  INT _snl_depth; 
  INT64 *_snl_line_numbers;
  
  INVAR_TABLE *_invar_table; // invariant expression map
  INVAR_TABLE *_all_inv_table; // all invariant expressions map
  ARRAY_DIRECTED_GRAPH16 *_array_graph; // array dependence graph
  DOLOOP_STACK *_do_stack; // do loop stack from innermost
  SX_INFO *_pi; // privatizability information
  
  const LNO_REGS *_target; // target architecture registers
  LNO_REGCLASS_INFO *_regclass_info; // target architer register file info
  ...
  LNO_REGS *_array_refs;  // how many array refs in the loop pre-unrolling
  LNO_REGS *_scalar_refs; // how many scalar refs in the loop pre-unrolling
  BOOL *_can_be_inner_orig;
  BOOL *_can_be_inner;
  INT _num_loops;
  INT _num_good;
  double _num_cycles_inner;
  LNO_REGS *_regs_inner;
  LNO_REGS *_refs_inner;
  ...
  BOOL *_can_be_unrolled;      // which loop can be unrolled
  INT _outermost_can_be_tiled;
  BOOL _trying_invariant;
  INT _invariant_refs_inner;
  double _invariant_ref_coeff;

  LNO_REGS *_vec_regfiles;      // vector regfiles
  LNO_REGS *_align_regfiles;    // alignment regfiles
  ...
  LNO_REGS *_regs;     // results
  LNO_REGS *_refs;     // results
  INT _num_tlb;        // results
  INT _unroll_prod;    // results

  INT _num_evaluations;

  SIMD_LOOP *_simd_loop; // SIMD information
  bool _at_analysis_phase;

  mBOOL _blocking_disabled;

  ARRAY_REF *_arl; // list of all array references
  LAT_DIRECTED_GRAPH16 *_lat_graph;  // graph of latencies
  ...
  LNO_REGS *_base; // how many regs needed to keep pipeline full
  LNO_REGS *_non_temp_scalars; // need a reg for each of these
  double _num_mem_units;
  double _se_overhead; // scalar expansion overhead
  ...
  static TI_RES_COUNT *OP_Resources(WN *wn, double *num_fp_instr,
				    INVAR_TABLE *invar_table);
  static double OP_Cycles(REGISTER_MODEL *rmodel, 
                          double *num_fp_instr,
                          INVAR_TABLE *invar_table,
                          MEM_POOL *pool);
  static INT OP_Resources_R(WN *wn,TI_RES_COUNT *resource_count,
			    double *num_fp_instr, INVAR_TABLE *invar_table);
  ...
  void Adjust_Vector_Regfiles (SYMBOL_TREE_NODE *sym_node);
  void Adjust_Vector_Regfiles (ARRAY_REF *arl, SYMBOL_TREE *sym_tree);
  
  double Unequal_Unroll_Penalty_Coeff(INT num_loops, INT *unroll_factors);
  
  void Try_Inner(BOOL *can_be_unrolled, INT outermost_can_be_tiled,
		 INT inner, INT num_loops, SX_INFO *pi);
...
public:
  ...
  LOOP_MODEL() {}
  LOOP_MODEL(WN *wn, BOOL *can_be_inner, BOOL *can_be_unrolled,
             INT outermost_can_be_tiled, BOOL trying_invariant,
             ARRAY_DIRECTED_GRAPH16 *array_graph,
             SX_INFO *pi, INT SNL_Depth,
	     INVAR_TABLE *invar_table) {
    Model(wn,can_be_inner,can_be_unrolled,outermost_can_be_tiled,trying_invariant,
          array_graph,pi,SNL_Depth,invar_table);
  }
  ~LOOP_MODEL();
  ...
  void SIMD_Model();
  void SIMD_Post_Model();

  void Setup_Nest_Info();
  void Setup_Target (void);
  void Setup_With_Simd();
  
  static INT Model_No() { return _model_no; }

  WN *Model_Inner_Loop() { return _wn; }

  INT Num_Good() const { return _num_good; }
  INT Num_Loops() const { return _num_loops; }
  INT Num_Fp_Regs() const { return _regs->Count_Of_Float(); }
  INT Num_Fp_Refs() const { return _refs->Count_Of_Float(); }
  INT Num_Int_Regs() const { return _regs->Count_Of_Int(); }
  INT Num_Int_Refs() const { return _refs->Count_Of_Int(); }
  ...
  INT Old_To_New_Order(INT old_i);
  INT Old_To_New_Order(INT old_i, INT inner);
  INT Inner_Loop() const { return _inner_loop; }
  BOOL Trying_Invariant() const { return _trying_invariant; }
  double Num_Cycles() const { return _num_cycles; }
  SIMD_LOOP *Simd_Loop(void) { return _simd_loop; } // SIMD information
  float Se_Overhead() const { return _se_overhead; }
  LNO_REGCLASS_INFO *Regclass_Info() { return _regclass_info; }
  ARRAY_REF *Array_Ref (void) const { return _arl; }
}; // LOOP_MODEL
...
class REGISTER_MODEL {
  ...
  LNO_REGCLASS_INFO *_regclass_info;
  LNO_REGS *_base; // how many regs needed to keep pipeline full
public:
  REGISTER_MODEL() {
    _pool = NULL;
  ...
  void Calculate_Register_Usage(
      WN *inner, LNO_REGS *regs_used_out);
...
class ARRAY_REF_NODE : public SLIST_NODE {
  ...
  // vector references
  LNO_REGS_IDX _vec;    // vector reference
  LNO_REGS_IDX _align;  // uses alignment register
  LNO_REGS_IDX _sel;    // yses a select register to emulate alignment
  INT _vec_regs;        // how many vector registers will be needed
  ...
  ARRAY_REF_NODE(ACCESS_ARRAY *array, WN* wn, BOOL is_store, INT element_size,
		 mUINT16 lex_number, TYPE_ID type) { 
    Array = array;
    Wn = wn;
    _type = type;
    ...
    _vec = LNO_REGS_IDX_UNDEFINED;
    _align = LNO_REGS_IDX_UNDEFINED;
    _sel = LNO_REGS_IDX_UNDEFINED;
    _vec_regs = 0;
  }
  ...
  bool Has_Load() const { return _has_load; }
  
  LNO_REGS_IDX Vec() const { return _vec; }
  void Set_Vec(LNO_REGS_IDX vec) { _vec = vec; }
  LNO_REGS_IDX Align() const { return _align; }
  void Set_Align(LNO_REGS_IDX align) { _align = align; }
  LNO_REGS_IDX Sel() const { return _sel; }
  void Set_Sel(LNO_REGS_IDX sel) { _sel = sel; }
  INT Vec_Regs() const { return _vec_regs; }
  void Set_Vec_Regs(LNO_REGS_IDX vec_regs) { _vec_regs = vec_regs; }
}; // ARRAY_REF_NODE
...
public:
  ...
  ARRAY_REF_LIST(MEM_POOL *pool, SYMBOL *base_array) :
    _pool(pool), Base_Array(base_array) {
    _is_scalar_expanded = FALSE;
  }
  ARRAY_REF_LIST(ARRAY_REF_LIST *orig, MEM_POOL *pool); 
  ~ARRAY_REF_LIST();

  void Calc_Regs_And_Refs(REG_REF_COUNT *rr, BOOL outer_vectorization);
  ...
  void Num_Refs(LNO_REGS *refs) const; // adds to refs the references in this
  void Print(FILE *fp) const;
}; // ARRAY_REF_LIST
...
private:
  ...
  void Build(WN *wn, INT SNL_Depth, INVAR_TABLE *invar_table);
  void Build_Rec(WN *wn, DLI_STACK *dli_stack, INT SNL_Depth, INVAR_TABLE *invar_table);
  void Build_Array(WN *wn, BOOL is_store, DLI_STACK *dli_stack, INT SNL_Depth);
  
public:
  ARRAY_REF(WN *wn, INT SNL_Depth, MEM_POOL *pool, INVAR_TABLE *invar_table) :
    _stack(pool), _pool(pool), _num_bad_fp(0), _num_bad_int(0), _lex_number(0) {
  ...
  INT Num_Invariants(INT loop_no);
  void Num_Refs(LNO_REGS *refs) const; // adds to refs the references in this
  void Add_References(WN *wn, INT SNL_Depth,INVAR_TABLE *invar_table) {
    Build(wn, SNL_Depth,invar_table);
  }
  void Enter_Scalar_Expand(WN *wn,SX_PNODE *pnode, BOOL *can_be_inner, INT num_loops);
  ...
  INT Lex_Number() const { return _lex_number; }
...
public:
  ...
  // Depv: a null pointer implies an "all equals" dependence
  // we use it to save space for non-loads/stores
  DEPV *Depv; 
};

typedef DIRECTED_GRAPH16<LAT_EDGE16,LAT_VERTEX16> DIRECTED_GRAPH16_LAT;

class LAT_DIRECTED_GRAPH16 : public DIRECTED_GRAPH16_LAT {
...
private:
  ...
  HASH_TABLE<VINDEX16,VINDEX16> _hash_table; // maps vertices in array graph to vertices in this
...
public:
  ...
  LAT_DIRECTED_GRAPH16(mUINT16 num_v, mUINT16 num_e, 
		       ...
		       ARRAY_DIRECTED_GRAPH16 *array_graph) :
    DIRECTED_GRAPH16_LAT(num_v,num_e), _pool(pool), 
    _num_dim(num_dim), _num_bad(num_bad), _hash_table(200,pool), _array_graph(array_graph),
    Lat_scc_vertex_map(NULL), Scc_lat_vertex_map(NULL) {}
  ...
  INT Add_Flow_Edges();
  INT Add_Vertices_Op_Edges(WN *wn, INVAR_TABLE *invar_table);
  
  EINDEX16 Add_Edge(VINDEX16 from, VINDEX16 to, DEPV *depv, INT latency,
		    UINT16 num_unused_dim) {
    EINDEX16 result = DIRECTED_GRAPH16_LAT::Add_Edge(from,to);
    if (result != 0) {
      _e[result].Depv = depv;
  ...
  INT Latency(EINDEX16 edge) { return(_e[edge].Latency); }
  DEPV *Depv(EINDEX16 edge) { return(_e[edge].Depv); }
  UINT16 Num_Unused_Dim(EINDEX16 edge) { return(_e[edge]._num_unused_dim); }
  VINDEX16 Add_Vertex(WN *wn) {
    VINDEX16 result = DIRECTED_GRAPH16_LAT::Add_Vertex();
    _v[result]._wn = wn;
    return result;
  }
  WN *Wn(VINDEX16 v) { return _v[v]._wn; }
  void Remove_Edge(EINDEX16 e) { DIRECTED_GRAPH16_LAT::Delete_Edge(e); }
  void Delete_Edge(EINDEX16 e) {
    CXX_DELETE_ARRAY(_e[e].Depv,_pool);
    DIRECTED_GRAPH16_LAT::Delete_Edge(e);
  }
  
  void Map_Vertex(VINDEX16 oldv, VINDEX16 newv) { _hash_table.Enter(oldv, newv); }
...
private:
  ...
  bool _is_reduction;
  INT  _weight;
  LNO_REGS_IDX _vec_idx;
  
  INT Symbol_Compare(SYMBOL *s);
  
public:
  SYMBOL_TREE_NODE(SYMBOL symbol, bool is_store, INT weight, bool is_reduction) {
    _symbol = symbol;
    _is_store = is_store;
    _is_reduction = is_reduction;
    _left = _right = NULL;
    _weight = weight;
    _vec_idx = LNO_REGS_IDX_UNDEFINED;
  }

  SYMBOL_TREE_NODE *Left() { return _left; }
  SYMBOL_TREE_NODE *Right() { return _right; }
  SYMBOL &Symbol() { return _symbol; }

  void Num_Refs(LNO_REGS *refs, LNO_REGS *non_temp_scalars) const;
  void Enter(SYMBOL *symbol, MEM_POOL *pool, bool is_store, INT weight, 
		  BOOL is_reduction);

  void Set_Vec_Idx(LNO_REGS_IDX vec_idx) { _vec_idx = vec_idx; }
  LNO_REGS_IDX Vec_Idx (void) const { return _vec_idx; }
}; // SYMBOL_TREE_NODE

//
// SYMBOL_TREE
//
// binary tree of SYMBOLs (ie, SYMBOL_TREE_NODEs)
//
class SYMBOL_TREE {
...
public:
  ...
  void Enter_Scalar_Refs(WN *wn, WN2INT* se_needed=NULL, ARRAY_REF *ar=NULL);
  void Enter_Scalar_Refs(WN *wn, ARRAY_REF *ar, SX_INFO *pi, BOOL *can_be_inner,
			 INT num_loops, INT outer);
  
  void Num_Refs(LNO_REGS *refs, LNO_REGS *non_temp_scalars) const;

  SYMBOL_TREE_NODE *Symbol_Node() { return _symbol_node; }
}; // SYMBOL_TREE
...
private:
  MEM_POOL *_pool;
  LNO_REGS_IDX _last_idx;
  
  // index to ISA_REGCLASS map
  INT _idx_regclass_size;
  ISA_REGCLASS *_idx_regclass; // [index] -> AT_TY_ID

  // index to AT_TY_ID map
  INT _idx_at_ty_size;
  AT_TY_ID *_idx_at_ty; // [index-_idx_regclass_size] -> AT_TY_ID

  // TYPE_ID to index map
  INT _type_id_idx_size;
  LNO_REGS_IDX *_type_id_idx; // [TYPE_ID] -> index

  // AT_TY_ID to index map
  AT_TY_TAB *_at_ty_tab;
  INT _at_ty_idx_size;
  LNO_REGS_IDX *_at_ty_idx; // [AT_TY_ID] -> index
  
  LNO_REGS *_target;
  
  void Init_Target_Regs();
  
public:
  LNO_REGCLASS_INFO(MEM_POOL *pool, AT_TY_TAB *at_ty_tab=NULL);
  ~LNO_REGCLASS_INFO();
  
  MEM_POOL *Pool() { return _pool; }
  
  LNO_REGS_IDX Last_Idx() const { return _last_idx; }
  LNO_REGS_IDX Regs_Count() const { return _last_idx+1; }
  
  LNO_REGS_IDX Index_Of_Type(TYPE_ID type) const;
  LNO_REGS_IDX Index_Of_At_Ty_Id(AT_TY_ID ty_id) const;
  
  SRC Idx_Source(LNO_REGS_IDX idx) const;
  ISA_REGCLASS Regclass_Of_Idx(LNO_REGS_IDX idx) const;
  AT_TY_ID At_Ty_Id_Of_Idx(LNO_REGS_IDX idx) const;
  AT_TY *At_Ty_Of_Idx(LNO_REGS_IDX idx) const;
  
  const char *Regclass_Name_Of_Idx(LNO_REGS_IDX idx);
  
  static const char *Regclass_Name(ISA_REGCLASS regclass);
  static INT Regclass_Reg_Count(ISA_REGCLASS regclass);
  static INT Reg_Count_Of_Type(TYPE_ID type);
  
  const LNO_REGS *Target() const { return _target; }
...
// REG_REF_COUNT
//
// Used by ARRAY_REF to count the number of registers, references and other
// statistics for evaluation
//
class REG_REF_COUNT {

public:
  LNO_REGS regs; // registers live in the inner loop
  LNO_REGS refs; // array references in the inner loop
  LNO_REGS var_stores; // inner loop variant stores
  LNO_REGS inv_stores; // inner loop invariant stores
  LNO_REGS unused_regs; // unused registers live in the inner loop
  
  REG_REF_COUNT(LNO_REGCLASS_INFO *regclass_info, MEM_POOL *pool);
  void Clear();
  void Print(FILE *f, INT indent=0, bool verbose=false);
};
