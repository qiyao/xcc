...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/move.cxx,v $ $Revision: 1.50 $";
#endif /* _KEEP_RCS_ID */
  ...
  case OPR_LSHR:
    {
      if (!((opr == OPR_DIV || opr == OPR_REM || opr == OPR_MOD)    
          && Safe_Spec_Map && WN_MAP_Get(Safe_Spec_Map, (WN*) wn_stat)
			       ...
			       DU_MANAGER* du)
{
  ...
  if (Cur_PU_Feedback) {
    // most of the time this code will be executed
    FB_Info_Loop loop_info = Cur_PU_Feedback->Query_loop(wn_loop);
    FB_Info_Branch branch_info(FB_FREQ(0.0, FB_FREQ_TYPE_GUESS),
			   loop_info.freq_positive);
    Cur_PU_Feedback->Annot_branch(wn_if, branch_info);
  }
  return wn_if_block; 
}
