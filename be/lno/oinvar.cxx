...
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/oinvar.cxx,v $ $Revision: 1.26 $";
...
static BOOL Process_Load(WN *load, BIT_VECTOR *result,DOLOOP_STACK *do_stack, 
			 INT num_elements,INVAR_TABLE *htable, MEM_POOL *pool,
			 bool outer_only, bool all_expr);
...
extern void Mark_Invar(WN *region, INT num_loops, DOLOOP_STACK *do_stack,
		       INVAR_TABLE *htable, MEM_POOL *pool, bool outer_only,
		       bool all_expr);
...
static void Gather_Invar(WN *inner, INT num_loops, DOLOOP_STACK *do_stack,
			 INVAR_TABLE *htable, WN_BV_STACK *invar_stack);
...
void Hoist_Outer_Invar(WN *wn_inner, INT num_loops, INT outer_reg_tile, BOOL can_tile)
{
  ...
  if (Cur_PU_Feedback)
    Cur_PU_Feedback->Verify("about to hoist invariants");
  ...
  if (Cur_PU_Feedback)
    Cur_PU_Feedback->Verify("finishded hoist invariants");
  if (LNO_Verbose) { 
    fprintf(stdout, 
...
// If all_expr, try to include in the table as many expressions as possible
//
		...
		bool all_expr)
{
  ...
  if (opcode == OPC_BLOCK) {
    ...
    while (kid) {
      WN *next = WN_next(kid);
      Mark_Invar(kid,num_loops,do_stack,htable,pool,outer_only,all_expr);
      kid = next;
    }
  } else if (OPCODE_is_expression(opcode)) {
    Mark_Expression(wn_inner,num_loops,do_stack,htable,pool,outer_only,all_expr);
  } else if (OPCODE_is_store(opcode)) {
    Mark_Invar(WN_kid0(wn_inner),num_loops,do_stack,htable,pool,outer_only,all_expr);
    if (WN_kid_count(wn_inner) == 2) {
      Mark_Invar(WN_kid1(wn_inner),num_loops,do_stack,htable,pool,outer_only,all_expr);
    }
  } else if (all_expr) {
    for (INT kid_idx = 0; kid_idx < WN_kid_count(wn_inner); kid_idx++) {
      Mark_Invar(WN_kid(wn_inner, kid_idx),num_loops,do_stack,htable,pool,outer_only,all_expr);
    }
  }
}
				   ...
				   bool all_expr)
{
  ...
  if (OPERATOR_is_load(oper)) {
    ...
    BOOL is_clean = Process_Load(wn,result,do_stack,num_loops,htable,pool,
				 outer_only, all_expr);
    if (is_clean) {
      if (all_expr || Contains_Work(wn)) {
        htable->Enter(wn,result);
      }
    }
    return result;
  } else if (oper == OPR_INTCONST || oper == OPR_CONST || oper == OPR_LDA) {
    BIT_VECTOR *result = CXX_NEW(BIT_VECTOR(num_loops,pool),pool);
    for (INT i=0; i<num_loops; i++) result->Set(i);
    if (all_expr) {
      htable->Enter(wn,result);
    ...
    // Mark all kids first. Use a copy of the result from the first kid for the current node.
    BIT_VECTOR *result = NULL;
    for (INT i = 0; i < WN_kid_count(wn); i++) {
      BIT_VECTOR *kid_res = Mark_Expression(WN_kid(wn, i), num_loops, do_stack, htable,
					    pool, outer_only, all_expr);
      if (i == 0 && kid_res != NULL) {
	result = CXX_NEW(BIT_VECTOR(num_loops, pool), pool);
	*result = *kid_res;
      }
    }
    ...
    if (WN_operator(WN_kid0(wn)) == OPR_PARM && !WN_Parm_By_Value(WN_kid0(wn))) {
      return NULL;
    }
    
    for (INT i = 1; i < WN_kid_count(wn); i++) {
      BIT_VECTOR *new_bv = htable->Find(WN_kid(wn, i));
			 ...
			 bool outer_only, bool all_expr)
{
  ...
  if (OPCODE_operator(opcode) == OPR_LDID) {
    ...
    WN *enclosing_loop = Enclosing_Do_Loop(load);
    INT enclosing_depth = Do_Depth(enclosing_loop);
    Is_True(enclosing_depth < do_stack->Elements(),
	    ("Enclosing loop %d >= do_stack->Elements() %d",
	     enclosing_depth, do_stack->Elements())); 
    
    INT outer_num_elements = num_elements - (do_stack->Elements() - (enclosing_depth + 1));
    ...
    BIT_VECTOR *tmp = NULL;
    if (messy || all_expr) {
      tmp = Mark_Expression(array, num_elements, do_stack, htable, pool,
			    outer_only, all_expr);
    }
    ...
    if (messy) {
      ...
      for (INT j = 0; j < aa->Num_Vec(); j++) {
        ...
        INT bad_symbolics = av->Non_Const_Loops() + outer_num_elements - av->Nest_Depth();
        for (INT i = 0; i < bad_symbolics; i++) {
	  result->Reset(i);
        }
        INT i0 = av->Nest_Depth() - outer_num_elements;
        for (INT i = MAX(0, bad_symbolics); i < outer_num_elements; i++) {
    ...
    for (EINDEX16 e = Array_Dependence_Graph->Get_In_Edge(v); e; 
	 e = Array_Dependence_Graph->Get_Next_In_Edge(e)) {
      ...
      INT maxlevel = MIN(da->Max_Level(),da->Num_Dim() + da->Num_Unused_Dim()-1);
      INT i0 = do_stack->Elements()-outer_num_elements;
      for (INT i = 0; i <= MIN(maxlevel - i0, outer_num_elements-1); i++) {
	result->Reset(i);
      }
    }
  }
...
static EQUIVALENCE_TYPE Sort_Invar_Expression(WN *wn, INVAR_TABLE *htable);
static void Sort_Equivalence_Class(WN *wn, INVAR_TABLE *htable);
static void Sort_Equivalence_Class_Rec(WN *wn, INVAR_TABLE *htable,
					WN_BV_STACK *wn_bv_stack);
static void Sort_Invar_Expressions_Rec(WN *wn, INVAR_TABLE *htable);
...
BOOL Sort_Invar_Expressions(WN *wn, INVAR_TABLE *htable)
{
