                 ...
                 INVAR_TABLE *invar_table)
{
  ...
  LNO_REGCLASS_INFO *regclass_info =
	CXX_NEW(LNO_REGCLASS_INFO(&LNO_local_pool,NULL),&LNO_local_pool);
  LNO_REGS *regs = CXX_NEW(LNO_REGS(regclass_info,&LNO_local_pool), &LNO_local_pool);
  ...
  test_model.Evaluate(inner_do_loop, se_needed, invar_table, &return_cost, regs);
  ...
  if (WN_first(WN_then(wn_if))) {
    ...
    then_model.Evaluate(inner_do_loop, se_needed, invar_table, &then_cost, regs);
  ...
  if (WN_first(WN_else(wn_if))) {
    ...
    else_model.Evaluate(inner_do_loop, se_needed, invar_table, &else_cost, regs);
				     ...
				     BOOL include_calls)
{
  ...
  INVAR_TABLE invar_table(500, &LNO_local_pool);
  Mark_Invar(WN_do_body(wn_inner), nloops, do_stack, 
             &invar_table, &LNO_local_pool, FALSE);
  
  machine_model.Evaluate(wn_inner, se_needed, &invar_table, &loop_cycles,regs); 
...
{
  INT parallel_debug_level = Get_Trace(TP_LNOPT2, TT_LNO_PARALLEL_DEBUG, 0)
...
INT PAR_STAT::Sanity_Check_Node(FILE* fp)
{
  ...
  PS_ASSERT(_next == NULL || _next->_prev == this, 
    (fp, "PAR_STAT: Bad _next pointer 0x%lx", this));  
  PS_ASSERT(_prev == NULL || _prev->_next == this, 
    (fp, "PAR_STAT: Bad _prev pointer 0x%lx", this));
  if (_parent != NULL) {
    PAR_STAT* ps = 0;
    for (ps = _parent->_first; ps != NULL; ps = ps->_next) 
      if (ps == this) 
        break; 
    PS_ASSERT(ps != NULL, (fp, "PAR_STAT: Bad _parent pointer 0x%lx", this));
  } 
  if (_first != NULL) { 
    PS_ASSERT(_last != NULL, 
      (fp, "PAR_STAT: _last not consistent with _first 0x%lx", this)); 
    PAR_STAT* pss = NULL; 
    for (PAR_STAT* ps = _first; ps != NULL; pss = ps, ps = ps->_next); 
    PS_ASSERT(pss == _last, 
      (fp, "PAR_STAT: _last not consistent with actual _last 0x%lx", this)); 
  } 
  if (_last != NULL) 
    PS_ASSERT(_first != NULL, 
      (fp, "PAR_STAT: _last not consistent with _first 0x%lx", this)); 
  PS_ASSERT(WN_opcode(_wn) == OPC_DO_LOOP || _first == NULL && _last == NULL,
    (fp, "PAR_STAT: STATEMENT is not leaf node 0x%lx", this));
  INT loop_count = 0; 
  for (PAR_STAT* ps = this; ps != NULL; ps = ps->_parent)
    if (WN_opcode(ps->_wn) == OPC_DO_LOOP)
      loop_count++; 
  ...
  PS_ASSERT(_depth == depth, (fp, "PAR_STAT: Improper depth 0x%lx", this)); 
  PS_ASSERT(WN_opcode(_wn) == OPC_DO_LOOP || !_is_parallel, 
    (fp, "PAR_STAT: _is_parallel set on STATEMENT 0x%lx", this)); 
  PS_ASSERT(WN_opcode(_wn) != OPC_DO_LOOP || _count > 0, 
    (fp, "PAR_STAT: _count <= 0 on DO LOOP 0x%lx", this));
  PS_ASSERT(_id <= id_count, (fp, "PAR_STAT: Improper _id value 0x%lx", this)); 
  PS_ASSERT(!_is_cloned || ps_root->Find(_wn, TRUE) != NULL, 
    (fp, "PAR_STAT: _is_cloned is set, but could not find original 0x%lx", 
    this));
  PS_ASSERT(WN_opcode(_wn) == OPC_DO_LOOP || !_is_cloned, 
    (fp, "PAR_STAT: _is_cloned set on STATEMENT 0x%lx", this));
  return error_count; 
}
