...
PF_VOLUME PF_LOOPNODE::Volume_For_Outer (mINT16 depth) {
  ...
  PF_PRINT(fprintf (TFile, "Vol-FO[0x%lx] depth (%d)\n",
                    this, depth));

  for (i=0; i<_child.Elements(); i++) {
    myvol += _child.Bottom_nth(i)->Volume_For_Outer(depth);
    PF_PRINT(fprintf (TFile, "      [0x%lx]: nest (%d)", this, i);
             myvol.Print (TFile));
    if (!myvol.Localized()) {
      // no longer localized - abort and return
      PF_PRINT(fprintf (TFile, "      [0x%lx]: not localized, return\n", this));
      return myvol;
    }
  }
  ...
  for (i=0; i<_bases.Elements(); i++) {
    ...
    PF_PRINT(
      fprintf (TFile, "      [0x%lx]: nest (%d)", this, i);
      myvol.Print (TFile);
    );
    if (!myvol.Localized()) {
      // no longer localized - abort and return
...
PF_VOLUME PF_LOOPNODE::Volume () {
  ...
  PF_PRINT(fprintf (TFile, "Volume[0x%lx]\n", this);)
  ...
  PF_PRINT(fprintf (TFile, "      [0x%lx]: now count local refs\n", this));
  ...
  for (i=0; i<_bases.Elements(); i++) {
    ...
    PF_PRINT(fprintf (TFile, "      [0x%lx]: ", this);
             _single_iter.Print (TFile));
  ...
  PF_PRINT(fprintf (TFile, "      [0x%lx]: now compute total iter\n", this));
...
void PF_LOOPNODE::Gen_Prefetch (PF_SPLIT_VECTOR* split_vec) {
  ...
  if (LNO_Analysis) {
    ...
    const char* var_name = ((ST_class(WN_st(index_wn)) != CLASS_PREG) ?
                      ...
                      (!Preg_Is_Dedicated(WN_offset(index_wn)) ?
...
void PF_LOOPNODE::Split_Loops (PF_SPLIT_VECTOR *split_vec) {
  ...
  if (loopnode == this) {
    ...
    if ((vec[_depth] > 1) && 
      ...
      extern WN_MAP version_map;
      PF_PRINT(fprintf (TFile, "Split loop [0x%lx] with stride %d\n",
                        this, vec[_depth]));
      // split this loop...
	...
	INT nloops = st_old.Elements();
      ...
      LWN_Copy_Frequency(expr, ifnode);
...
void PF_LOOPNODE::Print (FILE *fp) {
  ...
  for (i=0; i<_child.Elements(); i++) {
    fprintf (fp, "    0x%lx ", _child.Bottom_nth (i));
  }
  if (_child.Elements()) {
