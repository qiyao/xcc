...
void SINGLE_LOOP::Insert_Pref (WN* pragma_wn, WN* pf_wn) {
  ...
  VB_PRINT(printf ("Insert_Pref: Manual prefetch in loop 0x%lx: \n", this);
           dump_tree (pf_wn));
...
void SINGLE_LOOP::Insert_Ref  (WN* ref_wn) {
  ...
  VB_PRINT(printf ("Insert_Ref: Reference in loop 0x%lx: \n", this);
           dump_tree (ref_wn));
...
void SINGLE_LOOP::Collect_Refs_Prefs (WN* wn) {
  ...
  case OPR_PREFETCH:
  {
    ...
    VB_PRINT(printf ("Manual prefetch in loop 0x%lx: \n", this);
             dump_tree (wn));
    Insert_Pref (NULL, wn);
    break;
  }
