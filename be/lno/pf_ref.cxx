...
BOOL PF_LG::Is_Outer_Tile () {
  ...
  for (i=ref_depth; i!=_depth; i--) {
    dli = ln->Get_LoopInfo ();
//    printf ("(is_outer_tile: depth %d e_n_i %" LLD_FMT ")\n",
...
void PF_LG::Print (FILE *fp) {
  fprintf (fp, "        Locality group: (0x%lx)\n", this);
  fprintf (fp, "          depth       : %d\n", _depth);
  ...
  INT i;
  for (i=0; i<Get_Dim(); i++)
    fprintf (fp, "          %4"LLD_FMT" %3"LLD_FMT" %3"LLD_FMT"\n", 
	_c[i], _min_iter[i], _max_iter[i]);
  fprintf (fp, "          Distance (bytes): Min %" LLD_FMT ", Max %" LLD_FMT "\n",
           _min_dist, _max_dist);
  fprintf (fp, "          References in this LG (%d) and their vecs\n",
           _refvecs.Elements()+1);
  for (i=0; i<_refvecs.Elements(); i++) 
    _refvecs.Bottom_nth(i)->Print (fp);
  fprintf (fp, "          Done with Locality group (0x%lx)\n", this);
}
...
#   endif
  ...
  if (Debug_Prefetch) {
    fprintf (TFile, "Just constructed a new UGS: 0x%lx. symbol = ", this);
...
PF_VOLUME PF_UGS::Volume (mINT16 depth) {
  ...
  PF_PRINT(fprintf (TFile, "Volugs[0x%lx] depth (%d)\n",
                    this, depth););
  ...
  PF_PRINT(fprintf (TFile, "Volugs[0x%lx] depth (%d) vol ",
                    this, depth);
           myvol.Print (TFile);
  );
  return myvol;
...
void PF_UGS::ComputePFVec (PF_LEVEL level, PF_LOCLOOP locloop) {
  ...
  PF_PRINT ({
    fprintf (TFile, "ugs    [0x%lx]. Ref: ", this);
  ...
  PF_PRINT(fprintf (TFile, "    spatial locality, basis is 0x%lx\n", this);
           sKerHs.Print (TFile));
...
void PF_UGS::Print (FILE* fp) {
  ...
  for (i=0; i<_refs.Elements(); i++) {
    ...
    fprintf (fp, "          [%d] 0x%lx ", i, _refs.Bottom_nth (i));
    aa->Print (fp);
    fprintf (fp, "\n");
  }
