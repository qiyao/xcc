...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/reverse.cxx,v $ $Revision: 1.13 $";
#endif /* _KEEP_RCS_ID */
...
static void RV_Traverse(WN* wn_tree) 
{
  if (WN_opcode(wn_tree) == OPC_DO_LOOP) {
    ...
    if (LNO_Verbose) {
      ...
      if (!dli->Is_Backward) {
	fprintf(stdout, "Loop 0x%lx is indexed forward\n", wn_tree); 
	fprintf(TFile, "Loop 0x%lx is indexed forward\n", wn_tree); 
      } else {
	fprintf(stdout, "Loop 0x%lx is indexed backward\n", wn_tree); 
	fprintf(TFile, "Loop 0x%lx is indexed backward\n", wn_tree); 
      }
    }
  }
