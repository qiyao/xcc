...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/scalar_expand.cxx,v $ $Revision: 1.149 $";
#endif /* _KEEP_RCS_ID */
				...
				WN** bsz_addr)
{
  ...
  ST* symbol_st = symbol.St();
  INT sz = TY_size(ST_type(symbol_st));
		    ...
		    BOOL is_local_array)
{
  ...
  OPCODE ldop;
  if (is_local_array) {
    ldop = OPCODE_make_op(OPR_LDA, Pointer_type, MTYPE_V);
  } else {
    ldop = OPCODE_make_op(OPR_LDID, Pointer_type, Pointer_type);
  }
  ...
  if (is_local_array) {
    WN_array_base(wn_array) = WN_CreateLda(ldop, se_ptrsym.WN_Offset(),
      se_ptrsym.Type,se_ptrsym.St());
  } else {
		     ...
		     WN** alias_host)
{
  ...
  LWN_Copy_Frequency_Tree(wn, load);
  ...
  if (wn_newstdf) du->Add_Def_Use(wn_newstdf, WN_array_base(wn_array));
                      ...
                      WN** alias_host)
{
  ...
  LWN_Copy_Frequency_Tree(wn, store);
  ...
  MEM_POOL* pool, BOOL find_restrict, WN** wn_outer_loop)
{
  ...
  while (i<class_stack->Elements()) {
    ...
    if (ST_class(WN_st(scalar_ref)) == CLASS_PREG &&
        Preg_Is_Dedicated(WN_offset(scalar_ref))) {
      if (find_restrict) 
	*wn_outer_loop = NULL; 
      CXX_DELETE(class_stack, pool);
      return (STACK<WN*>*)NULL;
...
static TY_IDX se_type_array[MTYPE_MAX] = {

  (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL,
  (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL,
  (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL,
  (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL,
  (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL,
  ...
  (TY_IDX)NULL, (TY_IDX)NULL, (TY_IDX)NULL

}; 

// for top level scalar expansions of a constant size use local arrays rather than alloca 
// this didn't matter a lot for MIPS, but does for Xtensa because of low register count
static INT Make_Local_Array_If_Top_Const(SYMBOL *ptr_array, WN *allocregion, WN *bsz, 
						const SYMBOL& symbol)
{
  if (WN_operator(bsz) == OPR_INTCONST) { 
    WN *parent = LWN_Get_Parent(allocregion);
    if (WN_opcode(parent) == OPC_BLOCK && WN_opcode(LWN_Get_Parent(parent)) == OPC_FUNC_ENTRY) {
      WN *tmp=allocregion;
      BOOL bad_cf = false;
      while (tmp) {
        if (OPCODE_is_non_scf(WN_opcode(tmp))) bad_cf=true; 
        tmp = WN_prev(tmp);
      }
      if (!bad_cf) {
        TYPE_ID wtype = symbol.Type;
	ST *st = Create_Local_Array_ST("se", Be_Type_Tbl(wtype),
			    WN_const_val(bsz)/TY_size(Be_Type_Tbl(wtype)));
        *ptr_array = SYMBOL(st, 0, Make_Pointer_Type(Be_Type_Tbl(wtype)));
	return TRUE;
      }
    }
  }
			       ...
			       WN* deallocregion)
{
  ...
  //
  //    except if this is a top level loop with constant bounds then use a local array
  ...
  if (use_sp) {
    ...
    if (Cur_PU_Feedback) {
      FB_Info_Call fb_info(FB_FREQ_UNKNOWN);
      Cur_PU_Feedback->Annot_call(call, fb_info);
    } 
    ...
    //LWN_Copy_Frequency_Tree(allocregion,sp_tmp_def);
  ...
  //LWN_Copy_Frequency_Tree(allocregion, newstdf);
  ...
  //LWN_Copy_Frequency_Tree(deallocregion, newstdf);
		          ...
		          INT nstrips)
{
  ...
  // Step two:  Get the space if alloca/mallocing.
  // Create the scalar expanded variable.
  WN *newstdf=NULL;
  WN *deallocregion = deallocregion = guard_tests != NULL && guard_tests[0] != NULL
      ? guard_tests[0] : allocregion;
  SYMBOL se_ptrsym;
  if (!Make_Local_Array_If_Top_Const(&se_ptrsym, allocregion, bsz, symbol)) {
    char se[3]="se";
    SE_Symbols_For_SE(&se_ptrsym, se, unique_se_id, symbol.Type);
    Update_MP_Local_Var(se_ptrsym.St(), 0, LWN_Get_Parent(region));
    newstdf = Get_Expansion_Space(se_ptrsym, bsz, se, unique_se_id, 
  ...
  MEM_POOL_Push(&LNO_local_pool);
  {
	...
	bounds, findxs, has_lcd, -1, newstdf==NULL);
	...
	wn_array = SE_Array(se_ptrsym, sz, dimcnt, order, used_loops, 
	  ...
	  ? -1 : Do_Depth(wn) - Do_Loop_Depth(outerloop),newstdf==NULL);
	...
	wn_array = SE_Array(se_ptrsym, sz, dimcnt, order, used_loops, 
	  bounds, indxs, has_lcd, -1,newstdf==NULL);
    ...
    SE_Assign_Lexcounts(deflist, uselist, allocregion, deallocregion, 
      se_ptrsym); 
  ...
  if (Cur_PU_Feedback)
    Cur_PU_Feedback->Verify("Finished scalar expansion");
