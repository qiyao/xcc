...
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/sclrze.cxx,v $ $Revision: 1.33 $";
  ...
  for (e = dep_graph->Get_Out_Edge(v); e; e=next_e) {
    ...
    if (OPCODE_is_load(opcode)) {
	  ...
	  if (Dominates(store_wn,load_wn)) {
		...
		LWN_Copy_Frequency_Tree(store_wn, preg_store);
		...
		LWN_Copy_Frequency(store_wn,preg_load);
		...
		Promote_Type(type),type),preg_num,preg_st,Be_Type_Tbl(type));
	      LWN_Copy_Frequency_Tree(load_wn,new_load);
