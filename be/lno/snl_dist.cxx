...
static char *rcs_id =   snl_dist_CXX "$Revision: 1.71 $";
  ...
  for (i = imperfect; i > loopd; i--) {
    ...
    for (WN* wn = above ? WN_first(loopp) : WN_next(loop);
	 ...
	 wn = next) {
      ...
      //has the effect of marking the frequency info
      //as a guess, rather than definite, which
      //allows freq propagation to work better.
      LWN_Scale_Frequency_Tree(wn, 1.0);
    }
    ...
    newdo = LWN_CreateDO(LWN_Copy_Tree(WN_index(olddo)),
			 ...
			 newblk);
    if (Cur_PU_Feedback) {
      Cur_PU_Feedback->FB_duplicate_node(olddo, newdo);
    }
  ...
  for (LWN_ITER* iter = LWN_WALK_TreeIter(newdo);
	...
	for (i = 0; i < imperfect - loopd; i++) {
	  ...
	  if (loop_stmt == loop) {
	    ...
	    SNL_DEBUG2(3, "SNL_Distribute: loop_stmt(0x%lx)->0x%lx",
		       loop_stmt, newdos[i]);
	  }
	}
      } 
