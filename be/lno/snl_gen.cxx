...
static char *rcs_id =   snl_CXX "$Revision: 1.20 $";
...
extern SIMD_LOOP* Cur_Simd_Loop;  // current SIMD_LOOP for 2D Simd tile
...
// unprofitable to recompute them.
  ...
  if (Cur_PU_Feedback)
    Cur_PU_Feedback->FB_adjust_loop(wn, -1);
					                 ...
					                 BOOL* failed)
{
  ...
  *failed = FALSE; 
  ...
  LWN_Copy_Frequency(outerloop, wnif);
				       ...
				       BOOL below_is_distributable) 
{
  ...
  if (!Valid_SNL_Region(region)) 
    DevWarn("SNL_General_Distribution: Invalid SNL_REGION [0x%lx,0x%lx]"
					  ...
					  SNL_TRANS_INDEX_DATA* td)
{
  ...
  LWN_Copy_Frequency_Tree(WN_do_body(WN_first(loop[d2-1])), wn_if);
  if (above_do)
			            ...
			            BOOL from_genperm)
{
	...
	*nrle++ = *rle++;
	...
	FmtAssert(td->tdata[i].alias_wn,
      ...
      if (t->Rectangular()) {
        INT num = t->Stripsz(i);
	if (Cur_PU_Feedback) {
	  INT32 orig_count = WN_MAP32_Get(FB_Map, WN_start(innerloop));
	  if (orig_count>0) {
	    INT32 orig_test = WN_MAP32_Get(FB_Map, WN_end(innerloop));
	...
	if (Cur_PU_Feedback) {
      ...
      dli->SIMD_If_Convert_Directive = olddli->SIMD_If_Convert_Directive;
  ...
  if (Cur_PU_Feedback) {
    for (INT i = 0; i <= first_in_stack; ++i) {
      ...
      if (tile_loop) {
	if (WN_MAP32_Get(FB_Map, tile_loop) == 0) {
	  INT32 count = WN_MAP32_Get(FB_Map, cur_loop);
	  INT32 step  = WN_MAP32_Get(FB_Map, WN_step(cur_loop));
	  step = MAX(step/20,1);
	  ...
	  for (INT j = 0; j<stack.Elements(); j++) {
	    ...
	    if (inter_loop == tile_loop) {
	      ...
	      INT32 count1 = WN_MAP32_Get(FB_Map, inter_loop);
	      count = count1;
  ...
  if (!Valid_SNL_Region(region)) 
    DevWarn("SNL_GEN_U_Ctiling: Invalid SNL_REGION [0x%lx,0x%lx]",
      region.First, region.Last); 
  return region;
}
			 ...
			 BOOL imperfect)
{
  ...
  //LWN_Copy_Frequency_Tree(loop1, stmt);
...
//   print a warning.
//-----------------------------------------------------------------------
extern SNL_REGION SNL_GEN_2D_Regtile(SNL_NEST_INFO* ni, INT u, INT simd_u)
{
  INT tilesz = u;
  FmtAssert(tilesz > 1, ("2D register tiling size < 2"));
...

  {
    ...
    LWN_Copy_Frequency_Tree(loop0, nest_copy);
    if (Prompf_Info != NULL && Prompf_Info->Is_Enabled()) {
      STACK<WN*> st_old(&PROMPF_pool);
      STACK<WN*> st_new(&PROMPF_pool);
    ...
    if (Cur_PU_Feedback)
      Cur_PU_Feedback->FB_unroll_loop(nest_copy, tilesz, TRUE);

    if (red_manager) red_manager->Unroll_Update(wn_holder, 2);
    Unrolled_DU_Update(wn_holder, 2, Do_Loop_Depth(loop0)-1, TRUE, FALSE);

    /* copy the internal edges here so we won't create false invariant */
    if (!dg->Add_Deps_To_Copy_Block(loop0, nest_copy, TRUE)) {
      ...
      if (!Valid_SNL_Region(region))
        DevWarn("SNL_General_2D_Regtile: Invalid SNL_REGION [0x%lx,0x%lx]",
          region.First, region.Last);
      return region;
    }
    ...
    region = SNL_Regtile_Loop(loop0, u, 2, TRUE,
                              est_register_usage,
                              &ni->Privatizability_Info(),
                              ni->Depth_Inner() - 1, TRUE, NULL,
			      NULL, NULL);
    ...
    if (simd_u > 1 && region.First != region.Last) {
      WN *remainder = region.Last;
      Renumber_Loops(remainder, remainder, dg);
      SNL_REGION region_temp(remainder, remainder);
      Rebuild_Access_Arrays(region_temp, &SNL_local_pool);
      
      WN *remainder_inner = Find_Next_Innermost_Do(remainder);
      
      // vectorize the inner loop of the remainder loop
      SNL_REGION region2 = SNL_Regtile_Loop(remainder_inner, 1, 1, TRUE,
					    est_register_usage,
					    &ni->Privatizability_Info(),
					    ni->Depth_Inner(),
					    TRUE,
    ...
    if (Cur_PU_Feedback) {
      const FB_Info_Loop loop_info = Cur_PU_Feedback->Query_loop(nest_copy);
      FB_Info_Branch fb_info(loop_info.freq_iterate, FB_FREQ_ZERO);
      Cur_PU_Feedback->Annot_branch(if1, fb_info);
    }
    ...
    if (simd_u > 1) {
      Renumber_Loops(loop1, loop1, dg);
      SNL_REGION region_temp(loop1, loop1);
      Rebuild_Access_Arrays(region_temp, &SNL_local_pool);
      
      // vectorize the inner loop of the unrolled loop
      SNL_REGION region2 = SNL_Regtile_Loop(loop1, 1, 1, TRUE,
					    est_register_usage,
					    &ni->Privatizability_Info(),
					    ni->Depth_Inner(),
					    TRUE,
	...
	Prompf_Info->Register_Startup(old_ids, new_ids, nloops); 
      ...
      if (!dg->Add_Deps_To_Copy_Block(nest_copy, lpcopy, TRUE)) {
        SNL_DEBUG0(0, "Add_Deps_To_Copy_Block() failed -- continuing");
	...
	dli->Num_Iterations_Profile = FALSE;
      ...
      if (simd_u>1) { /* vectorize the inner loop */
	SNL_REGION region_t  = SNL_Regtile_Loop(wn, 1, 1, TRUE,
						est_register_usage,
						&ni->Privatizability_Info(),
						ni->Depth_Inner(),
						TRUE,
    ...
    if (uextra) {
      ...
      WN* lpcopy = LWN_Copy_Tree(nest_copy, TRUE, LNO_Info_Map);
      LWN_Copy_Frequency_Tree(nest_copy, lpcopy);
      if (Prompf_Info != NULL && Prompf_Info->Is_Enabled()) {
	STACK<WN*> st_old(&PROMPF_pool); 
	STACK<WN*> st_new(&PROMPF_pool); 
	Prompf_Assign_Ids(nest_copy, lpcopy, &st_old, &st_new, FALSE); 
	...
	Set_Loop_Statements(newbody, wn, wn_encloser);
      ...
      /*** We could vectorize these wind-down portions if simd_u > 1
           but we choose not to because it doesn't seem like it's
           worth it. Also, we may run into memory alignment problems. ***/

      LWN_Insert_Block_Before(WN_then(if1), NULL, lpcopy);
      LWN_Adjust_Frequency(if1, lpcopy);
      // the du info gets screwed up.  Handle the two uses of i in the bounds,
      // and all the uses of i'.
      Fix_Do_Du_Info(WN_start(lpcopy), NULL, TRUE, loop0, 0);
      Fix_Do_Du_Info(WN_end(lpcopy), NULL, TRUE, loop0, 0);
    ...
    if (simd_u>1) { /* vectorize the inner loop of nest_copy */
      WN *nest_copy_inner = Find_Next_Innermost_Do(nest_copy);

      /* the nest copy has been moved inside the 2D tiled loop.
	 First extract nest_copy from the block */
      WN *p_block = LWN_Get_Parent(nest_copy);
      WN *wn_next = WN_next(nest_copy);
      nest_copy   = LWN_Extract_From_Block(p_block, nest_copy);

      SNL_REGION region4  = SNL_Regtile_Loop(nest_copy_inner, 1, 1, TRUE,
					     est_register_usage,
					     &ni->Privatizability_Info(),
					     ni->Depth_Inner(),
					     TRUE,
      ...
      /* insert the nest_copy back */
      LWN_Insert_Block_Before(p_block, wn_next, nest_copy);
    }
...
//-----------------------------------------------------------------------
// NAME: SNL_GEN_2D_Simdtile
// FUNCTION: Perform a general 2D SIMD tiling on the loop nest speci-
//   fied by 'ni' using the tile size 'simd_u' for the SIMD tiled loop.
//-----------------------------------------------------------------------

extern SNL_REGION SNL_GEN_2D_Simdtile(SNL_NEST_INFO* ni, INT simd_u)
{
  INT tilesz = simd_u;
  FmtAssert(tilesz > 1, ("2D SIMD tiling size < 2"));
...

  {
    ...
    wn_holder[0] = loop0;
    wn_holder[1] = nest_copy;
    LWN_Scale_Frequency_Tree(nest_copy, 1.0/tilesz);
    ...
    BOOL		lextra = lmono != SNL_MONO_INVARIANT;
    BOOL		uextra = umono != SNL_MONO_INVARIANT;
    ...
    // Extract L* and U* first
    lstid = LWN_Extract_From_Block(lstid);
    ustid = LWN_Extract_From_Block(ustid);

    // Do the inner SIMD unrolling.
    region = SNL_Regtile_Loop(loop0, 1, 2, TRUE,
                              est_register_usage,
                              &ni->Privatizability_Info(),
                              ni->Depth_Inner() - 1, TRUE, NULL,
			      NULL, NULL, simd_u);

    // store the initialization and finalization codes of the
    // SIMDizer inner loop
    STACK<WN*> above_loop1(&SNL_local_pool);
    STACK<WN*> below_loop1(&SNL_local_pool);
    {
      WN*  wnprnt = LWN_Get_Parent(loop1);
      WN*  wnnext = NULL;
      bool is_above = true;
      for (WN* wn = WN_first(wnprnt); wn; wn = WN_next(wn)) {
	  if (wn == loop1) {
	    is_above = false;
	  } else if (is_above) {
	    above_loop1.Push(wn);
    ...
    // also get the finalization code after loop0 to below_loop1
    for (INT i = 0; i < Cur_Simd_Loop->Post_Loop().Elements(); i++) {
      WN *wn = Cur_Simd_Loop->Post_Loop().Bottom_nth(i);
      below_loop1.Push(wn);
    }
    ...
    // Inset L* and U* back
    WN* p1block = LWN_Get_Parent(loop1);
    LWN_Insert_Block_Before(p1block, loop1, lstid);
    LWN_Insert_Block_Before(p1block, loop1, ustid);
    ...
    // The above_loop1 goes before loop1
    for (INT i = above_loop1.Elements() - 1; i >= 0; i--) {
      WN *cur = above_loop1.Bottom_nth(i);
      LWN_Extract_From_Block(LWN_Get_Parent(cur), cur);
      LWN_Insert_Block_After(WN_then(if1), NULL, cur);
    }

    // The below_loop goes after loop1
    for (INT i = 0; i < below_loop1.Elements(); i++) {
      WN *cur = below_loop1.Bottom_nth(i);
      LWN_Extract_From_Block(LWN_Get_Parent(cur), cur);
      LWN_Insert_Block_Before(WN_then(if1), NULL, cur);
    }
	...
	Set_Loop_Statements(newbody, wn, wn_encloser);
      ...
      // Replace scalar reference with scalar expanded array
      Cur_Simd_Loop->Set_TD_Rem_Loop(lpcopy);
      Cur_Simd_Loop->Substitute_Scalar_Expansion(lpcopy);
