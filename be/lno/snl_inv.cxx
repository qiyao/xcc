...
static char *rcs_id =   snl_CXX "$Revision: 1.17 $";
			     ...
			     MEM_POOL* pool)		
{
  ...
  BOOL          is_concurrent[SNL_MAX_LOOPS];
  BOOL          is_simd_if_convert[SNL_MAX_LOOPS];
  ...
  for (i = 0; i < nloops; i++) {
    ...
    is_concurrent[i] = dli[i]->Concurrent_Directive;
    is_simd_if_convert[i] = dli[i]->SIMD_If_Convert_Directive;
  }
  ...
  for (i = 0; i < nloops; i++) {
    ...
    if (dli[permutation[i]]->Concurrent_Directive) {
      for (INT ii = i+1; ii < nloops; ii++) {
	if (permutation[i] < permutation[ii] && !is_concurrent[ii]) {
	  dli[permutation[i]]->Concurrent_Directive = FALSE;
	  break;
	}
      }
    }
			       ...
			       BOOL report_prompf)
{
  ...
  for (INT s = 0; s < strips; s++) {
    ...
    if (Cur_PU_Feedback) {
      ...
      INT32 outer = WN_MAP32_Get(FB_Map, permloop[0]);
      INT32 outer_trip = WN_MAP32_Get(FB_Map, WN_step(permloop[0]));
      ...
      INT32 count = WN_MAP32_Get(FB_Map, stripstep);
      INT32 tripc = count/MAX(1,WN_MAP32_Get(FB_Map, stripdo));
    ...
    dli->Has_Non_Intrinsic_Calls = olddli->Has_Non_Intrinsic_Calls;
...
static SNL_REGION SNL_INV_Register_Tile(INT nloops,
				  ...
				  EST_REGISTER_USAGE est_register_usage, 
                                  INT depth_inner,
                                  SX_INFO* pinfo,
                                  const INT* simdstripsz = NULL)
{
  SNL_REGION   region(permloop[0], permloop[0]);
  ARRAY_DIRECTED_GRAPH16*       dg = Array_Dependence_Graph;

  if (regstripsz == NULL && simdstripsz == NULL) {
    if (!Valid_SNL_Region(region))
      DevWarn("SNL_INV_Register_Tile: Invalid SNL_REGION [0x%lx,0x%lx]",
	region.First, region.Last);
    return region;
  } 

  INT i;
  BOOL update_access_simd=FALSE;
  for (i = 0; i < nloops; i++) {
    if (simdstripsz && simdstripsz[i]) {
	update_access_simd=TRUE;
    }
  }
  if (simdstripsz && simdstripsz[i]) {
      Renumber_Loops(region.First, region.Last, dg);
  ...
  for (i = 0; i < nloops; i++) {
    ...
    Is_True(i!=nloops-1 || !regstripsz || regstripsz[i]<=1,
	    ("Innermost loop has register blocking strip"));
    
    if ((regstripsz == NULL || regstripsz[i] <= 1) && 
	(simdstripsz == NULL || simdstripsz[i] <= 1)) {
      Is_True(!regstripsz || regstripsz[i] == 1, 
	      ("Unroll quantity %d", regstripsz[i]));
      continue;
    }

    HASH_TABLE<WN*,WN*>* ht=NULL;
    INT depth = depth_inner - nloops + 1 + i;
    SX_INFO* pinfo2=NULL;
    SNL_REGION region2 = SNL_Regtile_Loop(permloop[i], 
					  (regstripsz) ? regstripsz[i] : 0,
                                          nloops-i, FALSE,
                                          est_register_usage,
                                          pinfo, depth, FALSE,
					  permloop,
                                          &ht, &pinfo2,
					  ((simdstripsz) ? simdstripsz[i] :0));
    if (i == 0)
      region = region2;

    // somewhat redundant, but necessary for the further calls to
    // SNL_Regtile_Loop() (and SIMD) inside this loop

    Renumber_Loops(region2.First, region2.Last, dg);
    Rebuild_Access_Arrays(region2, &SNL_local_pool);

    // call this routine on wind-down loops, to get more register
    // unrolling, when conditions are favorable.
    
    if (LNO_Outer_Unroll_Deep && ht && pinfo2 &&
	(simdstripsz && (i < nloops-1) || i < nloops-2)) {
      ...
      SNL_REGION region2 = SNL_INV_Register_Tile(nloops-i-1,
						 (regstripsz) ? regstripsz+i+1 : NULL, 
						 permloop2+i+1, est_register_usage,
						 depth_inner, pinfo2,
						 ((simdstripsz) ? simdstripsz+i+1 : NULL));
  ...
  for (i = 0; i < nloops-1; i++) {
    if ((regstripsz && regstripsz[i] > 1 || simdstripsz && simdstripsz[i] > 1)
	&& Iterations(permloop[i], &SNL_local_pool) == 1) {
      found_unity = TRUE;
				     ...
				     INT simdstripsz[]) 
{
  ...
  if (!Valid_SNL_Region(region)) 
    DevWarn("SNL_Invariant_Transforms: Invalid SNL_REGION [0x%lx,0x%lx]",
      region.First, region.Last);
  return region;
}
