...
static char *rcs_id =   snl_nest_CXX "$Revision: 1.77 $";
...
static BOOL SNL_Is_Transformable(WN* wn, INT outer_depth, DO_LOOP_INFO* dli)
{
  ...
  if (live_at_entry) { 
    SNL_DEBUG2(3, "Loop %s(0x%lx) not transformable: live on entry/exit",
               index_name, wn);
    return FALSE;
  }
  ...
  if (WN_end(wn) == NULL) {
    FmtAssert(0, ("Loop %s (0x%lx) has missing upper bound", index_name, wn));
  }
  ...
  if (Mono(SNL_UBexp(WN_end(wn)), index_symbol) != SNL_MONO_INVARIANT ||
    ...
    DevWarn("Loop %s (0x%lx) has surprising form for upper bound",
            index_name, wn, WN_opcode(WN_end(wn)));
    return FALSE;
  }
  ...
  _problem(NULL)
{
  ...
  if (nloops == 0 || (nloops == 1 && (Simd_Target == SIMD_TARG_UNKNOWN)))
    goto return_point;
