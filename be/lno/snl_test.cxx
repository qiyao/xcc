...
/** $Revision: 1.141 $
*** $Date: 2000/08/30 17:21:32 $
*** $Author: nenad $
*** $Source: /isms/cmplrs.src/osprey1.0/be/lno/RCS/snl_test.cxx,v $
**/
...
static char *rcs_id =   snl_test_CXX "$Revision: 1.141 $";
...
static BOOL Est_Num_Iters_Suspect(DOLOOP_STACK* stack)
{
  ...
  if (depth == 0) {
    /* single level loop, not a suspect. */
    Is_True(Simd_Target != SIMD_TARG_UNKNOWN, ("depth == 0"));
    return FALSE;
  }
                                              ...
                                              BOOL* changed)
{
  ...
  *changed = FALSE;
  ...
  for (i = 0; i < smat.Nloops(); i++) {
    ...
    /* allow single level loop to pass through when SIMD is on */
    if ((Simd_Target != SIMD_TARG_UNKNOWN) && smat.Nloops() == 1 &&
	!sinfo.Body_Deps().All_Stars() && !dli->Has_Bad_Mem)  {
      Is_True(i==0, ("Index i steps out of bound"));
      break;
    }

    if (i < smat.Nloops() - 1) {
      if (dli->Has_Bad_Mem) {
        SIMD_Msg(AT_MSG_SIMD_BAD_LOOP,  ni->Dostack().Bottom_nth(depth));
      } else {
        SIMD_Msg(AT_MSG_SIMD_DATA_DEPENDENCE, ni->Dostack().Bottom_nth(depth));
      }
    }
    ...
    for (INT j = sinfo.Body_Deps().Bad_Deps().Lastidx(); j >= 0; j--) {
      if (sinfo.Body_Deps().Bad_Deps()[j].loop == i) {
        ...
        for (INT ii = 0; ii <= i; ii++) {
          ...
          fprintf(LNO_Analysis,
                  ii==0 ? "%d" : " %d", Srcpos_To_Line(WN_Get_Linenum(lp)));
        }
  ...
  Is_True(i<=smat.Nloops(),("i is more than the number of loops."));
  if (i == smat.Nloops() ||
      sinfo.Body_Deps().All_Stars() ||
    ...
    if (!Valid_SNL_Region(region))
      DevWarn("Do_Automatic_Transformation: Invalid SNL_REGION [0x%lx,0x%lx]",
  ...
  LOOP_MODEL lm;
  {
    INVAR_TABLE htable(500,&LNO_local_pool);
    if (LNO_Run_Oinvar && trying_invariant) {
	  ...
	  Sort_Invar_Expressions(innerloop,&htable)) {
        INVAR_TABLE htable2(500,&LNO_local_pool);
        ...
        lm.Model(innerloop, can_be_inner, can_be_unrolled,
                 outermost_can_be_tiled, trying_invariant,
  ...
  for (outermost = 0; outermost < ni->Depth_Inner(); outermost++) {
	...
	(lm.Simd_Loop() && lm.Simd_Loop()->Simd_Unroll()[outermost] != 0))
      break;
  }
  ...
  if (outermost >= ni->Depth_Inner() && 
      (lm.Simd_Loop()==NULL || lm.Simd_Loop()->Simd_Unroll()[outermost]==0)) {
  ...
  /* SIMD related data structure */
  BOOL  simdstripping = FALSE;
  INT   simdstripsz[SNL_MAX_LOOPS];
  ...
  for (i = outermost; i <= ni->Depth_Inner(); i++) {
    ...
    if (!LNO_Outer_Unroll_Model_Only
      ...
      INT idx = order[i-outermost] + outermost;
      regstripsz[i-outermost] = lm.Block_Number(idx);
      if (i-outermost < nloops-1 && regstripsz[i-outermost] != 1)
        regstripping = TRUE;
      if (lm.Simd_Loop()) {
	simdstripsz[i-outermost] = lm.Simd_Loop()->Simd_Unroll()[idx];
	if (i-outermost <= nloops-1 && simdstripsz[i-outermost] > 1) {
	  simdstripping = TRUE;
	}
      } else {
	simdstripsz[i-outermost] = 1;
      }
    }
    else {
  ...
  /* disable register blocking for loops below SIMD loop, it may run
   into trouble understanding the updating/alignment load/store */
  if (simdstripping == TRUE && regstripping == TRUE) {
    INT simd_depth = 0;
    for (i = 0; i < nloops; i++) {
      if (simdstripsz[i] > 1) {
	simd_depth = order[i];
    ...
    for (i = 0; i < nloops; i++) {
      INT reg_depth = order[i];
      if (regstripsz[i] > 1 && reg_depth > simd_depth) {
	regstripsz[i] = 1;
      }
    }
  }
  ...
  if (!reordering && !cachestripping && !regstripping && !simdstripping) {
    DO_LOOP_INFO* dli = Get_Do_Loop_Info(innerloop);
    dli->Est_Register_Usage = reg_usage;
    if (!Valid_SNL_Region(region))
      DevWarn("Do_Automatic_Transformation: Invalid SNL_REGION [0x%lx,0x%lx]",
  ...
  INT* simdstripszp = simdstripping ? simdstripsz : NULL;
  ...
  if (trying_invariant) {
    ...
    if (ti == NULL || ti->Nloops() == nloops) {
      region = SNL_INV_Transforms(wn_outer, orderp, ni, nloops, ti, 
        ...
        &rg_kernel, simdstripszp);
    }
    else if (!regstripszp && !simdstripszp) {
	...
	if (might_reset_last && rg_local.Last != region.Last)
      ...
      INT sd1[SNL_MAX_LOOPS];
      INT sd2[SNL_MAX_LOOPS];
      INT i;
	...
	sd2[i-(nloops-ti->Nloops())] = simdstripsz[i], sd1[i] = 1;
      }
	
      region = SNL_INV_Transforms(wn_outer, orderp, ni, nloops, NULL, 
	rg1, reg_usage, want_se_and_dist, NULL,FALSE, &rg_kernel, sd1);
      DOLOOP_STACK kernel_stack(&LNO_local_pool);
      Find_Kernel_Stack(&rg_kernel, ti->Nloops(), &kernel_stack);
      for (i = 0; i < kernel_stack.Elements(); i++) {
        WN* wn_new_outer = kernel_stack.Bottom_nth(i);
...

  {
    ...
    if (!(simdstripsz[nloops-2] > 1)) {
      /* don't perform scalar expansion and distribution when vectorizing
	 outer loop */
      BOOL has_lcd = ni->Privatizability_Info().Lcd_Depth() != -1;
      SNL_REGION dregion = 
	SNL_GEN_Distribution
	...
	if (uup == NULL || ti == NULL || nloops == ti->Nloops())
	  ctregion = 
	    SNL_GEN_U_Ctiling
	    (ni->Dostack().Bottom_nth(firstin), nloops, 
	     uup, ti, ni->Bi(), &(ni->Privatizability_Info().Plist), 
	     reg_usage, TRUE, FALSE);
	else {
	  // in case you want to do a permutation, and then tile only some
	  // of the inner loops.
	  ctregion = SNL_GEN_U_Ctiling
	    (ni->Dostack().Bottom_nth(firstin), nloops, uup, NULL, ni->Bi(),
	     &(ni->Privatizability_Info().Plist), reg_usage, TRUE, FALSE);
	  (void) SNL_GEN_U_Ctiling
	    (ni->Dostack().Bottom_nth(firstin), nloops, 
    ...
    if (simdstripsz[nloops-2] > 1) {
      SNL_REGION regregion = SNL_GEN_2D_Simdtile(ni, simdstripsz[nloops-2]);
...
static void SNL_Bound_Non_Const_Level(WN *wn, INT &non_const_level)
{
  FmtAssert(WN_opcode(wn) == OPC_DO_LOOP, ("Requires DO"));
  DO_LOOP_INFO *dli = Get_Do_Loop_Info(wn);

  ACCESS_ARRAY *lb_a = dli->LB;
  if (lb_a->Non_Const_Loops() > non_const_level) {
    non_const_level = lb_a->Non_Const_Loops();
  }

  ACCESS_ARRAY *ub_a = dli->UB;
  if (ub_a->Non_Const_Loops() > non_const_level) {
    non_const_level = ub_a->Non_Const_Loops();
  }
}
		          ...
		          INT nloops)
{
  ...
  while (nloops >= 2 && wn != NULL &&
	 (!Do_Loop_Is_Good(wn) ||
	  Do_Loop_Has_Non_Intrinsic_Calls(wn) ||
	  Do_Loop_Has_Gotos(wn))) {
    SIMD_Msg(AT_MSG_SIMD_BAD_LOOP,  wn);
    nloops--;
    wn = Good_Do_Next_Innermost(wn);
    // FmtAssert(wn, ("Can't find loop in deep loop nest"));
  }
  ...
  // Exclude outer loop that contains hoisted bounds for inner loops
  // for (k = 0; k < n; k++) {
  //     _ab1 = (1<<n);
  //     _ab2 = (1<<k);
  //    for (j = 0; j < _ab2; j++) {
  ...
  // }
  //
  // loop k needs to be excluded from the SNL nests 
  //
  
  INT non_const_level = 0;
  for (WN *wn_in = Good_Do_Next_Innermost(wn); wn_in != NULL; 
       wn_in = Good_Do_Next_Innermost(wn_in)) {
    SNL_Bound_Non_Const_Level(wn_in, non_const_level);
  }
  if (non_const_level > Do_Depth(wn)) {
    nloops = nloops  - (non_const_level - Do_Depth(wn));
    MEM_POOL_Push(&LNO_local_pool);
    {
      DOLOOP_STACK stack(&LNO_local_pool);
      Build_Doloop_Stack(wn_inner, &stack);
      wn = stack.Bottom_nth(non_const_level);
    }
  ...
  if (nloops <= 0 ||
      (nloops == 1 && 
       ((Simd_Target == SIMD_TARG_UNKNOWN) || /* don't SIMD if inner loop has a problem */
	ni.Problem(ni.Depth_Inner()).Problem!=SNL_LOOP_PROBLEM_NONE &&
	ni.Problem(ni.Depth_Inner()).Problem!=SNL_LOOP_PROBLEM_SCALAR))) {
    if ((Simd_Target != SIMD_TARG_UNKNOWN) && nloops == 1) {
      SIMD_Msg(AT_MSG_SIMD_DATA_DEPENDENCE, ni.Dostack().Bottom_nth(ni.Depth_Inner()));
    }
    if (LNO_Verbose) {
  ...
  FmtAssert((nloops >= 2 || (Simd_Target != SIMD_TARG_UNKNOWN)) &&
  ...
  if (Simd_Info && Simd_Info->AT_Analysis_Phase()) {
    Simd_Info->SIMD_At()->Reset_Analyzed();
  }
  ...
  bool analyzed = false;
  if (Simd_Info && Simd_Info->AT_Analysis_Phase()) {
    analyzed = Simd_Info->SIMD_At()->Analyzed();
  }
  
  if (Simd_Target != SIMD_TARG_UNKNOWN && !changed && !analyzed) {
    if (nloops <= 1) {
      if (LNO_Verbose) {
	printf("SIMD not transforming depth 1 loop. \n");
      }
    } else {
      SNL_Transform(Find_Next_Innermost_Do(wn_original), nloops_original - 1); 
    }
...
extern void SNL_Phase(WN* func_nd)
{
  ...
  for (INT i = 0; i < ffi->Num_Snl(); i++) {
    ...
    /* Mark the SIMD pools for current SNL */
    SIMD_Push_Pool();

    SNL_Upper_Bound_Standardize(wn, nloops); 
    Hoist_Bounds_One_Level(wn); 
    SIMD_Reverse_Backward_Loops(wn);
    SNL_Transform(wn, nloops);
    if (LNO_Analysis)
      fprintf(LNO_Analysis,")\n");

    /* Release the SIMD pools after current SNL */
    SIMD_Pop_Pool();
  }

  if (snl_cnt == 0) {
    SIMD_Msg(AT_MSG_SIMD_NO_SNL, func_nd);
  }
#if 0  
  /* Delete the SIMD pools */
  SIMD_Finalize();
#endif
}
