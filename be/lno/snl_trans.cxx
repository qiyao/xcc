...
static char *rcs_id =   snl_CXX "$Revision: 1.255 $";
                                            ...
                                            WN* to)
{
  ...
  SNL_DEBUG2(3, "SNL_Change_Reduction_Loop_Stmts(0x%lx, 0x%lx)", from, to);
				      ...
				      INT blex)
{
  ...
  Is_True(awn == dg->Get_Wn(dg->Get_Source(e)),
          ("Bad awn 0x%lx 0x%lx", awn, dg->Get_Wn(dg->Get_Source(e))));
  Is_True(bwn == dg->Get_Wn(dg->Get_Sink(e)),
          ("Bad bwn 0x%lx 0x%lx", bwn, dg->Get_Wn(dg->Get_Sink(e))));
					     ...
					     BOOL update_access)
{
  ...
  if (!Valid_SNL_Region(region))
    DevWarn("SNL_Remove_Unity_Trip_Loop: Invalid SNL_REGION [0x%lx,0x%lx]",
			      ...
			      WN* wn_next) 
{
  if (!Valid_SNL_Region(*local_region))
    DevWarn("RUL_Region_Update: Invalid Local SNL_REGION [0x%lx,0x%lx]",
  ...
  if (local_region->First == NULL) { 
    ...
    if (region->Last == wn)
      region->Last = wn_prev; 
    if (!Valid_SNL_Region(*region))
      DevWarn("RUL_Region_Update: Invalid SNL_REGION [0x%lx,0x%lx]",
	region->First, region->Last);
    return; 
  } 
  ...
  if (!Valid_SNL_Region(*region))
    DevWarn("RUL_Region_Update: Invalid Input SNL_REGION [0x%lx,0x%lx]",
      region->First, region->Last);
}
                                           ...
                                           BOOL update_access)
{
  ...
  if (WN_opcode(wn_tree) == OPC_DO_LOOP) {
	...
	if (wn == old_region_last)
      ...
      if (!Valid_SNL_Region(region))
        DevWarn("SNL_Remove_Useless_Loops: Invalid SNL_REGION [0x%lx,0x%lx]",
          region.First, region.Last);
      return region; 
    } 
	...
	region.Last = NULL; 
      ...
      if (!Valid_SNL_Region(region))
        DevWarn("SNL_Remove_Useless_Loops: Returning SNL_REGION [0x%lx,0x%lx]",
          region.First, region.Last);
      return region;
    }
  }
...
LNO_FB_MP_Tile(WN* wn_tile_loop, INT tile_loop_tripcount, WN *wn_orig_loop)
{
  ...
  if (new_orig_fil.freq_iterate.Value() >=
    ...
    new_orig_fil.freq_zero = new_orig_fil.freq_exit * 0.1;
                     ...
                     MEM_POOL *pool)
{
  ...
  if (Cur_PU_Feedback) {
    Cur_PU_Feedback->FB_tile_loop(wn_loop, outer_tile, stripsz[0]);
  }
				   ...
				   INT simd_u)
{
  ...
  if (simd_u > 1) {
    return SNL_Simd_Loop(outerloop, simd_u, nloops, unroll_just_inner, 
			 est_register_usage, pinfo, pinfo_depth, 
			 no_further_unroll, permloop,
			 loop_map_ptr, wdpinfo_ptr);
  }
  ...
  if (do_winddown) {
    ...
    wdloop = Wind_Down(outerloop, u, FALSE, ru);
    region.Last = wdloop;
  ...
  if (Cur_PU_Feedback) {
    Cur_PU_Feedback->FB_unroll_loop(outerloop, u, true /* jamming */);
  }
  INT i;
  for (i = 1; i < u; i++) {
  ...
  if (!Valid_SNL_Region(region))
    DevWarn("SNL_Regtile_Loop: Invalid SNL_REGION [0x%lx,0x%lx]",
      region.First, region.Last);
  return region;
}
