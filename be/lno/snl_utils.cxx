...
static char *rcs_id =   snl_utils_CXX "$Revision: 1.144 $";
...
static void Check_Zero_Linear_Coefficients(WN* wn_ref, ACCESS_VECTOR *av)
{
  ...
  for (INTSYMB_NODE *lin_node=lin_iter.First(); !lin_iter.Is_Empty();
    ...
    SC_ASSERT(lin_node->Coeff != 0, 
      ("Access vector for 0x%lx has 0 linear coefficient", wn_ref));  
}
...
void Increase_By(WN* wn, INT c, WN* parent, INT kid)
{
  ...
  switch (opr) {
   ...
   default:
    ...
    LWN_Copy_Frequency_Tree(wn, exp);
...
static SANITY_CHECK_RVAL SNL_Sanity_Check_Loop(WN* wn, INT depth)
{
  ...
  SC_ASSERT(dli->LB && dli->UB && dli->Step,
	    ("sanity check failed: Missing LB, UB or Step (0x%lx)", wn));
  ...
  if (!dli->LB->Too_Messy) {
    for (INT i = 0; i < dli->LB->Num_Vec(); i++) {
      ...
      if (!av->Too_Messy) {
        ...
        SC_ASSERT(lval < 0,
                  ("sanity check failed: lbval=%d depth=%d index=%s (0x%lx)",
  ...
  if (!dli->UB->Too_Messy) {
    for (INT i = 0; i < dli->UB->Num_Vec(); i++) {
      ...
      if (!av->Too_Messy) {
        ...
        SC_ASSERT(uval > 0,
                  ("sanity check failed: ubval=%d depth=%d index=%s (0x%lx)",
  ...
  if (!r.has_io) {
    SC_ASSERT((r.has_loops == FALSE) == (dli->Is_Inner != FALSE),
              ("sanity check failed: index=%s (0x%lx): has_loops: %d %d",
...
WN* SNL_Sanity_Check_Exp(WN* wn)
{
  ...
  if (OPCODE_is_store(opc) || OPCODE_is_load(opc)) {
    if (opr == OPR_LDID && WN_st(wn) == Tie_Output_Volatile_Preg) {
      ;
    } else if (OPCODE_has_1ty(opc) && TY_is_volatile(WN_ty(wn))) {
      rval = wn;
  ...
  if (opr == OPR_STID) {
	...
	&& !ST_has_nested_ref(WN_st(wn))
      ...
      fprintf(stdout,
              "sanity check warning(%s): missing uses for def (0x%lx) of %s <%s>\n",
    ...
    if ((!dl || dl->Len() == 0) && !Is_Tie_Output_Parm(wn)) {
      fprintf(stdout, "sanity check warning(%s): missing defs for use ",
        Cur_PU_Name);
      fprintf(stdout, "(0x%lx) of %s <%s> <id %d:%d>\n", wn, SYMBOL(wn).Name(),
  ...
  for (WN* wn = WN_first(body); wn; wn = WN_next(wn)) {
    ...
    switch (opc) {
     ...
     case OPC_REGION:
      ...
      break;
     case OPC_ASM_STMT:
       // Skip ASM statements completely
       break;
     default:
...
void SNL_Sanity_Check_Region(SNL_REGION region)
{
  ...
  if (!Valid_SNL_Region(region)) { 
    DevWarn("SNL_Sanity_Check_Region: Invalid SNL region 0x%lx->0x%lx", 
  ...
  if (u) {
    for (v = 0; v < i_nloops; v++) {
      ...
      for (INT j = 0; j < i_nloops; j++) {
        if ((*u)(v,j)) {
          wtype = Promote_Type(Max_Wtype(wtype, idata[j].pre_symbol.Type));
          if (first_non_zero == -1)
            first_non_zero = j;
        }
      }
                             ...
                             INT only_in_nloops)
{
  ...
  if (opr == OPR_DO_LOOP) {
    FmtAssert(Du_Mgr->Du_Get_Use(WN_start(wn)),
              ("Didn't find uses for %s", WN_index(wn)));
    FmtAssert(Du_Mgr->Du_Get_Use(WN_step(wn)),
              ("Didn't find uses for %s", WN_step(wn)));
  }
}
...
extern void Rebuild_Access_Arrays(SNL_REGION region, 
				  MEM_POOL* pool)
{
