...
{ 
  ...
  LWN_Copy_Frequency(outer_tile_loop,newbegin);
					  ...
					  BOOL want_normal) 
{
  for (INT i = 0; i < inner_tile_stack->Elements(); i++) { 
    ...
    LWN_Copy_Frequency_Tree(WN_kid1(WN_end(wn_inner)), wn_new);
						...
						WN* inner_tile_loop)
{
  ...
  if (WN_operator(lower_bound) == OPR_INTCONST
    ...
    LWN_Copy_Frequency(WN_start(inner_tile_loop), wd_lower_bound);
...
{ 
   ...
   if (Cur_PU_Feedback) {
     ...
     INT32 loop_count = WN_MAP32_Get(FB_Map, WN_start(outer_tile_loop));
     INT32 trip_count = WN_MAP32_Get(FB_Map, WN_step(outer_tile_loop));
     if (loop_count>0) {
   ...
   if (LNO_Verbose) {
     ...
     for (i = 0; i < inner_tile_stack->Elements(); i++) { 
       fprintf(stdout, "0x%lx", inner_tile_stack->Bottom_nth(i));  
     ...
     for (i = 0; i < inner_tile_stack->Elements(); i++) { 
       fprintf(TFile, "0x%lx", inner_tile_stack->Bottom_nth(i));  
       if (i < inner_tile_stack->Elements() - 1)
	 fprintf(TFile, ",");
     }
     fprintf(TFile, ")\n");
