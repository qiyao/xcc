                 ...
                 MEM_POOL* pool) : Plist(pool)
{
  ...
  for (const SX_PNODE* n = ii.First(); !ii.Is_Empty(); n = ii.Next()) {
    ...
    if (n->Reduction_Carried_By()) {
      ...
      FmtAssert(red, ("Loop 0x%lx(%s) had bad pnode reduction",
			         ...
			         WN* wn_loop_stmt) 
{
  ...
  const INT bufsz = 64;
  char buf[bufsz];
  FmtAssert(parent, ("Bad loop_stmt %s(0x%lx) for %s(0x%lx)",
