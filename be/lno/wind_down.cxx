                     ...
                     EST_REGISTER_USAGE est_register_usage)
{
  ...
  WN_kid0(newbegin) = LWN_CreateLdid(OPCODE_make_op(OPR_LDID, Promote_Type(wtype0), wtype0),
                                     WN_start(loop));
  LWN_Set_Parent(WN_kid0(newbegin), newbegin);
  Fix_Do_Du_Info(newbegin, NULL, FALSE, loop, 0);

  DO_LOOP_INFO* dli = Get_Do_Loop_Info(newloop);
  dli->Est_Num_Iterations = dli->Est_Num_Iterations % unroll_factor;
  if (Cur_PU_Feedback) {
    Cur_PU_Feedback->FB_wind_down_loop(newloop, dli->Est_Num_Iterations);
  }
  ...
  dli->SIMD_If_Convert_Directive = Get_Do_Loop_Info(loop)->SIMD_If_Convert_Directive;
