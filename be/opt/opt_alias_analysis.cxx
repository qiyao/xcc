...
void OPT_STAB::Analyze_Range(WN *wn_arr, POINTS_TO *pt)
{
  ...
  pt->Set_byte_size( pt->Byte_Size() + (upper - lower) );
...
void OPT_STAB::Analyze_Base_Flow_Free(POINTS_TO *pt, WN *wn)
{
  ...
  if (WN_operator(wn)==OPR_INTRINSIC_CALL) {
	...
	TIE_MACRO_ID tie_macro_id = Intrinsic_To_Tie_Macro_Id(WN_intrinsic(wn));
	TIE_MACRO_p tie_macro = tie_info->tie_macro(tie_macro_id);

	if (tie_macro->is_simple_addr_load_store()) {

	  INT base_proto_idx =
		tie_macro->simple_addr_load_store_base_proto_index();
	  INT base_wn_idx = tie_macro->proto_to_whirl_index(base_proto_idx);
	  WN* base_wn = WN_kid0(WN_kid(wn,base_wn_idx));
	  INT offset = 0;
	  INT offset_proto_idx =
		tie_macro->simple_addr_load_store_offset_proto_index();
	  if (offset_proto_idx != -1) {
	    INT offset_wn_idx =
			tie_macro->proto_to_whirl_index(offset_proto_idx);
	    WN* offset_wn = WN_kid0(WN_kid(wn,offset_wn_idx));
	    offset = (INT)WN_const_val(offset_wn);
	  }

	  Simplify_Pointer(base_wn, pt);
	  pt->Shift_ofst(offset);
	  pt->Lower_to_base(wn);
	  //pt->Set_ty(WN_object_ty(wn));
	  //Collect_f90_pointer_info(pt, wn);
	  Update_From_Restricted_Map(wn, pt);
	} else if (Alias_Pointer_Disjoint || Alias_Pointer_Restricted) {
	  // With -OPT:alias=restrict/disjoint, assume that aliasing only happens through kids
	  INT num_addr_kid = 0;
	  for (INT i=0; i<WN_kid_count(wn); i++) {
      	    POINTS_TO alias;
            alias.Init();
            alias.Set_expr_kind(EXPR_IS_UNKNOWN);
            Simplify_Pointer_Arith(WN_kid0(WN_kid(wn,i)), &alias);
            if (alias.Expr_kind() == EXPR_IS_ADDR) {
	      if (!num_addr_kid) {
	        pt->Copy_fully(alias);
	      } else {
	        pt->Meet(&alias, (ST *) WN_kid0(WN_kid(wn,i)));
	      }
	      pt->Set_ofst_kind(OFST_IS_UNKNOWN);
	      num_addr_kid++;
	    }
          }
	  if (!num_addr_kid) { // address must come from state create a fake base
				// liberally assume no aliasing
 	   ST* st         = New_ST(CURRENT_SYMTAB);
           ST_Init (st, Save_Str("tie_intrinsic"), CLASS_VAR, SCLASS_AUTO, EXPORT_LOCAL, Be_Type_Tbl(MTYPE_I4));
           Set_ST_is_temp_var(st);
           pt->Set_base(st);
           pt->Set_base_kind(BASE_IS_FIXED);
...
// ====================================================================
// Create the chi list for entry block.  It creates OPR_OPT_CHI node
// in the ENTRY_BB block.
// ====================================================================
void
...
void OPT_STAB::Allocate_mu_chi_and_virtual_var(WN *wn, BB_NODE *bb)
{
  ...
  case OPR_INTRINSIC_CALL:
    occ = Enter_occ_tab(wn, 0); // need mu for global ref and chi at this node.
    if (opr==OPR_INTRINSIC_CALL) {
      ...
      if (INTRN_is_tie_intrinsic(intr)) {
        ...
        if (tie->reads_memory() || tie->writes_memory()) { // treat kid PARMS like reference parameters
          for (INT kidno=0; kidno < WN_kid_count(wn); kidno++) {
	    WN *kid = WN_kid(wn,kidno);
            vp_idx = Identify_vsym(kid);
            occ = Enter_occ_tab(kid, vp_idx); // treat OPR_PARM as ILOAD
            occ->Points_to()->Analyze_Parameter_Base(WN_kid0(kid), *this);
            Update_From_Restricted_Map(kid, occ->Points_to());
            occ->Points_to()->Set_alias_class(Alias_classification()->Alias_class(kid));
            vp_idx = Adjust_vsym(vp_idx, occ);
	  }
	}
      }
				      ...
				      INT32 num_parms, BB_NODE *bb)
{
  ...
  for (idx = BS_Choose( alias_set );
    ...
    READ_WRITE how = Rule()->Aliased_with_Call(wn, aux_stab[idx].Points_to());
					...
					INT32 num_parms)
{
  ...
  for (idx = BS_Choose( alias_set );
    ...
    if (WN_operator(wn) == OPR_INTRINSIC_CALL && INTRN_is_tie_intrinsic(WN_intrinsic(wn))) {
      TIE_MACRO_ID tie_macro_id = Intrinsic_To_Tie_Macro_Id(WN_intrinsic(wn));
      TIE_MACRO_p tie_macro = tie_info->tie_macro(tie_macro_id);
      BOOL aliased = true;
      // analyze the address argument
      if (tie_macro->is_simple_addr_load_store()) {
	aliased = false;
        INT base_proto_idx = tie_macro->simple_addr_load_store_base_proto_index();
        INT base_wn_idx = tie_macro->proto_to_whirl_index(base_proto_idx);
        WN *actual = WN_kid(wn,base_wn_idx);
        OCC_TAB_ENTRY *p_occ = Get_occ(actual);
	if (p_occ) {
         POINTS_TO     *pt2 = p_occ->Points_to();
         if (Rule()->Aliased_Memop(pt, pt2)) {
	   aliased = true;
         }
        }
      } else if (Alias_Pointer_Disjoint || Alias_Pointer_Restricted) {
	// restrict case, only consider aliased if one of the non-constant parameters might be 
	// a pointer that is aliased with wn
	aliased = false;
        for (INT32 i = 0; i < num_parms; i++) {
          WN *actual = WN_actual(wn,i);
	  if (WN_operator(WN_kid0(actual)) != OPR_INTCONST) {
            OCC_TAB_ENTRY *p_occ = Get_occ(actual);
	    if (p_occ && p_occ->Points_to()->Expr_kind() == EXPR_IS_ADDR) {
              POINTS_TO     *pt2 = p_occ->Points_to();
              if (Rule()->Aliased_Memop(pt, pt2)) {
	        aliased = true;
              }
	...
	how = NO_READ_NO_WRITE;
        ...
        how = Rule()->Aliased_with_Call(wn, pt);
      }
    } else {
      how = Rule()->Aliased_with_Call(wn, pt);
    }
  ...
  if (!OPCODE_is_black_box(opc)) {
	...
	if (sym_pt->Ip_alias_class() == OPTIMISTIC_AC_ID) {
	  ...
	  if ((sym_pt->Ip_alias_class() != PESSIMISTIC_AC_ID) &&
	    ...
	    DevWarn("Transfer_alias_class_to_occ_and_aux: Inconsistent IP alias class: %lu vs. %lu",
		   sym_pt->Ip_alias_class(), WN_MAP32_Get(WN_MAP_ALIAS_CLASS, wn));
...
OPT_STAB::Compute_FSA_stmt_or_expr(WN *wn)
{
  ...
  if (OPERATOR_is_scalar_istore (opr) || opr == OPR_MSTORE) {
	...
	if (aux_stab[v].Stype() != VT_UNIQUE_VSYM &&
	    !Rule()->Aliased_Memop(occ->Points_to(), aux_stab[v].Points_to())) {
  ...
  if (opr == OPR_INTRINSIC_CALL && INTRN_is_tie_intrinsic(WN_intrinsic(wn))) {

    TIE_MACRO_ID tie_macro_id = Intrinsic_To_Tie_Macro_Id(WN_intrinsic(wn));
    TIE_MACRO_p tie_macro = tie_info->tie_macro(tie_macro_id);

    occ = Get_occ(wn);
    if (tie_macro->is_simple_addr_load()) {
      if (Phase() == MAINOPT_PHASE) {
	// Found the original restrict mapping.
	// This map overrides any new unique pointer/restricted pointer
	// discovered later.
    ...
    } else if (tie_macro->is_simple_addr_store()) {
    ...
    if (tie_macro->is_simple_addr_load_store() || Alias_Pointer_Disjoint || Alias_Pointer_Restricted) {
      if (occ->Points_to()->Base_kind() == BASE_IS_UNKNOWN &&
	  !occ->Points_to()->F_param()) {
	...
	if (tie_macro->is_simple_addr_store() && occ->Points_to()->Const()) {
	  USRCPOS srcpos;
	  USRCPOS_srcpos(srcpos) = WN_Get_Linenum(wn);
	  ErrMsgLine(EC_Store_Const, USRCPOS_linenum(srcpos));
	}
        ...
        Is_True(Rule()->Aliased_Memop(occ->Points_to(), occ->Points_to())
              || (tie_macro->is_simple_addr_load() && occ->Points_to()->Const()),
		("intrinsic_call not aliased to itself!"));
      }
      ...
      if (occ->Points_to()->Based_sym() == NULL) {
        if (tie_macro->is_simple_addr_load_store()) {
          ...
          INT base_wn_idx = tie_macro->proto_to_whirl_index(base_proto_idx);
          WN *base_wn = WN_kid0(WN_kid(wn,base_wn_idx));
	  Analyze_Based_Pointer(occ->Points_to(), base_wn);
        } else if (Alias_Pointer_Disjoint || Alias_Pointer_Restricted) { 
          INT depth = 0;
	  INT num_addr_kid = 0;
          ST *st = NULL;
	  for (INT i=0; i<WN_kid_count(wn); i++) {
            ST *new_st = Find_Based_Pointer(WN_kid0(WN_kid(wn,i)), &depth);
	    if (new_st) {
	      st = new_st;
              num_addr_kid++;
            }
	  ...
	  if (!num_addr_kid) { // address must come from state, create a fake base
 	    ...
 	    st         = New_ST(CURRENT_SYMTAB);
            ST_Init (st, Save_Str("tie_intrinsic"), CLASS_VAR, SCLASS_AUTO, EXPORT_LOCAL, Be_Type_Tbl(MTYPE_I4));
            Set_ST_is_temp_var(st);
            occ->Points_to()->Set_based_sym(st);
            occ->Points_to()->Set_based_sym_depth(depth);
          } else if (num_addr_kid == 1) {
            occ->Points_to()->Set_based_sym(st);
            occ->Points_to()->Set_based_sym_depth(depth);
          }
        }
