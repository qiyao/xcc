...
// $Revision: 1.72 $
// $Date: 2000/05/19 20:24:47 $
// $Author: murthy $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_alias_class.cxx,v $
//
// ====================================================================
  ...
  _memops_classified(FALSE), _mem_pool_valid(TRUE)
{
  ...
  // In the past, the _const_addr_class was kept separate from the
  // Global_class(), but we have decided we want to acknowledge that
  // an integer constant can (with a cast) be passed as a pointer-type
  // parameter and then dereferenced. Dereferenced parameters are
  // analyzed as pointing to the Global_class(), so for this
  // initialization we need to set up the _const_addr_class to
  // coincide with the Global_class().	-- RK 040820
#define CONST_ADDR_POINTS_TO_GLOBALS 0
#if CONST_ADDR_POINTS_TO_GLOBALS
...
#else
  _const_addr_class = Global_class();
#endif
...
ALIAS_CLASSIFICATION::New_base_id(const ST *st, TY_IDX ty)
{
  ...
  // If this base_id is a global, it goes in the global class.
	   ...
	   storage_class == SCLASS_MERGE_STRING ||
				   ...
				   ALIAS_CLASSIFICATION &ac)
{
  ...
  if (cpt == NULL && that_cpt == NULL) {
    ...
    cpt->Set_mystery_class(cpt->Mystery_class() &&
			   that_cpt->Mystery_class());
    cpt->Union(*that_cpt);
  }
					     ...
					     BOOL expr_must_point)
{
  ...
  if (OPCODE_is_leaf(opc)) {
	...
	if (expr_must_point &&
	  ...
	  ldid_class->Set_mystery_class(TRUE);
	}
	...
	if (expr_must_point && !ref->Is_pointer_class()) {
	  ...
	  ref->Class_pointed_to()->Set_mystery_class(TRUE);
  ...
  // information available to the callee.
    ...
    if (expr_must_point && !t.Obj_class()->Is_pointer_class()) {
      ...
      t.Obj_class()->Class_pointed_to()->Set_mystery_class(TRUE);
    ...
    // Unsure of this next line. -- RK 040820
    t_member->Alias_class()->Set_mystery_class(TRUE);
    ...
    if (expr_must_point && t.Obj_class() == NULL) {
      ...
      obj_acr->Set_mystery_class(TRUE);
						  ...
						  const WN *const parm_wn)
{
  if (WN_Call_Never_Return(call_wn)) {
    ...
    ST *st = (WN_operator(call_wn) == OPR_CALL ? WN_st(call_wn) : NULL);
    ...
    if (st != NULL) {
      ...
      INTRINSIC intr = WN_intrinsic(call_wn);
      if (INTRN_is_tie_intrinsic(intr)) {
        no_side_effects = 
        !tie_info->tie_macro(Intrinsic_To_Tie_Macro_Id(intr))->has_side_effect();
      }
      else {
        no_side_effects = intrn_info[intr].has_no_side_effects;
      }
    }
    if (no_side_effects) {
      return TRUE;
...
ALIAS_CLASSIFICATION::Finalize_ac_map_wn(WN *wn)
{
  ...
  if (OPCODE_is_call(opc)) {
	...
	acr->Print(TFile, Global_class());
      ...
      // During the analysis phase, we try to detect dereferences of
      // integer constants, using expr_must_point, _const_addr_class,
      // and so forth. But in spite of our best efforts, it is
      // possible that some dereference of a constant has gotten
      // through our net. One way this can happen is for the integer
      // constant to get folded into the offset field of an ILOAD or
      // ISTORE node, while the address kid of that node contains an
      // expression intended by the programmer to compute a variable
      // offset.
      //
      // Because this sort of stuff can happen (and has), we have an
      // additional safety net here in an attempt to reduce the number
      // of cases we will compile incorrectly. The intention is to
      // catch all dereferences of integer constants and put them
      // together in _const_addr_class. This net, too, has holes in
      // it, but will catch a few cases that aren't caught already. It
      // remains important to realize that, like the meager support
      // that existed before for doing points-to analysis in the
      // presence of dereferenced constants, this hack is still
      // incorrect because some dereferences of constants can still
      // sneak through: in some situations where integer constants are
      // dereferenced, memops will be classified too optimistically.
      //
      // For indirects whose base addresses should point to something
      // but don't, assume they are dereferences of constants and
      // remap their class_id to the ID associated with
      // _const_addr_class. All other indirects will point to
      // something because their base address expressions will have
      // been analyzed and will appear in classes containing real
      // program objects (chiefly either WHIRL nodes or base IDs, but
      // also possibly malloc/alloca memory, etc.).
      //
      // A potential challenge here is to determine whether the
      // current memop's class actually has been analyzed as pointing
      // to something. This is the raison d'etre of the
      // ARC_FLAG_MYSTERY_CLASS bit. When a class is created anew
      // because something dereferences an address expression but that
      // expression has not yet been analyzed as pointing to anything
      // (i.e., its class has no Class_pointed_to()), the new class is
      // marked with the MYSTERY_CLASS bit because no real program
      // object is known to belong in that class. When two classes are
      // joined, the resulting class's MYSTERY_CLASS flag is set if
      // and only if both original classes' MYSTERY_CLASS flags were
      // set, because if either original class has that flag cleared,
      // it contains a real program object (and hence the class
      // resulting from the join operation will contain a real program
      // object). Now the analysis is over, and any remaining class
      // with the MYSTERY_CLASS flag set is a class corresponding to
      // the dereference of an address expression whose
      // Class_pointed_to() contains no real program object after the
      // analysis. Hence we conservatively remap the ID for that class
      // to the ID for the _const_addr_class on the assumption that we
      // are dealing with an unanalyzed dereference of a constant
      // address.	-- RK 040820
      if (acr->Mystery_class()) {
	// This is the case where the user has dereferenced a constant
	// and we haven't caught it already.
	if (Tracing()) {
	  fprintf(TFile, "Invoked second *((int *) <constant>) safety net on\n");
	  fdump_wn_no_st(TFile, wn);
	  fprintf(TFile, "Remapping class ID %d ", acr->Id());
	}
	acr = _const_addr_class;
	if (Tracing()) {
	  // For now, try the _const_addr_class, because this is no
	  // less safe than what we are already doing in the analysis
	  // phase. Ding-Kai is concerned that dereferenced parameters
	  // might contain constants as well, and in such a case we
	  // already might be too optimistic because we don't reach
	  // this case here (i.e., we get caught by the constant
	  // dereference code in the analysis phase).
	  //
	  // For additional safety, it might be reasonable to use
	  // PESSIMISTIC_AC_ID instead. If we decided to do that, we
	  // would need to give up the acr variable because there is
	  // no class and hence no representative for that AC_ID.
	  fprintf(TFile, "to %d.\n", acr->Id());
	  acr->Print(TFile, Global_class());
	}
      }
      else {
	// This is the common case, where the user is not wacky and
	// pointers actually point to something. We don't need to do
	// anything here.
      }
