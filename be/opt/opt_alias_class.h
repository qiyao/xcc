...
// $Revision: 1.28 $
// $Date: 2000/04/06 15:21:15 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_alias_class.h,v $
//
// ====================================================================
...
class ALIAS_CLASS_REP : public U_F_REP<ALIAS_CLASS_MEMBER> {
  ...
  enum ACR_FLAGS {
    ...
    ACR_FLAG_MYSTERY_CLASS    = 0x4,   // items are all products of expr_must_point
  };
...
public:
  ...
  void              Set_mystery_class(BOOL f = TRUE)
    {
      if (f) {
	_flags |= ACR_FLAG_MYSTERY_CLASS;
      }
      else {
        _flags &= ~ACR_FLAG_MYSTERY_CLASS;
      }
  ...
  BOOL              Mystery_class(void) const
    { return _flags & ACR_FLAG_MYSTERY_CLASS; }
