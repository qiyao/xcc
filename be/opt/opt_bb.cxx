...
#ifdef _KEEP_RCS_ID
static char *rcs_id = 	opt_bb_CXX"$Revision: 1.179 $";
#endif /* _KEEP_RCS_ID */
...
BB_LOOP::Print(FILE *fp) const
{
  if (Well_formed()) {
    ...
    if (Tail()) 
      ...
      fprintf(fp, " no tail\n");
    fprintf(fp, Test_at_entry() ? "test-at-entry, " : "not test-at-entry, ");
    fprintf(fp, Test_at_exit() ? "test-at-exit, " : "not test-at-exit, ");
    fprintf(fp, Exit_early() ? "exit-early\n" : "not exit-early\n");
