...
// Module: opt_bdce.cxx
// $Revision: 1.45 $
// $Date: 2000/04/13 18:55:25 $
// $Author: dew $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_bdce.cxx,v $
//
// ====================================================================
...
BITWISE_DCE::Bits_in_var(CODEREP *v)
{
  ...
  if (ST_class(Opt_stab()->Aux_stab_entry(v->Aux_id())->St()) == CLASS_PREG)
    if (MTYPE_is_longlong(v->Dtyp()) || MTYPE_is_longlong(v->Dsctyp())) {
      return Bits_in_type(MTYPE_U8);
    } else {
				 ...
				 BOOL stmt_visit)
{
  UINT64 new_livebits;
  if (Tracing())
    fprintf(TFile, "Mark_tree_bits_live(cr%d,%" LLX_FMT ",%d)\n",
  ...
  case CK_OP: {
...
#if defined(TARG_MIPS) || defined(TARG_XTENSA)
    ...
    case OPR_ASHR:
      ...
      if (cr->Opnd(1)->Kind() == CK_CONST) {
        ...
        if ((shift_amt < 0) || (shift_amt >= MTYPE_size_min(dtyp))) shift_amt = MTYPE_size_min(dtyp) -1;
#endif
	// need to do extra work to determine if the sign bit is live
...
BITWISE_DCE::Mark_stmt_live(STMTREP *stmt) 
{
  ...
  if (opr == OPR_PREFETCH)
	...
	Mark_tree_bits_live(stmt->Rhs(), UINT64_MAX,
			       _copy_propagate /*stmt_visit*/);
      }
      break;
    default: 
  ...
  FOR_ALL_NODE(stmt, stmt_iter, Init()) {
	...
	// if they are used
        Mark_stmt_live(stmt);
	     ...
	     Opt_stab()->Aux_stab_entry(stmt->Lhs()->Aux_id())->Has_nested_ref()) {
	// has_nested_ref symbols can be accessed by run-time so we
	// don't know if they are used
        Mark_stmt_live(stmt);
    }
    else if (stmt->Opr() == OPR_STID && 
             (MTYPE_is_tie(stmt->Lhs()->Dsctyp()) ||
              MTYPE_is_xtbool(stmt->Lhs()->Dsctyp()))) {
	...
	if (ST_class(s) == CLASS_PREG && rhs->Offset() < 0 &&
	  ...
	  // pr3538. for asms, we need the STID/LDID that loads the result
	  // value, if the result has a tie type, even if that result
	  // is dead. We need the STID/LDID so we can determine the
	  // result type, and thus know what type of register to use.
	  Mark_stmt_live(stmt);
	}
      }
    }
    else if (stmt->Opr() == OPR_AGOTO) {
      // PR# 5879
      Mark_stmt_live(stmt);
    }
  }
			    ...
			    CODEREP *opnd)
{
...
#ifdef TARG_XTENSA
  if ((opnd->Dtyp() == MTYPE_I8 || opnd->Dtyp() == MTYPE_U8) && to_bit != 64) {
    return FALSE;
  }
#endif
  switch (opnd->Kind()) {
  ...
  case CK_VAR: {
    ...
    if (aux->Is_dedicated_preg()) {
      /* old behavior is too conservative "return FALSE;" */
      if (Language == LANG_KR_C ||
	  !Opt_stab()->Is_prototyped_func()) {
	return FALSE; /* use old behavior for KR_C */
      }
      /* use dedicated register sign bit/dsctyp to get the right bit. Peng. */
    ...
    if (ST_class(aux->St()) == CLASS_PREG) {
      ...
      FmtAssert(opnd->Defstmt()->Op() == OPC_OPT_CHI &&
                opnd->Defstmt()->Bb()->Kind() == BB_ENTRY, 
                ("BITWISE_DCE::Preg defined by chi but is not entry chi"));
      return TRUE;
      }
	...
	return FALSE;
      } else if (opnd->Defstmt() == NULL) {
	return FALSE; // no definition as in volatiles
      }
      else {
	...
	if (rhs->Kind() == CK_VAR &&
	    ...
	    opnd->Is_sign_extd()==sign_xtd &&
	    aux->Value_size()==from_bit &&
	    MTYPE_size_min(opnd->Dtyp())==to_bit) {
	  // CVTLs are redundant for formals which were changed into PREG by
	  // AUX_STAB_ENTRY::Change_to_new_preg in Do_local_rvi
	  return TRUE;
	}
...
BITWISE_DCE::Delete_cvtls(CODEREP *cr, STMTREP *use_stmt)
{
  ...
  case CK_OP:
    ...
    if (opr == OPR_CVTL) {
      ...
      if (cr->Get_opnd(0)->Kind()==CK_OP &&
	  cr->Get_opnd(0)->Opr()==OPR_CVT &&
	  MTYPE_is_tie(cr->Opnd(0)->Dsctyp())) {

          /* CVTL for CVT from TIE type need to be preserved */

      } else if ((((Livebits(cr) & ~Bitmask_of_size(cr->Offset())) == 0) 
#ifdef TARG_XTENSA
	   && (cr->Dtyp() == cr->Opnd(0)->Dtyp() || 
	       cr->Dtyp() != MTYPE_I8 && cr->Dtyp() != MTYPE_U8 &&
	       cr->Opnd(0)->Dtyp() != MTYPE_I8 && 
	       cr->Opnd(0)->Dtyp() != MTYPE_U8))
#endif
	  ||
...
#ifdef TARG_XTENSA
	   && (dtyp == cr->Opnd(0)->Dtyp() || 
	       dtyp != MTYPE_I8 && dtyp != MTYPE_U8 &&
	       dsctyp != MTYPE_I8 && dsctyp != MTYPE_U8 &&
	       cr->Opnd(0)->Dtyp() != MTYPE_I8 && 
	       cr->Opnd(0)->Dtyp() != MTYPE_U8)
#endif
	       ) {
...
BITWISE_DCE::Print_nodes_with_dead_bits(FILE *fp) 
{
  ...
  FOR_ALL_ELEM(bucket, codemap_iter, Init(Htable())) {
	...
	fprintf(fp, " has live bits 0x%" LLX_FMT "\n", Livebits(cr));
      }
    }
  }
