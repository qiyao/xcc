...
static char *rcs_id = 	opt_canon_CXX"$Revision: 1.42 $";
#endif /* _KEEP_RCS_ID */
...
void
CANON_CR::Trim_Scale(WN *wn, WN *kid, WN *parent, CODEMAP *htable)
{
  const OPCODE op = WN_opcode(kid);
  MTYPE typ;
  INT64 base_offset = Canonicalize_Memory_Offset(wn, parent, Scale());
  ...
  if (Tree() == NULL) {
    ...
    Set_scale(0);
#else
    // the above assumption has code size problem.  Go ahead anyway. P.T.
    Set_tree(htable->Add_const(typ, base_offset));
    Set_scale(Scale() - base_offset);
#endif
    return;
  }
  ...
  // Fold base_offset into LDA node so that it can be hoisted
  if (Tree()->Kind() == CK_OP && Tree()->Opr() == OPR_ADD) {
    if (Tree()->Opnd(0)->Kind() == CK_LDA) {
      Set_tree(htable->Add_bin_node(OPCODE_make_op(OPR_ADD, typ, MTYPE_V),
				    htable->
				    Add_lda_node(Tree()->Opnd(0),
						 base_offset + 
						 Tree()->Opnd(0)->Offset()),
                                    Tree()->Opnd(1)));
      return;
    }
    else if (Tree()->Opnd(1)->Kind() == CK_LDA) {
      Set_tree(htable->Add_bin_node(OPCODE_make_op(OPR_ADD, typ, MTYPE_V),
                                    Tree()->Opnd(0),
                                    htable->
				    Add_lda_node(Tree()->Opnd(1),
						 base_offset +
						 Tree()->Opnd(1)->Offset())));
      return;
    }
  ...
  Set_tree(htable->Add_bin_node(OPCODE_make_op(OPR_ADD, typ, MTYPE_V),
				Tree(), htable->Add_const(typ, base_offset)));
}
