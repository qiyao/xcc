...
// Module: opt_cfg.cxx
// $Revision: 1.344 $
// $Date: 2000/09/14 17:45:18 $
// $Author: mpm $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_cfg.cxx,v $
//
// ====================================================================
//
/* 
...
static char *rcs_id = 	opt_cfg_CXX"$Revision: 1.344 $";
#endif /* _KEEP_RCS_ID */
			...
			SRCPOS	srcpos)
{
  ...
  if ( WN_operator(cond) == OPR_CAND ) {
    ...
    *created_branch = NULL;
    ...
    WN_Set_Linenum( end_cond, srcpos );
...
CFG::Create_loop_info( BB_NODE *body_bb, WN *loop_wn )
{
  ...
  if ( loop_info == NULL ) {
    ...
    loop_info = WN_CreateLoopInfo ( index_var, loop_trip, 
		  0/*estcount*/, Cur_loop_depth(),
		  0/*flags*/, AT_REGION_ID_UNKNOWN );
  }

  body_bb->Set_label_loop_info(loop_info);
  body_bb->Set_linenum(WN_Get_Linenum(loop_wn));
}
...
CFG::Create_blank_loop_info( BB_NODE *body_bb )
{
  ...
  WN *loop_info = WN_CreateLoopInfo ( index_var, loop_trip, 0 /*estcount*/,
				      Cur_loop_depth(), 0 /*flags*/, AT_REGION_ID_UNKNOWN );
...
CFG::Lower_do_loop( WN *wn, END_BLOCK *ends_bb )
{
  ...
  if (dohead_bb->Label_wn()) {
    WN_Set_Linenum(dohead_bb->Label_wn(),WN_Get_Linenum(wn));
  }
  ...
  if ( loop_info == NULL || !WN_Loop_Nz_Trip(loop_info) ) {
    ...
    entry_test_bb = Create_conditional( endcopy, dohead_bb, exit_bb, FALSE,
					&wn_top_branch, WN_Get_Linenum(wn) );
  }
  ...
  if (body_bb->Label_wn()) {
    WN_Set_Linenum(body_bb->Label_wn(),WN_Get_Linenum(wn));
  }
  ...
  if (dotail_bb->Label_wn()) {
    WN_Set_Linenum(dotail_bb->Label_wn(),WN_Get_Linenum(wn));
  }

  // Create the loop exit test
  WN *wn_back_branch;
  BB_NODE *cond_bb = Create_conditional( WN_end(wn), body_bb, dotail_bb, TRUE,
					 &wn_back_branch, WN_Get_Linenum(wn));
...
// Check that the type of all mins/maxs underneath match the type of cond
static 
BOOL Check_Minmax_Types(TYPE_ID cond_type, WN *wn)
{
    if (WN_operator(wn) == OPR_MAX || WN_operator(wn) == OPR_MIN) {
        return (WN_rtype(wn) == cond_type);
    } else {
	for (INT i=0; i<WN_kid_count(wn); i++) {
		if (!Check_Minmax_Types(cond_type,WN_kid(wn,i))) {
			return FALSE;
		}
	}
    }
...
/*--------------------------------------------------------------------------*
 * Given an expression of assigned value x = expr, get the load operand,    *
 * simply named as ldopnd.                                                  *
 *                                                                          *
 * (1) Skipping over the top level CVT                                      *
 * (2) From MIN(const, LOD) or MAX(const, LOD), return the LOD              *
 * (3) From CVTL(MIN(const, LOD)) or CVTL(MAX(const, LOD)), return the LOD  *
 * *is_cvtl is true if the parent of a MIN/MAX is a cvtl                    *
 *--------------------------------------------------------------------------*/
static
WN* Get_Ldopnd_From_Assigned_Value_Rec(WN *expr, WN *& constant)
{
    if (WN_operator(expr) == OPR_MAX || WN_operator(expr) == OPR_MIN) {
      if (constant != NULL) {
	/* multiple level of nested MAX/MIN */
	return expr;
      }
	...
	constant = kid0;
	return Get_Ldopnd_From_Assigned_Value_Rec(kid1, constant);
      } else if (WN_operator(kid1) == OPR_INTCONST) {
	constant = kid1;
	return Get_Ldopnd_From_Assigned_Value_Rec(kid0, constant);
      }
    }
    return expr;
}

WN* Get_Ldopnd_From_Assigned_Value(WN *expr, WN *& constant, WN **is_cvtl)
{
    *is_cvtl = NULL;
    if (WN_operator(expr) == OPR_CVT) {
      expr = WN_kid0(expr);
    }
    if (WN_operator(expr) == OPR_CVTL && 
	(WN_rtype(expr) == WN_rtype(WN_kid0(expr)))) {
      if (WN_operator(WN_kid0(expr)) == OPR_MAX || 
	  WN_operator(WN_kid0(expr)) == OPR_MIN) {
        *is_cvtl = expr;
        expr = WN_kid0(expr);
      }
    }
...
// tree equivalent 
// ====================================================================
static BOOL WN_tree_equiv (WN *wn1, WN* wn2) 
{
  if (!wn1 && !wn2) return TRUE;    // both are NULL
  if (!wn1 || !wn2) return FALSE;   // one (but not both) is NULL

  /* this will only work when the WN_st is changed to AUX in the WOPT */
  if (WN_has_aux(wn1) && WN_has_aux(wn2) &&
      WN_aux(wn1) == WN_aux(wn2) &&
      WN_operator(wn1) == WN_operator(wn2)) {
    /* Checking WN_operator() above is added for fixing pr11582.  */
    return TRUE;
  }
  ...
  if (WN_opcode(wn1) == OPC_BLOCK) {
    ...
    for (INT i=0; i<WN_kid_count(wn1); i++)
      if (!WN_tree_equiv (WN_kid(wn1,i), WN_kid(wn2,i))) return FALSE;
    return TRUE;
  }
}

static BOOL WN_one_less(WN *wn1, WN *wn2)
{
  if (WN_operator(wn1) == OPR_INTCONST && WN_operator(wn2) == OPR_INTCONST) {
    return (WN_const_val(wn1) == (WN_const_val(wn2) - 1));
  }
  return FALSE;
}

// ====================================================================
// Try to combine an if_then_else into MIN/MAX                       
// ====================================================================
WN* 
CFG::If_is_intrinsic(WN *wn) 
{
  Is_True(WN_operator(wn) == OPR_IF, ("Not an IF statement"));
  ...
  if (WOPT_Enable_Simple_If_Conv &&
    ...
    BOOL types_ok = Check_Minmax_Types(WN_desc(WN_if_test(wn)), then_expr);
    if (types_ok) types_ok = Check_Minmax_Types(WN_desc(WN_if_test(wn)), else_expr);

    // MIN/MAX/ABS recognition
    if (// The STID <var> is not volatile
	types_ok && !_opt_stab->Is_volatile(WN_aux(store))) {
      WN         *cond       = WN_if_test(wn);
      if (WN_kid_count(cond) == 2) {
	WN       *then_const = NULL;
	WN       *else_const = NULL;
	WN  	 *then_cvtl, *else_cvtl;
	WN       *then_ldopnd  = 
	  Get_Ldopnd_From_Assigned_Value(then_expr, then_const, &then_cvtl);
	WN       *else_ldopnd  =
	  Get_Ldopnd_From_Assigned_Value(else_expr, else_const, &else_cvtl);
	OPERATOR  op      = WN_operator(cond);
	WN       *left    = WN_kid0(cond);
	WN       *right   = WN_kid1(cond);
	OPERATOR  else_op = WN_operator(else_expr);
	OPERATOR  res_op  = OPERATOR_UNKNOWN;

	WN *tmp=else_expr;
	while (WN_operator(tmp) == OPR_CVT || WN_operator(tmp) == OPR_CVTL) {
          tmp = WN_kid0(tmp);
	}
	else_op = WN_operator(tmp);
	
	if (!then_const || !else_const) {
	  switch (op) {
	  case OPR_GT:
	  case OPR_GE:
	    if (WN_tree_equiv(right, else_ldopnd) &&
		(WN_tree_equiv(left, then_ldopnd) || 
		 (op == OPR_GT) && WN_one_less(then_ldopnd, left) ||
		 (op == OPR_GE && WN_one_less(left, then_ldopnd)))) {
	      /* if (x>=y) 
	       *   t =  x;
	       * else
	       *   t =  y;
	       */
              if (else_opr == OPR_MAX) break;
	      /* generate MAX(then_expr, else_expr) */
	      if (else_const != NULL && else_op == OPR_MIN) {
		/* if (x>=y)
		 *    t = x;
		 * else 
		 *    t = MIN(y, else_const);
		 */
		/* to safely generate MAX(x, MIN(y, else_const)) *
		 * we need to check that else_const >= x         */
		if (WN_operator(then_ldopnd) != OPR_INTCONST ||
		    !(WN_const_val(else_const) >= WN_const_val(then_ldopnd))) {
		  break;
		}
		if (else_cvtl) { // check that CVTL is a nop
		  INT cvtl_limit;
		  if (MTYPE_is_unsigned(WN_rtype(else_expr))) {
		    cvtl_limit = 1 << (WN_cvtl_bits(else_cvtl));
		  } else {
		    cvtl_limit = 1 << (WN_cvtl_bits(else_cvtl)-1);
		  }
		  if (WN_const_val(else_const) >= cvtl_limit || 
		      WN_const_val(else_const) < -cvtl_limit ||
		      WN_const_val(then_ldopnd) >= cvtl_limit ||
		      WN_const_val(then_ldopnd) < -cvtl_limit) {
		    break;
		  }
		}
	      ...
	      res_op = OPR_MAX;
	    } else if (WN_tree_equiv(left, else_ldopnd) &&
		(WN_tree_equiv(right, then_ldopnd) ||
		 (op == OPR_GE) && WN_one_less(then_ldopnd, right) ||
		 (op == OPR_GT && WN_one_less(right, then_ldopnd)))) {
	      /* if (y>=x) 
	       *   t =  x;
	       * else
	       *   t =  y;
	       */
              if (else_opr == OPR_MIN) break;
	      /* generate MIN(then_expr, else_expr) */
	      if (else_const != NULL && else_op == OPR_MAX) {
		/* if (y>=x)
		 *    t = x;
		 * else 
		 *    t = MAX(y, else_const);
		 */
		/* to safely generate MIN(x, MAX(y, else_const)) *
		 * we need to check that s_else_const <= x         */
		if (WN_operator(then_ldopnd) != OPR_INTCONST || 
		    !(WN_const_val(else_const) <= WN_const_val(then_ldopnd))) {
		  break;
		}
		if (else_cvtl) { // check that CVTL is a nop
		  INT cvtl_limit;
		  if (MTYPE_is_unsigned(WN_rtype(else_cvtl))) {
		    cvtl_limit = 1 << (WN_cvtl_bits(else_cvtl));
		  } else {
		    cvtl_limit = 1 << (WN_cvtl_bits(else_cvtl)-1);
		  }
	  ...
	  case OPR_LE:
	    if (WN_tree_equiv(right, else_ldopnd) &&
		(WN_tree_equiv(left, then_ldopnd) ||
		 (op == OPR_LT) && WN_one_less(left, then_ldopnd) ||
		 (op == OPR_LE && WN_one_less(then_ldopnd, left)))) {
	      /* if (x<=y) 
	       *   t =  x;
	       * else
	       *   t =  y;
		...
		if (else_cvtl) { // check that CVTL is a nop
	      ...
	      res_op = OPR_MIN;
		       ...
		       (WN_tree_equiv(right, then_ldopnd) ||
			(op == OPR_LE) && WN_one_less(right, then_ldopnd) ||
			(op == OPR_LT) && WN_one_less(then_ldopnd, right))) {
	      /* if (y<=x) 
	       *   t =  x;
	       * else
	       *   t =  y;
	  ...
	  if (res_op != OPERATOR_UNKNOWN) {
	    // Generate a MIN/MAX expression
	    Is_True(res_op == OPR_MAX || res_op == OPR_MIN,
		    ("Unexpected operator"));

	    if (!MTYPE_is_integral(WN_rtype(then_expr))) {
	      DevWarn("potential missed non-integral min/max. ISA support for float?");
	      return NULL;
	    }

	    WN *min_max;
	    if (else_cvtl) {
	      if (else_expr == else_cvtl) { // Delete no-op cvtl
		else_expr = WN_kid0(else_expr);
		WN_Delete(else_cvtl);
	      } else if (WN_operator(else_expr) == OPR_CVT &&
			      WN_kid0(else_expr) == else_cvtl) {
                WN_kid0(else_expr) = WN_kid0(else_cvtl);
		WN_Delete(else_cvtl);
	      } else {
	        Is_True(0, ("cvtl not found "));
	      }
	    }
	    min_max= WN_CreateExp2(OPCODE_make_op(res_op,
						       WN_desc(cond),
						       MTYPE_V),
					then_expr, else_expr);
	    WN_kid0( store ) = min_max;
	...
	/* identify ABS */
	bool is_then = false;
	if (!MTYPE_is_integral(WN_desc(cond)) && MTYPE_is_unsigned(WN_desc(cond))) {
	  // DO NOTHING WITH UNSIGNED
	  ;
	} else if (else_op == OPR_NEG && 
	    WN_tree_equiv(WN_kid0(else_expr), then_expr)) {
	  /* if (Cond) 
	   *   t =  x;
	   * else
	   *   t = -x;
	   */
	  if (((op == OPR_GT || op == OPR_GE) &&
	       WN_tree_equiv(left, then_expr) &&
	       WN_operator(right) == OPR_INTCONST &&
	       WN_const_val(right) == 0) ||
	      ((op == OPR_LT || op == OPR_LE) &&
	       WN_tree_equiv(right, then_expr) &&
	       WN_operator(left) == OPR_INTCONST &&
	       WN_const_val(left) == 0)) {
	    /* cond: x >= 0 || 0 <= x */
	    is_then = true;
	    res_op = OPR_ABS;
	  }
	} else if (WN_operator(then_expr) == OPR_NEG &&
		   WN_tree_equiv(WN_kid0(then_expr), else_expr)) {
	  /* if (Cond) 
	   *   t = -x;
	   * else
	   *   t =  x;
	   */
	  if (((op == OPR_LT || op == OPR_LE) &&
	       WN_tree_equiv(left, else_expr) &&
	       WN_operator(right) == OPR_INTCONST &&
	       WN_const_val(right) == 0) ||
	      ((op == OPR_GT || op == OPR_GE) &&
	       WN_tree_equiv(right, else_expr) &&
	       WN_operator(left) == OPR_INTCONST &&
	       WN_const_val(left) == 0)) {
	    /* Cond: x <= 0 || 0 >= x */
	    is_then = false;
	...
	// generate ABS
	if (res_op != OPERATOR_UNKNOWN) {
	  Is_True(res_op == OPR_ABS, ("Unexpeted operator"));
	  WN *kid = (is_then) ? then_expr : else_expr;

	  TYPE_ID res_type = WN_rtype(kid);
	  if (!MTYPE_is_integral(WN_rtype(kid))) {
	    DevWarn("potential missed non-integral abs. ISA support for float?");
	    return NULL;
	  } else {
	    res_type = Mtype_TransferSign(WN_desc(cond), res_type);
	  }

	  WN *abs = 
	    WN_CreateExp1(OPCODE_make_op(res_op, res_type, MTYPE_V), kid);
	  WN_kid0( store ) = abs;
	  WN_Set_Linenum( store, WN_Get_Linenum(wn) );
	  return store;
	}
...
CFG::Lower_if_stmt( WN *wn, END_BLOCK *ends_bb )
{
  ...
  WN    *then_wn    = WN_then(wn);
  BOOL   empty_then = FALSE;
  if (then_wn != NULL && WN_opcode(then_wn) == OPC_BLOCK) {
    /* remove useless EVAL */
    WN *nextstmt = NULL;
    for (WN *stmt = WN_first(then_wn); stmt != NULL; stmt = nextstmt) {
      nextstmt = WN_next(stmt);
      if (WN_operator(stmt) == OPR_EVAL &&
	  WN_operator(WN_kid0(stmt)) == OPR_INTCONST) {
	WN_EXTRACT_FromBlock(then_wn, stmt);
	WN_Delete(stmt);
      }
    }
  }
  ...
  if (else_wn != NULL && WN_opcode(else_wn) == OPC_BLOCK) {
    /* remove useless EVAL */
    WN *nextstmt = NULL;
    for (WN *stmt = WN_first(else_wn); stmt != NULL; stmt = nextstmt) {
      nextstmt = WN_next(stmt);
      if (WN_operator(stmt) == OPR_EVAL &&
	  WN_operator(WN_kid0(stmt)) == OPR_INTCONST) {
	WN_EXTRACT_FromBlock(else_wn, stmt);
	WN_Delete(stmt);
      }
    }
  }
  ...
  if (WOPT_Enable_Simple_If_Conv &&
	...
	(WN_operator(WN_first(else_wn)) == OPR_STID ||
	 (WN_operator(WN_first(else_wn)) == OPR_IF &&
	  (else_wn_1 = If_is_intrinsic(WN_first(else_wn))) != NULL)))) &&
	...
	WN_aux(WN_first(else_wn)) == WN_aux(WN_first(then_wn))) ||
       (else_wn_1 != NULL && 
	WN_aux(else_wn_1) == WN_aux(WN_first(then_wn))))) {

    // Get the STID from either the first statement of the non-empty block
    WN *stmt = (else_wn_1) ? else_wn_1 : 
      WN_first((empty_then) ? else_wn : then_wn);
    ...
    WN *then_expr = empty_then ? load : WN_kid0(WN_first(then_wn));
    WN *else_expr = empty_else ? load : WN_kid0((else_wn_1) ? 
						else_wn_1: WN_first(else_wn));
    WN *tmp_then = then_expr;
    while (WN_operator(tmp_then) == OPR_CVTL) tmp_then = WN_kid0(tmp_then); 
    WN *tmp_else = else_expr;
    while (WN_operator(tmp_else) == OPR_CVTL) tmp_else = WN_kid0(tmp_else); 
    OPERATOR then_opr = WN_operator(tmp_then);
    OPERATOR else_opr = WN_operator(tmp_else);

    if (then_opr == OPR_MIN || then_opr == OPR_MAX) {
      if (else_opr != OPR_MIN && else_opr != OPR_MAX) { // canonicalize
        OPCODE cond_opcode = WN_opcode(WN_if_test(wn));
        OPERATOR cond_op = OPCODE_operator(cond_opcode);
	if (cond_op == OPR_LT || cond_op == OPR_LE || cond_op == OPR_GE || cond_op == OPR_GT){
	  WN *tmp = then_expr; then_expr = else_expr; else_expr = tmp;
	  tmp = then_wn; then_wn = else_wn; else_wn = tmp;
	  WN_then(wn) = then_wn; WN_else(wn) = else_wn;
	  OPERATOR op_tmp = then_opr; then_opr = else_opr; else_opr = op_tmp;
	  OPERATOR new_cond_op;
	  switch (cond_op) {
	  case OPR_GT: new_cond_op = OPR_LE; break;
	  case OPR_GE: new_cond_op = OPR_LT; break;
	  case OPR_LE: new_cond_op = OPR_GT; break;
	  case OPR_LT: new_cond_op = OPR_GE; break;
	  }
	  OPCODE new_opcode = OPCODE_make_op(new_cond_op,OPCODE_rtype(cond_opcode),
			  			OPCODE_desc(cond_opcode));
	  tmp = WN_if_test(wn);
	  WN_set_opcode(tmp,new_opcode);
	}
      }
    }

    then_opr = WN_operator(then_expr);
    else_opr = WN_operator(else_expr);
	...
	if (!then_const || !else_const) {
	  ...
	  case OPR_GE:
		...
		(WN_tree_equiv(left, then_ldopnd) || 
		 ...
		 (op == OPR_GE && WN_one_less(left, then_ldopnd)))) {
	       ...
	       */
              if (else_opr == OPR_MAX) break;
	      /* generate MAX(then_expr, else_expr) */
	      if (else_const != NULL && else_opr == OPR_MIN) {
		/* if (x>=y)
		 *    t = x;
		 * else 
		 *    t = MIN(y, else_const);
		...
		(WN_tree_equiv(right, then_ldopnd) ||
		 ...
		 (op == OPR_GT && WN_one_less(right, then_ldopnd)))) {
	       ...
	       */
              if (else_opr == OPR_MIN) break;
	      /* generate MIN(then_exp, else_expr) */
	      if (else_const != NULL && else_opr == OPR_MAX) {
		/* if (y>=x)
		 *    t = x;
		 * else 
		 *    t = MAX(y, else_const);
		...
		if (else_cvtl) { // check that CVTL is a nop
	      ...
	      res_op = OPR_MIN;
	    } else if (else_opr == then_opr) {
	      if (else_opr == OPR_MAX && 
		  (WN_tree_equiv(left, WN_kid0(then_expr)) ||
		   WN_tree_equiv(left, WN_kid1(then_expr))) &&
		  (WN_tree_equiv(right, WN_kid0(else_expr)) ||
		   WN_tree_equiv(right, WN_kid1(else_expr)))) {
		/* generate MAX(right, then_expr) */
		res_op = OPR_MAX;
	      } else if (else_opr == OPR_MIN &&
			 (WN_tree_equiv(right, WN_kid0(then_expr)) ||
			  WN_tree_equiv(right, WN_kid1(then_expr))) &&
			 (WN_tree_equiv(left, WN_kid0(else_expr)) ||
			  WN_tree_equiv(left, WN_kid1(else_expr)))) {
		/* generate MIN(left, then_expr) */
		res_op = OPR_MIN;
	      } else {
		break;
	      }
		...
		min_max = WN_CreateExp2(OPCODE_make_op(res_op, WN_desc(cond),
					       MTYPE_V),
				(res_op == OPR_MAX) ? right : left, 
				then_expr);
		WN_kid0( store ) = min_max;
		WN_Set_Linenum( store, WN_Get_Linenum(wn) );
	  ...
	  case OPR_LE:
		...
		min_max = 
		  WN_CreateExp2(OPCODE_make_op(res_op, WN_desc(cond),
					       MTYPE_V),
				(res_op == OPR_MIN) ? right : left, 
				then_expr);
		WN_kid0( store ) = min_max;
		WN_Set_Linenum( store, WN_Get_Linenum(wn) );
		Add_one_stmt( store, NULL );
...
#ifdef TARG_XTENSA
    // don't generate select for long long or double
    if (MTYPE_is_longlong(dsctyp) || MTYPE_is_double(dsctyp))
      goto end_label;
#endif

    // profitability check
#ifdef TARG_XTENSA
    // Do not if convert if 
    // 	1. The conditional constant is non-zero or opr = LE or GT
    // 	2. There is either a then or an else and not both
    // 	3. The store is to a global variable
    // 	This avoids expensive speculative stores where Xtensa 
    // 	probably doesn't have conditional moves
    if ((_opt_stab->External(WN_aux(store)) || _opt_stab->Local_static(WN_aux(store))) &&
	(empty_then || empty_else)) {
      if (op == OPR_LE || op == OPR_GT) { 
	goto end_label;
      } 
      if (WN_kid_count(cond) != 2) {
	goto end_label;
      }
      if ((WN_operator(WN_kid0(cond)) != OPR_INTCONST) && 
          (WN_operator(WN_kid1(cond)) != OPR_INTCONST)) {
	goto end_label;
      }
      if ((WN_operator(WN_kid0(cond)) == OPR_INTCONST && 
			    WN_const_val(WN_kid0(cond)) != 0) ||
        (WN_operator(WN_kid1(cond)) == OPR_INTCONST && 
			    WN_const_val(WN_kid1(cond)) != 0)) {
	goto end_label;
      }
    }
#endif

    WN *then_tmp_expr = then_expr;
    while (WN_operator(then_tmp_expr) == OPR_CVT ||
	   WN_operator(then_tmp_expr) == OPR_CVTL) {
      then_tmp_expr = WN_kid0(then_tmp_expr);
    }
    then_opr = WN_operator(then_tmp_expr);

    WN *else_tmp_expr = else_expr;
    while (WN_operator(else_tmp_expr) == OPR_CVT ||
	   WN_operator(else_tmp_expr) == OPR_CVTL) {
	    else_tmp_expr = WN_kid0(else_tmp_expr);
    }
    else_opr = WN_operator(else_tmp_expr);
    ...
    if ( // The STID <var> is not volatile
      ...
      // is a const or LDA.
      ((then_opr == OPR_LDID && 
	_opt_stab->Safe_to_speculate(WN_aux(then_tmp_expr)) &&
	!_opt_stab->Is_volatile(WN_aux(then_tmp_expr))) ||
	...
	then_opr == OPR_CONST)) &&
      ...
      // is a const or LDA.
      ((else_opr == OPR_LDID && 
	_opt_stab->Safe_to_speculate(WN_aux(else_tmp_expr)) &&
	!_opt_stab->Is_volatile(WN_aux(else_tmp_expr))) ||
       (else_opr == OPR_INTCONST ||
	else_opr == OPR_CONST))
      ||

      // Same ILOAD from different field
      (then_opr == OPR_ILOAD && else_opr == OPR_ILOAD &&
       WN_tree_equiv(WN_kid0(then_tmp_expr), WN_kid0(else_tmp_expr)) &&
       TY_pointed(WN_load_addr_ty(then_tmp_expr)) == 
       TY_pointed(WN_load_addr_ty(else_tmp_expr)) &&
       WN_field_id(then_tmp_expr) != WN_field_id(else_tmp_expr)))
    {
...
CFG::Connect_agotos()
{
  ...
  // Connect agotos
  if ((_rgn_level == RL_RVI2 ||
	// for -O3, pre-opt in LNO may cause label
	// to be un-reachable in main opt but is address taken
	Opt_Level==3 && _rgn_level == RL_MAINOPT) &&
      _agoto_pred_vec.Lastidx() < 0 && 
		_agoto_succ_vec.Lastidx() >= 0) {
     // special case where the address of a label is taken but no gotos 
     // in the function, leave the label in in case the user is doing something
     // funky
     for (i=0; i<=_agoto_succ_vec.Lastidx(); i++) {
	_entry_vec.AddElement(_agoto_succ_vec[i]);
     }
  } else {
    for (i = 0; i <= _agoto_pred_vec.Lastidx(); i++) {
      for (j = 0; j <= _agoto_succ_vec.Lastidx(); j++) {
...
void    
CFG::Invalidate_and_update_aux_info(bool is_whirl)
{
  // handle blocks that don't exit
  Process_multi_entryexit( is_whirl );
...
INT
CFG::Remove_critical_edge(bool is_whirl)
{
  INT inserted = 0;
  ...
  FOR_ALL_NODE( bb, cfg_iter, Init() ) {
    ...
    if ( (bb->Succ()->Len() > 1 && 
	  (
	   ...
	   ||
	   is_whirl && bb->Branch_wn() && 
	   WN_operator(bb->Branch_wn()) != OPR_AGOTO
	   )
	...
	if ( succ->Pred()->Len() > 1) {
	  if (!Is_backedge(bb, succ) ||
	    ...
	    if (!is_whirl) {
	      ...
	      inserted++;
	    } else if (bb->Next() == succ) {
	      // for WHIRL node, only if the edge is a fall through
	      BB_NODE *new_bb = Add_bb_to_edge(bb, succ);
	      inserted++;
	    }
	  }
  ...
  if (inserted > 0) {
    Invalidate_and_update_aux_info(is_whirl);
    Invalidate_loops();
    Analyze_loops();
  }
