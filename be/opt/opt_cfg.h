...
// Module: opt_cfg.h
// $Revision: 1.170 $
// $Date: 2000/04/06 15:21:46 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_cfg.h,v $
//
// ====================================================================
//
/* 
...
#ifdef _KEEP_RCS_ID
static char *opt_cfgrcs_id =	opt_cfg_INCLUDED"$Revision: 1.170 $";
#endif /* _KEEP_RCS_ID */
...
public:
  ...
  void         Invalidate_and_update_aux_info(bool is_whirl=false);
  ...
  INT          Remove_critical_edge(bool is_whirl=false);
...
class DFSBB_ITER {
  ...
  BB_NODE *Dfs_Bb(INT32 n) const{ return ((n >= 0 && n < Bbs_count()) ? _dfs_bbs[n] : NULL); }
...
class POBB_ITER {
  ...
  BB_NODE *Po_Bb(INT32 n) const{ return ((n >= 0 && n < Bbs_count()) ? _po_bbs[n] : NULL); }
  INT32    Bbs_count(void) const{ return _bbs_count; }
...
class DPOBB_ITER {
  ...
  BB_NODE *Dpo_Bb(INT32 n) const{ return ((n >= 0 && n < Bbs_count()) ? _dpo_bbs[n] : NULL); }
  INT32    Bbs_count(void) const{ return _bbs_count; }
