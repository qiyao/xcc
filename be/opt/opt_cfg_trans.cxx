...
reconstruct_CFG(successor_graph& g, CFG *cfg, bool trace)
{
  ...
  if (trace) {
    ...
    cfg->Print(TFile, false, (IDTYPE) -1);
    if (cfg->Feedback())
...
struct comp_zones {
  ...
  bool operator()(int x, int y) const {
    ...
    // this is needed to get around g++ bug on x86
    // basically, when x==y, the vx is stored to a temp mem
    // and then reloaded until vy is computed
    // so vx go through truncation/extension while vy does not
    // (note x86 double is 80-bits internally)
    // so vx!=vy even when x==y which results in
    // segfaults in the sort functions which uses this comparator
    if (x==y)
      return false;
...
{
  // check OPT_Space 
  if (z.code_expansion() > 0 && OPT_Space) {
    return false;
  }
