...
static char *rcs_id = 	opt_combine_CXX"$Revision: 1.13 $";
#endif /* _KEEP_RCS_ID */
...
#ifdef TARG_XTENSA
  /* For Xtensa FPU, don't uncombine a * recip(b) if hardware divide
     is not supported but recip is supported. */
  extern DLL_SHARED BOOL xt_hard_float_div;
  extern DLL_SHARED BOOL xt_hard_float_recip;
  if (WN_rtype(old_wn) == MTYPE_F4 &&
      !xt_hard_float_div &&
      xt_hard_float_recip)
    return FALSE;
#endif
