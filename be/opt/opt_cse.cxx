...
// Module: opt_cse.cxx
// $Revision: 1.156 $
// $Date: 2000/04/06 15:22:05 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_cse.cxx,v $
//
// ====================================================================
//
/* 
			     ...
			     CODEREP *old_temp, CODEREP *multiplier ) const
{
  ...
  bool is_cvtl = false;
  if ( Worklist()->Exp()->Kind() == CK_OP &&
	 Worklist()->Exp()->Opr() == OPR_CVTL )
    {
      is_cvtl = true;
      // Cand: cvtl(i)
      // Iv  : i=cvtl(i+incr_amt)
      int cvtl_size = Worklist()->Exp()->Offset();

      OPCODE cvtl_opc = OPCODE_make_op(OPR_CVTL,
				    old_temp->Dtyp(),
				    MTYPE_V);
      new_rhs = Htable()->Add_unary_node_and_fold(cvtl_opc,new_rhs);
      new_rhs->Set_offset(cvtl_size);
  }
  ...
  if (MTYPE_IS_INTEGER(OPCODE_desc(new_stid->Op())) && !is_cvtl) {
