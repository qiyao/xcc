...
class DCE {
  private:
    ...
    BOOL         _need_rename;      // need rename due to transformation
    ...
    void Set_need_rename(void) { _need_rename = TRUE; } 
    ...
    BOOL Defined_by_zero_version(const CODEREP *cr) const;
    ...
    void Mark_chinode_live( CHI_NODE *chi, STMTREP *def_stmt , BOOL forced = FALSE ) const;
    ...
    BOOL logif_Is_potential_infinite_loop( BB_NODE *bb ) const;
...
public:
    ...
    BOOL Need_rename(void) const { return _need_rename; } 
...
DCE::Check_constant_cond_br( BB_NODE *bb ) const
{
  ...
  if ( Tracing() ) {
    fprintf( TFile, "DCE::Remove_br in bb:%d (%lx)\n",bb->Id(),bb);
  }
...
DCE::Replace_condition_with_constant( BB_NODE *bb, INT64 val ) const
{
  ...
  if ( Tracing() ) {
    fprintf( TFile, "Replaced bb:%d condition with %" LLD_FMT "\n",
	     bb->Id(), val );
  }
}
    ...
    if (bb1 != NULL) {
      ...
      for (stmt = bb->First_stmtrep(); stmt; stmt = next_stmt) {
	    ...
	    stmt->Opr() == OPR_EXC_SCOPE_END) {
          if (Tracing()) {
	    fprintf(TFile,
	    stmt->Opr() == OPR_EXC_SCOPE_BEGIN ?
	      "Moving OPR_EXC_SCOPE_BEGIN from bb:%d to bb:%d\n":
	      "Moving OPR_EXC_SCOPE_END from bb:%d to bb:%d\n",
	    bb->Id(), bb1->Id());
	  }
...
DCE::Unreachable_code_elim( void ) const
{
  ...
  for ( bb = _cfg->First_bb(); bb != NULL; bb = nextbb ) {
    ...
    } else if (bb->Labnam() && LABEL_addr_saved(bb->Labnam())) {
	; // be conservative with address taken labels
    } else {
      // found to be unreached
      if ( Tracing() ) {
	fprintf( TFile, "DCE: Removing stmts in BB%d (0x%lx)\n", 
		 bb->Id(), bb );
      }
...
DCE::Required_store( const STMTREP *stmt, OPERATOR oper ) const
{
  ...
  // pr3538. for asms, we need the STID/LDID that loads the result value, if
  // the result has a tie type, even if that result is dead. We need
  // the STID/LDID so we can determine the result type, and thus know
  // what type of register to use.
  if (MTYPE_is_tie(lhs->Dsctyp()) || MTYPE_is_xtbool(lhs->Dsctyp())) {
    const CODEREP *rhs = stmt->Rhs();

    if (rhs->Kind() == CK_VAR) {
      ST *s = Opt_stab()->St(rhs->Aux_id());
      if (ST_class(s) == CLASS_PREG && rhs->Offset() < 0 &&
	  stmt->Is_required_by_asm()) {
	return TRUE;
      }
    }
  }
  ...
  else if (ST_has_nested_ref(s)) {
    // has_nested_ref symbols can be accessed by run-time so we
...
{
  Warn_todo( "DCE::Required_call", stmt,oper );
  return ( TRUE );
}
...
DCE::Loop_pragma(WN_PRAGMA_ID pragma) const
{
  switch (pragma) {
    ...
    case WN_PRAGMA_SIMD_IF_CONVERT:
...
DCE::Required_stmt( const STMTREP *stmt ) const
{
  ...
  case OPR_GOTO:
    if (opr==OPR_TRUEBR || opr==OPR_FALSEBR) {
      BB_NODE* bb = stmt->Bb();
      if (stmt==bb->Branch_stmtrep() && logif_Is_potential_infinite_loop(bb)) {
	// PR8044 keep condition branch of potential infinite loop alive
	return TRUE;
      }
    }
...
DCE::Aliased( const POINTS_TO *pt1, const POINTS_TO *pt2 ) const
{
  ...
  /* handle MU in RETURN */ 
  if (pt1 == Opt_stab()->Points_to_globals()) {
    return ( _alias_rule->Aliased_with_Global(pt2) );
  } else if (pt2 == Opt_stab()->Points_to_globals()) {
    return ( _alias_rule->Aliased_with_Global(pt1) );
  }
...
BOOL
DCE::Defined_by_zero_version(const CODEREP *cr) const
{
  if (cr->Is_flag_set(CF_DEF_BY_CHI)) {
    return (cr->Defchi()->RESULT()->Is_flag_set(CF_IS_ZERO_VERSION));
  } else if (cr->Is_flag_set(CF_DEF_BY_PHI)) {
    return (cr->Defphi()->RESULT()->Is_flag_set(CF_IS_ZERO_VERSION));
  }
  return cr->Is_flag_set(CF_IS_ZERO_VERSION);
}
...
void
DCE::Mark_chinode_live( CHI_NODE *chi, STMTREP *def_stmt, BOOL forced ) const
{
  ...
  // This optimization was turned off because they create overlapped 
  // live-ranges, but the bug seems to be in not checking the 
  // MU operand matches the current 'CHI'. -- p.t.

  // may not need to make this chi live if it's not aliased
  if (!forced && def_stmt != NULL &&
      def_stmt->Opr() != OPR_OPT_CHI &&
      def_stmt->Opr() != OPR_REGION && !def_stmt->Black_box() &&
      Enable_dce_alias() &&
      Mu_stack()->Elements() > 0 &&
      !chi->RESULT()->Is_flag_set(CF_IS_ZERO_VERSION) &&
      !chi->RESULT()->Is_flag_set(CF_INCOMPLETE_USES) )
  {
    MU_NODE *tos_mu = Mu_stack()->Top();
    FmtAssert(Points_to_stack()->Elements() > 0, 
	      ("DCE: Points_to_stack is empty but Mu_stack is not"));
    if ( tos_mu->OPND() == chi->RESULT() &&
	 !Aliased( def_stmt->Points_to(Opt_stab()), 
		   Points_to_stack()->Top() ) &&
	 !Defined_by_zero_version(chi->OPND()) ) {
  ...
  // This stays to be off because it generate overlapping live range
#if 0
...
DCE::Mark_zero_version_chinode_live( STMTREP *stmt ) const
{
  ...
  FOR_ALL_NODE( chi, chi_iter, Init(chi_list)) {
	...
	Mark_chinode_live(chi, stmt, TRUE);
	made_chi_live = TRUE;
      }
    }
  }
...
//   }
  ...
  case OPR_BACKWARD_BARRIER:
	...
	Mark_chinode_live(cnode, stmt, TRUE);
      }
    }
    break;
...
// ====================================================================
// Check if a logif bb is in a common loop with some of its successors
// and there exists at least one succeesor which is not in that common
// loop then bb can be part of a conditional infinite loop
// ====================================================================

BOOL 
DCE::logif_Is_potential_infinite_loop( BB_NODE *bb ) const
{

  if (bb->Kind()!=BB_LOGIF || bb->Loopdepth()==0)
    return false;

  BB_LIST *succlist = NULL;
  BB_LOOP* common_loop = NULL;
  BOOL has_loop_exit = false;
  BOOL has_loop = false;
  for (succlist = bb->Succ(); succlist != NULL; succlist = succlist->Next()) {

    // does this successor share a common loop nest?
    BB_NODE *succ = succlist->Node();
    BB_LOOP* bb_loop = bb->Innermost();
    BB_LOOP* succ_loop = succ->Innermost();
    INT bb_depth = bb->Loopdepth();
    INT succ_depth = succ->Loopdepth();
    if (bb_loop==NULL || succ_loop==NULL) {
      has_loop_exit = true;
      continue;
    }

    while (bb_depth>succ_depth) {
      if (bb_loop) bb_loop = bb_loop->Parent();
      else
	break;
      bb_depth--;
    }
    while (bb_depth<succ_depth) {
      if (succ_loop) succ_loop = succ_loop->Parent();
      else
	break;
      succ_depth--;
    }

    if (bb_depth!=succ_depth) {
      has_loop_exit = true;
      continue;
    }

    while (bb_depth && bb_loop!=succ_loop) {
      if (bb_loop) bb_loop = bb_loop->Parent();
      else break;
      if (succ_loop) succ_loop = succ_loop->Parent();
      else break;
    ...
    if (bb_loop && bb_loop==succ_loop) {
      has_loop = true;
      if (common_loop == NULL)
	common_loop = bb_loop;
      else if (common_loop != bb_loop)
	has_loop_exit = true;
    }
  }

  // if not lowered fully (as in preopt), the loop analysis is less precise
  // so recognize more potential infinite loops edges to prevent
  // aggressive dead conditional branch removal in preopt
  if (!_cfg->Lower_fully()) {
    if (has_loop) {
      return true;
    }
  } else if (has_loop && has_loop_exit)
      return true;
...
DCE::Remove_dead_statements( void ) const
{
  ...
  for ( bb = _cfg->First_bb(); bb != NULL; bb = nextbb ) {
	...
	bb->Set_reached();
      ...
      if ( Tracing() ) {
	fprintf( TFile, "DCE_A: BB%d unreached (0x%lx)\n", bb->Id(), bb );
      }
	...
	if ( Tracing() ) {
	  fprintf( TFile, "DCE_A: Removed BB%d (%lx)\n", bb->Id(), bb );
	}
      }
      else {
	bb->Set_reached();
			     ...
			     BOOL *paths_removed)
{
  ...
  if (need_rename) {
    ...
    if (Get_Trace( TP_GLOBOPT, DCE_DUMP_FLAG )) {
      fprintf(TFile, "%sAfter rename in DCE\n%s", DBar, DBar );
      Cfg()->Print( TFile );
    }
  }
