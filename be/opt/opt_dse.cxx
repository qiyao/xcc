...
static char *rcs_id = 	opt_dse_CXX"$Revision: 1.110 $";
#endif /* _KEEP_RCS_ID */
...
DSE::Required_call( const WN *wn ) const
{
  ...
  // not a required call.
  Warn_todo("DSE::Required_call see if the callee has side-effects",wn);
  return ( TRUE );
}
...
// return true if the statement previous to 'wn' is an OPR_ASM_STMT,
// possibly separated by OPR_STIDs.
static BOOL Is_required_by_asm(const WN *wn)
{
  WN *prev = WN_prev(wn);
  while (prev) {
    OPERATOR oper = WN_operator(prev);
    if (oper == OPR_ASM_STMT) {
      return TRUE;
    }
    if (oper != OPR_STID) {
      break;
...
DSE::Required_stid( const WN *wn ) const
{
  ...
  if (MTYPE_is_tie(WN_desc(wn)) || MTYPE_is_xtbool(WN_desc(wn))) {
    WN *rhs = WN_kid0(wn);
    if (WN_operator(rhs) == OPR_LDID) {
      IDTYPE du = WN_ver(rhs);	// pointer to ver_stab
      ST *s = Opt_stab()->St(Opt_stab()->Du_aux_id(du));
      if (ST_class(s) == CLASS_PREG && WN_offset(rhs) < 0 &&
	  Is_required_by_asm(wn)) {
	// pr3538. for asms, we need the STID/LDID that loads the result
	// value, if the result has a tie type, even if that result is
	// dead. We need the STID/LDID so we can determine the result
	// type, and thus know what type of register to use.
	return TRUE;
      }
    }
  }
