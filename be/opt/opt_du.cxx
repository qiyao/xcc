...
// Module: opt_du.cxx
// $Revision: 1.129 $
// $Date: 2000/04/06 15:22:33 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_du.cxx,v $
//
// Revision history:
...
static char *rcs_id = 	opt_du_CXX"$Revision: 1.129 $";
#endif /* _KEEP_RCS_ID */
...
// ====================================================================
// Return the WN * of the outermost DO loop in 'loop' that
// encloses 'wn_bb'.
// ====================================================================

static WN *
Find_loop_stmt (BB_LOOP *loop, BB_NODE *wn_bb)
{
  if (!loop)
    return NULL;

  BB_LOOP *tloop = wn_bb->Innermost();
  if (!loop->Contains(tloop))
    return NULL;

  WN *loop_stmt = NULL;
  while (true) {
    Is_True(tloop, ("Null BB_LOOP"));

    WN *tloop_stmt = tloop->Loopstmt();
    if (!tloop_stmt || WN_operator(tloop_stmt) != OPR_DO_LOOP)
      break;
    
    loop_stmt = tloop_stmt;
...
static void
Update_loop_info( DU_MANAGER *du_mgr, WN *wn, BB_NODE *wn_bb )
{
  /* Now update the loop information for this variable, if any.
     We change it from a BB_LOOP structure to a WN *. */
  DEF_LIST *deflist = du_mgr->Ud_Get_Def(wn);
  if (deflist) {
    WN *loop_stmt = Find_loop_stmt(deflist->Loop_info(), wn_bb);
    deflist->Set_loop_stmt(loop_stmt);
    
    if ( du_mgr->Tracing() && loop_stmt != NULL ) {
      fprintf( TFile, "Use WN:<%d:%d> has Loop_stmt WN:<%d:%d> %s\n",
	       OPCODE_mapcat(WN_opcode(wn)),
	       WN_map_id(wn),
	       OPCODE_mapcat(WN_opcode(loop_stmt)),
	       WN_map_id(loop_stmt),
	       OPCODE_name(WN_opcode(loop_stmt)) );
    }
  }
}
...
EMITTER::Compute_use_def(DU_MANAGER *du_mgr)
{
  ...
  if (0 && du_mgr->Opt_phase() == PREOPT_LNO_PHASE && 
      !Has_do_loop() &&
