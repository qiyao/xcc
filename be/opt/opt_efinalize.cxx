...
// $Revision: 1.42 $
// $Date: 2000/09/05 03:03:08 $
// $Author: murthy $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_efinalize.cxx,v $
//
// ====================================================================
//
/* 
...
bool
Different_TIE_Iload_Occur(EXP_OCCURS *occ1, EXP_OCCURS *occ2) {

  if (occ1==NULL || occ2==NULL)
    return false;

  CODEREP* cr1 = occ1->Occurrence();
  CODEREP* cr2 = occ2->Occurrence();

  if (cr1==NULL || cr2==NULL)
    return false;

  OPERATOR opr1 = (cr1->Kind()==CK_IVAR)?cr1->Opr():OPERATOR_UNKNOWN;
  OPERATOR opr2 = (cr2->Kind()==CK_IVAR)?cr2->Opr():OPERATOR_UNKNOWN;

  if (opr1!=OPR_ILOAD || opr2!=OPR_ILOAD)
    return false;

  MTYPE dtyp1 = cr1->Dtyp();
  MTYPE dtyp2 = cr2->Dtyp();
  MTYPE dsctyp1 = cr1->Dsctyp();
  MTYPE dsctyp2 = cr2->Dsctyp();

  if ((MTYPE_is_tie(dtyp1) || MTYPE_is_tie(dtyp2) ||
       MTYPE_is_tie(dsctyp1) || MTYPE_is_tie(dsctyp2)) &&
      (dtyp1!=dtyp2 || dsctyp1 != dsctyp2)) {
    return true;
  }
					       ...
					       EXP_OCCURS_PAIR *comp_occurs)
{
  ...
  FOR_ALL_NODE(occ, occ_iter, Init()) {
	...
	if (def == NULL || !def->Bb()->Dominates(occ->Bb()) ||
	    Different_TIE_Iload_Occur(def,occ)) {
