...
// $Revision: 1.59 $
// $Date: 2000/04/06 15:22:48 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_ehoist.cxx,v $
//
// ====================================================================
//
/* 
...
private:
  ...
  UINT           _injured:1;
  UINT           _aggressive:1;  // aggressively anticipated 
  EXP_ID         _exp_id:30;     // the exp_id that the following information represents
...
public:
  ...
  BOOL      Aggressive(void)       { return (_aggressive != 0); }
  HOIST_SUMMARY *Next(void) const  { return _next; }
  HOIST_SUMMARY *Cd_hs(void) const { 
    /* This is kind of a hack, but it helps us keep the per-expression
       initialization sparse. Init_per_expr() is called for those
       HOIST_SUMMARY objects associated with BBs containing real or
       phi-pred occurrences, but the Cd_hs() BB of such an occurrence
       might not contain a real or a phi-pred occurrence itself. Hence
       the initialization might not have occurred yet for the Cd_hs()
       object, and we need to do it here. */
    if ((_cd_hs != NULL) && (_cd_hs->_exp_id != _exp_id)) {
      _cd_hs->Init_per_expr (_exp_id);
    }
    return _cd_hs; 
  }
  ...
  void  Set_aggressive(void)                { _aggressive = 1;      }
...
public:
  ...
  void   Update_succ_count_rec(HOIST_SUMMARY *, UINT, CODEREP *, EXP_OCCURS *,
			       BOOL aggr=FALSE);
...
//
void
EXP_HOISTING::Update_succ_count_rec(HOIST_SUMMARY *hs, UINT which_succ, CODEREP *expr, EXP_OCCURS *exp_occur, BOOL aggr)
{
  ...
  if (!hs->Succ_processed(which_succ)) {
    ...
    if (exp_occur == NULL && hs->Exp_occur()) {
      exp_occur = hs->Exp_occur();
    }
    if ((hs->Anticipated() // this node becomes fully anticipated
	 || hs->Aggressive()) // aggresive
	&&              
...
{
  Is_True(hs->Anticipated() || hs->Aggressive(),
  ...
  if (hs->Exp_id() == Cur_exp_id() &&
...
#else
    // The above comment is not quite true in the case when the
    // current expression is an ILOAD. In particular, case (2) is a
    // little bit too strict. The following can happen, where p and q
    // are pointers whose indirects are assigned the same vsym v:
    //
    //                bb1:
    //           ... *p mu(v_1) (e-ver 1 def_occur)
    //           /          \
    //          /            \
    //        bb2:          bb3:
    //      ... *p ...       *q =     (v_2 <- chi(v_1))
    ...
    //                         ... *p mu(v_2) (e-ver 1)
    //
    // In this example, all the *p occurrences have e-version 1
    // because, even though there is a chi for *p's vsym in BB3, alias
    // analysis proves that the store to *q doesn't alias with *p
    // (this alias query takes place in, e.g.,
    // ESSA::Ilod_modified_real_occ_real_occ()). We will consider
    // hoisting (and indeed we will typically hoist) from bb4 and bb5
    // to bb3, but the def-occur for e-version 1 is above the chi,
    // hence the vsym (i.e., the Ivar_mu_node()->OPND()) is different
    // between hs->Hoisted_occur()->Occurrence() and
    // def_occur->Occurrence(), which causes CR_match_kids() to be
    // false, indicating a mismatch and causing the above assertion to
    // fail.
    //
    // Because of this case we replace the above assertion with a more
    // relaxed version. class ESSA's member functions are out of scope
    // here, so we unfortunately can't easily do a lot of checking in
    // the ILOAD case. What we would like to assert in that case is
    // that if CR_match_kids (with arguments as in the stricter
    // assertion above) returns false, we can follow the U-D chain of
    // Ivar_mu_node()->OPND() from hs->Hoisted_occur()->Occurrence()
    // through a sequence of unaliased chi nodes back to
    // def_occur->Occurrence()->Ivar_mu_node().
    // -- RK 20050606
    Is_True(hs->Hoisted_occur()->Occurrence() == cr ||
	    cr->Kind() == CK_IVAR ||
	    cr->Coderep_id() == 0 || // inserted expr not rehashed
...
EXP_HOISTING::Generate_hoisted_occur(EXP_WORKLST *exp_worklst)
{
  ...
  FOR_ALL_NODE (real_occ, real_occ_iter, Init(Exp_worklst()->Real_occurs().Head())) {
    ...
    /* be aggressive if more than one real occ, and,
       the expression is safe to speculate */
    BOOL aggr = (Exp_worklst()->Real_occurs().Head() != 
		 Exp_worklst()->Real_occurs().Tail()) && 
      inCODEKIND(real_occ->Occurrence()->Kind(), CK_RCONST|CK_CONST|CK_LDA);
	...
	hs->Def_occur_allows_hoisting(real_occ->Def_occur())) {
      ...
      Update_succ_count_rec(hs->Cd_hs(), hs->Which_cd_succ(), real_occ->Occurrence(), real_occ, aggr);
    }
  }
  ...
  FOR_ALL_NODE (phi_pred, phi_pred_iter, Init(Exp_worklst()->Phi_pred_occurs().Head())) {
	...
	if (exp_phi->Need_insertion(opnd_num)) {
	  ...
	  /* be aggressive if the expression is safe to speculate */
	  BOOL aggr = inCODEKIND(insert_cr->Kind(), CK_RCONST|CK_CONST|CK_LDA);
	  ...
	  if (hs->Cd_hs() != NULL &&           // it has a CD
	    ...
	    /* set the aggressive anticipated for hs->Cd_hs() */
	    Append_hoist_worklist(hs);
	    hs->Set_exp_occur(phi_pred);
	    Update_succ_count_rec(hs->Cd_hs(), hs->Which_cd_succ(), insert_cr, NULL, aggr);
	    break;
	  }
	}
      }
