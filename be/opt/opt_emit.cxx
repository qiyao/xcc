...
static char *rcs_id = 	opt_emit_CXX"$Revision: 1.270 $";
#endif /* _KEEP_RCS_ID */
...
Valid_doloop_condition( BB_NODE *bb_end, const OPT_STAB *opt_stab )
{
  ...
  for (INT i = 0; i < rhs->Kid_count(); i++)
    if (loop_info->Index_relative_expr(rhs->Opnd(i), index_cr, 0))
...
Build_new_loop_info( WN *do_loop, WN *old_info )
{
  ...
  if ( old_info != NULL ) {
    ...
    at_region_id = WN_loop_at_region_id(old_info);
  }
  else {
    est_trips = 0;
    depth = 0;
    lflags = 0;
    at_region_id = AT_REGION_ID_UNKNOWN;
  }
  ...
  if ( old_info == NULL || new_kid_count != WN_kid_count(old_info) ) {
    new_info = WN_CreateLoopInfo( induction, trip_count,
				  est_trips, depth, lflags, at_region_id );
  }
  else {
    ...
    WN_loop_at_region_id(old_info) = at_region_id;
...
{
  FmtAssert(FALSE, ("opt_emit: Raise unknown statement", emitter,bb ));
  return NULL;
}
  ...
  while (bb != NULL) {
	...
	goto ordinary;
      ...
      if (bb->Kind() == BB_WHILEEND) {
        if (prev_bb) {
          prev_bb_first = prev_bb->Firststmt();
          prev_bb_last = prev_bb->Laststmt();
        }
	...
	if (prev_bb->Laststmt() != NULL)
	  ...
	  prev_bb->Set_laststmt(bb->Laststmt());
          if (prev_bb->Prev() && 
              prev_bb->Prev()->Firststmt() == prev_bb_first &&
              prev_bb->Prev()->Laststmt() == prev_bb_last) {
            prev_bb->Prev()->Set_firststmt(bb->Firststmt());
            prev_bb->Prev()->Set_laststmt(bb->Laststmt());
          }
	}
      }
