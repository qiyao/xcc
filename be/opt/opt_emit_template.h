  ...
  case CK_OP:
...
#ifdef TARG_XTENSA
	      else if (exp->Dsctyp() == MTYPE_I4 && exp->Dtyp() == MTYPE_U8) {
		// U8I4CVT (I4I4LDID) should become I8I4LDID
		// else we loose sign extension
		kid->Set_dtyp(MTYPE_I8);
	      }
#endif
	...
	if (opr == OPR_ARRAY) {
	  ...
	  if (opr==OPR_INTRINSIC_CALL) {
	    emitter->Alias_Mgr()->
		Gen_alias_id(wn, exp->Points_to(emitter->Opt_stab()));
	...
	if (proj_defstmt != NULL &&
	  ...
	  wn = WN_CreateExp1(exp->Op(),
			     Gen_exp_wn(exp->Get_opnd(0), emitter));
	  if (exp->Opr()==OPR_OUTPART) {
	    FmtAssert(exp->Offset()!=0,("Missing offset for OPR_OUTPART"));
	    WN_outpart(wn)=exp->Offset();
	  }
	  ...
	  // match for MIN/MAX
	  if (exp->Opr()==OPR_SELECT && exp->Get_opnd(0)->Non_leaf()) {

	    CODEREP* cond_cr = exp->Get_opnd(0);
	    CODEREP* opnd1_cr = exp->Get_opnd(1);
	    CODEREP* opnd2_cr = exp->Get_opnd(2);
	    OPERATOR cond_opr=cond_cr->Opr();
	    TYPE_ID cond_type = cond_cr->Dsctyp();
	    TYPE_ID select_type = exp->Dtyp();

	    if ((cond_opr==OPR_LT ||
		 cond_opr==OPR_LE ||
		 cond_opr==OPR_GT ||
		 cond_opr==OPR_GE) &&
		MTYPE_is_integral(cond_type) &&
		cond_type == select_type &&
		MTYPE_byte_size(cond_type)==4) {

	      bool is_signed = MTYPE_signed(cond_type);
	      CODEREP* cond_opnd0_cr = cond_cr->Get_opnd(0);
	      CODEREP* cond_opnd1_cr = cond_cr->Get_opnd(1);

	      OPERATOR minmax_opr = OPERATOR_UNKNOWN;
	      if ((cond_opnd0_cr == opnd1_cr && cond_opnd1_cr == opnd2_cr)) {
		if (cond_opr==OPR_LT || cond_opr==OPR_LE)
		  minmax_opr = OPR_MIN;
		else
		  minmax_opr = OPR_MAX;
	      } else if (cond_opnd0_cr == opnd2_cr &&
			 cond_opnd1_cr == opnd1_cr) {
		if (cond_opr==OPR_LT || cond_opr==OPR_LE)
		  minmax_opr = OPR_MAX;
		else
		  minmax_opr = OPR_MIN;
	      ...
	      if (minmax_opr == OPR_MIN) {
		wn = WN_CreateExp2(is_signed? OPC_I4MIN : OPC_U4MIN,
					opnd1, opnd2);
	      } else if (minmax_opr == OPR_MAX) {
		wn = WN_CreateExp2(is_signed? OPC_I4MAX : OPC_U4MAX,
					opnd1, opnd2);
	      }
	    }
	  }
	...
	/* Put back any arithmetic flags.*/
	if (opr == OPR_MPY)
	  WN_arith_flag(wn) = exp->Arith_flags();
      }
      break;
    }
    break;
...
//                            emitter->Alias_Mgr());
  ...
  case OPR_DEALLOCA:
	...
	WN_call_flag(rwn) = srep->Call_flags();
      if (opr ==OPR_FORWARD_BARRIER || opr ==OPR_BACKWARD_BARRIER)
	WN_barrier_flag(rwn) = WN_barrier_flag(srep->Barrier());
...
#ifdef TARG_XTENSA
	       && (rhs_cr->Dsctyp() != MTYPE_I8 && rhs_cr->Dsctyp() != MTYPE_U8 && rhs_cr->Get_opnd(0)->Dtyp() != MTYPE_I8 && rhs_cr->Get_opnd(0)->Dtyp() != MTYPE_U8)
#endif	       
	       ) {
  ...
  case OPR_ISTBITS:
	...
	return FALSE;	// omit generating this istore
      ...
      CODEREP *lhs = srep->Lhs();
      /* CVTL-RELATED start (performance) */
      if ( rhs_cr->Kind() == CK_OP && 
	   rhs_cr->Opr() == OPR_CVTL  &&
	   rhs_cr->Get_opnd(0)->Kind() == CK_OP &&
	   rhs_cr->Get_opnd(0)->Opr() == OPR_CVT &&
	   MTYPE_is_tie(rhs_cr->Get_opnd(0)->Dsctyp())) {
	   // CVTL for TIE CVT need to be preserved (PR4642)
      } else if ( !emitter->For_preopt() &&
	  WOPT_Enable_Cvt_Folding &&
	  rhs_cr->Kind() == CK_OP && 
	  ( rhs_cr->Opr() == OPR_CVT &&
	   MTYPE_is_integral( rhs_cr->Dsctyp() ) 
