...
{ 
  ...
  case OPR_CVT:
      {
	TYPE_ID src_mtype = Dsctyp();
	TYPE_ID dst_mtype = Dtyp();
...
EOCC::Collect_real_occurrences( void )
{
  ...
  FOR_ALL_ELEM (bb, dpo_iter, Init()) {
	...
	if (OPERATOR_is_scalar_store (stmt_opr) &&
	  ...
	  if (WOPT_Enable_Cvt_Folding && rhs_cr->Kind() == CK_OP && 
	      (rhs_cr->Opr() == OPR_CVT && MTYPE_is_integral(rhs_cr->Dsctyp()) 
		   ...
		   MTYPE_is_tie(rhs_cr->Get_opnd(0)->Dsctyp())==false))) &&
    ...
    if (phi_list) {
      ...
      PHI_NODE *last = NULL;
      PHI_NODE *next_phi;
      for (phi = phi_list->Head(); phi != NULL; phi = next_phi) {
	next_phi = phi->Next();
	if (phi->Dse_dead()) {
	  Is_Trace(Tracing(),(TFile,
			      "====== EOCC::Collect_real_occurrences, Delete dead Phi ======\n"));
	  Is_Trace_cmd(Tracing(),phi->Print(phi_list->In_degree(),TFile));

	  Etable()->Htable()->Remove_var_phi_hash(phi);
      ...
      phi_list->Set_Head(first);
      phi_list->Set_Tail(last);
#endif
