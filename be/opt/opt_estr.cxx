...
// Module: opt_estr.cxx
// $Revision: 1.48 $
// $Date: 2000/04/06 15:23:08 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_estr.cxx,v $
//
// ====================================================================
//
/* 
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = opt_estr_CXX"$Revision: 1.48 $";
#endif /* _KEEP_RCS_ID */
...
{
  if (cr->Kind() == CK_OP && cr->Opr() == OPR_CVTL) {
    return (cr->Offset() == 16);
  }
...
STR_RED::Is_const_or_loop_invar( CODEREP *expr, BB_NODE *bb ) const
{
  if ( inCODEKIND(expr->Kind(), CK_LDA|CK_CONST|CK_RCONST) ) {
    ...
    // treat cvt of an int const as const
    // this may be created from repairing an injury and the
    // cvt cannot be folded (e.g., I8I4CVT ( INTCONST ))
    return TRUE;
  }
  else {
			   ...
			   CODEREP **multiplier ) const
{
  ...
  if ( use_cr->Kid_count() == 2 ) {
	...
	*multiplier = NULL;
    ...
    const OPERATOR opr = use_cr->Opr();
    Is_True( opr == OPR_NEG || opr == OPR_CVT || opr == OPR_CVTL,
    ...
    *iv_use = use_cr->Opnd(0);
    ...
    else if ( opr == OPR_CVT || opr == OPR_CVTL ) {
      // multiplier is 1
      *multiplier = NULL;
    }
    else {
