...
// $Revision: 1.295 $
// $Date: 2000/08/03 23:54:11 $
// $Author: mpm $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_etable.cxx,v $
//
// ====================================================================
//
/* 
...
#include "config_targ_options.h"// xt_hard_float
...
#ifdef TARG_XTENSA
#include "libti.h"		// TI_ISA_LC_Value_In_Class
#endif
...
EXP_OCCURS::Stmt(void) const
{
  ...
  if (!stmt->Is_RHS_saved()) return stmt;
...
/* Find the cost for rematerialization of an INTCONST of type 'from_type' and
   converting it into a TIE type 'to_type' using the best available rtor or mtor
   proto. Return -1 if the rematerialization is not possible. */

static INT
int_to_tie_convert_rematerial_cost (TYPE_ID from_type, TYPE_ID to_type)
{
  INT cost = -1;

  {  
    TIE_MACRO_p rtor = tie_info->mtype_rtor_macro(from_type, to_type);
    if (rtor) {
      if (!rtor->is_pure() || rtor->has_side_effect())
	return -1;
      
      // l32r <intconst> + rtor
      INT rtor_cost = 1 + rtor->num_instructions();
      if (cost < 0 || rtor_cost < cost)
	cost = rtor_cost;
    }
  }

  {
    TIE_MACRO_p mtor = tie_info->mtype_mtor_macro(from_type, to_type);
    if (mtor) {
      if (mtor->has_side_effect())
	return -1;

      // l32r <.rodata> + mtor
      INT mtor_cost = 1 + mtor->num_instructions();
      if (cost < 0 || mtor_cost < cost)
	cost = mtor_cost;
    }
  }
...
static BOOL
create_homeable_tie_preg (CODEMAP *htable, CODEREP *exp, AUX_ID &id)
{
  if (exp->Kind() != CK_OP)
    return FALSE;
  
  TYPE_ID rtype = exp->Dtyp();
  
  INT rematerial_cost = -1;
  
  // Handled TIE operations:
  //   x = _TIE_op(<int_const>)
  //   x = (_TIE_type)<int_const>
  
  BOOL is_tie_op = (exp->Opr() == OPR_INTRINSIC_OP &&
		    INTRN_is_tie_intrinsic(exp->Intrinsic()));
  
  if (is_tie_op) {
    TIE_MACRO_p tie_macro = tie_info->tie_macro(Intrinsic_To_Tie_Macro_Id(exp->Intrinsic()));
    if (!tie_macro->is_whirl_intrinsic_op() || tie_macro->num_protos() != 2)
      return FALSE;
    
    // Re-materialization cost includes a l32r for the int const
    rematerial_cost = tie_macro->num_instructions() + 1;

    // Return value needs to be sign- or zero- extended.
    if (tie_macro->output_mtype(tie_info) != rtype)
      rematerial_cost++;
  }
  
  BOOL is_tie_cvt = (exp->Opr() == OPR_CVT &&
		     MTYPE_is_tie(rtype) &&
		     MTYPE_is_integral(exp->Dsctyp()));
  
  if (!is_tie_op && !is_tie_cvt)
    return FALSE;
  
  // can re-materialize single kid exp only
  // e.g. a TIE intrinsic with inout or multiple inputs cannot be re-materialized
  if (exp->Kid_count()!=1)
    return FALSE;
  
  CODEREP *parm_cr = exp->Opnd(0);
  CODEREP *intconst_cr = parm_cr;

  if (is_tie_op) {
    if (parm_cr->Kind() != CK_IVAR)
      return FALSE;
    intconst_cr = parm_cr->Ilod_base();
  }
  
  if (intconst_cr->Kind() != CK_CONST)
    return FALSE;
  
  if (is_tie_cvt) {
    rematerial_cost = int_to_tie_convert_rematerial_cost(exp->Dsctyp(), rtype);
    if (rematerial_cost < 0)
      return FALSE;
  }
  
  Is_True(rematerial_cost >= 0, ("Bad rematerialization cost"));
  
  TIE_MACRO_p storei_macro = tie_info->mtype_storei_macro(rtype);
  TIE_MACRO_p loadi_macro = tie_info->mtype_loadi_macro(rtype);
  INT spill_cost = (MTYPE_is_tie_packed(rtype) ? 0 :
		    ((storei_macro && loadi_macro) ?
		     (tie_info->mtype_storei_macro(rtype)->num_instructions() +
		      tie_info->mtype_loadi_macro(rtype)->num_instructions()) :
		     (MTYPE_is_xtbool(rtype) ? 10 : 2)));
  
  if (spill_cost < rematerial_cost)
    return FALSE;
  
  WN *intconst_wn = WN_Intconst(intconst_cr->Dtyp(), intconst_cr->Const_val());
  WN *home_wn = NULL;

  if (is_tie_op) {
    WN *parms[1];
    parms[0]= WN_CreateParm(parm_cr->Dtyp(), intconst_wn,
			    parm_cr->Ty_index(), WN_PARM_BY_VALUE);
    
    home_wn = WN_Create_Intrinsic(OPR_INTRINSIC_OP, rtype,
				  exp->Dsctyp(), exp->Intrinsic(), 1, parms);
  } else {
    Is_True(is_tie_cvt, ("Expected TIE convert"));
    home_wn = WN_CreateExp1(OPR_CVT, rtype, exp->Dsctyp(), intconst_wn);
  }

  id = htable->Sym()->Create_preg(rtype, "homeable_tie", home_wn);
  return TRUE;
}
...
EXP_OCCURS::Get_temp_cr(EXP_WORKLST *wk, CODEMAP *htable)
{
...
#if !defined(TARG_IA32) && !defined(TARG_XTENSA)
  ...
  if (wk->Preg() == 0) {
    ...
    case PK_VNFRE:
	 ...
	 // 
        AUX_ID id;
	if (!create_homeable_tie_preg(htable, exp, id)) {
          id = htable->Sym()->Create_preg(dtyp);
	}
...
{
  Warn_todo("EXP_OCCURS::Is_DT_descendant_of: not implemented", anc);
  return FALSE;
}
...
EXP_WORKLST::Is_the_same_as(const CODEREP *cr)
{
  ...
  if (Exp()->Kind() == CK_IVAR && cr->Kind() == CK_IVAR) {       // IVAR
    ...
    else if (MTYPE_is_tie(cr->Dtyp()) && MTYPE_is_tie(Exp()->Dtyp()) &&
	     (cr->Dtyp() != Exp()->Dtyp()))
      return FALSE;	// Do not CSE TIE CK_IVAR if result types differ

#ifdef TARG_XTENSA
    if (MTYPE_size_min(cr->Dtyp()) != MTYPE_size_min(Exp()->Dtyp())) {
      /* disallow I8I4ILOAD<->I4I4ILOAD/U8U4ILOAD<->U4U4ILOAD CSE */
      return FALSE;
    }
#endif
	...
	cr->Intrinsic() != Exp()->Intrinsic())
      ...
      return FALSE;
    else if (cr->Opr() == OPR_OUTPART && cr->Offset() != Exp()->Offset())
      return FALSE;
...
// Does this load/store have an updating version
static BOOL
Updating_Load_Store(OPCODE opc)
{
  if (xt_hard_float && (opc == OPC_F4F4ILOAD || opc == OPC_F4ISTORE))
    return TRUE;
  
  TYPE_ID desc = OPCODE_desc(opc);

  if (MTYPE_is_tie(desc)) {
    TIE_MACRO_p macro = (OPCODE_operator(opc) == OPR_ILOAD ?
                         tie_info->mtype_loadi_macro(desc) :
                         tie_info->mtype_storei_macro(desc));
  
    if (macro && macro->num_instructions() == 1) {
      TOP top = TI_TOP_Topcode(macro->inst_opcode_name(0));
      if (top != TOP_UNDEFINED &&
          TI_TOP_Nonupdate_To_Update(top) != TOP_UNDEFINED)
        return TRUE;
    }
  }
...
// Should we exclude this expression from the
// list of candidates for strength reduction
static BOOL
Exclude_SR_Candidate(STMTREP *stmt, CODEREP *cr, OPCODE opc)
{
  // 0 => always strength reduce
  if (WOPT_Enable_New_SR_Limit == 0)
    return FALSE;

  // 1 => strength reduce all loads and stores
  // 2 => strength reduce loads and store with updating versions
  if (opc != OPCODE_UNKNOWN &&
      (OPCODE_operator(opc)==OPR_ILOAD || OPCODE_operator(opc)==OPR_ISTORE) &&
      (WOPT_Enable_New_SR_Limit == 1 ||
       (WOPT_Enable_New_SR_Limit == 2 && Updating_Load_Store(opc))))
    return FALSE;

  // Don't limit SR for very small loops since they are 
  // unlikely to suffer from excessive register pressure.
  // Current (arbitrary) limit: 3 BBs, 10 STMTs.
  if (stmt->Bb()->Innermost()) {
    BB_NODE_SET *true_body_set = stmt->Bb()->Innermost()->True_body_set();
    if (true_body_set->Size() <= 3) {
      BB_NODE *bb;
      BB_NODE_SET_ITER bb_iter;
      INT32 num_stmts = 0;  
      FOR_ALL_ELEM(bb, bb_iter, Init(true_body_set))
        num_stmts += bb->Stmtlist()->Len();
      if (num_stmts <= 10)
        return FALSE;
    }
  }

  // Expression is of the form: var1 +/- var2 or var +/- const
  // Always limit adds/subtracts of variables and symbolic constants,
  // but allow SR for literal constants out of LC_simm8 range [-128,127]
  if (cr->Opr() == OPR_ADD || cr->Opr() == OPR_SUB) {
    INT ck_range = CK_VAR | CK_RCONST;
    if ((cr->Opnd(0)->Kind() == CK_CONST &&
         TI_ISA_LC_Value_In_Class(cr->Opnd(0)->Const_val(), LC_simm8)) ||
        (cr->Opnd(1)->Kind() == CK_CONST &&
         TI_ISA_LC_Value_In_Class(cr->Opnd(1)->Const_val(), LC_simm8)))
      ck_range |= CK_CONST;
    if (inCODEKIND(cr->Opnd(0)->Kind(), ck_range) &&
        inCODEKIND(cr->Opnd(1)->Kind(), ck_range))
      return TRUE;
  }
		     ...
		     OPCODE opc, BOOL no_estr)
{
  ...
  switch (cr->Kind()) {
    ...
    case CK_OP:		// non-terminal
      {
        BOOL save_new_sr_limit = WOPT_Enable_New_SR_Limit;
        // Always reset SR limit when processing a multiply
        if (cr->Opr() == OPR_MPY) {
          WOPT_Enable_New_SR_Limit = 0;
        }
	...
	if (OPCODE_is_volatile(cr->Op())) {
	  ...
	  if (cr->Exp_has_e_num()) {
		...
		worklist->Set_exclude_sr_cand();
                ...
                EXP_WORKLST *worklist = Get_worklst(cr, FALSE, TRUE);
                if (worklist && 
                    Exclude_SR_Candidate(stmt, worklist->Exp(), opc)) {
                  worklist->Set_exclude_sr_cand();
                }
              }
	    }
        ...
        WOPT_Enable_New_SR_Limit = save_new_sr_limit;
      }
      break;
	...
	if (all_kids_are_terminal) {
	  ...
	  if (occur->Stmt()->Iv_update() ||
	    ...
	    x->Set_omitted();
		     ...
		     (MTYPE_is_tie(x->Dtyp()) || MTYPE_is_tie(x->Dsctyp())) &&
		     tie_info->mtype_rtor_macro(x->Dsctyp(),x->Dtyp())==NULL) {

	    // do not move a TIE conversion without rtor macro to avoid
	    // unnecessary spiiling to memory
	    x->Set_omitted();
	  } else {
...
EXP_OCCURS::Print(FILE *fp, BOOL show_phi_preds) const
{
  ...
  case OCC_REAL_OCCUR:
    if (Occ_kind() == OCC_PHI_PRED_OCCUR) {
      fprintf(fp, (For_spre())? " (iphi-succ in BB%d)" : " (phi-pred in BB%d)",
	      ((BB_NODE *) Bb())->Id());
    }
    if (Sunk_lvalue())
    ...
    break;
  case OCC_PHI_OCCUR:
    fprintf(fp, (For_spre())? " (iphi in BB%d) " : " (phi in BB%d) ", Bb()->Id());
    if (T_ver_owns_coderep()) 
      fprintf(fp, " (owns coderep)");
