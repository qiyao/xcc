...
//
// $Revision: 1.24 $
// $Date: 2000/04/06 15:23:28 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_expr.cxx,v $
//
// ====================================================================
//
/* 
...
//  
//  CODEREP is constant folding candidate:
//  +/-(CK_LDA|CK_CONST|CK_RCONST, CK_VAR)
static bool
Cr_is_folding_candidate(CODEREP *cr)
{
  if (cr->Kind() == CK_OP && (cr->Opr() == OPR_ADD || cr->Opr() == OPR_SUB)) {
    CODEREP *kid0 = cr->Opnd(0);
    CODEREP *kid1 = cr->Opnd(1);
    return (inCODEKIND(kid0->Kind(), CK_LDA|CK_CONST|CK_RCONST) && 
	    (kid1->Kind() == CK_VAR) 
	    ||
	    inCODEKIND(kid1->Kind(), CK_LDA|CK_CONST|CK_RCONST) &&
	    (kid0->Kind() == CK_VAR));
  }
  return false;
}
...
//  
//  CODEREP evaluates to a compile/link time constant
//  +/-(CK_LDA|CK_CONST|CK_RCONST, CK_LDA|CK_CONST|CK_RCONST)
static bool
Cr_eval_to_constant(CODEREP *cr)
{
  if (cr->Kind() == CK_OP && (cr->Opr() == OPR_ADD || cr->Opr() == OPR_SUB)) {
    CODEREP *kid0 = cr->Opnd(0);
    CODEREP *kid1 = cr->Opnd(1);
...
Rehash_tree_rec(CODEREP *newcr,
                ...
                BB_NODE *bb,
                OPT_STAB *sym,
		bool     is_folding_candidate = false)
{
  switch (newcr->Kind()) {
  case CK_LDA:
  case CK_CONST:
  ...
  case CK_VAR:
    ...
    if (prop &&
      ...
      else if (is_folding_candidate && Cr_eval_to_constant(rhs)) {
	/* allow the folding of CK_LDA + const to propagate to const + var.
	   fix for xcalibur #1991. Peng                                     */
	fold_it = TRUE;
      }
      else if (rhs->Kind() == CK_VAR ||
	       /* allow const + var1 to copy propagate to const + var2.
		  fix for xcalibur #1991. Peng                              */
	       is_folding_candidate && Cr_is_folding_candidate(rhs)) {

	CODEREP *var = rhs;
	if (rhs->Kind() != CK_VAR) {
	  /* get the VAR in the expr */
	  Is_True(rhs->Kind() == CK_OP, ("Rehash_tree_rec, rhs is not OP"));
	  var = (rhs->Opnd(0)->Kind() == CK_VAR) ? 
	    rhs->Opnd(0) : rhs->Opnd(1);
	  Is_True(var->Kind() == CK_VAR, ("Rehash_tree_rec, var is not VAR"));
	}

        ST *s = sym->St(var->Aux_id());
        ...
        if (defbb == bb) {
          fold_it = ! Has_overlapping_liverange
            (/* WOPT_Enable_Ocopy_Lookupstmt+bb->Loopdepth(),
		changed to scan the whole BB length for xcalibur #1991. 
		Peng.*/
	     defbb->Stmtlist()->Len(), var, defstmt);
        }
        else if (defbb == bb->Prev() && bb->Idom() == defbb) {
          fold_it = ! Has_overlapping_liverange
            (WOPT_Enable_Ocopy_Lookupstmt+bb->Loopdepth(), var, bb->First_stmtrep());
          if (fold_it)
            fold_it = ! Has_overlapping_liverange
              (WOPT_Enable_Ocopy_Lookupstmt+bb->Loopdepth(), var, defstmt);
	  if (fold_it) {
	    // Fix 481883:  fix overlapped live range with dead phis
	    PHI_LIST_ITER phi_iter;
	    PHI_NODE     *phi;
	    AUX_ID aux_id = var->Aux_id();
	...
	// do not propagate if sign-extension load can potentially skipped
	// see pr10121
	if (fold_it &&
	    MTYPE_size_min(newcr->Dtyp())!=MTYPE_size_min(newcr->Dsctyp()))
	  fold_it = FALSE;
      }
      if (fold_it) {
