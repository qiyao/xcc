...
// Module: opt_fb.cxx
// $Revision: 1.22 $
// $Date: 2000/08/15 19:02:17 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_fb.cxx,v $
//
// ====================================================================
//
/* 
...
{
  if ( _trace )
    fprintf( TFile, "  OPT_FEEDBACK::Add_edge(%d --> %d, %f)\n", 
	     nx_src, nx_dst, freq.Value() );
...
OPT_FEEDBACK::Remove_edge( IDTYPE ex )
{
  ...
  if ( _trace )
    fprintf( TFile, "  OPT_FEEDBACK::Remove_edge(ex %d, %d --> %d [%f])\n", 
	     ex, edge.source, edge.destination, edge.freq.Value() );
...
OPT_FEEDBACK::Change_edge_freq( IDTYPE ex, FB_FREQ freq_new )
{
  ...
  if ( _trace )
    fprintf( TFile, "  OPT_FEEDBACK::Change_edge_freq(ex %d, %f, %f)\n", 
	     ex, freq_old.Value(), freq_new.Value() );
...
OPT_FEEDBACK::Set_edge_dest( IDTYPE ex, IDTYPE nx_dst_new ) {
  ...
  if ( _trace )
    fprintf( TFile, "  OPT_FEEDBACK::Set_edge_dest(ex %d, nx_dst %d, %f)\n",
	     ex, nx_dst_new, freq_edge.Value() );
    ...
    _fb_opt_edges( mempool_allocator<OPT_FB_EDGE>( pool ) )
{
  ...
  for ( BB_NODE *bb = cfg->First_bb(); bb != NULL; bb = bb->Next() ) {
    ...
    if ( wn_last == NULL ) {
      ...
      // see such a case?!?
      if ( bb->Succ() ) {
        if (bb->Kind() != BB_ENTRY && node.freq_total_in == FB_FREQ_ZERO)
          freq = FB_FREQ_ZERO;
...
OPT_FEEDBACK::Emit_feedback( WN *wn, BB_NODE *bb ) const
{
  ...
  case OPR_WHILE_DO:
    {
      // bb->Loopend()->Id() == nx
      //start by marking the entire thing uninitialized, stale 
      //data may be attached to the wn, this clears it out.
      FB_Info_Loop new_info;
      Cur_PU_Feedback->Annot_loop(wn, new_info);
			  ...
			  map<vertex_id, vertex_id>& nx_old_to_new )
{
  ...
  for ( i = 0; i < edges_clone.size(); ++i ) {
    ...
    FB_FREQ scale = (node.freq_total_in.Zero() ? FB_FREQ_ZERO :
                     freq_transfer_node[nx_src] / node.freq_total_in);
...
OPT_FEEDBACK::Verify( CFG *cfg, const char *const phase )
{
  ...
  if ( ! valid ) {
    ...
    if (PU_has_exc_scopes(Get_Current_PU()))
      ...
      Is_True( FALSE, ( "OPT Feedback invalid" ) );
