...
static char *rcs_id = opt_fold_CXX"$Revision: 1.63 $";
#endif /* _KEEP_RCS_ID */
...
extern DLL_SHARED BOOL xt_mul16; // in "config_targ_options.h"
extern DLL_SHARED BOOL xt_mul32; // in "config_targ_options.h"
extern DLL_SHARED BOOL xt_hifi2; // in "config_targ_options.h"
...
FOLD::CR_Simplify_Tree(CODEREP *cr)
{
  ...
  /* Restore any arith flags... */

  if ((result != NOHASH) &&
      (OPCODE_operator(SIMPNODE_opcode(result)) == arith_flags_opr)) {
    result->Set_arith_flags(arith_flags);
  }
...
CR_CreateIntconst(OPCODE opc, INT64 val)
{
   ...
   cr->Init_const(rtype,val);
   MTYPE cr_rtype = cr->Dtyp();
   if (cr_rtype!=rtype && MTYPE_is_longlong(rtype))
     cr->Set_dtyp_strictly(rtype);
