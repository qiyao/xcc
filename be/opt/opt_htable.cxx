...
// $Revision: 1.547 $
// $Date: 2000/06/26 03:04:12 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_htable.cxx,v $
//
// ====================================================================
//
/* 
...
CODEREP::Copy(const CODEREP &cr)
{
  ...
  if (kind == CK_VAR) {
    ...
    case OPR_INTRINSIC_OP:
      ...
      Set_intr_call_occ(cr.Intr_call_occ());
      break;
    case OPR_TAS:
    ...
    case OPR_ASM_INPUT:
      ...
      break;
    case OPR_MPY:
      Set_arith_flags(cr.Arith_flags());
      break;
    }
  }
...
BOOL
CODEREP::Match(CODEREP* cr, INT32 mu_vsym_depth, OPT_STAB *sym, bool match_all)
{
  // compare this with cr, return TRUE if identical
  Is_True(match_all || cr->Kind() != CK_VAR, 
  ...
  case CK_VAR:
	...
	return FALSE;
      ...
      // CK_VAR nodes never need to be matched
      break;
    }
  ...
  case CK_OP:
    ...
    if (Op() == cr->Op() && Kid_count() == cr->Kid_count()) {
      for (INT i = 0; i < Kid_count(); i++) {
	if (match_all) {
	  if (!Get_opnd(i)->Match(cr->Get_opnd(i), mu_vsym_depth, sym, 
				  match_all)) {
	    return FALSE;
	  }
	} else if (Get_opnd(i) != cr->Get_opnd(i)) {
	  return FALSE;
	}
      }
      switch (Opr()) {
  ...
  case CK_IVAR:
	...
	return FALSE;
      ...
      // for straight or converting TIE iload, the types have to be the same
      if (cr_opr == OPR_ILOAD && cr_opc != ivar_opc &&
	  (MTYPE_is_tie(OPCODE_rtype(ivar_opc)) ||
	   MTYPE_is_tie(OPCODE_rtype(cr_opc)) ||
	   MTYPE_is_tie(OPCODE_desc(ivar_opc)) ||
	   MTYPE_is_tie(OPCODE_desc(cr_opc)))) {
	return FALSE;
      }
	...
	if (cr->Ilod_base() == Istr_base() &&
	  ...
	  if (Ivar_occ()->Lno_dep_vertex() == 0)
	    Ivar_occ()->Set_lno_dep_vertex(cr->Ivar_occ()->Lno_dep_vertex());  // preserve LNO info
	  return TRUE; 
	} 
	else return FALSE;
      } 
...
CODEREP::Print_node(INT32 indent, FILE *fp) const
{
  ...
  case CK_OP:
    ...
    case OPR_COMPOSE_BITS:
      ...
      break;
    case OPR_ARRAY:
      fprintf(fp, " %d %" LLD_FMT "", Num_dim(), Elm_siz());
      break;
    case OPR_INTRINSIC_CALL:
    case OPR_INTRINSIC_OP:
  ...
  case CK_CONST:
    fprintf(fp, ">");	// mark line visually as htable dump
    for (i = 0; i < indent; i++) fprintf(fp, " ");
    fprintf(fp, "LDC %s %" LLD_FMT "", MTYPE_name(Dtyp()), Const_val());
    break;
  case CK_RCONST:
    fprintf(fp, ">");	// mark line visually as htable dump
    for (i = 0; i < indent; i++) fprintf(fp, " ");
    fprintf(fp, "LDRC %s 0x%lx", MTYPE_name(Dtyp()), Const_id());
    break;
  case CK_DELETED:
  default:
...
CODEREP::Print_str(BOOL name_format) const
{
  ...
  case CK_CONST:
    if (name_format)
      sprintf(buf,"LDC %" LLD_FMT "",Const_val());
    break;
  case CK_RCONST:
    if (name_format)
      sprintf(buf,"LDRC 0x%lx",Const_id());
    break;
  default:
...
// Returns TRUE if the coderep is defined by the entry chi list.
//
BOOL
...
// Returns TRUE only if the coderep is defined by the entry chi list
// and its value is unspecified at entry to the PU.
//
// It is possible to have OPR_OPT_CHI statements in BBs other than the
// FUNC_ENTRY BB; such statements (usually? always?) correspond to the
// beginning of C++ exception handling regions. When such a region is
// entered (by catching an exception), the values of local variables
// are NOT undefined, so we must not return TRUE here in such a
// case. Hence we check that the BB containing the OPR_OPT_CHI
// statement is indeed the unique FUNC_ENTRY BB. -- RK 20050608
BOOL
CODEREP::Undef_at_entry(OPT_STAB *opt_stab) const
{
  Is_True(Kind() == CK_VAR, ("CODEREP::Undef_at_entry: expected CK_VAR"));

  STMTREP *defstmt = Get_defstmt();
  if (defstmt != NULL &&
      defstmt->Opr() == OPR_OPT_CHI &&
      defstmt->Bb() == opt_stab->Cfg()->Func_entry_bb() &&
      ST_sclass(opt_stab->Aux_stab_entry(Aux_id())->St()) == SCLASS_AUTO)
    return TRUE;
  return FALSE;
}
...
#ifdef TARG_XTENSA
    if ((MTYPE_is_longlong(cr->Dtyp()) || MTYPE_is_longlong(exist_cr->Dtyp())) 
	&& cr->Dtyp() != exist_cr->Dtyp()) {
      OPCODE   opc; 
      INT      need_cvt = Need_type_conversion(exist_cr->Dtyp(), cr->Dtyp(), &opc);
		   ...
		   COPYPROP *copyprop)
{
  const OPCODE op = WN_opcode(wn);
  WN   *kid = WN_kid(wn, 0);
  BOOL  propagated = Add_expr(kid, opt_stab, stmt, ccr, copyprop, wn);
#ifdef TARG_MIPS
  ...
  // Fix PR#5962.  Keep CVT to U8/I8 for Xtensa.
#ifndef TARG_XTENSA
		 ...
		 UINT field_id, BOOL is_store)
{
  ...
  if (is_store && dtyp!=dsctyp && dsctyp != MTYPE_BS) {
    if (!MTYPE_is_tie(dtyp) && !MTYPE_is_tie(dsctyp) &&
	!MTYPE_is_xtbool(dtyp) && !MTYPE_is_xtbool(dsctyp)) {
  ...
  /* CVTL-RELATED start (correctness) */
  if (need_cvt == NEED_CVT) {
    retv->Reset_flag( CF_MADEUP_TYPE );
    CODEREP *new_cr = Alloc_stack_cr(0);
			     ...
			     MTYPE to_dsctyp, TY_IDX to_ty, UINT field_id)
{
  ...
  CODEREP * retval = this;
  if ( (Is_flag_set(CF_MADEUP_TYPE) && 
	(!MTYPE_is_longlong(to_dtyp) || MTYPE_is_longlong(Dtyp())))
       /* the check for longlong is to prevent reversing the normalization
	of {I,U}8{I,U}4LDID to {I,U}8{I,U}4CVT({I,U}4{I,U}4LDID as done in
	Add_def. (PR#6154) */
       || Dsctyp() == MTYPE_UNKNOWN ) {
  ...
  } else if (!MTYPE_is_tie(to_dtyp) && !MTYPE_is_xtbool(to_dtyp) &&
	     !MTYPE_is_tie(to_dsctyp) && !MTYPE_is_xtbool(to_dsctyp) &&
	     !MTYPE_is_tie(Dtyp()) && !MTYPE_is_xtbool(Dtyp()) &&
	     !MTYPE_is_tie(Dsctyp()) && !MTYPE_is_xtbool(Dsctyp())) {
...
#ifdef TARG_XTENSA
      if ((MTYPE_is_longlong(dtyp) || MTYPE_is_longlong(retv->Dtyp())) &&
	  dtyp != retv->Dtyp()) {
	OPCODE   opc; 
	INT      need_cvt = Need_type_conversion(retv->Dtyp(), dtyp, &opc);
	if (need_cvt == NEED_CVT) {
	  CODEREP *new_cr = Alloc_stack_cr(0);
	  retv->IncUsecnt();
	  new_cr->Init_expr( opc, retv );
		      ...
		      CANON_CR *base_ccr)	// gives the LDA info
{
  ...
  if (dstmt != NULL) {
	...
	CODEREP *tos = Sym()->Top_coderep(vaux);
	if (tos != retv) {
	  // Special hack to avoid overlapped live ranges with
	  // __restrict pointers that are misused by the user. We
	  // check the top of stack for vaux and see whether it is the
	  // same zero version we are considering using. If so,
	  // proceed as usual. If not, we assert that a __restrict
	  // pointer is involved (in order to catch cases I didn't
	  // anticipate) and we use the version from the top of the
	  // rename stack for vaux instead.  -- RK 20050412
	  //
	  // Assert that the POINTS_TO for this ILOAD has the
	  // restricted_pt attribute set, and use the top-of-stack
	  // version if available. If no top-of-stack is available
	  // (which can happen due to DSE eliminating phis that
	  // appeared dead because the alias with the restricted
	  // access wasn't visible), we simply issue an error message
	  // and die.
	  //
	  // It is possible that we should always die, because it may
	  // be the case that DSE could have eliminated some SSA for
	  // the variable, but not all, leaving us with a nonzero top
	  // of stack that is not the current version at the present
	  // spot in the program. For now, and in the interest of
	  // warning and continuing where we can, I assume that
	  // situation is not possible, but I am worried about it. If
	  // overlapped live range assertions fail after the warning
	  // is issued, it simply means that situation has probably
	  // happened, and the warning should be turned into an
	  // unconditional fatal error. -- RK 20050526
	  //
	  // In fact, now we always issue a hard error and die. The
	  // above potential situation, and the fact that the user was
	  // probably going to get wrong code anyway, was too
	  // worrisome. -- RK 20050526
	  OCC_TAB_ENTRY *occ = (OCC_TAB_ENTRY *) WN_MAP_Get(Sym()->WN_sym_map(), wn);
	  POINTS_TO *pt = occ->Points_to();
	  Is_True(pt->Restricted(), ("CODEMAP::Iload_folded: Restricted attribute expected.\n"));
	  // To fall back to handling cases where the above Is_True
	  // assertion would fail in the production compiler as we
	  // would have handled them before (which may still be
	  // problematic, but...), we enclose the replacement code
	  // in "if".
	  if (pt->Restricted()) {
	    ErrMsg(EC_WOPT_bad_restrict_die,
		   (pt->Based_sym() == 0 ?
		    "<unknown>" :
		    ST_name(pt->Based_sym())));
	    // UNREACHED
	    return 0;
	  }
	  // Fall through to old code predating this special hack.
	}

	if (retv->Is_flag_set(CF_IS_ZERO_VERSION)) {
	  // The special hack above doesn't apply; we will use the
	  // zero version.
	...
	// pr12158 for TIE types, need to check that all of the following matches:
	//   the Dtyp() of the scalar variable
	//   the rtype of the ILOAD
	//   the desc of the ILOAD
	if (MTYPE_is_tie(retv->Dtyp())==false ||
	    (retv->Dtyp()==OPCODE_rtype(op) &&
	     OPCODE_rtype(op) == OPCODE_desc(op))) {
	  base_ccr->Tree()->DecUsecnt();	// for the LDA coderep
	  _ssa->Value_number_mu_node(Sym()->Get_mem_mu_node(wn));
	  goto return_pt;
	}
 ...
 return_pt:
  ...
  {
    /* CVTL-RELATED start (correctness) */
    TYPE_ID dtyp = OPCODE_rtype(op);
    retv = retv->Var_type_conversion(this, dtyp,
...
// Should we fold constant additive term into scale?
// For Xtensa, we want to limit folding of negative constants
// into load/store offsets, since they may require an additional
// instruction to generate the address.
// Outside of loops and with -OPT:space it is always better not to fold.
// In loops we may be able to avoid negative offsets becasue of GOC,
// but extensive experiments showed that disabling it here doesn't
// hurt performance and slightly improves code size.
// If we find otherwise, we can uncomment the loop depth test below.
static inline BOOL
Fold_const_term(STMTREP *stmt, OPERATOR opr, INT64 const_val)
{
#ifdef TARG_XTENSA
  extern DLL_SHARED UINT32 WOPT_Always_Fold_Const_Term;
  return (WOPT_Always_Fold_Const_Term == 2 ||
          (stmt->Opr() != OPR_ILOAD && stmt->Opr() != OPR_ISTORE) ||
          MTYPE_is_tie(stmt->Desc()) ||
          MTYPE_is_xtbool(stmt->Desc()) ||
          (WOPT_Always_Fold_Const_Term == 1 && stmt->Bb()->Loopdepth() > 0 && !OPT_Space) ||
          (opr == OPR_ADD && const_val >= 0) || 
          (opr == OPR_SUB && const_val <= 0));
#else
  return TRUE;
#endif
}
...
CODEMAP::Add_expr(WN *wn, OPT_STAB *opt_stab, STMTREP *stmt, CANON_CR *ccr,
		  COPYPROP *copyprop, WN *cvt_parent)
{
  ...
  if (OPCODE_is_leaf(op)) {
	...
	stmt->Set_volatile_stmt();
	  ...
	  if (retv->Kind() == CK_VAR) 
	    ...
	    if (retv->Opr() == OPR_CVTL || retv->Opr() == OPR_OUTPART)
	      cr->Set_offset(retv->Offset());
	    ...
	    retv->DecUsecnt();
          ...
          if (retv->Kind() == CK_OP) {
            ...
            if (WOPT_Enable_CRSIMP &&
                ...
                Fold_const_term(stmt, opr, retv->Get_opnd(1)->Const_val())) {
    ...
    base_ccr.Trim_Scale(wn, WN_kid(wn, 0), cvt_parent, this);
    ...
    if (oper == OPR_INTRINSIC_CALL ) {
      OCC_TAB_ENTRY* occ = (OCC_TAB_ENTRY *)WN_MAP_Get(opt_stab->WN_sym_map(), wn);
      cr->Set_intrinsic(WN_intrinsic(wn));
      cr->Set_intr_call_occ(occ);
    }
    ...
    // Set arith flags if necessary.
    if (oper == OPR_MPY)
      cr->Set_arith_flags(WN_arith_flag(wn));
	...
	break;
      case OPR_OUTPART:
	cr->Set_offset((mINT32)WN_outpart(wn));
	break;
      case OPR_CVTL:
	cr->Set_offset((mINT32)WN_cvtl_bits(wn));
	break;
      case OPR_INTRINSIC_CALL:
	/* shouldn't erlier condition catches OPR_INTRINSIC_CALL already? DKC */
	Is_True(0,("Should not reach here"));
	{ OCC_TAB_ENTRY* occ = (OCC_TAB_ENTRY *)WN_MAP_Get(opt_stab->WN_sym_map(), wn);
	  cr->Set_intr_call_occ(occ);
	  cr->Set_intrinsic(WN_intrinsic(wn));
	}
	break;
      case OPR_INTRINSIC_OP:
	/* shouldn't erlier condition catches OPR_INTRINSIC_OP already? DKC */
	Is_True(0,("Should not reach here"));
	cr->Set_intrinsic(WN_intrinsic(wn));
	break;
      case OPR_TAS:
  ...
  case OPR_PREFETCH:
    {
      ...
      base_ccr.Set_scale(base_ccr.Scale() + WN_offset(Wn()));
      base_ccr.Trim_Scale(Wn(), WN_kid0(Wn()), NULL, htable);
...
#ifdef TARG_XTENSA
    /* because we don't CSE I8I4LDID with I4I4LDID, try
       to make more cases toward the smaller size */
    if (dtyp == MTYPE_I8 && dsctyp == MTYPE_I4) {
      dtyp = MTYPE_I4;
    } else if (dtyp == MTYPE_U8 && dsctyp == MTYPE_U4) {
      dtyp = MTYPE_U4;
    }
#endif
...
#ifdef TARG_XTENSA
	/* because we don't CSE I8I4ILOAD with I4I4ILOAD, try
	   to make more cases toward the smaller size */
	if (dtyp == MTYPE_I8 && dsctyp == MTYPE_I4) {
	  dtyp = MTYPE_I4;
	} else if (dtyp == MTYPE_U8 && dsctyp == MTYPE_U4) {
      ...
      base_ccr.Set_scale(base_ccr.Scale() + WN_offset(Wn()));
      base_ccr.Trim_Scale(Wn(), WN_kid(Wn(), 1), NULL, htable);
      if (base_ccr.Tree() != NULL && 
	  base_ccr.Tree()->Kind() == CK_LDA )
      {
	// is the result of the load volatile?
...
//  Set_asm_stmt_flags(WN_asm_flag(Wn()));
  ...
  default:
    char buf[128];
    sprintf(buf, "STMTREP::Enter_lhs: %s is not implemented yet",
...
{ 
  if (Kind() == CK_VAR) {
    ...
    OCC_TAB_ENTRY *occ = Intr_call_occ();
    if (occ)
      return occ->Points_to();
    return NULL;
  }
  else
    return NULL;
...
STMTREP::Print(FILE *fp) const
{
  ...
  if (_opr==OPR_FORWARD_BARRIER || _opr==OPR_BACKWARD_BARRIER) 
    fprintf(fp, " barrier-flag:0x%x", WN_barrier_flag(Barrier()));
  fprintf(fp, " b=%s",Print_bit());
...
{
  fprintf(fp, " SR %lx b=%s ", stmt, stmt->Print_bit());
  stmt->Print_node(fp);
  fprintf(fp, " kids<");
...
BOOL
STMTREP::Is_required_by_asm(void) const
{
  const STMTREP *prev = Prev();
  while (prev) {
    const OPERATOR opr = prev->Opr();
    if (opr == OPR_ASM_STMT) {
      return TRUE;
    }
    if (opr != OPR_STID) {
