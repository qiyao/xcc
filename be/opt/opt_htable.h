...
/* valgrind overrides new, so CXX_NEW_VARIANT doesn't call
   our mempool allocator, so CXX_NEW_VARIANT ends up allocating
   none of the extra pointers that makes the variable sized 
   CODEREPS work.

   This in turn results in spurious errors when using valgrind.

   By changing the kid count we allocate much larger CODEREPS
   which is wasteful memory wise but will prevent the spurious 
   valgrind messages unless you have a coderep with more than 10 
   kids, which is pretty unusual.

   The price you pay is the extra memory, plus the off chance that
   there really are memory errors in the extra sized allocations
   that go on with CODEREPS.
*/
...
private:
  ...
  MTYPE     _dtyp:7;                 // data type
  MTYPE     dsctyp:7;                // descriptor type for various opcode
  INT32     usecnt:11;               // number of times this node's
                                     // expression appears.
...
#define ILLEGAL_BB 0xfffff	     // initialization value for emit_bb
  ...
  union {
    ...
    struct {                         // for code kind ISOP
      OPERATOR  _opr:8;	             // 
      mUINT32   _arith_flags:10;     // wn arith flags
      ...
      OCC_TAB_ENTRY	*occ;        // for INTRINSIC_CALL
      CODEREP  *kids[KIDCOUNT];             // array of kid pointers
    } isop;
    struct {                         // for code kind IVAR(ILOD)
      ...
      //CODEREP *base[KIDCOUNT];            // dynamically created, give STORE's
				     // base address
...
#define Alloc_stack_cr(cnt) ((CODEREP*)alloca(sizeof(CODEREP)+(cnt)*sizeof(CODEREP *)))
  ...
  UINT32 Extra_ptrs_used(void) const
    {
      switch (Kind()) {
      case CK_OP:
	if (Kid_count() > KIDCOUNT) {
	  return Kid_count() - KIDCOUNT;
	}
	else {
	  return 0;
	}
  ...
  BOOL      Match(CODEREP *cr, 
		  ...
		  OPT_STAB *opt_stab = NULL,
		  bool match_all = false);    // compare two nodes
  ...
  void	    Set_dtyp_const_val(MTYPE dt, INT64 v) { Is_True(Kind() == CK_CONST,
					...
					if ((v == (v << 32) >> 32) &&
					    dt != MTYPE_I8 && dt != MTYPE_U8)
					  _dtyp = MTYPE_I4;
					else _dtyp = MTYPE_I8;
					u2.isconst.const_val = v; }
  void      Set_dtyp_strictly(MTYPE dt) { _dtyp = dt; }
  MTYPE     Dsctyp(void) const        { return 0x7f&dsctyp; }
  ...
  BOOL      Def_at_entry(void) const; // the CR's definition is at entry
  BOOL      Undef_at_entry(OPT_STAB *) const; // the CR's definition is at entry
					      // and its value is undetermined
  ...
  INT64     Const_val(void) const     { Is_True(Kind() == CK_CONST,
					...
					Is_True(!MTYPE_float(0x7f&_dtyp),
  ...
  void      Set_opr(OPERATOR c)       { Is_True(Non_leaf(),
					...
					(Kind() == CK_OP) ? u2.isop._opr = c :
						u2.isivar._opr = c; }
  mUINT32   Arith_flags(void) const   { Is_True(Kind() == CK_OP,
				        ("CODEREP::Arith_flags, illegal kind %s",
					  Print_kind()));
					return u2.isop._arith_flags; }
  void      Set_arith_flags(mUINT32 f) { Is_True(Kind() == CK_OP,
					 ("CODEREP::Set_arith_flags, illegal kind"));
					u2.isop._arith_flags = f; }
  ...
  OCC_TAB_ENTRY* Intr_call_occ(void) const   { Is_True((Kind()==CK_OP &&
		(u2.isop.occ==NULL || Opr()==OPR_INTRINSIC_CALL)),
						  ("CODEREP::Intr_call_occ, illegal kind"));
					  return u2.isop.occ; }
  void      Set_intr_call_occ(OCC_TAB_ENTRY* occ){ Is_True((Kind()==CK_OP),
						  ("CODEREP::Intr_call_occ, illegal kind"));
					      u2.isop.occ = occ; }
  ...
  BOOL      Ivar_has_e_num(void) const { 
    Is_True(Kind() == CK_IVAR,
...
//    MTYPE      _rhs_type;     // rhs type for assignment statements
  ...
  void Init(void)		{ _lhs = _rhs = NULL;
				  ...
				  _u3._wn = NULL; 
				  // clear largest field in u4
				  _u4._label_flags = 0;
				  // clear largest field in u5
...
public:
  ...
  // Is the previous non-STID statement an OPR_ASM_STMT?
  BOOL      Is_required_by_asm(void) const;
  ...
  MU_LIST  *Mu_list(void) const	        { return _mu_list; }
  void      Set_mu_list( MU_LIST *mu )  { _mu_list = mu; }
