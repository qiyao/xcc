...
static char *rcs_id = 	opt_htable_emit_CXX"$Revision: 1.38 $";
#endif /* _KEEP_RCS_ID */
...
ML_WHIRL_EMITTER::Build_loop_info( BB_NODE *label_bb )
{
  ...
  UINT16 at_region_id = WN_loop_at_region_id(label_bb->Label_loop_info());
  ...
  // Set the do-while flag
  if (((bb_loop->Flags() & LOOP_PRE_REPEAT) == LOOP_PRE_REPEAT) ||
      ((bb_loop->Flags() & LOOP_REPEAT) == LOOP_REPEAT))
    lflags |= WN_LOOP_DOWHILE;
  else
    lflags &= ~WN_LOOP_DOWHILE;
...
ML_WHIRL_EMITTER::Emit(void)
{
  ...
  FOR_ALL_ELEM(bb, cfg_iter, Init(Cfg())) {
	...
	Is_Trace(Trace(),(TFile,"Push_region(RGN %d), prev_wn = 0x%lx\n",
	...
	Is_Trace(Trace(),(TFile,"Pop_region(RGN %d), prev_wn = 0x%lx\n",
