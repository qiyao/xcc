...
static char *rcs_id = 	opt_ivr_CXX"$Revision: 1.203 $";
#endif /* _KEEP_RCS_ID */
...
{
  Is_True(!Preg_Is_Dedicated(preg_num),
          ("Set_Preg_Name: Cannot set name of dedicated PREG"));
  Set_PREG_name(Preg_Table[Get_Preg_Idx(preg_num)], name);
}
...
// Does the expr evalutes to a numeric constant?
//
BOOL
...
//    If IV_EXPR_VALID is still set and the reurn value is non-NULL,
//    an IV found.
//
CODEREP *
IV_EXPR::Find_IV(CODEREP *iv_expr)
{
  ...
  case CK_OP:
	...
	Reset_iv_expr_valid();
        return NULL;   // if both has IV or neither has IV.
      }
...
IV_EXPR::Get_const(CODEREP *cr)
{
  ...
  Is_True(FALSE, ("Get_const:  inproper formatted IV expr."));
  return 0;
}
...
IV_EXPR::Find_step(CODEREP *iv_expr)
{
  ...
  case CK_IVAR:
    Is_True(Loop()->Invariant_cr(iv_expr), ("Find_step:  Improper formatted IV EXPR."));
    return 0;
...
CODEREP::Propagatable_for_ivr(OPT_STAB *sym) const
{
  ...
  case CK_OP:
    ...
    if (! Op_can_be_propagated(Op(), sym->Phase(), 
			       (Kid_count()) ? (Opnd(Kid_count()-1)->Kind()== CK_CONST) : false) ) 
      return FALSE;
    // Reference 644395 for situations when INTRINSIC_OP cannot be copy propagated.
    if (Opr() == OPR_INTRINSIC_OP)
      return FALSE;
    return TRUE;
  }
...
IVR::Satisfy_primary_IV(const IV_CAND *iv_cand, BOOL must_be_I4) const
{
  ...
  // Or the IV is initialized with constant 0.
  AUX_STAB_ENTRY *psym =
    Opt_stab()->Aux_stab_entry(iv_cand->Var()->Aux_id());
  if (psym->Byte_size() < 4 &&
      !(iv_cand->Init_value()->Kind() == CK_CONST &&
	iv_cand->Init_value()->Const_val() == 0)) {
...
IVR::Choose_primary_IV(const BB_LOOP *loop)
{
  ...
  if (Phase() != MAINOPT_PHASE) {
    ...
    goto mainopt_primary_selection;

  generate_new_primary:
    {
                                ...
                                CODEREP *replacement)
{
  ...
  case CK_OP:
	...
	Is_True(FALSE,
		("Replace_IV_with_invar:  inproper formatted IV expr."));
      }
  ...
  Is_True(FALSE, ("Replace_IV_with_invar:  inproper formatted IV expr."));
  return NULL;
}
			...
			IV_CAND *primary_cand)
{
...
#ifdef TARG_XTENSA
  // do not compute trip count if the step is not power of 2
  if (step->Kind() == CK_CONST &&
       !(bound->Kind() == CK_CONST && init->Kind() == CK_CONST)) {
    INT64 val = step->Const_val();
    if (val<(INT64)0) val = -val;
    if (((val != 0) && ((val & (val-1)) == 0)) == false) {
      return NO_TRIP_COUNT;
    }
  }
  ...
  if (!valid_entry_test) {
	...
	if (bb->Next() == Loop()->Preheader() &&   // entry test falls thru
	  ...
	  if (rhs == entry_test ||
	      rhs->Match(entry_test, 0, NULL, true) /* PR#6266 */) 
	    // same condition
	    valid_entry_test = TRUE;
	}
      }
  ...
  // yet still can't find the entry test (PR#6266, bound is store to
  // a preg in the preheader.
  if (!valid_entry_test && 
      bound->Kind() == CK_VAR &&
      !bound->Is_flag_set((CR_FLAG)(CF_DEF_BY_PHI|CF_DEF_BY_CHI)) &&
      bound->Defstmt() != NULL) {
    // generated init cmp_opc bound
    CODEREP *entry_test =
      Htable()->Add_bin_node_and_fold(cmp_opc, init,
                                      bound->Defstmt()->Rhs(), cmp_stmt->Bb());
    if (entry_test->Kind() == CK_CONST) {
      if (entry_test->Const_val() != 0)  // TRUE
	valid_entry_test = TRUE;
    } else {
	...
	if (bb->Next() == Loop()->Preheader() &&   // entry test falls thru
	  ...
	  if (rhs==entry_test || rhs->Match(entry_test, 0, NULL, true)) 
	    // same condition, use Match instead of == because loop
	    // invariant IVAR may be fixed by calling 
	    // Convert_iload_to_loop_invariant, where the the MU
	    // changed (PR #6266).
	    valid_entry_test = TRUE;
	}
      }
  ...
  //
  // This is not a good fix.  If the loop has a valid entry test but
  // the program mismatched it, 'need_guard' is set to true and
  // trip count will have an extra MAX instruction hurting performance.
  if (!valid_entry_test) {
...
#ifdef TARG_XTENSA
  // the original SGI version doesn't work for U4 induction variable
  // for (U4 i = 0; i<U4MAX; i++) will only execute half of the U4MAX
  if (MTYPE_is_unsigned(OPCODE_desc(cmp_opc))) {
    if (valid_entry_test) {
      tripcount_type = MTYPE_U4;
    } else if ((init->Kind() == CK_CONST && init->Const_val() == 0) &&    
	       (direction == UP || 
		step->Kind() == CK_CONST && step->Const_val()>0)) {
      tripcount_type = MTYPE_U4;
    } else { 
      // We may want to just give up. Or we can add an entry test and
      // set the tripcount_type to MTYPE_U4. May be later.
      tripcount_type = MTYPE_I4;
    }
  } else {
    tripcount_type = MTYPE_I4;
  }
  ...
  if (allow_remainder) {
	...
	CODEREP *neg_step = Htable()->Add_bin_node_and_fold(subop, zero, step);
	...
	if (MTYPE_is_integral(iv_type) && MTYPE_is_unsigned(iv_type) &&
	    MTYPE_bit_size(iv_type) < MTYPE_bit_size(dtype)) {
	  OPCODE cvtlop = OPCODE_make_op(OPR_CVTL, dtype, MTYPE_V);
	  diff = Htable()->Add_unary_node(cvtlop, diff);
	  diff->Set_offset(MTYPE_bit_size(iv_type));
	  FOLD ftmp;
	  crtmp = ftmp.Fold_Expr(diff);
	  if (crtmp) diff = crtmp; 
	}
  ...
  if (Phase() == MAINOPT_PHASE && !test_at_entry && need_slt_opt &&
    ...
    //  divisable
      ...
      Warn_todo("SLT: adjust the upper bound to make (bound - init) divisable by the step.");
    }
...
#ifdef TARG_XTENSA
      ((test_at_entry && cmp_stmt->Opr() == OPR_FALSEBR) ||
       (!test_at_entry && cmp_stmt->Opr() == OPR_TRUEBR))
#else
      test_at_entry && cmp_stmt->Opr() == OPR_FALSEBR
#endif
      && need_inv_slt_opt 
  ...
  // ************************************************************************
  if (need_guard) {
    OPCODE min_op = OPCODE_make_op(OPR_MAX, tripcount_type, MTYPE_V);
				      ...
				      IV_CAND *primary) 
{
  ...
  Is_True(stmt->Opr() == OPR_TRUEBR ||
	  ...
	  ("Don't know how to handle loop end block without OPR_TRUEBR/FALSEBR"));
  ...
  *trip_iv_found = trip_cand;
  ...
  if (loopinfo->Test_at_entry()) {
    ...
    if (trip_iv_expr != based_iv) 
      trip_init_adjusted = Replace_IV_with_invar(trip_iv_expr, based_iv, trip_init);

    CODEREP *cmp = swapped ?
      Htable()->Add_bin_node_and_fold(cond->Op(), trip_bound, trip_init_adjusted) :
      Htable()->Add_bin_node_and_fold(cond->Op(), trip_init_adjusted, trip_bound);
	...
	OPCODE select_op = OPCODE_make_op(OPR_SELECT, trip_count->Dtyp(), MTYPE_V);
	CODEREP *zero = Htable()->Add_const(MTYPE_I4, 0);
	CODEREP *select = Htable()->Add_tertiary_node(select_op, cmp, trip_count, zero);
	...
	case OPR_GE:
	  if (cmp->Opnd(0) == select->Opnd(1) && 
	    ...
	    select = Htable()->Add_bin_node_and_fold(
	      OPCODE_make_op(OPR_MAX, trip_count->Dtyp(), MTYPE_V), select->Opnd(1), select->Opnd(2));
	  }
	  break;
	case OPR_LT:
	case OPR_LE:
                               ...
                               BB_LOOP *loop)
{
  ...
  CODEREP *iter_count =
    Htable()->Add_bin_node_and_fold(subop, primary->Var(), primary->Init_value());
  ...
  if (secondary->Step_value()->Kind() == CK_CONST && 
	...
	OPCODE addop = OPCODE_make_op(OPR_ADD, dtype, MTYPE_V);
	second_iv =
          Htable()->Add_bin_node_and_fold(addop, iter_inc, secondary->Init_value());
      }
...
// Two CR are equivlent (has same value) if cr reaches def_cr
...
IVR::Convert_all_ivs(BB_LOOP *loop)
{
  ...
  for (iv_cand_iter = iv_cand_container.begin(); 
       ...
       iv_cand_iter++) {
    IV_CAND *cur_iv = *iv_cand_iter;
    CODEREP *new_init = Htable()->Convert_to_loop_invar(cur_iv->Init_value(), loop);
  ...
  if (_trace) {
    if (trip_count != NULL)  {
      if (trip_count->Kind() == CK_CONST)
	fprintf(TFile, "IVR: exit trip count is %" LLD_FMT "\n",
  ...
  if (primary && trip_iv && ivr_generated_primary) {
    ...
    Set_Preg_Name((PREG_NUM) Opt_stab()->St_ofst(primary_id), ST_name(trip_st));
    if (_trace) 
      fprintf(TFile, "IVR: change preg %" LLD_FMT " index name to %s.\n", 
	      Opt_stab()->St_ofst(primary_id),
	      ST_name(trip_st));
  }

  //  Fix 454209:  if the loop executes exactly one and it is MAINOPT_PHASE
  //  and the test is at the end, then remove the loop-back edge to avoid
  //  moving invariant out of the loop or perform strength reduction unnecessarily.
  if (Phase() == MAINOPT_PHASE &&
      trip_count != NULL &&
      trip_count->Kind() == CK_CONST &&
    ...
    if (_trace) 
      fprintf(TFile, "IVR: loop executes one -- removing loop back edge at BB%d\n", 
	      test->Id());
  ...
  if (trip_iv && trip_iv != primary && trip_count && !loop->Exit_early()) {
    // update exit stmt for trip_iv
    // Fix #6291.
    Update_exit_stmt(trip_iv, loop->Merge(), loop);
  }
  ...
  for (iv_cand_iter = iv_cand_container.begin(); 
    ...
    if (MTYPE_size_min(secondary->Dtype()) <
      ...
      if (_trace) 
	fprintf(TFile, "IVR: skip secondary IV sym%d because secondary size < primary size.\n",
    ...
    //  Fix 469578:
    //
    //   Disable secondary IV replacement if sizeof(secondary IV) > sizeof(primary IV)
    //   and sizeof(trip IV) > sizeof(primary IV).
    //
    //   The objective is not to replace is not to replace every 'long' or 'ptr' 
    //   with 'int' by IVR unless the range of 'int' is sufficient (i.e.,
    //   the trip count is less than 2^32).
    //
    //   The required condition is sizeof(trip IV) <= sizeof(primary IV) because
    //   if the loop exit condition is based on a I4/U4 IV, the loop should not
    //   execute more than 2^32 times.
    //
    if ((MTYPE_size_min(secondary->Dtype()) > MTYPE_size_min(primary->Dtype())) &&
	(trip_iv == NULL ||
	 (MTYPE_size_min(trip_iv->Dtype())   > MTYPE_size_min(primary->Dtype())))) {
      if (_trace) 
	fprintf(TFile, "IVR: skip secondary IV sym%d because secondary IV size > primary IV size.\n"
    ...
    // This fix involves adding a new flag AUXF_DONT_REPLACE_IV to the opt_stab.
    if (Opt_stab()->Aux_stab_entry(aux_id)->Dont_replace_iv()) {
      if (_trace)
	fprintf(TFile, "IVR: skip secondary IV sym%d because of Dont_replace_iv.\n", 
		secondary->Var()->Aux_id());
      continue;
    }

    TY_IDX ty_idx = Opt_stab()->Aux_stab_entry(aux_id)->Ty();
    if (ty_idx && 
        TY_kind(ty_idx) == KIND_POINTER &&
        MTYPE_is_tie(TY_mtype(TY_pointed(ty_idx)))) {
      if (_trace)
        fprintf(TFile, 
                "IVR: skip secondary IV sym%d used in TIE type addressing\n", 
                secondary->Var()->Aux_id());
      continue;
    }
    ...
    if (secondary->Init_value() != NULL) {
      ...
      if (_trace) 
	fprintf(TFile, "IVR skip secondary IV sym%d because it cannot find init value.\n",
		secondary->Var()->Aux_id());
...
// Process the loop inside out.  Extra IV can be recognized for the
// outer loop if the trip count of the inner loop can be determined first.
BOOL
...
// Apply IVR for the whole COMP UNIT
void
