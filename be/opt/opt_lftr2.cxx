...
static char *rcs_id =   opt_lftr2_CXX"$Revision: 1.65 $";
#endif /* _KEEP_RCS_ID */
...
LFTR::Remove_comp_occur(EXP_OCCURS *occur)
{
  ...
  Is_Trace(Trace(), (TFile, "  --- checking 0x%lx kid %d\n",
  ...
  FOR_ALL_NODE(comp, comp_iter, Init(lftr_var_head->Occ_list())) {
    ...
    // ignore them
    Is_Trace(Trace(), (TFile, "  --- against  0x%lx kid %d\n",
...
LFTR::Print(FILE *fp)
{
  ...
  fprintf(fp,"_trace=%c, _lftr_on=%c, _mem_pool=0x%lx, _etable=0x%lx\n",
