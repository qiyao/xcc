...
static char *rcs_id = 	opt_loop_CXX"$Revision: 1.61 $";
#endif /* _KEEP_RCS_ID */
...
// ====================================================================
// Determine if expr is constant relative to the index variable in the
// loop.  i.e., all parts of the expression are loop-invariant except
// for the index
// NOTE: Level should always be 0 when called from another function
// ====================================================================
BOOL
BB_LOOP::Index_relative_expr( CODEREP *expr, const CODEREP *index,
  INT32 level ) const
{
  Is_True( index->Kind() == CK_VAR,
    ("BB_LOOP::Index_relative_expr: index is not var") );

  if ( level == 0 && ! expr->Contains(index) ) {
    return FALSE;
  }
  ...
  switch ( expr->Kind() ) {
    case CK_OP:
      ...
      for ( INT32 ekid = 0; ekid < expr->Kid_count(); ekid++ ) {
        if ( ! Index_relative_expr( expr->Opnd(ekid), index, level+1 ) )
          return FALSE;
      }
      }
      return TRUE;
...
// If expr is an invariant, Find_variant returns NULL.
// Otherwise, it returns the a CK_VAR/CK_IVAR that is not an invariant.
//
...
//  Returns TRUE if the expr is a linear function of variant, i.e.,
//    expr == c1 * variant + c2.
//
...
inline BOOL Is_linear_function_of(CODEREP *expr, CODEREP *variant, BB_LOOP *loop)
{
  LINEAR_FUNCTION l = Linear_function(loop, expr);
  return (l.type == INVAR_EXPR || l.type == LINEAR_EXPR) && l.variant == variant;
}
...
// ====================================================================
// Invariant_cr (public)
// Non-recursive wrapper for Invariant_cr_rec (private)
// resets ISOP_INVARIANT_VISITED bit if Invariant_cr_rec returns TRUE
// ====================================================================
BOOL
...
// ====================================================================
// Invariant_cr_rec (private)
// Determine if the value is loop invariant for the given loop.
// ====================================================================
BOOL
BB_LOOP::Invariant_cr_rec( CODEREP *cr ) const
{
  ...
  // which kinds are always constant
  if ( inCODEKIND(cr->Kind(), CK_LDA|CK_CONST|CK_RCONST) ) {
    ...
    if ( cr->Is_flag_set(CF_DEF_BY_PHI) ) {
      ...
      // see if the statement to which this chi is attached is in a
      // block that's outside this loop
	...
	case CK_OP:
	 {
	  if (cr->Is_isop_flag_set(ISOP_INVARIANT_VISITED) && WOPT_Enable_Prune)
	    return TRUE;
...
// Returns TRUE if there is CALL that can access the index 
// in the WHIRL tree
//
BOOL
...
// Returns TRUE if there is CALL in the tree
//
BOOL
...
static BOOL
Found_aliasing_store_in_loop(POINTS_TO *pt, TY_IDX ty, BB_LOOP *loop, BB_NODE *bb, OPT_STAB *opt_stab)
{
...
CODEMAP::Convert_iload_to_loop_invariant(BB_LOOP *loop, CODEREP *cr)
{
  ...
  case CK_IVAR:
	...
	if (defbb == NULL || 
	    !loop->True_body_set()->MemberP( defbb))
	  return;
	...
	if (Get_Trace(TP_GLOBOPT, IVR_DUMP_FLAG)) {
	  fprintf(TFile, "IVR:  convert iload to invar in loop with body BB %d.\n", startbb->Id());
	}
...
//  Compute the dependence of stmt w.r.t to the set of statement starting after 'stmt'
//  and terminates at <end_bb, after_this_stmt>.
//
...
Compute_dependence(STMTREP *stmt, BB_NODE *end_bb, STMTREP *after_this_stmt)
{
  Is_True(after_this_stmt == NULL || after_this_stmt->Bb() == end_bb,
	  ("Can_sink: inconsistency input"));
  Is_True(stmt->Bb()->Dominates(end_bb), 
	  ("Can_sink: inconsistency input"));  // this impl didn't know liveness
...
inline BOOL RAISE(BOOL r, char *msg)
{
  if (Get_Trace(TP_GLOBOPT, EMIT_DUMP_FLAG))
    if (!r)
      fprintf(TFile, "Can_raise_to_doloop: doloop not raised because %s\n", msg);
  return r;
}
#else
inline BOOL RAISE(BOOL r, char *msg/*ARUGSUSED*/) { return r; }
#endif
...
Can_raise_to_doloop(BB_LOOP *loop, BOOL repair, CODEMAP *htable)
{
  ...
  // 
  // Message from Dror:
  // I still think the proposed solution is fine.  If the outer loop was originally
  // (in the source) a while loop, and it has an early exit, do not raise it to a
  // DO loop.  MP loops can not have early exits, so this is not an issue.
  //  
...
#ifdef Is_True_On
  {
	...
	last_stmt_in_loopback == loopback->Branch_stmtrep())
      last_stmt_in_loopback = last_stmt_in_loopback->Prev();
    Is_True(incr_stmt == last_stmt_in_loopback, ("incr stmt not last stmt in loopback"));
  }
#endif
