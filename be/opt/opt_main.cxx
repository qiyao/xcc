...
// Module: opt_main.cxx
// $Revision: 1.454 $
// $Date: 2000/08/25 19:39:54 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_main.cxx,v $
//
// ====================================================================
//
/* 
...
extern DLL_SHARED BOOL Enable_WN_Simp;
	      ...
	      ALIAS_MANAGER *alias_mgr)
{
  ...
  if (Cur_PU_Feedback)
    Cur_PU_Feedback->Verify("Pre_Optimizer");
  ...
  if (phase == MAINOPT_PHASE) {
    ...
    if (lower_flags != LOWER_NULL) {
      ...
      if (Cur_PU_Feedback) {
	Cur_PU_Feedback->Reset_Root_WN(wn_tree);
	//can't verify feedback now, b/c tree needs cleanup 
      }
...
#ifdef TARG_XTENSA
  /* Before generating the CODEREP, perform global offset
     canonicalization. */
  if (phase == MAINOPT_PHASE)
    {
      SET_OPT_PHASE("Init Offset Canon");
      Init_Offset_Canonicalization(wn_tree, &Opt_global_pool, TRUE,
				   Get_Trace(TP_GLOBOPT, SSA_DUMP_FLAG));
    }

  /* Mark OPR_MPY nodes that can be implemented with a 16-bit
     multiply. This first pass runs before the optimizer since PRE can
     obscure the information used to determine which multiplies are
     16-bit. */
  if (phase == MAINOPT_PHASE || 
      phase == PREOPT_LNO_PHASE || 
      phase == PREOPT_PHASE)
    {
      /* Also, collect typecast information for more conservative
         ANSI type based alias analysis. */
      alias_mgr->Collect_typecast_alias_classes(wn_tree);

      Find_Mul16(wn_tree,NULL, alias_mgr);
    }
#endif
  ...
  if ( phase == MAINOPT_PHASE ) {
	...
	comp_unit->Htable()->Verify_var_phi_hash();
	comp_unit->Do_load_pre(FALSE, /* WOPT_Enable_Const_PRE do_consts */
...
#ifdef TARG_XTENSA
      /* Mark OPR_MPY nodes that can be implemented with a 16-bit
	 multiply. This second pass is to catch anything created by
	 the optimizer. */
      Find_Mul16(opt_wn, NULL, alias_mgr);

      /* Find and apply bit-insert patterns. */
      Find_Bit_Insert(opt_wn);
#endif
