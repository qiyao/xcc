...
static char *rcs_id = 	opt_mu_chi_CXX"$Revision: 1.49 $";
#endif /* _KEEP_RCS_ID */
...
OPT_STAB::Enter_occ_tab(WN *wn, AUX_ID aux_id)
{
  ...
  if (occ->Is_stmt()) {
    if (WN_operator(wn)==OPR_INTRINSIC_CALL) {
      occ->Points_to()->Init();
      occ->Points_to()->Set_expr_kind(EXPR_IS_INVALID);
      occ->Points_to()->Set_base_kind(BASE_IS_INVALID);
