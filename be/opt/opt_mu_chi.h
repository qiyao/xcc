...
// Module: opt_mu_chi.h
// $Revision: 1.31 $
// $Date: 2000/05/19 20:31:19 $
// $Author: murthy $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_mu_chi.h,v $
//
// ====================================================================
//
/* 
...
#ifdef _KEEP_RCS_ID
static char *opt_mu_chircs_id = opt_mu_chi_INCLUDED"$ $Revision: 1.31 $";
#endif /* _KEEP_RCS_ID */
...
class OCC_TAB_ENTRY {
  ...
  INT32         _lno_dep_vertex;  // vertex index for LNO dep graph
...
public:
  ...
  OCC_TAB_ENTRY(const OCC_TAB_ENTRY &occ)
  {
    ...
    _wn     = occ._wn;
    _u1     = occ._u1;
    _lno_dep_vertex     = occ._lno_dep_vertex;
  ...
  INT32         Lno_dep_vertex(void) const  { return _lno_dep_vertex; }
  PF_POINTER    *Pf_pointer(void) const          { return _u1._is_mem._pf_pointer; }
  
  POINTS_TO     *Points_to(void) {
    Is_True(WN_operator(_wn) == OPR_PARM || OPCODE_is_load(WN_opcode(_wn)) || OPCODE_is_store(WN_opcode(_wn)) || WN_operator(_wn)==OPR_INTRINSIC_CALL, 
  ...
  void  Set_lno_dep_vertex(INT32 idx)  { _lno_dep_vertex= idx; }
