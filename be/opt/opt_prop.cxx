...
// Module: opt_prop.cxx
// $Revision: 1.270 $
// $Date: 2000/10/17 21:23:11 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_prop.cxx,v $
//
// Revision history:
...
// ===================================================================
// Imported function from be/com/emulate.cxx             
// ===================================================================
		       ...
		       BOOL inside_cse, INT32 *height, BOOL in_array)
{
  ...
  case CK_OP: {
    ...
    if (Past_ret_reg_def() && 
	(x->Opr() == OPR_INTRINSIC_OP || 
	 OP_is_emulated(x->Op(), x->Opr(), 
			(x->Kid_count()>1) && x->Opnd(1)->Kind()==CK_CONST))) {
      return NOT_PROPAGATABLE;
    }
    ...
    *height = 0;
    ...
    if (! Op_can_be_propagated(x->Op(), Htable()->Phase(), (x->Kid_count()>0) ?(x->Opnd(x->Kid_count()-1)->Kind()== CK_CONST) : false) ) {
      x->Set_propagatability(NOT_PROPAGATABLE);
      return NOT_PROPAGATABLE;
    }
...
CODEREP::Convert_type(CODEMAP *htable, CODEREP *expr, BOOL icopy_phase)
{
  ...
  } else if (MTYPE_is_tie(rhs_type) || MTYPE_is_tie(dsc_type) ||
	     MTYPE_is_xtbool(rhs_type) || MTYPE_is_xtbool(dsc_type)) {

    // if the expr is the same converting load, then no futher conversion is needed
    // see PR9155
    if (expr->Dtyp() == Dtyp() && expr->Dsctyp() == Dsctyp())
      return expr;
...
COPYPROP::Prop_const_init_scalar(CODEREP *x, AUX_ID var_aux_id)
{
  ...
  if (const_initialized) {
	...
	// TENSILICA: Do not do Targ_Conv() if it is TIE type
	if (MTYPE_is_tie(x->Dtyp()) || MTYPE_is_xtbool(x->Dtyp()))
	  return NULL;
		   ...
		   BOOL inside_cse, BOOL in_array)
{
  ...
  /* Some symbols are explicitly excluded from forward substitution
     until MAINOPT. */
  ST *st_x = Opt_stab()->Aux_stab_entry(x->Aux_id())->St();
  if (Htable()->Phase() != MAINOPT_PHASE && st_x && ST_dont_prop(st_x))
    return NULL;
...
COPYPROP::Copy_propagate_cr(CODEREP *x, BB_NODE *curbb, 
			    BOOL inside_cse, BOOL in_array, BOOL in_iload)
{
  CODEREP *expr;
  CODEREP *cr = Alloc_stack_cr(x->Extra_ptrs_used());
  BOOL need_rehash;
  ...
  case CK_VAR: 
    {
      ...
      if (id_cr &&
	  x->Dsctyp()!=x->Dtyp() &&
	  (MTYPE_is_tie(x->Dsctyp()) ||
	   MTYPE_is_tie(x->Dtyp())) &&
	  (x->Dsctyp()!=id_cr->Dsctyp() ||
	   x->Dtyp()!=id_cr->Dtyp())) {
	// Do not propagate into TIE converting ldid (PR10469)
      } else if (id_cr) {
  ...
  case CK_OP:
    ...
    } else if (opr == OPR_ADD && in_iload && 
	       Htable()->Phase() != MAINOPT_PHASE &&
	       !WOPT_Enable_Copy_Prop_Ops_Into_Array &&
	       /* we added the following checks due to the hack in 

		  void COPYPROP::Copy_propagate(BB_NODE *bb):

		  QUOTE:

	           push the last version to be the current version 
		   because I'm sure I can substitute all current 
		   version with the last version
		   
		  otherwise we may have a dangling use and cause
		  an overlapping live range problem.  p.t.  ------------ */
	       x->Opnd(0)->Kind() == CK_VAR &&
	       !Opt_stab()->NULL_coderep(x->Opnd(0)->Aux_id()) &&
	       Opt_stab()->Top_coderep(x->Opnd(0)->Aux_id()) == x->Opnd(0)) {
      // skip the copy propagation into the array base
      for  (INT32 i = 1; i < x->Kid_count(); i++) {
	expr = Copy_propagate_cr(x->Opnd(i), curbb, inside_cse, in_array);
	if (expr) {
	  need_rehash = TRUE;
	  cr->Set_opnd(i, expr);
                                 ...
                                 PROP_THRU_PHI_PREFERENCE *ppref)
{
  ...
  case CK_OP: {
	...
	! Op_can_be_propagated(lexp->Op(), Htable()->Phase(), 
			       (lexp->Kid_count() > 0) ? 
			       (lexp->Opnd(lexp->Kid_count()-1)->Kind()==CK_CONST) : false))
      return FALSE;
...
// Modify copy propagation to add a new feature underneath the
// existing undef_prop feature: -WOPT:undef_prop=on by itself behaves
// as before. That behavior is that if a variable's value is undefined
// for one operand of a two-operand phi, and is defined by an
// assignment for the other operand, the value for the defined operand
// is forward-substituted for the variable version defined by the phi.
//
// Now, if -WOPT:undef_aggressive=on is also specified, the forward
// substitution can take place even through arbitrary collections of
// phi definitions, provided there is a single ultimate definition
// reaching into the (potentially cyclic) graph of phis defining the
// variable in question. For the substitution to be legal, all phi
// operands in that portion of the SSA graph must either be undefined
// (i.e., defined by entry chi) or must be defined by the unique
// reaching definition.
//
// COPYPROP member functions to support this feature:
// BOOL Find_unique_definition(CODEREP *, slist<PHI_NODE *>, STMTREP *);
// CODEREP *Find_unique_definition(CODEREP *);
// and modifications to CODEREP *Identical_phi_opnd(PHI_NODE *, BB_NODE *).
//
// TODO: The way Identical_phi_opnd() is structured still imposes an
// unnecessary restriction that the phi at which the search begins has
// to have exactly two operands. There is no reason not to support
// phis with more operands. They are rare, though, so it's probably
// good enough as is for now.
//
// -- RK 20050519

// ====================================================================
// Recursive function to do the main work of the 1-ary
// Find_unique_definition() wrapper function below.
//
// Returns TRUE iff the search succeeds. In such a case,
// *candidate_defstmt is set to point to the defining statement whose
// LHS should be taken as the value of the original phi operand.
// ====================================================================
BOOL
COPYPROP::Find_unique_definition(CODEREP            *phi_opnd,
				 slist<PHI_NODE *>  &visited,
				 STMTREP           **candidate_defstmt) {
  if (phi_opnd->Is_flag_set(CF_DEF_BY_PHI)) {
    // Recursively search the operands of the defining phi, unless
    // they've been searched already.
    PHI_NODE *phi = phi_opnd->Defphi();
    if (!phi->Visited()) {
      phi->Set_visited();
      visited.push_front(phi);

      PHI_OPND_ITER phi_opnd_iter(phi);
      CODEREP *opnd;
      FOR_ALL_ELEM(opnd, phi_opnd_iter, Init()) {
	BOOL result = Find_unique_definition(opnd,
					     visited,
					     candidate_defstmt);
	if (!result) {
	  return FALSE;
  ...
  else if (phi_opnd->Is_flag_set(CF_DEF_BY_CHI)) {
    if (!phi_opnd->Undef_at_entry(Opt_stab())) {
      return FALSE;
    }
    else {
      // We assume the entry definition is unique, so we don't need to
      // check that this definition at entry is the same as any others
      // we might see during the search.
    }
  }
  else if (phi_opnd->Is_flag_set(CF_IS_ZERO_VERSION)) {
    return FALSE;
  }
  else {
    STMTREP *defstmt = phi_opnd->Defstmt();
    FmtAssert(defstmt != 0,
	      ("COPYPROP::Find_unique_definition: non-chi, non-phi definition of nonzero version must be stmt"));
    if (*candidate_defstmt == 0) {
      *candidate_defstmt = defstmt;
    }
    else if (defstmt != *candidate_defstmt) {
      return FALSE;
    }
  }
  return TRUE;
...
// ====================================================================
// A function to search through the set of definitions reaching
// the CODEREP phi_opnd and determine whether all those definitions
// are either
//   1. entry chi, or
//   2. a single defstmt D.
// If not, we return our argument value.
// If so, we return the LHS of D.
//
// The idea is that with the appropriate change of versions and
// rehashing, the LHS of D can be taken as the value for phi_opnd if
// WOPT_Enable_Undef_Prop and WOPT_Enable_Undef_Aggressive_Prop are
// both true.
//
// At the beginning of a search initiated from Identical_phi_opnd(),
// we assume the Visited() flag for every phi is clear. We set the
// flag for each phi encountered in our search to indicate that the
// phi has been analyzed already.
// ====================================================================
CODEREP *
COPYPROP::Find_unique_definition(CODEREP *phi_opnd) {
  // A stack of phis visited, so we can reset their Visited() flags
  // when we're done.
  slist<PHI_NODE *> visited_phis;
  STMTREP *candidate_defstmt = 0;

  CODEREP *result;
  BOOL success = Find_unique_definition(phi_opnd,
					visited_phis,
					&candidate_defstmt);

  if (success) {
    if (candidate_defstmt != 0) {
      // If there is anything but a constant in the candidate
      // expression, assignments to those variables could render
      // propagation incorrect. Such a case is now screened out in the
      // caller (under permit_only_constant_rhs), so we don't check
      // here.
      result = candidate_defstmt->Lhs();
    }
    else {
      // It is possible that candidate_defstmt is 0 because we can
      // have a symbol with plenty of phi nodes whose every definition
      // is by a chi, and never by a statement.
      result = phi_opnd;
    }
  }
  else {
    // It is always safe for this function to return its operand.
    result = phi_opnd;
  }

  while (!visited_phis.empty()) {
    PHI_NODE *phi = visited_phis.front();
    visited_phis.pop_front();
    phi->Reset_visited();
  }
...
COPYPROP::Identical_phi_opnd(PHI_NODE *phi, BB_NODE *bb)
{
  ...
  BOOL permit_only_constant_rhs = FALSE;

  // for 2 operands phi's, do not require identical codereps for the two rhs
  CODEREP *lopnd = phi->OPND(0);
  CODEREP *ropnd = phi->OPND(1);

  if (WOPT_Enable_Undef_Prop) {
    if (lopnd->Is_flag_set(CF_DEF_BY_CHI)) {
      if (lopnd->Undef_at_entry(Opt_stab())) {
	if (WOPT_Enable_Undef_Aggressive_Prop) {
	  // RK 20050519
	  ropnd = Find_unique_definition(ropnd);
	}
	permit_only_constant_rhs = TRUE;
	lopnd = ropnd;
      }
      else {
	return NULL;
    ...
    if (ropnd->Is_flag_set(CF_DEF_BY_CHI)) {
      if (ropnd->Undef_at_entry(Opt_stab())) {
	if (WOPT_Enable_Undef_Aggressive_Prop) {
	  // RK 20050519
	  lopnd = Find_unique_definition(lopnd);
	}
	permit_only_constant_rhs = TRUE;
	ropnd = lopnd;
      }
  ...
  if (permit_only_constant_rhs &&
      (!ldefstmt->Rhs()->Contains_only_constants() ||
       ((rdefstmt->Rhs() != ldefstmt->Rhs()) &&		// minor speed optimization
	!rdefstmt->Rhs()->Contains_only_constants()))) {
    return NULL;
  }
...
COMP_UNIT::Do_copy_propagate()
{
  ...
  if (WOPT_Enable_Undef_Aggressive_Prop) {
    ...
    BB_NODE *bb;
    // Mark all phi nodes as unvisited.
