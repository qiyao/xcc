...
// $Revision: 1.14 $
// $Date: 2000/10/29 20:22:43 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_revise_ssa.cxx,v $
//
// ====================================================================
//
/* 
...
OPT_REVISE_SSA::Update_chi_list_for_old_var(STMTREP *stmt, AUX_ID i)
{
  ...
  FOR_ALL_NODE(cnode, chi_iter, Init(stmt->Chi_list())) 
	...
	if (st != NULL) ty = ST_type(st);
        ...
        if (sym->Mclass() & MTYPE_CLASS_TIE || 
	    sym->Mclass()==MTYPE_CLASS_XTBOOL || sym->Mtype()==MTYPE_M)
	  rtype = sym->Mtype();
        else
...
OPT_REVISE_SSA::Form_extract(CODEREP *cr)
{
  ...
  case CK_VAR:
    if (cr->Bit_field_valid()) {
      ...
      x->Set_dtyp(cr->Dtyp());
      if (MTYPE_is_tie(cr->Dtyp()) || MTYPE_is_xtbool(cr->Dtyp()))
        x->Set_dsctyp(cr->Dtyp());
      else
...
OPT_REVISE_SSA::Form_extract_compose(void)
{
  ...
  FOR_ALL_NODE( bb, cfg_iter, Init() ) {
	...
	if (opr == OPR_STBITS) {
	  ...
	  v->Set_dtyp(lhs->Dtyp());
          if (MTYPE_is_tie(lhs->Dtyp()) || MTYPE_is_xtbool(lhs->Dtyp()))
            v->Set_dsctyp(lhs->Dtyp());
          else
