...
// Module: opt_rvi.cxx
// $Revision: 1.97 $
// $Date: 2000/04/06 15:25:04 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_rvi.cxx,v $
//
// ====================================================================
//
/* 
...
static char *rcs_id = 	opt_rvi_CXX"$Revision: 1.97 $";
#endif /* _KEEP_RCS_ID */
...
RVI::Perform_phase2( WN *entry_wn )
{
  ...
  // build dominator tree for more aggressive hoisting. p.t.
  cfg.Compute_dom_tree(TRUE);

  // we don't build dom-tree, so we need to do this little fixup
  /* I don't know what the comment means, but it seems harmless  p.t. */
  cfg.Remove_fake_entryexit_arcs();

  if (WOPT_Enable_Edge_Placement) {
    
    INT count = cfg.Remove_critical_edge(true /* is_whirl */);
    if (count) {
      if ( Tracing() ) {
	fprintf( TFile, "%sBefore RVI::Perform_phase2\n%s", DBar, DBar );
	fprintf( TFile, "After edge Placement: BBs are placed on %d edges\n",
		 count );
	cfg.Print(TFile);
  ...
  // use domintor tree to hoisting lda
  Hoisting_lda_lcm();
...
static ST *Generate_Pu_Static_Literal_Symbol(TYPE_ID mtype, int int_const)
{
  ST *lc;
  char buf[256];
  static int lc_cnt=0;
  ...
  sprintf(buf,".tie_lc%d_%s%d", lc_cnt++, prefix, abs_const);
  ST_Init( lc, Save_Str(buf), CLASS_VAR,
  ...
  INITV_Init_Integer(inv, mtype, int_const);
  return lc;
}


static
void Canonicalize_Tie_Constant_Offset(WN* root, MEM_POOL* pool, BOOL tracing) {

  WN_ITER* wni = WN_WALK_TreeIter(root);
  DYN_ARRAY<WN*> tie_const(pool);
  UTL_Map<int, ST*, UTL_Map_IntHash> const_st(pool);

  if (tracing)
    fprintf(TFile, "%sConver Tie Constant to PU Static Symbol\n%s", DBar, DBar);

  while (wni) {
    WN* wn = wni->wn;
    if (WN_operator(wn)==OPR_CVT &&
	MTYPE_is_tie(WN_rtype(wn)) &&
	MTYPE_byte_size(WN_desc(wn)) <= 4 &&
	WN_operator(WN_kid0(wn))==OPR_INTCONST) {

	TIE_MACRO_p rtor_macro = tie_info->mtype_rtor_macro(
					WN_desc(wn), WN_rtype(wn));
	TIE_MACRO_p mtor_macro = tie_info->mtype_mtor_macro(
					WN_desc(wn), WN_rtype(wn));

	if (mtor_macro &&
	    (rtor_macro==NULL ||
	     mtor_macro->num_instructions()<rtor_macro->num_instructions())) {

	  int int_const = WN_const_val(WN_kid0(wn));
	  tie_const.AddElement(wn);
	  ST* dummy_st;
	  if (const_st.find(int_const, dummy_st)==false) {
	    TYPE_ID mtype = WN_desc(wn);
	    ST* lc = Generate_Pu_Static_Literal_Symbol(mtype, int_const);
	    const_st.insert(int_const, lc);
	  }
	}
    }
  ...
  for (int i=0; i<tie_const.Elements(); i++) {
    WN* cvt = tie_const[i];
    TYPE_ID mtype = WN_desc(cvt);
    int int_const = WN_const_val(WN_kid0(cvt));
    ST* int_const_st;
    FmtAssert(const_st.find(int_const, int_const_st),
		("Missing ST for %d", int_const));
    WN* ldid = WN_CreateLdid(OPR_LDID, Mtype_TransferSign(mtype,MTYPE_I4),
				mtype, 0, int_const_st,
				MTYPE_To_TY(mtype), 0);
    if (tracing) {
	fprintf(TFile, "Replacing ");
	fdump_wn(TFile, WN_kid0(cvt));
    }
    WN_DELETE_Tree(WN_kid0(cvt));
    WN_kid0(cvt) = ldid;
    if (tracing) {
	fprintf(TFile, "with ");
	fdump_wn(TFile, WN_kid0(cvt));
...
RVI::Perform_RVI( WN *entry_wn, ALIAS_MANAGER *alias_mgr )
{
  ...
  Is_True(Verify_alias(alias_mgr,phase1_wn), 
	  ("RVI::Perform_RVI, alias verification failed from RVI phase 1"));
  ...
  if (WOPT_Enable_Expand_MPY_Const) {
    actions |= LOWER_MPY_DIV_CONST;
  }

#ifdef TARG_XTENSA

  /* Before running RVI on LDAs, perform global offset
     canonicalization. */

  Set_Error_Phase("TIE Constant Offset Canon");
  Canonicalize_Tie_Constant_Offset(phase1_wn, _rvi_ppoolp, _tracing);

  Set_Error_Phase("Init Offset Canon");
  Init_Offset_Canonicalization(phase1_wn, _rvi_ppoolp, FALSE, _tracing);
#endif
  
  WN *lda_wn = WN_Lower( phase1_wn, actions, alias_mgr, "RVI" );

  Is_True(Verify_alias(alias_mgr, lda_wn), 
	  ("RVI::Perform_RVI, alias verification failed from lda_wn"));

#ifdef TARG_XTENSA
  Finalize_Offset_Canonicalization();
#endif
  ...
  Is_True(Verify_alias(alias_mgr, phase2_wn), 
	  ("RVI::Perform_RVI, alias verification failed from RVI phase 2"));
...
RVI::Is_base_lda( const WN *wn ) const
{
  Is_True( WN_operator(wn) == OPR_LDA,
    ("RVI::Is_base_lda (not): %s", WN_operator(wn)) );
...
{
  Warn_todo( "RVI::Is_ldid_candidate: returns true", ldidwn );
  return TRUE;
}
...
{
  Warn_todo( "RVI::Is_stid_candidate: returns true", stidwn );
  return TRUE; 
}
...
void 
RVI::Get_wn_local_lda_attributes( BB_NODE *bb, WN *wn, BOOL *check_lda, BOOL hoist_intconst )
{
  IDX_32 bitpos;
  RVI_NODE *rvi_node;
  ...
  if ( ! Black_box(opc) ) {
    ...
    TIE_MACRO_p macro = 
      ((opr == OPR_INTRINSIC_CALL || opr == OPR_INTRINSIC_OP) 
       && INTRN_is_tie_intrinsic(WN_intrinsic(wn))) ?
      tie_info->tie_macro(Intrinsic_To_Tie_Macro_Id(WN_intrinsic(wn))) : NULL;

    // handle kids if necessary, deal with case of constant child
    for ( INT ikid = 0; ikid < WN_kid_count(wn); ikid++ ) {

      // If the type of TIE parameter is not immediate or label
      // try to hoist integer constant used as argument.
      BOOL tie_no_immediate =
        (macro
         && !macro->proto_is_immed(macro->whirl_to_proto_index(ikid)) 
         && !macro->proto_is_label(macro->whirl_to_proto_index(ikid))); 

      BOOL  is_lda;
      WN   *kid = WN_kid(wn, ikid);

      Get_wn_local_lda_attributes( bb, kid, &is_lda, tie_no_immediate );
	...
	if ( Is_lda_candidate( wn, kid, ikid ) ) {
      ...
      } else if (WN_operator(kid) == OPR_INTCONST &&
		 (hoist_intconst || Is_const_candidate( wn, kid, ikid ) ) ) {
	/* added handling of constant here for better hoisting. p.t. */
	rvi_node = Add_to_const_table( kid );
	rvi_node->Add_reference( bb, TRUE/*is_load*/, Rvi_ppool() );
	bitpos = rvi_node->Bitpos();
	bb->Loc_appear()->Union1D( bitpos );
      } else if (WN_operator(kid) == OPR_CONST) {
	/* added floating point constant here */
	rvi_node = Add_to_const_table( kid );
	rvi_node->Add_reference( bb, TRUE/*is_load*/, Rvi_ppool() );
	bitpos = rvi_node->Bitpos();
	bb->Loc_appear()->Union1D( bitpos );
...
/*---------------------------------------------------------------------*
 * Perform hoisting of LDA using least common ancestor in the          *
 * dominator tree                                                      *
 *---------------------------------------------------------------------*/

void
RVI::Hoisting_lda_lcm()
{
  RVI_CTAB_ITER ctab_iter;
  RVI_NODE *cnode;
  FOR_ALL_NODE( cnode, ctab_iter, Init( Rvi_ctab() ) ) {
    RVI_LRBB_ITER  lrbb_iter;
    RVI_LRBB      *lrbb;
    RVI_LRBB_LIST *appearance = cnode->Appearances();

    /* appears in more than one BB */
    if (appearance->Head() != appearance->Tail()) {
      /* find the least common ancestor in the dominator tree */
      BB_NODE *lcm           = appearance->Head()->Bb();
      BOOL     lcm_is_a_lrbb = TRUE;

      FOR_ALL_NODE( lrbb, lrbb_iter, Init( appearance ) ) {
	BB_NODE *cur_bb = lrbb->Bb();

	if (cur_bb->Dominates(lcm)) {
	  lcm           = cur_bb;
	  lcm_is_a_lrbb = TRUE;
	} else if (!lcm->Dominates(cur_bb)) {

	  /* find the common dominator */
	  BB_NODE *idom = cur_bb->Idom();
	  while ( idom && !idom->Dominates(lcm) ){
	    idom = idom->Idom();
	  }
	  if (idom && idom != Cfg()->Fake_entry_bb()) {
	    lcm           = idom;
	    lcm_is_a_lrbb = FALSE;
	  } else {
	    DevWarn("Hoisting_lda_lcm: common ancestor is fake entry or NULL");
	  }
	}

      } // FOR_ALL_NODE

      /* create a hoisting appearance at the lcm */
      if (!lcm_is_a_lrbb) {
	Is_True(appearance->Find(lcm) == NULL, 
		("Hoisting_lda_lcm: lcm already in RVI_LRBB_LIST"));
	RVI_LRBB *lcm_lrbb = CXX_NEW(RVI_LRBB(lcm), Rvi_ppool());
	lcm_lrbb->Set_load_cnt( lcm_lrbb->Load_cnt() + 1 );
	appearance->Append( lcm_lrbb );

	/* set the local data flow information */
	lcm->Loc_appear()->Union1D( cnode->Bitpos() );
      }
...
RVI::Get_bb_local_lda_attributes( BB_NODE *bb )
{
  ...
  if ( first == NULL ) {
    ...
    FOR_ALL_ELEM( wn, stmt_iter, Init(bb->Firststmt(),bb->Laststmt()) ){
      BOOL is_lda;
      Get_wn_local_lda_attributes( bb, wn, &is_lda, FALSE );
      Is_True( !is_lda,
