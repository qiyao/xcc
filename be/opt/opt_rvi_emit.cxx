...
// $Revision: 1.41 $
// $Date: 2000/04/06 15:25:08 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_rvi_emit.cxx,v $
//
// ====================================================================
//
/* 
...
static char *rcs_id = 	opt_rvi_emit_CXX"$Revision: 1.41 $";
#endif /* _KEEP_RCS_ID */
  ...
  *new_wn = NULL;
  ...
  if ( ! Rvi()->Black_box(opc) ) {
    ...
    for ( INT ikid = 0; ikid < WN_kid_count(wn); ikid++ ) {
      ...
      //
      /* Handle constant together with LDA for better hoisting p.t.*/
      WN *kid = WN_kid(wn, ikid);
      OPERATOR oper = WN_operator(kid);

      if ((oper != OPR_LDA || Rvi()->Is_lda_candidate(wn, kid, ikid)) ||
	  (oper != OPR_INTCONST || Rvi()->Is_const_candidate(wn, kid, ikid)) ||
	  (oper != OPR_CONST)) {
        WN *new_kid;
  ...
  if ( opr == OPR_LDA || opr == OPR_INTCONST || opr == OPR_CONST ) {
    // these constants are always candidates for our expected targets
    bitpos = Rvi()->Get_bitpos(wn);
    if ( bitpos != ILLEGAL_BP ) {
      if ( (annotation = bb->Rvi_anns()->Find( bitpos )) != NULL ) {
