...
// Module: opt_ssa.cxx
// $Revision: 1.250 $
// $Date: 2000/07/25 00:39:50 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_ssa.cxx,v $
//
// Revision history:
...
static char *rcs_id = 	opt_ssa_CXX"$Revision: 1.250 $";
#endif /* _KEEP_RCS_ID */
...
SSA::Insert_identity_assignment_4_loopexit(BB_NODE *start, AUX_ID aux_id, MEM_POOL *def_bb_pool)
{
  ...
  if (sym->Is_real_var() &&
      ...
      !sym->Is_flag_const_init())
    {
	  ...
	  ((size == 2 && !OPT_IV_Short_Wrap_Around_Safe) || size < 2)) {
...
SSA::Create_CODEMAP(void)
{
  ...
  {
    extern DLL_SHARED BOOL Simp_Canonicalize;
		       ...
		       COPYPROP *copyprop, EXC *exc)
{
  ...
  FOR_ALL_ELEM (succ, bb_iter, Init(bb->Succ())) {
    ...
    if (pos != -1) {
      // pos can be -1 if bb is the fake_entry_bb and its successors
      // may not find bb in the predecessor list after
      // Remove_fake_entryexit_arcs() which makes the pred/succ
      // relation not symmetrical
      FOR_ALL_ELEM (phi, phi_iter, Init(succ->Phi_list())) {
	if (phi->Live()) {
