...
// Module: opt_sym.cxx
// $Revision: 1.396 $
// $Date: 2000/08/31 21:23:21 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_sym.cxx,v $
//
// ====================================================================
//
/* 
...
extern DLL_SHARED WN_MAP Prompf_Id_Map;
#endif // Is_True_On
...
OPT_STAB::Count_syms(WN *wn)
{
  ...
  if (OPERATOR_has_aux(WN_operator(wn)) &&
	...
	if (ST_is_const_initialized(st) ||
            (ST_is_initialized(st) && ST_sclass(st) == SCLASS_PSTATIC)) {
...
Check_volatility( WN *wn, ST *st )
{
  ...
  if (TY_is_volatile(WN_ty(wn)) || TY_is_volatile(WN_object_ty(wn)))
    return TRUE;
		       ...
		       TY_IDX wn_object_ty, BOOL is_volatile, const WN* wn)
{
  ...
  case OPR_LDID:
	...
	no_register = TRUE;
      ...
      byte_size = (MTYPE_size_min(WN_desc(wn))+7) >> 3;
  ...
  case OPR_STID:
	...
	no_register = TRUE;
      ...
      if (ST_sclass(st) == SCLASS_REG) {
        byte_size = TY_size(ST_type(st));
        value_size = MTYPE_size_min(WN_desc(wn));
      }
    }
    mtype = WN_desc(wn);
    mclass = Get_mtype_class(mtype);
  ...
  while (idx && aux_stab[idx].St() != NULL) {
    ...
    if (kind_match &&
	!(aux_stab[idx].Mtype() != mtype &&
	  (MTYPE_is_tie(aux_stab[idx].Mtype()) || MTYPE_is_tie(mtype) ||
	   MTYPE_is_xtbool(aux_stab[idx].Mtype()) || MTYPE_is_xtbool(mtype))) &&
	...
	// get value size info for PREGs
        if (opr==OPR_STID && ST_sclass(st) == SCLASS_REG &&
	    !aux_stab[idx].Is_dedicated_preg()) {
	  Is_True((aux_stab[idx].Value_size()==0 ||
		   aux_stab[idx].Value_size()==value_size),
		  ("In compatible value size"));
	  aux_stab[idx].Set_value_size(value_size);
	}
      }
  ...
  // from Fred's email on pro64 mailing list
  ...
  // if this symbol is created from a TIE converting ldid, disable some SSA
  // optimization since the SSA created version may miss the needed conversion
  if (opr==OPR_LDID && (MTYPE_is_tie(WN_rtype(wn)) || MTYPE_is_tie(WN_desc(wn))) &&
      WN_rtype(wn)!=WN_desc(wn)) {
    sym->Set_has_tie_conversion();
  }
...
AUX_ID
OPT_STAB::Enter_ded_preg(ST *st, INT64 ofst, TY_IDX ty, mUINT16 mclass)
{
  ST_CHAIN_INFO *st_chain_info = st_chain_map->Lookup(ST_st_idx(st));
  AUX_ID         idx;
  if (st_chain_info != NULL) {
...
OPT_STAB::Convert_IO_statement( WN *iown, WN *wn, INT level )
{
  ...
  if ( opr == OPR_IO_ITEM && WN_intrinsic(wn) == (INTRINSIC) IOC_VARFMT ) {
...
AUX_STAB_ENTRY::Equivalent(AUX_STAB_ENTRY *sym)
{
  if (this->Base() == sym->Base() &&
    ...
    if (MTYPE_is_tie(this->Mtype()) || MTYPE_is_xtbool(this->Mtype()) ||
        MTYPE_is_tie(sym->Mtype()) || MTYPE_is_xtbool(sym->Mtype())) {

      return this->Mtype() == sym->Mtype();
...
OPT_STAB::Create(COMP_UNIT *cu, REGION_LEVEL rgn_level)
{
  ...
  aux_stab.Alloc_array((mUINT32)(aux_sym_cnt*1.2 + 10));
...
void OPT_STAB::Print_aux_entry(AUX_ID i, FILE *fp)
{
  ...
  case VT_SPECIAL_VSYM:
    ...
    if (aux_stab[i].St() != NULL) {
      fprintf(fp, "VAR   st=%s base=%s byte ofst=%" LLD_FMT " byte size=%" LLD_FMT ""
  ...
  case VT_OTHER:
    fprintf(fp, "SYMBL st=%s base=%s ofst=%" LLD_FMT "\n",
	    aux_stab[i].St_name(), aux_stab[i].Base_name(),
...
BOOL
AUX_STAB_ENTRY::Is_local_rvi_candidate(BOOL varargs_func) const
{ 
#if 0
  if (Is_real_var()) {
    fprintf(stderr, "Is_real_var\n");
  } else {
    fprintf(stderr, "NOT: Is_real_var\n");
  }
  if (Has_nested_ref()) {
    fprintf(stderr, "Has_nested_ref\n");
  } else {
    fprintf(stderr, "NOT: Has_nested_ref\n");
  }
  if (Disable_local_rvi()) {
    fprintf(stderr, "Disable_local_rvi\n");
  } else {
    fprintf(stderr, "NOT: Disable_local_rvi\n");
  }
  if (ST_class(st) != CLASS_PREG &&
      ST_sclass(st) != SCLASS_FORMAL_REF &&
      ST_sclass(st) != SCLASS_PSTATIC) {
    fprintf(stderr, "Pass ST_class/ST_sclass\n");
  } else {
    fprintf(stderr, "Fail ST_class/ST_sclass\n");
  }
  if (!(varargs_func && ST_sclass(st) == SCLASS_FORMAL)) {
    fprintf(stderr, "Pass vararg test\n");
  } else {
    fprintf(stderr, "Fail vararg test\n");
  }
  if (Is_volatile()) {
    fprintf(stderr, "Is_volatile\n");
  } else {
    fprintf(stderr, "NOT: Is_volatile\n");
  }
  if (Cr_list()) {
    fprintf(stderr, "Cr_list\n");
  } else {
    fprintf(stderr, "NOT: Cr_list\n");
  }
  if (points_to.Local() && points_to.No_alias() && !points_to.F_param()) {
    fprintf(stderr, "Pass points_to\n");
  } else {
    fprintf(stderr, "Fail points_to\n");
  }
  if (!Has_multiple_signs()) {
    fprintf(stderr, "Pass Has_multiple_sign\n\n");
  } else {
    fprintf(stderr, "Fail Has_multiple_sign\n\n");
  }
#endif
...
AUX_STAB_ENTRY::Has_multiple_signs(void) const
{
  ...
  if (Cr_list()) {
    BOOL sign_extd;
    BOOL has_tie_conversion=false;
    FOR_ALL_NODE(cr, iter, Init(Cr_list())) {
      if (!cr->Is_flag_set(CF_MADEUP_TYPE))
	sign_extd = cr->Is_sign_extd();
      if (!has_tie_conversion &&
	  cr->Dtyp()!=cr->Dsctyp() &&
	  (MTYPE_is_tie(cr->Dtyp()) ||
	   MTYPE_is_tie(cr->Dsctyp())))
	has_tie_conversion = true;
    }
	...
	return TRUE;
      ...
      if (cr->Is_flag_set(CF_DEF_BY_CHI)) {
	if (!WOPT_Enable_Rvi_Uninitialized) {
	  return TRUE;
	}

	/* do not do local rvi on un-initialized TIE convering load 
	 * it causes a preg of the result type to be loaded with desc
	 * type but the two types may be in different register files
	 * which is not ok in code selection
	 *
	 * see pr6859
	 */
	if (has_tie_conversion) {
	...
	/* live in to the program/region */
	if (cr->Defstmt()->Op()==OPC_REGION) {
	  /* region */
	  return TRUE;
	} else if (cr->Defstmt()->Op() == OPC_OPT_CHI) {
	  BB_NODE *bb = cr->Defstmt()->Bb();
	  if (bb->Kind() != BB_ENTRY || ST_sclass(st) == SCLASS_FORMAL) {
	    /* defined by entry CHI and is not an input parameter */
	    /* fix gnat #2216 */
	    return TRUE;
	  }
	  if (bb->Kind() == BB_ENTRY) {
	    /* defined by entry CHI and is exception handler */
	    WN *entry_wn = bb->Entrywn();
	    if (WN_opcode(entry_wn)==OPC_LABEL &&
		WN_Label_Is_Handler_Begin(entry_wn)) {
	      return TRUE;
	    }
	  }
	}
...
void VER_STAB_ENTRY::Print(FILE *fp) const
{
  ...
  switch (Type()) {
  case WHIRL_STMT:
    fprintf(fp, "Wn  *0x%lx ", Wn());
    fprintf(fp, " use=");
    Print_use(Wn(), fp);
    break;
  case PHI_STMT:
    fprintf(fp, "Phi *0x%lx ", Phi());
    fprintf(fp, " use=");
    Print_use(Phi(), Bb(), fp);
    if (!Phi()->Live()) 
      fprintf(fp, "^");
    else
      fprintf(fp, "ref_wn: 0x%lx", Ref_wn());
    break;
  case CHI_STMT:
    fprintf(fp, "Chi *0x%lx ", Chi());
...
OPT_STAB::Collect_nested_ref_info(void)
{
  ...
  /* Not const */ vector <ST_TAB *>::iterator symtab;
