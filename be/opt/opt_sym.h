...
// Module: opt_sym.h
// $Revision: 1.280 $
// $Date: 2000/08/31 21:23:30 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_sym.h,v $
//
// Revision history:
...
// some flags for an aux_stab_entry
enum AUXF_FLAGS {
...
enum AUXF2_FLAGS {
  ...
  AUXF2_HAS_TIE_CONVERSION
     			= 0x80,    // symbol created from converting ldid.
  				   // make some SSA optimizations less aggressive
  // 				   // for this symbol
};
...
//  Auxilliary symbol table used by preopt.
//
...
private:
  ...
  mINT8   _more_flags;		      // overflow field for flags field (AUXF2_FLAGS)
  mUINT16 _mclass;     		      // mtype class, e.g. INT, FLOAT, COMPLEX
  mTYPE_ID _mtype;                    // mtype (for TIE and XTBOOL types)
  ...
  AUX_ID    st_chain;                 // chain of aux_sym pointing to
				      ...
				      // is found via ST_temp() in the
				      // old symbol table scheme, or
				      // via TODO in the new one.
  ...
  void     Set_mclass(mUINT16 mclass)   { _mclass = mclass; }
  ...
  void     Clear_flags(void)
                        { _flags = 0; _more_flags = 0; }

  void     Set_const_init(void)
			{ _flags |= AUXF_CONST_INIT; }

public:
  OPT_VAR_TYPE  Stype(void) const     { return (OPT_VAR_TYPE) stype; }
  mUINT16    Mclass(void) const         { return _mclass; }
  ...
  BOOL     Has_tie_conversion(void) const    { return _more_flags & AUXF2_HAS_TIE_CONVERSION; }
  void     Set_has_tie_conversion(void)      { _more_flags |= AUXF2_HAS_TIE_CONVERSION;       }
  ...
  BOOL     Is_non_dedicated_preg(void) const
    {
      return (Is_preg() && !Preg_Is_Dedicated(_st_ofst));
    }
...
private:
  ...
  WN                        *pu_wn;
  BOOL			    _rgn_trace;	     // trace flag for region bounds
  ...
  ID_MAP<ST_CHAIN_INFO *,
         ST_IDX>            *st_chain_map;   // mapping from ST_IDX to
					     // head of ST chain
  IDTYPE                    _orig_last_preg; // the original last preg number
  IDTYPE                    _last_preg_num;  // the last preg number
  AUX_ID                    _default_vsym;   // default virtual sym
  AUX_ID                    _return_vsym;    // return sym
  OPT_PHASE		    _phase;	     // the optimization phase
  BOOL                      _flow_free_analysis;  // alias analysis mode
  BOOL                      _allow_sim_type;  // allow simulated types
  BOOL                      _has_exc_handler;// PU has exception handling
  ...
  //  bit vector of the OPT_ST attributes
  //
  BS                        *_addr_saved;
  BS                        *_addr_passed;
  ...
  BS                        *_weak_var;
  BS                        *_weak_base;
  // The following is derived; see opt_alias_class.cxx
  // ALIAS_CLASSIFICATION::Classify_memops().
  BS                        *_inaccessible_to_callees;
  // For now, don't put a _not_auto bit set in OPT_STAB; _not_auto
  // information is needed only in POINTS_TO (see
  // opt_alias_analysis.h), and is derived in
  // OPT_STAB::Collect_ST_attr (see opt_sym.cxx).
  BS                        *_indirect;      // derived -- see Collect_ST_Attr()
  BS                        *_call_by_value; // derived -- see Collect_ST_Attr()
  BS                        *_call_by_ref;   // derived -- see Collect_ST_Attr()
  BS                        *_asm_alias;     // derived -- see Collect_ST_Attr()
...
public:
  ...
  AUX_ID   Enter_ded_preg(ST *, INT64, TY_IDX, mUINT16);
...
// Iterator for AUX_STAB
//    iterates from 1 to aux_stab.sizeof() - 1
//
class AUX_STAB_ITER {
private:
  INT32     sym_id;      // the symbol id
...
public:
        AUX_STAB_ITER(OPT_STAB *symtab)     { sym_id = 1; stab = symtab; }
       ~AUX_STAB_ITER(void)		    {}
  void  Init(void) const                    {}
  INT32 First(void)                         { return (sym_id = 1); }
  BOOL  Is_Empty(void)                      { return sym_id > stab->Lastidx(); }
  INT32 Next(void)                          { return ++sym_id; }
  INT32 Cur(void) const                     { return sym_id; }
};

// Iterator for AUX_STAB
//    iterates from aux_stab.sizeof() - 1 down to 1.
//
...
public:
  AUX_STAB_REVERSE_ITER(OPT_STAB *symtab) :
    stab(symtab), sym_id(symtab->Lastidx())
      { }
  ~AUX_STAB_REVERSE_ITER(void)	    {}
  void  Init(void) const            {}
  INT32 First(void)                 { return (sym_id = stab->Lastidx()); }
...
//  Iterator for VER_STAB
//      iterates from 0 to ver_stab.sizeof()-1
//
class VER_STAB_ITER {
private:
  INT32     ssa_id;      // the symbol id
  VER_STAB_ARRAY_TYPE *ver_stab;        // symbol table
...
public:
        VER_STAB_ITER(VER_STAB_ARRAY_TYPE *darray)     
	                                    { ssa_id = 2 ; ver_stab = darray; }
       ~VER_STAB_ITER(void)		    {}
  void  Init(void) const                    {}
  INT32 First(void)                         { return (ssa_id = 2); }
  BOOL  Is_Empty(void)                      { return ssa_id >= ver_stab->Size(); }
  INT32 Next(void)                          { return ++ssa_id; }
  INT32 Cur(void) const                     { return ssa_id; }
};
