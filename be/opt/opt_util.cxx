...
#ifdef _KEEP_RCS_ID
static char *rcs_id = 	opt_util_CXX"$Revision: 1.37 $";
#endif /* _KEEP_RCS_ID */
...
void Warn_todo(char *msg, ... )
{
  ...
  va_start ( vp, msg );
  vsprintf(msg_buf, msg, vp);     // if msg is too long, it might overrun the buf
  len = strlen(msg_buf);
...
INT Report_statistics()
{
  ...
  if (Get_Trace(TKIND_INFO, TINFO_TIME)) {
	...
	peak_times[i] = p;
      fprintf(TFile,
	      "%8d %6.2f%% (cum %8d %5.2f, peak %5.2f%%) ms %5"LLD_FMT"k mem in %s",
