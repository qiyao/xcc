...
// Module: opt_vn.cxx
// $Revision: 1.39 $
// $Date: 2000/04/06 15:26:19 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/be/opt/RCS/opt_vn.cxx,v $
//
// ====================================================================
//
/* 
...
VN::_valnum_op(CODEREP *cr)
{
   ...
   else
   {
	 ...
	 } else if (cr->Opr() == OPR_OUTPART) {
	    // We treat a OUTPART as a binary operator!
	    //
	    vn0 = _valnum_expr(cr->Opnd(0));
	    vn1 = _valnum_integer(cr->Offset(), FALSE);
	    vn_expr_ptr = VN_EXPR::Create_Binary(opc, vn0, vn1);
...
VN::_valnum_memloc_load(CODEREP *cr)
{
   ...
   if (cr->Is_ivar_volatile() || opr == OPR_ILDBITS || 
       MTYPE_is_tie(dtype) /* disable it for TIE type */)
   {
      ...
      // TODO: do not give up on TIE_type 
