...
static char *rcs_id = 	opt_wn_CXX"$Revision: 1.191 $";
#endif /* _KEEP_RCS_ID */
...
#include "config_targ_options.h"// xt_minmax
...
Ldid_from_mtype( MTYPE mtype )
{
  ...
  if (MTYPE_is_tie(mtype) || MTYPE_is_xtbool(mtype))
    return OPCODE_make_op(OPR_LDID, mtype, mtype);
...
WN_add_lno_info(WN *wn, CODEREP *cr)
{
  Is_True(OPCODE_is_load(WN_opcode(wn)) || OPCODE_is_store(WN_opcode(wn)) ||
	  ...
	  ("opcode is not load/store nor TIE intrinsic call."));

  if (WN_operator(wn)==OPR_INTRINSIC_CALL) {
    if (Current_Dep_Graph != NULL) {
      VINDEX16 vertex = cr->Intr_call_occ()->Lno_dep_vertex();
      if (vertex != 0) {
    ...
    return;
  } else if (Current_Dep_Graph != NULL && cr->Kind() == CK_IVAR) {
    VINDEX16 vertex = cr->Ivar_occ()->Lno_dep_vertex();
...
{
  fprintf(fp, "\tpref1=0x%lx trip1=%d ", PF_PTR_wn_pref_1L(p), PF_PTR_lrnum_1L(p));
  fprintf(fp, "pref2=0x%lx trip2=%d\n", PF_PTR_wn_pref_2L(p), PF_PTR_lrnum_2L(p));
}
...
Identity_assignment_type( AUX_STAB_ENTRY *sym )
{
  ...
  if ( ! Is_Simple_Type( ty ) ) {
    MTYPE mtype;
    if (sym->Mclass() & MTYPE_CLASS_TIE || sym->Mclass()==MTYPE_CLASS_XTBOOL)
      mtype = sym->Mtype();
    else
      mtype = Mtype_from_mtype_class_and_size(MTYPE_type_class(sym->Mtype()), 
					      sym->Byte_size());
...
Create_identity_assignment(AUX_STAB_ENTRY *sym, AUX_ID aux_id, TY_IDX ty)
{
  ...
  if (sym->Mclass() &  MTYPE_CLASS_TIE || sym->Mclass() == MTYPE_CLASS_XTBOOL) {
    ...
    ldidop = Ldid_from_mtype_class_and_size(MTYPE_type_class(sym->Mtype()), sym->Byte_size());
    stidop = Stid_from_mtype_class_and_size(MTYPE_type_class(sym->Mtype()), sym->Byte_size());
  }
...
BOOL 
Op_can_be_propagated(OPCODE op, OPT_PHASE phase, bool const_amount)
{
  ...
  if (phase != MAINOPT_PHASE && !WOPT_Enable_Copy_Prop_LNO_Ops &&
      ...
      (OPCODE_operator(op) == OPR_DIV || OPCODE_operator(op) == OPR_REM ||
       OPCODE_operator(op) == OPR_ASHR || OPCODE_operator(op) == OPR_LSHR ||
       (OPCODE_operator(op) == OPR_SHL && !const_amount)))
    return FALSE;
...
#ifdef TARG_XTENSA
  // Do not propagate MIN/MAX when we don't have those instructions.
  // When returning more than one register (newtests/nenad/lcc_struct)
  // CG will complain about defs of dedicated registers that are not 
  // reaching a CALL or an EXIT BB.
  if (!xt_minmax && (opr == OPR_MIN || opr == OPR_MAX))
    return FALSE;
#endif
