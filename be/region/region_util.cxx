...
#elif defined(_WIN32)
__declspec(dllexport) void Print_points_to(FILE * fp, POINTS_TO * ptmp);
#else
...
void RID_set_print(FILE *FD, RID *rid)
{
  ...
  fprintf(FD,
	  ...
	  "bounds_defined=%c, parent_block=0x%lx\n",
...
void RID_Fprint(FILE *FD, RID *rid)
{
  ...
  fprintf(FD, "  parent_block=0x%lx, cginfo=0x%lx, has_return=%c, "
...
void REGION_CS_print(REGION_CS_ITER *iter)
{
  ...
  fprintf(TFile, "parent_block = 0x%lx\n", iter->parent_block);
...
WN *REGION_remove_and_mark(WN *pu, REGION_CS_ITER *iter)
{
  ...
  WN_verifier(rwn, FALSE);
  WN_verifier(new_rwn, FALSE);
  return new_rwn;
}
