				...
				INT *flag, TY_IDX *ty)
{
   ...
   *ty = 0;
   ...
   while (!done) {
	 ...
	 *flag |= F90_THROUGH_ARRAY;
	 ...
	 if (WN_operator(kid) == OPR_INTCONST) {
	    ADD_PIECE(kid);
	    addr = WN_kid0(addr);
	    if (trace_dependence) fprintf(TFile,"  Analyze_one_address (Add %" LLD_FMT ")\n",WN_const_val(kid));
	    continue;
	 }
	 kid = WN_kid0(addr);
	 if (WN_operator(kid) == OPR_INTCONST) {
   ...
   if (trace_dependence) {
      if (*sym) {
	 fprintf(TFile,"sym: %s, base_offset %" LLD_FMT ", const_offset %" LLD_FMT ", size %" LLD_FMT "\n",
...
static DIR_FLAG Analyze_index(WN *i1, WN *i2) {
   ...
   if (trace_dependence) {
      if (e_constant) fprintf(TFile,"E = %" LLD_FMT "\n",e);
      if (l_constant) fprintf(TFile,"L = %" LLD_FMT "\n",l);
      if (s_constant) fprintf(TFile,"S1,S2 = %" LLD_FMT " %" LLD_FMT "\n",s1,s2);
   }
...
static void check_for_duplicates(WN *pu,char *str)
{
   ...
   while (ti) {
	 ...
	 p = GET_P_MAP(k);
	 if ((p != NULL) && (p != w)) {
	    fprintf(TFile,"\n%s: Multiparented node p=%08lx, w=%08lx, k=%d\n",str,p,w,i);
