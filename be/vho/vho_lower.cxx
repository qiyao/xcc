...
/* the order of the element in Promoted_Mtype need to correspond to the mtype
   ...
   allowed
*/
TYPE_ID Promoted_Mtype [MTYPE_MAX] = {
...
typedef struct switch_case_table_entry_t {
  WN    * wn;
  WN    *low_val;  //colapse continguous switches to the same label
  WN    *high_val;
  INT32   count;
  FB_FREQ freq;
} VHO_SWITCH_ITEM;
...
#define VHO_SWITCH_high_val(i)    (VHO_Switch_Case_Table[i].high_val)
#define VHO_SWITCH_low_val(i)    (VHO_Switch_Case_Table[i].low_val)
...
static OPCODE    VHO_Switch_LE_Opcode;     /* opcode for <=  comparison       */
static OPCODE    VHO_Switch_GE_Opcode;     /* opcode for >=  comparison       */
...
#define SWITCH_key(i) WN_const_val(VHO_SWITCH_wn(i))
#define SWITCH_high_key(i) WN_const_val(VHO_SWITCH_high_val(i))
#define SWITCH_low_key(i) WN_const_val(VHO_SWITCH_low_val(i))
 ...
 *
 * static WN *
 * VHO_Switch_Generate_If_Else ( SRCPOS srcpos, WN *block )
 *
...
VHO_Switch_Generate_If_Else ( SRCPOS srcpos , WN *block)
{
  ...
  for ( i = 0; i < VHO_Switch_Ncases; i++ ) {
    ...
    case_low_val = VHO_SWITCH_low_val(i);
    case_high_val = VHO_SWITCH_high_val(i);
    if (case_low_val == case_high_val) {
      ...
      Is_True (case_low_val != case_high_val, ("bad low_val or high_val"));
      low_val     = WN_CreateIntconst ( VHO_Switch_Int_Opcode,
                                    WN_const_val(case_low_val) );
      high_val     = WN_CreateIntconst ( VHO_Switch_Int_Opcode,
                                    WN_const_val(case_high_val) );
      LABEL_IDX label;
      LABEL_Init (New_LABEL (CURRENT_SYMTAB, label), 0, LKIND_DEFAULT);
      WN *label_wn = WN_CreateLabel ( (ST_IDX) 0, label, 0, NULL );
      WN_Set_Linenum ( label_wn, srcpos );

      test      = WN_CreateExp2 ( VHO_Switch_GT_Opcode,
                                WN_COPY_Tree ( VHO_Switch_Index ), high_val );
      wn_high        = WN_CreateTruebr ( WN_label_number(label_wn), test );
      WN_Set_Linenum ( wn_high, srcpos );
      WN_INSERT_BlockAfter ( block, WN_last(block), wn_high );

      test      = WN_CreateExp2 ( VHO_Switch_GE_Opcode,
                                WN_COPY_Tree ( VHO_Switch_Index ), low_val );
      wn_low        = WN_CreateTruebr ( WN_label_number(case_goto), test );
      WN_Set_Linenum ( wn_low, srcpos );
      WN_INSERT_BlockAfter ( block, WN_last(block), wn_low );

      if ( Cur_PU_Feedback ) {
        Cur_PU_Feedback->Annot( wn_high, FB_EDGE_BRANCH_TAKEN, (freq_rest - VHO_SWITCH_freq(i))/2);
        Cur_PU_Feedback->Annot( wn_high, FB_EDGE_BRANCH_NOT_TAKEN, (freq_rest + VHO_SWITCH_freq(i))/2 );
        Cur_PU_Feedback->Annot( wn_low, FB_EDGE_BRANCH_TAKEN, VHO_SWITCH_freq(i));
        Cur_PU_Feedback->Annot( wn_low, FB_EDGE_BRANCH_NOT_TAKEN, (freq_rest - VHO_SWITCH_freq(i))/2 );
        freq_rest -= VHO_SWITCH_freq(i);
      }

      WN_INSERT_BlockAfter ( block, WN_last(block), label_wn );
    }
  }

  if ( wn && Cur_PU_Feedback )
...
VHO_Switch_Generate_Compgoto ( SRCPOS srcpos )
{
  ...
  case_goto  = VHO_SWITCH_low_val(0);
  ...
  n =   WN_const_val(VHO_SWITCH_high_val(VHO_Switch_Ncases - 1))
      - curr_value + 1;
  ...
  while (j < n) {
	...
	freq_new[j] = FB_FREQ_ZERO;
      ...
      if (VHO_Switch_Signed) {
        while (case_value <= SWITCH_high_key(i)) {
          wn = WN_CreateGoto ( (ST_IDX) NULL, WN_label_number(case_goto) );
          if ( Cur_PU_Feedback ) {
	    freq_new[j] = VHO_SWITCH_freq(i)/(SWITCH_high_key(i) - SWITCH_low_key(i) + 1);
          }
          case_value++;
          WN_Set_Linenum ( wn, srcpos );
          WN_INSERT_BlockAfter ( block, WN_last(block), wn );
	  ...
	  j++;
        ...
        while ((UINT64) case_value <= (UINT64) SWITCH_high_key(i)) {
          wn = WN_CreateGoto ( (ST_IDX) NULL, WN_label_number(case_goto) );
          if ( Cur_PU_Feedback ) {
	    freq_new[j] = VHO_SWITCH_freq(i)/((UINT64) SWITCH_high_key(i) - SWITCH_low_key(i) + 1);
          }
          case_value++;
          WN_Set_Linenum ( wn, srcpos );
          WN_INSERT_BlockAfter ( block, WN_last(block), wn );
...
static void
VHO_Switch_Generate_Binary_Search ( INT32 left, INT32 right, WN * block, 
				FB_FREQ range_default )
{
  INT32    mid;
  INT64    low_value, high_value;
  WN     * test;
  WN     * wn;
  SRCPOS   srcpos;
  ...
  mid   = ( left + right + 1 ) >> 1;
  low_value = SWITCH_low_key(mid);
  high_value = SWITCH_high_key(mid);

  FB_FREQ right_default = range_default; // how much frequency for all 
					// the default clauses from mid to right
  FB_FREQ left_default = range_default;
  FB_FREQ right_freq;
  FB_FREQ left_freq;
  FB_FREQ mid_freq;
  if (Cur_PU_Feedback) {
    if (mid > left || mid < right) { // guess that if we are in a default case
				     // it's equally likely that we go right
				     // as left
      right_default *= 0.5;
      left_default = range_default - right_default;
    } 
    left_freq = left_default;
    right_freq = right_default;
    mid_freq = VHO_SWITCH_freq(mid);
    for (int i=left; i<mid; i++) {
      left_freq += VHO_SWITCH_freq(i);
    }
    for (int i=mid+1; i<=right; i++) {
      right_freq += VHO_SWITCH_freq(i);
    }
  }
  ...
  if ( lt_label == gt_label ) { // left == mid == right
    ...
    if (low_value == high_value) {
      ...
      WN_Set_Linenum ( wn, srcpos );
      if ( Cur_PU_Feedback ) {
        Cur_PU_Feedback->Annot( wn, FB_EDGE_BRANCH_TAKEN, range_default );
        Cur_PU_Feedback->Annot( wn, FB_EDGE_BRANCH_NOT_TAKEN, mid_freq);
      }
      ...
      if ( Cur_PU_Feedback ) {
        Cur_PU_Feedback->Annot( wn, FB_EDGE_BRANCH_TAKEN, left_default );
        Cur_PU_Feedback->Annot( wn, FB_EDGE_BRANCH_NOT_TAKEN, right_default+mid_freq);
      }
      WN_Set_Linenum ( wn, srcpos );
      WN_INSERT_BlockAfter ( block, WN_last(block), wn );
      ...
      wn = WN_CreateTruebr ( lt_label, test );
      if ( Cur_PU_Feedback ) {
        Cur_PU_Feedback->Annot( wn, FB_EDGE_BRANCH_TAKEN, right_default );
        Cur_PU_Feedback->Annot( wn, FB_EDGE_BRANCH_NOT_TAKEN, mid_freq);
      }
      WN_Set_Linenum ( wn, srcpos );
      WN_INSERT_BlockAfter ( block, WN_last(block), wn );
    ...
    WN_Set_Linenum ( wn, srcpos );
    if ( Cur_PU_Feedback ) {
      Cur_PU_Feedback->Annot( wn, FB_EDGE_BRANCH_TAKEN, left_freq );
      Cur_PU_Feedback->Annot( wn, FB_EDGE_BRANCH_NOT_TAKEN,
			      right_freq+mid_freq );
    }
    ...
    WN_Set_Linenum ( wn, srcpos );
    if ( Cur_PU_Feedback ) {
      Cur_PU_Feedback->Annot( wn, FB_EDGE_BRANCH_TAKEN, right_freq );
      Cur_PU_Feedback->Annot( wn, FB_EDGE_BRANCH_NOT_TAKEN, mid_freq);
    }
  ...
  if ( mid > left ) {
    ...
    WN_INSERT_BlockAfter ( block, WN_last(block), wn );
    VHO_Switch_Generate_Binary_Search ( left, mid - 1, block, left_default );
  }
  ...
  if ( mid < right ) {
    ...
    WN_INSERT_BlockAfter ( block, WN_last(block), wn );
    VHO_Switch_Generate_Binary_Search ( mid + 1, right, block, right_default );
  }
} /* VHO_Switch_Generate_Binary_Search */


/*
 * Are two labels really equivalent, i.e. are the in the same block separated only by labels
 *
 */
typedef HASH_TABLE<INT, INT> LABEL_HASH_TABLE;
static BOOL Equivalent_Labels(LABEL_HASH_TABLE *table, INT label1, INT label2)
{
  INT i =table->Find(label1);;
  INT j =table->Find(label2);;
  return (i && j && i==j);
}

// Map all the switch labels into a unique id
// where two labels share the same id if they are adjacent with no other instruction in between
static void Find_Labels(LABEL_HASH_TABLE *label_table, WN *switch_wn)
{
  int i=1;
  WN *set = WN_next(switch_wn);
  while (set) {
    if (WN_operator(set) == OPR_LABEL) {
      label_table->Enter(WN_label_number(set),i);
    } else {
      i++;
    }
    set = WN_next(set);
...
VHO_Lower_Switch ( WN * wn )
{
  ...
  LABEL_HASH_TABLE *label_table = CXX_NEW(LABEL_HASH_TABLE(200, MEM_local_pool_ptr), MEM_local_pool_ptr);
  Find_Labels(label_table,wn);
  ...
  VHO_Switch_LT_Opcode  = OPCODE_make_op ( OPR_LT,
                                           MTYPE_I4,
                                           Promoted_Mtype [mtype] );
  VHO_Switch_LE_Opcode  = OPCODE_make_op ( OPR_LE,
                                           MTYPE_I4,
                                           Promoted_Mtype [mtype] );
  VHO_Switch_GT_Opcode  = OPCODE_make_op ( OPR_GT,
                                           MTYPE_I4,
                                           Promoted_Mtype [mtype] );
  VHO_Switch_GE_Opcode  = OPCODE_make_op ( OPR_GE,
                                           MTYPE_I4,
                                           Promoted_Mtype [mtype] );
...
#ifdef VHO_DEBUG
    VHO_Switch_Format = "  [%2d] lowvalue = %" LLD_FMT ", highvalue = %" LLD_FMT ", label = %d\n";
#endif /* VHO_DEBUG */
...
#ifdef VHO_DEBUG
    VHO_Switch_Format = "  [%2d] lowvalue = %" LLU_FMT ", highvalue = %" LLU_FMT ", label = %d\n";
#endif /* VHO_DEBUG */
  ...
  for ( VHO_Switch_Ncases = 0, case_goto = WN_first(block);
    ...
    if ( VHO_Switch_Debug )
      fprintf ( TFile, VHO_Switch_Format, VHO_Switch_Ncases,
		WN_const_val(case_goto), WN_const_val(case_goto),WN_label_number(case_goto) );
#endif /* VHO_DEBUG */
  ...
  for ( i = 0, case_goto = WN_first(block);
    ...
    VHO_SWITCH_low_val(i) = VHO_SWITCH_high_val(i) = VHO_SWITCH_wn(i);
    if ( Cur_PU_Feedback )
      VHO_SWITCH_freq(i) = Cur_PU_Feedback->Query( wn, FB_EDGE_SWITCH( i ) );
  }


  // group together ranges that jump to the same label
  qsort ( VHO_Switch_Case_Table, VHO_Switch_Ncases,
          sizeof(VHO_SWITCH_ITEM), VHO_Switch_Compare_Value );

  i = 0; j=0;
  if (VHO_Switch_Ncases) {
    while (i <VHO_Switch_Ncases) {
      VHO_SWITCH_wn(j) = VHO_SWITCH_wn(i);
      VHO_SWITCH_low_val(j) = VHO_SWITCH_high_val(j) = VHO_SWITCH_wn(j);
      WN *wn = VHO_SWITCH_wn(j);
      i++;
      while (i < VHO_Switch_Ncases && 
             Equivalent_Labels(label_table, WN_label_number(wn), WN_label_number(VHO_SWITCH_wn(i))) &&
	     SWITCH_high_key(j) == SWITCH_key(i) - 1) {
	  VHO_SWITCH_high_val(j) = VHO_SWITCH_wn(i);
	  VHO_SWITCH_freq(j) += VHO_SWITCH_freq(i);
	  i++;
      }
      if (i < VHO_Switch_Ncases) {
        j++;
      }
    }
    VHO_Switch_Ncases=j+1;
  }
  ...
  if ( VHO_Switch_Debug ) {
    ...
    fprintf ( TFile, "SWITCH TABLE after grouping\n" );

    for ( i = 0; i < VHO_Switch_Ncases; i++ ) {

      WN *case_goto = VHO_SWITCH_wn(i);
      fprintf ( TFile, VHO_Switch_Format, i,
                WN_const_val(VHO_SWITCH_low_val(i)), WN_const_val(VHO_SWITCH_high_val(i)), WN_label_number(case_goto) );
    }
  }
#endif /* VHO_DEBUG */
  ...
  if ( VHO_Switch_Ncases <= VHO_Switch_If_Else_Limit ) {
    ...
    block = WN_CreateBlock();
    wn = VHO_Switch_Generate_If_Else ( srcpos, block );
  }

  else {

    if ((VHO_Switch_Signed  &&
          (((SWITCH_high_key(VHO_Switch_Ncases-1) - 
            SWITCH_low_key(0)) / VHO_Switch_Ncases) < VHO_Switch_Density)) ||
        (!VHO_Switch_Signed &&
          ((((UINT64) SWITCH_high_key(VHO_Switch_Ncases-1) - 
            (UINT64) SWITCH_low_key(0)) / VHO_Switch_Ncases) < VHO_Switch_Density))) {
  ...
  CXX_DELETE(label_table,MEM_local_pool_ptr);
  ...
  if ( comma_block ) {
    ...
    if (call) {
      ...
      if (desc == MTYPE_M) {
        ...
        const char *name = preg_name ?: vho_lower_comma_name;
        PREG_NUM preg    = Create_Preg (rtype, name);
...
vho_lower_cselect ( WN * wn_cselect, WN * block, BOOL_INFO * bool_info )
{
  ...
  // the case for MCSELECT requires a test result
  // see PR11151
  ...
  if ( !vcselect ) {
    ...
    if (MTYPE_is_tie(preg_mtype) || MTYPE_is_xtbool(preg_mtype))
      opcode = OPCODE_make_op ( OPR_LDID, preg_mtype, preg_mtype );
    else
      opcode = OPCODE_make_op ( OPR_LDID, Promoted_Mtype [preg_mtype],
                              preg_mtype );
    wn = WN_CreateLdid ( opcode, preg, preg_st, preg_ty_idx );
...
*/
    ...
    case OPR_COMMA:
      ...
      wn = vho_lower_comma ( wn, block, bool_info, preg_name );
/*
      comma_block = vho_lower_block (WN_kid0(wn));
      WN_INSERT_BlockLast ( block, comma_block );
...
*/
    ...
    case OPR_RROTATE:
    {
      ...
      WN* rwn = WN_Lshr (rtype, WN_COPY_Tree (wn0), WN_COPY_Tree(wn1));
      WN* lwn = WN_Shl  (rtype, wn0,
		  WN_Sub(rtype, WN_Intconst(MTYPE_I4, size), wn1));
      wn  = WN_Bior (rtype, lwn, rwn);
      break;
    }
...
vho_lower_stid ( WN * wn, WN * block )
{
  ...
  /* If possible, use the LHS variable/register name for any
     pseudo-registers created for the RHS if possible. */
  ...
  WN_kid0(wn) = vho_lower_expr ( WN_kid0(wn), block, NULL, name );
  ...
  switch ( WN_operator(wn) ) {
    ...
    case OPR_ISTORE:
      ...
      if (    VHO_Istore_Opt
	   ...
	   && ST_class(WN_st_idx(WN_kid1(wn))) != CLASS_FUNC
#endif
	 ) {
...
static void
rename_tie_branch_test_result(WN* copy)
{

  if (WN_opcode(copy)!=OPC_I4COMMA)
    return;

  WN* block = WN_kid0(copy);
  if (WN_opcode(block)==OPC_BLOCK) {
    WN* last = WN_last(block);
    if (last==NULL)
      return;

    WN* intr = WN_prev(last);
    if (intr &&
	WN_opcode(intr)==OPC_I4INTRINSIC_CALL &&
	INTRN_is_tie_intrinsic(WN_intrinsic(intr)) &&
	WN_opcode(last)==OPC_I4STID) {

      TIE_MACRO_ID tie_macro_id =
		Intrinsic_To_Tie_Macro_Id(WN_intrinsic(intr));
	...
	return;
      ...
      PREG_NUM preg = Create_Preg (MTYPE_I4, "renamed_tie_branch_result");
      ST* preg_st     = MTYPE_To_PREG ( MTYPE_I4 );
      TY_IDX preg_ty_idx = WN_ty(last);
      WN* new_stid =
	WN_CreateStid ( OPC_I4STID, preg, preg_st, preg_ty_idx, WN_kid0(last) );
      WN_INSERT_BlockBefore ( block, last, new_stid );
      WN* new_ldid =
          WN_CreateLdid ( OPC_I4I4LDID, preg, preg_st, preg_ty_idx );
      WN_kid0(last)=new_ldid;
    }
  }
}
...
vho_lower_while_do ( WN * wn, WN *block )
{
  ...
  rename_tie_branch_test_result(copy);
...
vho_lower_if ( WN * wn, WN *block )
{
  ...
  if ( WN_operator(WN_if_test(wn)) == OPR_RCOMMA ) {
    ...
    // the lowering of then and else has to be done after the lowering of
    // the if test or the debug line number for if test is wrong
