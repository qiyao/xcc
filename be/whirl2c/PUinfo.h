 ...
 * Module: PUinfo.h
 * $Revision: 1.28 $
 * $Date: 2000/04/06 02:20:20 $
 * $Author: mtibuild $
 * $Source: /isms/cmplrs.src/osprey1.0/be/whirl2c/RCS/PUinfo.h,v $
 *
 * Revision history:
...
struct CallSite {
  ...
  const WN   *store[MAX_NUMBER_OF_REGISTERS_FOR_RETURN];  /* Stores from return registers */
...
#define CALLSITE_store(ci,i) (ci)->store[i]
...
extern CALLSITE *PUinfo_find_callsite (const CALLSITE *callsite_list, const WN *call_wn);
...
{
   const WN    *return_wn;
   const WN    *store[MAX_NUMBER_OF_REGISTERS_FOR_RETURN];  /* Stores into return registers */
...
#define RETURNSITE_store(ci,i) (ci)->store[i]
...
typedef struct RETURN_PREG
{
   ...
   MTYPE       mtype[MAX_NUMBER_OF_REGISTERS_FOR_RETURN];   /* Mtypes for the return pregs (or MTYPE_V) */
   STAB_OFFSET offset[MAX_NUMBER_OF_REGISTERS_FOR_RETURN];  /* The preg-numbers for the return pregs */
} RETURN_PREG;
...
#define PUINFO_RETURN_CHARACTER Func_Return_Character(PUINFO_FUNC_TY)
        ...
        /*----- Information about TIE intrinsic ops -----*/
        /*-----------------------------------------------*/

typedef struct TieOpSite TIEOPSITE;
struct TieOpSite {
  const WN   *iop_wn;        /* the special TIE intrinsic op whirl node */
  const WN   *stmt;          /* the statement containing the TIE op */
  WN   *setup_block;   /* the setup block created for the TIE op */
  WN   *ldid_new_preg; /* LDID whirl node from the new data register */
  TIEOPSITE  *next;
};

#define TIEOPSITE_iop_wn(oi) (oi)->iop_wn
#define TIEOPSITE_stmt(oi) (oi)->stmt
#define TIEOPSITE_setup_block(oi) (oi)->setup_block
#define TIEOPSITE_ldid_new_preg(oi) (oi)->ldid_new_preg
#define TIEOPSITE_next(oi) (oi)->next

typedef struct WN_list WN_LIST;
struct WN_list {
  const WN *wn;
  WN_LIST *next;
};

#define WN_LIST_wn(i) (i)->wn
#define WN_LIST_next(i) (i)->next
...
extern TIEOPSITE *PUinfo_Get_TieOpSites(void);
...
extern WN *PUinfo_Packed_Tie_Preg_Outpart (INT16 preg_num, INT outpart_idx);

extern RETURN_PREG PUinfo_Get_ReturnPreg(TY_IDX return_ty, Return_Info_Dir dir);
