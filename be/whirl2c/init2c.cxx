 ...
 * Module: init2c.c
 * $Revision: 1.28 $
 * $Date: 2000/04/06 02:20:30 $
 * $Author: mtibuild $
 * $Source: /isms/cmplrs.src/osprey1.0/be/whirl2c/RCS/init2c.cxx,v $
 *
 * Revision history:
 ...
 */
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/whirl2c/RCS/init2c.cxx,v $ $Revision: 1.28 $";
#endif /* _KEEP_RCS_ID */
	       ...
	       INITV_IDX    initv)
{
   ...
   else if (INITV_ofst(Initv_Table[initv]) % TY_size(sym_ty) == 0)
   {
      ...
      Number_as_String(INITV_ofst(Initv_Table[initv])/TY_size(sym_ty), "%" LLD_FMT ""));
      Append_Token_Special(tmp_tokens, ']');
   }
   ...
   while (*current_offset < desired_offset && *current_inv != 0)
   {
	 ...
	 for (pad_byte = 0; pad_byte < tcon_size; pad_byte++)
	 {
	    ...
	    Append_Token_String(tokens, 
	       Number_as_String(tcon_bytes.byte[pad_byte], "%" LLU_FMT ""));
	 }
...
static void
INITV2C_non_block_aggregrate(TOKEN_BUFFER tokens,
			     TY_IDX       ty, 
			     INITV_IDX    first_initv)
{
   INT	initv_repeat = INITV_repeat(first_initv);
   INITV_IDX initv = first_initv;
   TCON	tcon;
   ...
   do {
     ...
     if (INITV_kind(Initv_Table[initv]) != INITVKIND_PAD) {

       /* Avoid preceeding the first initializer with a comma */
       if (initv != first_initv || initv_repeat < INITV_repeat(initv))
	 Append_Token_Special(tokens, ',');
       
       if (INITV_kind(Initv_Table[initv]) == INITVKIND_SYMOFF) {
	   // TODO: Here we make the assumption that the LHS type
	   // is a pointer to the RHS (initv) symbol. Ideally, we should keep
	   // track of the current field (for structure types) to determine the
	   // correct LHS type and output a typecast if necessary.
	   ST_IDX initv_st = INITV_st(Initv_Table[initv]);
	   TY_IDX sym_ty = (ST_class(initv_st) == CLASS_FUNC) ? 
	     ST_pu_type(initv_st) : ST_type(initv_st);
	   INITV2C_symoff(tokens, Stab_Pointer_To(sym_ty), initv);
       } else {
         tcon = TCON_For_Initv(initv);
         /* Translate the constant value */
         TCON2C_translate(tokens, tcon);
       }
     ...
     initv_repeat =  (initv != 0) ? INITV_repeat(initv) : 0;
     INIT2C_Next_Initv(&initv, &initv_repeat);

   } while (initv);

   Append_Token_Special(tokens, '}');

} /* INITV2C_non_block_aggregrate */
		  ...
		  INITV_IDX    initv)
{
   ...
   case INITVKIND_SYMOFF:
      /* although there is no block, these can actually be the initv
       * for an aggregate type because the gcc front end does not
       * generate block for them for some cases (e.g., file global int array)
       */
      if (TY_Is_Array(ty) || TY_Is_Union(ty) ||  TY_Is_Struct(ty))
        INITV2C_non_block_aggregrate(tokens, ty, initv);
      else
