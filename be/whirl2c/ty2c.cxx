 ...
 * Module: ty2c.c
 * $Revision: 1.48 $
 * $Date: 2000/04/06 02:20:56 $
 * $Author: mtibuild $
 * $Source: /isms/cmplrs.src/osprey1.0/be/whirl2c/RCS/ty2c.cxx,v $
 *
 * Revision history:
 ...
 */
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/whirl2c/RCS/ty2c.cxx,v $ $Revision: 1.48 $";
#endif /* _KEEP_RCS_ID */
...
TY2C_Prepend_Alignment_Type(TOKEN_BUFFER tokens, INT64 align)
{
   ...
   default:
      Is_True(FALSE, 
	      ("Unexpected alignment (%" LLD_FMT ") in TY2C_Prepend_Alignment_Type()",
	       align));
   }
...
TY2C_scalar(TOKEN_BUFFER decl_tokens, TY_IDX ty, CONTEXT context)
{
	...
	/* a TIE type */
      const char * name = tie_info->get_demangled_ctype_name(Mtype_To_Tie_Type_Id(mtype));
      Prepend_Token_String(decl_tokens, name);
    } else {
...
TY2C_array(TOKEN_BUFFER decl_tokens, TY_IDX ty, CONTEXT context)
{
   if (Stab_Array_Has_Dynamic_Bounds(ty))
   {
       ...
       *      ("Expected size of dynamic array to be zero (size=%" LLU_FMT ")",
       *       (UINT64)TY_size(ty)));
       */
