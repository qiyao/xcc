 ...
 * $Revision: 1.26 $
 * $Date: 2000/04/06 02:21:11 $
 * $Author: mtibuild $
 * $Source: /isms/cmplrs.src/osprey1.0/be/whirl2c/RCS/w2cf_symtab.cxx,v $
 *
 * Revision history:
 ...
 */
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/whirl2c/RCS/w2cf_symtab.cxx,v $ $Revision: 1.26 $";
#endif /* _KEEP_RCS_ID */
...
W2CF_Identical_Symkinds(W2CF_SYMBOL *sym1, W2CF_SYMBOL *sym2)
{
   ...
   if (identical/*kinds*/)
   {
	 ...
	 break;
      case SYMKIND_TY:
	 identical = TY_IDX_index(W2CF_SYMBOL_ty(sym1)) == TY_IDX_index(W2CF_SYMBOL_ty(sym2));
	 break;
      case SYMKIND_ST:
...
W2CF_Symtab_Nameof_Preg(const TY_IDX preg_ty, PREG_NUM preg_num)
{
    ...
    * name appended to the suffix (hence the "+32").
    */
   valid_name = (!Preg_Is_Dedicated(preg_num)) 
...
W2CF_Dump_Symhdr(W2CF_SYMHDR *symhdr,W2CF_SYMTAB *symtab)
{
  ...
  if (symhdr != 0) 
  {
    printf ("symhdr: 0x%x, hashval=0x%" LLX_FMT ", next_symid=%d, next_symhdr=0x%x \n",
            symhdr,
...
W2CF_Dump_Symtab(W2CF_SYMTAB *symtab)
{
  if (symtab != 0) 
  {
    printf ("symtab: 0x%x, label=%d, num=%d, hash_tbl=0x%x, up=0x%x, down=0x%x next=0x%x\n",  
            symtab,
    ...
    printf ("         strbuf: size=%d, next=0x%x chars=0x%x\n",
...
W2CF_Dump_Symbol(W2CF_SYMBOL *sym,W2CF_SYMTAB *symtab)
{
  if (sym != NULL) 
  {
    ...
    printf ("    symbol: 0x%x, id=%d, next=0x%x, str_idx 0x%d",
	    sym,
   ...
   case SYMKIND_ST:
   {
     ...
     printf("%s ST 0x%x %s",hdr,st,ST_name(st));
     break;
   }
   case SYMKIND_PREG:
   {
