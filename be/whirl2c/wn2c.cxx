 ...
 * Module: wn2c.c
 * $Revision: 1.157 $
 * $Date: 2000/04/06 02:21:26 $
 * $Author: mtibuild $
 * $Source: /isms/cmplrs.src/osprey1.0/be/whirl2c/RCS/wn2c.cxx,v $
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/whirl2c/RCS/wn2c.cxx,v $ $Revision: 1.157 $";
#endif /* _KEEP_RCS_ID */
...
static const TIEOPSITE *WN2C_Next_TieOpSite_Op = NULL;
static const TIEOPSITE *WN2C_Next_TieOpSite_Stmt = NULL;

static BOOL
Is_Stmt_ReturnSite_Store(const WN *stmt, const RETURNSITE *returnsite)
{
  for (INT i = 0; i < MAX_NUMBER_OF_REGISTERS_FOR_RETURN; i++)
    if (stmt == RETURNSITE_store(returnsite, i))
      return TRUE;
...
WN2C_Skip_Stmt(const WN *stmt)
{
   return ((W2C_No_Pragmas && \
	   ...
	   (WN2C_Next_ReturnSite != NULL &&
	    Is_Stmt_ReturnSite_Store(stmt, WN2C_Next_ReturnSite)) ||

	   (WN2C_Prev_CallSite != NULL &&
            Is_Stmt_CallSite_Store(stmt, WN2C_Prev_CallSite)) ||
...
static STATUS WN2C_outpart(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context);

// TIE intrinsic call handling

#define WN_has_offset(x) (OPCODE_has_offset(WN_opcode(x)))

static STATUS
WN2C_translate_tie_intrinsic(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context);

static INT tie_intrinsic_stid_skip = 0;
	      ...
	      CONTEXT      context)
{
   ...
   CONTEXT_reset_needs_lvalue(context); /* needs the actual value of operands */
		...
		CONTEXT      context)
{
   ...
   BOOL bit_op = OPCODE_operator(opcode)==OPR_EXTRACT_BITS ||
     OPCODE_operator(opcode)==OPR_COMPOSE_BITS;
   ...
   // handle bit extraction operators
   if (bit_op) {
     INT bit_offset = WN_bit_offset(wn);
     INT bit_size = WN_bit_size(wn);
     Append_Token_Special(tokens, ',');
     TCON2C_translate(tokens,Host_To_Targ(MTYPE_I4,bit_offset));
     Append_Token_Special(tokens, ',');
     TCON2C_translate(tokens,Host_To_Targ(MTYPE_I4,bit_size));
   }
		    ...
		    STAB_OFFSET offset)
{
    ...
    * the base_ty not be a struct or a tie type and load_ty not be
    * a tie type, then just return the base_ty;
    *
   ...
   if (TY_Is_Array(base_ty))
   {
      ...
      return load_ty;
	     ...
	     !MTYPE_is_tie(TY_mtype(base_ty)) &&
	     !MTYPE_is_tie(TY_mtype(load_ty))) ||
	       ...
	       CONTEXT      context)
{
   ...
   else
   {
	 ...
	 addr_ty = Stab_Pointer_To(ST_type(st));
	  ...
	  TY_kind(ST_type(st)) == KIND_SCALAR &&
	  addr_offset == (STAB_OFFSET) 0 &&
	  WN2C_assignment_compatible_types(object_ty,ST_type(st))) {
	// process simple scalar variables without assuming that
	// they reside in memory
	if (!WN2C_compatible_lvalues(object_ty,ST_type(st))) {
...
WN2C_Append_Symtab_Types(TOKEN_BUFFER tokens, UINT lines_between_decls)
{
   ...
   for (INT i = 1; i < TY_Table_Size(); i++)
   {
     TY_IDX ty = make_TY_IDX(i);
     Set_TY_align(ty,1<<31); // maximum alignment for the structure
		       ...
		       CONTEXT      context)
{
   ...
   RETURN_PREG return_info = PUinfo_Get_ReturnPreg(return_ty,Return_Info_Outgoing);
   ...
   else /* Return through a preg; the usual case */
   {
      ...
      const WN *result_store = CALLSITE_store(WN2C_Prev_CallSite, 0);
	 ...
	 Is_True(WN_operator(result_store) == OPR_ISTORE &&
		 ...
		 ("Unexpected result_store in WN2C_Function_Call_Lhs()"));
			   ...
			   CONTEXT context)
{
   ...
   else /* Return through a preg; the usual case */
   {
      ...
      const WN *result_store = RETURNSITE_store(WN2C_Next_ReturnSite, 0);
	 ...
	 Is_True(WN_operator(result_store) == OPR_STID, 
		 ("Unexpected result_store in WN2C_Function_Return_Value()"));
			     ...
			     CONTEXT       context)
{
   ...
   while (stmt) {
     ...
     // If a TIE intrinsic call has been recently translated, then
     // tie_intrinsic_stid_skip is the number of STID statements following
     // the call that need to be skipped.
     if (!CONTEXT_tie_intrinsic(context) && tie_intrinsic_stid_skip>0) {
       tie_intrinsic_stid_skip--;
       stmt = WN_next(stmt);
       continue;
     }

     // If there are any special TIE intrinsic operations associated with this
     // statement, process their setup block first
     if (WN2C_Next_TieOpSite_Stmt &&
	 TIEOPSITE_stmt(WN2C_Next_TieOpSite_Stmt) == stmt)
       {
	 if (TIEOPSITE_ldid_new_preg(WN2C_Next_TieOpSite_Stmt) == NULL)
	   next_stmt = WN_next(stmt);
	 else
	   next_stmt = stmt;
	 stmt = TIEOPSITE_setup_block(WN2C_Next_TieOpSite_Stmt);
	 WN2C_Next_TieOpSite_Stmt = TIEOPSITE_next(WN2C_Next_TieOpSite_Stmt);
	 CONTEXT_set_tie_intrinsic_setup(context);
       }
     else
       next_stmt = WN_next(stmt);
	     ...
	     WN_operator(stmt) != OPR_BACKWARD_BARRIER)
	 {
	   ...
	   Append_Token_String(tokens, Number_as_String(freq,"%" LLD_FMT ""));
...
WN2C_Append_Purple_Funcinfo(TOKEN_BUFFER tokens)
{
   ...
   Append_Token_String(tokens, Number_as_String(id, "%" LLU_FMT ""));
   Append_Token_Special(tokens, ',');
   Append_Token_String(tokens, Number_as_String(sclass, "%" LLD_FMT ""));
   Append_Token_Special(tokens, ',');
   Append_Token_String(tokens, Number_as_String(export_class, "%" LLD_FMT ""));
   Append_Token_Special(tokens, ',');
	 ...
	 Concat2_Strings(    info_str,
          Concat2_Strings(   "1st <", 
           Concat2_Strings(  Ptr_as_String(pfptr->wn_pref_1L),
            Concat2_Strings( ", ",
             Concat2_Strings(Number_as_String(pfptr->lrnum_1L,"%" LLD_FMT ""),
			     "> ")))));
   }
   if (pfptr->wn_pref_2L)
   {
	 ...
	 Concat2_Strings(    info_str,
          Concat2_Strings(   "2nd <", 
           Concat2_Strings(  Ptr_as_String(pfptr->wn_pref_2L),
            Concat2_Strings( ", ",
             Concat2_Strings(Number_as_String(pfptr->lrnum_2L,"%" LLD_FMT ""),
			     "> ")))));
   }
...
WN2C_Prompf_Construct_Start(TOKEN_BUFFER tokens, const WN *construct)
{
   ...
   if (construct_id != 0)
   {
      ...
      Append_Token_String(tokens, Number_as_String(construct_id, "%" LLU_FMT ""));
      Append_Token_String(tokens, "*/");
   }
...
{
  DevWarn("==> wn2c cannot handle operator <%s> (%d): construct ignored!",
...
WN2C_binaryop(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   Is_True(WN_operator(wn) == OPR_COMPOSE_BITS || WN_kid_count(wn) == 2, 
...
{
   Is_True(WN_operator(wn) == OPR_EXTRACT_BITS || WN_kid_count(wn) == 1,
	   ("Expected 1 kid in WN2C_unaryop()"));
   ...
   if (WN2C_IS_INFIX_OP(WN_opcode(wn)))
      ...
      WN2C_funcall_op(tokens, WN_opcode(wn), NULL, WN_kid0(wn), wn, context);
   else
...
WN2C_func_entry(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   WN2C_Prev_CallSite = NULL;
   WN2C_Next_TieOpSite_Stmt = PUinfo_Get_TieOpSites();
   WN2C_Next_TieOpSite_Op = PUinfo_Get_TieOpSites();
   ...
   WN2C_Next_ReturnSite = NULL;
   WN2C_Used_Return_Value = FALSE;
   WN2C_Next_TieOpSite_Stmt = NULL;
   WN2C_Next_TieOpSite_Op = NULL;
...
WN2C_compgoto(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (switch_mty == MTYPE_M) {
     switch_mty = WN_opc_rtype(WN_compgoto_idx(wn));
   }
...
WN2C_do_loop(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (W2C_Emit_Cgtag && loop_info != NULL)
   {
      ...
      Append_Token_String(tokens, Number_as_String((INTPTR)loop_info, "%ull"));
      Append_Token_String(tokens, "*/");
      Append_Indented_Newline(tokens, 1);
   }
...
WN2C_istore(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   TY_IDX wn_ty = WN_Tree_Type(wn);
   
   Is_True(WN_operator(wn) != OPR_ISTORE || TY_Is_Pointer(wn_ty),
   ...
   */
   ...
   else
   {
      WN2C_memref_lhs(lhs_tokens, 
		      ...
		      WN_store_offset(wn), /* offset from this address */
		      wn_ty,               /* ref type for stored object */
		      TY_pointed(wn_ty),   /* type for stored object */
		      WN_opc_dtype(wn),    /* base-type for stored object */
		      context);
   }
...
WN2C_stid(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   // Skip any TIE packed types...
   if (MTYPE_is_tie_packed(WN_desc(wn)))
       return EMPTY_STATUS;
...
WN2C_call(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   // Handle TIE intrinsic calls separately. This is necessary because of the
   // special copy-in/copy-out semantics of TIE macros and corresponding
   // WHIRL code.
   if (WN_operator(wn) == OPR_INTRINSIC_CALL &&
       INTRN_is_tie_intrinsic((INTRINSIC)WN_intrinsic(wn)))
     is_tie_intrinsic = TRUE;

   if (is_tie_intrinsic &&
       !CONTEXT_tie_intrinsic(context) &&
       !CONTEXT_tie_intrinsic_setup(context))
     return WN2C_translate_tie_intrinsic(tokens,wn,context);
   ...
   else /* OPR_INTRINSIC_CALL */
   {
      ...
      if (is_tie_intrinsic &&
	  !CONTEXT_tie_intrinsic_setup(context)) {
	// in a tie_intrinsic_setup context there is no information about
	// this intrinsic call in the callsite list.
	// return_ty should be set to VOID in the ELSE branch

	CALLSITE *callsite;
	if (WN2C_Prev_CallSite == NULL)
	  callsite = PUinfo_find_callsite(PUinfo_Get_CallSites(),wn);
	else
	  callsite = PUinfo_find_callsite(WN2C_Prev_CallSite,wn);
	Is_True(callsite && CALLSITE_call(callsite) == wn,
		("Can't find the call site"));
	func_ty = (TY_IDX) 0;
	return_ty = CALLSITE_return_ty(callsite);
	return_to_param = FALSE;
	last_arg_idx = WN_kid_count(wn) - 1;

	// figure out if the first argument is a dummy
   ...
   // in a tie_intrinsic_setup context there is no information about
   // this intrinsic call in the callsite list
   // but the return type should be VOID so no special processing is
   // required
   if (!CONTEXT_tie_intrinsic_setup(context)) {
...
WN2C_prefetch(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (WN_operator(wn) == OPR_PREFETCH)
   {
	 ...
	 Concat3_Strings("/* PREFETCH(", Ptr_as_String(wn), ")"));
      (void)WN2C_translate(tokens, WN_kid0(wn), context);
      Append_Token_String(tokens, 
	 Concat2_Strings("OFFS=", Number_as_String(WN_offset(wn), "%" LLD_FMT "")));
   }
   ...
   Append_Token_String(tokens, 
      Concat2_Strings("strid1=", 
		      Number_as_String(PF_GET_STRIDE_1L(pflag), "%" LLD_FMT "")));
   Append_Token_String(tokens, 
      Concat2_Strings("strid2=", 
		      Number_as_String(PF_GET_STRIDE_2L(pflag), "%" LLD_FMT "")));
   Append_Token_String(tokens, 
      Concat2_Strings("conf=", 
		      Number_as_String(PF_GET_CONFIDENCE(pflag), "%" LLD_FMT ""))); 
   Set_Current_Indentation(Current_Indentation()-3); 
...
WN2C_iload(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (W2C_Only_Mark_Loads && !TY_Is_Pointer(wn_ty))
   {
      char buf[64];
      sprintf(buf, "#<%p>#", wn);
   ...
   else
   {
     ...
     TY_IDX ty = WN_ty(wn);
     if (ty!=wn_ty && TY_Is_Structured(ty) && TY_Is_Pointer(load_addr_ty)
	 && TY_pointed(load_addr_ty)==ty) {
       // structure fields
       load_addr_ty = Stab_Pointer_To(wn_ty);
     }
      WN2C_memref_lhs(expr_tokens, 
		      &loaded_ty,
		      WN_kid0(wn),         /* address */
		      WN_load_offset(wn),  /* offset from this address */
		      load_addr_ty,        /* ref type for loaded object */
		      wn_ty,               /* type for loaded object */
		      WN_opc_dtype(wn),    /* base-type for stored object */
		      context);
    ...
    */
   if (!WN2C_arithmetic_compatible_types(loaded_ty, wn_ty)) {
      WN2C_prepend_cast(expr_tokens, wn_ty, FALSE/*pointer_to_type*/);
   }
...
WN2C_array(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   TY_IDX       ety, ety_orig;     /* Type of element indexed */
   ...
   else
   {
      ...
      ety_orig = TY_AR_etype(TY_pointed(base_ty));
      ety = ety_orig;
      
      // A hack to undo array flattening done in LNO
      dim = TY_AR_ndims(TY_pointed(base_ty));
      if (dim == 1 && WN_num_dim(wn)>1) {
	while (dim < WN_num_dim(wn)) {
	  if (TY_Is_Array(ety)) {
	    dim += TY_AR_ndims(ety);
	    ety = TY_AR_etype(ety);
	  } else {
...
WN2C_intrinsic_op(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (WN2C_Next_TieOpSite_Op &&
       TIEOPSITE_iop_wn(WN2C_Next_TieOpSite_Op)==wn) {
     WN2C_translate(tokens,TIEOPSITE_ldid_new_preg(WN2C_Next_TieOpSite_Op),context);
     WN2C_Next_TieOpSite_Op=TIEOPSITE_next(WN2C_Next_TieOpSite_Op);
   } else {
     /* A buffer to hold the tokens representing the function call */
     call_tokens = New_Token_Buffer();
     Append_Token_String(call_tokens,
...
WN2C_ldid(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   expr_tokens = New_Token_Buffer();   
   TY_IDX load_ty = WN_Tree_Type(wn);
   if (ST_sym_class(WN_st(wn)) == CLASS_PREG)
   {
	 ...
	 case MTYPE_CQ:
            sprintf(buffer, "reg%d",
		    Dedicated_Preg_First_Incoming_Ret(TI_ISA_Regclass_Float()));
	    Append_Token_String(expr_tokens, buffer); 
	    break;
	 case MTYPE_M: 
    ...
    */
   if (!WN2C_arithmetic_compatible_types(object_ty, load_ty))
   {
      
      if (!TY_Is_Structured(object_ty) && !TY_Is_Structured(load_ty))
...
static STATUS
WN2C_outpart(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
  Is_True(WN_kid_count(wn) == 1, ("Expected one kid for OPR_OUTPART"));
  
  WN *kid = WN_kid0(wn);
  FmtAssert(WN_operator(kid) == OPR_LDID &&
	    ST_sym_class(WN_st(kid)) == CLASS_PREG,
	    ("Unhandled TIE op in whirl2c."));
  
  /* OPR_OUTPART is used to extract the scalar values in a packed TIE type.
     We should have already preprocessed all TIE ops with packed result types
     and assigned a proper symbol to each OUTPART. Now we just look them up
     in the map. */
  
  PREG_NUM packed_preg = WN_load_offset(kid);
  WN *outpart = PUinfo_Packed_Tie_Preg_Outpart(packed_preg, WN_outpart((WN *)wn));
  FmtAssert(outpart != NULL, ("Can't handle packed output type TIE ops"));
  
  return WN2C_translate(tokens, outpart, context);
}
...
// This function handles TIE intrinsic calls. It may replace the call with
// a block of statements implementing the copy-in/copy-out semantics of the
// TIE macro. It also sets the tie_intrinsic_stid_skip variable to the correct
// number of STID statements following the call that need to be skipped.
...
WN2C_translate_tie_intrinsic(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context) {
  ...
  Is_True(OPCODE_is_stmt(WN_opcode(wn)),
	  ("expected statement node, not %s", OPCODE_name(WN_opcode(wn))));
  Is_True(WN_operator(wn)==OPR_INTRINSIC_CALL,
	  ("expected intrinsic call, not %s", OPCODE_name(WN_opcode(wn))));
  INTRINSIC id = (INTRINSIC) WN_intrinsic(wn);
  Is_True(INTRN_is_tie_intrinsic(id),
	  ("expected TIE intrinsic call, not %s",OPCODE_name(WN_opcode(wn))));

  CONTEXT_set_tie_intrinsic(context);
  tie_intrinsic_stid_skip = 0;
  
  TIE_MACRO_ID tie_macro_id = Intrinsic_To_Tie_Macro_Id(id);
  TIE_MACRO_p tie_macro = tie_info->tie_macro(tie_macro_id);
  
  if (tie_macro->no_output() || // no output -- nothing special
      tie_macro->is_c_function()) // C function -- whirl2c will handle it
    return_status = WN2C_translate(tokens,wn,context);
  else {
    WN *new_block = WN_CreateBlock();
    WN *copy_wn = WN_COPY_Tree((WN *)wn);
    WN_INSERT_BlockLast(new_block,copy_wn);
    
    WN *return_store = NULL;
    BOOL translate_new_block = FALSE;

    WN* store = WN_next(wn);
    if (tie_macro->is_conditional_branch()) { // the return value
      tie_intrinsic_stid_skip++;
      return_store = WN_COPY_Tree(store);
      WN_INSERT_BlockLast(new_block,return_store);
      store = WN_next(store);
    }
	...
	continue;
      ...
      FmtAssert(store,
		("Internal error: missing copy-out after TIE intrinsic call"));
      
      WN* return_ldid = WN_kid0(store);
      TYPE_ID proto_mtype = tie_macro->proto_mtype_id(tie_info,i);

      FmtAssert(WN_operator(store)==OPR_STID ||
                WN_operator(store)==OPR_ISTORE,
		("Internal error: missing STID after TIE intrinsic call"));
      if (MTYPE_is_tie(WN_desc(store)) || MTYPE_is_tie(proto_mtype))
        FmtAssert(WN_desc(store)==proto_mtype,
                  ("Internal error: wrong MTYPE after TIE intrinsic call"));
      ...
      WN* param_ldid = WN_kid0(WN_kid(copy_wn,i));
	...
	// the symbol that the return value is stored to is the same
	// as the one used as an argument, so
	// do nothing
      } else {
	translate_new_block = TRUE;
	...
	if (tie_macro->proto_is_inout(i)) {
	  WN* param_stid =
	    WN_StidIntoPreg(proto_mtype, arg_preg, MTYPE_To_PREG(proto_mtype),
			    param_ldid);
	  WN_INSERT_BlockBefore(new_block,copy_wn,param_stid);
	}

	WN_kid0(WN_kid(copy_wn,i))=WN_LdidPreg(proto_mtype, arg_preg);

	WN *copy_store = WN_COPY_Tree(store);
	WN_DELETE_Tree(WN_kid0(copy_store));
	WN_kid0(copy_store)=WN_LdidPreg(proto_mtype, arg_preg);
	WN_INSERT_BlockLast(new_block,copy_store);
      }
      tie_intrinsic_stid_skip++;
      store = WN_next(store);
    } // for all tie function arguments
    
    if (translate_new_block) {
      // fix the call site list
      CALLSITE *callsite;
      if (WN2C_Prev_CallSite == NULL)
	callsite = PUinfo_find_callsite(PUinfo_Get_CallSites(),wn);
      else
	callsite = PUinfo_find_callsite(WN2C_Prev_CallSite,wn);
      FmtAssert(CALLSITE_call(callsite)==wn,
		("Internal error: wrong call site for TIE intrinsic call"));
      CALLSITE_call(callsite)=copy_wn;
      if (return_store) {
	FmtAssert(CALLSITE_store(callsite, 0),
		  ("Internal error: no return store for TIE intrinsic call"));
	CALLSITE_store(callsite, 0)=return_store;
      } else
	FmtAssert(CALLSITE_store(callsite, 0) == NULL,
		  ("Internal error: invalid return store1 for TIE intrinsic call"));
      FmtAssert(CALLSITE_store(callsite, 1) == NULL,
		("Internal error: invalid return store2 for TIE intrinsic call"));
      
      // translate the new block
      return_status = WN2C_translate(tokens,new_block,context);
    } else {
      // Essentially, no transformation was necessary
      // copy_wn should still represent the same tree as wn
      return_status = WN2C_translate(tokens,wn,context);
    }
    
    WN_DELETE_Tree(new_block); // cleanup
  }
  
  CONTEXT_reset_tie_intrinsic(context);
  return return_status;
} // WN2C_translate_tie_intrinsic
