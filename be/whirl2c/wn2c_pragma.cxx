...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/whirl2c/RCS/wn2c_pragma.cxx,v $ $Revision: 1.37 $";
#endif /* _KEEP_RCS_ID */
...
extern DLL_SHARED BOOL Run_w2fc_early;     /* Defined in be.so */
...
{
   if (val1 != -1)
      Append_Token_String(tokens, Number_as_String(val1, "%" LLD_FMT ""));

   if (val2 != -1)
   {
      Append_Token_Special(tokens, ',');
      Append_Token_String(tokens, Number_as_String(val2, "%" LLD_FMT ""));
   }
} /* Append_Arg_Numbers */
                           ...
                           INT32        size)
{
   ...
   if (PF_GET_STRIDE_1L(pflag) > 0)
   {
      if (PF_GET_STRIDE_2L(pflag) > 0)
      {
         Append_Token_String(tokens, 
            ...
            Concat2_Strings(Number_as_String(PF_GET_STRIDE_1L(pflag), "%" LLD_FMT ""),
            Concat2_Strings(",",
   ...
   if (size > 0)
   {
      Append_Token_Special(tokens, ',');
      Append_Token_String(tokens, 
         Concat2_Strings("size=", Number_as_String(size, "%" LLD_FMT "")));
   }
...
WN2C_process_pragma(TOKEN_BUFFER tokens, const WN **next, CONTEXT context)
{
   ...
   case WN_PRAGMA_SIMD_IF_CONVERT:
      WN2C_Append_Pragma_Newline(tokens, CONTEXT_srcpos(context));
      Append_Token_String(tokens, "simd_if_convert");
      break;
