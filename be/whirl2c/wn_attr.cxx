 ...
 * Module: wn_attr.c
 * $Revision: 1.45 $
 * $Date: 2000/09/14 17:47:27 $
 * $Author: mpm $
 * $Source: /isms/cmplrs.src/osprey1.0/be/whirl2c/RCS/wn_attr.cxx,v $
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/whirl2c/RCS/wn_attr.cxx,v $ $Revision: 1.45 $";
#endif /* _KEEP_RCS_ID */
...
WN_intrinsic_name(INTRINSIC intr_opc)
{
   ...
   else
   {
      ...
      name =
	 Concat3_Strings("<INTR: ", Number_as_String(intr_opc, "%" LLD_FMT ""), ">");
   }
#endif /*BUILD_WHIRL2F*/

   if (name)
     name = tie_info->get_demangled_macro_name(name);
...
WN_intrinsic_return_ty(OPCODE wn_opc, INTRINSIC intr_opc, const WN *call)
{
   ...
   case IRETURN_SZT:               /* size_t */
     ret_ty = Stab_Mtype_To_Ty(MTYPE_U4);
     break;

   case IRETURN_PC:                /* pointer to char */
     ret_ty = Stab_Pointer_To(Stab_Mtype_To_Ty(MTYPE_U1));
     break;
...
WN_Tree_Type(const WN *wn)
{
   ...
   if (OPCODE_is_expression(WN_opcode(wn)))
   {
	...
	if (TY_Is_Structured(ty) && WN_field_id(wn)>0) {
	  // handle structure fields more intelligently
	  // check if the descriptor type is the same as the field type
	  // and if so, return it as the type of wn
	  UINT field_id = WN_field_id(wn);
	  UINT cur_field_id = 0;
	  FLD_HANDLE fld = FLD_get_to_field(ty,field_id,cur_field_id);
	  if (!fld.Is_Null() && cur_field_id==field_id) {
	    TY_IDX desc_ty = Stab_Mtype_To_Ty(WN_desc(wn));
	    if (desc_ty == FLD_type(fld)) {
	      ty = desc_ty;
	    }
	  }
	}
	break;
	 ...
	 {
	   TY_IDX ety_orig = TY_AR_etype(TY_pointed(ty));
	   TY_IDX ety = ety_orig;
	   INT dim = TY_AR_ndims(TY_pointed(ty));
	 ...
	 ty = WN_ty(wn);
	 if (TY_Is_Pointer(ty)) {
	   TY_IDX typ = TY_pointed(ty);
	   if (TY_Is_Structured(typ) && WN_field_id(wn)>0) {
	     // handle structure fields more intelligently
	     // check if the descriptor type is the same as the field type
	     // and if so, return it as the type of wn
	     UINT field_id = WN_field_id(wn);
	     UINT cur_field_id = 0;
	     FLD_HANDLE fld = FLD_get_to_field(typ,field_id,cur_field_id);
	     if (!fld.Is_Null() && cur_field_id==field_id) {
	       TY_IDX desc_ty = Stab_Mtype_To_Ty(WN_desc(wn));
	       if (desc_ty == FLD_type(fld)) {
		 ty = Stab_Pointer_To(desc_ty);
	       }
	     }
	   }
	 }
