 ...
 * Module: init2f.c
 * $Revision: 1.19 $
 * $Date: 2000/04/06 02:21:47 $
 * $Author: mtibuild $
 * $Source: /isms/cmplrs.src/osprey1.0/be/whirl2f/RCS/init2f.cxx,v $
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/whirl2f/RCS/init2f.cxx,v $ $Revision: 1.19 $";
#endif
			  ...
			  INT           repeat)
{
   ...
   if (repeat > 1)
   {
      ...
      Prepend_Token_String(*init_tokens, Number_as_String(repeat, "%" LLU_FMT ""));
   }
			  ...
			  WN2F_CONTEXT  context)
{
   ...
   if (string_size != TY_size(array_etype))
   {
      ...
      Append_Token_String(tokens, 
			  Number_as_String(string_ofst+1, "%" LLU_FMT ""));
      Append_Token_Special(tokens, ':');
      Append_Token_String(tokens, 
			  Number_as_String(string_ofst+string_size, "%" LLU_FMT ""));
      Append_Token_Special(tokens, ')');
   }
		      ...
		      const ARRAY_SEGMENT *aseg)   /* Array segment info */
{
   ...
   Append_Token_String(tokens, 
		       Number_as_String(aseg->end_ofst/TY_size(aseg->etype),
					"%" LLU_FMT ""));
   Append_Token_Special(tokens, ',');
   Append_Token_String(tokens, Number_as_String(1, "%" LLU_FMT ""));
   Append_Token_Special(tokens, ')');
