			       ...
			       TY_IDX       element_ty)
{
   ...
   Append_Token_Special(decl_tokens, ',');
   Append_Token_String(decl_tokens, Number_as_String(1, "%" LLU_FMT ""));
   Append_Token_Special(decl_tokens, '<');
   Append_Token_Special(decl_tokens, '>');
   Append_Token_Special(decl_tokens, ',');
   Append_Token_String(decl_tokens, 
		       Number_as_String(TY_size(element_ty), "%" LLU_FMT ""));
   Append_Token_String(decl_tokens, "#>");
...
TY2F_Purple_Assumed_Sized_Array(TOKEN_BUFFER decl_tokens, ST *st, TY_IDX ty)
{
   ...
   else
   {
      ...
      Append_Token_Special(decl_tokens, ',');
      Append_Token_String(decl_tokens,
			  Number_as_String(TY_AR_ndims(ty), "%" LLU_FMT ""));
      Append_Token_Special(decl_tokens, '<');
	...
	TY2F_Append_ARB(decl_tokens,arb,TRUE);
      ...
      Append_Token_Special(decl_tokens, ',');
      Append_Token_String(decl_tokens, 
			  Number_as_String(TY_size(TY_AR_etype(ty)), "%" LLU_FMT ""));
      Append_Token_String(decl_tokens, "#>");
      Append_Token_Special(decl_tokens, ')');
   }
		 ...
		 STAB_OFFSET  fld_ofst)
{
   ...
   Append_Token_Special(tokens, '(');
   Append_Token_String(tokens, Number_as_String(fld_ofst+1, "%" LLD_FMT ""));
   Append_Token_Special(tokens, ')');
   Append_Token_Special(tokens, ',');
...
TY2F_scalar(TOKEN_BUFFER decl_tokens, TY_IDX ty_idx)
{
   ...
   if (TY_size(ty) > 0)
   {
	 ...
	 kind_spec = Concat3_Strings("(",Number_as_String(kind_type, "%" LLD_FMT ""),")");
	 ...
	 Prepend_Token_String(
			      ...
			      Concat3_Strings(base_name,
					      "*", 
					      Number_as_String(TY_size(ty), "%" LLD_FMT "")));
      }
   }
   else
   {
			...
			STAB_OFFSET  arr_ofst)
{
  ...
  if (TY_Is_Character_String(arr_ty_idx))
	...
	*/
      Append_Token_String(tokens, Number_as_String(arr_ofst+1, "%" LLD_FMT ""));
      Append_Token_Special(tokens, ':');
      Append_Token_String(tokens, Number_as_String(arr_ofst+1, "%" LLD_FMT ""));
    }
	...
	if (arr_ofst == 0)
	  {
	    Prepend_Token_String(idx_tokens, Number_as_String(1LL, "%" LLD_FMT ""));
	  }
	else if (ARB_const_stride(arb)) /* Constant stride */
	  {
	    idx = arr_ofst/ARB_stride_val(arb) + 1;
	    Prepend_Token_String(idx_tokens, Number_as_String(idx, "%" LLD_FMT ""));
...
TY2F_Dump_Fld_Path(FLD_PATH_INFO *fld_path)
{
  ...
  while (fld_path != NULL)
	...
	printf (" offset 0x%x",(mINT32) fld_path->arr_ofst);
      ...
      if (fld_path->arr_wn != NULL)
	printf (" tree 0x%x",fld_path->arr_wn);
