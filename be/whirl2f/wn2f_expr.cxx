 ...
 * $Author: mpm $
 * $Source: /isms/cmplrs.src/osprey1.0/be/whirl2f/RCS/wn2f_expr.cxx,v $
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/be/whirl2f/RCS/wn2f_expr.cxx,v $ $Revision: 1.9 $";
#endif
 ...
 */
static const char *Conv_Op[MTYPE_CORE_LAST+1][MTYPE_CORE_LAST+1];

/* Only built-in types (<=MTYPE_CORE_LAST) will have converting operations */
#define Get_Conv_Op(mtype1,mtype2)					\
	(((mtype1)<=MTYPE_CORE_LAST && (mtype2)<=MTYPE_CORE_LAST)?	\
	 Conv_Op[(mtype1)][(mtype2)] : NULL)
	     ...
	     MTYPE        to_mtype)
{
   ...
   if (Get_Conv_Op(from_mtype,to_mtype) == NULL)
   {
      ASSERT_WARN(Get_Conv_Op(from_mtype,to_mtype) != NULL,
   ...
   else
   {
       ...
       */
      Prepend_Token_String(tokens, Get_Conv_Op(from_mtype,to_mtype));
   }
...
WN2F_cvtl(TOKEN_BUFFER tokens, WN *wn, WN2F_CONTEXT context)
{
   ...
   if (Get_Conv_Op(TY_mtype(dtype),TY_mtype(rtype)) != NULL)
   {
