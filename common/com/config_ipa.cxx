...
#define DEFAULT_PU_LIMIT	800
#define DEFAULT_HARD_LIMIT	(DEFAULT_PU_LIMIT + (DEFAULT_PU_LIMIT >> 2))
#define DEFAULT_SMALL_PU	20
#define DEFAULT_SMALL_CALLEE	200
#define DEFAULT_MIN_HOTNESS	50
...
UINT32	IPA_Callsite_Bias = 100;
UINT32	IPA_Level_Bias = 100;		/* should be 1 with -IPA */
UINT32	IPA_Callee_Loop_1 = 260;
UINT32	IPA_Callee_Loop_2 = 50;
UINT32	IPA_Callsite_Loop_1 = 195;
UINT32	IPA_Callsite_Loop_2 = 120;

UINT32	IPA_Min_Callsites = 1;
UINT32	IPA_Max_Callsites = 10;
UINT32	IPA_Nonaggr_Callee_Limit = 40;
UINT32  IPA_Few_Calls = 4;
UINT32  IPA_Small_Caller_Size = 50;
...
BOOL    IPA_PU_Minimum_Size_Set = FALSE;
...
UINT32	IPA_Max_Jobs = 1;	/* concurrent backend compilations */
...
*/
    ...
    { OVK_BOOL, OV_INTERNAL,    FALSE, "xpres",             "",
          0, 0, 0,              &IPA_Autotie,   NULL,
          "Use XPRES to generate TIE, then compile with that TIE " },
...
BOOL	INLINE_Requested = FALSE;	/* Must inline PUs marked as 'inline */
BOOL	INLINE_Requested_Only = FALSE;	/* Must inline PUs marked as 'inline and nothing else*/
...
BOOL	INLINE_Aggressive_Set = FALSE;  /* Is -INLINE:aggressive set? */
...
BOOL	INLINE_Enable_Restrict_Pointers = FALSE; // Allow restrict pointers
					// as formal parameter
BOOL	INLINE_Enable_DVE = TRUE; // allow static dead variable elimination
...
UINT32	INLINE_Random_Seed = 1;		/* Seed for random inlining */
BOOL	INLINE_Random_Seed_Set = FALSE;	/* Is -INLINE:random_seed set? */
...
BOOL	INLINE_Skip_Error_Funcs = TRUE;	/* Don't inline error-like functions */
...
static OPTION_DESC Options_INLINE[] = {
    ...
    { OVK_BOOL, OV_SHY,		FALSE, "aggressive",	"",
	  0, 0, 0,	&INLINE_Aggressive,	&INLINE_Aggressive_Set},
    ...
    { OVK_LIST, OV_VISIBLE,     FALSE, "library", "lib",
          0, 0, 0,      &INLINE_List_Names,     NULL,
         "Identify libraries where inliner should search for subprograms" },
    ...
    { OVK_BOOL, OV_VISIBLE,	TRUE,	"dve",	"",
	  0, 0, 0,		&INLINE_Enable_DVE, NULL,
	  "Allow elimination of dead static variables" },
    ...
    { OVK_BOOL,	OV_SHY,		FALSE, "preemptible",	"preempt",
	  ...
	  "Enable inlining of non-static functions" },
    { OVK_BOOL,	OV_SHY,		FALSE, "requested",	"requested",
	  0, 0, 0,	&INLINE_Requested,	NULL,
	  "Inline all functions marked as 'inline'" },
    { OVK_BOOL,	OV_SHY,		FALSE, "requested_only",	"requested_only",
	  0, 0, 0,	&INLINE_Requested_Only,	NULL,
	  "Inline all functions marked as 'inline' but nothing else" },
    ...
    { OVK_UINT32, OV_INTERNAL,	FALSE, "random_seed", "",
	  0, 1, UINT32_MAX, &INLINE_Random_Seed, &INLINE_Random_Seed_Set,
	  "Seed for random inlining" },
    { OVK_BOOL, OV_INTERNAL,    FALSE, "var_dim_array",             "",
          0, 0, 0,              &IPA_Enable_Inline_Var_Dim_Array,   NULL,
          "Enable inlining of PU with param that is variable-dim array " },
    { OVK_LIST,	OV_VISIBLE,	FALSE, "in",	"in",
	  0, 0, 0,	&INLINE_List_Names,	NULL,
    ...
    { OVK_BOOL, OV_INTERNAL,    FALSE, "inlined_pu",	"inlined_pu",
    ...
    { OVK_BOOL, OV_INTERNAL,    FALSE, "error",		"",
          0, 0, 0,              &INLINE_Skip_Error_Funcs,   NULL,
          "Do not inline error-handling functions" },
    { OVK_UINT32, OV_VISIBLE,	FALSE, "bloat",	"",
	  DEFAULT_BLOAT_FACTOR, 0, UINT32_MAX, &IPA_Bloat_Factor,
	  &IPA_Bloat_Factor_Set}, 
    { OVK_UINT32,OV_VISIBLE,	FALSE, "hard_caller_limit", "",
	  DEFAULT_HARD_LIMIT, 0, UINT32_MAX,
	  &IPA_PU_Hard_Limit, &IPA_PU_Hard_Limit_Set }, 
    { OVK_UINT32, OV_VISIBLE,	FALSE, "caller_limit", "",
	  DEFAULT_PU_LIMIT, 0, UINT32_MAX, &IPA_PU_Limit, &IPA_PU_Limit_Set},
    { OVK_UINT32, OV_SHY,	FALSE, "callee_limit", "callee_limit",
	  DEFAULT_SMALL_CALLEE, 0, UINT32_MAX, &IPA_Small_Callee_Limit, NULL},
    { OVK_UINT32, OV_VISIBLE,	FALSE, "small_callee_limit", "small_callee",
	  DEFAULT_SMALL_PU, 1, UINT32_MAX, &IPA_PU_Minimum_Size, 
	  &IPA_PU_Minimum_Size_Set},
    { OVK_UINT32, OV_SHY,	FALSE, "callsite_bias", "callsite_bias",
        100,   1, UINT32_MAX, &IPA_Callsite_Bias, NULL},
    { OVK_UINT32, OV_SHY,	FALSE, "level_bias", "level_bias",
	  1,   1, UINT32_MAX, &IPA_Level_Bias, NULL},
    { OVK_UINT32, OV_SHY,	FALSE, "callee_loop1", "callee_loop1",
        260,   0, UINT32_MAX, &IPA_Callee_Loop_1, NULL},
    { OVK_UINT32, OV_SHY,	FALSE, "callee_loop2", "callee_loop2",
         50,   0, UINT32_MAX, &IPA_Callee_Loop_2, NULL},
    { OVK_UINT32, OV_SHY,	FALSE, "callsite_loop1", "callsite_loop1",
        195,   0, UINT32_MAX, &IPA_Callsite_Loop_1, NULL},
    { OVK_UINT32, OV_SHY,	FALSE, "callsite_loop2", "callsite_loop2",
         30,   0, UINT32_MAX, &IPA_Callsite_Loop_2, NULL},
    { OVK_UINT32, OV_SHY,	FALSE, "min_callsites", "min_calls",
          1,   0, UINT32_MAX, &IPA_Min_Callsites, NULL},
    { OVK_UINT32, OV_SHY,	FALSE, "max_callsites", "max_calls",
         10,   0, UINT32_MAX, &IPA_Max_Callsites, NULL},
    { OVK_UINT32, OV_SHY,	FALSE, "nonaggr_callee_limit", "",
         40,   0, UINT32_MAX, &IPA_Nonaggr_Callee_Limit, NULL},
    { OVK_UINT32, OV_SHY,	FALSE, "few_calls", "",
          3,   0, UINT32_MAX, &IPA_Few_Calls, NULL},
    { OVK_UINT32, OV_SHY,	FALSE, "small_caller_limit","small_caller",
         50,   0, UINT32_MAX, &IPA_Small_Caller_Size, NULL},
    { OVK_COUNT }	    /* List terminator -- must be last */
};
