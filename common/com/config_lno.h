 ...
 * Module: config_lno.h
 * $Revision: 1.24 $
 * $Date: 2000/04/06 02:23:48 $
 * $Author: mtibuild $
 * $Source: /isms/cmplrs.src/osprey1.0/common/com/RCS/config_lno.h,v $
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *config_lno_h_rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/common/com/RCS/config_lno.h,v $ $Revision: 1.24 $";
#endif /* _KEEP_RCS_ID */
...
extern DLL_SHARED BOOL Run_autopar;
...
typedef struct lno_flags {
  ...
  UINT32 Simd_level;
  UINT32 Simd_loop_before;
  UINT32 Simd_loop_after;
  UINT32 Simd_verbose;
  BOOL Aligned_formal_pointers;
  BOOL Aligned_pointers;
  BOOL Aligned_accesses;
  BOOL Aligned_loads;
  BOOL Aligned_stores;
  BOOL Fold_arrays;
  BOOL Simd_shr_safe;
  BOOL Simd_if_conv;
  BOOL Simd_agg_if_conv;
  BOOL Simd_check_guards;
...
extern DLL_SHARED LNO_FLAGS *Current_LNO;
extern DLL_SHARED struct MHD *Current_MHD;

/* And this is always the invariant bottom of stack: */
extern DLL_SHARED LNO_FLAGS Initial_LNO;
...
#define LNO_Test_call_info		Current_LNO->Test_call_info

// Annotate call info on calls (if possible, as determined by IPA or
// TIE analysis)
#define LNO_Call_info		        Current_LNO->Call_info
...
// The SIMDization level
#define LNO_SIMD_Level          	Current_LNO->Simd_level

// SIMD loops:
  // if Before<After:  before Before or after After inclusive
  // if After<=Before: between After and Before inclusive
#define LNO_SIMD_Loop_Before          	Current_LNO->Simd_loop_before
#define LNO_SIMD_Loop_After          	Current_LNO->Simd_loop_after

// SIMD Verbose
#define LNO_SIMD_Verbose                Current_LNO->Simd_verbose

// Assumes that passed as formal parameters and used as array bases point
// to aligned arrays (SIMD)
#define LNO_Aligned_Formal_Pointers     Current_LNO->Aligned_formal_pointers

// Assumes that all pointers used as array bases point to aligned arrays (SIMD)
#define LNO_Aligned_Pointers            Current_LNO->Aligned_pointers

// Assumes that all accesses are aligned (SIMD)
#define LNO_Aligned_Accesses            Current_LNO->Aligned_accesses

// Assumes that all loads are aligned (SIMD)
#define LNO_Aligned_Loads               Current_LNO->Aligned_loads

// Assumes that all stores are aligned (SIMD)
#define LNO_Aligned_Stores              Current_LNO->Aligned_stores

// Fold the array of array access constructs
#define LNO_Fold_Arrays                 Current_LNO->Fold_arrays

// Move shift right out of loop when the vector register has guard bits
#define LNO_Simd_Shr_Safe               Current_LNO->Simd_shr_safe

// If conversion
#define LNO_Simd_If_Conv                Current_LNO->Simd_if_conv
#define LNO_Simd_Aggressive_If_Conv     Current_LNO->Simd_agg_if_conv

// Check for enough guard bits.
#define LNO_Simd_Check_Guards           Current_LNO->Simd_check_guards
