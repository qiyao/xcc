...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *config_opt_rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/common/com/RCS/config_opt.cxx,v $ $Revision: 1.46 $";
#endif /* _KEEP_RCS_ID */
...
extern DLL_SHARED BOOL WOPT_Enable_Goto;	/* In case config_wopt.h is later */

/* ====================================================================
 *
...
BOOL Align_Arrays = TRUE;      /* Align arrays to allow vectorization */
...
BOOL Sqrt_Split_Allowed = FALSE;		/* change sqrt(a) --> recip(rsqrt(a)) ? */
BOOL Sqrt_Split_Set = FALSE;	/* ... option seen? */
...
BOOL Sqrt_Allowed = TRUE;		/* generate SQRT instruction? */
BOOL Sqrt_Set = FALSE;		/* ... option seen? */
BOOL Rsqrt_Allowed = FALSE;		/* generate RSQRT instruction? */
BOOL Rsqrt_Set = FALSE;		/* ... option seen? */
BOOL Recip_Allowed;		        /* generate RECIP instruction? */
BOOL Recip_Set = FALSE;		/* ... option seen? */
...
BOOL Enable_HiFi2_Ops = FALSE; /* Enable use of HiFi2 instructions. */

/***** Limits on optimization *****/
#define DEFAULT_OLIMIT		9000
#define DEFAULT_O3_OLIMIT	12000	/* allow more time for -O3 compiles */
...
BOOL OPT_IV_Short_Wrap_Around_Safe = FALSE;
static BOOL OPT_IV_Short_Wrap_Around_Safe_Set = FALSE;/* ... option seen? */
...
BOOL Enable_extract_compose=FALSE; /* Disable these IA64-specific optimizations
                                      by default because the compiler doesn't deal
                                      too well with them and, in general, MIPS, IA32
                                      and Xtensa don't benefit from them. This flag
                                      is also set in config_targ.cxx */
BOOL OPT_Delay_Post = TRUE;
BOOL OPT_Postfix_St = TRUE;

BOOL OPT_Malloc_Calls = FALSE;

/***** Instrummentation Related Options *****/
INT32 Instrumentation_Phase_Num = 0;
INT32 Instrumentation_Enabled = 0;
INT32 Instrumentation_Bits = 0;
UINT32 Instrumentation_Actions = 0;
BOOL Instrumentation_Unique_Output = FALSE; // always create unique output
BOOL Feedback_Strict_Verify = FALSE;
...
BOOL	Keep_Inline_Funcs = FALSE; /* Don't delete inline functions */
BOOL	Keep_Static_Funcs = FALSE; /* Don't delete static functions */
...
static OPTION_DESC Options_OPT[] = {
  ...
  { OVK_BOOL,	OV_VISIBLE,	TRUE, "align_arrays",		"align_array",
    0, 0, 0,	&Align_Arrays,	NULL,
    "Allow array alignment to improve loop vectorization" },
  ...
  { OVK_BOOL,	OV_VISIBLE,	TRUE, "hifi2_ops",	"hifi2_ops",
    0, 0, 0,	&Enable_HiFi2_Ops, NULL,
    "Enable use of HiFi2 instructions to generate more efficient code" },
  ...
  { OVK_BOOL, OV_INTERNAL,    FALSE, "keep_inline",	"",
          0, 0, 0,              &Keep_Inline_Funcs,   NULL,
          "Do not delete inline functions" },
  { OVK_BOOL, OV_INTERNAL,    FALSE, "keep_static",	"",
          0, 0, 0,              &Keep_Static_Funcs,   NULL,
          "Do not delete static (non-inline) functions" },
  ...
  { OVK_BOOL,	OV_INTERNAL,	FALSE, "space_flix",	"space_flix",
    0, 0, 0,	&OPT_Flix_Space, NULL,
    "Do space optimization for flix instructions" },

  { OVK_UINT32,	OV_VISIBLE,	FALSE, "space_opt_threshold",	"space_opt",
    10, 0, UINT32_MAX,    &OPT_Space_Threshold,	NULL,
    "Threshold for optimizing functions for space" },

  { OVK_BOOL,	OV_VISIBLE,	TRUE, "sqrt",		"sqrt",
    0, 0, 0,	&Sqrt_Allowed, &Sqrt_Set,
    "Allow use of sqrt instruction" },

  { OVK_BOOL,	OV_VISIBLE,	TRUE, "sqrt_split",		"sqrt_split",
    0, 0, 0,	&Sqrt_Split_Allowed, &Sqrt_Split_Set ,
    "Allow splitting of sqrt(a) into recip(rsqrt(b))" },

  { OVK_BOOL,	OV_INTERNAL,	FALSE, "estimate_ccount",	"",
    0, 0, 0,    &OPT_Estimate_Ccount,	NULL,
    "Estimate function cycle counts instead of using rsr.ccount" },
  ...
  { OVK_BOOL,	OV_VISIBLE,	FALSE, "zcl",			"zcl",
    0, 0, 0,    &Enable_ZCL,	&Enable_ZCL_overridden,
    "Enable zero-overhead loops" },
  ...
  { OVK_BOOL,	OV_VISIBLE,	TRUE, "iv_short_wrap_around_safe", "",
    0, 0, 0,	&OPT_IV_Short_Wrap_Around_Safe,	&OPT_IV_Short_Wrap_Around_Safe_Set,
    "Allow IV which may wrap around short integer" },
  ...
  { OVK_INT32,  OV_VISIBLE,	TRUE, "instrument_bits",	 "",
    0, 0, 64,	&Instrumentation_Bits, NULL,
    "Bit size for instrumentation counters" },
  ...
  { OVK_BOOL,	OV_INTERNAL,	FALSE,	"fb_opt_strict",	"",
    0, 0, 0,	&Feedback_Strict_Verify,	NULL,
    "Require strict verification of feedback data (without guessing)" },
  ...
  { OVK_BOOL,	OV_INTERNAL,	TRUE, "delay_post",	"delay_post",
    0, 0, 0,	&OPT_Delay_Post,	NULL,
    "Delay post(increment/decrement)" },

  { OVK_BOOL,	OV_INTERNAL,	TRUE, "postfix_st",	"postfix_st",
    0, 0, 0,	&OPT_Postfix_St,	NULL,
    "Use lhs ST in postfix ops" },

  { OVK_BOOL,	OV_INTERNAL,	TRUE, "malloc",	"malloc",
    0, 0, 0,	&OPT_Malloc_Calls,	NULL,
    "Mark malloc-like calls as returning new memory" },
