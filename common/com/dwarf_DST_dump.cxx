...
static const char rcs_id[] = "$Source: /isms/cmplrs.src/osprey1.0/common/com/RCS/dwarf_DST_dump.cxx,v $ $Revision: 1.56 $";
#endif
...
{
   DST_put_string(at_name);
   sprintf(&tmp_buffer[0], "(0x%" LLX_FMT ")", num);
   DST_put_string(&tmp_buffer[0]);
}
...
{
   DST_put_string(at_name);
   sprintf(&tmp_buffer[0], "(%" LLD_FMT ")", num);
   DST_put_string(&tmp_buffer[0]);
}
...
{
   DST_put_string(at_name);
   sprintf(&tmp_buffer[0], "(%" LLU_FMT ")", num);
   DST_put_string(&tmp_buffer[0]);
}
...
DST_put_assoc(const char *at_name, DST_flag flag, DST_ASSOC_INFO assoc)
{
   ...
   if (DST_IS_assoc_fe(flag))
      DST_put_hex64_attribute("FE", (UINT64)(INTPTR)DST_ASSOC_INFO_fe_ptr(assoc));
   else {
      DST_put_string("ST");
...
DST_put_subprogram(DST_flag flag, DST_SUBPROGRAM *attr)
{
   ...
   else if (DST_IS_declaration(flag))
   {
	 ...
	 DST_put_string(" prototyped");
      ...
      DST_put_idx_attribute(" parent", DST_SUBPROGRAM_decl_parent(attr), FALSE);
   ...
   else /* definition */
   {
	 ...
	 DST_put_string(" prototyped");
      ...
      DST_put_idx_attribute(" clone_origin",
			 ...
			 FALSE);
      DST_put_idx_attribute(" parent", DST_SUBPROGRAM_def_parent(attr), FALSE);
...
DST_put_variable(DST_flag flag, DST_VARIABLE *attr)
{
   ...
   else /* definition */
   {
	...
	DST_put_string(" assumed_size");
      ...
      DST_put_idx_attribute(" specification",
                            DST_VARIABLE_def_specification(attr), TRUE);
      DST_put_idx_attribute(" abstract_origin",
