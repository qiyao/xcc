...
#ifdef _WIN32 
   convert_to_win32_path(src_path);
   convert_to_win32_path(comp_dir);
#endif
		  ...
		  DST_access   access)
{
   ...
   if (is_declaration)
   {
      ...
      DST_SUBPROGRAM_decl_parent(attr) = parent;
      DST_SUBPROGRAM_decl_origin(attr) = origin;
      DST_SUBPROGRAM_decl_inline(attr) = inlin;
      DST_SUBPROGRAM_decl_access(attr) = access;
#ifdef CPLUSPLUSIL
   ...
   else
   {
      ...
      DST_SUBPROGRAM_def_parent(attr) = parent;
      DST_SUBPROGRAM_def_type(attr) = type;
		      ...
		      BOOL         is_incomplete)
{
   ...
   DST_STRUCTURE_TYPE_inheritance(attr) = DST_INVALID_IDX;
		  ...
		  BOOL         is_incomplete)
{
   ...
   DST_UNION_TYPE_inheritance(attr) = DST_INVALID_IDX;
...
DST_mk_class_type(USRCPOS      decl,      /* Source location */
		  ...
		  DST_size_t   byte_size, /* Size of union, if known */
		  DST_INFO_IDX abstract_origin,
		  DST_INFO_IDX inheritance,
		  BOOL         is_incomplete,
		  BOOL         vtype_self_reference)
{
   DST_INFO_IDX    info_idx;
   DST_ATTR_IDX    attr_idx;
   DST_flag        flag = DST_no_flag;
   ...
   if (vtype_self_reference)
     DST_CLASS_TYPE_inheritance(attr) = info_idx;
   else
     DST_CLASS_TYPE_inheritance(attr) = inheritance;
...
DST_mk_inheritance(USRCPOS      decl,     /* Source location */
		   DST_INFO_IDX type,     /* Type of member */
		   DST_access   access,   /* AT_accessibility code */
		   ...
		   DST_size_t   memb_loc) /* Byte-offset of member container */
{
   ...
   DST_INHERITANCE_access(attr) = access;
