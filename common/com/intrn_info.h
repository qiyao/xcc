...
extern DLL_SHARED const intrn_info_t intrn_info[INTRINSIC_CORE_LAST+1];

#define Intrinsic_To_Tie_Macro_Id(i)    ((i)-INTRINSIC_CORE_LAST-1)
#define Tie_Macro_Id_To_Intrinsic(i)    ((INTRINSIC)((i)+INTRINSIC_CORE_LAST+1))

inline BOOL INTRN_is_tie_intrinsic (const INTRINSIC i)
{
  return (i > INTRINSIC_CORE_LAST) && (i <= Intrinsic_Last);
}


inline BOOL INTRN_by_value (const INTRINSIC i)
{
  return((INTRN_is_tie_intrinsic(i)) ?
	 FALSE :
	 intrn_info[i].is_by_val);
}

inline BOOL INTRN_is_pure (const INTRINSIC i)
{
  return((INTRN_is_tie_intrinsic(i)) ?
	 tie_info->tie_macro(Intrinsic_To_Tie_Macro_Id(i))->is_pure() :  
	 intrn_info[i].is_pure);
}

inline BOOL INTRN_has_no_side_effects (const INTRINSIC i)
{
  return((INTRN_is_tie_intrinsic(i)) ?
	 (tie_info->tie_macro(Intrinsic_To_Tie_Macro_Id(i))->has_side_effect() == FALSE) :  
	 intrn_info[i].has_no_side_effects);
}
...
inline BOOL INTRN_is_actual (const INTRINSIC i)
{
  return((INTRN_is_tie_intrinsic(i)) ?
	 FALSE :
	 intrn_info[i].is_actual);
}

inline BOOL INTRN_is_leaf (const INTRINSIC i)
{
  return((INTRN_is_tie_intrinsic(i)) ?
	  !tie_info->tie_macro(Intrinsic_To_Tie_Macro_Id(i))->has_branch() :
	  FALSE);
}
...
inline INTRN_RETKIND INTRN_return_kind (const INTRINSIC i)
{
  return((INTRN_is_tie_intrinsic(i)) ?
	 IRETURN_UNKNOWN :
	 intrn_info[i].return_kind);
}

inline char * INTRN_c_name (const INTRINSIC i)
{
  return((INTRN_is_tie_intrinsic(i)) ?
	 tie_info->macro_name(Intrinsic_To_Tie_Macro_Id(i)) :  
	 intrn_info[i].c_name);
}
