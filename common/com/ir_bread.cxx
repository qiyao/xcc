...
#include <libelf/libelf.h>		    /* for all Elf stuff */
...
static void *local_fhandle = NULL;	/* file handle */
...
#ifndef _WIN32
static void *
read_file (char *filename, off_t* mapped_size, char* file_revision, int *ret_fd)
#else
static void *
read_file (char *filename, off_t* mapped_size, char* file_revision, int *ret_fd, void **retMapHd)
#endif
{
    int fd;
    INT st;
    ...
    *retMapHd = (void *)
      CreateFileMapping((HANDLE)_get_osfhandle(fd), NULL, 
			PAGE_READONLY, 0, stat_buf.st_size, NULL);
    if (*retMapHd == NULL)
       return (void *) ERROR_RETURN;
    map_addr = (char *)MapViewOfFileEx((HANDLE) *retMapHd, FILE_MAP_COPY,
...
#ifdef _WIN32
void *
WN_open_input (char *filename, off_t *mapped_size, int *fd, void **mapHd)
#else
void *
WN_open_input (char *filename, off_t *mapped_size, int *fd)
#endif
{
    if (filename == 0) {
	errno = ENOENT;
    ...
    errno = 0;
#ifndef _WIN32
    return read_file (filename, mapped_size, file_revision, fd);
#else
...
#ifdef _WIN32
extern void *
WN_inline_open_file(char* file_name, off_t *mapped_size, char* file_revision, void **mapHd)
#else
extern void *
...
#endif
{
    ...
    errno = 0;
#ifndef _WIN32
    return read_file (file_name, mapped_size, file_revision, 0);
#else
...
#ifdef _WIN32
void
WN_free_input (void *handle, void **mapHd, off_t mapped_size)
#else
void
...
open_specified_input (char *input_file,
	char **ir_input, void **fhandle, void **mapHd, off_t *mapped_size, int *fd = 0)
#else
static void
open_specified_input (char *input_file, 
	char **ir_input, void **fhandle, off_t *mapped_size, int *fd = 0)
#endif
{
    Set_Error_Phase ( "Reading WHIRL file" );
    *ir_input = input_file;
#ifndef _WIN32
    *fhandle = WN_open_input (input_file, mapped_size, fd);
#else
...
void *
Open_Local_Input (char *input_file, int *fd)
{
#ifdef _WIN32
	open_specified_input (input_file,
		&local_ir_file, &local_fhandle, &local_mapHandle, &local_mapped_size, fd);
#else
	open_specified_input (input_file, 
		&local_ir_file, &local_fhandle, &local_mapped_size, fd);
#endif
