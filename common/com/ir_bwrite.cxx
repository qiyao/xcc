...
extern DLL_SHARED WN **prefetch_ldsts;
extern DLL_SHARED INT num_prefetch_ldsts;
extern DLL_SHARED INT max_num_prefetch_ldsts;

extern DLL_SHARED WN **alias_classes;
extern DLL_SHARED INT num_alias_class_nodes;
extern DLL_SHARED INT max_alias_class_nodes;

extern DLL_SHARED WN **ac_internals;
extern DLL_SHARED INT num_ac_internal_nodes;
extern DLL_SHARED INT max_ac_internal_nodes;
...
ir_bwrite_signal_handler (int sig, int err_num)
{
    ...
    if (Doing_mmapped_io && err_num > 0 && !strerror(err_num))
...
write_output (UINT64 e_shoff, const typename ELF::Elf_Shdr& strtab_sec,
              Output_File *fl, const ELF& tag, unsigned int output_type)
{
    INT i;
...
create_temp_file (Output_File *fl)
{
    ...
    fd = mkstemp (path);
#else
    /* solaris versions < 2.6 dont' have mkstemp, here is a reasonable
       approximation */
    {
//#ifndef _WIN32
	    int mode = O_RDWR | O_CREAT | O_EXCL ;
//#else
//		int mode = _S_IREAD | S_IWRITE;
//#endif
    do {
      strcpy (path, tmpdir);
      strcat (path, DEFAULT_TEMPLATE);
...
#elif defined(_WIN32)
    _chsize(fl->output_fd, MAPPED_SIZE);
#endif
...
WN_write_feedback (PU_Info* pu, Output_File* fl)
{
    ...
    pu_hdr.pu_opt_flags = Calc_Feedback_Flags();
...
void
WN_close_output (Output_File *fl, unsigned int output_type)
{
	char errBuf[100];
    if (fl->output_fd < 0)
	...
	write_output (offset, strtab_sec, fl, ELF32(), output_type);
    } else
#endif
    {
	Elf64_Shdr strtab_sec;
	UINT64 e_shoff = layout_sections (strtab_sec, fl);
	write_output (e_shoff, strtab_sec, fl, ELF64(), output_type);
    }
#ifndef _WIN32
   MUNMAP(fl->map_addr, fl->mapped_size);
...
void
Close_Output_Info (unsigned int output_type)
{
    WN_write_revision (ir_output);

    WN_close_output(ir_output, output_type);
}

#ifdef BACK_END


/* Traverse the PU tree and save the original subsection states
   in 'save_state'. Set all subsections to missing except the AUTOTIE
   subsection. */
static INT
set_Autotie_PU_Info_State (PU_Info *pu_tree,
                           INT save_state[][WT_SUBSECTIONS], INT pu_idx)
{
  for (PU_Info *pu = pu_tree; pu; pu = PU_Info_next(pu)) {
    for (unsigned int k = 0; k < WT_SUBSECTIONS; k++) {
      save_state[pu_idx][k] = PU_Info_state(pu, k);
      if (k != WT_AUTOTIE)
        Set_PU_Info_state(pu, k, Subsect_Missing);
    }
    
    pu_idx++;
    
    if (PU_Info_child(pu)) {
      pu_idx = set_Autotie_PU_Info_State(PU_Info_child(pu), save_state, pu_idx);
    }
  }
...
/* Traverse the PU tree and restore the original subsection states
   from 'orig_state'. */
static INT
restore_Autotie_PU_Info_State (PU_Info *pu_tree,
                               INT orig_state[][WT_SUBSECTIONS], INT pu_idx)
{
  for (PU_Info *pu = pu_tree; pu; pu = PU_Info_next(pu)) {
    for (unsigned int k = 0; k < WT_SUBSECTIONS; k++) {
      Set_PU_Info_state(pu, k, orig_state[pu_idx][k]);
    }
    
    pu_idx++;
    
    if (PU_Info_child(pu)) {
      pu_idx = restore_Autotie_PU_Info_State(PU_Info_child(pu), orig_state, pu_idx);
    }
  }
...
void
Write_Global_Autotie_Info (PU_Info *pu_tree)
{
    Set_Error_Phase ("Writing WHIRL file" );

    /* override the MIPS_WHIRL_PU_SECTION's type to be AUTOTIE, since
       it contains autotie information and not whirl. */
    Section *at_section = get_section (WT_PU_SECTION, MIPS_WHIRL_PU_SECTION, ir_output);
    at_section->shdr.sh_type = SHT_XTENSA_AUTOTIE;

    /* We only want to output the autotie subsection, so temporarily
       set all the others to "missing". Save the original subsection
       states in 'orig_state' and restore them later. */
    INT pu_cnt = Number_PU_Infos(pu_tree, 0, /* set_maptab */ FALSE);
    INT orig_state[pu_cnt][WT_SUBSECTIONS];
    set_Autotie_PU_Info_State(pu_tree, orig_state, 0);
    WN_write_PU_Infos (pu_tree, ir_output);
    restore_Autotie_PU_Info_State(pu_tree, orig_state, 0);
}

void
Write_Autotie_PU_Info (PU_Info *pu)
{
  Temporary_Error_Phase ephase("Writing WHIRL file");

  static char *scn_name = "xpres";
  Section *cur_section = ir_output->cur_section;

  /* make sure we're in the right section */
  if (strcmp(cur_section->name, MIPS_WHIRL_PU_SECTION) != 0 ||
      PU_Info_state(pu, WT_AUTOTIE) != Subsect_InMem)
    ErrMsg (EC_IR_Scn_Write, scn_name, ir_output->file_name);

  /* libauto formats all the autotie information into a single buffer,
     which we just output. */
  AT_PU *at_pu = (AT_PU *)PU_Info_autotie_ptr(pu);
  INT buf_sz, align;
  char *buf = at_pu->Format_Output(&buf_sz, &align);

  off_t autotie_base = ir_b_align(ir_output->file_size, align, 0);

  ir_b_save_buf(buf, buf_sz, align, 0, ir_output);

  Set_PU_Info_state(pu, WT_AUTOTIE, Subsect_Written);
  PU_Info_subsect_size(pu, WT_AUTOTIE) = ir_output->file_size - autotie_base;
  PU_Info_subsect_offset(pu, WT_AUTOTIE) =
    autotie_base - cur_section->shdr.sh_offset;
}
#endif
