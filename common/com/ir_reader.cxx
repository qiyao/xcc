...
#define OPCODE_has_alias_info(opc)	(OPCODE_is_load(opc) ||\
                                  	 ...
                                  	 OPCODE_operator(opc) == OPR_INTRINSIC_CALL)
...
static void ir_put_wn(WN * wn, INT indent)
{
    ...
    fprintf(ir_ofile, OPCODE_name(opcode) + strlen("OPC_"));
    ...
    case OPR_OUTPART:
      fprintf(ir_ofile, " %d", WN_outpart(wn));
      break;
	...
	fprintf(ir_ofile, " %" LLD_FMT "", WN_element_size(wn));
    ...
    if (OPCODE_has_value(opcode)) {
      if (!(opcode == OPC_PRAGMA && WN_pragma(wn)==WN_PRAGMA_ASM_CONSTRAINT)) {
	fprintf(ir_ofile, " %" LLD_FMT "", WN_const_val(wn));
	/* Also print the hex value for INTCONSTs */
	if (OPCODE_operator(opcode) == OPR_INTCONST || opcode == OPC_PRAGMA) {
	    fprintf(ir_ofile, " (0x%" LLX_FMT ")", WN_const_val(wn));
	}
      }
    }
	...
	&& WN_st_idx(wn) != (ST_IDX) 0 && opcode != OPC_PRAGMA) {

      if (ST_class(WN_st(wn)) == CLASS_PREG) {
	{
	  if (Preg_Is_Dedicated(WN_offset(wn)))
	  {
	    ISA_REGCLASS rc = Dedicated_Preg_Regclass(WN_offset(wn));
	    INT reg = Dedicated_Preg_Isa_Reg(WN_offset(wn));

	    const ISA_REGCLASS_INFO *rcinfo = TI_ISA_Regclass_Info(rc);
	    fprintf(ir_ofile, " # $%s", TI_ISA_Regclass_Reg_Name(rcinfo, reg));
	  }
	  else
	  {
	    /* reference to a non-dedicated preg */
	    if (Get_Preg_Idx(WN_offset(wn))
      ...
      } else if (ST_assigned_to_dedicated_preg(WN_st(wn))) {
	ST* st = WN_st(wn);
	ST_IDX idx = ST_st_idx (st);
	ST_ATTR_IDX d;
	ST_ATTR_ITER st_attr_iter = Make_st_attr_iter (0);
	int table_size = Scope_tab[CURRENT_SYMTAB].st_attr_tab->Size();
	for (int i=0; i<table_size; i++) {
	  ST_ATTR st_attr = St_Attr_Table[i];
	  if (ST_ATTR_st_idx(st_attr)==idx) {
	    PREG_NUM preg = ST_ATTR_reg_id(st_attr);
	    ISA_REGCLASS rc = Dedicated_Preg_Regclass(preg);
	    INT reg = Dedicated_Preg_Isa_Reg(preg);

	    const ISA_REGCLASS_INFO *rcinfo = TI_ISA_Regclass_Info(rc);
	    fprintf(ir_ofile, " # assigned to PREG(%d) $%s",
			preg, TI_ISA_Regclass_Reg_Name(rcinfo, reg));
	  }
	}
      }
    }
    ...
    if ((OPCODE_operator(opcode) == OPR_LOOP_INFO) &&
	(WN_loop_at_region_id(wn) != AT_REGION_ID_UNKNOWN)) {
      fprintf(ir_ofile, " <at_region %u>", WN_loop_at_region_id(wn));
    }
	...
	if (ir_put_map && ( WN_map_id(wn) != -1 )) {
		...
		fprintf(ir_ofile, " <map %08lx>", WN_MAP_Get( ir_put_map, wn ));
		break;
...
static void ir_put_stmt(WN * wn, INT indent)
{
  ...
  if (wn) {
    ...
    case OPC_ASM_STMT:
      ...
      ir_put_stmt(WN_kid(wn,1), indent+1);
      ir_put_marker("ASM_INPUTS", indent+1);
      {
	INT i;
	for (i = 2; i < WN_kid_count(wn); i++) {
	  ir_put_expr(WN_kid(wn,i), indent+2);
	}
      }
      ir_put_marker("END_ASM_INPUTS", indent+1);
      break;
...
static void fdump_DUMPDEP(FILE *f, DUMPDEPp head, struct ALIAS_MANAGER *alias)
{
	...
	else if (WN_operator(node) == OPR_INTRINSIC_CALL)
	{
	    fprintf(f, "TIE_CALL[%d] ", DUMPDEP_id(p));
	}
...
extern void fdump_tree(FILE *f, WN *wn)
{
  ...
  if (!wn) {
    ...
    fprintf(ir_ofile, "unknown opcode in (WN *) 0x%lx\n", wn);
  ir_ofile = save;
}
