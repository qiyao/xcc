...
/** $Revision: 1.40 $
*** $Date: 2000/04/06 02:26:37 $
*** $Author: mtibuild $
*** $Source: /isms/cmplrs.src/osprey1.0/common/com/RCS/opcode.h,v $
**/
...
#ifdef _KEEP_RCS_ID
static char *opcode_rcs_id = opcode_INCLUDED "$Revision: 1.40 $";
#endif /* _KEEP_RCS_ID */
...
extern DLL_SHARED BOOL Operator_To_Opcode_Table_Inited;
void Init_Operator_To_Opcode_Table(void);

#define OPCODE_make_op_MACRO(opr,rtype,desc)			\
		((OPCODE) ((opr) |				\
			   ((rtype) << OPERATOR_BIT_SIZE) |	\
			   ((desc) << (OPERATOR_BIT_SIZE+RTYPE_BIT_SIZE))))
