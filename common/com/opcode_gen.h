...
BOOL Is_Valid_Core_Opcode_Parts(OPERATOR opr, TYPE_ID rtype, TYPE_ID desc);
#else
...
OPCODE_operator (OPCODE op)
{
  ...
  return (OPERATOR) (op & OPERATOR_BIT_MASK);
}
...
{
  Is_True(Is_Valid_Opcode (op), ("Bad opcode %d", op));
  return (TYPE_ID) ((op >> OPERATOR_BIT_SIZE) & RTYPE_BIT_MASK);
}
...
{
  Is_True(Is_Valid_Opcode (op), ("Bad opcode %d", op));
  return (TYPE_ID) ((op >> (OPERATOR_BIT_SIZE+RTYPE_BIT_SIZE)) & DESC_BIT_MASK);
}
