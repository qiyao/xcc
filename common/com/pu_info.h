...
#define WT_AUTOTIE	0x9		/* autotie analysis information */
...
#define PU_Info_autotie_ptr(pu) \
     (void *) PU_Info_subsect_ptr((pu), WT_AUTOTIE)
...
#define Set_PU_Info_autotie_ptr(pu,x) \
     PU_Info_subsect_ptr((pu), WT_AUTOTIE) = (void *) (x)
...
/* Return the number of PUs under 'pu_tree'. If 'set_maptab' is TRUE,
   initialize the PU info maptab field with the global index of the PU. */
extern INT Number_PU_Infos (PU_Info *pu_tree, INT num_PUs, BOOL set_maptab);

extern DLL_SHARED PU_Info *Current_PU_Info;
...
#ifdef FIL
extern DLL_SHARED PU_Info *Last_PU;
#endif
