...
// **** changed _PTR to x_PTR to eliminate conflict
//      on Windows
...
template <class ARRAY_Ptr, class T, class x_PTR, class REF>
...
public:
  ...
  typedef x_PTR                      pointer;
  typedef REF                       reference;

private:
    
    ARRAY_Ptr segmented_array;
    x_PTR ptr;				// pointer to the current element
    x_PTR segment_last;			// ptr after the current segment
...
public:
    ...
    x_PTR Ptr () const			{ return ptr; }
    x_PTR operator->() const              { return ptr; }
    UINT Index () const {
