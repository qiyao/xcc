...
ST_size (const ST *st)
{
  ...
  case CLASS_PREG:
    if (ST_sclass(st) == SCLASS_FORMAL)
      return TY_size(Promoted_Parm_Type(st));
...
{
    Is_True(!Preg_Is_Dedicated(i),
	    ("Preg_Name:  not valid for dedicated preg"));

    const PREG& preg = Preg_Table[Get_Preg_Idx(i)];
		     ...
		     SCOPE *scope_tab, SYMTAB_IDX level)
{
	...
	// pregs of simulated types have to allocate space for
	// all the parts of the lowered types.
	if (MTYPE_is_tie_packed(mtype)) {
	  for (int i=0; i<MTYPE_num_pregs(mtype); i++) {
		(void) New_PREG_explicit (scope_tab, level, preg_idx2);
                Set_PREG_name_idx ((*scope_tab[level].preg_tab)[preg_idx2], 0);
	  }
	} else {
	  switch (mtype) {
	    ...
	    case MTYPE_CQ:
		// reserve space for 7 more pregs
		(void) New_PREG_explicit (scope_tab, level, preg_idx2);
		Set_PREG_name_idx ((*scope_tab[level].preg_tab)[preg_idx2], 0);
		(void) New_PREG_explicit (scope_tab, level, preg_idx2);
		Set_PREG_name_idx ((*scope_tab[level].preg_tab)[preg_idx2], 0);
	    ...
	    case MTYPE_B:
		FmtAssert( false, ("Xtensa doesn't have MTYPE_B registers"));
//		// bool needs 2 pregs cause can have complement predicate tn
//		(void) New_PREG_explicit (scope_tab, level, preg_idx2);
//		Set_PREG_name_idx ((*scope_tab[level].preg_tab)[preg_idx2], 0);
		break;
	...
	return (PREG_NUM) Get_Preg_Num(preg_idx);
}
...
ST::Print (FILE *f, BOOL verbose) const
{
	...
	if (ty_idx != 0 && TY_tylist (ty) != 0) {
	    ...
	    if (flags & PU_OPTIMIZE_FOR_SPACE)	fprintf (f, " optimize_for_space");
	...
	if (base_idx != 0) {
	    ...
	    fprintf (f, "\t\tAddress: %" LLD_FMT "(%s<%d,%d>)  ", offset,
	...
	if (flags) {
	    ...
	    if (flags & ST_INITV_IN_OTHER_ST)	fprintf (f, " st_used_as_initialization");
	    ...
	    if (flags & ST_HAS_NAMED_SECTION)
		fprintf (f, " has_named_section");
	}

	if (flags_ext) {
	  fprintf (f, "\n\t\tEFlags:\t0x%08x", flags_ext);
	  if (flags_ext & ST_IMPLICIT_INDIRECT)
	    fprintf (f, " implicit_indirect");
	  if (flags_ext & ST_EXPLICIT_LITERAL_REF)
	    fprintf (f, " explicit_literal_ref");
	  if (flags_ext & ST_EXTERNAL_INLINE_FUNC)
	    fprintf (f, " external_inline_func");
	}
...
{
    fprintf (f, "\t%6"LLD_FMT"  %-8s\t", ofst, &Str_Table[name_idx]);
...
PU::Print (FILE *f) const
{
    ...
    fprintf (f, ", flags 0x%016"LLX_FMT",\n"
...
{
    fprintf (f, "size %" LLD_FMT ", align %d, flags 0x%04x, section %d, scninfo %d\n",
...
{
    fprintf (f, "0x%x (%s) --> ", st_idx,
	     (ST_class (st_idx) == CLASS_CONST ?
	      "<constant>" : ST_name (st_idx)));
    switch (kind) {
...
ST *MTYPE_TO_PREG_array[MTYPE_MAX];

ST *Int_Preg, *Float_Preg, *Return_Val_Preg, *Tie_Output_Volatile_Preg;

TY_IDX MTYPE_TO_TY_array[MTYPE_MAX];

TY_IDX Quad_Type, Void_Type, FE_int_Type, FE_double_Type;
TY_IDX Spill_Int_Type, Spill_Float_Type;
TY_IDX Tie_Output_Volatile_Type;
TY_IDX Tie_Output_Struct_Type;
...
Create_All_Preg_Symbols ()
{
	...
	if (MTYPE_To_PREG (i) != NULL)
	    continue;
	if (MTYPE_is_tie(i)==false && MTYPE_is_xtbool(i)==false &&
	    MTYPE_byte_size(i) < 4) {
    ...
    if (Tie_Output_Volatile_Preg == NULL) {

	TY_IDX ty_idx;
	TY &ty=New_TY(ty_idx);
	TY_Init(ty, 0, KIND_SCALAR, MTYPE_I4, Save_Str("tie_output_preg_type"));
	Set_TY_is_volatile(ty_idx);
	Tie_Output_Volatile_Type = ty_idx;

	ST *st = New_ST (GLOBAL_SYMTAB);
	ST_Init (st, Save_Str (".preg_tie_output"),
		 CLASS_PREG, SCLASS_REG, EXPORT_LOCAL, ty_idx);
	Tie_Output_Volatile_Preg = st;
    }
...
Set_up_all_preg_symbols ()
{
	...
	if (Tie_Output_Volatile_Preg == NULL) {
	    if (strcmp (ST_name (&st), ".preg_tie_output") == 0) {
		Tie_Output_Volatile_Preg = &st;
		Tie_Output_Volatile_Type = ST_type(st);
		continue;
	    }
	}
...
Create_Special_Global_Symbols ()
{
	...
	if (i > MTYPE_CORE_LAST) {
	  int bit_size = MTYPE_byte_size(i)*8;
	  int alignment = MTYPE_alignment(i)*8;
	  int rounded_size = ((bit_size + alignment - 1) / alignment) * alignment;
	  rounded_size = rounded_size / 8;
	  TY_Init (ty, rounded_size, ty_kind, i,
		 Save_Str (MTYPE_name (i)));
	} else {
	  TY_Init (ty, MTYPE_byte_size (i), ty_kind, i,
		 Gen_predef_type_name (MTYPE_name (i)));
	}
    ...
    // now create a struct TY of 20 integer field to hold TIE intrinsic op
    // output up to 20 results
    TY &ty = New_TY (Tie_Output_Struct_Type);
    TY_Init (ty, 4, KIND_SCALAR, MTYPE_M, Save_Str("tie_output_struct_type"));
#if 0
    TY_Init (ty, 80, KIND_STRUCT, MTYPE_M, Save_Str("tie_output_struct_type"));

    for (int i=0; i<20; i++) {
      FLD_HANDLE fld = New_FLD();
      char buf[256];
      sprintf(buf,"Tie_Output_Fld_%d", i);
      FLD_Init (fld, Save_Str(buf), MTYPE_To_TY(MTYPE_I4), 4*i);
      if (i==0)
        Set_TY_fld (ty, fld);
      if (i==19)
...
Initialize_Special_Global_Symbols ()
{
	...
	if (MTYPE_byte_size (mtype) != TY_size (ty)) {
	  if (!MTYPE_is_tie(mtype) && !MTYPE_is_xtbool(mtype))
	    continue;
	  int bit_size = MTYPE_byte_size(mtype)*8;
	  int alignment = MTYPE_alignment(mtype)*8;
	  int rounded_size = ((bit_size + alignment - 1) / alignment) * alignment;
	  rounded_size = rounded_size / 8;
	  if (rounded_size!=TY_size(ty))
	    continue;
	}
	switch (mtype) {
	case MTYPE_V:
