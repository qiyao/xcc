...
inline BOOL
ST_initv_in_other_st (const ST& s)	{ return s.flags & ST_INITV_IN_OTHER_ST;}
inline void
Set_ST_initv_in_other_st (ST& s)	{ s.flags |= ST_INITV_IN_OTHER_ST; }
inline void
Clear_ST_initv_in_other_st (ST& s)	{ s.flags &= ~ST_INITV_IN_OTHER_ST; }
...
// Extended ST flags
inline BOOL
ST_has_named_ro_section (const ST* s)	{ return s->flags_ext & ST_HAS_NAMED_RO_SECTION; }
inline void
Set_ST_has_named_ro_section (ST* s)	{ s->flags_ext |= ST_HAS_NAMED_RO_SECTION; }
inline void
Clear_ST_has_named_ro_section (ST* s)	{ s->flags_ext &= ~ST_HAS_NAMED_RO_SECTION; }

inline BOOL
ST_is_switchjump (const ST* s)	{ return s->flags_ext & ST_SWITCHJUMP; }
inline void
Set_ST_is_switchjump (ST* s)	{ s->flags_ext |= ST_SWITCHJUMP; }
inline void
Clear_ST_is_switchjump (ST* s)	{ s->flags_ext &= ~ST_SWITCHJUMP; }


inline BOOL
ST_implicit_indirect (const ST* s)	{ return s->flags_ext & ST_IMPLICIT_INDIRECT; }
inline void
Set_ST_implicit_indirect (ST* s)	{ s->flags_ext |= ST_IMPLICIT_INDIRECT; }
inline void
Clear_ST_implicit_indirect (ST* s)	{ s->flags_ext &= ~ST_IMPLICIT_INDIRECT; }

inline BOOL
ST_explicit_literal_ref (const ST* s)	{ return s->flags_ext & ST_EXPLICIT_LITERAL_REF; }
inline void
Set_ST_explicit_literal_ref (ST* s)	{ s->flags_ext |= ST_EXPLICIT_LITERAL_REF; }
inline void
Clear_ST_explicit_literal_ref (ST* s)	{ s->flags_ext &= ~ST_EXPLICIT_LITERAL_REF; }

inline BOOL
ST_suppress_asm_type(ST* s)             { return s->flags_ext & ST_SUPPRESS_ASM_TYPE; }
inline void
Set_ST_suppress_asm_type (ST* s)	{ s->flags_ext |= ST_SUPPRESS_ASM_TYPE; }
inline void
Clear_ST_suppress_asm_type (ST* s)	{ s->flags_ext &= ~ST_SUPPRESS_ASM_TYPE; }


inline BOOL
ST_dont_prop(ST* s)                     { return s->flags_ext & ST_DONT_PROP; }
inline void
Set_ST_dont_prop (ST* s)	        { s->flags_ext |= ST_DONT_PROP; }
inline void
Clear_ST_dont_prop (ST* s)	        { s->flags_ext &= ~ST_DONT_PROP; }

inline BOOL
ST_used_string(ST* s)                   { return s->flags_ext & ST_USED_STRING; }
inline void
Set_ST_used_string (ST* s)	        { s->flags_ext |= ST_USED_STRING; }
inline void
Clear_ST_used_string (ST* s)	        { s->flags_ext &= ~ST_USED_STRING; }

inline BOOL
ST_is_external_inline_func(ST* s)              { return s->flags_ext & ST_EXTERNAL_INLINE_FUNC; }
inline void
Set_ST_is_external_inline_func (ST* s)	        { s->flags_ext |= ST_EXTERNAL_INLINE_FUNC; }
inline void
Clear_ST_is_external_inline_func (ST* s)	{ s->flags_ext &= ~ST_EXTERNAL_INLINE_FUNC; }
...
inline BOOL
PU_optimize_for_space (const PU& pu)	{ return (pu.flags & PU_OPTIMIZE_FOR_SPACE) != 0; }
inline void
Set_PU_optimize_for_space (PU& pu)	{ pu.flags |= PU_OPTIMIZE_FOR_SPACE; }
inline void
Clear_PU_optimize_for_space (PU& pu)	{ pu.flags &= ~PU_OPTIMIZE_FOR_SPACE; }
...
inline STR_IDX
ST_ATTR_ro_data_section_name (const ST_ATTR& st_attr)
{
    Is_True (st_attr.kind == ST_ATTR_RO_DATA_SECTION_NAME,
	     ("attribute is not for an rodata section name"));
    Is_True (ST_sym_class(*ST_ptr(ST_ATTR_st_idx(st_attr))) == CLASS_FUNC,
	     ("attribute must be on a function"));
  return st_attr.u.section_name;
}

inline void
Set_ST_ATTR_ro_data_section_name (ST_ATTR& st_attr, STR_IDX name)
{
    Is_True (ST_sym_class(*ST_ptr(ST_ATTR_st_idx(st_attr))) == CLASS_FUNC,
	     ("attribute must be on a function"));
    st_attr.kind = ST_ATTR_RO_DATA_SECTION_NAME;
    st_attr.u.section_name = name;
}

inline mUINT32
ST_ATTR_fill (const ST_ATTR& st_attr)
{
    Is_True (st_attr.kind == ST_ATTR_FILL,
	     ("attribute is not for a fill"));
  return st_attr.u.value;
}

inline void
Set_ST_ATTR_fill (ST_ATTR& st_attr, mUINT32 value)
{
    st_attr.kind = ST_ATTR_FILL;
    st_attr.u.value = value;
}
