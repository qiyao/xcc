...
enum ST_SCLASS
{
    ...
    SCLASS_EH_DESC	= 13,		// eh descriptor
    SCLASS_EH_REGION	= 14,		// eh region
    SCLASS_EH_REGION_SUPP = 15,		// eh region supp
    SCLASS_DISTR_ARRAY	= 16,		// distributed array
    SCLASS_COMMENT	= 17,		// comment section
    SCLASS_THREAD_PRIVATE_FUNCS	= 18, // thread-private constr/destr funcs
    SCLASS_LITERAL_POOL = 19,	      // per-function shareable literal pool for xtensa.
    SCLASS_MERGE_STRING	= 20,		// (linker allocated mergeable string)
    SCLASS_COUNT	= 21		// total number of classes
...
enum ST_FLAGS
{
    ...
    ST_INITV_IN_OTHER_ST= 0x00000100,	// CLASS_VAR: represents datapool
...
enum ST_FLAGS_EXT
{
    ST_IMPLICIT_INDIRECT    = 0x01,       // used only by g++fe, access to the symbol
                                          // must assume an implicit indirection
    ST_EXPLICIT_LITERAL_REF = 0x02,       // any literal referencing the symbol must
                                          // be used only to reference the symbol and not
                                          // adjacent objects
    ST_SUPPRESS_ASM_TYPE    = 0x04,       // don't output a .type directive for this symbol
    ST_HAS_NAMED_RO_SECTION = 0x08,       // text ST with named ro_data section
    ST_DONT_PROP            = 0x10,       // don't forward substitute until mainopt
    ST_SWITCHJUMP           = 0x20,       // Cisco style jump table 
    ST_USED_STRING          = 0x40,       // a used, merge string
    ST_EXTERNAL_INLINE_FUNC = 0x80        // an interface only PU, can be deleted if not inlined
};
...
enum ST_ATTR_KIND
{
    ...
    ST_ATTR_RO_DATA_SECTION_NAME = 3,	// valid only for ST's that represent
                                        // functions. rodata generated by the 
                                        // compiler goes here
    ST_ATTR_FILL 		 = 4	
};
...
#define PU_OPTIMIZE_FOR_SPACE	0x0000000800000000LL 
						// few cycles are spent in PU 
...
extern DLL_SHARED SCOPE		*Scope_tab;
...
extern DLL_SHARED FILE_INFO	File_info;
extern DLL_SHARED PU_TAB	Pu_Table;
extern DLL_SHARED SYMBOL_TABLE	St_Table;
extern DLL_SHARED TY_TAB	Ty_tab;
extern DLL_SHARED TYPE_TABLE	Ty_Table;
extern DLL_SHARED FLD_TAB	Fld_Table;
extern DLL_SHARED TYLIST_TAB	Tylist_Table;
extern DLL_SHARED ARB_TAB	Arb_Table;
extern DLL_SHARED STRING_TABLE  Str_Table;
extern DLL_SHARED TCON_TAB	Tcon_Table;
extern DLL_SHARED INITV_TAB	Initv_Table;
extern DLL_SHARED INITO_TABLE	Inito_Table;
extern DLL_SHARED PREG_TABLE	Preg_Table;
extern DLL_SHARED ST_ATTR_TABLE	St_Attr_Table;
extern DLL_SHARED LABEL_TABLE	Label_Table;
// some BLK_TAB items are really local, but make global to ease management
extern DLL_SHARED BLK_TAB	Blk_Table;

// global variables

extern DLL_SHARED SYMTAB_IDX Current_scope;
extern DLL_SHARED PU* Current_pu;
