...
ST_Verify_Flags (const ST &s)
{
  ...
  if (ST_is_initialized (s)) {
    ...
    Is_True(ST_sclass(s) == SCLASS_PSTATIC ||
	    ...
	    ST_sclass(s) == SCLASS_LITERAL_POOL ||
...
ST_Verify_Fields(const ST &s) 
{
  ...
  case CLASS_UNK:
    ...
    break;
  case CLASS_PREG:
    Is_True( ((&s == Return_Val_Preg) || (&s == Tie_Output_Volatile_Preg) ||
  ...
  if ( ST_base_idx(s) != ST_st_idx(s)) {
	...
	Is_True( ST_storage_class(s) == ST_storage_class(*sb) ||
		 ((ST_sym_class(s) == CLASS_FUNC) &&
		  (ST_storage_class(s) == SCLASS_EXTERN &&
		   ST_storage_class(*sb) == SCLASS_TEXT)),
...
void INITO::Verify(UINT level) const 
{
  ...
  Is_True(ST_is_initialized (St_Table[st_idx]) ||
	  ST_is_switchjump(&(St_Table[st_idx])),
	   ("ST_IS_INITIALIZED or ST_SWITCHJUMP not set"));
...
void INITV::Verify(UINT i) const
{
#ifdef Is_True_On
