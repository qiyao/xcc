...
#define WTYPE_base_type(w) (((w)<=MTYPE_CORE_LAST)?(WINFO[w].base_type):UNKNOWN_TYPE)
#define WTYPE_comp_type(w) (((w)<=MTYPE_CORE_LAST)?(WINFO[w].comp_type):UNKNOWN_TYPE)
#define WTYPE_size(w)      (((w)<=MTYPE_CORE_LAST)?(WINFO[w].size):0)
...
WN *WN_CreateAgoto(WN *addr)
{
  ...
  Is_True(WN_rtype(addr)==Pointer_type || WN_operator(addr) == OPR_INTCONST,
		 ...
		 UINT field_id)
{
  ...
  if ( FE_Cvtl_Opt ) {
    ...
    if (    (    (    WN_operator(value) == OPR_CVTL
...
#ifndef TARG_XTENSA
         // [IU]4[IU]8CVT must be explicit for emulation
      ...
      WN* value_kid0 = WN_kid0(value);
      if (WN_operator(value)==OPR_CVTL &&
	  (WN_operator(value_kid0)==OPR_INTRINSIC_OP &&
	   INTRN_is_tie_intrinsic(WN_intrinsic(value_kid0)) ||
	  (WN_operator(value_kid0)==OPR_CVT &&
	   MTYPE_is_tie(WN_desc(value_kid0))))) {
      } else
	value = WN_kid0(value);
    }
  }
...
WN *WN_CopyNode (const WN* src_wn)
{
    ...
    WN_set_bit_offset_size(wn, WN_bit_offset(src_wn), WN_bit_size(src_wn));
...
#ifdef BACK_END
/*
**	If Cur_PU_Feedback and the WN has an id, the map will
**	always exist (though it might be incorrect).
**
**	Validate the feedback value when tracing is on by use of this special value
**	(unfortunately it is a valid value, though incredibly unlikely)
*/
BOOL Valid_Feedback(const WN *wn)
{
  if (wn==NULL)
    return FALSE;

  FmtAssert((OPCODE_is_scf(WN_opcode(wn)) || OPCODE_is_stmt(WN_opcode(wn))),
	    ("expected statement level wn"));
  if (Cur_PU_Feedback)
  {
    if (WN_has_map_id(wn))
    {
#ifdef Is_True_On
      INT32 freq = WN_Get_Feedback(wn);
      if (freq == WN_FB_PROBABLY_UNINITIALIZED	&&
	  Get_Trace(TP_FEEDBACK, TP_FEEDBACK_WN))
      {
	DevWarn("Valid_Feedback: Feedback probably missing %d\n",
...
INT32 WN_Get_Feedback(const WN *wn)
{
  if (wn)
  {
    FmtAssert((OPCODE_is_scf(WN_opcode(wn)) || OPCODE_is_stmt(WN_opcode(wn))),
    ...
    if (Cur_PU_Feedback && WN_has_map_id(wn))
      return WN_MAP32_Get(WN_MAP_FEEDBACK, wn);
  }
  return (INT32) 0;
}

void WN_Set_Feedback(WN *wn, INT32 freq)
{
  if (Cur_PU_Feedback)
    WN_MAP32_Set( WN_MAP_FEEDBACK, wn, freq);
}
#endif /* BACK_END */
...
WN *WN_CreateLoopInfo (WN *induction, WN *trip, UINT16 trip_est, UINT16 depth,
		       INT32 flags, UINT16 at_region_id)
{
  WN *wn;
  INT16 nkids = 0;
  ...
  WN_loop_at_region_id(wn) = at_region_id;
			...
			INTRINSIC intrinsic, INT32 n, WN *kids[])
{
   ...
   if (!call) {
    ...
    /* Parent pointer (if it exists) for returned node must be NULL */
    if (WN_SimpParentMap) WN_MAP_Set(WN_SimpParentMap, call, NULL);
  }
...
WN *WN_Inverse(TYPE_ID type, WN *tree)
{
  ...
  */
  if (MTYPE_float(type))
  {
    if (MTYPE_is_quad(type)==FALSE && MTYPE_RECIP_ALLOWED(type))
    {
...
WN_CreateTn( TN *tn )
{
  WN *wn = WN_Create(OPR_TN,MTYPE_V,MTYPE_V,0);
  WN_tn(wn) = tn;
  return wn;
}

WN *
WN_CreateOutpart( WN *intr_out, INT part, TYPE_ID rtype )
{
  WN *wn = WN_Create(OPR_OUTPART,rtype,MTYPE_V,1);
  WN_kid0(wn) = intr_out;
  WN_outpart(wn) = part;
  return wn;
}
