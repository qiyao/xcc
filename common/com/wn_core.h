...
#define MAX_FIELD_ID 0xfffffffe
#define UNKNOWN_FIELD_ID 0xffffffff
...
public:
   ...
   struct {
      ...
      TYPE_ID           rtype       : 7;  /* result */
      mINT16            kid_count   :10; /* gives kid_count for free */
      TYPE_ID           desc        : 7;  /* descriptor type */
      INT32             map_id;
      UINT		field_id;   /* field_id or (bit_offset:7,bit_size:7) */
   } common;
...
//friend inline TY_IDX&     WN_loadx_addr_ty (WN *);
  ...
  friend inline UINT32&     WN_arith_flag (WN *);
  friend inline UINT32      WN_arith_flag (const WN *);
  ...
  friend inline UINT32&     WN_barrier_flag (WN *);
  friend inline UINT32      WN_barrier_flag (const WN *);
...
//friend inline INT32&      WN_num_dim (WN *);
  ...
  friend inline UINT16      WN_loop_flag (const WN *);
  friend inline UINT16&     WN_loop_flag (WN *);
  friend inline UINT16      WN_loop_at_region_id (const WN *);
  friend inline UINT16&     WN_loop_at_region_id (WN *);
  ...
  friend inline TN*&	    WN_tn(WN *);
  friend inline INT32&      WN_outpart(WN *);
...
inline UINT32 WN_arith_flag (const WN *wn) { return wn->u1u2.uu.ua.arith_flag; }
inline UINT32& WN_arith_flag (WN *wn) { return wn->u1u2.uu.ua.arith_flag; }
...
inline UINT32 WN_barrier_flag (const WN *wn) { return wn->u1u2.uu.ua.barrier_flag; }
inline UINT32& WN_barrier_flag (WN *wn) { return wn->u1u2.uu.ua.barrier_flag; }
...
inline UINT16 WN_loop_flag (const WN* wn) { return wn->u1u2.uu.ub.li.loop_flags; }
inline UINT16& WN_loop_flag (WN* wn) { return wn->u1u2.uu.ub.li.loop_flags; }
inline UINT16 WN_loop_at_region_id (const WN* wn) { return wn->u1u2.uu.ub.li.at_region_id; }
inline UINT16& WN_loop_at_region_id (WN* wn) { return wn->u1u2.uu.ub.li.at_region_id; }
...
inline UINT  	  WN_field_id (const WN* wn) { return((UINT)(wn->common.field_id)); }
inline void	  WN_set_field_id (WN* wn, UINT n) { wn->common.field_id = (n > MAX_FIELD_ID) ? UNKNOWN_FIELD_ID : n; }
inline UINT	  WN_bit_offset (const WN* wn) { return ((UINT)(wn->common.field_id & 0x3fff)) >> 7; }
inline UINT	  WN_bit_size (const WN* wn) { return wn->common.field_id & 0x7f; }
inline void	  WN_set_bit_offset_size (WN* wn, UINT ofst, UINT siz) { wn->common.field_id = ((ofst << 7) + siz); }
...
inline TN *&WN_tn(WN *wn) { return wn->u1u2.uu.ua.tn; }
inline INT32& WN_outpart(WN *wn) { return wn->u1u2.uu.ua.outpart; }
...
#define WN_arith_flag(x)        ((x)->u1u2.uu.ua.arith_flag)
...
#define WN_barrier_flag(x)      ((x)->u1u2.uu.ua.barrier_flag)
...
#define WN_loop_flag(x)	        ((x)->u1u2.uu.ub.li.loop_flags)
#define WN_loop_at_region_id(x)	((x)->u1u2.uu.ub.li.at_region_id)
...
#define WN_field_id(x)		((UINT)((x)->common.field_id))
#define WN_set_field_id(x,y)	((x)->common.field_id = (y > MAX_FIELD_ID) ? UNKNOWN_FIELD_ID : y)
#define WN_bit_offset(x)	(((UINT)((x)->common.field_id & 0x3fff)) >> 7)
#define WN_bit_size(x)		((x)->common.field_id & 0x7f)
#define WN_set_bit_offset_size(x,y,z) ((x)->common.field_id = ((y << 7) + z))
...
#define WN_tn(x) ((x)->u1u2.uu.ua.tn)
#define WN_outpart(x) ((x)->u1u2.uu.ua.outpart)
...
/*REFERENCED*/
#ifdef _WIN32
/* Work around MinGW gcc bug with jump tables in inline functions */ 
static
#else
inline 
#endif
UINT32 WN_flag(const WN *wn)
{
  ...
  case OPR_MPY:
    return (WN_arith_flag(wn));
  case OPR_FORWARD_BARRIER:
  case OPR_BACKWARD_BARRIER:
    return (WN_barrier_flag(wn));
  default:
...
void WN_set_flag(WN *wn, UINT32 flag)
{
  ...
  case OPR_ASM_STMT:
    ...
    break;
  case OPR_MPY:
    WN_arith_flag(wn) = flag;
    break;
  case OPR_FORWARD_BARRIER:
  case OPR_BACKWARD_BARRIER:
    WN_barrier_flag(wn) = flag;
    break;
  default:
...
/* Loop has a non-zerio trip count. */
...
/* Loop originates from a DO-WHILE. */
#define WN_Loop_Dowhile(x)		(WN_loop_flag(x) & WN_LOOP_DOWHILE)
#define WN_Set_Loop_Dowhile(x)  	(WN_loop_flag(x) |= WN_LOOP_DOWHILE)
#define WN_Reset_Loop_Dowhile(x)	(WN_loop_flag(x) &= ~WN_LOOP_DOWHILE)
...
/* Macros for OPR_{FORWARD|BACHWARD}_BARRIER flag access */

#define WN_BARRIER_NO_REORDER_MEMORY	0x0001
#define WN_BARRIER_FLUSH_MEMORY		0x0002
#define WN_BARRIER_NO_REORDER		0x0004
#define WN_BARRIER_FLUSH		0x0008

#define WN_Barrier_No_Reorder_Memory(x)	(WN_barrier_flag(x) & WN_BARRIER_NO_REORDER_MEMORY)
#define WN_Set_Barrier_No_Reorder_Memory(x) (WN_barrier_flag(x) |= WN_BARRIER_NO_REORDER_MEMORY)
#define WN_Barrier_Flush_Memory(x)	(WN_barrier_flag(x) & WN_BARRIER_FLUSH_MEMORY)
#define WN_Set_Barrier_Flush_Memory(x)	(WN_barrier_flag(x) |= WN_BARRIER_FLUSH_MEMORY)
#define WN_Barrier_No_Reorder(x)	(WN_barrier_flag(x) & WN_BARRIER_NO_REORDER)
#define WN_Set_Barrier_No_Reorder(x)	(WN_barrier_flag(x) |= WN_BARRIER_NO_REORDER)
#define WN_Barrier_Flush(x)		(WN_barrier_flag(x) & WN_BARRIER_FLUSH)
#define WN_Set_Barrier_Flush(x)		(WN_barrier_flag(x) |= WN_BARRIER_FLUSH)

/* Macros for arithmetic OPR flag access */

#define WN_MPY_16BIT            0x0001
#define WN_MPY_16BIT_SET        0x0002
#define WN_MPY_16BIT_SIGNED     0x0004
	
#define WN_Mpy_16Bit(x)			(WN_arith_flag(x) & WN_MPY_16BIT)
#define WN_Set_Mpy_16Bit(x)		(WN_arith_flag(x) |= WN_MPY_16BIT)
#define WN_Reset_Mpy_16Bit(x)		(WN_arith_flag(x) &= ~WN_MPY_16BIT)
#define WN_Mpy_16Bit_Set(x)		(WN_arith_flag(x) & WN_MPY_16BIT_SET)
#define WN_Set_Mpy_16Bit_Set(x)		(WN_arith_flag(x) |= WN_MPY_16BIT_SET)
#define WN_Reset_Mpy_16Bit_Set(x)	(WN_arith_flag(x) &= ~WN_MPY_16BIT_SET)
#define WN_Set_Mpy_16Bit_Signed(x)      (WN_arith_flag(x) |= WN_MPY_16BIT_SIGNED)
#define WN_Set_Mpy_16Bit_Unsigned(x)    (WN_arith_flag(x) &= ~WN_MPY_16BIT_SIGNED) 
#define WN_Mpy_16Bit_Signed(x)          (WN_arith_flag(x) & WN_MPY_16BIT_SIGNED)
