...
WN_MAP WN_SimpParentMap = (WN_MAP) 0;
...
WN *WN_Simplify_Tree(WN *t, ALIAS_MANAGER *alias_manager)
{
   ...
   /* Save the arith_flags if 't' has any, so we can reattach them
      after simplifying. */

   if (opr == OPR_MPY) {
     arith_flags_opr = opr;
     arith_flags = WN_arith_flag(t);
   }
   ...
   if (result && (WN_operator(result) == arith_flags_opr)) {
     WN_arith_flag(result) = arith_flags;
   }
