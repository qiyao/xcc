...
/* check for valid opcode, e.g. for xtbool types  to avoid assertion */
#define OPC_FROM_OPR(opr,type) (Is_Valid_Core_Opcode_Parts(opr,type,MTYPE_V)? \
				(OPCODE_make_op(opr,type,MTYPE_V)) :	 \
				(OPCODE_UNKNOWN))
#define RELOPC_FROM_OPR(opr,type) (OPCODE_make_op(opr,MTYPE_I4,type))
...
static void show_rule(char * rule) 
{
   if (trace_rules) {
...
inline simpnode SIMPNODE_GetDefinition(simpnode x)
{
  ...
  if (SIMPNODE_operator(x) == OPR_LDID && ST_class(WN_st(x)) == CLASS_PREG) {
    ...
    if (home) {
      TYPE_ID ty = SIMPNODE_rtype(x);
      TYPE_ID home_ty = SIMPNODE_rtype(home);
      if (SIMP_IS_TYPE_FLOATING(ty) == SIMP_IS_TYPE_FLOATING(home_ty) &&
	  SIMP_IS_TYPE_COMPLEX(ty) == SIMP_IS_TYPE_COMPLEX(home_ty))
      x = home;
    }
  }
#else
...
inline BOOL SIMP_Is_Lda(simpnode x)
{
  x = SIMPNODE_GetDefinition(x);
  return    SIMPNODE_operator(x)==OPR_LDA
    || SIMPNODE_operator(x)==OPR_LDA_LABEL;
}
...
static simpnode simp_factor (simpnode k0, simpnode k1, OPERATOR op1,
	...
	(i op1 j) opc2 (i op1 k) -> i op1 (j opc2 k).
        ---------      ---------
           k0             k1
	...
	*/
{
   ...
   if (SIMPNODE_opcode(k0) == opc1 && SIMPNODE_opcode(k1) == opc1) {

      /* 
         Suppose we have only xt_mul16, not xt_mul32.
         Given an expr lhs, and this function simplifies it into rhs as follows, 
         in which lhs has two mul16 MPY and rhs one mul32 MPY. 

            lhs                  rhs
         a*b opc2 a*c =======> a*(b opc2 c)

         The simplication is correct. But there is a catch! If this
         simplication is performed after cg has lowered lhs, which actually
         happens at 

           be/com/wn_lower.cxx:lower_expr:WN_Simplify_Rebuild_Expr_Tree line 6487,

         mul32 MPY is left without being lowered into a call, which will cause 
         isel to fail to find a match and thus cause compiling time assertion failure.

         Solutions: we can either let this simplication happens and re-lower this
         rhs in lower_expr(); or disable this simplication in this function.  
         Disabling has performance advantage, although introducing machine-dependend
         checking on xt_mul16 and xt_mul32; so we disable the simplication!
        */
      extern DLL_SHARED BOOL xt_mac16;
      if ( (!xt_mul32) && (xt_mul16 || xt_mac16 || Enable_HiFi2_Ops) && (op1 == OPR_MPY) &&
#ifdef WN_SIMP_WORKING_ON_WHIRL
	   WN_Mpy_16Bit(k0) &&
	   WN_Mpy_16Bit(k1)
#else
	   (k0->Arith_flags() & WN_MPY_16BIT) &&
	   (k1->Arith_flags() & WN_MPY_16BIT)
#endif
	 ) {
        return NULL;
      }
			  ...
			  BOOL k0const, BOOL k1const)
{
   ...
   if (opc == SIMPNODE_opcode(k0)) {
      ...
      bool do_simp = true;
#ifdef WN_SIMP_WORKING_ON_WHIRL
      if ((SIMPNODE_operator(k0) == OPR_MPY) &&
          !xt_mul32 && WN_Mpy_16Bit(k0)) {
        do_simp = false;
      }
#endif
      if (do_simp) {
...
All of these require Rsqrt_Allowed to generate RSQRT, and
Sqrt_Allowed to generate SQRT.
   ...
   if (op == OPR_RECIP) {
	 ...
	 break;
      ...
      SIMP_DELETE(k0);
              ...
              Sqrt_Split_Allowed &&
              !MTYPE_SQRT_ALLOWED(ty) &&
              MTYPE_RECIP_ALLOWED(ty) &&
              MTYPE_RSQRT_ALLOWED(ty)) {
     SHOW_RULE("  SQRT(x)    RECIP(RSQRT(x)) ");
     r = SIMPNODE_SimpCreateExp1(OPC_FROM_OPR(OPR_RSQRT, ty), k0);
     r = SIMPNODE_SimpCreateExp1(OPC_FROM_OPR(OPR_RECIP, ty), r);
   }
			  ...
			  BOOL k0const, BOOL k1const)
{
   ...
   if (op == OPR_CVT &&
	 ...
	 if (Is_Valid_Core_Opcode_Parts(k0op,dest_type,source_type)) {
	 ...
	 if (k0op == OPR_LDID && MTYPE_is_tie(dest_type)) {
	   newopc = OPCODE_UNKNOWN; /* Suppress for TIE type */
	 }
#ifdef TARG_XTENSA
	 if (MTYPE_is_longlong(dest_type) && MTYPE_size_min(source_type) <= 32) {
	   newopc = OPCODE_UNKNOWN; /* Suppress for long long type */
	 }
#endif
...
#if !defined(TARG_XTENSA) && !defined(EMULATE_LONGLONG)
   /* We do emulate long longs on Xtensa, but EMULATE_LONGLONG is not defined */
   ...
   if (op == OPR_CVT &&
	 ...
	 if (Is_Valid_Core_Opcode_Parts(OPR_CVT,dest_type,source_type)) {
   ...
   if (op == OPR_TRUNC &&
      ...
      source_value_type = get_value_type(k0k0);
      if (convert_precise(source_value_type,inter_type)) {
	 if (Is_Valid_Core_Opcode_Parts(OPR_TRUNC,dest_type,source_type)) {
			 ...
			 simpnode k0, simpnode k1, BOOL k0const, BOOL k1const)
{
   ...
   /* Reassociate (l+ a*c ) + (a*c2) into a*c + a*c2 + l */
   if (reassoc && !issub && SIMPNODE_operator(k0)==OPR_ADD && SIMPNODE_operator(k1)==OPR_MPY) {
      simpnode second_mul = SIMPNODE_kid1(k0);
      if (SIMPNODE_operator(second_mul) == OPR_MPY) {
        if (SIMP_Is_Constant(SIMPNODE_kid1(second_mul)) && SIMP_Is_Constant(SIMPNODE_kid1(k1))) {
	  if (!SIMPNODE_Simp_Compare_Trees(SIMPNODE_kid0(second_mul), SIMPNODE_kid0(k1))) {
	    if (SIMPNODE_rtype(k0) == ty && SIMPNODE_rtype(k1) == ty) {
              r = SIMPNODE_SimpCreateExp2(opc, 
                        SIMPNODE_SimpCreateExp2(opc, k1, second_mul), 
                        SIMPNODE_kid0(k0));
              SIMP_DELETE(k0);
              return r;
            }
   ...
   /* Guard against reassociating 
      (s0 lda + s1 c) + s2 x or s0 x + (s1 lda + s2 c) #2449 Peng. */
   if (num_ops == 3) {
     if (!SIMP_Is_Constant(x[2]) &&
	 (SIMP_Is_Lda(x[0]) && SIMP_Is_Constant(x[1]) ||
	  SIMP_Is_Constant(x[0]) && SIMP_Is_Lda(x[1]))
	 ||
	 !SIMP_Is_Constant(x[0]) &&
	 (SIMP_Is_Lda(x[1]) && SIMP_Is_Constant(x[2]) ||
	  SIMP_Is_Constant(x[1]) && SIMP_Is_Lda(x[2]))) {
       num_ops = 0;
     }
   }
   ...
   if (num_ops != 0) {
	 ...
	 if (num_const == 1 && constant_moved) {
	   /* group constant with LDA #1991, #2499. Peng */
	    if (SIMP_Is_Lda(x[0])) {
	      SHOW_RULE(" s0 lda + s1 x1 + c => (s0 lda + c) + s1 x1 ");
	      if (s[0]) {
		x[0] = SIMPNODE_SimpCreateExp1(negop, x[0]);
	      }
	      r = SIMPNODE_SimpCreateExp2(SELECT_ADD_SUB(s[2]), x[0], x[2]);
	      r = SIMPNODE_SimpCreateExp2(SELECT_ADD_SUB(s[1]), r, x[1]);
	    } else if (SIMP_Is_Lda(x[1])) {
	     SHOW_RULE(" s0 x0 + s1 lda + c => (s1 lda + c) + s0 x0 ");
	      if (s[1]) { 
		x[1] = SIMPNODE_SimpCreateExp1(negop, x[1]);
	      }
	      r = SIMPNODE_SimpCreateExp2(SELECT_ADD_SUB(s[2]), x[1], x[2]);
		       ...
		       simpnode k0, simpnode k1, BOOL k0const, BOOL k1const)
{
   ...
   // Delete noop CVTs that interfere with 16-bit multiplication analysis
   // If we have 16 bit multiplies but not 32
   // replace I4MPY(I4U4CVT(),I4U4CVT()) with I4MPY(,)
   extern DLL_SHARED BOOL xt_mac16;
   if ( (!xt_mul32) && (xt_mul16 || xt_mac16 || xt_hifi2) && 
	ty == MTYPE_I4 && (SIMPNODE_opcode(k0) == OPC_I4U4CVT) &&
			  (SIMPNODE_opcode(k1) == OPC_I4U4CVT)) { 
   ...
   if (Enable_Cfold_Aggressive && Roundoff_Level >= ROUNDOFF_SIMPLE) {
	 ...
	 SIMP_DELETE_TREE(k0);
      } else if (first_op == OPR_RSQRT &&
                 SIMPNODE_Simp_Compare_Trees(k1,SIMPNODE_kid0(k0))==0 &&
                 MTYPE_SQRT_ALLOWED(ty)) {
	 SHOW_RULE("rsqrt(a)*a");
	 r = SIMPNODE_SimpCreateExp1(OPC_FROM_OPR(OPR_SQRT,ty),k1);
	 SIMP_DELETE_TREE(k0);
      } else if (second_op == OPR_RSQRT &&
                 SIMPNODE_Simp_Compare_Trees(k0,SIMPNODE_kid0(k1))==0 &&
                 MTYPE_SQRT_ALLOWED(ty)) {
	 SHOW_RULE("a*rsqrt(a)");
		     ...
		     simpnode k0, simpnode k1, BOOL k0const, BOOL k1const)
{
   ...
   if (k0const && SIMP_IS_TYPE_FLOATING(ty) && !SIMP_IS_TYPE_COMPLEX(ty)) {
	 ...
	 SIMP_DELETE(k0);
	      ...
	      && Div_Split_Allowed && MTYPE_RECIP_ALLOWED(ty)) {
   ...
   if (Roundoff_Level >= ROUNDOFF_SIMPLE) {
	 ...
	 SIMP_DELETE_TREE(k1);
      ...
      if (SIMPNODE_operator(k1)==OPR_SQRT
...
#ifndef TARG_XTENSA
	 /* disable this for XTENSA because we don't have BNOT instruction */
      } else if (c1 == -1) {
	 SHOW_RULE("j^-1");
		       ...
		       simpnode k0, simpnode k1, BOOL k0const, BOOL k1const)
{
   ...
   if (k1const) {
	 ...
	 if (rty != ty && rty != MTYPE_BS &&
	     (op == OPR_LSHR || op == OPR_ASHR) &&
	     SIMP_IS_TYPE_UNSIGNED(rty) &&
	     SIMP_TYPE_SIZE(rty) < SIMP_TYPE_SIZE(SIMP_TYPE(k0))) {
...
simp_eq_neq (OPCODE opc, simpnode k0, simpnode k1, BOOL k0const, BOOL k1const)
{
   ...
   if (k1const && SIMP_IS_TYPE_INTEGRAL(ty)) {
	...
	if (((SIMPNODE_operator(k0k0) == OPR_LSHR) ||
	    ...
	    if (iseq) {
	      r = SIMPNODE_SimpCreateExp2(RELOPC_FROM_OPR(OPR_GE,ty),r,SIMP_INTCONST(ty,0));
	    } else {
	      r = SIMPNODE_SimpCreateExp2(RELOPC_FROM_OPR(OPR_LT,ty),r,SIMP_INTCONST(ty,0));
	    }
		       ...
		       simpnode k0, simpnode k1, BOOL k0const, BOOL k1const)
{
   ...
   */
   ...
   if (SIMP_IS_TYPE_INTEGRAL(ty) && add_fold_ok &&
      ...
      if (c1==1 && op == OPR_LT) {
	 SHOW_RULE("i < j+1");
	 r = SIMPNODE_SimpCreateExp2(RELOPC_FROM_OPR(OPR_LE,ty),k0,SIMPNODE_kid0(k1));
      }
      if (c1==1 && op == OPR_GE) {
	 SHOW_RULE("i >= j+1");
	 r = SIMPNODE_SimpCreateExp2(RELOPC_FROM_OPR(OPR_GT,ty),k0,SIMPNODE_kid0(k1));
      }
      if (c1==-1 && op == OPR_GT) {
	 SHOW_RULE("i > j-1");
	 r = SIMPNODE_SimpCreateExp2(RELOPC_FROM_OPR(OPR_GE,ty),k0,SIMPNODE_kid0(k1));
      }
      if (c1==-1 && op == OPR_LE) {
	 SHOW_RULE("i <= j-1");
	 r = SIMPNODE_SimpCreateExp2(RELOPC_FROM_OPR(OPR_LT,ty),k0,SIMPNODE_kid0(k1));
      }
      if (r) {
	 SIMP_DELETE(SIMPNODE_kid1(k1));
	 SIMP_DELETE(k1);
   ...
   */
   ...
   if (SIMP_IS_TYPE_INTEGRAL(ty) && add_fold_ok &&
      ...
      if (c1==-1 && op == OPR_LT) {
	 SHOW_RULE("j-1 < i");
	 r = SIMPNODE_SimpCreateExp2(RELOPC_FROM_OPR(OPR_LE,ty),SIMPNODE_kid0(k0),k1);
      }
      if (c1==-1 && op == OPR_GE) {
	 SHOW_RULE("j-1 >= i");
	 r = SIMPNODE_SimpCreateExp2(RELOPC_FROM_OPR(OPR_GT,ty),SIMPNODE_kid0(k0),k1);
      }
      if (c1==1 && op == OPR_GT) {
	 SHOW_RULE("j+1 > i");
	 r = SIMPNODE_SimpCreateExp2(RELOPC_FROM_OPR(OPR_GE,ty),SIMPNODE_kid0(k0),k1);
      }
      if (c1==1 && op == OPR_LE) {
	 SHOW_RULE("j+1 <= i");
	 r = SIMPNODE_SimpCreateExp2(RELOPC_FROM_OPR(OPR_LT,ty),SIMPNODE_kid0(k0),k1);
      }
      if (r) {
	 SIMP_DELETE(SIMPNODE_kid1(k0));
	 SIMP_DELETE(k0);
...
static simpnode SIMPNODE_SimplifyExp2_h(OPCODE opc, simpnode k0, simpnode k1) 
{
	...
	**********************************/
	 ...
	 if (k1const && (SIMPNODE_opcode(k0) == opc) &&
	     (SIMP_Is_Constant(SIMPNODE_kid1(k0)) ||
	      SIMP_Is_Lda(SIMPNODE_kid1(k0))) 
	     /* Add link time constant for xcalibur #1991. Peng. */
	     ) {
...
simpnode SIMPNODE_SimplifyExp2(OPCODE opc, simpnode k0, simpnode k1) 
{
   ...
   /* we will not simplify for xtbool or TIE types */
   if (MTYPE_is_xtbool(rtype) || MTYPE_is_xtbool(desc) ||
...
simpnode simp_cvtl(OPCODE opc, INT16 cvtl_bits, simpnode k0)
{
   ...
   switch (opr0) {
    ...
    case OPR_MAX: case OPR_MIN:
      k1 = SIMPNODE_kid0(k0);
      k2 = SIMPNODE_kid1(k0);
      if (SIMPNODE_rtype(k0) == SIMPNODE_rtype(k1) &&
          SIMPNODE_rtype(k0) == SIMPNODE_rtype(k2) &&
          SIMPNODE_desc(k1) == SIMPNODE_desc(k2)) {
        OPERATOR opr1 = SIMPNODE_operator(k1);
        OPERATOR opr2 = SIMPNODE_operator(k2);
        if (opr1 == OPR_ILOAD || (opr1 == OPR_LDID && ST_class(SIMPNODE_st_idx(k1)) != CLASS_PREG)) {
          if (opr2 == OPR_ILOAD || (opr2 == OPR_LDID && ST_class(SIMPNODE_st_idx(k2)) != CLASS_PREG)) {
            TYPE_ID type = SIMPNODE_desc(k1);
	    if (type == MTYPE_I2 || type == MTYPE_U2) {
	      if (cvtl_bits == 16) {
	        SHOW_RULE("CVTL (MIN/MAX SHORT_LOAD)");
                r = k0;
              }
            } else if (type == MTYPE_I1 || type == MTYPE_U1) {
	      if (cvtl_bits == 8) {
                r = k0;
              }
            }
			       ...
			       simpnode k2) 
{
   ...
   if (SIMP_Is_Int_Constant(k0)) {
	...
	const TYPE_ID k_rtype = OPCODE_rtype(k0op);
	if (k_rtype == MTYPE_B && opc_rtype != MTYPE_B) {
	  r = SIMPNODE_SimpCreateExp1(
			OPCODE_make_op(OPR_CVT, opc_rtype, MTYPE_B), r);
        } else if (MTYPE_bit_size(opc_rtype) > 32 && 
	           MTYPE_bit_size(k_rtype) <= 32) {
	  r = SIMPNODE_SimpCreateExp1(
				...
				simpnode addr) 
{
   ...
   if (TY_is_volatile(TY_pointed(load_addr_ty))) return (r);
   if (!SIMPNODE_simp_initialized) SIMPNODE_Simplify_Initialize();
