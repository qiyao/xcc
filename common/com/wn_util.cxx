...
/** $Revision: 1.40 $
*** $Date: 2000/04/06 02:30:07 $
*** $Author: mtibuild $
*** $Source: /isms/cmplrs.src/osprey1.0/common/com/RCS/wn_util.cxx,v $
**/
#ifdef USE_PCH
...
static TY_IDX
field_type (TY_IDX ty_idx, UINT field_id)
{
  UINT cur_field_id = 0;
...
/* The next two routines are copied form be/opt/opt_wn.cxx */

/* Check if a high level type (e.g. a struct type) has any volatility */
static BOOL
TY_includes_volatile(TY_IDX ty)
{
  ...
  if (TY_kind(ty) == KIND_STRUCT) {
	...
	if (TY_includes_volatile(fld_ty))
	  return TRUE;
...
/* Check if a high level pointer type has any volatility */
static BOOL
Pointer_TY_includes_volatile(TY_IDX ty)
{
  if (ty == TY_IDX_ZERO) return FALSE;
  if (TY_is_volatile(ty)) return TRUE;
  return TY_includes_volatile(TY_pointed(ty));
}
...
WN_Is_Volatile_Mem(const WN *wn)
{
  ...
  if (OPCODE_has_1ty(opc) || OPCODE_has_2ty(opc)) {
	...
	OPCODE_operator(opc) == OPR_MSTORE) {
      ...
      return TY_is_volatile(pointed) || TY_includes_volatile(pointed);
    } else if (OPCODE_operator(opc) == OPR_ILOAD ||
	OPCODE_operator(opc) == OPR_MLOAD) {
      TY_IDX pointed = TY_pointed (Ty_Table[WN_load_addr_ty (wn)]);
      DevAssert(pointed, ("TY_pointed of ILOAD/MLOAD type is NULL"));
      return TY_is_volatile(pointed) || TY_includes_volatile(pointed);
    } else {
      return (TY_is_volatile(WN_ty(wn)) || TY_includes_volatile(WN_ty(wn))) ||
	OPCODE_has_2ty(opc) &&
		(TY_is_volatile(WN_load_addr_ty(wn)) ||
		 TY_includes_volatile(WN_load_addr_ty(wn)));
    }
  } else if (OPCODE_operator(opc) == OPR_INTRINSIC_CALL &&
    ...
    if (tie_macro->is_simple_addr_load_store()==false) {

      if (tie_macro->reads_memory() || tie_macro->writes_memory()) {

        // for TIE intrinsic calls which reads or writes memory
	// conservatively assume a volatile pointer passed indicates
	// a volatile memory read or write
        INT num_kids = WN_kid_count(wn);
	for (INT i=0; i<num_kids; i++) {
          WN* parm_wn = WN_kid(wn,i);
	  TY_IDX parm_ty_idx = WN_ty(parm_wn);
	  if (TY_is_volatile(parm_ty_idx))
	    return TRUE;
	}
      }
    }

    int offset_index = tie_macro->simple_addr_load_store_offset_proto_index();
    if (offset_index!= -1) {
	WN* offset_wn = WN_kid0(WN_kid(wn,offset_index));
	if (WN_operator(offset_wn)!=OPR_INTCONST ||
	    WN_const_val(offset_wn)!=0)
	  return FALSE;
    }
    int base_index = tie_macro->simple_addr_load_store_base_proto_index();
    if (base_index== -1)
	  return FALSE;

    WN* parm_wn = WN_kid(wn,base_index);
    TY_IDX parm_ty_idx = WN_ty(parm_wn);
    if (TY_is_volatile(parm_ty_idx))
      return TRUE;

    WN* base_wn = WN_kid0(WN_kid(wn,base_index));
    if (WN_operator(base_wn)==OPR_ADD) {
      WN* kid0 = WN_kid0(base_wn);
    ...
    TY& pointer_ty = Ty_Table[WN_ty (base_wn)];
    if (TY_kind (pointer_ty) != KIND_POINTER)
      return FALSE;
    TY_IDX pointed = TY_pointed (pointer_ty);
    if (pointed==0)
      return FALSE;
    return TY_is_volatile(pointed) || TY_includes_volatile(pointed);
...
// Quick and dirty scheme to handle simple cases of GNU -Wno-returntype
// 1.  If all branches in a top level must execute nested if return values, don't warn
// 2.  Otherwise if there exists a childless return, warn
// return 1 if should warn, 0 otherwise


typedef enum { yes, no, maybe } choices;


// rule 1.  If all branches in a top level if return values, don't warn
static choices WFE_Check_Top_If(WN *wn) 
{
  OPCODE opc = WN_opcode(wn);
  OPERATOR opr = OPCODE_operator(opc);
  if (opc == OPC_BLOCK) {
    for (WN *tmp = WN_first(wn); tmp; tmp = WN_next(tmp)) {
    	choices ch = WFE_Check_Top_If(tmp);
	if (ch == yes) {
          return yes;
        } else if (ch == no) {
          return no;
    ...
    choices ch1, ch2;
    ch1 = WFE_Check_Top_If(WN_then(wn));
    ch2 = WFE_Check_Top_If(WN_else(wn));
    if (ch1 == yes && ch2 == yes) {
      return yes;
    } else if (ch1 == no || ch2 == no) {
      return no;
    } else {
      return maybe;
    }
  } else if (OPCODE_is_non_scf(opc) && (opc != OPC_LABEL)) {
    return no;
  } else if (OPCODE_is_scf(opc)) {
    for (INT i=0; i<WN_kid_count(wn); i++) {
      if (WFE_Check_Top_If(WN_kid(wn,i)) == no) {
        return no;
      }
    }
    return maybe;
...
// rule 2, any childless return statements
static INT Childless_Return(WN *wn)
{
  if (WN_opcode(wn) == OPC_BLOCK) {
    for (WN *tmp=WN_first(wn); tmp ; tmp = WN_next(tmp)) {
	if (Childless_Return(tmp)) {
	  return 1;
	}
    }
  } else if (WN_operator(wn) == OPR_RETURN) {
    return 1;
  } else {
    for (INT kid=0; kid < WN_kid_count(wn); kid++) {
	if (Childless_Return(WN_kid(wn,kid))) {
	  return 1;
	}
    }
  }
...
{
  WN *body = WN_func_body(func_nd);

  // first check for first condition
  if (WFE_Check_Top_If(body) == yes) {
	return 0;
  }
  
  // now check for second cond
  return Childless_Return(body);
}
