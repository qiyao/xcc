...
/* Intrinsic_Last is initialized to the last of the core intrinsic and
   will be incremented when TIE intrinsics are added
*/
UINT32 Intrinsic_Last = INTRINSIC_CORE_LAST;
...
#include "wutil90.h"
  ...
  INTRN_SAVEREGS,		"SAVEREGS",

  INTRN_I4CLZ,			"I4CLZ",
  INTRN_I4CTZ,			"I4CTZ",
  INTRN_UMULSIDI3,		"UMULSIDI3",

  /*
  INTRINSIC_CORE_LAST,		"INTRINSIC_CORE_LAST"

    INTRINSIC_CORE_LAST has the same value as INTRN_SAVEREGS (see wintrinsic.h)
    so this should not have a separate entry. DK
...
get_intrinsic_name ( INTRINSIC opcode )
{
  ...
  if ( ! init_intrinsic ) {
    ...
    for (INT opc = INTRINSIC_FIRST; opc <= INTRINSIC_CORE_LAST; opc++ ) {
  ...
  FmtAssert(opcode>=INTRINSIC_FIRST && opcode<=Intrinsic_Last,
	    ("Illegal intrinsic opcode: %d\n", opcode));

  if (opcode <=INTRINSIC_CORE_LAST)
    return intrinsic_name_table [opcode].name;
  else /* a TIE intrinsic */
    return tie_info->macro_name(Intrinsic_To_Tie_Macro_Id(opcode));
}
