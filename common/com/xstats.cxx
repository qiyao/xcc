...
void
Count_WN_Operator (WN *wn, TYPE_ID rtype, INT32& bbs, INT32& stmts,
		   INT32& calls)
{
	 ...
	 * use the halfway average of 1.5 bb */
	++bbs;
	static BOOL even = TRUE;
	if (even) ++bbs;
	even = !even;
    } else if (opr == OPR_CSELECT) {
	++bbs;
    } else if (opr == OPR_DIV) {
#ifdef TARG_XTENSA
        /* DIVs are almost always turned into calls */
	++bbs;
        ++calls;
	stmts += 3;
#endif
...
{
	Count_WN_Operator (node, WN_rtype (node), *bbs, *stmts,
			   PU_WN_Call_Cnt);
