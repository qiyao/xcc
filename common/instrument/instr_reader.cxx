...
Process_Feedback_File(char *fb_name)
{
  ...
  if ((fp = fopen(fb_name, "rb")) == NULL) {
       profile_error("Unable to open file: %s", fb_name);
  }

  Get_File_Header(fp, fb_name, &fb_hdr);

  /* optimization_flags are now examined per-PU, rather than
     per feedback file, because that allows source files to be compiled
     with different optimization levels.  Observe that we leave this
     field in the header to ease debugging.

  UINT32 flags = Calc_Feedback_Flags();
  if (flags != fb_hdr.opt_flags) {
    ErrMsg(EC_FB_File_Version, fb_name);
  }
  */
  Total_Cycle_Count += FB_FREQ(fb_hdr.total_cycle_count);
	       ...
	       Fb_Hdr& fb_hdr, Pu_Hdr *pu_hdr_table, char *str_table) 
{
  ...
  pu_handle = CXX_NEW(PU_Profile_Handle(entry_name, 
					pu_hdr_entry.pu_checksum, 
					pu_hdr_entry.pu_opt_flags),
		      MEM_pu_nz_pool_ptr);

  pu_handle->cycle_count = pu_hdr_entry.pu_cycle_count;
...
// Given a PU handle, return the optimization flags
INT32
Get_PU_Optimization_Level(PU_PROFILE_HANDLE pu_handle)
{
  return pu_handle->opt_flags;
}
...
Get_Short_Circuit_Profile(PU_PROFILE_HANDLE pu_handle, INT32 id)
{
   FB_Circuit_Vector& Short_Circuit_Table
     = pu_handle->Get_Short_Circuit_Table();
   if (id >= Short_Circuit_Table.size())
     ErrMsg(EC_FB_File_Old, Cur_PU_Name);
...
Fb_Hdr::Byteswap()
{
  byteswap(&fb_version);
  byteswap(&opt_flags);
  byteswap(&fb_profile_offset);
  byteswap(&fb_pu_hdr_offset);
  byteswap(&fb_pu_hdr_ent_size);
  byteswap(&fb_pu_hdr_num);
  byteswap(&fb_str_table_offset);
  byteswap(&fb_str_table_size);
  byteswap(&phase_num);
  byteswap(&total_cycle_count);
  byteswap((INT32*)(&total_cycle_count)+1);
  INT32 tmp = *((INT32*) &total_cycle_count);
  *((INT32*)&total_cycle_count) = *((INT32*)&total_cycle_count+1);
  *((INT32*)&total_cycle_count+1) = tmp;
}

void
Pu_Hdr::Byteswap()
{
  byteswap(&pu_checksum);
  byteswap(&pu_opt_flags);
  byteswap(&pu_name_index);
  byteswap(&pu_file_offset);
  byteswap(&pu_cycle_count);
  byteswap(&pu_inv_offset);
  byteswap(&pu_num_inv_entries);
  byteswap(&pu_br_offset);
  byteswap(&pu_num_br_entries);
  byteswap(&pu_switch_offset);
  byteswap(&pu_switch_target_offset);
  byteswap(&pu_num_switch_entries);
  byteswap(&pu_cgoto_offset);
  byteswap(&pu_cgoto_target_offset);
  byteswap(&pu_num_cgoto_entries);
  byteswap(&pu_loop_offset);
  byteswap(&pu_num_loop_entries);
  byteswap(&pu_scircuit_offset);
  byteswap(&pu_num_scircuit_entries);
  byteswap(&pu_call_offset);
  byteswap(&pu_num_call_entries);
}
...
Get_File_Header(FILE *fp, char *fname, Fb_Hdr *fb_hdr)
{
  ...
  prof_FREAD(fb_hdr, sizeof(Fb_Hdr), 1, fp,
        "Error while reading from: %s", fname);
  
  if (!Same_Byte_Sex) 
    fb_hdr->Byteswap();
  
  /* This will catch cross endian touble as well. */
  if (fb_hdr->fb_version != INSTR_CURRENT)
    ErrMsg(EC_FB_File_Version, fname);
}
		 ...
		 Pu_Hdr *pu_hdr_table)
{
  ...
  prof_FREAD(pu_hdr_table, sizeof(Pu_Hdr), fb_hdr.fb_pu_hdr_num, fp, ERR_READ,
	fname);

  if (!Same_Byte_Sex)
    for (UINT i = 0; i < fb_hdr.fb_pu_hdr_num; i++) 
      pu_hdr_table[i].Byteswap();
}
...
Get_Str_Table(FILE *fp, char *fname, Fb_Hdr& fb_hdr, char *str_table)
{
  ...
  prof_FREAD(str_table, sizeof(char), fb_hdr.fb_str_table_size, fp, ERR_READ,
        fname);
}
                    ...
                    long pu_ofst, FILE *fp, char *fname)
{
  ...
  prof_FREAD (&(Inv_Table.front ()), sizeof(FB_Info_Invoke),
	 pu_hdr_entry.pu_num_inv_entries, fp, ERR_READ, fname);

  if (!Same_Byte_Sex) {
    for (FB_Invoke_Vector::iterator invoke = Inv_Table.begin(); 
         invoke != Inv_Table.end(); 
         ++invoke) {
      invoke->Byteswap();
    }
  }
                    ...
                    long pu_ofst, FILE *fp, char *fname)
{
  ...
  prof_FREAD (&(Br_Table.front ()), sizeof(FB_Info_Branch),
	 pu_hdr_entry.pu_num_br_entries, fp, ERR_READ, fname);

  if (!Same_Byte_Sex) {
    for (FB_Branch_Vector::iterator branch = Br_Table.begin(); 
         branch != Br_Table.end(); 
         ++branch) {
      branch->Byteswap();
    }
                    ...
                    long pu_ofst, FILE *fp, char *fname)
{
  ...
  prof_FREAD (&(targets_vector.front ()), sizeof(INT32),
  ...
  vector<INT32>::iterator target (targets_vector.begin ());
  for (FB_Switch_Vector::iterator first (Switch_Table.begin ());
       first != Switch_Table.end (); 
       ++first, ++target) {
    ...
    prof_FREAD (&(first->freq_targets.front ()), sizeof(FB_FREQ), *target, fp,
                ERR_READ, fname);  
                    ...
                    long pu_ofst, FILE *fp, char *fname)
{
  ...
  prof_FREAD (&(target_vectors.front ()), sizeof(INT32),
  ...
  vector<INT32>::iterator target (target_vectors.begin ());
  for (FB_Switch_Vector::iterator first (Cgoto_Table.begin ());
       first != Cgoto_Table.end (); 
       ++first, ++target) {
                    ...
                    long pu_ofst, FILE *fp, char *fname)
{
  ...
  prof_FREAD (&(Loop_Table.front ()), sizeof (FB_Info_Loop),
	 pu_hdr_entry.pu_num_loop_entries, fp, ERR_READ, fname);

  if (!Same_Byte_Sex) {
    for (FB_Loop_Vector::iterator loop = Loop_Table.begin(); 
         loop != Loop_Table.end(); 
         ++loop) {
      loop->Byteswap();
    }
                    ...
                    long pu_ofst, FILE *fp, char *fname)
{
  ...
  prof_FREAD (&(Scircuit_Table.front ()), sizeof(FB_Info_Circuit),
	 pu_hdr_entry.pu_num_scircuit_entries, fp, ERR_READ, fname);

  if (!Same_Byte_Sex) {
    for (FB_Circuit_Vector::iterator circuit = Scircuit_Table.begin(); 
         circuit != Scircuit_Table.end(); 
         ++circuit) {
      circuit->Byteswap();
    }
  }
                    ...
                    long pu_ofst, FILE *fp, char *fname)
{
  ...
  prof_FREAD (&(Call_Table.front ()), sizeof(FB_Info_Call),
	 pu_hdr_entry.pu_num_call_entries, fp, ERR_READ, fname);

  if (!Same_Byte_Sex) {
    for (FB_Call_Vector::iterator call = Call_Table.begin(); 
         call != Call_Table.end(); 
         ++call) {
      call->Byteswap();
    }
