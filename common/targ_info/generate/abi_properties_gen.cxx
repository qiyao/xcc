...
//
//  $Revision: 1.13 $
//  $Date: 2000/04/06 02:31:55 $
//  $Author: mtibuild $
//  $Source: /isms/cmplrs.src/osprey1.0/common/targ_info/generate/RCS/abi_properties_gen.cxx,v $
...
typedef struct abi {
  ...
  list<ABI_PROPERTY> reg_flags[ISA_MAX_REGCLASS+1][ISA_MAX_REGISTER+1];
				// Register flag properties
  list<ABI_PROPERTY> reg_values[ISA_MAX_REGCLASS+1][ISA_MAX_REGISTER+1];
				// Register value properties
  const char *reg_names[ISA_MAX_REGCLASS+1][ISA_MAX_REGISTER+1];
				// Register names
} *ABI;
...
/////////////////////////////////////
void Reg_Property(ABI_PROPERTY prop, ISA_REGCLASS rc, ...)
/////////////////////////////////////
//  See interface description.
/////////////////////////////////////
{
...
/////////////////////////////////////
void Reg_Names(ISA_REGCLASS rc, INT minreg, INT maxreg, const char **names)
/////////////////////////////////////
//  See interface description.
/////////////////////////////////////
{
...
/////////////////////////////////////
{
  ...
  for (abi_iter = abis.begin(); abi_iter != abis.end(); ++abi_iter) {
	...
	mask |= prop->v;
      ...
      fprintf(cfile, "    0x%0*" LL_FORMAT "x%s,\n",
		     Type_Size(count) / 4,
		     mask,
		     Type_Suffix(count));
    }
    ...
    if (count != 0) {
      fprintf(cfile, "    {\n");
      for (rc = 0; rc <= ISA_MAX_REGCLASS; ++rc) {
	int reg;
	const ISA_REGCLASS_INFO *cinfo
	  = ISA_REGCLASS_Info((ISA_REGCLASS)rc);

	fprintf(cfile, "      /* ISA_REGCLASS_%s */\n",
		       ISA_REGCLASS_INFO_Name(cinfo));
	int cursor = fprintf(cfile, "      {");
	for (reg = 0; reg <= ISA_MAX_REGISTER; ++reg) {
	  unsigned long long mask = 0;
	...
	if (name == NULL) name = ISA_REGCLASS_INFO_Reg_Name(cinfo, reg);
	if (name == NULL) name = "";
  ...
  fprintf(hfile, "\nextern DLL_SHARED const char *ABI_PROPERTIES_ABI_Name(ABI_PROPERTIES_ABI abi);\n");
  fprintf(cfile, "\nconst char *ABI_PROPERTIES_ABI_Name(ABI_PROPERTIES_ABI abi)\n"
		 "{\n"
  ...
  fprintf(hfile, "\ninline const char *ABI_PROPERTY_Reg_Name(\n"
		 ...
		 "  INT reg)\n"
		 "{\n"
		 "  extern DLL_SHARED const ABI_PROPERTIES *ABI_PROPERTIES_target_props;\n"
  ...
  fprintf(hfile, "\n");
  fprintf(hfile, "inline BOOL ABI_PROPERTY_Set(\n"
	  "  ISA_REGCLASS rc,\n"
	  "  INT reg,\n"
	  "  ABI_PROPERTY prop)\n"
	  "{\n");

  fprintf(hfile, "  extern DLL_SHARED const ABI_PROPERTIES *ABI_PROPERTIES_target_props;\n"
