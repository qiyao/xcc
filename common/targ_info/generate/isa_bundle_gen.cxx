...
/////////////////////////////////////
{
  ...
  const char *isa_exec_type_format = "  %3" LLD_FMT ",  /* %s: ";
  ...
  // XTENSA: Because the bundling information is dynamic, always allow a
  // maximum of 64 exec units.
#ifndef TARG_XTENSA
  if (index <= 8) {
  ...
  fprintf (hfile, "\n#ifndef ISA_BUNDLE_MAX_SLOTS");
  fprintf (hfile, "\n#define ISA_BUNDLE_MAX_SLOTS (%d)\n", MAX_SLOTS);
  fprintf (hfile, "\n#endif\n");
  ...
  fprintf(hfile, "\nextern DLL_SHARED const ISA_EXEC_UNIT_PROPERTY ISA_EXEC_unit_prop[];\n");
  fprintf(hfile, "\n");
  ...
  fprintf (hfile, "\ninline ISA_BUNDLE_INFO "
                 ...
                 "{\n"
		 "  extern DLL_SHARED const ISA_BUNDLE_INFO ISA_BUNDLE_info[];\n"
...
/////////////////////////////////////
{
  ...
  if (comp == END) {
    ...
    do {
      ...
      if (comp >= FSLOT) {
	sprintf(pack_comp_type_name_buf,
		pack_comp_type_name[FSLOT],
		/* slot index */ comp - FSLOT);
	sprintf(pack_comp_name_buf,
		pack_comp_name[FSLOT],
		/* slot index */ comp - FSLOT);
      } else {
	strcpy(pack_comp_type_name_buf, pack_comp_type_name[comp]);
	strcpy(pack_comp_name_buf, pack_comp_name[comp]);
      }
...
/////////////////////////////////////
{
  ...
  fprintf(hfile, "\ninline const ISA_BUNDLE_PACK_INFO *ISA_BUNDLE_Pack_Info(void)\n"
		 "{\n"
		 "  extern DLL_SHARED const ISA_BUNDLE_PACK_INFO ISA_BUNDLE_pack_info[];\n"
  ...
  for (i = 0; i < MAX_PACK_COMPS; ++i) {
    ...
    if (i >= FSLOT) {
      ...
      strcpy(pack_comp_name_buf, pack_comp_name[i]);
    }
    fprintf(cfile, "  %2d, /* %s */\n", index, pack_comp_name_buf);
  }
  fprintf(cfile, "};\n");

  fprintf(hfile, "\ninline INT ISA_BUNDLE_Pack_Info_Index(ISA_BUNDLE_PACK_COMP comp)\n"
		 "{\n"
		 "  extern DLL_SHARED const mUINT8 ISA_BUNDLE_pack_info_index[%d];\n"
