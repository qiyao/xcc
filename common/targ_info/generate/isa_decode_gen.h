...
//
//  STATE Create_State(const char *tag, STATE_TYPE stype, int pos, int width)
//	Create a intermediate (non-final) state. <tag> is a short string
//	to uniquely identify the state. The string is used as part
//	of an identifier, so it should only characters legal in an
//	identifier. <stype> identifies the source for the transition
//	value for the state: INST_STATE selects the instruction word,
//	UNIT_STATE selcts the execution unit. <pos> and <width> specify 
//	a bit field in the instruction word or the execution unit that 
//	will determine the next state.
//
//  STATE Final(TOPCODE topcode)
...
//  $Revision: 1.3 $
//  $Date: 2000/04/06 02:32:15 $
//  $Author: mtibuild $
//  $Source: /isms/cmplrs.src/osprey1.0/common/targ_info/generate/RCS/isa_decode_gen.h,v $

#ifndef isa_decode_gen_INCLUDED
#define isa_decode_gen_INCLUDED

#ifdef _KEEP_RCS_ID
static const char isa_decode_gen_rcs_id[] = "$Source: /isms/cmplrs.src/osprey1.0/common/targ_info/generate/RCS/isa_decode_gen.h,v $ $Revision: 1.3 $";
#endif /* _KEEP_RCS_ID */
...
extern STATE Create_State(const char *tag, STATE_TYPE stype, int pos, int width);
