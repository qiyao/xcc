...
//
//  $Revision: 1.7 $
//  $Date: 2000/04/06 02:32:18 $
//  $Author: mtibuild $
//  $Source: /isms/cmplrs.src/osprey1.0/common/targ_info/generate/RCS/isa_enums_gen.cxx,v $
...
static const char * const interface[] = {
  ...
  " *",
  " *   typedef (enum) ISA_ENUMCLASS",
  " *       An enumeration of the enum classes.",
  " *",
  " *   typedef (enum) ISA_ENUMCLASS_VALUE",
  " *       An enumeration of the enum class values.",
  " *",
  " *   typedef (struct) ISA_ENUMCLASS_INFO",
  " *       Contains info about first and last ECV in the EC.",
  " *       The contents are private.",
  " *",
  " *   typedef (struct) ISA_ENUMCLASS_VALUE_INFO",
  " *       Contains info about name and int-value of the ECV.",
  " *       The contents are private.",
  " *",
  " *   const char * ISA_EC_Name (ISA_ENUMCLASS)",
  " *       Returns name of EC.",
  " *",
  " *   ISA_ENUMCLASS_VALUE ISA_EC_First_Value (ISA_ENUMCLASS)",
  " *       Returns the first ECV for the specified EC.",
  " *",
  " *   ISA_ENUMCLASS_VALUE ISA_EC_Last_Value (ISA_ENUMCLASS)",
  ...
  " *",
  " *   const char * ISA_ECV_Name (ISA_ENUMCLASS_VALUE)",
  " *       Returns name of ECV.",
  " *",
  " *   INT ISA_ECV_Intval (ISA_ENUMCLASS_VALUE)",
...
#define FNAME "targ_isa_enums"
  ...
  FILE* hfile = fopen(buf, "wb");
  sprintf (buf, "%s.c", FNAME);
  FILE* cfile = fopen(buf, "wb");
  sprintf (buf, "%s.Exported", FNAME);
  FILE* efile = fopen(buf, "wb");
  ...
  fprintf(hfile, "} ISA_ENUMCLASS_VALUE;\n");

  fprintf(hfile, "\ntypedef struct {\n"
		"  char *name;\n"
		"  ISA_ENUMCLASS_VALUE first;\n"
		"  ISA_ENUMCLASS_VALUE last;\n"
		"} ISA_ENUMCLASS_INFO;\n");
  fprintf(hfile, "extern DLL_SHARED const ISA_ENUMCLASS_INFO ISA_ENUMCLASS_info[];\n");
  fprintf(efile, "ISA_ENUMCLASS_info\n");
  fprintf(cfile, "const ISA_ENUMCLASS_INFO ISA_ENUMCLASS_info[] = {\n");
  for ( iec = all_ec.begin(); iec != all_ec.end(); ++iec) {
  ...
  fprintf(hfile, "\ntypedef struct {\n"
		"  char *name;\n"
		"  INT intval;\n"
		"} ISA_ENUMCLASS_VALUE_INFO;\n");
  fprintf(hfile, "extern DLL_SHARED const ISA_ENUMCLASS_VALUE_INFO ISA_ENUMCLASS_VALUE_info[];\n\n");
  fprintf(efile, "ISA_ENUMCLASS_VALUE_info\n");
  fprintf(cfile, "const ISA_ENUMCLASS_VALUE_INFO ISA_ENUMCLASS_VALUE_info[] = {\n");
  for ( iecv = all_ecv.begin(); iecv != all_ecv.end(); ++iecv) {
  	fprintf(cfile, "\t{ \"%s\",\t%d },\n", iecv->ecv_name, iecv->ecv_int);
  }
  fprintf(cfile, "};\n\n");

  fprintf(hfile, "inline const char * ISA_EC_Name (ISA_ENUMCLASS ec)\n"
		 "{\n"
		 "  return ISA_ENUMCLASS_info[ec].name;\n"
		 "}\n\n");

  fprintf(hfile, "inline ISA_ENUMCLASS_VALUE ISA_EC_First_Value (ISA_ENUMCLASS ec)\n"
		 "{\n"
		 "  return ISA_ENUMCLASS_info[ec].first;\n"
		 "}\n\n");

  fprintf(hfile, "inline ISA_ENUMCLASS_VALUE ISA_EC_Last_Value (ISA_ENUMCLASS ec)\n"
		 "{\n"
		 "  return ISA_ENUMCLASS_info[ec].last;\n"
		 "}\n\n");

  fprintf(hfile, "inline const char * ISA_ECV_Name (ISA_ENUMCLASS_VALUE ecv)\n"
		 "{\n"
		 "  return ISA_ENUMCLASS_VALUE_info[ecv].name;\n"
		 "}\n\n");

  fprintf(hfile, "inline INT ISA_ECV_Intval (ISA_ENUMCLASS_VALUE ecv)\n"
		 "{\n"
		 "  return ISA_ENUMCLASS_VALUE_info[ecv].intval;\n"
		 "}\n\n");
