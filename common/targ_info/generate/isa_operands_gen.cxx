...
//
//  $Revision: 1.37 $
//  $Date: 2000/04/06 02:32:38 $
//  $Author: mtibuild $
//  $Source: /isms/cmplrs.src/osprey1.0/common/targ_info/generate/RCS/isa_operands_gen.cxx,v $
...
struct operand_value_type {
  ...
  ISA_REGSUBCLASS register_subclass;
  ISA_LITCLASS literal_class;
  ISA_ENUMCLASS enum_class;
  bool is_register;
  bool is_signed;
  bool is_pcrel;
...
static const char * const interface[] = {
  ...
  " *       operand info 'oinfo'.",
  " *",
  " *   ISA_OPERAND_USE ISA_OPERAND_INFO_Opnd_Use(",
  " *     const ISA_OPERAND_INFO *oinfo,",
  " *     INT opnd",
  " *   )",
  ...
  " *       operand info 'oinfo'.",
  " *",
  " *   ISA_OPERAND_USE ISA_OPERAND_INFO_Res_Use(",
  " *     const ISA_OPERAND_INFO *oinfo,",
  " *     INT result",
  " *   )",
  " *       Get the operand use type of result 'result' specified by the",
  " *       operand info 'oinfo'.",
  " *",
  ...
  " *       some architectures.",
  " *",
  " *   ISA_REGCLASS ISA_OPERAND_VALTYP_Register_Class(",
  " *     const ISA_OPERAND_VALTYP *otype",
  " *   )",
  " *       Get the register class for the operand specified by 'otype'.",
  " *",
  " *   ISA_REGSUBCLASS ISA_OPERAND_VALTYP_Register_Subclass(",
  " *     const ISA_OPERAND_VALTYP *otype",
  " *   )",
  " *       Get the register subclass for the operand specified by 'otype'.",
  " *",
  " *   ISA_LITCLASS ISA_OPERAND_VALTYP_Literal_Class(const ISA_OPERAND_VALTYP *otype)",
  " *       Get the literal class for the operand specified by 'otype'.",
  " *",
  " *   ISA_ENUMCLASS ISA_OPERAND_VALTYP_Enum_Class(",
  " *     const ISA_OPERAND_VALTYP *otype",
  " *   )",
  ...
  " *",
  " *   BOOL ISA_OPERAND_VALTYP_Is_Continuation(const ISA_OPERAND_VALTYP *otype)",
  " *       Return a boolean to specify if the operand specifed",
  " *       by 'otype' is a continuation of an multireg operand.",
  " *",
  ...
  " *",
  " *   INT TOP_Immediate_Operand(TOP topcode, ISA_LITCLASS *lclass)",
  ...
  " *",
  " *   INT TOP_Relocatable_Operand(TOP topcode, ISA_LITCLASS *lclass)",
...
/////////////////////////////////////
{
  ...
  /* Create an instruction group for TOP_UNDEFINED. */
  OPERANDS_GROUP oper_group = new operands_group;

  cur_oper_group = oper_group;
  oper_group->name = "UNDEFINED";
  ...
  op_groups[(int)TOP_UNDEFINED] = oper_group;
...
/////////////////////////////////////
{
  ...
  result->name = name;
  result->register_class = ISA_REGCLASS_UNDEFINED;
  result->register_subclass = ISA_REGSUBCLASS_UNDEFINED;
...
/////////////////////////////////////
{
  ...
  cur_oper_group->res_use[result_index] = (OPERAND_USE_TYPE)result_use;
}
...
/////////////////////////////////////
{
  ...
  const char *max_operands_limit_name = "ISA_OPERAND_max_operands_limit";
  const char *max_results_limit_name = "ISA_OPERAND_max_results_limit";
  enum {
    FLAG_IS_REG	    = 0x1,
    FLAG_IS_SIGNED  = 0x2,
    FLAG_IS_FPU_INT = 0x4,
    FLAG_IS_PCREL   = 0x8,
    FLAG_IS_CONTINUATION   = 0x10
  };
  ...
  for (err = false, code = 0; code < TOP_count; ++code) {
    if (!op_groups[code] && (code != TOP_UNDEFINED)) {
...
#define FNAME "targ_isa_operands"
  ...
  fprintf(hfile, "\n#define ISA_OPERAND_INFO_flag_is_pcrel\t%d\n", FLAG_IS_PCREL);
  fprintf(hfile, "#define ISA_OPERAND_INFO_flag_is_register\t%d\n", FLAG_IS_REG);
  fprintf(hfile, "#define ISA_OPERAND_INFO_flag_is_signed\t%d\n", FLAG_IS_SIGNED);
  fprintf(hfile, "#define ISA_OPERAND_INFO_flag_is_fpu_int\t%d\n", FLAG_IS_FPU_INT);
  fprintf(hfile, "#define ISA_OPERAND_INFO_flag_is_continuation\t%d\n", FLAG_IS_CONTINUATION);
  ...
  for (ivti = all_operand_types.begin(); ivti != all_operand_types.end(); ++ivti) {
    ...
    const ISA_REGCLASS_INFO *rcinfo = ISA_REGCLASS_Info(val_type->register_class);
    const ISA_REGSUBCLASS_INFO *scinfo = ISA_REGSUBCLASS_Info(val_type->register_subclass);
    flags = 0;
    ...
    if (val_type->is_continuation) flags |= FLAG_IS_CONTINUATION;
    flag_mask |= flags;
    fprintf(cfile, "  { ISA_REGCLASS_%-10s, ISA_REGSUBCLASS_%-10s,\n"
		   "    %3d, %s, %2d, 0x%02x }, /* %s */\n",
		   ISA_REGCLASS_INFO_Name(rcinfo),
		   ISA_REGSUBCLASS_INFO_Name(scinfo),
  ...
  fprintf (hfile, "\nenum {\n"
		  ...
		  "  %s=%d\n"
		  "};\n",
		  max_operands_limit_name, 64,
		  max_results_limit_name, 64);
  ...
  fprintf (hfile, "\ntypedef struct {\n"
		  ...
		  max_results_name, max_results_name);
  ...
  for (ogi = all_groups.begin(); ogi != all_groups.end(); ++ogi) {
    ...
    pos = fprintf(cfile, "  { 0, %d, {", oper_group->opnd_count);
  ...
  fprintf(hfile, "\ninline const ISA_OPERAND_INFO *"
		 ...
		 "{\n"
		 "  extern DLL_SHARED const %s ISA_OPERAND_info_index[];\n"
		 "  extern DLL_SHARED const ISA_OPERAND_INFO ISA_OPERAND_info[];\n"
		 "  INT index = ISA_OPERAND_info_index[(INT)topcode];\n"
		 "  return &ISA_OPERAND_info[index];\n"
		 "}\n",
		 info_index_type);

  fprintf(hfile, "\ninline void *ISA_OPERAND_INFO_Tie_Info("
		   "const ISA_OPERAND_INFO *oinfo)\n"
		 "{\n"
		 "  return oinfo->tie_info;\n"
		 "}\n");
  ...
  fprintf(hfile, "\ninline const ISA_OPERAND_VALTYP *ISA_OPERAND_INFO_Operand(\n"
		 ...
		 "  INT opnd)\n"
		 "{\n"
		 "  extern DLL_SHARED const ISA_OPERAND_VALTYP ISA_OPERAND_operand_types[];\n"
  ...
  fprintf(hfile, "\ninline ISA_REGCLASS ISA_OPERAND_VALTYP_Register_Class(\n"
		 "  const ISA_OPERAND_VALTYP *otype)\n"
		 "{\n"
		 "  return (ISA_REGCLASS)otype->rclass;\n"
		 "}\n");

  fprintf(hfile, "\ninline ISA_REGSUBCLASS ISA_OPERAND_VALTYP_Register_Subclass(\n"
		 "  const ISA_OPERAND_VALTYP *otype)\n"
		 "{\n"
		 "  return (ISA_REGSUBCLASS)otype->rsubclass;\n"
		 "}\n");

  fprintf(hfile, "\ninline ISA_LITCLASS ISA_OPERAND_VALTYP_Literal_Class("
		   "const ISA_OPERAND_VALTYP *otype)\n"
		 "{\n"
		 "  return (ISA_LITCLASS)otype->lclass;\n"
		 "}\n");

  fprintf(hfile, "\ninline ISA_ENUMCLASS ISA_OPERAND_VALTYP_Enum_Class(\n"
		 "  const ISA_OPERAND_VALTYP *otype)\n"
		 "{\n"
		 "  return (ISA_ENUMCLASS)otype->eclass;\n"
		 "}\n");
  ...
  fprintf(hfile, "\ninline BOOL ISA_OPERAND_VALTYP_Is_Continuation("
		   "const ISA_OPERAND_VALTYP *otype)\n"
		 "{\n"
		 "  return (otype->flags & 0x%02x) != 0;\n"
		 "}\n",
  ...
  fprintf(hfile, "\ninline ISA_OPERAND_USE ISA_OPERAND_INFO_Opnd_Use(\n"
		 "  const ISA_OPERAND_INFO *oinfo,\n"
		 "  INT opnd)\n"
		 "{\n"
  ...
  fprintf(hfile, "\ninline ISA_OPERAND_USE ISA_OPERAND_INFO_Res_Use(\n"
		 "  const ISA_OPERAND_INFO *oinfo,\n"
		 "  INT result)\n"
		 "{\n"
		 "  return (ISA_OPERAND_USE)oinfo->ruse[result];\n"
		 "}\n");
  ...
  fprintf(hfile, "\nextern INT TOP_Immediate_Operand(TOP topcode, ISA_LITCLASS *lclass);\n");
  ...
  fprintf(cfile, "\n"
		 ...
		 "    ISA_LITCLASS lit_class = ISA_OPERAND_VALTYP_Literal_Class(vtype);\n"
  ...
  fprintf(hfile, "\nextern INT TOP_Relocatable_Operand(TOP topcode, ISA_LITCLASS *lclass);\n");
  fprintf(efile, "TOP_Relocatable_Operand\n");
  fprintf(cfile, "\nINT TOP_Relocatable_Operand(TOP topcode, ISA_LITCLASS *lclass)\n"
		 "{\n"
		 "  extern DLL_SHARED const mINT8 ISA_OPERAND_relocatable_opnd[];\n"
		 "  INT iopnd = ISA_OPERAND_relocatable_opnd[(INT)topcode];\n"
		 "  if (lclass && iopnd >= 0) {\n"
		 "    const ISA_OPERAND_INFO *opinfo = ISA_OPERAND_Info(topcode);\n"
		 "    const ISA_OPERAND_VALTYP *vtype = ISA_OPERAND_INFO_Operand(opinfo,iopnd);\n"
		 "    *lclass = (ISA_LITCLASS)ISA_OPERAND_VALTYP_Literal_Class(vtype);\n"
		 "  }\n"
		 "  return iopnd;\n"
		 "}\n");
  ...
  fprintf(cfile, "\nINT TOP_Find_Operand_Use(TOP topcode, ISA_OPERAND_USE use)\n"
		 ...
		 "  for (i = 0; i < opnds; ++i) {\n"
		 "    ISA_OPERAND_USE this_use = ISA_OPERAND_INFO_Opnd_Use(oinfo, i);\n"
		 "    if (this_use == use) return i;\n"
		 "  }\n"
		 "  return -1;\n"
		 "}\n");

  fprintf(hfile, "\nextern INT TOP_Find_Result_Use(TOP topcode, "
		 "ISA_OPERAND_USE use);\n");
  fprintf(efile, "TOP_Find_Result_Use\n");
  fprintf(cfile, "\nINT TOP_Find_Result_Use(TOP topcode, ISA_OPERAND_USE use)\n"
		 "{\n"
		 "  INT i;\n"
		 "  const ISA_OPERAND_INFO *oinfo = ISA_OPERAND_Info(topcode);\n"
		 "  INT results = ISA_OPERAND_INFO_Results(oinfo);\n"
		 "  for (i = 0; i < results; ++i) {\n"
		 "    ISA_OPERAND_USE this_use = ISA_OPERAND_INFO_Res_Use(oinfo, i);\n"
		 "    if (this_use == use) return i;\n"
		 "  }\n"
		 "  return -1;\n"
		 "}\n");
  ...
  fprintf(hfile, "\nextern void TOP_Result_Uses(TOP topcode, "
		 "ISA_OPERAND_USE *uses);\n");
  fprintf(efile, "TOP_Result_Uses\n");
  fprintf(cfile, "\nvoid TOP_Result_Uses(TOP topcode, ISA_OPERAND_USE *uses)\n"
		 "{\n"
		 "  INT i;\n"
		 "  const ISA_OPERAND_INFO *oinfo = ISA_OPERAND_Info(topcode);\n"
		 "  INT results = ISA_OPERAND_INFO_Results(oinfo);\n"
