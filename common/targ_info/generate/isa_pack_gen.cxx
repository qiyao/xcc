...
//
//  $Revision: 1.24 $
//  $Date: 2000/04/06 02:32:48 $
//  $Author: mtibuild $
//  $Source: /isms/cmplrs.src/osprey1.0/common/targ_info/generate/RCS/isa_pack_gen.cxx,v $
...
#define FNAME "targ_isa_pack"
  ...
  for (err = false, top = 0; top < TOP_count; ++top) {
    bool is_dummy = TOP_has_property((TOP)top, PROP_dummy);
    bool is_simulated = TOP_has_property((TOP)top, PROP_simulated);
    if (!top_specified[top]) {
      if (!is_simulated && !is_dummy && (top != TOP_UNDEFINED)) {
	...
	err = true;
      ...
      fprintf(stderr, "### Error: pack specification for simulated op %s\n",
		      TOP_Name((TOP)top));
      err = true;
    } else if (top == TOP_UNDEFINED) {
      fprintf(stderr, "### Error: pack specification for TOP_UNDEFINED\n");
      err = true;
    }
  }
  ...
  fprintf(hfile, "\ntypedef %s ISA_PACK_INST;\n",
		 inst_bits > 32 ? "UINT64" : "UINT32");
  ...
  if (inst_words > 1) {
    ...
    for (top = 0; top < TOP_count; ++top ) {
      ...
      int words = op_pack ? op_pack->desc->max_word + 1 
			  : (TOP_has_property((TOP)top, PROP_dummy) ? 0 : 1);
  ...
  fprintf(hfile, "\ninline UINT64 ISA_PACK_Init_Mask(TOP topcode, INT iword)\n"
		 "{\n"
		 "  extern DLL_SHARED const mUINT%d ISA_PACK_init_mask[%d][%d];\n"
  ...
  fprintf(hfile, "\ninline INT ISA_PACK_Inst_Words(TOP topcode)\n"
		 "{\n");
  if (inst_words == 1) {
    fprintf(hfile, "  return TOP_has_property(topcode, PROP_dummy) ? 0 : 1;\n");
  } else {
    fprintf(hfile, "  extern DLL_SHAREd const mUINT8 ISA_PACK_inst_words[%d];\n"
  ...
  fprintf(hfile, "\ninline const ISA_PACK_INFO *ISA_PACK_Info(TOP topcode)\n"
		 "{\n"
		 "  extern DLL_SHARED const %s ISA_PACK_info_index[];\n"
		 "  extern DLL_SHARED const ISA_PACK_INFO ISA_PACK_info[];\n"
  ...
  fprintf(hfile, "\ninline const ISA_PACK_ADJ_INFO *ISA_PACK_Adj_Info(TOP topcode)\n"
		 "{\n"
		 "  extern DLL_SHARED const ISA_PACK_ADJ_INFO ISA_PACK_adj_info[];\n"
		 "  extern DLL_SHARED const mUINT8 ISA_PACK_adj_info_index[];\n"
