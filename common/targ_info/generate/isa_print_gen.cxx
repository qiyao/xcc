 ...
 */
#define MAX_OPNDS ISA_OPERAND_max_operands_limit
#define MAX_RESULTS ISA_OPERAND_max_results_limit
...
#define FNAME "targ_isa_print"
  ...
  for (err = false, top = 0; top < TOP_count; ++top) {
	...
	err = true;
      ...
      fprintf(stderr, "### Error: print specification for TOP_UNDEFINED\n");
      err = true;
    }
  }
  ...
  fprintf(hfile, "\n#define ISA_PRINT_MAX_OPERANDS\t\t%d\n", MAX_OPNDS);
  fprintf(hfile, "#define ISA_PRINT_MAX_RESULTS\t\t%d\n", MAX_RESULTS);
  ...
  fprintf(hfile, "\nextern DLL_SHARED const ISA_PRINT_INFO ISA_PRINT_info[%d];\n",
						list_index + 1);
  ...
  fprintf(hfile, "\nextern DLL_SHARED const unsigned char ISA_PRINT_info_index[%d];\n", TOP_count);
  ...
  if (asmname) {
    ...
    fprintf(hfile, "\ninline const char *ISA_PRINT_AsmName(TOP topcode)\n"
		   "{\n"
		   "  extern DLL_SHARED const char * const ISA_PRINT_asmname[];\n"
