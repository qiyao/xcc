...
struct isa_register_class {
  ...
  list<ISA_REGSUBCLASS> subclasses;
};

static list<ISA_REGCLASS> rclasses; // All the classes
static list<ISA_REGSUBCLASS> subclasses; // All the sub classes
...
static const char * const interface[] = {
  ...
  " *",
  " *   BOOL ISA_REGCLASS_INFO_Multiple_Save(",
  " *     const ISA_REGCLASS_INFO *info",
  " *   )",
  " *       Return a flag that indicates if the registers in the class",
  ...
  " *",
  " *   const char *ISA_REGCLASS_INFO_Name(",
  " *     const ISA_REGCLASS_INFO *info",
  " *   )",
  " *       Return the name of the class described by 'info'.",
  " *",
  " *   const char *ISA_REGCLASS_INFO_Reg_Name(",
  " *     const ISA_REGCLASS_INFO *info,",
  " *     INT reg_index",
  " *   )",
  ...
  " *",
  " *   const ISA_REGSUBCLASS_INFO *ISA_REGSUBCLASS_Info(",
  " *     ISA_REGSUBCLASS sc",
  " *   )",
  " *",
  ...
  " *       by 'info'.",
  " *",
  " *   INT ISA_REGSUBCLASS_INFO_Count(",
  " *     const ISA_REGSUBCLASS_INFO *info",
  " *   )",
  " *",
  ...
  " *       by 'info'.",
  " *",
  " *   UINT ISA_REGSUBCLASS_INFO_Member(",
  " *     const ISA_REGSUBCLASS_INFO *info,",
  " *     INT n",
  " *   )",
  " *",
  ...
  " *",
  " *   const char *ISA_REGSUBCLASS_INFO_Reg_Name(",
  " *     const ISA_REGSUBCLASS_INFO *info,",
  " *     INT index",
  " *   )",
  " *",
...
/////////////////////////////////////
ISA_REGCLASS ISA_Register_Class_Create(
  const char *name,
  int bit_size,
  bool can_store,
  bool multiple_save
...
/////////////////////////////////////
{
  ISA_REGCLASS result = new isa_register_class;
...
/////////////////////////////////////
{
  ISA_REGSUBCLASS result = new isa_register_subclass;
...
/////////////////////////////////////
{
  list <ISA_REGCLASS>::iterator rc_iter;
  list <ISA_REGSUBCLASS>::iterator rsc_iter;
  int i;

  int max_reg = 0;
  for (rc_iter = rclasses.begin(); rc_iter != rclasses.end(); ++rc_iter) {
    ISA_REGCLASS rclass = *rc_iter;
    int class_max = 0;
    int class_min = 0;
...
#define FNAME "targ_isa_registers"
  ...
  if (max_reg>MIN_REGISTER)
    fprintf(hfile, "\n#define ISA_MAX_REGISTER (%d)\n", max_reg);
  else
    fprintf(hfile, "\n#define ISA_MAX_REGISTER (%d)\n", MIN_REGISTER);
  ...
  fprintf(hfile, "\ntypedef enum {\n");
  fprintf(hfile, "  ISA_REGCLASS_UNDEFINED,\n");
  for (rc_iter = rclasses.begin(); rc_iter != rclasses.end(); ++rc_iter) {
    ISA_REGCLASS rclass = *rc_iter;
    fprintf(hfile, "  ISA_REGCLASS_%s,\n", rclass->name);
  }
  fprintf(hfile, "  ISA_MIN_REGCLASS = ISA_REGCLASS_%s,\n",
	  rclasses.front()->name);
  fprintf(hfile, "  ISA_MAX_REGCLASS = ISA_REGCLASS_%s,\n",
	  rclasses.back()->name);
  fprintf(hfile, "  ISA_REGCLASS_COUNT = "
		 "ISA_MAX_REGCLASS - ISA_MIN_REGCLASS + 1\n");

  fprintf(hfile, "} ISA_REGCLASS;\n");

  fprintf(hfile, "\ntypedef mUINT16 mISA_REGCLASS;\n");

  fprintf(hfile, "\n#define FORALL_REGCLASS(cl) \\\n"
		 "\tfor (cl = ISA_MIN_REGCLASS; \\\n"
		 "\t     cl <= ISA_MAX_REGCLASS; \\\n"
		 "\t     cl = (ISA_REGCLASS)(cl + 1))\n");

  fprintf(hfile, "\n#define FORALL_REGCLASS_IN_REVERSE(cl) \\\n"
		 "\tfor (cl = ISA_MAX_REGCLASS; \\\n"
		 "\t     cl >= ISA_MIN_REGCLASS; \\\n"
		 "\t     cl = (ISA_REGCLASS)(cl - 1))\n");
  ...
  fprintf(hfile, "\ntypedef struct {\n"
		 ...
		 "  mBOOL multiple_save;\n"
		 "  const char *name;\n"
		 "  const char *reg_name[ISA_MAX_REGISTER+1];\n"
  		 "} ISA_REGCLASS_INFO;\n");

  fprintf(efile, "ISA_REGCLASS_info\n");
  ...
  for (rc_iter = rclasses.begin(); rc_iter != rclasses.end(); ++rc_iter) {
    ...
    for (reg_iter = rclass->regsets.begin();
	 ...
	 ++reg_iter
      ...
      ISA_REGISTER_SET regset = *reg_iter;
      fprintf(cfile, "  { 0x%02x, %d, %d, %d, %1d, %1d, \"%s\",",
	      regset->isa_mask,
	      regset->min_regnum,
	      regset->max_regnum,
	      rclass->bit_size,
  ...
  fprintf(efile, "ISA_REGCLASS_info_index\n");

  fprintf(cfile, "\nmUINT16 ISA_REGCLASS_info_index[] = {\n");
  fprintf(cfile, "  %d,  /* ISA_REGCLASS_%s */\n", 0, "UNDEFINED");
  int index = 1;
  for (rc_iter = rclasses.begin(); rc_iter != rclasses.end(); ++rc_iter) {
    ISA_REGCLASS rclass = *rc_iter;
    list<ISA_REGISTER_SET>::iterator reg_iter;
  ...
  fprintf(hfile, "\ntypedef enum {\n");
  fprintf(hfile, "  ISA_REGSUBCLASS_UNDEFINED,\n");
  for (rsc_iter = subclasses.begin(); rsc_iter != subclasses.end(); ++rsc_iter) {
    ISA_REGSUBCLASS subclass = *rsc_iter;
      fprintf(hfile, "  ISA_REGSUBCLASS_%s,\n", subclass->name);
    }
  if (subclasses.empty()) {
    fprintf(hfile, "  ISA_MIN_REGSUBCLASS = 1,\n");
    fprintf(hfile, "  ISA_MAX_REGSUBCLASS = 0,\n");
  } else {
    fprintf(hfile, "  ISA_MIN_REGSUBCLASS = ISA_REGSUBCLASS_%s,\n",
	    subclasses.front()->name);
    fprintf(hfile, "  ISA_MAX_REGSUBCLASS = ISA_REGSUBCLASS_%s,\n",
	    subclasses.back()->name);
  }
  fprintf(hfile, "  ISA_REGSUBCLASS_COUNT = "
		 "ISA_MAX_REGSUBCLASS - ISA_MIN_REGSUBCLASS + 1\n");

  fprintf(hfile, "} ISA_REGSUBCLASS;\n");

  fprintf(hfile, "\ntypedef mUINT16 mISA_REGSUBCLASS;\n");

  fprintf(hfile, "\n#define FORALL_REGSUBCLASS(sc) \\\n"
		 "\tfor (sc = ISA_MIN_REGSUBCLASS; \\\n"
		 "\t     sc <= ISA_MAX_REGSUBCLASS; \\\n"
  ...
  fprintf(hfile, "\ntypedef struct {\n"
		 ...
		 "  mUINT16 count;\n"
		 "  mUINT16 members[ISA_MAX_REGISTER+1];\n"
		 "  const char *reg_name[ISA_MAX_REGISTER+1];\n"
  		 "} ISA_REGSUBCLASS_INFO;\n");

  fprintf(efile, "ISA_REGSUBCLASS_info\n");

  fprintf(cfile, "\nconst ISA_REGSUBCLASS_INFO"
		   " ISA_REGSUBCLASS_info[] = {\n");
  fprintf(cfile, "  { \"%s\", ISA_REGCLASS_%s, 0, { 0 }, { 0 } },\n", 
		 "UNDEFINED", "UNDEFINED");
  for (rsc_iter = subclasses.begin(); rsc_iter != subclasses.end(); ++rsc_iter) {
    ISA_REGSUBCLASS subclass = *rsc_iter;
    fprintf(cfile, "  { \"%s\", ISA_REGCLASS_%s, %d,", 
  ...
  fprintf(hfile, "\ninline const ISA_REGCLASS_INFO *ISA_REGCLASS_Info(\n"
		 "  ISA_REGCLASS rc\n"
		 ")\n"
		 "{\n"
		 "  extern DLL_SHARED const ISA_REGCLASS_INFO ISA_REGCLASS_info[];\n"
		 "  extern mUINT16 ISA_REGCLASS_info_index[];\n"
		 "  INT index = ISA_REGCLASS_info_index[(INT)rc];\n"
		 "  return &ISA_REGCLASS_info[index];\n"
		 "}\n");

  fprintf(hfile, "\ninline INT ISA_REGCLASS_INFO_First_Reg(\n"
		 "  const ISA_REGCLASS_INFO *info\n"
		 ")\n"
		 "{\n"
  ...
  fprintf(hfile, "\ninline INT ISA_REGCLASS_INFO_Last_Reg(\n"
		 "  const ISA_REGCLASS_INFO *info\n"
		 ")\n"
		 "{\n"
  ...
  fprintf(hfile, "\ninline INT ISA_REGCLASS_INFO_Bit_Size(\n"
		 "  const ISA_REGCLASS_INFO *info\n"
		 ")\n"
		 "{\n"
  ...
  fprintf(hfile, "\ninline BOOL ISA_REGCLASS_INFO_Can_Store(\n"
		 "  const ISA_REGCLASS_INFO *info\n"
		 ")\n"
		 "{\n"
  ...
  fprintf(hfile, "\ninline BOOL ISA_REGCLASS_INFO_Multiple_Save(\n"
		 "  const ISA_REGCLASS_INFO *info\n"
		 ")\n"
		 "{\n"
  ...
  fprintf(hfile, "\ninline const char *ISA_REGCLASS_INFO_Name(\n"
		 "  const ISA_REGCLASS_INFO *info\n"
		 ")\n"
		 "{\n"
		 "  return info->name;\n"
  ...
  fprintf(hfile, "\ninline const char *ISA_REGCLASS_INFO_Reg_Name(\n"
		 "  const ISA_REGCLASS_INFO *info,\n"
		 "  INT reg_index\n"
		 ")\n"
		 "{\n"
  ...
  fprintf(hfile, "\ninline const ISA_REGSUBCLASS_INFO *ISA_REGSUBCLASS_Info(\n"
		 "  ISA_REGSUBCLASS sc\n"
		 ")\n"
		 "{\n"
		 "  extern DLL_SHARED const ISA_REGSUBCLASS_INFO ISA_REGSUBCLASS_info[];\n"
		 "  return &ISA_REGSUBCLASS_info[sc];\n"
		 "}\n");

  fprintf(hfile, "\ninline const char *ISA_REGSUBCLASS_INFO_Name(\n"
		 "  const ISA_REGSUBCLASS_INFO *info\n"
		 ")\n"
		 "{\n"
		 "  return info->name;\n"
  ...
  fprintf(hfile, "\ninline ISA_REGCLASS ISA_REGSUBCLASS_INFO_Class(\n"
		 "  const ISA_REGSUBCLASS_INFO *info\n"
		 ")\n"
		 "{\n"
		 "  return (ISA_REGCLASS)info->rclass;\n"
		 "}\n");

  fprintf(hfile, "\ninline INT ISA_REGSUBCLASS_INFO_Count(\n"
		 "  const ISA_REGSUBCLASS_INFO *info\n"
		 ")\n"
		 "{\n"
		 "  return info->count;\n"
  ...
  fprintf(hfile, "\ninline UINT ISA_REGSUBCLASS_INFO_Member(\n"
		 "  const ISA_REGSUBCLASS_INFO *info,\n"
		 "  INT n\n"
		 ")\n"
		 "{\n"
  ...
  fprintf(hfile, "\ninline const char *ISA_REGSUBCLASS_INFO_Reg_Name(\n"
		 "  const ISA_REGSUBCLASS_INFO *info,\n"
		 "  INT n\n"
		 ")\n"
		 "{\n"
  ...
  fprintf(cfile, "\nvoid ISA_REGISTER_Initialize(void)\n"
		 ...
		 "  for (rc = ISA_MIN_REGCLASS; "
			 "rc <= ISA_MAX_REGCLASS; ++rc) {\n"
		 "    INT i = ISA_REGCLASS_info_index[rc];\n"
		 "    const ISA_REGCLASS_INFO *info = "
			    "&ISA_REGCLASS_info[i];\n"
		 "    while ((info->isa_mask & mask) == 0) ++info, ++i;\n"
		 "    ISA_REGCLASS_info_index[rc] = i;\n"
		 "  }\n"
		 "}\n");
