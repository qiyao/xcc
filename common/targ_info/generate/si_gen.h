...
//
//          void Machine( const char* name, ISA_SUBSET isa, int argc,
//			  char** argv )
...
//				      int format_id= TI_SI_INVALID_FORMAT_ID )
//              Creates a resource with <count> instances available per
//              machine cycle.  <name> is given to the resource for debugging
//              and documentation purposes. <format_id> indicates which format
//		does this resource correspond to. A value of
//		TI_SI_INVALID_FORMAT_ID (default) should be used for
//		non-format related resources.
//
...
//              results are available.
//
//	    bool Any_Operand_Access_Time_Is_Defined( int topcode )
//	        Has Any_Operand_Access_Time() been called for the current
//	        instruction group?
//
//          int Get_Any_Operand_Access_Time( int topcode )
//              return the access time of the operands of the instructions
//              in the current instruction group relative to execution.
//
//	    bool Operand_Access_Time_Is_Defined( int topcode, int operand_index )
//	        Has Operand_Access_Time() been called for <operand_index>
//	        for the current instruction group?
//
//          int Get_Operand_Access_Time( int topcode, int operand_index )
//              return the access time of the <operand_index>'th operand of
//              the current instruction group relative to
//              execution.  This does not imply that all the instructions in
//              the current group actually have this many operands, only that
//              those with enough operands have this access time.
//
//	    bool Any_Result_Available_Time_Is_Defined( int topcode )
//	        Has Any_Result_Available_Time() been called for the current
//	        instruction group?
//
//          int Get_Any_Result_Available_Time( int topcode )
//              Similar to Get_Any_Operand_Access_Time, but specifies time the
//              results are available.
//
//	    bool Result_Available_Time_Is_Defined( int topcode, int result_index )
//	        Has Result_Available_Time() been called for <result_index>
//	        for the current instruction group?
//
//          int Get_Result_Available_Time( int topcode, int result_index )
//              Similar to Get_Operand_Access_Time, but specifies time the
//              results are available.
//
...
//  $Revision: 1.9 $
//  $Date: 2000/04/06 02:33:28 $
//  $Author: mtibuild $
//  $Source: /isms/cmplrs.src/osprey1.0/common/targ_info/generate/RCS/si_gen.h,v $
...
extern RESOURCE RESOURCE_Create( const char* name, int count, int format_id = TI_SI_INVALID_FORMAT_ID );
#else
extern RESOURCE RESOURCE_Create( const char* name, int count, int format_id = -1 );
#endif
...
extern bool Any_Operand_Access_Time_Is_Defined( int topcode );
extern int Get_Any_Operand_Access_Time( int topcode );
extern bool Operand_Access_Time_Is_Defined( int topcode, int operand_index );
extern int Get_Operand_Access_Time( int topcode, int operand_index );
extern bool Any_Result_Available_Time_Is_Defined( int topcode );
extern int Get_Any_Result_Available_Time( int topcode );
extern bool Result_Available_Time_Is_Defined( int topcode, int result_index );
extern int Get_Result_Available_Time( int topcode, int result_index );
...
extern int Get_Resource_Requirement_Count( int topcode, RESOURCE resource,
					   int time );
int Get_Max_Resource_Cycle( int topcode );
