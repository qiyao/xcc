...
main()
{
  ...
  OPERAND_VALUE_TYPE ph, dh, bwh, aclr, ldhint, sthint, lfhint,
		     indwh, ph_pvec_ih, ipwh, ldtype, sttype, fldtype,
		     sem, mwh, mbtype4;
  ...
  ph_pvec_ih = ISA_Enum_Opnd_Type_Create("ph_pvec_ih", 8, UNSIGNED, EC_ph_pvec_ih);
...
/* ===== (<%pr%p6>) <r7>=[<r7>] ===== */
  Instruction_Group("O_21",
	...
	TOP_mov_f_pkr,	TOP_mov_f_pmc,	TOP_mov_f_pmd,
	TOP_mov_f_rr,
	TOP_UNDEFINED);
  Operand(0, pr, predicate);
  Result(0, int64);
...
/* ===== (<%pr%p6>) [<r7>]<eq><r7> ===== */
  Instruction_Group("O_39",
	...
	TOP_mov_t_ibr,	TOP_mov_t_pkr,	TOP_mov_t_pmc,
	TOP_mov_t_pmd,	TOP_mov_t_rr,
	TOP_UNDEFINED);
  Operand(0, pr, predicate);
  Operand(1, int64);
  Operand(2, int64);
...
/* ===== <ipwh>,<ph_pvec_ih>,<@pcrel25>,<pcrel13> ===== */
...
/* ===== (<%pr%p6>) <b3>=<mwh>,<ph_pvec_ih>,<r7>,<pcrel13> ===== */
...
/* ===== intrncall  R0 <- intrinsic_call (O0, I1, O2) ===== */
