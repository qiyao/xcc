...
{
  INT i;
  for (i = 0; i < ISA_BUNDLE_MAX_SLOTS; i++) {
  ...
  Set_TI_BUNDLE_slot_count(bundle, ISA_BUNDLE_MAX_SLOTS);
...
)
{
  ...
  if (slot >= ISA_BUNDLE_MAX_SLOTS) return FALSE;
...
#define SLOTS_COMPATIBLE(_prop, _bundle_index, _slot_index) \
 ((_prop == 0) || \
  ((_prop & TI_ISA_Exec_Slot_Prop(_bundle_index, _slot_index)) == \
   TI_ISA_Exec_Slot_Prop(_bundle_index, _slot_index)))

  /* Loop through the bundle and slot info. */
  for (i = 0; i < TI_ISA_Num_Bundles(); ++i) {
    match = FALSE;
    for (j = 0; j < ISA_BUNDLE_MAX_SLOTS; ++j) {
      if (j >= TI_ISA_Num_Slots(i)) {
	...
	if ((j == slot) || TI_BUNDLE_slot_filled(bundle, j) ||
	  ...
	  if (!SLOTS_COMPATIBLE(slot_prop, i, j) ||
	      (TI_BUNDLE_stop_bit(bundle, j) != TI_ISA_Exec_Stop(i, j))) {
	    match = FALSE;
	    break;
	  }
	}
...
)
{
  ...
  if (slot == (ISA_BUNDLE_MAX_SLOTS - 1))
      return TRUE;
  ...
  for (i = 0; i < TI_ISA_Num_Bundles(); ++i) {
    ...
    for (j = 0; j < TI_ISA_Num_Slots(i); ++j) {
      ...
      if (TI_ISA_Exec_Stop(i, j) != stop_bit_value) {
...
)
{
  ...
  TI_BUNDLE_slot_mask(bundle) |= property <<
    (TI_ISA_Tag_Shift() * (ISA_BUNDLE_MAX_SLOTS - slot - 1));
#endif
}
...
{
  if (slot < 0 || slot > ISA_BUNDLE_MAX_SLOTS) {
