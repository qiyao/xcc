...
#include <stdio.h>
/* anyone that includes libelf.h needs to include elf.h before it,
   for reasons explained in elf.h */
#include <elf.h>
#include <libelf.h>
...
list<UNWIND_ELEM> ue_list;
list<UNWIND_ELEM>::iterator ue_iter;
...
Print_Unwind_Elem (UNWIND_ELEM ue, char *msg)
{
	...
	case UE_SAVE_M:
		fprintf(TFile, "save %s", REGISTER_name(
				CLASS_REG_PAIR_rclass(ue.rc_reg),
        			CLASS_REG_PAIR_reg(ue.rc_reg) ));
		fprintf(TFile, " to mem %" LLD_FMT "(%s)",
			ue.u.offset,
	...
	for (bb = REGION_First_BB; bb; bb = BB_next(bb)) {
		...
		if (BB_preds(bb) != NULL) {
			...
			FOR_ALL_BB_PREDS (bb, blst) {
				...
				if (Trace_Unwind) fprintf (TFile, "[%d %" LLX_FMT "], ",
...
Emit_Unwind_Directives_For_OP(OP *op, FILE *f)
{
  ...
  next_when += OP_Real_Inst_Bytes(op);
}

...
{
  	__unw_error_t st = 0;
  	if (Trace_Unwind) fprintf(TFile, "prolog header size %d\n", size);
...
{
  	__unw_error_t st = 0;
  	if (Trace_Unwind) fprintf(TFile, "body header size %d\n", size);
...
Create_Unwind_Descriptors (__unw_info_t *uinfo)
{
  ...
  if (ue_iter == ue_list.end()) {
    ...
    if (Current_PU_Stack_Model == SMODEL_SMALL) {
      if (Trace_Unwind) fprintf(TFile, "fixed stack frame of size %" LLD_FMT ", set at when %d\n", 
  ...
  for (ue_iter = ue_list.begin(); ue_iter != ue_list.end(); ++ue_iter) {
	...
	ISA_REGCLASS save_rc = CLASS_REG_PAIR_rclass(ue_iter->save_rc_reg);
		    ...
		    INT       high_pc)
{
  ...
  if (CG_emit_unwind_info && simple_unwind) {
...
static char *drop_these[] = {
	...
	".debug_typenames",
	".debug_funcnames",
     // we don't use debug_frame for Xtensa.
	".debug_frame",
	0
};
