...
/* For xtensa there are multiple instruction sizes, so we may want to
...
inline INT32
CGTARG_Branch_Taken_Penalty(void)
{
  return CGTARG_branch_taken_penalty_overridden ?
    CGTARG_branch_taken_penalty : 2;
}

inline INT32
CGTARG_Issue_Width(void)
{
  return 1;
}
...
{
  return (xt_density) ? TOP_nop_n : TOP_nop;
}

inline TOP
CGTARG_Noop_Top(ISA_EXEC_UNIT_PROPERTY unit)
{
  return TI_ISA_Noop_Top(unit);
}
...
CGTARG_Simulated_Top(OP *op, ISA_EXEC_UNIT_PROPERTY unit)
{
  ...
  if (!TI_ISA_Property_Set(PROP_simulated, top)) return top;
...
/*
 *     TN *CGTARG_Return_Enum_TN(OP *op, TI_ISA_ENUMCLASS class, INT *pos)
 *     if 'op' has an enum operand which matches the TI_ISA_ENUMCLASS 
 *     <class> type, return the enum TN and its operand position.
 */
inline TN* 
CGTARG_Return_Enum_TN(OP *op, ISA_ENUMCLASS_VALUE enum_class, INT *pos)
{
  INT i;
