...
#ifdef _KEEP_RCS_ID
static char *config_asm_rcs_id = "$Source: /osprey.src/osprey1.0/common/com/ia64/RCS/config_asm.h,v $ $Revision: 1.29 $";
#endif /* _KEEP_RCS_ID */
 ...
 *
 * NOTE: Since we don't want the gdb to print out these labels.  It is
 * best to have prefix .L
 */
 ...
 */
extern DLL_SHARED char *AS_ADDRESS;
extern DLL_SHARED char *AS_ADDRESS_UNALIGNED;
...
#define ASM_DIR_ALIGN(p, s)             fprintf(Asm_File, "\t%s %" LLD_FMT "\n", AS_ALIGN,(INT64)STB_align(s));

/* gas .skip fills with zeroes */
#define ASM_DIR_ZERO(fl, l)	fprintf(fl, "\t%s %" LLD_FMT "\n", AS_SPACE, (INT64)l) 
#define ASM_DIR_SKIP(fl, l) 	fprintf(fl, "\t%s %" LLD_FMT "\n", AS_SPACE, (INT64)l)
