...
float targ_sqrtf(float x);

#if defined(__sun__)
/* Solaris doesn't provide this function.  To the best of my knowledge,
   the casting method here does not change the actual results of the call,
   just the speed.  */
...
static TCON complex_sqrt(TCON c0)
{
   ...
   switch (TCON_ty(c0)) {
	...
	TCON_R4(r) = TCON_IR4(r) = 0.;
	 ...
	 TCON_R4(r) = targ_sqrtf(0.5 * (fmag + fr) );
	 TCON_IR4(r) = fi / TCON_R4(r) / 2;
      } else {
	 TCON_IR4(r) = targ_sqrtf(0.5 * (fmag - fr) );
	...
	TCON_R8(r) = TCON_IR8(r) = 0.;
	 ...
	 TCON_R8(r) = std::sqrt(0.5 * (dmag + dr) );
	 TCON_IR8(r) = di / TCON_R8(r) / 2;
      } else {
	 TCON_IR8(r) = std::sqrt(0.5 * (dmag - dr) );
...
#undef DEBUG_FOLD
#ifdef DEBUG_FOLD
   printf("Folding %s on 0x%" LLX_FMT " (%s), 0x%" LLX_FMT " (%s), result",OPCODE_name(op),
   ...
   *folded = TRUE;
    ...
    */
    ...
    case OPC_F8SQRT:
      if (TCON_R8(c0) >= 0) {
	TCON_R8(c0) = std::sqrt(TCON_R8(c0));
      } else {
	*folded = FALSE;
      }
      break;
    case OPC_F4SQRT:
      if (TCON_R4(c0) >= 0) {
	TCON_R4(c0) = targ_sqrtf(TCON_R4(c0));
      } else {
	*folded = FALSE;
      }
      break;
...
#define FROM_TO(type_from, type_to) (type_from)*(MTYPE_MAX)+(type_to)
  ...
  TCON_iv3(r) = 0;
  ty_from = TCON_ty(c);
  Is_True ( ty_to > MTYPE_UNKNOWN && ty_to <= MTYPE_CORE_LAST,
	    ("Bad dest type in Targ_Conv: %s", Mtype_Name(ty_to)) );
  Is_True ( ty_from > MTYPE_UNKNOWN && ty_from <= MTYPE_CORE_LAST,
...
static TCON Targ_Power(TCON base, TCON exp, BOOL *folded, TYPE_ID btype)
{
   ...
   switch (TCON_ty(base)) {
    case MTYPE_F4:
      TCON_ty(r) = MTYPE_F4;
      TCON_R4(r) = std::pow (TCON_R4(base), TCON_R4(exp));
      break;

    case MTYPE_F8:
      TCON_ty(r) = MTYPE_F8;
      TCON_R8(r) = std::pow (TCON_R8(base),TCON_R8(exp));
      break;
...
#define RAD_TO_DEGQ (180.0l/M_PIL)
   ...
   switch ((INTRINSIC) intrinsic) {
    ...
    case INTRN_F4EXP:
      TCON_R4(c0) = std::exp(TCON_R4(c0));
      break;
    case INTRN_F8EXP:
      TCON_R8(c0) = std::exp(TCON_R8(c0));
      break;

    case INTRN_F4LOG:
      TCON_R4(c0) = std::log(TCON_R4(c0));
      break;

    case INTRN_F8LOG:
      TCON_R8(c0) = std::log(TCON_R8(c0));
      break;

    case INTRN_F4LOG10:
      TCON_R4(c0) = std::log10(TCON_R4(c0));
      break;

    case INTRN_F8LOG10:
      TCON_R8(c0) = std::log10(TCON_R8(c0));
      break;

    case INTRN_F4COS:
      TCON_R4(c0) = std::cos(TCON_R4(c0));
      break;

    case INTRN_F8COS:
      TCON_R8(c0) = std::cos(TCON_R8(c0));
      break;

    case INTRN_F4SIN:
      TCON_R4(c0) = std::sin(TCON_R4(c0));
      break;

    case INTRN_F8SIN:
      TCON_R8(c0) = std::sin(TCON_R8(c0));
      break;

    case INTRN_F4TAN:
      TCON_R4(c0) = std::tan(TCON_R4(c0));
      break;

    case INTRN_F8TAN:
      TCON_R8(c0) = std::tan(TCON_R8(c0));
      break;

    case INTRN_F4COSD:
      TCON_R4(c0) = std::cos(DEG_TO_RAD*TCON_R4(c0));
      break;

    case INTRN_F8COSD:
      TCON_R8(c0) = std::cos(DEG_TO_RAD*TCON_R8(c0));
      break;

    case INTRN_F4SIND:
      TCON_R4(c0) = std::sin(DEG_TO_RAD*TCON_R4(c0));
      break;

    case INTRN_F8SIND:
      TCON_R8(c0) = std::sin(DEG_TO_RAD*TCON_R8(c0));
      break;

    case INTRN_F4TAND:
      TCON_R4(c0) = std::tan(DEG_TO_RAD*TCON_R4(c0));
      break;

    case INTRN_F8TAND:
      TCON_R8(c0) = std::tan(DEG_TO_RAD*TCON_R8(c0));
      break;


    case INTRN_F4ACOS:
      TCON_R4(c0) = std::acos(TCON_R4(c0));
      break;

    case INTRN_F8ACOS:
      TCON_R8(c0) = std::acos(TCON_R8(c0));
      break;

    case INTRN_F4ASIN:
      TCON_R4(c0) = std::asin(TCON_R4(c0));
      break;

    case INTRN_F8ASIN:
      TCON_R8(c0) = std::asin(TCON_R8(c0));
      break;

    case INTRN_F4ATAN:
      TCON_R4(c0) = std::atan(TCON_R4(c0));
      break;

    case INTRN_F8ATAN:
      TCON_R8(c0) = std::atan(TCON_R8(c0));
      break;

    case INTRN_F4ACOSD:
      TCON_R4(c0) = RAD_TO_DEG*std::acos(TCON_R4(c0));
      break;

    case INTRN_F8ACOSD:
      TCON_R8(c0) = RAD_TO_DEG*std::acos(TCON_R8(c0));
      break;

    case INTRN_F4ASIND:
      TCON_R4(c0) = RAD_TO_DEG*std::asin(TCON_R4(c0));
      break;

    case INTRN_F8ASIND:
      TCON_R8(c0) = RAD_TO_DEG*std::asin(TCON_R8(c0));
      break;

    case INTRN_F4ATAND:
      TCON_R4(c0) = RAD_TO_DEG*std::atan(TCON_R4(c0));
      break;

    case INTRN_F8ATAND:
      TCON_R8(c0) = RAD_TO_DEG*std::atan(TCON_R8(c0));
      break;

    case INTRN_F4COSH:
      TCON_R4(c0) = std::cosh(TCON_R4(c0));
      break;

    case INTRN_F8COSH:
      TCON_R8(c0) = std::cosh(TCON_R8(c0));
      break;

    case INTRN_F4SINH:
      TCON_R4(c0) = std::sinh(TCON_R4(c0));
      break;

    case INTRN_F8SINH:
      TCON_R8(c0) = std::sinh(TCON_R8(c0));
      break;

    case INTRN_F4TANH:
      TCON_R4(c0) = std::tanh(TCON_R4(c0));
      break;

    case INTRN_F8TANH:
      TCON_R8(c0) = std::tanh(TCON_R8(c0));
      break;

    case INTRN_F4ATAN2:
      TCON_R4(c0) = std::atan2(TCON_R4(c0),TCON_R4(c[1]));
      break;

    case INTRN_F8ATAN2:
      TCON_R8(c0) = std::atan2(TCON_R8(c0),TCON_R8(c[1]));
      break;

    case INTRN_F4ATAN2D:
      TCON_R4(c0) = RAD_TO_DEG*std::atan2(TCON_R4(c0),TCON_R4(c[1]));
      break;

    case INTRN_F8ATAN2D:
      TCON_R8(c0) = RAD_TO_DEG*std::atan2(TCON_R8(c0),TCON_R8(c[1]));
      break;

    case INTRN_F4CIS:
      TCON_ty(c0) = MTYPE_C4;
      TCON_R4(c0) = std::cos(TCON_R4(c[0]));
      TCON_IR4(c0) = std::sin(TCON_R4(c[0]));
      break;

    case INTRN_F8CIS:
      TCON_ty(c0) = MTYPE_C8;
      TCON_R8(c0) = std::cos(TCON_R8(c[0]));
      TCON_IR8(c0) = std::sin(TCON_R8(c[0]));
      break;
    ...
    case INTRN_FQEXP:
      TCON_R16(c0) = RLD_To_R16(std::exp(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQLOG:
      TCON_R16(c0) = RLD_To_R16(std::log(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQLOG10:
      TCON_R16(c0) = RLD_To_R16(std::log10(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQCOS:
      TCON_R16(c0) = RLD_To_R16(std::cos(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQSIN:
      TCON_R16(c0) = RLD_To_R16(std::sin(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQTAN:
      TCON_R16(c0) = RLD_To_R16(std::tan(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQCOSD:
      TCON_R16(c0) = RLD_To_R16(std::cos(DEG_TO_RADQ*R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQSIND:
      TCON_R16(c0) = RLD_To_R16(std::sin(DEG_TO_RADQ*R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQTAND:
      TCON_R16(c0) = RLD_To_R16(std::tan(DEG_TO_RADQ*R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQCOSH:
      TCON_R16(c0) = RLD_To_R16(std::cosh(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQSINH:
      TCON_R16(c0) = RLD_To_R16(std::sinh(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQTANH:
      TCON_R16(c0) = RLD_To_R16(std::tanh(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQACOS:
      TCON_R16(c0) = RLD_To_R16(std::acos(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQASIN:
      TCON_R16(c0) = RLD_To_R16(std::asin(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQATAN:
      TCON_R16(c0) = RLD_To_R16(std::atan(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQACOSD:
      TCON_R16(c0) = RLD_To_R16(RAD_TO_DEGQ*std::acos(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQASIND:
      TCON_R16(c0) = RLD_To_R16(RAD_TO_DEGQ*std::asin(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQATAND:
      TCON_R16(c0) = RLD_To_R16(RAD_TO_DEGQ*std::atan(R16_To_RLD(TCON_R16(c0))));
      break;
    case INTRN_FQATAN2:
      TCON_R16(c0) = RLD_To_R16(std::atan2(R16_To_RLD(TCON_R16(c0)),R16_To_RLD(TCON_R16(c[1]))));
      break;
    case INTRN_FQATAN2D:
      TCON_R16(c0) = RLD_To_R16(RAD_TO_DEGQ*std::atan2(R16_To_RLD(TCON_R16(c0)),R16_To_RLD(TCON_R16(c[1]))));
      break;

    case INTRN_FQCIS:
      TCON_ty(c0) = MTYPE_CQ;
      TCON_R16(c0) = RLD_To_R16(std::cos(R16_To_RLD(TCON_R16(c[0]))));
      TCON_IR16(c0) = RLD_To_R16(std::sin(R16_To_RLD(TCON_R16(c[0]))));
      break;
...
// target library version of sqrtf
// avoid using native version, it exposes a compiler bug
// on Solaris, and we would like to be consistent across
// targets
//
typedef union
{
...
const float	one	= 1.0, tiny=1.0e-30;
float targ_sqrtf(float x)
{
	float z;
	INT32 	sign = (INT32)0x80000000; 
	UINT32 r,hx;
	INT32 ix,s,q,m,t,i;
