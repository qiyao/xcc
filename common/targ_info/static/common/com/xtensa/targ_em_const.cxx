 ...
 * Module: targ_const.c
 * $Revision: 1.13 $
 * $Date: 2000/04/06 02:29:30 $
 * $Author: mtibuild $
 * $Source: /osprey.src/osprey1.0/common/com/ia64/RCS/targ_em_const.cxx,v $
 *
 * Revision history:
...
#define MAX_LEN 8
    ...
    // if 0 terminated string, print as such
    if (str[len-1] == '\0') {
      fprintf(fl, "\t%s \"", AS_STRING);
      for ( i=0; i<len-1; ++i ) {
        if (Has_Control_Char(str+i,1)) {
          if (str[i] == '\n') {
            fprintf(fl,"\\n");
	  } else if (str[i] == '\t') {
            fprintf(fl,"\\t");
	  } else {
            fprintf (fl, "\\%03o", str[i]);
	  }
	} else {
          fprintf(fl,"%c",str[i]);
	}
  ...
  INTSC maxc)			/* max count per directive */
{
  ...
  do {
    ...
    if ((val >= INT32_MIN) && (val <= INT32_MAX)) 
      fprintf(fl, "\t%s\t%" LLD_FMT "", dir, val);
    else
      fprintf(fl, "\t%s\t0x%" LLX_FMT "", dir, val);
    for (i = 1; i < this_rc; ++i) {
      if ((val >= INT32_MIN) && (val <= INT32_MAX)) 
	fprintf(fl, ", %" LLD_FMT "", val);
      else
	fprintf(fl, ", 0x%" LLX_FMT "", val);
    }
    fprintf(fl, "\n");
  } while (rc -= this_rc);
}
