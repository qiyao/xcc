...
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/common/util/RCS/c_qtenscale.c,v $ $Revision: 1.4 $";
...
#if QUAD_DEBUG
    printf("After c_qwmultu: prod = %" LLX_FMT " %" LLX_FMT " %" LLX_FMT " %" LLX_FMT "\n", 
	   prod[0], prod[1], prod[2], prod[3]);
#endif
    qnorm_and_round(p, &norm, prod);
#if QUAD_DEBUG
    printf("After qnorm_and_round: p = %" LLX_FMT " %" LLX_FMT "\n", 
	   p[0], p[1]);
#endif
    *bexp += twoexp[hi] - norm;
#if QUAD_DEBUG
