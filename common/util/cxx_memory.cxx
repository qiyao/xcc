...
#include "errors.h"
/*
   TENSILICA on Windows:

   cxx_memory.cxx must be a part of every DLL for Windows 
   because the linking of the operators new and delete wasn't 
   working when new and delete lived in be.dll only.

   But we still need the dummy_mempool vars to be common. So
   I have moved them to glob.cxx, which conveniently is only
   a part of be.dll and the front ends. So everything works,
   but there are several copies of this code loaded when 
   be does its thing.
*/
...
#else
  ptr = (void*) MEM_POOL_Alloc (_dummy_new_mempool, sz+_dummy_pad);
#endif
