...
INT Warn_Unaligned = 0;	/* Warn about unaligned references*/
		    ...
		    const UINT        limit)
{
  ...
  s->count++;
  /* only print the "reached the limit warning" in cases where
     there might be a lot of them. */
  if (s->count == limit && limit != 1) {
    DevWarn("Count limit reached on the following:");
  }
...
/* Obtain a backtrace and print it to stdout and the error file. */
#ifdef linux
#ifdef Is_True_On
#include <execinfo.h>
#endif
...
my_print_trace (void)
{
  ...
  for (i = 2; i < size; i++) {
    if (strings[i] && strrchr(strings[i], '('))
      printf ("###   %s\n", strrchr(strings[i], '('));
  }
    /* Then write to error file if enabled: */
  if ( Init_Error_File() ) {
