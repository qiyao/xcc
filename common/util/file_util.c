...
void convert_to_win32_path(char * path)
{
   if (!path)
      return;
   else {
...
void convert_to_posix_path(char * path)
{
   if (!path)
      return;
   else {
...
Last_Pathname_Component ( char *pname )
{
  ...
  while (cp != pname) {
    if (is_dir_separator (*cp)) return cp+1;
    --cp;
  }
  if (is_dir_separator (*cp)) return cp+1;
  return cp;
...
normalize_path(char * path)
{
  ...
  while (inp != NULL && *inp != '\0') {
    if (is_dir_separator (inp[0])) {
      if (is_dir_separator (inp[1]))
	inp+= 1;
      else if (inp[1] == '.' && is_dir_separator (inp[2]))
	inp += 2;
      else if (inp[1] == '.' && inp[2] == '.' && is_dir_separator (inp[3])) {
	/* Skip up one level up output path */
	for (tmp = outp-1;
	     tmp >= path && !is_dir_separator (*tmp);
	     tmp -= 1);
        /* Check that we skipped one level up, but not past ".." */
        if (tmp >= path && is_dir_separator (tmp[0]) &&
	  (tmp[1] != '.' || tmp[2] != '.'))
	  outp = tmp;
	else {
	  *outp++ = '/';
...
Make_Absolute_Path (char *filename)
{
   ...
   else
   {
      ...
      if (!is_dir_separator (cwd[cwd_length - 1]))
      {
