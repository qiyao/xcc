...
static const char rcs_id[] = "$Source: /isms/cmplrs.src/osprey1.0/common/util/RCS/flags.c,v $ $Revision: 2.28 $";
#endif
...
extern DLL_SHARED char *SBar;	/* Line of dashes from tracing.h */

/* ====================================================================
 *
 * Get_Numeric_Flag
...
Process_Command_Line_Group (char *flag, OPTION_GROUP *opt_groups)
{
    ...
    /* hack for NT. 
       ...
       Because colons are valid parts of path names, options of the form:

       -TARG:option1:option2:option3

       create problems when the options contain path names. So break
       up your options into the form:

       -TARG:option1 -TARG:option2 -TARG:option3 
    */
    sep[1] = 0; //OGROUP_separator(group);
    sep[2] = 0;
		     ...
		     BOOL internal, BOOL full, BOOL update )
{
	...
	desc++ )
  {
	...
	break;
    case OVK_INT64:
	fprintf ( tf, "%" LLD_FMT "", *((INT64 *) ODESC_variable(desc)));
	break;
    case OVK_UINT32:
	fprintf ( tf, "%u", *((UINT32 *) ODESC_variable(desc)));
	break;
    case OVK_UINT64:
	fprintf ( tf, "%" LLU_FMT "", *((UINT64 *) ODESC_variable(desc)));
	break;
...
void Save_or_restore_options(char *memory, INT32 size, BOOL save)
{
  ...
  OPTION_GROUP *group;
  BOOL trace = Get_Trace(TP_MISC, 0x80, 0); /* full command line tracing */
  INT32 incr, offset = 0;
