...
ID_MAP<NODE_TYPE, KEY_TYPE>::Hash(const KEY_TYPE key, const UINT32 tbl_size) const
{
...
template <class NODE_TYPE, class KEY_TYPE> void
ID_MAP<NODE_TYPE, KEY_TYPE>::Add_to_free_list(const mINT32 idx)
{
...
template <class NODE_TYPE, class KEY_TYPE> void
ID_MAP<NODE_TYPE, KEY_TYPE>::Remove_from_free_list(const mINT32 idx)
{
  Is_Trace(_tracing && _verbose,
...
template <class NODE_TYPE, class KEY_TYPE> mINT32
ID_MAP<NODE_TYPE, KEY_TYPE>::Alloc_from_free_list(void)
{
  Is_Trace(_tracing && _verbose,
...
#ifdef DEBUG_ID_MAP
template <class NODE_TYPE, class KEY_TYPE> void
ID_MAP<NODE_TYPE, KEY_TYPE>::Verify(void) const
{
				    ...
				    const NODE_TYPE node)
{
  ...
  if (_table[idx]._node != _not_found_value) {
    ...
    Is_Trace(_tracing && _verbose,
	     (TFile, "    displacing [H(%" LLU_FMT ")=%d; %d] to %d\n",
	...
	fprintf(TFile, "Insert %" LLU_FMT " at idx = %d (%d)\n",
		Key_as_llu(key), idx,
		Hash(key, _table_size));
	fprintf(TFile, " -> displaced = %d\n", displaced);
	fprintf(TFile, "start [H(%" LLU_FMT ")=%d]\n",
