...
#else
  fprintf(TFile,
          "MEM_Trace: Not available; not compiled with MEM_STATS\n");
#endif  
}
...
Trace_Memory_Allocation (
  ...
  const ST *const pu )	/* Program unit	to print */
{
  if ( Get_Trace ( TKIND_ALLOC, phase, 0 ) ) {
    fprintf ( TFile,
...
Allocate_Block (MEM_POOL *pool)
{
  ...
  if (special_address >= ((char *) MEM_BLOCK_ptr(block)) &&
    ...
    fprintf(TFile, "Pool %s given %lu bytes from 0x%lx to 0x%lx\n",
	    MEM_POOL_name(pool), MEM_BLOCK_avail(block),
  ...
  if (purify_pools) {
    ...
    *(MEM_PTR*)ret_val = MEM_POOL_last_alloc(pool);
    ...
    if (purify_pools_trace)
      printf ("pool %s, alloc 0x%lx, size %ld, (0x%lx - 0x%lx)\n",
	      MEM_POOL_name(pool), (size_t)ret_val+8, size,
	      (size_t)ret_val+8, (size_t)ret_val+size);
    return ((MEM_PTR) ((size_t)ret_val+8));
  }

#ifdef MEM_STATS
  FmtAssert(MEM_POOL_blocks(pool) != NULL,
            ("Alloc with uninitialized MEM_POOL %s, %d",file,line));
...
)
{
  ...
  if (purify_pools) {
    ...
    if (old_block && !foundit) {
      DevWarn ("Realloc without a previous alloc, pool %s, 0x%lx",
    ...
    if (purify_pools_trace)
      printf ("pool %s, realloc 0x%lx, new size %ld, (0x%lx - 0x%lx)\n",
	      MEM_POOL_name(pool), ret_val, new_size,
	      ret_val, (size_t)ret_val + new_size - 8);
    return ret_val;
  }
...
)
{
  ...
  if (purify_pools) {
	...
	(void) printf("MEM_POOL_Push %s 0x%lx<-- free push (called by M_P_Initialize)\n",
		      MEM_POOL_name(pool), pool);
      }
      else {
	(void) printf ("MEM_POOL_Push %s 0x%lx\n", MEM_POOL_name(pool), pool);
      }
    }
...
)
{
  ...
  if (purify_pools) {
    ...
    if (purify_pools_trace_x)
      printf ("MEM_POOL_Pop %s 0x%lx\n", MEM_POOL_name(pool), pool);
    tmp = MEM_POOL_last_alloc(pool);
    while (tmp) {
      next = (*(MEM_PTR*) tmp);
      if (purify_pools_trace) printf ("pool %s, pop-free 0x%lx\n",
                                      MEM_POOL_name(pool), (size_t)tmp+8);
      free (tmp);
      tmp = next;
    }
  ...
  for (bp = MEM_POOL_BLOCKS_block(bsp); bp; bp = next_bp) {
	...
	special_address < ((char *) MEM_BLOCK_ptr(bp) +
			   MEM_BLOCK_avail(bp))) {
      fprintf(TFile, "Pool %s freed %lu bytes from 0x%lx to 0x%lx\n",
	      MEM_POOL_name(pool),
	      (MEM_BLOCK_avail(bp) +
...
void MEM_POOL_FREE(MEM_POOL *pool, void *data)
{
  ...
  if (data && purify_pools) {
    ...
    if (purify_pools_trace)
      printf ("pool %s, free 0x%lx\n", MEM_POOL_name(pool), data);
    if (!foundit) {
      /* free anyway, so that it shows up as a purify error */
      free (cur);
      FmtAssert(0,("MEM_POOL_FREE: pool %s, could not find pointer 0x%lx\n",
                   MEM_POOL_name(pool), data));
    }
    free (cur);
    return;
...
void MEM_POOL_Delete(MEM_POOL *pool)
{
   ...
   */
  {
    ...
    while (*listp != pool && *listp) listp = &MEM_POOL_rest(*listp);
    Is_True(*listp, ("Didn't find pool in initialized_pools"));
    *listp = MEM_POOL_rest(pool);
...
#endif
    if (purify_pools_trace_x) 
      printf ("MEM_POOL_Delete %s 0x%lx\n", MEM_POOL_name(pool), pool);
...
)
{
  ...
  if (purify_pools_trace_x)
    printf ("MEM_POOL_Initialize %s 0x%lx\n", MEM_POOL_name(pool), pool);
