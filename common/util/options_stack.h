...
// $Revision: 1.7 $
// $Date: 2000/04/06 02:36:16 $
// $Author: mtibuild $
// $Source: /isms/cmplrs.src/osprey1.0/common/util/RCS/options_stack.h,v $
//
// Revision history:
...
class OPTIONS_STACK {
 ...
 public:
  OPTIONS_STACK(MEM_POOL *pool) : _options_stack(pool) {
    ...
    _trace = Get_Trace(TP_MISC, 0x80, 0); /* full command line tracing */
  ...
  /* Tensilica:
     These should be considered private to everything
     but the one function "Process_Pragma_Options" in
     be/be/driver.cxx.

     Process_Pragma_Options used to be part of OPTIONS_STACK,
     but due to linking complications on NT, it has moved else
     where */
