 ...
 * Module: tracing.c
 * $Revision: 2.35 $
 * $Date: 2000/10/09 19:46:56 $
 * $Author: dehnert $
 * $Source: /isms/cmplrs.src/osprey1.0/common/util/RCS/tracing.c,v $
 *
 * Revision history:
...
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/common/util/RCS/tracing.c,v $ $Revision: 2.35 $";
...
static PDESC Phases[] = {
  ...
  { TP_TDT,		"TDT",	"TDT building" },

  /* Front end phases: */
  { TP_SCAN,		"SCN",	"Lexical scanner" },
  { TP_PARSE,		"PAR",	"Parser" },
  { TP_SEMANTICS,	"SEM",	"Semantic analyzer" },
  { TP_PRE_IRB,		"PIR",	"Pre-IRB transformations" },
  ...
  { TP_PEEP,		"PEP",	"Peephole optimizer" },
  { TP_FLOWOPT,		"FLW",	"Control flow optimization" },
  { TP_GCM,		"GCM",	"Global code motion" },
  { TP_LAN,		"LAN",	"Software pipelining loop analysis" },
  ...
  { TP_CGALL,		"CGALL","All cg phases" },

  { TP_TEMP,		"TMP",	"Temporary use" },

  /* TIE lowering/optimization */
  { TP_TIE,		"TIE",	"TIE related info" },
 ...
 * Get_Trace
 *
 * Determine whether the trace flag given by func/arg is set.  If block
 * is not NULL, and BBs were explicitly enabled for tracing, return
 * FALSE unless the given BB is in the enabled list.
 *
 * ====================================================================
 */

BOOL
Get_Trace ( INT func, INT arg, INT bb_id )
{
  BOOL result;
  INT16 i;
