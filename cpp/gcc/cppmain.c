...
#if defined(__CYGWIN__) && !defined(OMIT_TENSILICA_CHANGES)
...
#if defined(__CYGWIN__) && !defined(OMIT_TENSILICA_CHANGES)
      size_t to_file_len;
      unsigned char *to_file_quoted, *p;
      static char fnamebuf[1024];

      /* force the file name to use the native Win32 style */
      if (*map->to_file)
	cygwin_conv_to_win32_path (map->to_file, fnamebuf);
      else
	fnamebuf[0] = 0;

      to_file_len = strlen (fnamebuf);
      to_file_quoted = alloca (to_file_len * 4 + 1);

      /* cpp_quote_string does not nul-terminate, so we have to do it
	 ourselves.  */
      p = cpp_quote_string (to_file_quoted,
			    (unsigned char *)fnamebuf, to_file_len);
      *p = '\0';
#else /* !__CYGWIN__ */
