...
static lang_info_t language_info[] = {
	...
	{'c',	0x00000002,	{"xt-xcc"EXE,  "sgicc"}},	/* cc */
	{'C',	0x00000004,	{"xt-xc++"EXE, "sgiCC"}},	/* c++ */
	{'f',	0x00000008,	{"f77",        "sgif77"}},	/* f77 */
	{'F',	0x00000010,	{"f90",        "sgif90"}},	/* f90 */
	{'a',	0x00000020,	{"xt-as"EXE,   "sgias"}},	/* as */
	{'l',	0x00000040,	{"xt-ld"EXE,   "sgild"}},	/* ld */
	{'I',	0x80000000,	{"int"}},		/* Internal option */
};

/* all these paths will be relative to .../Tensilica/
   thanks to some changes in main.c --SCA */

#define NAMEPREFIX	"xt-"
#define BINPATH		"/bin"
#define LIBPATH		"/lib/xcc-lib"
#define ALTLIBPATH	""
#define PHASEPATH	LIBPATH
#define GNUPHASEPATH    "/bin"
...
static phase_info_t phase_info[] = {
   ...
   {'p',  0x0000000000000010LL,	"tradcpp0"EXE, PHASEPATH, FALSE}, /* cpp */
   {'p',  0x0000000000000020LL,	"cpp0"EXE, PHASEPATH, FALSE}, /* gcpp */
   {'p',  0x0000000000000040LL,	"cpp0"EXE, PHASEPATH, FALSE}, /* gcpp_plus */
   {'p',  0x0000000000000080LL,	"cpp0"EXE, PHASEPATH, FALSE}, /* c_cpp */
   {'p',  0x0000000000000100LL,	"cpp0"EXE, PHASEPATH, FALSE}, /* cplus_cpp */
   ...
   {'f',  0x0000000000100000LL,	"gfec"EXE,PHASEPATH,	TRUE }, /* c_gfe */
   {'f',  0x0000000000200000LL,	"gfecc"EXE,PHASEPATH,	TRUE }, /* cplus_gfe */
   ...
   {'i',  0x0000000010000000LL,	"inline"EXE,PHASEPATH,	TRUE},	/* inline */
   {'i',  0x0000000020000000LL,	"ipl"EXE,	PHASEPATH,	TRUE},	/* ipl */
   {'i',  0x00000000f0000000LL,	"",	"",		TRUE},	/* ipl, inline*/

   {'b',  0x0000000100000000LL,	"be"EXE,PHASEPATH,	TRUE},	/* be */
   /* We use 'B' for options to be passed to be via ipacom. */

   {'a',  0x0000001000000000LL,	"asm",	PHASEPATH,	FALSE},	/* as */
   {'a',  0x0000002000000000LL,	NAMEPREFIX "as" EXE,	BINPATH,	FALSE},	/* as */
   {'a',  0x0000003000000000LL,	"",	"",		FALSE},	/* any_as */

   {'d',  0x0000008000000000LL, "dsm_prelink", PHASEPATH,FALSE},/* dsm_prelink*/
   {'t',  0x0000010000000000LL,	"auto"EXE,	PHASEPATH,	TRUE},	/* autotie */
   {'j',  0x0000020000000000LL,	"ipa_link"EXE, PHASEPATH,	TRUE},	/* ipa_link */
   {'l',  0x0000040000000000LL,	"collect2", GNUPHASEPATH,TRUE},	/* collect */
   {'l',  0x0000080000000000LL,	NAMEPREFIX "ld"EXE, BINPATH, FALSE}, /* ld */
   {'l',  0x0000100000000000LL,	NAMEPREFIX "ld"EXE, BINPATH, FALSE}, /* ldplus */
   {'l',  0x00001e0000000000LL,	"jhgjg",	"kljlhj",		TRUE},	/* any_ld */
   {'c',  0x0000200000000000LL, "cord", BINPATH,	FALSE},	/* cord */
   {'x',  0x0000400000000000LL, "pixie", BINPATH,   FALSE}, /* pixie */
   {'x',  0x0000800000000000LL, "prof",  BINPATH,   FALSE}, /* prof */

   {'R',  0x0001000000000000LL, NAMEPREFIX "ar",  BINPATH,      FALSE}, /* ar */

   {'S',  0x0010000000000000LL,	"crt",	LIBPATH,	FALSE},	/* startup */
   {'I',  0x0020000000000000LL,	"inc",	"/usr/asdfasdf",	FALSE},	/* include */
...
static source_info_t source_info[] = {
	...
	{"s"},				/* s */
	{"S"},				/* S */
	{"M"},				/* in liner used to be I, but this causes problems
					   with case-insensitive file systems produced by companies
					   out of redmond.
					*/
	{"B"},				/* B */
	{"N"},				/* N */
...
source_kind_t source_kind = S_NOTHING;
source_kind_t default_source_kind = S_NOTHING;
...
get_named_language (string name)
{
	...
	/* Append a ".exe" suffix if there isn't already one.  */
	if (strcmp(name + name_len - 4, ".exe") != 0) {
	    strcpy(new_name + name_len, ".exe");
	}
#endif
	for (i = L_NONE; i < L_LAST; i++) {
...
{
	source_kind_t i, res = S_o;
	int j;
	if (suf == NULL) return res;
	for (i = S_NOTHING; i < S_LAST; i++) {
...
{
  //src could be NULL if we are getting input from stdin
	if (ignore_suffix || (src == NULL)) {
		if (default_source_kind != S_NOTHING)
          ...
          source_kind_t res = get_source_kind_from_suffix (get_suffix(src));
          /* .c extension is treated as .C when compiled with C++ */
          if (res == S_c && invoked_lang == L_CC)
            res = S_C;
          return res;
        }
}
