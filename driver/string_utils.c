...
/* if no suffix in string, then add suffix */
extern string
change_suffix (string s, string suffix)
{
	...
	if (p == NULL) {
		p = concat_strings(new, ".");
		return concat_strings(p, suffix);
        }
	else if (strlen(suffix) <= strlen(p)) {
...
/* add string that has already been allocated */
static void
add_existing_string_to_beginning (string_list_t *list, string s)
{
	string_item_t *p;
	p = (string_item_t *) malloc(sizeof(string_item_t));
	p->name = s;
...
/* add string to end of list */
extern void
add_string_to_beginning (string_list_t *list, string s)
{
	/* don't worry about blanks in this version of add_string */
	add_existing_string_to_beginning (list, string_copy(s));
}
...
/* prepend string list b to the beginning of string list a */
extern void
prepend_string_lists (string_list_t *a, string_list_t *b)
{
	if (a->head == NULL) {
		/* first list is empty */
		a->head = b->head;
