...
extern int is_xtbool_type (tree type);
...
/* Build a call to abort.  */

static tree
call_abort (void)
{
  tree fn = IDENTIFIER_GLOBAL_VALUE (get_identifier ("abort"));
...
#ifdef TENSILICA_CHANGES
  if (TREE_CODE (TYPE_MAIN_VARIANT(TREE_TYPE (arg))) == TIE_TYPE
      && is_xtbool_type(TREE_TYPE(arg))) {
