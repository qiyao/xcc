...
extern int is_tie_intrinsic(tree fndecl);
...
struct binding_level
  {
    ...
    /* True if this level is the top-level function body scope. */
    unsigned function_body_scope : 1;
     ...
     int tag_transparent, keep;
{
  ...
  /* The scope following the function parameters scope must
     be the top-level function-body scope. */
  if (current_binding_level && current_binding_level->parm_flag == 1)
    newlevel->function_body_scope = 1;
...
/* Return non-zero if restrict modifier at this scope level
   is OK. */
int
binding_level_keep_restrict (void)
{
  /* The restrict qualifier is kept for variables declared
     at the global scope or at the top-level function-body
     scope. */
  if (current_binding_level &&
      (global_bindings_p() || 
       current_binding_level->function_body_scope))
    return 1;
     ...
     tree newdecl, olddecl;
{
  ...
  if (types_match)
	...
	DECL_SECTION_NAME (newdecl) = DECL_SECTION_NAME (olddecl);
      if (DECL_ROSECTION_NAME (newdecl) == NULL_TREE)
	DECL_ROSECTION_NAME (newdecl) = DECL_ROSECTION_NAME (olddecl);
  ...
  if (TREE_CODE (newdecl) == FUNCTION_DECL)
	...
	DECL_INLINE (olddecl) = 1;
      ...
      DECL_UNINLINABLE_ATTRIBUTE (newdecl) = DECL_UNINLINABLE_ATTRIBUTE (olddecl)
	= (DECL_UNINLINABLE_ATTRIBUTE (newdecl) || DECL_UNINLINABLE_ATTRIBUTE (olddecl));

      DECL_ALWAYS_INLINE (newdecl) = DECL_ALWAYS_INLINE (olddecl)
	= (DECL_ALWAYS_INLINE (newdecl) || DECL_ALWAYS_INLINE (olddecl));
  ...
  if (TREE_CODE (newdecl) == FUNCTION_DECL)
    {
      ...
      if (DECL_TEMPLATE_INSTANTIATION (olddecl) && DECL_EXPLICIT_INSTANTIATION(olddecl)) {
	      // avoid deleting explicit instantiations of templates
	      newdecl->decl.used_attribute = 1;
      }
...
#ifdef TARG_XTENSA
      /* We want statics to be given comdat linkage so that all
         versions of the function refer to the same static. The
         back-end is then free to inline or not. */
     ...
     int flags;
{
  ...
  /* clear the new_pragmas list
     Note: this might be a good place to process global pragmas */
  while (new_pragmas)
    pragma_list_remove_el(&new_pragmas);
	  ...
	  if (TREE_CODE (parm) == PARM_DECL)
	    {
		  ...
		  TREE_CODE (TREE_TYPE (parm)) == TIE_TYPE)
		{
		  cp_error ("TIE type parameter `%D' not supported", parm);
...
finish_stmt ()
{
  /* transfer any new pragmas into the file pragmas and associate them
     with the last statement.
     WARNING: If there is a pragma as a last statement in a function,
     it will be inserted as a first statement. This can be "fixed" probably
     by checking the code of the last statement. */
  PRAGMA_INFO *pi;
  while (new_pragmas) {
    pi=pragma_list_pop(&new_pragmas);
    pi->next_stmt=last_tree;
    pragma_list_push(&file_pragmas,pi);
  }
