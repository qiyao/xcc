...
#ifndef TARG_XTENSA
  /* xtensa: use default provided by toplev.c */
     ...
     const char *main_filename;
{
  ...
  if (impl_file_chain == 0)
	...
	main_input_filename = input_filename;
      ...
      filename = lbasename (main_input_filename);
     ...
     struct obstack *obstackp;
{
  ...
  while (c != EOF)
      ...
      else
	{
	  ...
	  {
	    /* Double backslashes in the filename because this is going
	       to be parsed as if it came from the input file.  This is
	       a hack, but the problem does not exist in GCC 3.x so it's
	       only temporary.  */
	    char fc, *fp = input_filename;
	    while ((fc = *fp++))
	      {
		obstack_1grow (obstackp, fc);
		if (fc == '\\')
		  obstack_1grow (obstackp, fc);
	      }
	  }
...
whitespace_cr (c)
     int c;
{
  /* TENSILICA: Disable this warning.  It was removed from GCC when the
     preprocessor was integrated into the frontends.  */
  static int newline_warning = 1;
...
linenum:
  ...
  if (main_input_filename == 0)
      ...
      if (ifiles)
	{
	  while (ifiles->next)
	    ifiles = ifiles->next;
	  ifiles->filename = lbasename (new_file);
	}
     ...
     PTR data;
{
  ...
  if (fflag)
	...
	args->value = REAL_VALUE_ATOF (copy, TYPE_MODE (args->type));
      args->conversion_errno = errno;
      if (REAL_VALUE_ISINF (args->value) && pedantic) {
        warning (flag_single_precision_constant ?
