  ...
  if (TYPE_POLYMORPHIC_P (TREE_TYPE (exprtype)))
      ...
      else
	{
	  tree retval;
          ...
          tree static_type, target_type, boff, tinfo;
	  ...
	  tinfo = get_tinfo_decl (target_type);
	  TREE_USED (tinfo) = 1;
	  TREE_SYMBOL_REFERENCED (DECL_ASSEMBLER_NAME (tinfo)) = 1;
	  td2 = build_unary_op (ADDR_EXPR, tinfo, 0);

	  static_type = TYPE_MAIN_VARIANT (TREE_TYPE (exprtype));
	  tinfo = get_tinfo_decl (static_type);
	  TREE_USED (tinfo) = 1;
	  TREE_SYMBOL_REFERENCED (DECL_ASSEMBLER_NAME (tinfo)) = 1;
	  td3 = build_unary_op (ADDR_EXPR, tinfo, 0);
