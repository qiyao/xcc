...
extern int is_out_or_inout_arg(tree fundecl, int parm_idx);
extern int convert_for_tie_type_ok(tree src, tree dst);
extern int is_xtbool1_type(tree type);  /* in wfe_tie.cxx */
extern int is_tie_intrinsic(tree fndecl);  /* in wfe_tie.cxx */
  ...
  for (valtail = values, typetail = typelist;
...
#ifdef TENSILICA_CHANGES
      if (!is_tie_intrinsic(fndecl)) {

	if (type == error_mark_node)
	  return error_mark_node;

        if (type && TREE_CODE(type)==TIE_TYPE) {
          error ("TIE type is not supported at parameter %d for function %s",
                i + 1, IDENTIFIER_POINTER(DECL_NAME(fndecl)));
        } else if (val && TREE_CODE(TREE_TYPE(val))==TIE_TYPE) {
          if (type && convert_for_tie_type_ok(TREE_TYPE(val), type)) {}
          else if (!type)
            error ("TIE type is not supported at parameter %d for function %s",
                  i + 1, IDENTIFIER_POINTER(DECL_NAME(fndecl)));
          else {
            error ("no conversion from TIE type %T to %T at parameter %d for function %s",      
                TREE_TYPE(val), type,
                i + 1, IDENTIFIER_POINTER(DECL_NAME(fndecl)));
	    error ("perhaps a TIE conversion macro is missing");
	  }
        } 
      }
#endif /* TENSILICA_CHANGES */
      ...
      if (type != 0)
	{
	  ...
	  if (!COMPLETE_TYPE_P (complete_type (type)))
	    ...
	    {
#ifdef TENSILICA_CHANGES
              if (is_out_or_inout_arg(fndecl, i) && TREE_READONLY(val)) {
error ("Cannot use read-only variable at parameter %d for TIE intrinsic %s",
                        i + 1, IDENTIFIER_POINTER(DECL_NAME(fndecl)));
              }
#endif /* TENSILICA_CHANGES */
     ...
     tree orig_op0, orig_op1;
{
  ...
  switch (code)
    ...
    case TRUTH_OR_EXPR:
#ifdef TENSILICA_CHANGES
      /* we are putting the same change as in the C front end (c-typeck.c)
	 but not sure why TRUTH_XOR_EXPR is not handled here and why
	 C front end special case when code{0,1} is
	 {INTEGER,POINTER,REAL,COMPLEX}_TYPE
      */
	if ((code0 == INTEGER_TYPE ||
	     code0 == TIE_TYPE && is_xtbool1_type(TREE_TYPE(op0))) &&
	    (code1 == INTEGER_TYPE ||
	     code1 == TIE_TYPE && is_xtbool1_type(TREE_TYPE(op1))) &&
          ...
          op1 = truthvalue_conversion (op1);
	  {
	    ...
	    TREE_CONSTANT (result) = TREE_CONSTANT (op0) & TREE_CONSTANT (op1);
	    return result;
	  }
        }
#endif /* TENSILICA_CHANGES */
    ...
    case NE_EXPR:
#ifdef TENSILICA_CHANGES
      if ((code0 == TIE_TYPE && is_xtbool1_type(TREE_TYPE(op0))) &&
	  (code1 == TIE_TYPE && is_xtbool1_type(TREE_TYPE(op1))))
	{
	  if (code==EQ_EXPR)
	    return build_unary_op (
			TRUTH_NOT_EXPR,
			build_binary_op (TRUTH_XOR_EXPR, op0, op1),
			1);
	  else
	    return build_binary_op (TRUTH_XOR_EXPR, op0, op1);
      }
#endif /* TENSILICA_CHANGES */
     ...
     tree expr;
{
  ...
  if (processing_template_decl)
   ...
   if (TREE_CODE(TREE_TYPE(expr)) == TIE_TYPE &&
       is_xtbool1_type(TREE_TYPE(expr)))
     return expr;
#endif
     ...
     int noconvert;
{
  ...
  switch (code)
    ...
    case TRUTH_NOT_EXPR:
#ifdef TENSILICA_CHANGES
      if (TREE_CODE(TREE_TYPE(arg)) == TIE_TYPE &&
	  is_xtbool1_type(TREE_TYPE(arg))) {
     ...
     tree type, expr;
{
  ...
  if (TREE_CODE (type) == REFERENCE_TYPE)
  ...
  else
    {
      ...
      if (type == error_mark_node ||
	  TYPE_MAIN_VARIANT(type) == error_mark_node ||
	  otype == error_mark_node ||
	  TYPE_MAIN_VARIANT(otype) == error_mark_node ||
	  value == error_mark_node ||
	  ovalue == error_mark_node)

	  return error_mark_node;

      if (TREE_CODE(type) == TIE_TYPE || TREE_CODE(otype) == TIE_TYPE)
	{
	  if (TYPE_MAIN_VARIANT(type) == TYPE_MAIN_VARIANT(otype))
	    /* no need to generate cast for the same type */;
	  else if (convert_for_tie_type_ok(otype,type)) {
	    /* try to convert for a TIE type */
	    value =  convert(type, value);
	  } else {
	    cp_error("incompatible cast from type `%T' to type `%T'",
		     otype, type);
	    cp_error ("perhaps a TIE conversion macro is missing");
	    value = error_mark_node;
	  }
	}
      else
        value = convert_force (type, value, CONV_C_CAST);
#else /* TENSILICA_CHANGES */
      value = convert_force (type, value, CONV_C_CAST);
#endif /* TENSILICA_CHANGES */
     ...
     int parmnum;
{
...
#ifdef TENSILICA_CHANGES
  if (codel == TIE_TYPE || coder == TIE_TYPE)
    {
      if (type == error_mark_node || rhs == error_mark_node)
	  return error_mark_node;

      /* try to convert for a TIE type */
      if (convert_for_tie_type_ok(rhstype,type)) {
  ...
  if (!can_convert_arg (type, rhstype, rhs))
	...
	if (is_tie_intrinsic(fndecl) &&
	  ...
	  tree type_v = build_pointer_type
			(cp_build_qualified_type(ttl,TYPE_QUAL_VOLATILE));

	  if (can_convert_arg(type_v, rhstype, rhs)) {
	    /* do not warn if this is a volatile pointer passed to
	       a TIE instrinsic
	     */
	    return perform_implicit_conversion (strip_top_quals (type_v), rhs);
	  }
	}
      }
#endif
