...
extern int is_xtbool1_type(tree type);
...
#ifdef TENSILICA_CHANGES
  if (is_xtbool1_type (type))
    return build1 (TRUTH_NOT_EXPR, integer_type_node, arg);
#endif /* TENSILICA_CHANGES */
...
#ifdef TARG_XTENSA
  // The following transformation is unsafe for LNO (PR 2943)
  else if (optimize > 2)
    return 0;
#endif
...
#ifdef TENSILICA_CHANGES
	  /* avoid bit field ref so type-based aliasing can be properly handled by
	  * back-end 
	  */
	  return 0;
#endif /* TENSILICA_CHANGES */
