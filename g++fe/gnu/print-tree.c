...
#define DEFAULT_MAX_INDENT 24
static int max_print_indent = DEFAULT_MAX_INDENT;
...
/* Same as debug_tree but specify the maximum indentiation amount
   allowed. */

void
debug_tree_limit (node, max_indent)
     tree node;
     int max_indent;
{
  max_print_indent = max_indent;
  debug_tree(node);
  max_print_indent = DEFAULT_MAX_INDENT;
}
