...
/* Create and return a TIE type of PRECISION bits.  */

tree
make_tie_type (bit_size, byte_alignment)
     int bit_size;
     int byte_alignment;
{
  int old;
  register tree type = make_node (TIE_TYPE);

  int alignment = byte_alignment * 8;
  int rounded_size = ((bit_size + alignment - 1) / alignment) * alignment;

  TYPE_PRECISION (type) = bit_size;

  /* the following is essentially the same as fixup_unsigned_type (type) */

  /* fixme: don't really know what values to give as min and max */
  /* the size of the type can be arbitrarily large and may be not
     reresentable by a tree */

  TYPE_MIN_VALUE (type) = build_int_2 (0, 0);
  TYPE_MAX_VALUE (type) = build_int_2 (0, 0);

  TREE_TYPE (TYPE_MIN_VALUE (type)) = type;
  TREE_TYPE (TYPE_MAX_VALUE (type)) = type;

  /* Lay out the type: set its alignment, size, etc. as in layout_type (type) */
  ...
  TYPE_ALIGN(type) = byte_alignment * 8;
