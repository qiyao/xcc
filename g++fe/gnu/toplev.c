...
#ifdef TARG_XTENSA
/* Make sure changes to 'flag_exceptions' default matches the default
   for config.cxx::CXX_Exceptions_On. */
#endif
int flag_exceptions;
...
lang_independent_options f_options[] =
{
  ...
  {"no-merge-constants", &flag_merge_constants, 0 },
...
#define ASM_FINISH_DECLARE_OBJECT(FILE, DECL, TOP, END)
  ...
  if (TREE_STATIC (decl) || DECL_EXTERNAL (decl)
      ...
      if (decode_reg_name (asmspec) >= 0)
	{
	  ...
	  make_decl_rtl (decl, asmspec, top_level);
#ifdef SGI_MONGOOSE
	  WFE_Record_Asmspec_For_Decl (decl, asmspec, decode_reg_name (asmspec));
#endif /* SGI_MONGOOSE */
	}
      else
...
#if !defined(OS2) && !defined(VMS) && (!defined(_WIN32) || defined (__CYGWIN__)) && !defined(__INTERIX) && !defined(TARG_XTENSA)
  if (flag_print_mem)
    {
