...
LINKED_LIST *new_pragmas;
LINKED_LIST *file_pragmas;

void linked_list_push (LINKED_LIST **lla, void *data) {
  LINKED_LIST *new_node = (LINKED_LIST *)xmalloc(sizeof(LINKED_LIST));
  new_node->data=data;
  new_node->next=*lla;
  *lla=new_node;
}

void *linked_list_pop (LINKED_LIST **lla) {
  void *data = NULL;
  LINKED_LIST *first = *lla;
  if (first) {
    data = first->data;
    *lla=first->next;
    free(first);
...
void linked_list_remove_el (LINKED_LIST **lla) {
  linked_list_pop(lla);
}

PRAGMA_INFO *new_pragma_info () {
  PRAGMA_INFO *pi=(PRAGMA_INFO *)xmalloc(sizeof(PRAGMA_INFO));
  memset(pi,0,sizeof(PRAGMA_INFO));
  return pi;
}

void delete_pragma_info (PRAGMA_INFO *pi) {
  free(pi->name);
  free(pi->id);
  free(pi);
}

void pragma_list_push (LINKED_LIST **lla, PRAGMA_INFO *pi) {
  linked_list_push(lla,(void *)pi);
}

PRAGMA_INFO *pragma_list_pop (LINKED_LIST **lla) {
  return (PRAGMA_INFO *)linked_list_pop(lla);
}

void pragma_list_remove_el (LINKED_LIST **lla) {
  PRAGMA_INFO *pi=pragma_list_pop(lla);
  delete_pragma_info(pi);
}
