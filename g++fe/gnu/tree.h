...
#define TYPE_ATTR_ALIGN(NODE) (TYPE_CHECK (NODE)->type.attr_align)
...
struct tree_type
{
  ...
  unsigned int attr_fill;  /* attribute fill amount*/
};

...
/* Records the rodata section name in a section attribute.  Used to pass
   the name from decl_attributes to make_function_rtl and make_decl_rtl.  */
#define DECL_ROSECTION_NAME(NODE) (DECL_CHECK (NODE)->decl.ro_section_name)
...
/* Restricted pointers. */
#define DECL_RESTRICT(NODE) (DECL_CHECK (NODE)->decl.restrict_flag)
/* Holds the alignment required for the datum.  */
#define DECL_ALIGN(NODE) (DECL_CHECK (NODE)->decl.u1.a.align)
#define DECL_FILL(NODE) (DECL_CHECK (NODE)->decl.attr_fill)
#define DECL_ATTR_ALIGN(NODE) (DECL_CHECK (NODE)->decl.attr_align)
...
#define DECL_UNINLINABLE_ATTRIBUTE(NODE) (FUNCTION_DECL_CHECK (NODE)->decl.uninlinable_attribute)

#define DECL_ALWAYS_INLINE(NODE) (FUNCTION_DECL_CHECK (NODE)->decl.always_inline)

#define DECL_NORETURN(NODE) (FUNCTION_DECL_CHECK (NODE)->decl.noreturn)
...
/* Used to indicate that this DECL has asmspec */
#define DECL_HAS_ASMSPEC(NODE) (DECL_CHECK (NODE)->decl.has_asmspec_flag)
...
/* WARNING: Do not add tree pointer fields without updating ggc-common.c */
...
struct tree_decl
{
  ...
  unsigned uninlinable_attribute : 1;
  unsigned always_inline : 1;
  unsigned noreturn : 1;
  unsigned pointer_depth : 2;
  unsigned non_addressable : 1;
  unsigned restrict_flag : 1;
  unsigned attr_align : 5;
  ...
  union tree_node *ro_section_name;
  ...
  unsigned used_attribute : 1;  /* was a used attribute used on a function declaration */
  unsigned int attr_fill;               /* for attribute fill */
};

...
enum tree_index
{
  ...
  TI_FIRST_TIE_TYPE,
  TI_LAST_TIE_TYPE = TI_FIRST_TIE_TYPE + 127,
  TI_MAX = TI_LAST_TIE_TYPE
#else
  TI_MAX
...
#ifdef TENSILICA_CHANGES
#define tie_type_node(i)                global_trees[TI_FIRST_TIE_TYPE+(i)]
#endif /* TENSILICA_CHANGES */
...
#ifdef TENSILICA_CHANGES
extern tree make_tie_type               PARAMS ((int, int));
#endif /* TENSILICA_CHANGES */
...
extern void debug_tree_limit		PARAMS ((tree, int));
#ifdef BUFSIZ
...
typedef struct pragma_info PRAGMA_INFO;

extern LINKED_LIST *new_pragmas; /* new pragmas since last statement */
extern LINKED_LIST *file_pragmas; /* all file pragmas associated with next stmt */
...
extern void linked_list_push (LINKED_LIST **lla, void *data);
extern void *linked_list_pop (LINKED_LIST **lla);
extern void linked_list_remove_el (LINKED_LIST **lla);

/*

  pragma_info structure:
  
  #pragma name([id[,arg1[,arg2]]])
  
  decl points to the declaration statement of id
...
extern PRAGMA_INFO *new_pragma_info ();
extern void delete_pragma_info (PRAGMA_INFO *pi);
extern void pragma_list_push (LINKED_LIST **lla, PRAGMA_INFO *pi);
extern PRAGMA_INFO *pragma_list_pop (LINKED_LIST **lla);
extern void pragma_list_remove_el (LINKED_LIST **lla);
