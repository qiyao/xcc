  ...
  char **envp)	/* Array of environment pointers */
{
	...
	/* unlike in C front end, WFE_XC_Decl_Init() should not be called
	   here because for C++, the init_decl_processing() is called in
	   compile_file() (see below and gnu/toplev.c) so WFE_XC_Decl_Init()
	   is called there after normal decl initialization is done.
	*/
