...
#include "cp-tree.h"
#include "wfe_symtab_util.h"
extern void warning (char*,...);	// from gnu
extern int decode_reg_name (const char *);	// from gnu
extern void import_export_decl(tree decl);
}
...
Create_TY_For_Tree (tree type_tree, TY_IDX idx)
{
	...
	case UNION_TYPE:
		{	// new scope for local vars
                char *type_name = 
                  (TYPE_NAME(type_tree) && !ANON_AGGR_TYPE_P(type_tree)) ?
                  Get_Name(TYPE_NAME(type_tree)) : NULL;
		...
		if (TYPE_BINFO(type_tree) &&
		  ...
		  for (i = 0; i < TREE_VEC_LENGTH(basetypes); ++i) {
			...
			!TREE_VIA_VIRTUAL(binfo)) {
                      char *base_class_name = Get_Name(TYPE_NAME(basetype));
		      char base_fld_name[strlen(base_class_name)+7];
		      sprintf(base_fld_name, "_base.%s", base_class_name);
		      ++next_field_id;
		      ++anonymous_fields;
		      next_field_id += TYPE_FIELD_IDS_USED(basetype);
		      fld = New_FLD();
		      FLD_Init (fld, Save_Str(base_fld_name), 
	...
	case FUNCTION_TYPE:
		...
		// if return type needs to be returned by an invisible
		// first argument, then change return type to void 
		// and add in implicit argument.
		if (WFE_Addressable_Type(TREE_TYPE(type_tree))) {
		  TY_IDX old = ret_ty_idx;
		  ret_ty_idx = MTYPE_To_TY (MTYPE_V);
		  Set_TYLIST_type (New_TYLIST (tylist_idx), ret_ty_idx); //Void return
		  Set_TY_tylist (ty, tylist_idx);
		  Set_TYLIST_type (New_TYLIST (tylist_idx), 
				  Make_Pointer_Type(old)); // implicit return
		} else {
		     ...
		     num_args++, arg = TREE_CHAIN(arg))
		{
			...
			if (!WFE_Keep_Zero_Length_Structs    &&
				...
				DevWarn ("zero length struct encountered in function prototype at line %d", lineno);
                          ...
                          if (WFE_Addressable_Type(TREE_VALUE(arg))) {  //switch to reference
				Set_TYLIST_type (New_TYLIST (tylist_idx), 
						Make_Pointer_Type(arg_ty_idx));
			  } else {
...
Create_ST_For_Tree (tree decl_node)
{
  ...
  bool	     interface_specified = false;
  ...
  switch (TREE_CODE(decl_node)) {
	...
	// this has to be done before the call to import_expor_decl()
	interface_specified =
		DECL_INTERFACE_KNOWN(decl_node) &&
		!DECL_DEFER_OUTPUT(decl_node);

	if (DECL_INLINE(decl_node) && TREE_PUBLIC(decl_node)) {
	  /* for inline function used, put it in linkonce section
	     and allow inliner to delete it */
	  import_export_decl(decl_node);
    ...
    case VAR_DECL:
      {
		...
		sclass = SCLASS_EXTERN;
		  ...
		  DECL_INITIAL(decl_node) != error_mark_node)
		sclass = SCLASS_UGLOBAL;
	      else if (TREE_STATIC(decl_node)) {
		if ((flag_no_common || DECL_SECTION_NAME(decl_node))
#ifdef TARG_XTENSA
		    /* Can't have compiler generated temporaries be
                       non-common since that may cause multiple
                       definitions of the same symbol (in particular
                       the tinfo objects generated for rtti). */
		    && !DECL_ARTIFICIAL(decl_node)
#endif
		   )
		  sclass = SCLASS_UGLOBAL;
		else
	...
	if (qualifiers != TYPE_UNQUALIFIED) {
	  ...
	  TREE_TYPE(decl_node) = build_qualified_type(type_tree, qualifiers);
	}
	...
	if (TYPE_READONLY (TREE_TYPE (decl_node)))
          Set_TY_is_const (ty_idx);
	if (TYPE_VOLATILE (TREE_TYPE (decl_node)))
	...
	if (DECL_REGISTER(decl_node) && DECL_HAS_ASMSPEC(decl_node) &&
	    decode_reg_name (name)>0) {
	  char* asm_name = name;
	  ...
	  WFE_Record_Asmspec_For_ST (
		   st, decl_node, asm_name, decode_reg_name (asm_name));
	} else {
          ST_Init (st, Save_Str(name), CLASS_VAR, sclass, eclass, ty_idx);
	}
  ...
  if (TREE_CODE (decl_node) == FUNCTION_DECL && DECL_THIS_INLINE(decl_node) &&
      interface_specified) {
    if (DECL_EXTERNAL (decl_node)) {
      Set_ST_is_external_inline_func (st);
    } else if (DECL_WEAK(decl_node) && !DECL_COMDAT(decl_node)) {
      Set_PU_no_delete(Pu_Table [ST_pu (st)]);
    }
  }

#ifdef TARG_XTENSA
  /* Get the linkone section name for 'decl_node', if there is one. */
  if (!DECL_EXTERNAL (decl_node))
    Assign_Unique_Section(decl_node);
#endif
