...
extern void WFE_Record_Asmspec_For_Decl (tree decl, char *asmspec, int reg);
extern void WFE_Record_Asmspec_For_ST (ST *st, tree decl, char *asmspec, int reg);
...
/* The DECL or ST to use for the current function's return value if it
   is to be returned by invisible first arg. Returns NULL_TREE if
   result should be returned in normal manner. */
extern tree Current_Function_Result_Decl (void);
extern ST *Current_Function_Result_ST (void);

/* Return true if 'type' is addressable, and must therefore be
   passed/returned by invisible reference. */
extern bool WFE_Addressable_Type (tree type);
