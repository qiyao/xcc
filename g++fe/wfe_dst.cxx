...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/g++fe/RCS/wfe_dst.cxx,v $ $Revision: 1.17 $";
#endif /* _KEEP_RCS_ID */
...
DST_get_context(tree intree)
{
  ...
  while (ltree) {
    ...
    if (TREE_CODE(ltree) == FUNCTION_DECL) {
      ...
      DST_INFO_IDX l_dst_idx = DECL_DST_IDX(ltree);
...
((DECL_NAME(d))?IDENTIFIER_POINTER(DECL_NAME(d)):"?"):\
 "?2")
  ...
  {
      if (!ANON_AGGR_TYPE_P(node) && TYPE_NAME (node))
        {
                          ...
                          tree field)
{
  ...
  // gdb wants the linkage name here
  char *name = IDENTIFIER_POINTER(DECL_ASSEMBLER_NAME(field));

  tree ftype = TREE_TYPE(field);
  DST_INFO_IDX fidx = Create_DST_type_For_Tree(ftype, 0, parent_ty_idx);

  DST_INFO_IDX field_idx = DST_mk_variable(
        src,             // srcpos
        name,            // user typed name (not mangled)
        fidx,            // user typed type
        0,               // offset (fortran uses non zero )
        0,               // front-end variable (this is only a declaration)
  ...
  // Connect this declaration with the definition via DW_AT_specification
  DECL_DST_SPECIFICATION_IDX(field) = field_idx;
  DST_INFO_IDX def_idx = DECL_DST_IDX(field);
  if (!DST_IS_NULL(def_idx)) {
    DST_add_specification_to_variable(def_idx, field_idx);
  }
}
		...
		tree fndecl)
{
    ...
    else if (TREE_CODE(TREE_TYPE(fndecl)) == METHOD_TYPE) {
    ...
    char *linkage_name = IDENTIFIER_POINTER(DECL_ASSEMBLER_NAME (fndecl));
    ...
    DST_virtuality  virtuality = DECL_PURE_VIRTUAL_P(fndecl) ?
      DW_VIRTUALITY_pure_virtual : DECL_VIRTUAL_P(fndecl) ?
      DW_VIRTUALITY_virtual : DW_VIRTUALITY_none;

    DST_access access = TREE_PRIVATE(fndecl) ?
      DW_ACCESS_private : TREE_PROTECTED(fndecl) ?
      DW_ACCESS_protected : DW_ACCESS_public;
    ...
    DST_vtable_elem_location vtable_elem_location = 
	    DECL_VINDEX (fndecl) ? TREE_INT_CST_LOW (DECL_VINDEX (fndecl)):0;
		...
		tree field)
{
    ...
    DST_access access = (TREE_PRIVATE(field) || DECL_VIRTUAL_P(field)) ?
      DW_ACCESS_private : TREE_PROTECTED(field) ?
      DW_ACCESS_protected : DW_ACCESS_public;
		...
		INT tsize)
{
	...
	if (TREE_CODE(type_tree) == RECORD_TYPE) {
          ...
          if (CLASSTYPE_DECLARED_CLASS(type_tree)) {

	    bool vtype_self_reference = false;

	    /* GNU extension: Record what type our vtable lives in.  */
	    DST_INFO_IDX bcidx = DST_INVALID_IDX;
	    if (TYPE_VFIELD (type_tree)) {

	      tree vtype = DECL_FCONTEXT (TYPE_VFIELD (type_tree));
	      if (type_tree==vtype)
		vtype_self_reference = true;
	      else {
	        TY_IDX itx =  TYPE_TY_IDX(vtype);
	        bcidx =
	          Create_DST_type_For_Tree (vtype, itx, idx);
	      }
	    }

            dst_idx = DST_mk_class_type(src,
                                        name,
                                        tsize,
                                        DST_INVALID_IDX, // not inlined
					bcidx,
                  /* 1 if incomplete */ TREE_PURPOSE(type_tree)== 0,
					vtype_self_reference);
          } else {
  	...
  	// Do the base classes
        ...
        if (TYPE_BINFO(type_tree) &&
          ...
          for (i = 0; i < TREE_VEC_LENGTH(basetypes); ++i) {
		    ...
		    bool use_gcc_offset = false;
		    INT32 gcc_offset=0;
		    if (TREE_CODE (binfo) == TREE_VEC) {
		      gcc_offset = tree_low_cst (BINFO_OFFSET (binfo), 0);
		      use_gcc_offset = true;
		    }
		    ...
		    DST_access access = TREE_VIA_PUBLIC(binfo) ?
                      DW_ACCESS_public : TREE_VIA_PROTECTED(binfo) ?
                      DW_ACCESS_protected: DW_ACCESS_private;
		    ...
		    DST_INFO_IDX inhx =
		      DST_mk_inheritance(src,
				...
				access,
			        virtuality,
				use_gcc_offset? gcc_offset : offset);
...
//
        ...
        if (TREE_CODE(TYPE_NAME(type_tree)) == TYPE_DECL &&
          ...
          return DST_Create_type(0, TYPE_NAME(type_tree));
        }
#endif
    }
   ...
   case INTEGER_TYPE:
    ...
    case POINTER_TYPE:
		  ...
		  if(TYPE_CONTEXT(type_tree)==NULL) {
		    current_scope_idx =
			DST_get_context(TYPE_CONTEXT(ttree));
		  }
                  DST_append_child(current_scope_idx,dst_idx);
...
DST_Create_var(ST *var_st, tree decl)
{
  ...
  // If it is external, then unless it hides some local it need not
  ...
  char *name = Get_Name(decl);

  BOOL class_var_found_member = FALSE;
  DST_INFO_IDX class_var_idx = DST_INVALID_INIT;
  if (context && TREE_CODE(context) == RECORD_TYPE) {
    // gdb likes linkage name for static data members
    name = IDENTIFIER_POINTER(DECL_ASSEMBLER_NAME(decl));
    // look for static data member declaration
    class_var_idx = DST_find_class_member(name, context);
    if (!DST_IS_NULL(class_var_idx)) {
      class_var_found_member = TRUE;
    }
  }

  DST_INFO_IDX type = TYPE_DST_IDX(TREE_TYPE(decl));

  DST_INFO_IDX dst = DST_mk_variable(
	src,                       // srcpos
	name,                      // name
	type,                      // user type name here
	0,                         // offset (fortran uses non zero )
  ...
  // If this is a def of a static data member,
  // add DW_AT_specification to point to class member
  if (class_var_found_member) {
    DST_add_specification_to_variable(dst, class_var_idx);
  }

  DST_append_child (DST_get_context(DECL_CONTEXT(decl)), dst);
  return dst;
}
  ...
  int is_declaration_only)
{
	...
	DST_INFO_IDX initinfo = DST_INVALID_INIT;
	...
	DST_INFO_IDX aroot = DST_INVALID_INIT;
...
DST_Create_Subprogram (ST *func_st,tree fndecl)
{
    ...
    // early return for an asm stmt treated as a function
    if (fndecl==NULL)
      return dst;
...
WFE_Set_Line_And_File (UINT line, const char* f)
{
	...
	else if (file[0] == '.' && is_dir_separator (file[1])) {
		// current dir
		dir = current_working_dir;
	}
	else {
