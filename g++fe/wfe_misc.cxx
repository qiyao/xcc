...
#if (TI_ISA_REGCLASS_MAX+1)*(TI_ISA_REGISTER_MAX+1) >= FIRST_PSEUDO_REGISTER
#error "FIRST_PSEUDO_REGISTER error"
#endif
...
WFE_Init (INT argc, char **argv, char **envp )
{
  ...
  if (strcmp(xt_abi_string, "call0") == 0)
    ABI_Name = "call0";
  else if (strcmp(xt_abi_string, "windowed") == 0)
    ABI_Name = "windowed";
#endif
  Init_Controls_Tbl();
  Argc = argc;
...
static int reg_class_begin[TI_ISA_REGCLASS_MAX+1];
static int reg_class_end[TI_ISA_REGCLASS_MAX+1];

ST* WFE_Dedicated_Preg_ST(int gcc_reg) {
  ST* st = NULL;
  ISA_REGCLASS rc;
  FOR_ALL_ISA_REGCLASS(rc) {
    if (reg_class_begin[rc] <= gcc_reg &&
      gcc_reg <= reg_class_end[rc]) {
      TYPE_ID mtype = TI_ISA_Mtype_For_Regclass(rc);
...
int first_pseudo_register = 0;

void
WFE_Target_Init ()
{
  /* Initialize register names using targ_info. */
  ...
  FOR_ALL_ISA_REGCLASS(rc) {
    ...
    bool reg_class_disabled=false;

    if (rc==ISA_REGCLASS_macc && !xt_mac16) {
      reg_class_disabled=true;
    } else if (rc==ISA_REGCLASS_branch && !xt_booleans) {
      reg_class_disabled=true;
    }
  
    reg_class_begin[rc] = pseudo;
    for (UINT i = 0; i < cnt; i++) {
      FmtAssert(pseudo < FIRST_PSEUDO_REGISTER,
		("Too many registers, this frontend supports at most %d",
		 FIRST_PSEUDO_REGISTER));
      
      if (reg_class_disabled==false)
        reg_names[pseudo] = TI_ISA_Regclass_Reg_Name(info, i);
      else
        reg_names[pseudo] = no_name;
      pseudo++;
    }
    reg_class_end[rc] = pseudo - 1;
  }

  first_pseudo_register = pseudo;
  
  for (; pseudo < FIRST_PSEUDO_REGISTER; pseudo++)
    reg_names[pseudo] = no_name;
}
...
char * WFE_Stmt_Kind_Name [wfe_stmk_last+1] = {
  ...
  "'while condition'",
  "'while body'",
  "'conditional while exit'",
  "'switch'",
  "'comma'",
  "'rcomma'",
  "'wfe_stmk_temp_cleanup'",
  "'wfe_stmk_gen_cleanup'",
  "'wfe_stmk_eh_cond'",
  "'wfe_stmk_try_block'",
  "'wfe_stmk_cleanup_point'",
  "'last'"
};
...
#ifdef WFE_DEBUG
  fprintf(stderr, "pushing %x, %s\n", wn, WFE_Stmt_Kind_Name[kind]);
#endif
...
WFE_Stmt_Pop (WFE_STMT_KIND kind)
{
  ...
  FmtAssert (wn_stmt_sp >= wn_stmt_stack,
             ("no more entries on stack in function wn_stmt_pop"));

#ifdef WFE_DEBUG
  if (wn_stmt_sp->kind != kind) {
    fprintf(stderr, "** stmt stack **\n");
    for (WN_STMT *scan = wn_stmt_sp ; scan >= wn_stmt_stack; scan--) {
      fprintf(stderr, "%x, %s\n",
	      scan->wn, WFE_Stmt_Kind_Name[scan->kind]);
    }
  }
#endif
...
#ifdef WFE_DEBUG
  fprintf(stderr, "popping %x, %s\n",
	  wn_stmt_sp->wn, WFE_Stmt_Kind_Name[wn_stmt_sp->kind]);
#endif
