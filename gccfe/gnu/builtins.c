...
static rtx expand_builtin_clz		PARAMS ((tree, rtx, rtx));
static rtx expand_builtin_ctz		PARAMS ((tree, rtx, rtx));
...
static rtx
expand_builtin_clz (arglist, target, subtarget)
     tree arglist;
     rtx target, subtarget;
{
  rtx op0;
  ...
  /* Compute the argument.  */
  op0 = expand_expr (TREE_VALUE (arglist), subtarget, VOIDmode, 0);
  /* Compute clz, into TARGET if possible.
     Set TARGET to wherever the result comes back.  */
  target = expand_unop (TYPE_MODE (TREE_TYPE (TREE_VALUE (arglist))),
			clz_optab, op0, target, 1);
  if (target == 0)
    abort ();
  return target;
}
...
static rtx
expand_builtin_ctz (arglist, target, subtarget)
     tree arglist;
     rtx target, subtarget;
{
  rtx op0;
  ...
  /* Compute the argument.  */
  op0 = expand_expr (TREE_VALUE (arglist), subtarget, VOIDmode, 0);
  /* Compute ctz, into TARGET if possible.
     Set TARGET to wherever the result comes back.  */
  target = expand_unop (TYPE_MODE (TREE_TYPE (TREE_VALUE (arglist))),
			ctz_optab, op0, target, 1);
  if (target == 0)
    abort ();
  return target;
}
     ...
     int ignore;
{
  ...
  switch (fcode)
    ...
    case BUILT_IN_CLZ:
      target = expand_builtin_clz (arglist, target, subtarget);
      if (target)
	return target;
      break;

    case BUILT_IN_CTZ:
      target = expand_builtin_ctz (arglist, target, subtarget);
      if (target)
	return target;
      break;
