...
extern int is_xtbool1_type(tree type);	/* in wfe_misc.cxx */
...
enum attrs {A_PACKED, A_NOCOMMON, A_COMMON, A_NORETURN, A_CONST, A_T_UNION,
	    ...
	    A_CONSTRUCTOR, A_DESTRUCTOR, A_MODE, A_SECTION, A_RODATA_SECTION, 
	    A_ALIGNED, A_FILLED, A_UNUSED, A_USED, A_FORMAT, A_FORMAT_ARG, A_WEAK, 
	    A_ALIAS, A_MALLOC,
	    A_NO_LIMIT_STACK, A_PURE,A_NOINLINE,A_INLINE,
#ifdef SGI_MONGOOSE
	    A_SYSCALL_LINKAGE, A_WIDEN_RETVAL
#endif /* SGI_MONGOOSE */
};

#define ATTR_COUNT (A_WIDEN_RETVAL + 1)
...
/* ATTR_COUNT is zero based, as is the size of the array, and one attribute has two names (A_SYSCALL_LINKAGE) so must add three for the array element count, plus each name has two entries */

static struct {enum attrs id; tree name; int min, max, decl_req;} attrtab[(ATTR_COUNT + 3) * 2];
...
init_attributes ()
{
  ...
  add_attribute (A_USED, "used", 0, 0, 0);
  ...
  add_attribute (A_RODATA_SECTION, "rodata_section", 1, 1, 1);
  add_attribute (A_ALIGNED, "aligned", 0, 1, 0);
  add_attribute (A_FILLED, "filled", 0, 1, 0);
  ...
  add_attribute (A_NOINLINE, "noinline", 0, 0, 1);
  add_attribute (A_INLINE, "always_inline", 0, 0, 1);
  /* if you create a new attribute, make sure you bump the size of
     attrtab by *2* entries. One for "foo" and one for "__foo__". */
}

...
static UINT64 Get_Integer_Value (tree exp)
{
  ...
  for (a = attributes; a; a = TREE_CHAIN (a))
	...
	case A_UNUSED:
	  if (is_type)
	    ...
	    node->decl.used_attribute = 1; /* Don't delete functions declared as unused */
	    TREE_USED (decl) = 1;
	  } else
	    warning ("`%s' attribute ignored", IDENTIFIER_POINTER (name));
	  break;
	...
	case A_RODATA_SECTION:
#ifdef ASM_OUTPUT_SECTION_NAME
	  if (TREE_CODE (decl) == FUNCTION_DECL)
	    {
	      if (DECL_ROSECTION_NAME (decl) != NULL_TREE
		  && strcmp (TREE_STRING_POINTER (DECL_ROSECTION_NAME (decl)),
			     TREE_STRING_POINTER (TREE_VALUE (args))) != 0)
		error_with_decl (node,
				 "rodata_section of `%s' conflicts with previous declaration");
	      else
		DECL_ROSECTION_NAME (decl) = TREE_VALUE (args);
	    }
	  else
	    error_with_decl (node,
			   "rodata_section attribute not allowed for `%s'");
#else
	  error_with_decl (node,
		  "section attributes are not supported for this target");
#endif
	...
	case A_NOINLINE:
	  {
	    if (TREE_CODE(decl) == FUNCTION_DECL) {
	      DECL_UNINLINABLE_ATTRIBUTE(decl) = 1;
	    } else {
  	      warning("noinline attribute ignored \n");
	    }
	  }
	  break;
	case A_INLINE:
	...
	case A_ALIGNED:
	  {
	    ...
	    if ((i = tree_log2 (align_expr)) == -1)
	      ...
	      if (i<(1<<5) &&
		  (is_type ||
		   ((TREE_CODE(decl) == VAR_DECL || TREE_CODE(decl) == FIELD_DECL) &&
		    (current_function_decl == NULL_TREE || TREE_STATIC (decl))))) {
		/* allow any alignment for global and static variables, and types */
		
		/* nothing */
	      } else {
		if ((1 << i) > align_units) {
		  warning ("requested alignment %d too large, assumed %d.",
			   (1 << i), align_units);
		  i = exact_log2(align_units);
		}
	      }
		...
		if (TREE_CODE (decl) != VAR_DECL &&
		  ...
		  DECL_ATTR_ALIGN (decl) = i;
		}
	      } else if (type) {
		TYPE_ALIGN (type) = (1 << i) * BITS_PER_UNIT;
		TYPE_ATTR_ALIGN (type) = i;
	      } else {
	...
	case A_FILLED:
	  {
	    ...
	    /* Strip any NOPs of any kind.  */
	    while (TREE_CODE (fill_expr) == NOP_EXPR
		   || TREE_CODE (fill_expr) == CONVERT_EXPR
		   || TREE_CODE (fill_expr) == NON_LVALUE_EXPR)
	      fill_expr = TREE_OPERAND (fill_expr, 0);

	    if (TREE_CODE (fill_expr) != INTEGER_CST)
	      {
		error ("requested fill is not a constant");
		continue;
	      }

	    if ((i = tree_log2 (fill_expr)) == -1)
	      error ("requested fill is not a power of 2");
	    else if (i > HOST_BITS_PER_INT - 2)
	      error ("requested fill is too large");
	    else {
	      
              if (!TREE_STATIC(decl)) {
		error_with_decl (decl,
				   "fill may only currently be specified on globals or statics `%s'");
              }
		...
		if (TREE_CODE (decl) != VAR_DECL &&
		    TREE_CODE (decl) != FIELD_DECL ) {
		  error_with_decl (decl,
				   "fill may not be specified for `%s'");
		} else {
		  DECL_FILL (decl) = i;
		}
	      } else {
		error_with_decl (decl, "fill may not be specified for `%s'");
	      }
	    }
	  }
	  break;
...
#ifdef TENSILICA_CHANGES
  if (TREE_CODE (TREE_TYPE (expr)) == TIE_TYPE &&
      is_xtbool_type(TREE_TYPE(expr))) {
    if (is_xtbool1_type(TREE_TYPE(expr))) {
      return expr;
    } else {
      error ("Cannot use xtbool 2,4,8,16 types here");
...
#ifdef TARG_XTENSA
          if (TREE_CODE(decl) == VAR_DECL &&
              !binding_level_keep_restrict())
            {
              if (!DECL_IN_SYSTEM_HEADER(decl))
                warning("`restrict' qualifier for inner-scope variable `%s' ignored",
                        IDENTIFIER_POINTER(DECL_NAME(decl)));
            }
          else if (TREE_CODE(decl) == FIELD_DECL)
            {
              if (!DECL_IN_SYSTEM_HEADER(decl))
                warning("`restrict' qualifier for field `%s' ignored",
                        IDENTIFIER_POINTER(DECL_NAME(decl)));
            }
          else
            {
              /* Propagate the restrict attribute to the backend. */
    ...
    int cplus_mode, no_builtins, no_nonansi_builtins;
{
  ...
  builtin_function ("__builtin_clz", int_ftype_int, BUILT_IN_CLZ,
		    BUILT_IN_NORMAL, NULL_PTR);
  builtin_function ("__builtin_ctz", int_ftype_int, BUILT_IN_CTZ,
		    BUILT_IN_NORMAL, NULL_PTR);
