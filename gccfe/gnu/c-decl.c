...
int warn_unknown_pragmas = 1; /* Tri state variable.  */  
...
binding_level_keep_restrict (void)
{
  /* The restrict qualifier is kept for variables declared
     ...
     scope (see the documentation for the 'tag_transparent'
     fields above). */
     ...
     int different_binding_level;
{
  ...
  if (TREE_CODE (newdecl) == FUNCTION_DECL)
	...
	DECL_INLINE (olddecl) = 1;
      ...
      DECL_UNINLINABLE_ATTRIBUTE (newdecl) = DECL_UNINLINABLE_ATTRIBUTE (olddecl)
	       		= (DECL_UNINLINABLE_ATTRIBUTE (newdecl) ||
			   DECL_UNINLINABLE_ATTRIBUTE (olddecl));

      DECL_ALWAYS_INLINE (newdecl) = DECL_ALWAYS_INLINE (olddecl)
	       		= (DECL_ALWAYS_INLINE (newdecl) ||
			   DECL_ALWAYS_INLINE (olddecl));

      DECL_NORETURN (newdecl) = DECL_NORETURN (olddecl)
	       		= (DECL_NORETURN (newdecl) ||
			   DECL_NORETURN (olddecl));
     ...
     But preserve OLDDECL's DECL_UID.  */
  {
    ...
    tree decl_dst_type = DECL_DST_TYPE(olddecl);
#endif /* SGI_MONGOOSE */
    ...
    DECL_DST_TYPE(olddecl) = decl_dst_type;
    DECL_DST_TYPE(newdecl) = decl_dst_type;
#endif /* SGI_MONGOOSE */
  }
      ...
      if (TREE_CODE (x) == TYPE_DECL)
        {
	      ...
	      DECL_ORIGINAL_TYPE (x) = tt;
              ...
              TYPE_NAME (tt) = x;
	      if (DECL_ALIGN(x)>TYPE_ALIGN(tt))
		TYPE_ALIGN (tt) = DECL_ALIGN (x);
	      if (DECL_ATTR_ALIGN(x)>TYPE_ATTR_ALIGN(tt))
		TYPE_ATTR_ALIGN (tt) = DECL_ATTR_ALIGN (x);
	      ...
	      else
		{
		  ...
		  if (!TREE_CONSTANT (size)) {
                    if (decl_context == PARM 
                        && size->common.side_effects_flag) {
		      error ("Declaration of a variable-size array parameter cannot have side effects");
                    }
                  }
#endif
		  if (pedantic)
...
#ifdef TENSILICA_CHANGES
	      else if (!is_tie_intrinsic(fndecl) &&
		       TREE_CODE (TREE_TYPE (parm)) == TIE_TYPE)
		{
		  error_with_decl (parm, "TIE type parameter `%s' not supported");
...
#ifdef TENSILICA_CHANGES
	  if (found && !is_tie_intrinsic(fndecl) &&
	      TREE_CODE (TREE_TYPE (found)) == TIE_TYPE)
	    {
	      error_with_decl (found, "TIE type parameter `%s' not supported");
	      TREE_TYPE (found) = error_mark_node;
	      DECL_ARG_TYPE (found) = error_mark_node;
	      layout_decl (found, 0);
	    }
#endif /* TENSILICA_CHANGES */
