     ...
     tree token;
{
  ...
  if (string == NULL)
	...
	case ps_idaa:
	  ...
	  if (ret_val) {
	    ret_val=WFE_idaa(idaa_name,idaa_id,idaa_arg1,idaa_arg2);
	    if (!ret_val)
	      warning("#pragma '%s' failed",idaa_name);
	  }
	  break;
	...
	case ps_frequency_hint:
	  if (state == ps_frequent || state == ps_never) {
	    WFE_Add_Frequency_Hint(state == ps_frequent);
	    ret_val = 1;
	  }
	  break;
	case ps_no_reorder_memory:
	  WFE_Add_Barrier(/*no_reorder_memory=*/TRUE,
			  /*flush_memory=*/FALSE,
			  /*no_reorder=*/FALSE,
			  /*flush=*/FALSE);
	  ret_val = 1;
	...
	case ps_flush_memory:
	  WFE_Add_Barrier(/*no_reorder_memory=*/FALSE,
			  /*flush_memory=*/TRUE,
			  /*no_reorder=*/FALSE,
			  /*flush=*/FALSE);
	  ret_val = 1;
	...
	case ps_super_swp:
	  if (state == ps_bad) {
	    warning("malformed or invalid '#pragma super_swp' ignored");
            ...
            WFE_Add_Super_SWP(super_swp_ii, super_swp_unroll);
	  }
          ret_val = 1;
	  break;
	case ps_swp_schedule:
	  if (state == ps_bad) {
	    warning("malformed (len>1024) or invalid '#pragma swp_schdule' ignored");
            ...
            WFE_Add_SWP_Schedule(swp_schedule_str);
	  }
          ret_val = 1;
	  break;
#endif
...
#endif
      /* any other supported idaa pragmas here and in wfe_decl.cxx */
