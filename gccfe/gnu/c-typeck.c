  ...
  for (valtail = values, typetail = typelist, parmnum = 0;
...
#ifdef TENSILICA_CHANGES
      if (!is_tie_intrinsic(fundecl)) {
	if (type && TREE_CODE(type)==TIE_TYPE) {
	  error ("TIE type is not supported at parameter %d for non-TIE function %s",
		parmnum + 1, IDENTIFIER_POINTER(DECL_NAME(fundecl)));
	  ...
	  if (type && convert_for_tie_type_ok(TREE_TYPE(val), type)) {}
	    ...
	    error ("no conversion from TIE type to parameter %d for non-TIE function %s",
		parmnum + 1, IDENTIFIER_POINTER(DECL_NAME(fundecl)));
      ...
      if (type != 0)
	{
	  ...
	  if (!COMPLETE_TYPE_P (type))
	    ...
	    {
#ifdef TENSILICA_CHANGES
	      if (is_out_or_inout_arg(fundecl, parmnum) && TREE_READONLY(val)) {
	  ...
	  (code1 == TIE_TYPE && is_xtbool1_type(TREE_TYPE(op1))))
	{
	  if (code==EQ_EXPR)
	    return build_unary_op (
			TRUTH_NOT_EXPR,
			build_binary_op (TRUTH_XOR_EXPR, op0, op1, 0),
			1);
	  else
	    return build_binary_op (TRUTH_XOR_EXPR, op0, op1, 0);
	}
#endif /* TENSILICA_CHANGES */
     ...
     int noconvert;
{
  ...
  switch (code)
    ...
    case TRUTH_NOT_EXPR:
#ifdef TENSILICA_CHANGES
      if (typecode == TIE_TYPE && is_xtbool1_type(TREE_TYPE(arg))) {
...
#ifdef TENSILICA_CHANGES
  if (type == error_mark_node || TREE_TYPE(value) == error_mark_node ||
      TYPE_MAIN_VARIANT(TREE_TYPE(value)) == error_mark_node)
    return error_mark_node;
  if (TREE_CODE(type)==TIE_TYPE ||
      TREE_CODE(TYPE_MAIN_VARIANT(TREE_TYPE(value)))==TIE_TYPE) {

    if (TYPE_MAIN_VARIANT(type) == TYPE_MAIN_VARIANT(TREE_TYPE(value)))
      return value;
    else if (!convert_for_tie_type_ok(TREE_TYPE(value), type)) {
  ...
  /* Conversions among pointers */
	      ...
	      == unsigned_type (TYPE_MAIN_VARIANT (ttr))))
	{
	  ...
	  /* Const and volatile mean something different for function types,
	    ...
	    {
#ifdef TENSILICA_CHANGES
	      if ((TYPE_QUALS (ttr) & ~TYPE_QUALS (ttl)) && errtype==0 &&
		  is_tie_intrinsic(fundecl) &&
		  (TYPE_QUALS (ttr) & ~TYPE_QUALS (ttl)) == TYPE_QUAL_VOLATILE)
	      {
...
#ifdef TENSILICA_CHANGES
  else if (codel == TIE_TYPE || coder == TIE_TYPE)
    {
     ...
     int require_constant, constructor_constant;
{
...
#ifdef TENSILICA_CHANGES
  if (TREE_CODE(type)==TIE_TYPE && require_constant &&
      TYPE_MAIN_VARIANT(type)!=TYPE_MAIN_VARIANT(TREE_TYPE(init))) {
    warning("Static initialization with conversion for TIE type not supported");
    warning_init("Will initialize using the integer value");
  }
#endif /* TENSILICA_CHANGES */
  ...
  if (inside_init == error_mark_node)  return error_mark_node;
...
#ifdef TENSILICA_CHANGES
      if (code == TIE_TYPE && require_constant)
        inside_init = init; /* no TIE conversion in static initialization */
      else
        inside_init = convert_for_assignment (type, init, _("initialization"),
				  NULL_TREE, NULL_TREE, 0);
#else
      inside_init
  ...
  if (TREE_CODE (constructor_type) == RECORD_TYPE
      ...
      if (TYPE_DOMAIN (constructor_type))
	{
	  ...
	  if (constructor_max_index 
	      && TYPE_SIZE (constructor_max_index) == -1
