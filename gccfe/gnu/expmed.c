  ...
  if (bitsize > BITS_PER_WORD)
      ...
      for (i = 0; i < nwords; i++)
	{
	  ...
	  if (result_part != target_part)
	    ...
	    if (target_part != const0_rtx)
	    /* additional check to avoid core dump.
	     * This could occur if a register variable (which will have
	     * SImode) of TIE type is assigned a TIE type value in a
	     * BIG endian config. The wordnum above could be messed up
	     * and target_part returned from operand_subword() is
	     * const0_rtx causing emit_move_insn() to abort (PR6537)
	     */
