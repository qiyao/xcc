...
#if !defined(__linux__) && !defined(__sun__) && !defined(__CYGWIN__) && !defined(_WIN32)
  unsigned int code : 8;
#else
...
/* Record that a struct/union type was incomplete so that we can
   update DST information for its members when it's completed.
   NOTE: TYPE_LANG_FLAG_0 and TYPE_LANG_FLAG_1 are used in c-tree.h.  */
#define C_TYPE_WAS_INCOMPLETE(type) TYPE_LANG_FLAG_2 (type)
#endif
...
#define DECL_DST_TYPE(NODE) (DECL_CHECK(NODE)->decl.decl_dst_type)
#endif /* SGI_MONGOOSE */
...
/* In a FUNCTION_DECL, nonzero if the function has an inline attribute
 * as opposed to the inilne keywork.  */
...
struct tree_decl
{
  ...
  union tree_node* decl_dst_type;	/* a qualified copy of the type */
#endif /* SGI_MONGOOSE */
};
...
extern int binding_level_keep_restrict  PARAMS ((void));
