 ...
 * Module: dst.c
 * $Revision: 1.17 $
 * $Date: 2000/07/05 19:01:33 $
 * $Author: murthy $
 * $Revision: 1.17 $
 * $Date: 2000/07/05 19:01:33 $
 * $Author: murthy $
 * $Source: /isms/cmplrs.src/osprey1.0/gccfe/RCS/wfe_dst.cxx,v $
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/gccfe/RCS/wfe_dst.cxx,v $ $Revision: 1.17 $";
#endif /* _KEEP_RCS_ID */
...
DST_get_context(tree intree)
{
  ...
  while (ltree) {
    ...
    if (TREE_CODE(ltree) == FUNCTION_DECL) {
      // This is the normal case for locals
      DST_INFO_IDX l_dst_idx;
      struct mongoose_gcc_DST_IDX l_tree_idx = DECL_DST_IDX(ltree);
      cp_to_dst_from_tree(&l_dst_idx, &l_tree_idx);
      return l_dst_idx;
    }

    Lmt_DevWarn(1, ("TODO: Handle scopes in DST_get_context"));

    if (DECL_P(ltree))
      ltree =  DECL_CONTEXT(ltree);
    else if (TYPE_P(ltree))
      ltree =  TYPE_CONTEXT(ltree);
    else if (TREE_CODE(ltree) == BLOCK)
      ltree = BLOCK_SUPERCONTEXT(ltree);
    else
      break;
  }

  // This is  the normal case for globals
...
Get_Dir_Dst_Info (char *name)
{
	...
	dir_dst_list.push_back (std::make_pair (new_name, ++last_dir_num));
	DST_mk_include_dir (new_name);
	return last_dir_num;
}
...
Get_File_Dst_Info (char *name, UINT dir)
{
	...
	// not found, so append file to dst list
        char *new_name = strdup(name);
	file_dst_list.push_back (std::make_pair (new_name, ++last_file_num));
	DST_enter_file (new_name, dir);
...
drop_path (char *s)
{
        char *t, *tail = 0;
#ifdef _WIN32
	if (s[0] && s[1]==':') s+=2; // skip over drive
#endif
	for (t = s; *t; t++) {
	...
	DST_INFO_IDX parent_idx  )
{
   ...
   // }
	...
	cp_to_dst_from_tree(&fidx,&g_idx);
        ...
        INT byteoff = Get_Integer_Value(DECL_FIELD_OFFSET(field));
        INT fld_offset_bytes = (BITSPERBYTE * byteoff + bitoff) / BITSPERBYTE;
	...
	if (isbit == 0) {
	  ...
	  if (tsize == 0) {
          ...
          UINT bit_size = Get_Integer_Value(DECL_SIZE(field));
	  UINT container_off = fld_offset_bytes - (fld_offset_bytes % align);
	  UINT into_cont_off = bitoff + (byteoff - container_off) * BITSPERBYTE;
          if (Target_Byte_Sex == LITTLE_ENDIAN) {
            into_cont_off = tsize * BITSPERBYTE - (into_cont_off + bit_size);
          }
	  ...
	  field_idx = DST_mk_member(
			...
			tsize, // container size, bytes
                        into_cont_off, // offset into container, bits
                        bit_size, // bitfield size
                        TRUE, // a bitfield
                        FALSE, // not a static member
                        FALSE, // Only TRUE for C++?
                        FALSE); // artificial (no)
...
// We have a struct/union. Create a DST record and enter it.
static DST_INFO_IDX
DST_enter_struct_union(tree type_tree, TY_IDX ttidx, TY_IDX idx, INT tsize)
{
  DST_INFO_IDX dst_idx ;
  struct mongoose_gcc_DST_IDX tdst = TYPE_DST_IDX(type_tree);
  cp_to_dst_from_tree(&dst_idx,&tdst);

  DST_INFO_IDX current_scope_idx = DST_get_context(TYPE_CONTEXT(type_tree));

  // not yet created, so create it
  if (DST_IS_NULL(dst_idx)) {

    C_TYPE_WAS_INCOMPLETE(type_tree) = (TREE_PURPOSE(type_tree) == NULL);

    // FIXME: Deal with scope here
    ...
    if (TREE_CODE(type_tree) == RECORD_TYPE) {
      dst_idx = DST_mk_structure_type(src,
                                      ...
                                      C_TYPE_WAS_INCOMPLETE(type_tree)
                                      );
    }
  ...
  // when an incomplete type is completed update DST info for members
  else if (C_TYPE_WAS_INCOMPLETE(type_tree) && TREE_PURPOSE(type_tree)) {
    C_TYPE_WAS_INCOMPLETE(type_tree) = 0;

    // reset the declaraion flag (see PR11842)
    DST_flag flag = DST_INFO_flag(DST_INFO_IDX_TO_PTR(dst_idx));
    DST_RESET_declaration(flag);
    DST_INFO_flag(DST_INFO_IDX_TO_PTR(dst_idx)) = flag;

    // pr1416 update the size 
    if (tsize && !DST_STRUCTURE_TYPE_byte_size( DST_ATTR_IDX_TO_PTR(
		DST_INFO_attributes(DST_INFO_IDX_TO_PTR(dst_idx)),
						DST_STRUCTURE_TYPE))) {
	    DST_STRUCTURE_TYPE_byte_size( DST_ATTR_IDX_TO_PTR(
		DST_INFO_attributes(DST_INFO_IDX_TO_PTR(dst_idx))
			,DST_STRUCTURE_TYPE)) = tsize;
    }


    // decend to where the real struct is and skip all the
    // dst for const or volatile qualifiers
    DST_INFO* info_ptr = DST_INFO_IDX_TO_PTR(dst_idx);
    while (DST_INFO_tag(info_ptr)==DW_TAG_volatile_type ||
	     DST_INFO_tag(info_ptr)==DW_TAG_const_type) {

      DST_ATTR_IDX attr_idx = DST_INFO_attributes(info_ptr);
      if (DST_INFO_tag(info_ptr)==DW_TAG_volatile_type) {
	DST_VOLATILE_TYPE *attr =
		DST_ATTR_IDX_TO_PTR(attr_idx, DST_VOLATILE_TYPE);
	dst_idx = DST_VOLATILE_TYPE_type(attr);
      } else {
	DST_CONST_TYPE *attr =
		DST_ATTR_IDX_TO_PTR(attr_idx, DST_CONST_TYPE);
	dst_idx = DST_CONST_TYPE_type(attr);
      }
      info_ptr = DST_INFO_IDX_TO_PTR(dst_idx);
    }
    DST_enter_struct_union_members(type_tree, dst_idx);
  } 
...
// and enter it.
static DST_INFO_IDX
DST_enter_enum(tree type_tree, TY_IDX ttidx, TY_IDX idx, INT tsize)
{
   ...
   if (DST_IS_NULL(dst_idx)) {
      ...
      dst_idx = DST_mk_enumeration_type(src,
                                        name1,
                                        e_tsize, // Type size.
                                        DST_INVALID_IDX, // Not inlined.
                                        (enum_entry==NULL_TREE)); //Incomplete?
      DST_append_child(current_scope_idx, dst_idx);

      cp_to_tree_from_dst(&tdst, &dst_idx);
      TYPE_DST_IDX(type_tree) = tdst;
...
DST_enter_array_type(tree type_tree, TY_IDX ttidx, TY_IDX idx, INT tsize)
{
  ...
  if (DST_IS_NULL(dst_idx)) {
    ...
    USRCPOS src;
    USRCPOS_clear(src); // should be: USRCPOS_srcpos(src) = Get_Srcpos();

    // compress arrays of arrays into a multi-dimensional array
    tree elem_tree = TREE_TYPE(type_tree);
    while (TREE_CODE(elem_tree) == ARRAY_TYPE)
      elem_tree = TREE_TYPE(elem_tree);

    // create DST for the element type
    DST_INFO_IDX elem_dst;
    struct mongoose_gcc_DST_IDX elem_aux =
      Create_DST_type_For_Tree(elem_tree, TYPE_TY_IDX(elem_tree), idx);
    cp_to_dst_from_tree(&elem_dst, &elem_aux);

    dst_idx = DST_mk_array_type(src,
                                0, // array types are not named
                                elem_dst, // element type DST_INFO_IDX
                                tsize, // type size
                                DST_INVALID_IDX, // not inlined
                                (tsize == 0)); // pass non-zero if incomplete.
    DST_append_child(comp_unit_idx, dst_idx);

    for (tree array_type = type_tree;
         TREE_CODE(array_type) == ARRAY_TYPE;
         array_type = TREE_TYPE(array_type)) {

      BOOL const_lb = FALSE;
      BOOL const_ub = FALSE;
      DST_cval_ref lb, ub;
      lb.ref = DST_INVALID_IDX;
      ub.ref = DST_INVALID_IDX;

      tree domain = TYPE_DOMAIN(array_type);
      if (domain) {
        tree lower = TYPE_MIN_VALUE(domain);
        if (lower && TREE_CODE(lower) == INTEGER_CST)
          lb.cval = Get_Integer_Value(lower), const_lb = TRUE;
        tree upper = TYPE_MAX_VALUE(domain);
        if (upper && TREE_CODE(upper) == INTEGER_CST)
          ub.cval = Get_Integer_Value(upper), const_ub = TRUE;
	  if (ub.cval+1 == (1LL << 32)) {
	    // a special case for zero length array such as int a[0];
	    // since the 'upper' is an unsigned int it will
	    // carry 0xffffffff and Get_Integer_Value() will return
	    // 0x00000000ffffffff which will be a large positive INT64 number
	    // for ub.cval (of type DST_bounds_t)
	    ub.cval = -1LL;
	  }
      }

      DST_INFO_IDX subrange_dst_idx = DST_mk_subrange_type(const_lb,
                                                           lb,
                                                           const_ub,
                                                           ub);
      DST_append_child(dst_idx, subrange_dst_idx);
    }
...
//
...
#else
        // typedefs
        if (TREE_CODE(TYPE_NAME(type_tree)) == TYPE_DECL &&
            DECL_ORIGINAL_TYPE(TYPE_NAME(type_tree))) {
          dst_idx = DST_Create_type(0, TYPE_NAME(type_tree));
          cp_to_tree_from_dst(&actual_retval, &dst_idx);
          return actual_retval;
        }
#endif
   ...
   case INTEGER_TYPE:
    ...
    case POINTER_TYPE:
		  ...
		  // if there is no context, use the context of the pointed type
		  // so the current DST can be removed if the pointed type is
		  // see PR11155
		  if(TYPE_CONTEXT(type_tree)==NULL) {
		    current_scope_idx = DST_get_context(TYPE_CONTEXT(ttree));
		  }
    ...
    case FUNCTION_TYPE:
		{
                  mongoose_gcc_DST_IDX tdst = TYPE_DST_IDX(type_tree);
                  cp_to_dst_from_tree(&dst_idx, &tdst);
                  ...
                  if (DST_IS_NULL(dst_idx)) {
                    ...
                    USRCPOS src;
                    USRCPOS_clear(src);
                    tree ret_tree = TREE_TYPE(type_tree);
                    TY_IDX ret_idx = TYPE_TY_IDX(ret_tree);

                    DST_INFO_IDX ret_dst;
                    mongoose_gcc_DST_IDX ret =
		      Create_DST_type_For_Tree (ret_tree, ret_idx, idx);
		    cp_to_dst_from_tree(&ret_dst, &ret);
   ...
   if (TYPE_VOLATILE(type_tree) || TY_is_volatile(ttidx)) {
     dst_idx = DST_mk_volatile_type(dst_idx);
     DST_append_child(current_scope_idx, dst_idx);
   }
   if (TYPE_READONLY(type_tree) || TY_is_const(ttidx)) {
...
DST_Create_var(ST *var_st, tree decl)
{
    ...
    dst = DST_mk_variable(
        src,                    // srcpos
        IDENTIFIER_POINTER(DECL_NAME(decl)), // ST_name(var_st),
...
DST_Create_Subprogram (ST *func_st, tree fndecl)
{
    ...
    if (fndecl) {
      fn_dst_idx = DECL_DST_IDX(fndecl);
      cp_to_dst_from_tree(&dst, &fn_dst_idx);
      if (!DST_IS_NULL(dst)) {
        return dst;
      }
    }
    ...
    USRCPOS src;
    USRCPOS_srcpos(src) = Get_Srcpos();
    DST_INFO_IDX ret_dst = DST_INVALID_INIT;
	  ...
	  char *copts[]) /* The array of option strings passed to fec(c) */
{
   ...
   char* buff = (char *) alloca (strlen(xt_xcc_string) +
                                 sizeof(INCLUDE_STAMP) +
                                 strlen(comp_info) + 6);
   sprintf(buff, "%s for %s %s", xt_xcc_string, INCLUDE_STAMP, comp_info);
   ...
   WFE_Set_Line_And_File (0, Src_File_Name);
}
...
/* Update DST for Array if the size is known after initialization is
   processed
*/
void
Update_DST_For_Array(tree array_decl)
{
  tree array_type = TREE_TYPE(array_decl);

  tree dst_array_type = DECL_DST_TYPE(array_decl);
  if (dst_array_type==NULL || array_type==NULL)
    return;

  if (TREE_CODE(array_type)==ARRAY_TYPE) {

    DST_INFO_IDX array_dst;
    struct mongoose_gcc_DST_IDX g_idx = TYPE_DST_IDX(dst_array_type);
    cp_to_dst_from_tree(&array_dst, &g_idx);
    DST_INFO_IDX invalid_dst = DST_INVALID_INIT;

    if (array_dst != invalid_dst) {

      DST_INFO_IDX subrange = DST_first_child(array_dst);
 
      if (subrange != invalid_dst) {

	DST_INFO *info_ptr;
	info_ptr = DST_INFO_IDX_TO_PTR(subrange);

	DST_flag flag = DST_no_flag;
	flag = DST_INFO_flag(info_ptr);
	DST_ATTR_IDX attr_idx = DST_INFO_attributes(info_ptr);
	DST_SUBRANGE_TYPE *attrs =
			DST_ATTR_IDX_TO_PTR(attr_idx, DST_SUBRANGE_TYPE);
	...
	if (!DST_IS_lb_cval(flag) &&
	    DST_SUBRANGE_TYPE_lower_ref(attrs)==invalid_dst &&
	    !DST_IS_ub_cval(flag) &&
	    DST_SUBRANGE_TYPE_upper_ref(attrs)==invalid_dst &&
	    const_lb && const_ub) {

          DST_SUBRANGE_TYPE_lower_cval(attrs) = lb.cval;
          DST_SET_lb_cval(flag);
          DST_SUBRANGE_TYPE_upper_cval(attrs) = ub.cval;
