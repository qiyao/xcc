...
void
WFE_Convert_To_Host_Order (int *buf)
{
  if (!Same_Byte_Sex)
    {
      int t = buf[0];
...
extern BOOL OPT_Delay_Post;
extern BOOL OPT_Postfix_St;
static WN *Delay_Block = NULL;
...
{
  for (INT32 i = wfe_save_expr_stack_last; i >= 0; i--) {
    if (wfe_save_expr_stack [i].exp == save_exp) {
		       ...
		       bool is_imagpart)
{
  ...
  if (code == PARM_DECL || code == VAR_DECL) {
    ...
    BOOL postcrement_delayed = FALSE;
    ...
    if (!WFE_Keep_Zero_Length_Structs &&
      ...
      if (Delay_Block &&
          ...
          (assign_code == POSTINCREMENT_EXPR || 
           assign_code == POSTDECREMENT_EXPR)) {
        WN_INSERT_BlockLast(Delay_Block, wn);
        postcrement_delayed = OPT_Postfix_St;
      }
      else 
        WFE_Stmt_Append(wn, Get_Srcpos());
    }
    if (need_result) {
      if (! result_in_temp || postcrement_delayed)
        wn = WN_CreateLdid(OPR_LDID, rtype, desc, 
...
WFE_Expand_Expr_With_Sequence_Point (tree exp, TYPE_ID mtype)
{
  ...
  if (mtype == MTYPE_V) {
    ...
    BOOL use_delay_block = (OPT_Delay_Post && TREE_CODE(exp) != CALL_EXPR);

    if (use_delay_block) {
      old_delay_block = Delay_Block;
      Delay_Block = WN_CreateBlock ();
    }

    wn = WFE_Expand_Expr (exp, FALSE);

    if (use_delay_block) {
      if (WN_first (Delay_Block))
        WFE_Stmt_Append (Delay_Block, Get_Srcpos ());
      else
        WN_Delete (Delay_Block);
      Delay_Block = old_delay_block;
    }
  }
    ...
    if (WN_first (comma_block)) {
      if (wn) {
	if (WN_rtype(wn)==MTYPE_XTBOOL && mtype==Boolean_type)
...
static void
Mark_Malloc_Like_Call(WN *call_wn, tree fndecl)
{
  extern BOOL OPT_Malloc_Calls;

  /* These conditions mimic those from gnu/calls.c:special_function_p */
  if (OPT_Malloc_Calls &&
      fndecl &&
      DECL_NAME(fndecl) &&
      TREE_PUBLIC(fndecl) &&
...
//		wn = WN_Lda (Pointer_Mtype, WN_offset (wn), WN_st (wn));
	...
	wn = WN_CreateLdid (OPR_LDID, rtype,
			    ...
			    field_id != 0 ? hi_ty_idx : ldid_ty_idx, field_id);
	if (cvt_rtype)
	...
	for (list = TREE_OPERAND (exp, 1); list; list = TREE_CHAIN (list)) {
	  ...
	  if (DECL_BUILT_IN (func)) {
		...
		if (arglist == 0
		  ...
		  /* If we are copying a constant string, then we can
                     use memcpy since we know the length of the string
                     being copied. */
		  arg1 = TREE_VALUE (arglist);
		  arg2 = TREE_VALUE (TREE_CHAIN (arglist));
		  ...
		  if (!len1) {
		    ...
		    if (len2) {
		      max_subst_cnt = 2;
		      subst_arg_wn[0] = subst_arg_wn[1] = NULL;
		      subst_arg_wn[2] = WN_Binary(OPR_ADD, MTYPE_U4,
						  WFE_Expand_Expr(len2),
						  WN_Intconst(MTYPE_U4, 1));
		      subst_arg_ty_idx[2] = Be_Type_Tbl(MTYPE_U4);
		      iopc = INTRN_MEMCPY;
		      break;
		    }
		  }
...
//                wn = WFE_Expand_Expr (TREE_VALUE (TREE_OPERAND (exp, 1)));
		...
		tmp   = WN_Shl (Pointer_Mtype, arg1, WN_Intconst(MTYPE_I4, 2));
		wn   = WN_Lshr (Pointer_Mtype, tmp, WN_Intconst(MTYPE_I4, 2));
		whirl_generated = TRUE;
		break;
	      }
...
*/
	  ...
	  if (intrinsic_op) {
		...
		//	   eval tie_macro()
		// case 2. tie_macro(a,b,c) becomes
		//	   b = tie_macro(a,b,c) assuming b is the inout arg
		// case 3. tie_macro(a,b,c,d) becomes
		//	   T = tie_macro(c,d)
		//	   a = outpart(T,1)
	  ...
	  if (iopc) {
	    ...
	    Mark_Malloc_Like_Call(call_wn, func);
	  }
        }
	...
	for (list = TREE_OPERAND (exp, 1);
	     list || (subst_cnt <= max_subst_cnt);
	     list = ((list) ? TREE_CHAIN (list) : list), subst_cnt++) {
	  if ((max_subst_cnt >= 0) && subst_arg_wn[subst_cnt])
	    {
	      arg_wn = subst_arg_wn[subst_cnt];
	      arg_ty_idx = subst_arg_ty_idx[subst_cnt];
	    }
	  else
	    {
	      FmtAssert(list, ("Missing substitute argument"));
	      arg_wn     = WFE_Expand_Expr (TREE_VALUE (list));
	      arg_ty_idx = Get_TY(TREE_TYPE(TREE_VALUE(list)));
	      if (tie_macro &&
	...
	if (tie_macro && tie_macro->is_c_function()) {
	  ...
	  WN_kid (call_wn, out_index) =
		WN_CreateParm (Mtype_comparison(out_mtype), ldid,
			       MTYPE_To_TY(out_mtype), WN_PARM_BY_VALUE);
	...
	if (tie_macro) {
	  ...
	  if (ret_mtype == MTYPE_M) { // copy the -1 preg to a temp area
	    ...
	    wn1 = WN_CreateLdid(OPR_LDID, rtype,
				...
				field_id);
            if (cvtl_size != 0)
              wn1 = WN_CreateCvtl(OPR_CVTL, rtype, MTYPE_V, cvtl_size, wn1);
	  }
