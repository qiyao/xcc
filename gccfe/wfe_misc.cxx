...
Prepare_Source ( void )
{
  ...
  while ( ++Source_Arg <= Argc ) {
	...
	if ( (Irb_File = fopen ( Irb_File_Name, "wb" )) == NULL ) {
...
//Initialize_C_Int_Model();
  ...
  if (strcmp(xt_abi_string, "call0") == 0)
    ...
    if (strcmp(xt_abi_string, "windowed") == 0)
      ABI_Name = "windowed";
    else
      error("Invalid abi name: %s", xt_abi_string);
#endif
  Init_Controls_Tbl();
  Argc = argc;
  Argv = argv;
...
INT WFE_Stmt_Empty() { return wn_stmt_sp < wn_stmt_stack; };
