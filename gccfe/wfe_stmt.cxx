...
WFE_Expand_Start_Cond (tree cond, int exitflag)
{
  ...
  if (WN_rtype(test)==MTYPE_XTBOOL)
    test = WN_Cvt(WN_rtype(test), MTYPE_I4, test);
...
WFE_Expand_End_Loop (void)
{
  ...
  if (!loop_info_stack [loop_info_i].exit_loop_if_false) {
    ...
    WFE_Stmt_Append (while_stmt, WN_linenum(while_body));
    WFE_Stmt_Push (while_body, wfe_stmk_while_body, (SRCPOS)0);
  }
