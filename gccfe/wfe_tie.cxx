...
extern "C" tree lookup_name(tree name);
extern "C" tree c_build_qualified_type(tree, int);	/* for c fe */
extern "C" tree build_qualified_type(tree, int);	/* for c++ fe */
...
get_type_node(const char *input_type_name) {
  ...
  if (!strcmp(type_name,"_TIE_int64"))
    ...
    if (id != NULL_TREE) {
      ...
      if (name != NULL_TREE)
	type_node = TREE_TYPE(lookup_name(get_identifier(type_name)));
    }
