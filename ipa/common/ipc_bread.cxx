...
STR_TO_IDX_MAP DST_Merged_File_Map; // global map for merging DST file tables
...
static void 
IP_READ_remap_pu_infos(PU_Info* base, const IPC_GLOBAL_IDX_MAP* idx_map)
{
...
void 
remap_ST_index(DST_ASSOC_INFO& idx) 
{
...
INT32 
remap_dst_entry(INT32 /* dummy */, DST_DW_tag tag, DST_flag flag,
...
void 
remap_dst(IP_FILE_HDR& s) 
{
  static UINT DST_Merged_File_Count = 0;
  ...
  DST_IDX file_dst_idx = DST_get_file_names(); 
  while (file_dst_idx != DST_INVALID_IDX) {

    DST_FILE_NAME *file_dst = DST_FILE_IDX_TO_PTR(file_dst_idx);
    const char *file_name = DST_STR_IDX_TO_PTR(DST_FILE_NAME_name(file_dst));

    STR_TO_IDX_MAP::iterator file_iter = DST_Merged_File_Map.find(file_name);
    if (file_iter == DST_Merged_File_Map.end())
      DST_Merged_File_Map[file_name] = ++DST_Merged_File_Count;

    IP_FILE_HDR_dst_file_idx_map(s).push_back(DST_Merged_File_Map[file_name]);
    file_dst_idx = DST_FILE_NAME_next(file_dst);
  }
...
static INT
IP_READ_fix_tree (WN *node, char *base, Elf64_Word size, IP_FILE_HDR& s)
{
    const IPC_GLOBAL_IDX_MAP* idx_map = IP_FILE_HDR_idx_maps(s);
    ...
    Count_WN_Opcode (node, &PU_WN_BB_Cnt, &PU_WN_Stmt_Cnt);
#endif
    ...
    if (opcode == OPC_BLOCK) {
        ...
        if (wn == (WN *) -1) {
            ...
            do {
                if (IP_READ_fix_tree (wn, base, size, s) == ERROR_VALUE)
                    return ERROR_VALUE;
                wn = WN_next(wn);
            } while (wn);
        }
    ...
    if (OPCODE_has_next_prev(opcode)) {
        ...
        // Remap file index for the merged DST file table
        SRCPOS srcpos = WN_Get_Linenum(node);
        SRCPOS_filenum(srcpos) = 
          IP_FILE_HDR_dst_file_idx_map(s)[SRCPOS_filenum(srcpos)];
        WN_Set_Linenum(node, srcpos);
    }
...
static WN *
IP_READ_get_tree (void *handle, PU_Info *pu, IP_FILE_HDR& s)
{
    ...
    if (IP_READ_fix_tree (wn, tree_base, size, s) == ERROR_VALUE)
      return reinterpret_cast<WN*>(ERROR_VALUE);

#if defined(BACK_END)
    PU_WN_BB_Cnt--; // don't count the last RETURN node in PU
#endif
    WN_next(wn) = NULL;
    WN_prev(wn) = NULL;
...
// IP_READ_pu.
  ...
  // Function object that updates all indices in an st_attr entry that
  // might have been changed by merging the global symtabs.  The only
  // such index is the STR_IDX.
  struct fix_st_attr_entry {
    const SYMSTR_IDX_MAP& str_map;

    fix_st_attr_entry(const IPC_GLOBAL_IDX_MAP* idx_map)
      : str_map(idx_map->sym_str)
      {}

    void operator()(UINT32, ST_ATTR* st_attr) const {
      if (ST_ATTR_kind(*st_attr) == ST_ATTR_SECTION_NAME)
        Set_ST_ATTR_section_name(*st_attr,
                                 str_map[ST_ATTR_section_name(*st_attr)]);
    }
  };
  ...
  void fix_local_st_attr_tab(SYMTAB_IDX level,
                             const IPC_GLOBAL_IDX_MAP* idx_map) {
    For_all(St_Attr_Table, level, fix_st_attr_entry(idx_map));
  }
...
IP_READ_pu (IPA_NODE* node, IP_FILE_HDR& s, INT p_index, MEM_POOL *pool)
{
  ...
  if (IP_PROC_INFO_state(proci) == IPA_ORIG) {
    ...
    if (IP_READ_get_tree (fhandle, pu, s) == (WN*) -1) {
    ...
    fix_local_st_attr_tab(CURRENT_SYMTAB, IP_FILE_HDR_idx_maps(s));
