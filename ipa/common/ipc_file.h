...
typedef __gnu_cxx::hash_map<const char*, UINT, __gnu_cxx::hash<const char*>,
			    str_eq> STR_TO_IDX_MAP;

extern STR_TO_IDX_MAP DST_Merged_File_Map;
...
struct IP_FILE_HDR
{
    ...
    int         file_desc;		// file descriptor from open()
    void *input_map_addr;		// address for file's memory map
    off_t input_map_size;		// size of file's memory map
    ...
    vector<UINT> dst_file_idx_map;      // Map from original DST file indices
                                        // to merged DST file table indices
    // constructor

    IP_FILE_HDR (const char *name, void *mmap_addr, off_t mmap_size = 0, int fd = -1) {
	memset(this, 0, sizeof(IP_FILE_HDR));
	file_name = name;
	file_desc = fd;
	input_map_addr = mmap_addr;
	input_map_size = mmap_size;
	MEM_POOL_Initialize (&mem_pool, const_cast<char *> (file_name),
			     FALSE /* non-zero mempool */);
	MEM_POOL_Push (&mem_pool);
        dst_file_idx_map.push_back(0);
    }
...
inline int
IP_FILE_HDR_file_desc (const IP_FILE_HDR& hdr) {
    return hdr.file_desc;
}
...
inline off_t
IP_FILE_HDR_input_map_size (const IP_FILE_HDR& hdr) {
    return hdr.input_map_size;
}
...
inline DST_TYPE 
IP_FILE_HDR_dst(const IP_FILE_HDR& h) {
  return h.dst;
}

inline void 
Set_IP_FILE_HDR_dst(IP_FILE_HDR& h, DST_TYPE dst) {
  h.dst = dst;
}

inline vector<UINT> &
IP_FILE_HDR_dst_file_idx_map(IP_FILE_HDR& h) {
  return h.dst_file_idx_map;
}
