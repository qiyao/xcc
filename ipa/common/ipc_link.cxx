...
#define DYNAMIC_LINKER "-dynamic-linker /lib/ld-linux-ia64.so.1"

extern "C" char* 
concat_names(char* a , char* b)
{
    char * buf;
...
#ifdef _WIN32
#define XT_LD_STRING "/bin/xt-ld.exe"
#else
#define XT_LD_STRING "/bin/xt-ld"
#endif
...
#elif defined(TARG_XTENSA)
    char * xttools_path = getenv("XTTOOLS");
    FmtAssert(xttools_path, (("XTTOOLS not set")));

    char * linker_path = (char*)malloc(strlen(xttools_path) + 
				       strlen(XT_LD_STRING) + 1);
    strcpy (linker_path, xttools_path);
    strcat (linker_path, XT_LD_STRING);
    ld_flags_part1->push_back (linker_path);
#else
...
void
ipa_init_autotie_line (void)
{
  autotie_flags = CXX_NEW (ARGV, Malloc_Mem_Pool);
}


void
ipa_add_autotie_flag (const char* str)
{
    autotie_flags->push_back (str);
}


ARGV *
ipa_autotie_line_argv (void)
{
  return autotie_flags;
}
