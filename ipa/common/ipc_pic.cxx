...
Link_Once(ST *st)
{
  if (ST_has_named_section(st)) {
    STR_IDX pu_sec_name_idx = Find_Section_Name_For_ST(st);
    char *name = Index_To_Str(pu_sec_name_idx);
    if (name && !strncmp(name, LINKONCE_PREFIX, strlen(LINKONCE_PREFIX))) {
      return true;
    }
  }
  return false;
...
struct Opt_global_var
{
	...
	if (TY_is_volatile (ty) ||
	    (aux_st.flags & ADDR_TAKEN_IN_OBJ) ||
...
#ifdef TARG_XTENSA
    DevWarn("TODO: Implement Count_elf_external_gots for PIC optimization");
    count += 20;
#else
...
{
#ifdef TARG_XTENSA
    DevWarn("TODO: Compute max_gpa_size for PIC optimization");
    return 10000;
#else
...
#ifdef TARG_XTENSA
	Gp_Area_Size () - IP_estimate_got_size () - 0;
	DevWarn("TODO: Compute used_gp_area for PIC optimization");
#else
...
#ifdef TARG_XTENSA
    DevWarn("TODO: Compute used_gp_area for PIC optimization");
    if ( (IP_estimate_got_size() + 0) > Gp_Area_Size () ) {
#else
