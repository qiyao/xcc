...
/* These are used to pass the information from WN_open_input (called 
 * by ipa_open_input) to Setup_File_Header (called by process_whirl).
 * This works because the call to ipa_open_input in ipa_process_whirl 
 * (gnu/ld/ipa.c) is immediately followed by the call to process_whirl.
 */
static void *mmap_addr = NULL;
static off_t mmap_size = 0;
static int   file_desc = -1;
#ifdef _WIN32
static void *mmap_handle = NULL;
#endif
	       ...
	       const Sym* elf_symtab) 
{
	...
	Setup_File_Header (file_name, mmap_addr, mmap_size, file_desc);
    mmap_addr = NULL;
    mmap_size = 0;
    file_desc = -1;
...
{
#ifdef _WIN32
    mmap_addr = WN_open_input(name, &mmap_size, &file_desc, &mmap_handle);
#else
    mmap_addr = WN_open_input(name, &mmap_size, &file_desc);
#endif
    return mmap_addr;
}

#endif // TARG_IA64 || TARG_XTENSA
