...
//There is another copy of this in symtab.cxx
...
static STR_IDX
Find_Orig_Section_Name_For_ST (const IPC_GLOBAL_TABS& original_tabs,
			       const ST *st)
{
    ST_IDX idx = ST_st_idx (st);
    for (INT i=0; i<original_tabs.st_attr_tab_size; i++) {
      ST_ATTR &attr = original_tabs.st_attr_tab[i];
      if (ST_ATTR_st_idx(attr) == idx && attr.kind == ST_ATTR_SECTION_NAME) {
        return ST_ATTR_section_name(attr);
      }
    }
    FmtAssert(0, ("didn't find section name for ST %s", ST_name(st)));
}
...
{
    ST_IDX idx = ST_st_idx (st);
    for (INT i=0; i<ST_ATTR_Table_Size(ST_IDX_level(idx)); i++) {
      ST_ATTR &attr = St_Attr_Table[i];
      if (ST_ATTR_st_idx(attr) == idx && attr.kind == ST_ATTR_SECTION_NAME) {
        return &Str_Table[ST_ATTR_section_name(attr)];
      }
    }
    return NULL;
}
...
static STR_IDX
Find_Orig_Ro_Section_Name_For_ST (const IPC_GLOBAL_TABS& original_tabs, const ST *st)
{
    ST_IDX idx = ST_st_idx (st);
    for (INT i=0; i<original_tabs.st_attr_tab_size; i++) {
      ST_ATTR &attr = original_tabs.st_attr_tab[i];
      if (ST_ATTR_st_idx(attr) == idx && attr.kind == ST_ATTR_RO_DATA_SECTION_NAME) {
	return ST_ATTR_ro_data_section_name(attr);
      }
    }
    FmtAssert(0, ("didn't find rodata section name for ST %s", ST_name(st)));
}
...
Find_Ro_Section_Name_For_ST (const ST *st)
{
    ...
    for (INT i=0; i<ST_ATTR_Table_Size(ST_IDX_level(idx)); i++) {
      ST_ATTR &attr = St_Attr_Table[i];
      if (ST_ATTR_st_idx(attr) == idx && attr.kind == ST_ATTR_RO_DATA_SECTION_NAME) {
        return &Str_Table[ST_ATTR_ro_data_section_name(attr)];
      }
    }
    return NULL;
}
...
Synch_ST_flags (ST& merged_st, const ST& original_st)
{
	...
	ST_IS_NOT_USED | ST_PT_TO_UNIQUE_MEM;
		 ...
		 const ST&              original_st)
{
	...
	// Merge section attributes
        char *orig_section_name=NULL;
        char *merged_section_name=NULL;
        if ( ST_has_named_section(&original_st)) {
          orig_section_name = &original_tabs.symstr_tab[
	      	  Find_Orig_Section_Name_For_ST(original_tabs,&original_st)];
        }
        if ( ST_has_named_section(&merged_st)) {
          merged_section_name = Find_Section_Name_For_ST(&merged_st);
        }
	if (orig_section_name) {
          if (merged_section_name) {
            if (strcmp(orig_section_name, merged_section_name)) {
              fprintf (stderr, "Error: Function %s defined multiple times "
		       "in different sections %s %s\n",
		       ST_name(merged_st), orig_section_name,
		       merged_section_name);
              exit(RC_USER_ERROR);
	    }
	  } else {
   	    Set_ST_has_named_section(&merged_st);
	  }
	}
	// Merge rodata section attributes
        char *orig_ro_section_name=NULL;
        char *merged_ro_section_name=NULL;
        if ( ST_has_named_ro_section(&original_st)) {
          orig_ro_section_name = &original_tabs.symstr_tab[
	      	  Find_Orig_Ro_Section_Name_For_ST(original_tabs,&original_st)];
        }
        if ( ST_has_named_ro_section(&merged_st)) {
          merged_ro_section_name = Find_Ro_Section_Name_For_ST(&merged_st);
        }
	if (orig_ro_section_name) {
          if (merged_ro_section_name) {
            if (strcmp(orig_ro_section_name, merged_ro_section_name)) {
              fprintf (stderr, "Error: Function %s defined multiple times "
		       "with different rodata sections %s %s\n",
		       ST_name(merged_st), orig_ro_section_name,
		       merged_ro_section_name);
              exit(RC_USER_ERROR);
	    }
	  } else {
   	    Set_ST_has_named_ro_section(&merged_st);
	  }
	}
    }
} // Synch_St_With_St
...
static void
Verify_Predefined_Symbols (const IP_FILE_HDR& hdr, const ST& st)
{
    ST mapped_st = st;
    Set_ST_name_idx (mapped_st, (*New_Symstr_Idx)[ST_name_idx (st)]);
    
    if (memcmp (&mapped_st, &St_Table[ST_st_idx (st)], sizeof(ST))) {
      fprintf(stderr, 
              "IPA Error: Incompatible TIE types in file %s\n"
              "  You cannot mix .o files built for different configurations\n"
              "  You must use the same --xtensa-params for all compile and link commands\n",
              hdr.file_name);
      exit(RC_USER_ERROR);
    }
			...
			const ST& original_st)
{
    ...
    block->insert (element, std::make_pair (desc, new_st_idx));
    return new_st_idx;
} // Process_Common_Element



// remember all linkonce static variables
typedef STACK<char *> STACK_OF_NAMES;
typedef STACK<ST_IDX *> STACK_OF_ST_IDX;
class LINK_ONCE // One variable name, maybe the same name from multiple sections
{
  STACK_OF_NAMES *_section_names;
  STACK_OF_ST_IDX *_st_idx_stack;
  MEM_POOL *_pool;
public:
  LINK_ONCE(MEM_POOL *pool) {
    _section_names = CXX_NEW(STACK_OF_NAMES(pool),pool);
    _st_idx_stack = CXX_NEW(STACK_OF_ST_IDX(pool),pool);
    _pool = pool;
  }
  ST_IDX *Find(char *sec_name) {
    for (INT i=0; i<_section_names->Elements(); i++) {
      if (!strcmp(sec_name, _section_names->Bottom_nth(i))) {
        return _st_idx_stack->Bottom_nth(i);
      }
    }
    return NULL;
  }
  void Enter(char *sec_name, ST_IDX * st_idx) {
    _section_names->Push(sec_name);
    _st_idx_stack->Push(st_idx);
  }
};


// Merge locals inside linkonce sections.  These are sort of like globals
// in that we want a single unique copy after merging.  They are sort of
// different in that there might be multiple different variables of the same
// name so long as the different variables are in different sections.
// Therefore, we keep track ourselves using a hash table per name which maps
// to a list of sections.  This assumes that each name will be in very few
// linkonce sections.
static ST_IDX
merge_local_linkonce_st (char *st_name,
			 char *section_name, 
			 const IPC_GLOBAL_TABS& original_tabs,
			 ST &original_st)
{
  typedef USER_HASH_TABLE<char*, LINK_ONCE *, String_Hash, String_Equal>
    NAME_TO_LINK_ONCE;
  static NAME_TO_LINK_ONCE *name_to_link_once=NULL;

  if (!name_to_link_once) {
    name_to_link_once = CXX_NEW(NAME_TO_LINK_ONCE(500,&merge_pool),&merge_pool);
  }

  LINK_ONCE *lo = name_to_link_once->Find(st_name);
  if (lo) { // we have seen the name before
    ST_IDX *st_idx = lo->Find(section_name);  
    if (st_idx) { // we have seen the name and it's from the same section
      Merge_St_With_St (original_tabs, St_Table[*st_idx], original_st,NULL);
      Invalidate_inito (original_st, original_tabs.inito_tab);
      return *st_idx;
    } else { // we have see the name but it's from a different section
      if (ST_sym_class(original_st) == CLASS_FUNC) {
        fprintf (stderr, "Error: Function %s defined multiple times in "
		 "different linkonce sections\n", ST_name(original_st));
        exit(RC_USER_ERROR);
      }
      ST_IDX st_idx = Enter_Original_St(original_tabs, original_st); 
      ST_IDX *st_idx_copy = CXX_NEW(ST_IDX,&merge_pool);
      *st_idx_copy = st_idx;
      char *section_name_copy =
	CXX_NEW_ARRAY(char, strlen(section_name)+1, &merge_pool);
      strcpy(section_name_copy, section_name);
      lo->Enter(section_name_copy,st_idx_copy);
      return st_idx;
    }
  } else { // we have not seen the name before
    LINK_ONCE *lo = CXX_NEW(LINK_ONCE(&merge_pool),&merge_pool);
    ST_IDX st_idx = Enter_Original_St(original_tabs, original_st); 
    ST_IDX *st_idx_copy = CXX_NEW(ST_IDX,&merge_pool);
    *st_idx_copy = st_idx;
    char *section_name_copy =
    ...
    strcpy(section_name_copy, section_name);
    lo->Enter(section_name_copy,st_idx_copy);
    char *st_name_copy = CXX_NEW_ARRAY(char, strlen(st_name)+1, &merge_pool);
    strcpy(st_name_copy, st_name);
    name_to_link_once->Enter(st_name_copy,lo);
    return st_idx;
  }
}
		...
		const IPC_GLOBAL_TABS& original_tabs)
{
    ...
    // We only merge external symbols (never file-static symbols except
    // linkonce ones).
    BOOL is_link_once = FALSE;
    char *section_name;
    if (ST_has_named_section(&original_st)) {
      section_name = &original_tabs.symstr_tab[
	      	Find_Orig_Section_Name_For_ST(original_tabs,&original_st)];
      if (section_name && !strncmp(section_name, LINKONCE_PREFIX,
				   strlen(LINKONCE_PREFIX))) {
        is_link_once = TRUE;
      }
    }
	...
	ST_export(original_st) == EXPORT_LOCAL_INTERNAL) {
        if (is_link_once) { 
          return merge_local_linkonce_st(st_name, section_name,
					 original_tabs, original_st);
	} else if (ST_raw_base_idx (original_st) == ST_st_idx (original_st))
...
typedef STACK<UINT> STACK_OF_INITV_IDX;
static INITV_IDX
Merge_Global_Initv_Rec(UINT , const IPC_GLOBAL_TABS &, INITV_IDX_MAP  &, STACK_OF_INITV_IDX *);
		   ...
		   INITV_IDX_MAP	 &initv_map)
{
    ...
    if (INITV_next(*original_initv) != 0) {
        STACK_OF_INITV_IDX *stack_of_initv_idx = CXX_NEW(STACK_OF_INITV_IDX(&merge_pool),&merge_pool);
	nested_initv_idx = 
	    Merge_Global_Initv_Rec(INITV_next(*original_initv), original_tabs, initv_map,stack_of_initv_idx); 
...
// fake recursive version of above, use data rather than stack to avoid stack overflow
static INITV_IDX
Merge_Global_Initv_Rec(UINT                  initv_idx,
		   const IPC_GLOBAL_TABS &original_tabs,
		   INITV_IDX_MAP	 &initv_map,
		   STACK_OF_INITV_IDX *stack_of_initv_idx)
{
   INITV_IDX return_val;
   INITV     *original_initv;
   INITV     *new_initv;
...
top:
    ...
    if (initv_map[initv_idx] != 0) {
	if (!stack_of_initv_idx->Elements()) {
	  return initv_map[initv_idx];
	} else {
	  return_val = initv_map[initv_idx];
 	  goto recursion_return_point;
        }
    }
    ...
    // Fake-Recursively process initv's linked together.
    //
    if (INITV_next(*original_initv) != 0) {
	stack_of_initv_idx->Push(initv_idx);
	initv_idx = INITV_next(*original_initv);
	goto top;
recursion_return_point:
	initv_idx = stack_of_initv_idx->Pop();
        original_initv = &original_tabs.initv_tab[initv_idx];
        new_idx = initv_map[initv_idx];
        new_initv = &Initv_Table[new_idx];
	nested_initv_idx = return_val;
	Set_INITV_next(*new_initv, nested_initv_idx);
    }
...
Merge_Global_St_Attr(const ST_ATTR* st_attr_tab, UINT32 size)
{
	...
	switch (old_st_attr.kind) {
	    ...
	    case ST_ATTR_RO_DATA_SECTION_NAME:
		ST_ATTR_Init (new_st_attr, st_idx, ST_ATTR_RO_DATA_SECTION_NAME,
			      (*New_Symstr_Idx) [ST_ATTR_ro_data_section_name (old_st_attr)]);
		break;

	    case ST_ATTR_FILL:
		ST_ATTR_Init (new_st_attr, st_idx, ST_ATTR_FILL, ST_ATTR_fill(old_st_attr));
		break;
		      ...
		      IP_FILE_HDR& hdr, MEM_POOL *mempool)
{
	...
	Verify_Predefined_Symbols (hdr, original_tabs.st_tab[idx]);
...
void
Sync_symbol_attributes (ST_IDX st_idx, UINT32 sym_attr, UINT32 export_type)
{
