...
typedef __gnu_cxx::hashtable<FLD_IDX, FLD_IDX, FLD_HASH,
			     __gnu_cxx::identity<FLD_IDX>,
			     FLD_IS_EQUIVALENT,
			     mempool_allocator<FLD_IDX> > FLD_HASH_TABLE;
...
typedef __gnu_cxx::hashtable<ARB_IDX, ARB_IDX, ARB_HASH,
			     __gnu_cxx::identity<ARB_IDX>,
			     ARB_IS_EQUIVALENT,
			     mempool_allocator<ARB_IDX> > ARB_HASH_TABLE;
...
typedef __gnu_cxx::hashtable<TYLIST_IDX, TYLIST_IDX, TYLIST_HASH,
			     __gnu_cxx::identity<TYLIST_IDX>,
			     TYLIST_IS_EQUIVALENT,
			     mempool_allocator<TYLIST_IDX> > TYLIST_HASH_TABLE;
...
#define INT64_ALIGNMENT 8
    ...
    Is_True (sizeof(TY) == 24 && __ALIGNOF(TY) == INT64_ALIGNMENT,
	     ("TY: %u should be 24, %u should be %u", 
              sizeof(TY), __ALIGNOF(TY), INT64_ALIGNMENT));
    Is_True (sizeof(FLD) == 24 && __ALIGNOF(FLD) == INT64_ALIGNMENT,
	     ("FLD: %u should be 24, %u should be %u", 
              sizeof(FLD), __ALIGNOF(FLD), INT64_ALIGNMENT));
    Is_True (sizeof(ARB) == 32 && __ALIGNOF(ARB) == INT64_ALIGNMENT,
	     ("ARB: %u should be 32, %u should be %u", 
              sizeof(ARB), __ALIGNOF(ARB), INT64_ALIGNMENT));

    ty_hash_table = CXX_NEW (TY_HASH_TABLE (1000, TY_HASH (),
					    __gnu_cxx::equal_to<TY>(), 
					    TY_EXTRACT_KEY (), pool),
			     pool);
