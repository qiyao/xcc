 ...
 * $Date: 2000/09/05 15:38:25 $
 * $Author: murthy $
 * $Source: /isms/cmplrs.src/osprey1.0/ipa/inline/RCS/inline_summarize.h,v $
 *
 * Description:
...
SUMMARIZE<INLINER>::Process_procedure (WN *w)
{
	...
	if ((PU_src_lang(pu) == PU_C_LANG) ||
		...
		if (ST_is_weak_symbol(st)) {
                    // Allow inlining of weak functions since gfecc
                    // marks even the simple template functions as weak
                    Lmt_DevWarn(1,("Request to inline a function marked weak"));
                }
#else
	...
	// that would need to be promoted if inlined or cloned
	    ...
	    if (ST_class(st2) == CLASS_VAR && is_variable_dim_array(ST_type(st2))) 
