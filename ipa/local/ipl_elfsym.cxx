...
extern "C" void 
WN_write_elf_symtab (const void* symtab, UINT64 size,
...
ST_to_Elfsym (const ST* st, typename ELF::Elf_Sym& elfsym, const ELF& tag)
{
	...
	default:
	    ...
	    /* For object symbols not connected to a section, indicate
	       alignment in the value field.  */
	    elfsym.st_value = TY_align (ST_type (st));
	    break;
	} 
	break;
...
namespace
{
	...
	Elf_Symtab (UINT n) : size (1), max_size (n+1) {
	    table = (Sym*) MEM_POOL_Alloc (Malloc_Mem_Pool,
					   (n+1) * sizeof(Sym));
	    memset ((char *) table, 0, sizeof(Sym)); // first entry always zero
	}
