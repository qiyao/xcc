...
extern DLL_SHARED BOOL Keep_Static_Funcs;
extern DLL_SHARED BOOL Keep_Inline_Funcs;
  ...
  while (iter.Wn () != NULL) {
    ...
    if (!is_iload && opr == OPR_LDA) {
      ...
      if (ST_class (st) == CLASS_VAR ||
          ...
          (ST_sclass (st) == SCLASS_MERGE_STRING && action == RECORD_SAVED)) {
...
SUMMARIZE<program>::Summarize (WN *w)
{
  ...
  if (!Has_alt_entry ()) {
	...
	CXX_NEW (CHI_CR_TO_INT_MAP (113, ptr_hash<CODEREP> (),
				    __gnu_cxx::equal_to<CODEREP*> (),
				    &Temp_pool),
		 &Temp_pool);
    Phi_To_Idx_Map =
	CXX_NEW (PHI_NODE_TO_INT_MAP (113, ptr_hash<PHI_NODE> (),
				      __gnu_cxx::equal_to<PHI_NODE*> (),
				      &Temp_pool),
		 &Temp_pool);
...
struct set_global_addr_taken_attrib
{
	...
	if (ST_class(st) == CLASS_CONST && ST_sclass(st) != SCLASS_MERGE_STRING)
	    return;
...
SUMMARIZE<program>::Update_Addr_Passed_Count (WN* opr_parm)
{
  ...
  for ( wni = WN_WALK_TreeIter(opr_parm); wni && WN_ITER_wn(wni) != 0;
    ...
    if (ST_class(s) == CLASS_CONST && ST_sclass(s) != SCLASS_MERGE_STRING)
      continue;
...
// TODO: Try to avoid this somewhat arbitrary restriction.
// Currently, the problem is that INITV whose value is the
// offset of a local variable isn't correctly updated when
// the function is inlined.

// If INITV has a SYMOFF for a symbol in CURRENT_SYMTAB
// mark the procedure as no_inline
static void
check_local_symoff_initv(SUMMARY_PROCEDURE *proc, INITV_IDX initv_idx)
{
  while (initv_idx) {
    const INITV& initv = Initv_Table[initv_idx];
    if (INITV_kind (initv) == INITVKIND_SYMOFF &&
        ST_IDX_level(INITV_st(initv)) == CURRENT_SYMTAB)
      proc->Set_no_inline();
    else if (INITV_kind (initv) == INITVKIND_BLOCK)
      check_local_symoff_initv(proc, INITV_blk(initv));
    initv_idx = INITV_next(initv);
  }
}

struct check_local_symoff_inito 
{
  SUMMARY_PROCEDURE *_proc;

  check_local_symoff_inito(SUMMARY_PROCEDURE *proc) : _proc(proc) {}

  void operator() (UINT32, const INITO* inito) const {
    check_local_symoff_initv(_proc, INITO_val(*inito));
  }
};
...
SUMMARIZE<program>::Process_procedure (WN* w)
{
    ...
    // search for INITOs whose values are offsets of local variables
    if (Scope_tab[CURRENT_SYMTAB].inito_tab->Size () > 1)
      For_all(Inito_Table, CURRENT_SYMTAB, check_local_symoff_inito(proc));
	...
	proc->Set_may_inline();
        if (Keep_Inline_Funcs) {
            proc->Set_no_delete();
	}
    }
    else if (Keep_Static_Funcs && ST_is_export_local(st)) 
    {
        proc->Set_no_delete();
    }
	...
	w2 = iter.Wn ();
	ST *st = OPERATOR_has_sym(WN_operator(w2)) ? WN_st(w2) : NULL;
	...
	if (st && (Do_Common_Const || Do_parallel_stuff)) {
   	...
   	if (st && (is_variable_dim_array(ST_type(st)))) 
        ...
        /* Mark local restrict accesses. */
        if (st && TY_is_restrict(ST_type(st)) &&
            (ST_sclass(st) == SCLASS_AUTO ||
             ST_sclass(st) == SCLASS_PSTATIC))
          proc->Set_has_restrict();
	...
	case OPR_DO_WHILE:
	    loopnest++;
	    if (loopnest > max_loop_depth) max_loop_depth = loopnest;
	    break;
	...
	case OPR_ASM_STMT:
    	    if (WN_Asm_Volatile(w2) || WN_Asm_Clobbers_Mem(w2)) proc->Set_has_side_effect();
            break; 
	...
	case OPR_REGION:
	    ...
	    else if (WN_pragma(w2) == WN_PRAGMA_PARALLEL_BEGIN) {
		proc->Set_has_parallel_region_pragma();
	    }

	    else if ((WN_pragma(w2) == WN_PRAGMA_DOACROSS) ||
		(WN_pragma(w2) == WN_PRAGMA_PARALLEL_DO) ||
		(WN_pragma(w2) == WN_PRAGMA_PARALLEL_BEGIN) ||
		(WN_pragma(w2) == WN_PRAGMA_PARALLEL_SECTIONS)) 
		proc->Set_has_noinline_parallel_pragma();

	    else if ((WN_pragma(w2) == WN_PRAGMA_PDO_BEGIN) ||
		(WN_pragma(w2) == WN_PRAGMA_PSECTION_BEGIN) ||
		(WN_pragma(w2) == WN_PRAGMA_SINGLE_PROCESS_BEGIN)) 
		Has_pdo_pragma = TRUE;

            else if (WN_pragma(w2) == WN_PRAGMA_ASM_CONSTRAINT) {
                PREG_NUM neg_preg = WN_pragma_asm_copyout_preg(w2);
                if (proc->Asm_Neg_Preg_First() == 0 ||
                    proc->Asm_Neg_Preg_First() < neg_preg)
                  proc->Asm_Neg_Preg_First() = neg_preg;
                if (proc->Asm_Neg_Preg_Last() == 0 ||
                    proc->Asm_Neg_Preg_Last() > neg_preg)
                  proc->Asm_Neg_Preg_Last() = neg_preg;
            }
    ...
    proc->Set_max_loop_depth(max_loop_depth);
	...
	if (Cur_PU_Feedback) {
		...
		ErrMsg(EC_FB_Unknown_Freq, ST_name(WN_st(w)));
		Cur_PU_Feedback = NULL;
	    }
	    else {
              fb->Set_cycle_count (cycle_count);
              fb->Set_ccount(Cur_PU_Feedback->Query_cycle_count());
...
SUMMARIZE<program>::Process_formal (WN *w, INT num_formals, SUMMARY_PROCEDURE *proc)
{
	...
	formal->Set_ty(Promoted_Parm_Type(formal_st));
        if (TY_kind(formal->Get_ty()) == KIND_STRUCT) {
	  proc->Set_has_struct_args();
        }
	if (is_variable_dim_array(formal->Get_ty())) {
	    formal->Set_is_var_dim_array();
	    proc->Set_has_var_dim_array();
	}
        if (TY_is_restrict(formal->Get_ty()))
          proc->Set_has_restrict();
    }
...
SUMMARIZE<program>::Get_symbol_index (const ST *st)
{
  ...
  if (ST_class(st) == CLASS_FUNC || ST_class(st) == CLASS_BLOCK || ST_class(st) == CLASS_NAME) {
