...
private:
    ...
    INT32 _asm_neg_preg_first;
    INT32 _asm_neg_preg_last;
...
#define PROC_HAS_PDO_PRAGMA 		0x20000000 // TY_is_non_pod set for
						   // LOCAL pragma's st 
#define PROC_HAS_RESTRICT               0x40000000 // PU has restrict locals or formals
#define PROC_HAS_STRUCT_ARGS               0x80000000 // PU has structure arguments
...
public:
    ...
    UINT32 Get_max_loop_depth() const	{ return _max_loop_depth; }
    void   Set_max_loop_depth(UINT32 d)	{ _max_loop_depth = d; }

    INT32 & Asm_Neg_Preg_First()	{ return _asm_neg_preg_first; }
    INT32 & Asm_Neg_Preg_Last()		{ return _asm_neg_preg_last; }
    ...
    void Set_has_restrict() 		{ _state |= PROC_HAS_RESTRICT;}
    BOOL Has_restrict() const 	        { return _state & PROC_HAS_RESTRICT;}
    ...
    void Set_has_struct_args ()	{ _state |= PROC_HAS_STRUCT_ARGS; }
    BOOL Has_struct_args() const	{ return (_state & PROC_HAS_STRUCT_ARGS); }
...
private:
    ...
    FB_FREQ _cycle_count;		// cycle count based on estimates
    FB_FREQ _ccount;			// cycle count based on rsr.ccount
...
public:
    ...
    void Set_ccount (FB_FREQ freq)		{ _ccount = freq; }
    FB_FREQ Get_ccount () const			{ return _ccount; }
