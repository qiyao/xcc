...
ORDERED_NODE_ITER::BuildVector (TRAVERSAL_ORDER order)
{
    ...
    if (order == LEVELORDER || order == INLINEORDER) {
    ...
    } else if (order == INLINEORDER) {
	Walk (GRAPH_root(g), visit);
        for (NODE_INDEX vertex=0; vertex<GRAPH_vmax(g); vertex++) { //without don't inline nodes, graph may not be connected
...
void ORDERED_NODE_ITER::MarkRecursive(mUINT8 visit[], NODE_INDEX r)
{
 ...
 for (NODE_INDEX vi = vitr.First_Succ(); vi != -1; vi = vitr.Next_Succ()) {
  if ((order != INLINEORDER) ||
      !((IPA_NODE *)NODE_user(&GRAPH_v_i(g, vi)))->Has_Noinline_Attrib()) { 
    if (visit[vi] != 2)
     {
...
void ORDERED_NODE_ITER::Walk (NODE_INDEX r, mUINT8 visit[])
{
    ...
    if (order == INLINEORDER) {
      for (NODE_INDEX vi = vitr.First_Succ(); vi != -1; vi = vitr.Next_Succ()) {
        if (!((IPA_NODE *)NODE_user(&GRAPH_v_i(g, vi)))->Has_Noinline_Attrib() && (visit[vi] != 1) && (visit[vi] != 2)) {
	    Walk (vi, visit);
        }
      }
    } else {
...
{
  INT i;
  INT cur_level_pos[IPA_CG_Max_Depth+1];
