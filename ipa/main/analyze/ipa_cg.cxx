...
static BOOL Trace_Call_Graph = FALSE;
...
static void
Random_inline_overrides()
{
  SRANDOM(INLINE_Random_Seed);

  UINT total_edges = IPA_Call_Graph->Edge_Size();
  char *marked = (char *) alloca(total_edges);
  memset(marked, 0, total_edges); 
  UINT must_inline = (UINT) (((RANDOM() % 101) / 100.0) * total_edges);

  for (UINT count = 0; count < must_inline;) {
    UINT32 idx = RANDOM() % total_edges;
    if (!marked[idx]) {
      count++;
      marked[idx] = TRUE;
      IPA_EDGE *ipa_edge = IPA_Call_Graph->Edge(idx);
      if (!ipa_edge->Has_Noinline_Attrib()) {
        ipa_edge->Set_Must_Inline_Attrib();
        if (Trace_IPA || Trace_Perf) {
          IPA_NODE* caller = IPA_Call_Graph->Caller(ipa_edge->Edge_Index());
          IPA_NODE* callee = IPA_Call_Graph->Callee(ipa_edge->Edge_Index());
...
Add_One_Node (IP_FILE_HDR& s, INT32 file_idx, INT i, NODE_INDEX& orig_entry_index)
{
    ...
    if (Trace_Call_Graph)
      fprintf(TFile, "[%d] Trying to add %s\n", i, ST_name(st));
	...
	if (Trace_Call_Graph || Trace_IPA || Trace_Perf)
	  fprintf(TFile, "  Deleted %s from %s (multiply defined procedure)\n",
    ...
    if (Trace_Call_Graph)
      fprintf(TFile, "  Added %s\n", ST_name(st));

    ipa_node = IPA_Call_Graph->Add_New_Node (st, file_idx, i, i);
	...
	Total_cycle_count += ipa_node->Get_cycle_count ();
        Total_ccount += ipa_node->Get_ccount ();

        if (Trace_IPA) {
          fprintf(TFile, "Estimated cycle count(%s): ", ST_name(st));
          ipa_node->Get_cycle_count().Print(TFile);
          fprintf(TFile, " (current total: ");
          Total_cycle_count.Print(TFile);
          fprintf(TFile, ")\n");
          fprintf(TFile, "Cycle count from rsr.ccount(%s): ", ST_name(st));
          ipa_node->Get_ccount().Print(TFile);
          fprintf(TFile, " (current total: ");
...
Add_Edges_For_Node (IP_FILE_HDR& s, INT i, SUMMARY_PROCEDURE* proc_array, SUMMARY_SYMBOL* symbol_array)
{
    ...
    if (Trace_Call_Graph)
      fprintf(TFile, "Adding edges from %s: %d callsites\n", 
              ST_name(caller_st), callsite_count);
    ...
    for (INT j = 0; j < callsite_count; ++j, ++callsite_index) {
      ...
      if ( callsite_array[callsite_index].Is_func_ptr() ) {
          ...
          if (Trace_Call_Graph)
            fprintf(TFile, "[%d] indirect call (function pointer)\n", j);
      } 
      // for direct calls
	  ...
	  Mark_inline_edge_overrides(ipa_edge);
          ...
          if (Trace_Call_Graph)
            ...
            append_icall_list (caller->Ocall_List(), 
                             &callsite_array[callsite_index]);
            if (Trace_Call_Graph)
              fprintf(TFile, "[%d] opaque call (non-WHIRL function)\n", j);
        }
      }
      else {
          if (Trace_Call_Graph)
            fprintf(TFile, "[%d] intrinsic call\n", j);
      }
    }
    return;
}
...
#ifdef _STANDALONE_INLINER
    // for standalone inline do this twice, the first time ignore local functions.
    // If there is a recursive cycle, it's better to put an edge to a non-local function rather than
    // a local one since the local one might be deletable
    for (v = 0;
	 visited_count != num_nodes && v < GRAPH_vmax(IPA_Call_Graph->Graph());
	 ++v) {
	if (NODE_fcnt(&GRAPH_v_i(IPA_Call_Graph->Graph(), v)) != -1 && !visited[v]) {
	    // static functions can never be called externally
	    // adding an external edge to them overconservatively prevents deleting them
            if(IPA_Call_Graph->Graph()->Node_User(v)->Is_Externally_Callable()) {
	      visited_count += connect_to_root (v, visited);
            }
         }
     }
...
Build_Call_Graph ()
{
  Trace_Call_Graph = Get_Trace(TP_IPA, IPA_TRACE_CALL_GRAPH);
  ...
  if (INLINE_Random_Seed_Set)
    Random_inline_overrides();

  IPA_Call_Graph_Built = TRUE;

  if (Trace_Call_Graph) IPA_Call_Graph->Print(TFile);
...
{ 
  ...
  if (Summary_Proc()->Is_alt_entry()) {
    ...
    if (!Has_frequency() && !Total_ccount.Zero()) { // uncalled function
        Set_PU_optimize_for_space(Get_PU());
        if (Trace_IPA)
          fprintf(TFile, "  should be optimized for space\n");
    } else if (Get_cycle_count().Known()) {
      FB_FREQ cycle_contrib = ((OPT_Estimate_Ccount || Total_ccount.Zero()) ?
                               Get_cycle_count() / Total_cycle_count :
                               Get_ccount() / Total_ccount);
      if (Trace_IPA)
        fprintf(TFile, "%s contribution: %g / 1000\n",
                Name(), cycle_contrib.Value() * 1000);
...
//  functions marked as INLINE that are not address taken are considered
//    to be not externally callable since legit for compiler to delete
//    and all copies must be identical
//----------------------------------------------------------------
BOOL
IPA_NODE::Is_Externally_Callable ()
{
    ...
    if (PU_is_mainpu(Get_PU()) ||
        AUX_ST_flags (aux_st, USED_IN_DSO|ADDR_TAKEN_IN_OBJ))
	return TRUE;
#endif // _LOCAL_INLINER
