...
private:
  ...
  UINT32 _max_loop_depth;		// deepest loop-nest depth in this PU
  UINT32 _orig_weight;			// original PU weight (before inlining)
...
public:
    ...
    _flags (0)
  {
    ...
    _max_loop_depth = summary_proc->Get_max_loop_depth();
    _orig_weight = _pu_size.Weight();
  ...
  UINT32 & Max_Loop_Depth()			{ return _max_loop_depth; }
  ...
  UINT32 Orig_Weight (void) const	{ return _orig_weight; }
  ...
  FB_FREQ Get_ccount () {
    SUMMARY_FEEDBACK* fb = Get_feedback();
    return (fb? fb->Get_ccount (): FB_FREQ_UNKNOWN);
  };
