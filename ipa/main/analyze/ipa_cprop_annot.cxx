...
typedef __gnu_cxx::hash_map<const void *, UINT32, eval_hasher, __gnu_cxx::equal_to<const void *>,
  ...
  for (UINT32 i = 0; i < Size; ++i) {
    if (!Top(i) && !Bottom(i)) {
      ...
      for (UINT32 j = 0; j < gvals.Elements(); ++j) {
        if (gvals[j].Value()) {
          fprintf(fp, "    offset = %" LLD_FMT ", element_size = %d, value = ",
