...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: /isms/cmplrs.src/osprey1.0/ipa/main/analyze/RCS/ipa_option.cxx,v $ $Revision: 1.15 $";
#endif /* _KEEP_RCS_ID */
...
#include "ipc_link.h"           /* for ipa_init_autotie_line() */
...
Process_IPA_Options ( INT argc, char **argv )
{
	...
	if (argv[i] != NULL && *(argv[i]) == '-' ) {
	    ...
	    if (!strcmp(argv[i],"-prof_opt") && i+1<argc) {
	      Profile_File_Name = strdup(argv[++i]);
	      continue;
	    }
	    ...
	    case 'X':
	        /* autotie... */
	        if ( strncmp ( argv[i], "-XP:", strlen( "-XP:" ) ) == 0 ) {
		    ipa_add_autotie_flag(argv[i]);
		    break;
	        }
    ...
    Trace_IPA = Get_Trace ( TP_IPA, IPA_TRACE_PROGRESS );
