	 ...
	 BOUNDS_ARRAY *bounds, BOOL extra_pad)
{
  ...
  if (pad_size == 0)
    {
      if (trace_split_common)
	fprintf(TFile, "pad size = %" LLD_FMT " for dim %d \n",(*bounds)[0].Get_Stride(), pad_dim);
...
//  INT size = abs(belement->Get_Upper() - belement->Get_Lower() + 1);
  ...
  if (padt > 110)
    {
      if (trace_split_common)
	fprintf(TFile, "padding threshold = %f, pad_size = %" LLD_FMT ", dim_size = %d \n", padt, pad_size, size);
      return FALSE;
    }
  if (trace_split_common)
    fprintf(TFile, "padding threshold = %f, pad_size = %" LLD_FMT ", dim_size = %d  \n", padt, pad_size, size);
...
Pad_Multi_Dims(COMMON_SNODE *common_snode)
{
  ...
  if ((TY_kind(ST_type(s)) == KIND_ARRAY)) {
	...
	if (b->Is_Constant()) {
	  ...
	  if (trace_split_common)
	    fprintf(TFile, "padding = %" LLD_FMT ", pad_size = %" LLD_FMT " \n",padding, pad_size);
	  if (padding) {
	    for (INT j =i; j >= 0; --j) {
  ...
  for (INT i=0 ; i<=last_idx;++i)
    {
      ...
      fprintf(fp, "offset = %" LLD_FMT ", size = %" LLD_FMT ", element_size = %d, split = %d\n",
