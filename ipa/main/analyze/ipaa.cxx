				       ...
				       const SUMMARY_ACTUAL &actual )	  
{
	...
	iref = callee_info.Is_formal_iref_elmt ( formal_num );
	dref = callee_info.Is_formal_dref_elmt ( formal_num );
	ref = iref | dref;
	...
	case ACTUAL_KIND_GLOBAL:
	    ...
	    if ( iref && ! callee_info.Is_eref_elmt ( merged_idx ) ) {
...
IPAA::Do_Simple_IPAA ( IPA_CALL_GRAPH &cg )
{
    ...
    Trace_Detail	= Get_Trace ( TP_IPA, IPA_TRACE_IPAA_DETAIL );
    Trace_Stats	= Get_Trace ( TP_IPA, IPA_TRACE_IPAA_STATS );
    Trace_Iterator = Get_Trace ( TP_IPA, IPA_TRACE_IPAA_ITERATOR );
