				       ...
				       const BOOL                   trace)
{
  ...
  vector<IP_ALIAS_CLASS_MEMBER *, IP_ALIAS_CLASS_MEMBER_ALLOC>::iterator arg;
  vector<IP_ALIAS_CLASS_MEMBER *, IP_ALIAS_CLASS_MEMBER_ALLOC>::iterator that_arg;
  ...
  // didn't match.
  vector<IP_ALIAS_CLASS_MEMBER *, IP_ALIAS_CLASS_MEMBER_ALLOC>::iterator start, stop;
  ...
  case IP_ACR_LAMBDA_TYPE:
	...
	fprintf(fp, " alloc(");
      ...
      fprintf(fp, " (&%lu<0x%lx>) <-- lambda(",
	      ...
	      Signature().Returns());
      for (vector<IP_ALIAS_CLASS_MEMBER *, IP_ALIAS_CLASS_MEMBER_ALLOC>::const_iterator arg =
...
IP_ALIAS_CLASSIFICATION::Finalize_ac_map_wn(WN *wn)
{
  ...
  if (OPCODE_is_call(opc)) {
	...
	fdump_tree(TFile, wn);
      ...
      // The following second "safety net" does not appear to be
      // needed in the IPA version of alias classification.

#define DONT_DO_THIS 0
#if DONT_DO_THIS
      // In spite of our best efforts, it is possible that some
      // dereference of a constant has gotten through our net, so we
      // have an additional safety net here. This net, too, has holes
      // in it, but will catch a few cases that aren't caught
      // already. It remains important to realize that, like the
      // meager support that existed before for doing points-to
      // analysis in the presence of dereferenced constants, this hack
      // is still incorrect: in some situations memops will be
      // classified too optimistically.
      //
      // For indirects that should point to something but don't,
      // assume they are dereferences of constants and remap their
      // class_id to the ID associated with _const_addr_class.
      //
      // We already know this is an indirect memop, so we don't need
      // to check.
      //
      // A potential challenge here is to determine whether the
      // current memop's class actually has been analyzed as pointing
      // to something. I'm not sure whether the following NULL check
      // as it stands will do the job.

      if (acr->Data_class_pointed_to()->Sort() == IP_ACR_BOTTOM_TYPE) {
	// This is the case where the user has dereferenced a constant
	// and we haven't caught it already.
	if (Tracing()) {
	  fprintf(TFile, "... !!! Invoked second *((int *) 4) safety net on\n");
	  fdump_wn_no_st(TFile, wn);
	  fprintf(TFile, "IPA Remapping class ID %d ", class_id);
	}
	...
	// This is the common case, where the user is not stupid and
