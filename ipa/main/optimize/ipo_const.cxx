...
Propagate_Constants (IPA_NODE* node, WN* w, VALUE_DYN_ARRAY* cprop_annot)
{
	...
	if (ST_sclass (formal) != SCLASS_FORMAL_REF) {
		...
		if (mod_ref_info && ! mod_ref_info->Is_formal_dmod_elmt (i)) {
		    ...
		    if (const_st) {
                      // nenad, 2001/12/07:
                      // Add the same check for addr_saved as bellow;
                      // even if formal is readonly, replacing it by LDA of
                      // global variable may cause addr_saved for that variable
                      if (annot_node.Is_global () && !ST_addr_saved (const_st))
                        continue;
