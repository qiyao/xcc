...
ST* Find_Return_Registers(TYPE_ID type, PREG_NUM *rreg1, PREG_NUM *rreg2)
{
  ...
  if (Preg_Is_Dedicated_Integer(*rreg1))
    return Int_Preg;
  else
    return Float_Preg;
}
...
Create_array(WN* base)
{
  ...
  for (i=0; i< ndims; ++i) {  
	...
	((TY_AR_lbnd_var(ty, i) == ST_IDX_ZERO) ||
	 (TY_AR_ubnd_var(ty, i) == ST_IDX_ZERO))
	  {
...
// Find the adjustment for negative PREGs used as ASM output operands,
// and adjust ASM negative PREGs in the caller to reflect inlining.
// This is necessary because they have to be unique withing a PU.
static inline INT32
Asm_Neg_Preg_Adjustment(IPA_NODE *caller, IPA_NODE *callee)
{
  INT32 adjustment = 
    (caller->Summary_Proc()->Asm_Neg_Preg_Last() != 0 &&
     callee->Summary_Proc()->Asm_Neg_Preg_Last() != 0) 
    ? (caller->Summary_Proc()->Asm_Neg_Preg_Last() -
       callee->Summary_Proc()->Asm_Neg_Preg_First() - 1)
    : 0;

  if (callee->Summary_Proc()->Asm_Neg_Preg_Last() != 0) {
#if 0
    printf("Inlining %s (%d..%d) into %s (%d..%d)\n\t"
           "ASM preg adjustment is %d\n", 
	   IPA_Node_Name(callee), 
	   callee->Summary_Proc()->Asm_Neg_Preg_First(),
	   callee->Summary_Proc()->Asm_Neg_Preg_Last(),
	   IPA_Node_Name(caller), 
	   caller->Summary_Proc()->Asm_Neg_Preg_First(),
	   caller->Summary_Proc()->Asm_Neg_Preg_Last(),
           adjustment);
#endif
    if (caller->Summary_Proc()->Asm_Neg_Preg_Last() != 0) {
      caller->Summary_Proc()->Asm_Neg_Preg_Last() +=
        (callee->Summary_Proc()->Asm_Neg_Preg_Last() -
         callee->Summary_Proc()->Asm_Neg_Preg_First() - 1);
    }
...
IPO_INLINE::SubstituteFormal (ST* formal, WN* actual, INT position)
{
    ...
    INT original_param_position = 
      Get_orig_param_position (position, Callee_node ()->Cprop_Annot ());
    ...
    if (st == NULL || ST_class(st) == CLASS_CONST || ST_addr_saved (st)) 
	return FALSE;
	...
	if (ST_class(s) != CLASS_FUNC &&
            (TY_kind(ST_type(s)) == KIND_SCALAR ||
...
// insert NODE before before one and after after two 
static void
Insert_Block_Around (WN* parent, WN* one, WN *two, WN* before, WN* after)
{
    static char msg[] = "Invalid argument to Insert_Block_Around";

    Is_True (one && two  && parent && WN_operator (parent) == OPR_BLOCK, (msg));
...
{
	Insert_Block_Around(parent, wn, wn, before, after);
}
...
// if call is inside a try block, also save before try and restore after catch
// Doing a pattern match to find the catch, if we can't find it then we don't do
// the save restore, which isn't a disaster, we just waste some memory in the caller
static void
Save_And_Restore_Stack (WN* call)
{
    // CALLER SIZE OPERATION
     ...
     // look for try block
     while (parent_wn && WN_opcode(parent_wn) != OPC_REGION) {
	     parent_wn = WN_Get_Parent(parent_wn, Parent_Map, Current_Map_Tab);
     }
     if (parent_wn && (WN_region_kind(parent_wn)== REGION_KIND_TRY)) {
       // look for the corresponding try
       WN *next = WN_next(parent_wn);
       while (next && WN_opcode(next) != OPC_REGION) {
         next = WN_next(next);
       }
       if (next && WN_region_kind(next) == REGION_KIND_CLEANUP) {
        next = WN_next(next);
        if (next && WN_operator(next) == OPR_CALL && 
		!strcmp(ST_name(WN_st(next)), "__cp_pop_exception")) {
       	    WN *exception_parent = WN_Get_Parent(parent_wn, Parent_Map, Current_Map_Tab);

            ST *st = New_ST (CURRENT_SYMTAB);
	    static int instance;
	    char name[64];
	    sprintf(name,"%d",instance++);
            ST_Init (st, Save_Str2 ( "__sp_try", name ), CLASS_VAR, SCLASS_AUTO, EXPORT_LOCAL,
			MTYPE_To_TY (Pointer_Mtype));
            Set_ST_is_temp_var ( st );

       	    WN* save = WN_Stid(Pointer_Mtype, 0, st, MTYPE_To_TY (Pointer_Mtype),
				 WN_CreateAlloca (WN_Intconst (MTYPE_U4, 0)));

       	    WN* restore = WN_CreateDealloca (1);
       	    WN_kid0 (restore) = WN_Ldid (Pointer_Mtype, 0, st, MTYPE_To_TY(Pointer_Mtype));
       	    Insert_Block_Around (WN_Get_Parent(parent_wn, Parent_Map, Current_Map_Tab), 
			    parent_wn, next, save, restore);
         }
       }
     }
...
IPO_INLINE::Clone_Callee(BOOL same_file)
{
  ...
  IPO_CLONE copy(caller_map_tab, callee_map_tab, 
		 ...
		 Callee_node()->Cloned_Symtab(),
                 &Ipo_mem_pool,
                 same_file, 0,
		 Asm_Neg_Preg_Adjustment(Caller_node(), Callee_node()));


  if (!Callee_node()->Cloned_Symtab()) {
      // nenad, 02/02/01: always call Promote_Statics;
      // Has_Pstatics may not be set if Pstatics are not used in PU
  ...
  if (Caller_node ()->Has_frequency ()) {
    ...
    if (call_freq.Known ()) {
      ...
      if (! callee_freq.Known ()) {
        DevWarn ("Inconsistent frequency count between callsite in %s and"
                 " callee %s",  Caller_node()->Name(), Callee_node()->Name());
      } 
      else {
        scale = call_freq / callee_freq;
        SUMMARY_FEEDBACK* fb = Callee_node ()->Get_feedback ();
        fb->Set_frequency_count (callee_freq - call_freq);
        fb->Set_cycle_count(fb->Get_cycle_count()-scale*fb->Get_cycle_count());
        fb->Set_ccount(fb->Get_ccount() - scale * fb->Get_ccount());
      }
...
IPO_INLINE::Process_Op_Code (TREE_ITER& iter, IPO_INLINE_AUX& aux)
{
	...
	if (WN_rtype (WN_kid0 (wn)) == MTYPE_M) {
...
#ifdef _STANDALONE_INLINER
	    if (stid_ty == TY_IDX_ZERO) { // This is potentially a COMMA node
	    ...
	    iter.Insert (WN_Stid (MTYPE_M, 0, tmp_st, stid_ty,
				  WN_kid0 (wn))); 
	} else if (MTYPE_is_integral (WN_rtype(wn))) {
...
Disambiguate_Aliased_Actuals (PARM_ATTR_VEC& parm_attr, PU& pu)
{
    ...
    memcpy (aliased, 0, sizeof(BOOL)*num_parm);
    memcpy (scalar, 0, sizeof(BOOL)*num_parm);
...
IPO_INLINE::Process_Copy_In (PARM_ITER parm, WN* copy_in_block)
{
	...
	WN* actual = IPO_Copy_Tree (parm->Actual_Wn ());
        ...
        if (Caller_node()->Feedback_Info())
          Caller_node()->Feedback_Info()->FB_clone(parm->Actual_Wn(),
                                                   actual,
                                                   FB_FREQ(1.0, true));
	...
	pair<WN*, ST*> copy_stmt = Create_Copy_Expr (actual, Call_Wn(),
...
IPO_INLINE::Process()
{
  ...
  if (Cur_PU_Feedback) {
    FB_VERIFY_STATUS fb_status = Cur_PU_Feedback->Verify("IPA/inline");
    if (fb_status == FB_VERIFY_INVALID) {
      if (PU_has_exc_scopes(Get_Current_PU()))
        DevWarn("Feedback Verify fails after inlining %s to %s",
                Callee_node()->Name (), Caller_node()->Name ());
      else
        Is_True(FALSE, ("Feedback Verify fails after inlining %s to %s",
                        Callee_node()->Name (), Caller_node()->Name ()));
    }
  }
