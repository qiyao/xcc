...
static NUM_CALLS_PROCESSED* Processed_As_Caller;
...
Init_Num_Calls_Processed (const IPA_CALL_GRAPH* cg, MEM_POOL* pool)
{
    ...
    Processed_As_Caller = CXX_NEW (NUM_CALLS_PROCESSED (cg, pool), pool);
...
// edges to routines marked as never to inline are ignored
		  ...
		  NODE_INDEX root) 
{
    ...
    for (child = vitr.First_Succ (); 
	 ...
	 child = vitr.Next_Succ ()) {
        if (!visited[child] && !((IPA_NODE *)NODE_user(&GRAPH_v_i(cg, child)))->Has_Noinline_Attrib())
			    ...
			    NODE_INDEX root)
{
    ...
    for (INT i=0; i<GRAPH_vmax(cg); i++) {
      if (NODE_fcnt(&GRAPH_v_i(cg,i)) != -1) {
        if (!visited[i]) { 
          Trans_Order_Walk (vect, visited, cg, i);
        }
      }
    }
...
#endif
      if (IP_FILE_HDR_input_map_size(*f)) {
#ifndef _WIN32
        munmap((char *)IP_FILE_HDR_input_map_addr(*f), 
                       IP_FILE_HDR_input_map_size(*f));
	close(IP_FILE_HDR_file_desc(*f));
#else
	UnmapViewOfFile(IP_FILE_HDR_input_map_addr(*f));
#endif
      }
    }
  }
