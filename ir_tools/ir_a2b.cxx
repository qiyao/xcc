...
main (INT argc, char *argv[])
{
    ...
    xt_isa_dlls = xtensa_path_array(xtensa_find_key(params, "isa-base-dlls"),
				    xtensa_find_key(params, "isa-tie-dll"), 0);
    if (!xt_isa_dlls) {
      fprintf (stderr, "Error: %s\n", xtensa_params_error_msg ());
      exit (1);
    }

    xt_xtie_dlls = xtensa_path_array(xtensa_find_key(params, "xml-base-dll"),
                                     xtensa_find_key(params, "xml-tie-dll"), 0);
    if (!xt_xtie_dlls) {
      fprintf (stderr, "Error: %s\n", xtensa_params_error_msg ());
      exit (1);
    }

    // TENSILICA: assuming windowed abi for now
    ABI_Name = "windowed";
    
    // always assume density option is available to avoid
    // re-defining mov.n scheduling info since
    xt_density = 1;
    ...
    a2b = (strcmp (progname, "ir_a2b") == 0);
#ifdef _WIN32
    b2a = (strcmp (progname, "ir_b2a.exe") == 0);
    b2a = b2a | (strcmp(progname, "ir_b2a") == 0);
#else
    b2a = (strcmp (progname, "ir_b2a") == 0);
#endif
    sel = (strcmp (progname, "ir_sel") == 0);
	...
	while (*argv[binarg] == '-') {
	   if (strncmp(argv[binarg], "-st", 3) == 0) {
		...
		IR_dump_map_info = TRUE;
	      ...
	      Sqrt_Allowed=TRUE;	/* Generate SQRT instruction? */
