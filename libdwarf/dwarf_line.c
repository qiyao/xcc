...
#include <alloca.h>
#ifdef _WIN32
#define Is_Absolute_Path(str) (str[0] == '/' || str[0] == '\\' || str[1] == ':')
#else
#define Is_Absolute_Path(str) (str[0] == '/')
#endif
...
)
{
    ...
    if (Is_Absolute_Path(file_entry->fi_file_name)) {
	...
	*/   
	...
	(Is_Absolute_Path(include_directories) ? 
	    0 : comp_dir_len +  1 ) +
    ...
    if (!Is_Absolute_Path(include_directories)) {
	if(comp_dir_len > 0) {
	  strcpy((char *)name_buffer,
           (char *)line->li_context->lc_compilation_directory);
