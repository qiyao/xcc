...
#ifdef _DIS_BUILD
#define ADDR64_DEFAULT	"0x%" LLX_FMT "\t"
#define VALUE64_DEFAULT	"0x%" LLX_FMT "\t"
...
#ifdef _64BIT_OBJECTS    
      sprintf(bufptr, "\t0x%" LLX_FMT "", (unsigned long long)*symbol_value);
#else
...
F_DIS_REGS ( char *buffer,unsigned regmask,ELF_ADDR reg_values[])
{
	...
	for (i = 0; regmask; i++, regmask >>= 1) {
		if (regmask & 1) {
#ifdef _DIS_BUILD
			  sprintf(buffer, "%s%s=0x%" LLX_FMT "", first ? "\t<" : ",",
				save.reg_names[i], 
	...
	for (; ; iadr += 4) {
		...
		if ((which == 1) && get_symname)
			if (symname = get_symname(symbol_value))
				...
				sprintf(buffer + strlen(buffer), "0x%" LLX_FMT "", 
