...
/* TO DO: Fix Me. Hack alert.
   On solaris, libelf.h includes "sys/elf.h" rather than elf.h 
   in fact, solaris's elf.h itself just includes sys/elf.h

   We have our own elf.h (this file), and want to use it rather than solaris's 
   sys/elf.h, which matches our own and makes the build choke on solaris. 
   This is a big mess.
   
   So prevent sys/elf.h's inclusion by defining the macro below. It 
   is a major hack, but it works.

   The really right thing to do would be to stop relying on all the elf
   stuff that sgi provides and go with the native platform's implementation
   of elf. However, the sgi guys determined that it would take too much  work 
   for linux, and it would be a similar amount for solaris. And even if it did, 
   xtensa relies on some linux specific stuff. Can it all be cleaned up? I don't 
   know.
*/
#if defined(sun)
#define _SYS_ELF_H
...
#define ET_AUTOTIE	(ET_LOPROC + 1)		/* file is a 32-bit or 64-bit WHIRL file*/
#endif
